...
object JPFScanPlugin extends AutoPlugin {
  ...
  def convertAllPlugins(parsedMap: Map[String, ParsedJPF],
                        pId: Iterable[String]): List[Project] = {

    def classpathDep(
        pluginId: String): Option[ClasspathDep[ProjectReference]] = {
    ...
    def convertOne(already: Set[String],
      ...
      if (already.contains(pId)) (already, processed)
        ...
        parsedMap
          .get(pId)
          .map {
            case pjpf: ParsedJPF if pjpf.isExtensionOnly =>
              (already + pId, processed)
            case pjpf @ ParsedJPF(baseDir, group, _, internalDeps, _, _) =>
              ...
              val prj = Project(toSbtPrj(pId), baseDir)
                .dependsOn(prjDeps: _*)
                .settings(
                  managedClasspath in Compile ++= (managedClasspath in (parentForPlugin(
                    pjpf), Compile)).value,
                  managedClasspath in Compile ++= {
                    jpfLibraryJars
                      .all(ScopeFilter(inProjects(depsWithExports(
                        deps,
                        Set.empty).map(toLocalProject).toSeq: _*)))
                      .value
                      .flatten
                  },
...
//    val adminPlugins = manifestMap.values.filter(_.adminConsole).map(_.id).toSet
    val pluginList = (if (buildConfig.hasPath("plugin.whitelist"))
                        buildConfig
                          .getStringList("plugin.whitelist")
                          .asScala
                          .toSet
