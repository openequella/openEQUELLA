...
object JPFRunnerPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    lazy val additionalPlugins =
      taskKey[Iterable[JPFRuntime]]("Additional JPF plugins to include")
    ...
    def runnerTasks(aggregate: ProjectReference) = {
      ...
      Seq(
        writeJars := {
          ...
          val allRuntimes = jpfRuntime
            .all(scope)
            .value ++ additionalPlugins.value
          val outBase = target.value / "jpfjars"
          IO.delete(outBase)
          allRuntimes.map {
            r =>
              val allCode = r.code.flatMap(f =>
                (f ** "*.class").pair(rebase(f, "classes/"), false))
              val allResources = r.resources.flatMap(f =>
                (f ** "*").pair(rebase(f, "resources/"), false))
              val allJars =
                r.jars.flatMap(f => flatRebase("lib/").apply(f).map((f, _)))
              val libs = allCode.headOption.map(_ =>
                ...
                allJars.map(f =>
                  JPFLibrary(f._1.getName, "code", f._2, Some("*")))
  ...
  def writeJPF(f: File, jars: Iterable[JPFLibrary]): (String, String) = {
    ...
    Option(root.getChild("requires")).foreach { r =>
      ...
      else r.setContent(newchildren.asJava)
    }
    if (jars.nonEmpty) {

      val rt = Option(root.getChild("runtime"))
        .getOrElse(insertPluginChildElement(root, "runtime"))
      rt.removeContent()
      jars.foreach { f =>
