...
case class LangString(group: String,
                      key: String,
                      pluginId: String,
                      value: String)
...
object PluginRefactor {
  ...
  val platformPlugins = Set("com.tle.platform.common",
  ...
  def cycleChecker(
      allImports: Iterable[PluginDeets]): Set[String] => CycleCheckResult = {
    ...
    def wouldCauseCycle(_toCheck: Set[String]): CycleCheckResult = {
      ...
      @tailrec
      def topLevel(mergeList: List[String],
                   checked: Set[String]): Option[List[String]] =
        mergeList match {
          case Nil => None
          case pId :: mt =>
            checkIter(List(pId),
  ...
  def choosePlugins(allImports: Seq[PluginDeets],
                    adminPlugins: Boolean): Iterable[String] = {
    ...
    def findSubset(size: Int,
      ...
      @tailrec
      def checkSubsets(iter: Iterator[Set[String]],
                       soFar: Int,
          ...
          if (!iter.hasNext) Left(Right(soFar, stats))
            ...
            wouldCauseCycle(nextSet) match {
              ...
              case CycleFound(badCycle) =>
                ...
                checkSubsets(iter,
                             soFar + 1,
                             stats.updated(bad, stats.getOrElse(bad, 0) + 1))
            }
          }
        }
      }
      ...
      checkSubsets(allSubsets, soFar, stats) match {
        case Left(Left(failed)) =>
          findSubset(Math.min(baseSet.size - 1, 10),
                     baseSet - failed,
                     0,
                     Map.empty)
    ...
    findSubset(Math.min(onlyAllowed.size - 1, 10),
               onlyAllowed.toSet,
               0,
               Map.empty) match {
      case Right(ok) => ok
  ...
  def mergePlugins(allBaseDirs: Seq[(File, Classpath)],
    ...
    cycleChecker(allPlugins)(toMerge.toSet) match {
      case CycleFound(cycle) =>
        println(s"Sorry that would cause a cycle: ${cycle}")
      case Success(_) =>
                ...
                params
                  ...
                  .map { fileElem =>
                    ...
                    Using.fileInputStream(propFile) { inp =>
                      if (IO.split(filename)._2 == "xml")
                        langProps.loadFromXML(inp)
        ...
        def reprefix(pId: String, e: Element, f: String => Boolean): Element = {
          e.getChildren("parameter")
            ...
            .foreach { p =>
              p.setAttribute(
                "value",
                pluginId + p.getAttributeValue("value").substring(pId.length))
            }
          e
        }

        def keyParameters(extPlugin: String,
                          ext: String): (Set[String], Set[String]) =
          (extPlugin, ext) match {
            case (_,
                  "portletRenderer" | "resourceViewer" | "connectorType" |
                  "portletType") =>
              (Set("nameKey", "linkKey", "descriptionKey"), Set())
            case ("com.tle.mycontent", "contentHandler") =>
            ...
            case ("com.tle.admin.fedsearch.tool", "configUI") =>
              (Set.empty, Set("class"))
            case ("com.tle.admin.usermanagement.tool", "configUI") =>
              (Set("name"), Set("class"))
            case ("com.tle.common.dynacollection", "usages") =>
              (Set("nameKey"), Set.empty)
            case ("com.tle.common.wizard.controls.resource", "relationTypes") =>
              (Set("nameKey"), Set.empty)
            case ("com.tle.admin.collection.tool", "extra") =>
              (Set("name"), Set("configPanel"))
            ...
            case ("com.tle.admin.taxonomy.tool", "dataSourceChoice") =>
              (Set("nameKey"), Set("configPanel"))
            case ("com.tle.admin.taxonomy.tool", "displayType") =>
              (Set("nameKey"), Set())
            case ("com.tle.admin.taxonomy.tool", "predefinedTermDataKey") =>
              (Set("name", "description"), Set())
            case ("com.tle.core.migration", "migration") =>
              (Set(), Set("id", "obsoletedby"))
            case ("com.tle.core.institution.convert", "xmlmigration") =>
              (Set(), Set("id", "obsoletedby"))
            case ("com.tle.core.scheduler", "scheduledTask") =>
              (Set(), Set("id"))
            case ("com.tle.web.settings", "settingsGroupingExtension") =>
              (Set("nameKey"), Set())
        ...
        val afterExt = exts.flatMap {
          case (bd, pId, e) =>
            (getPluginId(e), e.getAttributeValue("point-id")) match {
              ...
              case (extPlugin, ext)
                  if keyParameters(extPlugin, ext)._1.nonEmpty =>
        ...
        def containsId(el: List[Element])(e: Element) =
          el.exists(e2 =>
            e2.getAttributeValue("id") == e.getAttributeValue("id"))

        plugElem.addContent(Uniqueify
          .uniqueSeq[Element](
            (i, e) =>
        ...
        pathsTo
          .groupBy(_._1)
          .filter(t => t._2.map(_._3).distinct.size > 1)
          .foreach {
            case (p, pids) =>
          ...
          .flatMap {
            case (bd, pId, e) =>
              e.getChildren("parameter").asScala.collect {
                case p
                    if Option(p.getAttributeValue("value"))
                      .exists(_.startsWith(pId))
                      && Option(p.getAttributeValue("id")).exists { paramId =>
                        val (keys, nonKeys) =
                          keyParameters(getPluginId(e),
                                        e.getAttributeValue("point-id"))
        ...
        val manifestName =
          if (canCommit) "plugin-jpf.xml" else "plugin-jpf2.xml"

        def writeManifest(fname: File, outDoc: Document): Unit = {
          val pluginJpf =
            new XMLOutputter(Format.getPrettyFormat).outputString(outDoc)
        ...
        allPlugins.foreach {
          ...
          case p => {
            Option(p.rootElem.getChild("requires")).foreach { r =>
              val usedMerged =
                r.getChildren().asScala.exists(e => allowedIds(getPluginId(e)))
