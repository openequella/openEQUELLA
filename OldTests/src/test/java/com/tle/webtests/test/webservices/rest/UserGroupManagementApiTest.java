...
public class UserGroupManagementApiTest extends AbstractRestApiTest
{
	...
	public void userSearchAndEditTest() throws Exception
	{
		...
		JsonNode userResult = getEntity(context.getBaseUrl() + API_PATH_USER + AUTOTEST_USER_UUID, token);
		...
		HttpResponse putResponse = getPut(context.getBaseUrl() + API_PATH_USER + TOKENUSER_UUID, userNodeReally, token);
		...
		JsonNode resultsNode = getEntity(context.getBaseUrl() + API_PATH_USER, token, "q", TOKENUSER_LOGON);
		assertEquals(resultsNode.get("available").asInt(), 1, "Expected exactly one '" + TOKENUSER_LOGON + "'");
		...
		putResponse = getPut(context.getBaseUrl() + API_PATH_USER + TOKENUSER_UUID, userNodeReally, token);
	...
	public void groupSearchTest() throws Exception
	{
		...
		assertEquals(selectedGroupsResponse.get("available").asInt(), 3, "Groups matching 'Select' available");
		...
		JsonNode notinhereResponse = doQuerySearch(API_PATH_GROUP, SUPERGROUP,
			ImmutableMap.of("user", TOKENUSER_UUID, "allParents", "false"), token);
		...
		JsonNode notinhereEitherResponse = doQuerySearch(API_PATH_GROUP, FLATGROUP,
			ImmutableMap.of("user", TOKENUSER_UUID), token);
		...
		JsonNode inhereindirectly = doQuerySearch(API_PATH_GROUP, SUPERGROUP, ImmutableMap.of("user", TOKENUSER_UUID),
			token);
		assertEquals(inhereindirectly.get("available").asInt(), 1, "tokenuser (in directly) in " + SUPERGROUP);

		// ... but the supergroup is included as groups of which token is a
		// member. NB: upwards recursion defaults to true
		JsonNode butisinhere = doQuerySearch(API_PATH_GROUP, COMMON_SUBSTRING, ImmutableMap.of("user", TOKENUSER_UUID),
			token);
		assertEquals(butisinhere.get("available").asInt(), 2, "groups including tokenuser");
		assertTrue(groupArrayContainsElement(butisinhere.get("results"), "name", SUPERGROUP),
			"Group includes supergroup");
		assertTrue(groupArrayContainsElement(butisinhere.get("results"), "name", SUBGROUP), "Group includes subgroup");

		// without upwards recursion, tokenuser is only a member of one group
		JsonNode onlyOneGroup = doQuerySearch(API_PATH_GROUP, null,
			ImmutableMap.of("user", TOKENUSER_UUID, "allParents", "false"), token);
	...
	private void doPagingTest(String apiPath, int totalThatExists, int skipOverFromStart, String token)
		throws Exception
	{
		...
		Map<?, ?> otherParams = ImmutableMap.of("start", Integer.toString(skipOverFromStart), "length",
		...
		assertEquals(userResponse.get("length").asInt(), totalThatExists - skipOverFromStart, "Expected to skip over "
			+ skipOverFromStart);
	...
	public void addAndDeleteGroupTest() throws Exception
	{
		...
		assertEquals(response.getStatusLine().getStatusCode(), 201, "Unexpected HTTP response from POST");
		...
		JsonNode eponymousGroups = doQuerySearch(API_PATH_GROUP, this.getClass().getSimpleName(), null, token);
		...
		while( deleted < howManyGroupsToDelete )
		{
			...
			// sanity check
			Assert.assertTrue(group.get("name").asText().contains(this.getClass().getSimpleName()),
			...
			assertTrue(deletionCode == 204, "Failed to delete group with name: " + group.get("name").asText() + " - "
				+ deletionCode + " (" + deletionResponse.getStatusLine().getReasonPhrase() + ")");
			++deleted;
		}
	 ...
	 * @throws Exception
	 */
	private JsonNode doQuerySearch(String incompleteApiPath, String query, Map<?, ?> otherParams, String token)
		throws Exception
	{
