...
public class PackageAndNavigationTest extends AbstractCleanupAutoTest
{
	...
	/**
	 * DTEC-14847 - Ensure that if a new node is created, "Multiple resources"
	 * checked but no resources selected for the tabs, then everything still
	 * keeps going without error.
	 */
	@Test
	...
	/**
	 * Redmine #5767 - Putting a double-quote in the name of a node cuts off
	 * everything after it. Try doing lots of quote-y things and ensure they
	 * work as expected.
	 */
	@Test
	...
	public void navigation()
	{
		...
		WizardPageTab wizard = initialItem(itemName);
		wizard.addFiles(2, false, Attachments.get("page.html"), Attachments.get("pageB.html"),
		...
		assertEquals(viewer.tabText("Tab holder", "Tab number 1", 0), "This is a verifiable attachment 3");
		assertEquals(viewer.tabText("Tab holder", "Tab number 2", 1), "This is a verifiable attachment 2");
		// DTEC-14853
		assertEquals(viewer.getTabOrder(), Lists.newArrayList("Tab number 1", "Tab number 2"));

		item = viewer.clickTitle();
		assertTrue(item.attachments().viewAttachment("A new node", new VerifyableAttachment(context)).isVerified());
	}
	...
	public void packageNavigation()
	{
		...
		WizardPageTab wizard = initialItem(itemName);
		UniversalControl control = wizard.universalControl(2);
		control.addResource(new FileUniversalControlType(control)).uploadPackageOption(Attachments.get(IMS_ZIP))
			.showStructure().save();
		SummaryPage item = wizard.save().publish();
