...
public class FileAttachmentControlTest extends AbstractCleanupAutoTest
{
	...
	public void webPages()
	{
		...
		WebPagesUniversalControlType pagesType = control.addResource(new WebPagesUniversalControlType(control));
		...
		assertTrue(item.attachments().viewAttachment("A Page", new VerifyableAttachment(context)).isVerified());
	...
	public void multipleControls()
	{
		...
		universalControl.addResource(new WebPagesUniversalControlType(universalControl)).addPage("Test page",
			"A heading");
	...
	public void specialCharacters()
	{
		...
		UniversalControl control = wizard.universalControl(2);
		FileAttachmentEditPage file = control.editResource(new FileUniversalControlType(control), FILE1);
		...
		assertTrue(item.attachments().viewAttachment(FILE4, new VerifyableAttachment(context)).isVerified());
	...
	public void file()
	{
		...
		UniversalControl control = wizard.universalControl(2);
		FileAttachmentEditPage file = control.editResource(new FileUniversalControlType(control), "page.html");
		file.setDisplayName("page2.html").save();

		SummaryPage item = wizard.save().publish();
		assertTrue(item.attachments().attachmentExists("page2.html"));
		assertTrue(item.attachments().viewAttachment("page2.html", new VerifyableAttachment(context)).isVerified());
	}
	...
	public void sameFile()
	{
		...
		assertTrue(attachments.viewAttachment("page(4).html", new VerifyableAttachment(context)).isVerified());
		...
		assertTrue(attachments.viewAttachment("page(6).html", new VerifyableAttachment(context)).isVerified());
	}
