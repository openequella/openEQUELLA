...
public class HierarchyApiTest extends AbstractRestApiTest
{
	...
	private static final String DETECTABLE_DELETABLE_PREFIX = OAUTH_CLIENT_ID + "_delete me if you see me";
	...
	public void createHierarchiesTest() throws Exception
	{
		...
		String jsonString = "{ " + "\"name\" : \"" + DETECTABLE_DELETABLE_PREFIX + "_a\","
			+ "\"shortDescription\" : \" deletable short description\","
			+ "\"longDescription\" : \"Meat Loaf: my greatest regret (Demetriou)\", " + "\"showResults\" : false,"
			+ "\"inheritFreetext\" : false, " + "\"freetext\" : \"Meat Loaf\","
		...
		jsonString = "{ " + "\"name\" : \"" + DETECTABLE_DELETABLE_PREFIX + "_b\","
			+ "\"shortDescription\" : \"b is the child of a (at first)\","
			+ "\"longDescription\" : \"Meat Loaf: my greatest regret (Jim Sharman)\", " + "\"showResults\" : false,"
			+ "\"inheritFreetext\" : false, " + "\"freetext\" : \"Rocky Horror\","
			+ "\"hideSubtopicsWithNoResults\" : true," + "\"parent\" : { \"uuid\" :  \"" + uuidOfParent + "\" },"
	...
	/**
	 * We have primed our persistent root-level hierarchy with the query "hierarchyApiTest",
	 * and there are 12 persistent items in the 'rest' institution matching that search.
	 * Similarly, there are 3 items matching a search on "Meat Loaf", and 2 on "Rocky Horror",
	 * noting that temporary hierarchies have been created with those searches in the previous
	 * 'dependsOn...' test.
	 * @throws Exception
	 */
	@Test(dependsOnMethods = {"createHierarchiesTest"})
	public void browseHeirarchiesTest() throws Exception
	{
		...
		URI browseUri = new URI(context.getBaseUrl() + API_BROWSE_TASK_PATH + '/' + hierarchyNode.get("uuid").asText());
		...
		JsonNode hierarchyLink = hierarchyNode.get("links") != null ? hierarchyNode.get("links").get("self") : null;
		...
		assertTrue(hierarchyLinkToChildren != null && hierarchyLinkToChildren.size() >= 1, "no childTopics");
		...
		searchBeanNode = result.get("searchResults") != null ? result.get("searchResults").get("results") : null;
		...
		assertTrue(numVirtualisedItems == 2, "only found " + numVirtualisedItems + " virtualised items");
	}
