...
public class AbstractCALTest extends AbstractCleanupTest
{
	...
	protected CALWizardPage setupBook(String bookName, String numPages, String collection, String isbn)
	{
	...
	protected SummaryPage createBook(String bookName, String numPages, String collection, String isbn)
	{
		return setupBook(bookName, numPages, collection, isbn).publish();
	}

	protected SummaryPage createPortion(String chapter, String title, String book, int firstPage, int lastPage,
		int numsections)
	{
		return createPortion(chapter, title, book, firstPage, lastPage, numsections, Attachments.get(ATTACH1_FILENAME));
	}

	protected SummaryPage createPortion(ContributePage contributePage, String chapter, String title, String book,
		int firstPage, int lastPage, int numsections)
	{
		return publishPortion(contributePage, chapter, title, book, firstPage, lastPage, numsections,
			Attachments.get(ATTACH1_FILENAME));
	}

	protected SummaryPage createPortion(String chapter, String title, String book, int firstPage, int lastPage,
		int numsections, URL filename)
	{
		return publishPortion(new ContributePage(context).load(), chapter, title, book, firstPage, lastPage,
			numsections, filename);
	}

	private SummaryPage publishPortion(ContributePage contributePage, String chapter, String title, String book,
		int firstPage, int lastPage, int numsections, URL filename)
	{
	...
	private void addSections(CALWizardPage calWizardPage, int firstPage, int lastPage, int numsections, URL filename)
	{
	...
	protected SummaryPage createJournalPortion(String title, String topic, String journal, int firstPage, int lastPage,
		int numsections)
	{
		return createJournalPortion(title, topic, journal, firstPage, lastPage, numsections,
			Attachments.get(ATTACH1_FILENAME));
	}
