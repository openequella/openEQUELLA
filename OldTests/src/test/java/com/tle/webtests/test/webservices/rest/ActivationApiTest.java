...
public class ActivationApiTest extends AbstractRestApiTest
{
	...
	public void testRetrieveActivations() throws Exception
	{
		...
		assertNull(result.get("error"), "Resonse unexpected: "
		...
		JsonNode resultNode = containsInSearchBeanResult(result, "apretendedpagefromapretendbook.txt", "description");
		...
		assertTrue(resultNum == resultAny, "Expected 'any' to return (" + resultNum
			+ ", same as <no param>) but we got " + resultAny);
		...
		assertTrue(resultNum == resultViaCourse, "Expected 'course' to return (" + resultNum
			+ ", same as <no param>) but we got " + resultViaCourse);
		...
		assertTrue(resultViaUninvolvedCourse == 0, "Expected zero (enquiring on unpopulated course) but we got "
			+ resultViaUninvolvedCourse);
	}
	...
	public void testCreateAndDeleteActivation() throws Exception
	{
		...
		String jsonStr = "{ " + "\"attachment\": \"" + KNOWN_ATTACHMENT_UUID + "\", " + "\"course\": { "
			+ "\"uuid\": \"" + KNOWN_COURSE_UUID + "\" " + " }, "
			+ "\"description\": \"Senatus populusque romanus.txt\", ";

		jsonStr += "\"item\": { " + "\"uuid\": \"" + KNOWN_ITEM_UUID + "\", " + "\"version\": 1 " + "  }, "
			+ "\"type\": \"cal\", " + "\"uuid\": \"" + IMPOSED_UUID + "\" " + " } ";
		...
		assertEquals(echoedUuid, IMPOSED_UUID, "The UUID we we got (" + echoedUuid + ") was not as expected.");
