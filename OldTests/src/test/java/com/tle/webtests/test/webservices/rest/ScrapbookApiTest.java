...
public class ScrapbookApiTest extends AbstractItemApiTest
{
	...
	private String createScrapbookItem(String type, String filename) throws ClientProtocolException, IOException
	{
		...
		HttpResponse response = postEntity(node.toString(), context.getBaseUrl() + "api/scrapbook", getToken(), true);
		assertResponse(response, 201, "failed to create scrapbook item");
		return response.getFirstHeader("Location").getValue();
	}

	private ObjectNode getCreatedScrapbookItem(String type, String filename) throws ClientProtocolException,
		IOException
	{
