...
/**
 * Test Reference: http://time/DTEC/test/editTest.aspx?testId=14778
 * Test Reference: http://time/DTEC/test/editTest.aspx?testId=14779
 * Preconditions: that there exists a collection titled 'Generic testing collection'
 * which defines items with a date filed (in this case in metadata path
 * /xml/calendar, where at least one item exists with a data value as 1962-10-30
 * and another with 2011-09-26. It shall be assumed no items exist with dates outside
 * the range of these two dates. The queries in this test will seek for values
 * less than, greater than or equal to these dates.
 * @author larry
 *
 */
...
{
	private final static String DATE_METADATA_PATH = "/xml/calendar";

	//	private final static String OLDEST_DATE_STR = "1962-10-30"; // Date somebody arrived on earth.
	//	private final static String NEWEST_DATE_STR = "2011-09-26"; // Date somebody else left the building.
	...
	public void aboveAndBelowDatesTest()
	{
		...
		assertTrue(howMany == 0, "Expected no items to store date newer than '" + newerThanNewest + "' but found "
			+ howMany);
		...
		assertTrue(howMany >= 2, "Expected at least 2 items to store date older than '" + newerThanNewest
			+ "' but found " + howMany);
		...
		assertTrue(howMany == 0, "Expected no items to store date older than '" + olderThanOldest + "' but found "
			+ howMany);
		...
		whereQueryPage.clearCriteria();
		howMany = howManyBeyondLimit(whereQueryPage, ">", olderThanOldest);
		assertTrue(howMany >= 2, "Expected at least 2 items to store date newer than '" + olderThanOldest
			+ "' but found " + howMany);
	}

	private int howManyBeyondLimit(WhereQueryPage<ItemAdminPage> whereQueryPage, String operand, String dateStr)
	{
