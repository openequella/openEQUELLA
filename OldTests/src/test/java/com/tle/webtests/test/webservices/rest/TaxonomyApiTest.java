...
public class TaxonomyApiTest extends AbstractRestApiTest
{
	...
	public void testGetTerm()
	{
		...
		try
		{
			...
			URI uri = new URI(context.getBaseUrl() + API_TAXONOMY_PATH + "/" + TAXONOMY_UUID + "/term/" + TERM_1_UUID);
	...
	public void testInsertAndDeleteTerm() throws IOException, URISyntaxException
	{
		...
		HttpResponse response = postEntity(node.toString(), context.getBaseUrl() + API_TAXONOMY_PATH + "/"
			+ TAXONOMY_UUID + "/term", getToken(), true);
	...
	public void testMoveTerm() throws IOException, URISyntaxException
	{
		...
		String putUrl = context.getBaseUrl() + API_TAXONOMY_PATH + "/" + TAXONOMY_UUID + "/term/"
	...
	{
		JsonNode entity = getEntity(context.getBaseUrl() + API_TAXONOMY_PATH + "/" + TAXONOMY_UUID + "/search",
			getToken(), "q", "*REST*", "restriction", "UNRESTRICTED", "limit", 20);
