...
public class KalturaContributionTest extends AbstractCleanupAutoTest
{
	...
	public void addExisting()
	{
		...
		KalturaUniversalControlType kaltura = control.addResource(new KalturaUniversalControlType(control));
		...
		wizard = item.adminTab().edit();
		control = wizard.universalControl(2);
		GenericAttachmentEditPage editPage = control.editResource(new KalturaUniversalControlType(control), vidName);
	...
	public void disableExistingKalturaServer()
	{
		...
		assertEquals(kse.getError(), "You must successfully test the Kaltura server details before saving.");
		kse.setEndpoint("");
		kse.test();
