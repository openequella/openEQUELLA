...
public class MoodleConnectorTest extends AbstractParallelMoodleTest
{
	...
	{
		String[] locations = new String[]{"1 September - 7 September", "8 September - 14 September",
				"15 September - 21 September", "22 September - 28 September", "29 September - 5 October",
				"6 October - 12 October", "13 October - 19 October", "20 October - 26 October",
				"27 October - 2 November", "3 November - 9 November"};
	...
	public void archivedTest()
	{
		...
		logon("AutoTest", "automated");
		FindUsesPage uses = SearchPage.searchExact(context, fullName).getResult(1).viewSummary().findUsesPage();
	...
	public void versionTest()
	{
		...
		findUses = new ItemAdminPage(context).load().exactQuery(fullName).getResult(1).viewSummary().findUsesPage();
		connectorHelper.selectConnector(findUses);
	...
	{
		logon("AutoTest", "automated");
		LMSExportPage lms = createTestItem(context.getFullName("New version version1")).adminTab().lmsPage();
		connectorHelper.selectConnector(lms);
		lms.selectSummary();
		lms.clickCourse(COURSE_NAME).clickSection(SECTION_NAME);
		lms = lms.publish();
	...
	public void setupMoodle()
	{
		...
		logon("TLE_ADMINISTRATOR", "tle010");
		ShowConnectorsPage page = new ShowConnectorsPage(context).load();
		ShowMoodleConnectorsPage
			.addMoodleConnection(page, getConnectorName(), context.getIntegUrl(), webToken, "admin");
		assertTrue(page.entityExists(getConnectorName()));
		EditMoodleConnectorPage editPage = page.editConnector(new EditMoodleConnectorPage(page), getConnectorName());
		assertTrue(editPage.testConnection());
		page = editPage.save();
		ShowMoodleConnectorsPage.addMoodleConnection(page, getConnectorName2(), context.getIntegUrl(), webToken,
			"admin", false);
	}
	...
	protected void cleanupAfterClass() throws Exception
	{
		...
		if( !Check.isEmpty(testConfig.getIntegrationUrl("moodle")) )
		{
			...
			coursePage = new MoodleLoginPage(context).load().logon("admin", "admin").clickCourse(ARCHIVED_COURSE_NAME);
			coursePage.setEditing(true);
			coursePage.deleteAllForWeek(WEEK);
