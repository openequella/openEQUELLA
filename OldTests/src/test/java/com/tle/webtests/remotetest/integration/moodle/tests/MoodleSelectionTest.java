...
public class MoodleSelectionTest extends AbstractMoodleSectionTest
{
	...
	public void selectItemTest()
	{
		...
		Assert.assertEquals(params.get("$PATH$")[0], "integ/gen/b9c151cb-9c70-4c0c-b020-da8d8333cf59/1/");
	...
	{
		MoodleCoursePage coursePage = new MoodleLoginPage(context).load().logon("manager", "``````")
			.clickCourse(COURSE_NAME);
		coursePage.setEditing(true);
		SelectionSession session = coursePage.selectEquellaResource(WEEK).equellaSession();
	...
	public void selectHtmlTitle()
	{
		...
		assertTrue(coursePage.hasResource(WEEK, fullName), "The title probably hasnt been html encoded");
		...
		assertTrue(resourcePage.switchToItem(new SummaryPage(context)).attachments().attachmentExists(alert));
	}
	...
	public Object[][] versionSelection()
	{
		...
		return new Object[][]{{DEFAULT_CURRENT, "ItemVersion1", false, 1, "Fixed to v1"},
				{DEFAULT_CURRENT, "ItemVersion2", false, 2, "Fixed to v2"},
				{FORCE_LATEST, "ItemVersion1", true, 2, ""}, {FORCE_LATEST, "ItemVersion2", true, 2, ""},
				{FORCE_CURRENT, "ItemVersion1", false, 1, ""}, {FORCE_CURRENT, "ItemVersion2", false, 2, ""}};
	}

	@Test(dataProvider = "versionSelection")
	public void versionSelectionTest(String course, String item, boolean latest, int expectedVersion, String selected)
	{
