...
public class RemoteAttachmentControlTest extends AbstractCleanupTest
{
	...
	public void googleBooks()
	{
		...
		GoogleBooksUniversalControlType books = control.addResource(new GoogleBooksUniversalControlType(control));
		...
		control.editResource(new GoogleBooksUniversalControlType(control), "This is a Test").setDisplayName("A Book")
			.save();
	...
	public void testGoogleBooksNoSelections()
	{
		...
		UniversalControl control = wizard.universalControl(2);
		GoogleBooksUniversalControlType googleBooks = control.addResource(new GoogleBooksUniversalControlType(control));
	...
	public void itunes()
	{
		...
		UniversalControl control = wizard.universalControl(2);
		iTunesUniversalControlType itunes = control.addResource(new iTunesUniversalControlType(control));
		GenericAttachmentEditPage editPage = itunes.addTrack("Texas A&M University", "CAMPUS COMMUNITY",
			"Camtasia Relay Test", "Group 1");
		...
		wizard = item.adminTab().edit();
		control = wizard.universalControl(2);
		control.editResource(new iTunesUniversalControlType(control), "Something").setDisplayName("A tune").save();
