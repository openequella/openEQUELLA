...
public class LTIAttachmentControlTest extends AbstractCleanupAutoTest
{
	...
	private static final String itemTitle = LTIAttachmentControlTest.class.getSimpleName() + " - Plain LTI";
	...
	public void testAddSimpleExternalTool()
	{
		...
		assertTrue(finis.getItemTitle().endsWith(itemTitle), "Expected title to end with \"" + itemTitle + "\"");
	}

	/**
	 * Presupposes success of earlier test to that item named <i>itemTile</i>
	 * can be found via search and reloaded
	 */
	...
	public void testEditOfExisting()
	{
		...
		LTIAttachmentEditPage editPage = underControl.editResource(new LTIAttachmentEditPage(underControl), rsrcTitle);

		String attachmentNewName = "new attachment name";
		editPage.enterValues(attachmentNewName, -1,
			"http://and-did-those-feet.in-ancient.ws/walk/upon/England%2Cs?mountains=green");
		editPage.enterAdvancedValues(null, null, "mykey=myvalue", Boolean.FALSE, Boolean.TRUE, Boolean.FALSE);
	...
	public void testAddAndEdit()
	{
		...
		ltiEdit.enterAdvancedValues("doesn't matter that its just a simple URL", "doesn't matter if it's not valid",
			null, Boolean.TRUE, null, null);
		ltiEdit.save();
		SummaryPage finis = wizard.save().publish();
		// double check the obvious
		assertNotNull(finis.getItemTitle(), "Item has unexpected null title");
		assertEquals(finis.getItemTitle(), addEditTitle, "Expected title to end with \"" + itemTitle + "\"");
	}
