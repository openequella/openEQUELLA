...
public class RunReportTest extends AbstractSessionTest
{
	...
	public void runMultiValueReport()
	{
		...
		MultiValueReportOptionsPage report = reports.getReport("MultiValue", new MultiValueReportOptionsPage(context));
	...
	public void runItemCountReport()
	{
		...
		ReportingPage reports = new ReportingPage(context).load();
		NoParamsReportWindow<ItemCountReportPage> report = reports.getReport("ItemCount", new ItemCountReportPage(
			context));
	...
	public void runUsersAndItemsReport()
	{
		logon("AutoTest", "automated");
		ReportingPage reports = new ReportingPage(context).load();
		NoParamsReportWindow<UsersReportPage> report = reports.getReport("Users", new UsersReportPage(context));
	...
	public void runFreetextReports()
	{
		logon("Autotest", "automated");
		ReportingPage reports = new ReportingPage(context).load();
		NoParamsReportWindow<FreeTextReportPage> report = reports.getReport("Freetext report",
			new FreeTextReportPage(context));
	...
	public void runCascadingParametersTest()
	{
		logon("Autotest", "automated");
		ReportingPage reports = new ReportingPage(context).load();
		CascadingParametersReportParametersPage report = reports.getReport("Cascading parameters report",
			new CascadingParametersReportParametersPage(context));
	...
	public void runDisplayTextParametersTest()
	{
		...
		logon("Autotest", "automated");
		ReportingPage reports = new ReportingPage(context).load();
		DisplayTextReportParametersPage report = reports.getReport("DisplayText parameters report",
			new DisplayTextReportParametersPage(context));
		...
		{
			assertEquals(results.getReportValue(1, false), "[" + COLLECTION_2_id + ", " + COLLECTION_3_id + "]");
		}
		else
		{
			assertEquals(reportDisplayCollections, "[" + COLLECTION_3 + ", " + COLLECTION_2 + "]");
			assertEquals(results.getReportValue(1, false), "[" + COLLECTION_3_id + ", " + COLLECTION_2_id + "]");
		}
