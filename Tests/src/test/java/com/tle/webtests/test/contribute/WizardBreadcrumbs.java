...
public class WizardBreadcrumbs extends AbstractCleanupAutoTest
{
	...
	public void testCrumbs()
	{
		...
		assertEquals(wizard.getBreadcrumbs(), MessageFormat.format("Basic Items > {0} > Editing", FULL_NAME));
		...
		assertEquals(clone.getBreadcrumbs(),
		...
		assertEquals(move.getBreadcrumbs(),
		...
		assertEquals(
			...
			MessageFormat.format("No mandatory > {0}… > Clone item into a collection",
