...
public class SearchAutoCompleteTest extends AbstractCleanupAutoTest
{
	...
	private static final String LONG_ITEM = "long words - pneumonoultramicroscopicsilicovolcanoconiosis supercalifragilisticexpialidocious pseudopseudohypoparathyroidism";
	...
	public void testAutoCompleteKeyActions()
	{
		...
		assertListEquals(ac.getAutoCompleteOptions(), ImmutableList.of(SHORT_ITEM,
			"link to drm allowing composition - package", "link to summary of drm allowing composition - package",
	...
	public void testFilteringOnAutoComplete() throws ParseException
	{
		...
		SearchPage sp = new SearchPage(context).load();
		QuerySection qs = new QuerySection(context).get();
		AutoCompleteOptions autoCompleteOptions = qs.autoCompleteOptions(FILTER_QUERY, FILTER_DATE_RESULT);
		assertListEquals(autoCompleteOptions.getAutoCompleteOptions(), ImmutableList.of(FILTER_DATE_RESULT));
		...
		assertListEquals(autoCompleteOptions.getAutoCompleteOptions(),
