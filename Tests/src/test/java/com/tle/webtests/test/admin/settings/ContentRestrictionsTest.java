...
public class ContentRestrictionsTest extends AbstractSessionTest
{
	...
	public void testAddBannedExt()
	{
		...
		assertFalse(crp.isExtPresent(DEMO_EXT), "Test assumes extname '" + DEMO_EXT + "' not already in table");
		...
		assertTrue(crp.isExtPresent(DEMO_EXT), "Test assumes extname '" + DEMO_EXT
			+ "' already in table, but not found.");
		...
		assertTrue((numRowsPrior - 1) == numRowsPost, "Expected " + (numRowsPrior - 1)
			+ " rows after deletion, but found " + numRowsPost);
	...
	public void testAddUserQuotas()
	{
		...
		RecipientSelectorPage selector = userQuotaPage.openRecipientSelector(new RecipientSelectorPage(context));
		...
		assertEquals(userQuotaPage.getSelectedExpression(), "Auto Test [AutoTest] OR Logged In User Role");
		...
		userQuotaPage = selector.okButtonClick(userQuotaPage);
		assertEquals(userQuotaPage.getSelectedExpression(), "Auto Test [AutoTest] OR NOT Guest User Role");

		crp = userQuotaPage.saveUserQuota(new ContentRestrictionsPage(context));
		String userQuota = crp.getUserQuotasInfoByExpression("Auto Test [AutoTest] OR NOT Guest User Role");
		assertEquals(userQuota, "450 MB");

		userQuotaPage = crp.editUserQuotasByExpression("Auto Test [AutoTest] OR NOT Guest User Role", userQuotaPage);
		selector = userQuotaPage.openRecipientSelector(new RecipientSelectorPage(context));
		...
		userQuotaPage = selector.okButtonClick(userQuotaPage);
		assertEquals(userQuotaPage.getSelectedExpression(),
			"Auto Test [AutoTest] OR NOT (Guest User Role OR Logged In User Role)");

		crp = userQuotaPage.saveUserQuota(crp);
		crp.deleteUserQuotasByExpression("Auto Test [AutoTest] OR NOT (Guest User Role OR Logged In User Role)");
		logout();
