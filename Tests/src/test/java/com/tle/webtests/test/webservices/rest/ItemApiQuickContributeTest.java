...
public class ItemApiQuickContributeTest extends AbstractItemApiTest
{
	...
	public void setUp() throws Exception
	{
		...
		QuickContributeAndVersionPage page = new QuickContributeAndVersionPage(context).load().selectCollection(
			QUICKCONTRIB_COLLECTION);
		page.save();
		logout();
	}
	...
	public void quickUploadTest() throws Exception
	{
		...
		assertEquals(itemNode.get("collection").get("uuid").getTextValue(), QUICKCONTRIB_COLLECTION_UUID);
	...
	public void errorsTest() throws Exception
	{
		...
		// match
		assertTrue(statusCode == 405 || statusCode == 500, "HTTP Status code, expected 405 or 500, but got "
			+ statusCode);
	...
	private HttpResponse putFile(String filename, URL resource, String token, boolean consume) throws IOException
	{
		String avatarUrl = context.getBaseUrl() + "api/item/quick/" + com.tle.common.URLUtils.urlEncode(filename);
