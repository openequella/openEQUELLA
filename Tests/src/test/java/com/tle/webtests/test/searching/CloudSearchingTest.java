...
public class CloudSearchingTest extends AbstractCleanupTest
{
	...
	public void testLocalQueryTransferredToCloudQuery()
	{
		...
		CloudSearchPage cloudSearch = searchPage.getSearchTabs().get().clickTab("cloud", new CloudSearchPage(context));
		assertEquals(cloudSearch.getQuery(), SEARCH_QUERY.toString(), "Query wasn't maintained in cloud tab switch");
	}
}
