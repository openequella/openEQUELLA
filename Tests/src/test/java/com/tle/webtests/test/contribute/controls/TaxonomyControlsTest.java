...
public class TaxonomyControlsTest extends AbstractCleanupTest
{
	...
	/**
	 * DTEC-14591. Ensure that choosing to "add" a term when one hasn't been
	 * selected does <b>not</b> add a blank selected term.
	 */
	...
	/**
	 * DTEC-14453. Double check that "Reload page" setting for the auto-complete
	 * edit box works as expected.
	 */
	@Test(dependsOnMethods = "setupTaxonomy")
	public void autoCompleteReloadPage() throws Exception
	{
		...
		wizardPage.autoTermControl(6).selectExistingTerm(term2, wizardPage);
	...
	/**
	 * DTEC-15040. Ensure that term selectors are doing the right thing in
	 * repeaters.
	 */
	@Test(dependsOnMethods = "setupTaxonomy")
	public void repeaterTest() throws Exception
	{
		...
		assertEquals(first.autoTermControl(1).getSelections().getSelections(), Lists.newArrayList(term1));
		assertEquals(first.popupTermControl(2).getSelections().getSelections(), Lists.newArrayList(term1));
		assertEquals(second.autoTermControl(1).getSelections().getSelections(), Lists.newArrayList(term2));
		assertEquals(second.popupTermControl(2).getSelections().getSelections(), Lists.newArrayList(term2));
		...
		assertEquals(first.autoTermControl(1).getSelections().getSelections(), Lists.newArrayList(term1));
		assertEquals(first.popupTermControl(2).getSelections().getSelections(), Lists.newArrayList(term1));
		assertEquals(third.autoTermControl(1).getSelections().getSelections(), Lists.newArrayList(term3));
		assertEquals(third.popupTermControl(2).getSelections().getSelections(), Lists.newArrayList(term3));
	 ...
	 * </li>
	 * <li>Index is any other valid number</li>
	 * </ul>
	 * </li>
	 * <li>Else target is a valid parent term:
	 * <ul>
	 * <li>Term has no existing children</li>
	 * <li>Index is zero</li>
	 * <li>Index is negative or greater than the number of root terms minus one.
	 * </li>
	 * <li>Index is any other valid number</li>
	 ...
	 * </ul>
	 * </li>
	 * <li>Moving a term to be an earlier sibling of any parent term</li>
	 * <li>Moving a term to be a later sibling of any parent term</li>
	 * </ul>
	 */
	@Test(dependsOnMethods = "setupTaxonomy")
	public void movingTerms() throws Exception
	{
		...
		termSoapService.move(TAX_UUID, "Move 0\\Move 02", "", termSoapService.listTerms(TAX_UUID, "").length);
		validate("Move 02", "", -1);
		...
		termSoapService.move(TAX_UUID, "Move 2\\Move 20", "Move 0",
			termSoapService.listTerms(TAX_UUID, "Move 0").length);
		validate("Move 20", "Move 0", -1);
		termSoapService.move(TAX_UUID, "Move 2\\Move 21", "Move 5",
			termSoapService.listTerms(TAX_UUID, "Move 0").length);
		...
		List<String> expectedRoot = Lists.newArrayList("Move 0", "Move 05", "Move 00", "Move 04", "Move 1", "Move 2",
			"Move 3", "Move 4", "Move 5", "Move 03", "Move 40", "Move 01", "Move 02");
		validate(termSoapService.listTerms(TAX_UUID, ""), expectedRoot);

		List<String> expectedNode0 = Lists.newArrayList("Move 12", "Move 10", "Move 24", "Move 14", "Move 22",
			"Move 20");
		validate(termSoapService.listTerms(TAX_UUID, "Move 0"), expectedNode0);

		List<String> expectedNode3 = Lists.newArrayList("Move 30", "Move 32", "Move 31", "Move 33", "Move 34",
			"Move 35");
		validate(termSoapService.listTerms(TAX_UUID, "Move 3"), expectedNode3);

		List<String> expectedNode4 = Lists.newArrayList("Move 41", "Move 440", "Move 42", "Move 43", "Move 44",
			"Move 45");
		validate(termSoapService.listTerms(TAX_UUID, "Move 4"), expectedNode4);

		List<String> expectedNode5 = Lists.newArrayList("Move 13", "Move 11", "Move 25", "Move 15", "Move 23",
			"Move 21");
	...
	public void moveInsertTest() throws Exception
	{
		...
		assertEquals(termSoapService.listTerms(TAX_UUID, "1").length, 0, "This node should have no children");
	...
	public void lockAndUnlock() throws Exception
	{
		...
		{
			assertEquals(message, "Taxonomy is locked by another user: adfcaf58-241b-4eca-9740-6a26d1c3dd58");
		}
	...
	public void taxonomySoapTest() throws Exception
	{
		...
		for( int i = 0; i < 10; i++ )
		{
			assertEquals(termSoapService.getData(TAX_UUID, "Root\\Term " + i, "data1"), String.valueOf(i));
		}
	...
	private void isTermOnItem(ItemId itemId, String node, String... terms) throws Exception
	{
		...
		{
			assertTrue(termList.contains(terms[i]),
	...
	public void setupTaxonomy() throws Exception
	{
		...
		termSoapService = soapHelper.createSoap(TermSoapService.class, "services/taxonomyTerm.service",
			"http://taxonomy.core.tle.com", null);
