...
public class SyncBlackboardTest extends AbstractBlackboardTest
{
	...
	public void replaceModule()
	{
		...
		BlackboardUploadBuildingBlockPage upload = new BlackboardUploadBuildingBlockPage(context).load();
	...
	{
		BlackboardWebServicesListPage webServices = new BlackboardWebServicesListPage(context).load().deleteEquella();
	...
	{
		new BlackboardLoginPage(context).load().logon(ADMIN_USERNAME, ADMIN_PASSWORD);
		BlackboardEquellaSettingsPage settingsPage = new BlackboardBuildingBlockListPage(context).load()
			.equellaSettings();
		sleepyTime(750);
