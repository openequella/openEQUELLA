...
public class MoodleOptionsTest extends AbstractSequentialMoodleTest
{
	...
	private void assertItem(boolean exists)
	{
		...
		Assert.assertEquals(moodleSelectionPage.equellaSession().homeExactSearch(SELECT_ITEM).isResultsAvailable(),
			exists);
	...
	public void restrictionTest()
	{
		...
		SummaryPage summary = equellaResource.equellaSession().homeExactSearch(SELECT_ITEM)
			.getResultForTitle(SELECT_ITEM, 1).viewSummary();
		...
		summary = equellaResource.equellaSession().homeExactSearch(SELECT_ITEM).getResultForTitle(SELECT_ITEM, 1)
			.viewSummary();
