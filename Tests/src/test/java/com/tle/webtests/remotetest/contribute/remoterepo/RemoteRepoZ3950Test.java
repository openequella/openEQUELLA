...
public class RemoteRepoZ3950Test extends AbstractCleanupAutoTest
{
	...
	public void testZ3950BasicSearchAndContribute()
	{
		...
		RemoteRepoBasicZ3950SearchPage rrZ3950 = rrp.clickRemoteRepository(Z3950_BASIC,
			new RemoteRepoBasicZ3950SearchPage(context));
		...
		RemoteRepoViewZ3950ResultPage z3950ViewResult = sr.viewResult(new RemoteRepoViewZ3950ResultPage(context));
	...
	public void testBathZeroAttributes()
	{
		...
		RemoteRepoAdvancedZ3950SearchPage rrZ3950 = rrp.clickRemoteRepository(Z3950_BATH_0,
			new RemoteRepoAdvancedZ3950SearchPage(context));

		RemoteRepoListPage searchResults = rrZ3950.exactQuery("Any field [Keyword]", "kanye west", "AND",
			"Title [Keyword]", "dark twisted fantasy", "AND", "Author [Precision Match]", "Kirk Walker Graves");
	...
	public void testZ3950AdvancedSearchAndContribute()
	{
		...
		RemoteRepoAdvancedZ3950SearchPage rrZ3950 = rrp.clickRemoteRepository(Z3950_ADVANCED,
			new RemoteRepoAdvancedZ3950SearchPage(context));

		RemoteRepoListPage searchResults = rrZ3950.exactQuery("Title", "Ubuntu", "AND", "Title", "Linux", "AND",
			"Title", "Fedora");

		String resultTitle = "Linux bibleboot up to Ubuntu, Fedora, KNOPPIX, Debian, openSUSE, and 13 other distributions /";
		assertTrue(searchResults.doesResultExist(resultTitle, 1));

		RemoteRepoSearchResult sr = searchResults.getResultForTitle(resultTitle, 1);
		RemoteRepoViewResultPage z3950ViewResult = sr.viewResult(new RemoteRepoViewZ3950ResultPage(context));
	...
	public void testZ3950Paging()
	{
		...
		RemoteRepoBasicZ3950SearchPage rrZ3950 = contribPage.openRemoteRepo(Z3950_BASIC,
			new RemoteRepoBasicZ3950SearchPage(context));
		rrZ3950.search("Test");
