...
public class CALSummaryPage extends AbstractPage<CALSummaryPage>
{
	...
	public <T extends AbstractPage<T>> CALActivatePage<T> activate(int chapter, String attachment, T returnTo)
	{
	...
	public <T extends AbstractPage<T>> CALActivatePage<T> activate(String chapterText, String attachment, T returnTo)
	{
	...
	{
		return By.xpath(sectionPath + "//td[@class='sectionAction']/button[contains(text(),'Activate')]");
	}
	...
	{
		return "//table[@id='copyright-table']//tr[td[@class='copyright-portion' and " + portionMatcher
			+ "]]/following-sibling::tr[td/div[@class='sectionAttachment' and "
			+ "*[contains(@class, 'copyright-sectionlink') and normalize-space(text())=" + quoteXPath(attachment)
			+ "]]][1]";
	...
	{
		return findSection("span[@class='chapterNumber' and normalize-space(text())=" + quoteXPath(chapterText) + "]",
			attachment);
	}
	...
	{
		String xpathExpression = findBookRow(chapterText, attachment) + "//td/div[@class='sectionCheckbox']/input";
	...
	{
		WebElement row = driver.findElement(By.xpath(findBookRow(getChapterNumText(chapter), attachment)));
		WebElement attachmentLink = row.findElement(By.xpath("//td/div/a[contains(@class,'copyright-sectionlink')]"));
	...
	{
		driver.findElement(
			By.xpath(findBookRow(chapterText, attachment)
				+ "//td/div[@class='sectionAttachment']/a[contains(@class, 'copyright-sectionlink')]")).click();
	}

	public boolean canViewSection(int chapter, String attachment)
	{
		return isPresent(By.xpath(findBookRow(getChapterNumText(chapter), attachment)
	...
	{
		WebElement summary = driver.findElement(BY_COPYRIGHT);
		List<WebElement> copyrightTableVals = summary.findElements(By.className("copyright-totalvalue"));
