...
public class MoodleCoursePage extends MoodleBasePage<MoodleCoursePage>
{
	...
	private MoodleSelectionPage clickEquellaResource(int week)
	{
		...
		WebElement link = waitForElement(findElement, By.xpath(".//a/span[text()='Add an activity or resource']/.."));
	...
	{
		return !getSectionElement(section).findElements(By.xpath(xpathForResource(name) + "/a/span")).isEmpty();
	}
	...
	{
		return new ResourceRow(getSectionElement(section), By.xpath(xpathForResource(name))).get().delete();
	}

	public MoodleCoursePage deleteLTIResource(int section, String name)
	{
		return new ResourceRow(getSectionElement(section), By.xpath(xpathForLTILink(name))).get().delete();
	}

	public MoodleCoursePage deleteResourceStartingWith(int section, String name)
	{
		ResourceRow resourceRow = new ResourceRow(getSectionElement(section), By.xpath(xpathForPartial(name)));
	...
	public MoodleEditResourcePage editResource(int section, String name)
	{
		...
		{
			scrollIntoViewAndClick(getSectionElement(section).findElement(
				By.xpath(xpathForResource(name) + "/../span//a[text()=" + quoteXPath("Edit") + "]")));
			editXPath = "/../span//a[@data-action=" + quoteXPath("update") + "]";
		}
		WebElement editAction = getSectionElement(section).findElement(By.xpath(xpathForResource(name) + editXPath));
	...
	{
		scrollIntoViewAndClick(getSectionElement(section).findElement(By.xpath(xpathForResource(name) + "/a")));
	...
	{
		return ".//li[contains(@class, 'lti')]//a/span[normalize-space(text())=" + quoteXPath(name) + "]/../..";
	}

	public String xpathForResource(String name)
	{
		return ".//li[contains(@class, 'equella')]//a/span[normalize-space(text())=" + quoteXPath(name) + "]/../..";
	}

	public String xpathForPartial(String name)
	{
		return ".//li[contains(@class, 'equella')]//a/span[contains(text(), " + quoteXPath(name) + ")]/../..";
	}
	...
	public void deleteAllForWeek(int week)
	{
		...
		ResourceRow row = new ResourceRow(
			section,
			By.xpath(".//li[contains(@class, 'equella') and contains(@class, 'activity')]//div[@class='activityinstance']"));
	...
	{
		return !driver.findElements(By.xpath("//div[contains(@class, 'block_equella_search')]")).isEmpty();
	}
	...
	public MoodleCoursePage deleteSearchBlock()
	{
		...
		{
			String xpath = "//div[contains(@class, 'block_equella_search')]//a[contains(@class, 'toggle-display')]";
			WebElement editLink = driver.findElement(By.xpath(xpath));
			editLink.click();
			element = driver
				.findElement(By
					.xpath("//div[contains(@class, 'block_equella_search')]//a//span[text()='Delete EQUELLA Search block']"));
		...
		else
		{
			element = driver.findElement(By
				.xpath("//div[contains(@class, 'block_equella_search')]//a[@title='Delete EQUELLA Search block']"));
			element.sendKeys(Keys.RETURN);
		}

		return ExpectWaiter.waiter(removalCondition(element),
			new MoodleNoticePage<MoodleCoursePage>(MoodleCoursePage.this, "Yes")).get();
	}
	...
	{
		return !driver.findElements(By.xpath("//div[contains(@class, 'block_equella_tasks')]")).isEmpty();
	}
	...
	public MoodleCoursePage deleteTasksBlock()
	{
		...
		if( this.getMoodleVersion() >= 26 )
		{
			String xpath = "//div[contains(@class, 'block_equella_tasks')]//a[contains(@class, 'toggle-display')]";
			WebElement editLink = driver.findElement(By.xpath(xpath));
			editLink.click();
			element = driver.findElement(By
				.xpath("//div[contains(@class, 'block_equella_tasks')]//a//span[text()='Delete EQUELLA Tasks block']"));
			element.click();
		}
		else
		{
			element = driver.findElement(By
				.xpath("//div[contains(@class, 'block_equella_tasks')]//a[@title='Delete EQUELLA Tasks block']"));
			element.sendKeys(Keys.RETURN);
		}
	...
	{
		getSectionElement(section).findElement(By.xpath(xpathForResource(name) + "/span/a[@title='Update']")).click();
		return new MoodleAddExternalToolPage(context).get();
	}
	...
	public class ResourceRow extends AbstractPage<ResourceRow>
	{
		...
		public MoodleCoursePage delete()
		{
			if( moodleVersion >= 26 )
			{
				scrollIntoViewAndClick(getLoadedElement().findElement(
					By.xpath("..//span//a[contains(@class, 'toggle-display')]")));
				deleteButton = getLoadedElement().findElement(
					By.xpath("..//span//a[@data-action=" + quoteXPath("delete") + "]"));
			...
			catch( NoAlertPresentException e )
			{
				if( moodleVersion >= 27 )
				{
					driver.findElement(By.xpath("//div[@class='confirmation-buttons']/input[@value='Yes']")).click();
					return MoodleCoursePage.this.get();
				}
