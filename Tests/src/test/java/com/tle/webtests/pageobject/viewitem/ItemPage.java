...
{
	private static final String ITEM_DETAILS_XPATH = "id('col2')/h3[text()='Details']/following-sibling::div[1]";
	...
	protected <P extends PageObject> P clickActionAccept(String action, WaitingPageObject<P> returnTo)
	{
		driver.findElement(getActionBy(action)).click();
		acceptConfirmation();
		return returnTo.get();
	...
	{
		return isPresent(By.xpath("//head/meta[@name=" + quoteXPath(name) + " and @content=" + quoteXPath(content)
			+ "]"));
	}
	...
	{
		return driver.findElement(By.xpath("//h3[text()=" + quoteXPath(pricingTierTitle) + ']')).isDisplayed();
	...
	{
		return isPresent(By.xpath("//div[@class='tiers']//tbody//td[text() = " + quoteXPath(pricingTier) + "]"));
	}
	...
	{
		return driver.findElement(By.id("sc_ps_subt")).findElement(By.xpath(xpathForRow(i) + "/td[1]")).getText();
	}
	...
	public String verifyPricingTierValueAddedAtIndex(int rowindex, int columnindex)
	{
		...
		return driver.findElement(By.id("sc_ps_subt"))
			.findElement(By.xpath(xpathForRow(rowindex) + "/td[" + columnid + "]")).getText();
	}
