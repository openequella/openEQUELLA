...
public class AdminConsoleWindow extends AbstractAppletWindow<AdminConsoleWindow>
{
	...
	private void setup()
	{
		ClientLauncher frame = GuiActionRunner.execute(new GuiQuery<ClientLauncher>()
		{
	...
	{
		windowHolder = WindowFinder
			.findFrame(FrameMatcher.withTitle(Pattern.compile("Client Launcher.*")).andShowing()).withTimeout(30000)
			.using(windowHolder.robot);

		windowHolder.comboBox(wait(JComboBox.class)).selectItem(
			institutionName + " - " + context.getBaseUrl() + institutionName + "/");
		windowHolder.button(wait(JButtonMatcher.withText("Launch").andShowing())).click();
		windowHolder = WindowFinder
			.findFrame(FrameMatcher.withTitle(Pattern.compile("Administration Console.*")).andShowing())
