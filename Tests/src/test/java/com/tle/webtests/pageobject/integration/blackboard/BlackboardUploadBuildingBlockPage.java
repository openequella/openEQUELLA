...
public class BlackboardUploadBuildingBlockPage extends AbstractPage<BlackboardUploadBuildingBlockPage>
{
	private static String BB9_BLOCK_URL = "http://dev-builds.equella.com/job/EQUELLA-Build/lastSuccessfulBuild/artifact/Source/Integration/Blackboard/bb91/product/equella-bb91.war";
	...
	public BlackboardBuildingBlockListPage uploadLatest()
	{
		try
		{
			...
			js.executeScript("document.getElementById('top_submitButtonRow').innerHTML += \"<input type='file' name='pluginFile_LocalFile0' id='pluginFile_chooseLocalFile1' title='Browse'>\";");
			js.executeScript("widget.InlineSingleLocalFilePicker.pickerMap['pluginFile'].required = false;");
			js.executeScript("document.getElementById('pluginFile_attachmentType').value = 'L';");
			js.executeScript("document.getElementById('pluginFile_fileId').value = arguments[0].value;", fileUpload);
			js.executeScript("var anElement = document.getElementById('pluginFile_chooseLocalFile'); anElement.parentNode.removeChild(anElement);");
			...
			if( blockFile == null )
			{
				String blockUrl = testConfig.getProperty("blackboard.buildingblock.download.url", BB9_BLOCK_URL);
			...
			waitForElement(By.xpath("//span[normalize-space(text())="
		...
		{
			e.printStackTrace();
			throw new SkipException("An error occurred downloading the building block: " + e.getLocalizedMessage());
		}
	...
	{
		TestConfig testConfig = getContext().getTestConfig();
		String username = testConfig.getProperty("blackboard.buildingblock.download.username", "autotest");
		String password = testConfig.getProperty("blackboard.buildingblock.download.password",
			"cb7967ae9be763b30aeef126c775128a");
