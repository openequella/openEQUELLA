...
public class SummaryPage extends ItemPage<SummaryPage>
{
	...
	public SalesHistoryPage viewSalesHistory()
	{
		clickSummaryLink("Sales history");
		return new SalesHistoryPage(context).get();
	}

	public String getItemTitle()
	{
		return driver.findElement(
			By.xpath("id('col1')//div[@class='item-title-and-desc']/h2[contains(@class, 'item-title')]")).getText();
	}
	...
	{
		return By.xpath("//h3[text()='Details']/following-sibling::div[@id='adjacentuls']/ul/li/a[text()="
			+ quoteXPath(title) + "]");
	}
	...
	{
		return new SelectionStatusPage(context).get().finishSelections().returnSelection(frame, returnTo);
	}

	public <T extends PageObject> T finishSelecting(WaitingPageObject<T> returnTo, WebElement frame)
	{
		return new SelectionStatusPage(context).get().finishSelections().returnSelection(returnTo, frame);
	}
	...
	{
		final List<WebElement> tiers = driver.findElements(By.xpath("//div[contains(@class,'pricing')]/div/h4"));
	...
	public PurchaseDetailsTable getPurchaseDetails()
	{
		if( isPresent(By.id("sc_pis_purchasedInfoTable")) )
		{
			return new PurchaseDetailsTable(context, By.id("sc_pis_purchasedInfoTable")).get();
		}
		return null;
	}

	/**
	 * Given that scripts can customise the output in an arbitrary fashion, this
	 * method allows the caller to pass in a n arbitrary class attribute name,
	 * and the text value of all (if any) matching elements is accumulated. No
	 * conclusions may be drawn from the order, as empty strings/null are
	 * ignored.
	 * 
	 * @param classAttrib
	 * @returnArrayList of String being the text values of matching elements if
	 *                  any.
	 */
	...
	// Access Edit Catalogues Page - Dinuk
	public ItemCataloguesPage editCataloguesPage()
	{
		return clickAction("Edit catalogues", new ItemCataloguesPage(context));
	}

	// Access Select Pricing Tiers Page - Dinuk
	public ItemPricingTiersPage editPricingTiersPage()
	{
		return clickAction("Set pricing tiers", new ItemPricingTiersPage(context));
	}
	...
	public List<String> getValuesByCustomDisplay()
	{
		...
		List<WebElement> elems = driver.findElements(By.xpath("//div[contains(@class,'displayNodeFull')]//p[text()]"));
