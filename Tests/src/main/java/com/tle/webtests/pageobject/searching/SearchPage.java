...
public class SearchPage extends AbstractQueryableSearchPage<SearchPage, ItemListPage, ItemSearchResult>
{
	...
	public SearchPage saveSearchToFavourites(String searchName)
	{
		...
		FavouriteSearchIntegrationPage favouriteSearchIntegrationPage = new FavouriteSearchIntegrationPage(context);
	...
	{
		return new SearchPage(context).load().search('"' + itemFullName + '"').viewFromTitle(itemFullName);
	}
