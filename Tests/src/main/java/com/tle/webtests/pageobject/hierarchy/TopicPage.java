...
public class TopicPage extends AbstractQueryableSearchPage<TopicPage, TopicListPage, TopicSearchResult>
{
	...
	{
		return isPresent(By.xpath("//div[contains(@class,'browse-topics')]//ul/li[" + position + "]/a[text()="
			+ quoteXPath(title) + "]"));
	}
	...
	public int topicCount(String title)
	{
		...
		{
			count = Integer.parseInt(topicAndCount.substring(title.length() + 2, topicAndCount.length() - 1));
		}
	...
	public <T extends PageObject> T saveSearch(String searchName, WaitingPageObject<T> targetPage)
	{
		...
		return dialog.favourite(searchName,
