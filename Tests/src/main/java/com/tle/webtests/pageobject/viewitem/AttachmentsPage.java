...
public class AttachmentsPage extends AbstractPage<AttachmentsPage>
{
	...
	{
		return isPresent(attachmentsList, new ByChained(attachmentBy(description), By.tagName("button")));
	}
	...
	{
		return By.xpath("li[not(contains(@class, 'inactive')) and .//a[normalize-space(text())="
	...
	public String attachmentDetails(String description)
	{
		...
		{
			row.click();
			waiter.until(ExpectedConditions2.visibilityOfElementLocated(attachmentsList,
				activeAttachmentBy(description)));
		}

		List<WebElement> meta = row.findElements(By.xpath(".//div[normalize-space(@class)='attachments-meta']"));
	...
	public <T extends PageObject> T selectAttachment(String description, WaitingPageObject<T> returnTo)
	{
		WebElement row = attachmentRow(description);
	...
	public <T extends PageObject> T selectAttachmentMultiple(String description, WaitingPageObject<T> returnTo)
	{
		SelectionStatusPage status = new SelectionStatusPage(context).get();
		return selectAttachment(description, status.updateWaiter()).finishSelections().returnSelection(returnTo);
	}
	...
	/**
	 * @param index the first attachment section index start with an empty
	 *            string, second section is "2", third is "3" and so forth
	 * @return
	 */
	...
	{
		return isPresent(attachmentsList, new ByChained(attachmentBy(description), By.className("preview-tag")));
	}
	...
	{
		WaitingPageObject<AttachmentsPage> waiter = ExpectWaiter.waiter(
			ExpectedConditions2.presenceOfElement(reorderOverlay), this);
		reorderLink.click();
		waiter.get();
	}
