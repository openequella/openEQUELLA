...
public abstract class AbstractResultList<T extends AbstractResultList<T, SR>, SR extends SearchResult<SR>>
	extends
		AbstractPage<T>
{
	...
	{
		WebElement firstChild = ((WrapsElement) getResultsDiv().findElement(By.xpath("*[1]"))).getWrappedElement();
		return ExpectWaiter.waiter(
			ExpectedConditions.and(ExpectedConditions.stalenessOf(firstChild),
			ExpectedConditions.visibilityOfElementLocated(By.xpath("id('searchresults')[div[@class='itemlist'] or h3]"))), this);
	}
	...
	public List<SR> getResults()
	{
		...
		{
			int count = getResultsDiv().findElements(By.xpath("//div[@class='itemresult-wrapper']")).size();
