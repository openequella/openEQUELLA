...
//		javascript.callEmbeddedSelenium(driver, "triggerEvent", nameField, "blur");
	...
	public void setDescription(String description)
	{
		...
		((JavascriptExecutor) driver).executeScript("document.body.innerHTML = " + quoteXPath(description));
	...
	public MoodleEditResourcePage insertImage(String item, String image, String alt)
	{
		...
		if( context.getTestConfig().isChromeDriverSet() )
		{
			...
			((JavascriptExecutor) driver)
				.executeScript("var tempVar = tinymce.DOM.setAttrib; tinymce.DOM.setAttrib = function(id, attr, val) { if (attr == 'src' && val.trim().match(/javascript\\s*:\\s*(\"\\s*\"|'\\s*')/)) {return;} else {tempVar.apply(this, arguments);} }");
			waiter.until(ExpectedConditions2.frameToBeAvailableAndSwitchToIt(driver,
				By.xpath("//div[@class='mceMiddle']//iframe")));
		}
		waitForElement(By.id("insert")).click();
		driver.switchTo().defaultContent();
		waiter.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[@class='mceMiddle']//iframe")));
		return get();
	}
	...
	public class FilePicker extends AbstractPage<FilePicker>
	{
		...
		public FilePicker()
		{
			super(MoodleEditResourcePage.this.context, MoodleEditResourcePage.this.driver, By
				.className("fp-generallayout"));
		}
		...
		public SelectFileDialog searchAndSelect(String item, String attachment)
		{
			...
			SummaryPage viewSummary = searchPage.exactQuery(item).getResultForTitle(item, 1).viewSummary();
	...
	public class SelectFileDialog extends AbstractPage<SelectFileDialog>
	{
		...
		{
			super(MoodleEditResourcePage.this.context, MoodleEditResourcePage.this.driver, By.className("fp-select"));
		}
