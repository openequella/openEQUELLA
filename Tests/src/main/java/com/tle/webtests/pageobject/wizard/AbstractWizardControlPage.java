...
public abstract class AbstractWizardControlPage<T extends AbstractWizardControlPage<T>> extends AbstractWizardTab<T>
{
	...
	{
		UniversalControl universalControl = universalControl(ctrlNum);
		UrlUniversalControlType urlControl = universalControl
			.addResource(new UrlUniversalControlType(universalControl));
		urlControl.addUrl(url, displayName, preview);
	}

	public boolean setCheck(int ctrlnum, String value, boolean checked)
	{
		WebElement check = driver.findElement(By.xpath("//input[@name=" + quoteXPath("c" + getControlNum(ctrlnum))
			+ " and @value = " + quoteXPath(value) + "]"));
	...
	protected T setCheckWaiter(int ctrlnum, String value, boolean checked, WaitingPageObject<T> waiter)
	{
	...
	public WaitingPageObject<T> getUpdateWaiter(int ctrlnum)
	{
		...
		return ExpectWaiter.waiter(ExpectedConditions2.updateOfElementLocated(existingControl, driver, by), this);
	}
	...
	public ExpectedCondition<WebElement> getNewAttachmentExpectation(String item)
	{
		return ExpectedConditions.visibilityOfElementLocated(By
			.xpath("//div[@class='universalresources']/table/tbody/tr/td[@class='name']/a[text()=" + quoteXPath(item)
				+ "]"));
	}
	...
	{
		return driver
			.findElement(By.xpath("id(" + quoteXPath(getControlId(ctrlNum)) + ")/div/p[@class='ctrlinvalidmessage']"))
			.getText().trim();
	}
