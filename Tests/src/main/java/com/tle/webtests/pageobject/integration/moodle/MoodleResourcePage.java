...
public class MoodleResourcePage extends AbstractPage<MoodleResourcePage>
{
	...
	{
		switchToItem();
		boolean external = isPresent(By.id("elv_itemName")) && isPresent(By.xpath("//iframe[@id='external-content']"));
		returnToMoodle();
	...
	public boolean isTokenError()
	{
		...
		boolean denied = !driver.findElements(
			By.xpath("//p[contains(text(),'You do not have permissions to use this token')]")).isEmpty();
		returnToMoodle();
		return denied;
	}
	...
	protected void switchToItem()
	{
		if( context.getTestConfig().isChromeDriverSet() )
		{
			...
			try
			{
				WebElement objectTag = driver.findElement(By.id("region-main")).findElement(By.tagName("object"));
				waiter.until(ChromeHacks.convertObjectToiFrame(context, objectTag));
			}
			catch( Exception e )
			{
				waiter.until(ExpectedConditions2.frameToBeAvailableAndSwitchToIt(context.getDriver(),
					By.id("contentframe")));
			}
		}
		else
