...
public class MoodleTestFactory
{
	...
	public Object[] createTests(
		@Optional("com.tle.webtests.remotetest.integration.moodle.AbstractParallelMoodleTest") String classType)
		throws Exception
	{
		...
		URL dirUrl = getClass().getClassLoader().getResource("com/tle/webtests/remotetest/integration/moodle");
		...
		Collections.sort(testObjects, new Comparator<AbstractMoodleTest>()
		{
			@Override
	...
	private void recurse(File dir, String packagePrefix, List<AbstractMoodleTest> testObjects, String classType)
		throws Exception
	{
		...
		for( File file : files )
		{
			...
			else if( file.isFile() && filename.endsWith(".class") && filename.indexOf('$') == -1 )
			{
				Class<?> clazz = getClass().getClassLoader().loadClass(
					packagePrefix + filename.substring(0, filename.length() - 6));

				if( Class.forName(classType).isAssignableFrom(clazz) && !Modifier.isAbstract(clazz.getModifiers()) )
				{
