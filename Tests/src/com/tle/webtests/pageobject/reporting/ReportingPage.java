...
public class ReportingPage extends AbstractPage<ReportingPage>
{
	...
	public <R extends AbstractReport<R>> NoParamsReportWindow<R> getReport(String reportName, R report)
	{
		return getReport(reportName, new NoParamsReportWindow<R>(context, report));
	}

	public <R extends AbstractReport<R>, T extends AbstractReportWindow<R, T>> T getReport(String reportName, T page)
	{
		final String currentWindowHandle = driver.getWindowHandle();
		page.setWindowHandle(currentWindowHandle);

		waitForElement(columnSort);
		ExpectedCondition<String> newWindowExpectation = ExpectedConditions2.newWindowOpenedAndSwitchedTo(driver);
