...
public abstract class AbstractAppletWindow<A extends AbstractAppletWindow<A>>
{
	...
	{
		List<JButton> buttons = Lists.newArrayList(windowHolder.robot.finder().findAll(windowHolder.component(),
			JButtonMatcher.withText(text).andShowing()));
	...
	{
		List<JTextComponent> edits = Lists.newArrayList(windowHolder.robot.finder().findAll(windowHolder.component(),
			JTextComponentMatcher.any().andShowing()));
	...
	{
		WindowFinder.findDialog(DialogMatcher.withTitle(Pattern.compile(".*" + partialTitle + ".*")).andShowing())
			.withTimeout(30000).using(windowHolder.robot);
		return (A) this;
	}
	...
	{
		Pause.pause(new Condition("Codition: " + cm.toString())
		{
			@Override
			public boolean test()
			{
	...
	{
		Pause.pause(new Condition("Codition: Waiting for download")
		{
			@Override
			public boolean test()
			{
				try
				{
					Component component = windowHolder.robot.finder().find(
						JLabelMatcher.withText("Downloading...").andShowing());
					return !component.isShowing();
				}
				catch( ComponentLookupException e )
				{
