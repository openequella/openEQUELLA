...
public class LanguageSettingsPage extends AbstractPage<LanguageSettingsPage>
{
	...
	public LanguageSettingsPage deleteContribLangByName(String name)
	{
		...
		contributionLanguageTable.findElement(
			new ByChained(getRowBy(name), By.xpath("td[@class='actions']/a[@class='unselect']"))).click();
	...
	private ExpectWaiter<LanguageSettingsPage> newContWaiter(String named)
	{
		return ExpectWaiter.waiter(
			ExpectedConditions2.visibilityOfElementLocated(contributionLanguageTable, getRowBy(named)), this);
	}

	private ExpectWaiter<LanguageSettingsPage> contDisappearWaiter(String named)
	{
		return ExpectWaiter.waiter(
			ExpectedConditions2.invisibilityOfElementLocated(contributionLanguageTable, getRowBy(named)), this);
	}

	private ExpectWaiter<LanguageSettingsPage> newPackWaiter(String named)
	{
		return ExpectWaiter.waiter(ExpectedConditions2.visibilityOfElementLocated(languagePacksTable, getRowBy(named)),
			this);
	}
	...
	{
		return ExpectWaiter.waiter(
			ExpectedConditions2.invisibilityOfElementLocated(languagePacksTable, getRowBy(named)), this);
	}
	...
	public LanguageSettingsPage uploadLanguagePack(URL langURL, String languageName, String fullname)
	{
	...
	public AbstractPage<?> addContributionLanguageAndCountry(String languageVal, String countryVal, String fullname)
	{
		// open the dialog
