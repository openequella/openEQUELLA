...
public abstract class AbstractSelectedStuff<T, P extends AbstractSelectedStuff<T, P>> extends AbstractPage<P>
{
	...
	private static final String TBODY_XPATH = TABLE_XPATH + "[not(contains(@class, 'no-selections'))]/tbody";
	...
	/**
	 * There is no guarantee that this exists, so callees should ensure they
	 * check for NotFoundException if the method is expected to be used in a
	 * "check if a selection has been made" situation. For example,
	 * "clickAction" doesn't check because the user can check first using the
	 * safe "getSelections" method.
	 */
	...
	public <PO extends PageObject> PO clickActionWithConfirm(String selection, String action, Boolean confirm,
		WaitingPageObject<PO> returnTo)
	{

		getTableBody().findElement(
			By.xpath(TD_NAME_XPATH + getAdditionalNameXpathConstraint(selection) + "/following-sibling::td/a[@title = "
				+ quoteXPath(action) + "]")).click();
		if( confirm != null )
		{
			if( confirm )
			{
	...
	{
		return By.xpath(TBODY_XPATH + '/' + TD_NAME_XPATH + getAdditionalNameXpathConstraint(selection));
	}
}
