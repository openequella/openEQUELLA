...
public class TleJUnitReportReporter implements IReporter
{
	@Override
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String defaultOutputDirectory)
	{
		...
		// A list of iterators for all the passed configuration, explanation
		// below
		// ListMultiMap<Class<?>, ITestResult> beforeConfigurations =
		// Maps.newListMultiMap();
		// ListMultiMap<Class<?>, ITestResult> afterConfigurations =
		// Maps.newListMultiMap();
		// for (Map.Entry<Class<?>, Set<ITestResult>> es :
		// passedConfigurations.entrySet()) {
		...
		// for (Map.Entry<Class<?>, List<ITestResult>> es :
		// beforeConfigurations.getEntrySet()) {
		...
		// for (Map.Entry<Class<?>, List<ITestResult>> es :
		// afterConfigurations.getEntrySet()) {
		...
		for( Map.Entry<Class<?>, Set<ITestResult>> entry : results.entrySet() )
		{
			...
			for( ITestResult tr : entry.getValue() )
			{
				...
				boolean isSkipped = tr.getStatus() == ITestResult.SKIP;
				...
				if( !isSuccess && t != null )
				{
					...
					testTag.errorTag = tr.getThrowable() instanceof AssertionError ? "error" : isSkipped ? "skipped"
						: "failure";
				}
				totalTime += time;
				testCount++;
			...
			String outputDirectory = defaultOutputDirectory + File.separator + "tlejunitreports";
	...
	/**
	 * Add the time of the configuration method to this test method. The only
	 * problem with this method is that the timing of a test method might not be
	 * added to the time of the same configuration method that ran before it but
	 * since they should all be equivalent, this should never be an issue.
	 */
	...
	private Throwable getThrowable(ITestResult tr, Map<Class<?>, Set<ITestResult>> failedConfigurations)
	{
		...
		if( result == null && tr.getStatus() == ITestResult.SKIP )
		{
			...
			for( Set<ITestResult> failures : failedConfigurations.values() )
			{
				...
				{
					// Naive implementation for now, eventually, we need to try
					// to find
					// out if it's this failure that caused the skip since
					// (maybe by
					// seeing if the class of the configuration method is
					// assignable to
					// the class of the test method, although that's not 100%
					// fool proof
