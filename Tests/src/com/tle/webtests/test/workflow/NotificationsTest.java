...
public class NotificationsTest extends AbstractCleanupTest
{
	...
	public void setupNotifications()
	{
		...
		wizard = new ContributePage(context).load().openWizard("Different Collection with moderation deadline");
		wizard.editbox(1, MOD_OVERDUE_REJECT);
		wizard.editbox(2, "Given the existence as uttered forth in the public works of Puncher and Wattman "
	...
	public void autoAcceptanceTest()
	{
		...
		SearchPage sp = new SearchPage(context).load();
		sp.setIncludeNonLive(true);
		ItemSearchResult itemSearchResult = sp.search(MOD_AUTO_ACCEPT).getResultForTitle(MOD_AUTO_ACCEPT, 1);
	...
	public void autoDeleteTest()
	{
		...
		sp = new SearchPage(context).load();
		summaryPage = sp.search(WATCHED_COLLECTION).getResultForTitle(WATCHED_COLLECTION, 1).viewSummary();
		summaryPage.archive();
		...
		summaryPage = myResourcesPage.exactQuery(MOD_OVERDUE_REJECT).getResultForTitle(MOD_OVERDUE_REJECT)
			.viewSummary();
		summaryPage.delete();
		np = new NotificationsPage(context).load();
		np.search(namePrefix);
	...
	private void createItemAndModeratePastFirstStep(String creatingUser, String pwd)
	{
		...
		WizardPageTab wizard = new ContributePage(context).load().openWizard(
			"Different Collection with moderation deadline");
		...
		mv.acceptToMessagePage().acceptWithMessage(
	...
	private void runNotifyNewTasksAsSystemSuperUserAndReloginAs(String reloginUser, String reloginPasswd)
	{
		logout();
		logon("TLE_ADMINISTRATOR", "tle010");
