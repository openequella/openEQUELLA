...
public class FavouritesInIntegrationTest extends AbstractCleanupTest
{
	...
	/**
	 * Institution, and AutoTest user, presumed to contain an item with this
	 * title as a favourite.
	 */
	private static String FAVOURITE_PRESUMED_TO_EXIST = "Hal's eying Flickr";
	/**
	 * Preserved in the exported contributions - tests/myresources/institution.tar.gz
	 * being the uuid for the "Generic Testing Collection".
	 */
	private static String GENERIC_TESTING_COLLECTIONS_ID_FROM_TEST_INSTITUTION = "ef19a911-0b3c-85ed-efd9-a56e56edcf37";
	...
	public void findFavourites()
	{
		...
		String returnedUrl = itp.getSignonUrl(ACTION, AUTOTEST_LOGON, "", "contributionCollectionIds="
			+ GENERIC_TESTING_COLLECTIONS_ID_FROM_TEST_INSTITUTION);
		...
		// We can expect Favourites to be already selected from the links 'Favourites | Contributed | Selected',
		...
		List<WebElement> linksToFavItems = favContribSelectedBox.findElements(By.xpath(".//div[@class='alt-links']/a"));
	...
	private void contributeFavourite()
	{
		...
		IntegrationTesterReturnPage returnPage = spat.selectItem(new IntegrationTesterReturnPage(context));
		String itSays = returnPage.returnedRow("result");
		assertTrue(itSays.toLowerCase().contains("success"), "Instead of result = 'success', it says " + itSays);
	}
}
