...
/**
 * Test Reference: http://time/DTEC/test/editTest.aspx?testId=15096
 * Preconditions: the wizard for COLLECTIONS_NAME is set up with 2 edit boxes
 * and a 3rd control being a checkbox, which enables a second page to exist in
 * the contribution sequence. This test presupposes that a second wizard page
 * exists but is not mandatory, hence if the user has not checked the box
 * indicating that they wish the resource to be added, then the item can be
 * saved and published as is (the only unilaterally mandatory data is the "Name"
 * field in the first editBox).
 * 
 * @author larry
 */
...
public class VaryingContribPageNmbrSaveTest extends AbstractCleanupAutoTest
{
	...
	/**
	 * Preserved in the exported contributions -
	 * tests/contribute/institution.tar.gz being the uuid for the
	 * COLLECTIONS_NAME collection.
	 */
	private static String COLLECTIONS_ID_FROM_TEST_INSTITUTION = "cfbde79c-dfee-4f44-b5a8-70d11055914b";

	/**
	 * Not having checked the "Resource" to enable a second page for the wizard
	 * we expect a dialogue box with the options to either Publish, Draft or
	 * Cancel, and we don't expect Submit (because there is no workflow
	 * moderation applicable, nor Complete Wizard (incorrect).
	 */
	@Test
	...
	/**
	 * Same as above except for the step to check the "Resource" to enable a
	 * second page for the wizard. Accordingly on attempting "Save" we expect a
	 * dialogue box with the options to either Complete Wizard, Save as Draft or
	 * Cancel, and we don't expect Submit (because there is no workflow
	 * moderation applicable, nor Publish (incorrect).
	 */
	@Test
	...
	/**
	 * Populate lists of expected and unexpected strings, verify content of
	 * Confirmation Dialogue's buttons.
	 * 
	 * @param askedToFinish
	 * @param confirmationDialog
	 */
	private void askedToFinishCannotPublish(boolean askedToFinish, ConfirmationDialog confirmationDialog)
	{
