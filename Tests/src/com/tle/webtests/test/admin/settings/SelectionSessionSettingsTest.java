...
{
	public static final String SELECTION_SESSION_SETTINGS_LINK = "Selection sessions"; // original property key: quickcontributeandversionsettings.title
	...
	public void testQuickContributeSetting()
	{
		...
		assertTrue(chosenCollection.equals(sssp.getSelectedCollection()), "Expected " + chosenCollection + " but got "
			+ sssp.getSelectedCollection() + '.');
		logout();
	}
	...
	public void testVersionViewOptions()
	{
		...
		assertTrue(numVersionViewOptions >= 4,
		...
		logout();
		sssp = logonToSelectionSessionSettings();
		assertTrue(sssp.getCheckedViewOptionValue().equals(secondOptionVal), "Expected " + secondOptionVal
			+ " to persist, but we have " + sssp.getCheckedViewOptionValue() + '.');
		...
		logout();
		sssp = logonToSelectionSessionSettings();
		assertTrue(sssp.getCheckedViewOptionValue().equals(thirdOptionVal), "Expected " + thirdOptionVal
			+ " to persist, but we have " + sssp.getCheckedViewOptionValue() + '.');
	}
