...
prepareDevConfig := {
  ...
  val pluginRoots =
    Seq(srcRoot / "Source/Plugins", srcRoot / "Platform", srcRoot / "Interface")
  ...
  val adminurl =
    if (bc.hasPath("adminurl")) bc.getString("adminurl")
  ...
  mc.setProperty("freetext.stopwords.file",
                 installerConfig / "en-stopWords.txt")
  ...
  imc.load(
    installerConfig / "plugins/com.tle.core.imagemagick/config.properties.unresolved")
  ...
  jpfWriteDevJars
    .all(
      ScopeFilter(
        inAggregates(LocalProject("allPlugins"), includeRoot = false)))
    .value
}
