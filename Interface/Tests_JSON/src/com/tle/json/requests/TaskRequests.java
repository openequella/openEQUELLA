...
public class TaskRequests extends AuthorizedRequests {
  ...
  public RequestSpecification searchRequest(
      String query, String filter, String order, String length) {
  ...
  public ObjectNode findTaskToModerate(
      final ItemId expectedId, final String query, final String taskId) {
  ...
  public static Function<ObjectNode, ObjectNode> resultAvailable(
      final ItemId expectedId, final String taskId) {
  ...
  public static Function<ObjectNode, Boolean> taskOrder(
      final ItemId expectedId, final String... tasks) {
  ...
  public void accept(ItemId item, String task, String message) {
    actionRequest(successfulRequest(), message)
        .post(getModUri(), getActionParams(item, task, "accept"));
  }
  ...
  public void rejectFail(ItemId item, String task, String message, String toTask) {
    rejectRequest(actionRequest(badRequest(), message), toTask)
        .post(getModUri(), getActionParams(item, task, "reject"));
  }

  public void comment(ItemId item, String task, String message) {
    actionRequest(successfulRequest(), message)
        .post(getModUri(), getActionParams(item, task, "comment"));
  }
  ...
  private String getModUri() {
    return getBaseUri().resolve(ItemRequests.PATH).toString()
        + "/{uuid}/{version}/task/{task}/{action}";
  }
}
