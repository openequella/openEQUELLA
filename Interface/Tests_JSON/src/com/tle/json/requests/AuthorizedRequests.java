...
public abstract class AuthorizedRequests
{
	...
	protected AuthorizedRequests(URI baseUri, TokenProvider tokenProvider, ObjectMapper mapper,
		PageContext pageContext, TestConfig testConfig)
	{
	...
	protected RequestSpecification auth()
	{
		...
		return (token == null ? RestAssured.given() : RestAssured.given().header("X-Authorization", token)).header(
			"X-Autotest-Key", pageContext.getFullName(""));
	}

	protected RequestSpecification jsonBody(RequestSpecification request, ObjectNode body)
	{
		return request.contentType("application/json").body(body.toString().getBytes(Charsets.UTF_8))
	...
	{
		return new Waiter<AuthorizedRequests>(this).withTimeout(20, TimeUnit.SECONDS).until(
