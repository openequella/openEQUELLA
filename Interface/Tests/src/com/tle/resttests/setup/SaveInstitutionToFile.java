...
public class SaveInstitutionToFile extends AbstractImportExport
{
	...
	{
		super(new TestConfig(SaveInstitutionToFile.class, true), shortName, shortName, password, baseDir);
	}

	public SaveInstitutionToFile(File baseDir, String shortName, String password, String fullUrl, boolean bucket)
	{
		super(new TestConfig(SaveInstitutionToFile.class, true), shortName, shortName, password, password, baseDir,
			fullUrl);
		this.bucket = bucket;
	}
	...
	{
		SaveInstitutionToFile instSaver = new SaveInstitutionToFile(new File(args[0]), args[1], args[2]);
		instSaver.export();
	}
	...
	protected String exportFrom(String offset, final File itemsDir, final boolean ignoreFiles) throws Exception
	{
		return exportFrom(offset, itemsDir, new Function<ObjectNode, Void>()
		{
			@Override
	...
	protected Integer[] exportFromEquella(int offset, File itemsDir, boolean ignoreFiles) throws Exception
	{
		return exportFromEquellaPerItem(offset, itemsDir, new Function<ObjectNode, Void>()
		{
			@Override
			public Void apply(ObjectNode entity)
			{
