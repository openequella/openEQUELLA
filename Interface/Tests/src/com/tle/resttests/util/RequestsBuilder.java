...
public class RequestsBuilder
{
	...
	{
		return new CollectionRequests(baseURI, tokens, MAPPER, test.getContext(), test, test.getTestConfig());
	}

	public WorkflowRequests workflows()
	{
		return new WorkflowRequests(baseURI, tokens, MAPPER, test.getContext(), test, test.getTestConfig());
	}

	public SchemaRequests schemas()
	{
		return new SchemaRequests(baseURI, tokens, MAPPER, test.getContext(), test, test.getTestConfig());
	}
	...
	{
		return new NotificationRequests(baseURI, tokens, MAPPER, test.getContext(), test.getTestConfig());
	}

	public InstitutionRequests institutions(TokenProvider tokenProvider)
	{
		return new InstitutionRequests(baseURI, tokenProvider, MAPPER, test.getContext(), test, test.getTestConfig(),
				test.getTestConfig().getAdminPassword());
	}

	public InstitutionRequests institutions()
	{
		return new InstitutionRequests(baseURI, MAPPER, test.getContext(), test, test.getTestConfig(), test.getTestConfig().getAdminPassword());
	}
	...
	{
		return new OAuthRequests(baseURI, tokens, MAPPER, test.getContext(), test, test.getTestConfig());
	}
