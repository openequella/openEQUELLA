...
public class UserRequestsBuilder
{
	...
	{
		return new UserRequests(baseURI, MAPPER, test.getContext(), test, test.getTestConfig(), test.getTestConfig().getAdminPassword());
	}

	public UserRequests users(TokenProvider tokenProvider)
	{
		return new UserRequests(baseURI, tokenProvider, MAPPER, test.getContext(), test, test.getTestConfig());
	}

	public GroupRequests groups()
	{
		return new GroupRequests(baseURI, MAPPER, test.getContext(), test, test.getTestConfig(), test.getTestConfig().getAdminPassword());
	}

	public GroupRequests groups(TokenProvider tokenProvider)
	{
		return new GroupRequests(baseURI, tokenProvider, MAPPER, test.getContext(), test, test.getTestConfig());
	}

	public RoleRequests roles()
	{
		return new RoleRequests(baseURI, MAPPER, test.getContext(), test, test.getTestConfig(), test.getTestConfig().getAdminPassword());
	}

	public RoleRequests roles(TokenProvider tokenProvider)
	{
		return new RoleRequests(baseURI, tokenProvider, MAPPER, test.getContext(), test, test.getTestConfig());
	}
