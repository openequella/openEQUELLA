...
public class OAuthApiEditTest extends AbstractEntityApiEditTest {
  ...
  public void testCreateBad() {
    normalRequests.createFail(
        normalRequests.badRequest(),
        OAuthClients.json(
            context.getFullName("Blank clientId"),
            "",
            "blah",
            RestTestConstants.USERID_AUTOTEST,
            "default"));
    normalRequests.create(
        OAuthClients.json(
            context.getFullName("dupetest"),
            "duplicateId",
            "blah",
            RestTestConstants.USERID_AUTOTEST,
            "default"));
  ...
  public void testSuccessiveEdits() {
    ObjectNode client =
        normalRequests.create(createJsonForEdit(context.getFullName("testSuccessiveEdits")));
    ...
    Assert.assertEquals(
        ...
        context.getFullName("first edit name"),
    ...
    Assert.assertEquals(
        client.get("name").asText(),
        context.getFullName("fifth edit name"),
        "Unexpect OAuth client name");
    Assert.assertEquals(
        client.get("description").asText(), "fifth edit desc", "Unexpect OAuth client description");
    Assert.assertEquals(
        client.get("descriptionStrings").get("en").asText(),
        "fifth edit desc",
        "Unexpect OAuth client description");
    ...
    Assert.assertEquals(
        client.get("name").asText(),
        context.getFullName("sixth edit name"),
        "Unexpect OAuth client name");
    Assert.assertEquals(
        client.get("description").asText(), "fifth edit desc", "Unexpect OAuth client description");
    Assert.assertEquals(
  ...
  protected ObjectNode createJsonForPrivs(String fullName) {
    return OAuthClients.json(
        fullName, "privClientId", "blah", RestTestConstants.USERID_AUTOTEST, "default");
  }

  @Override
  protected ObjectNode createJsonForEdit(String fullName) {
    return OAuthClients.json(
        fullName, "oauthEditClientId", "blah", RestTestConstants.USERID_AUTOTEST, "default");
  }
}
