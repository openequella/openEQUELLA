...
public class UserApiTest extends AbstractRestAssuredTest
{
	...
	{
		ObjectNode user = Users.json("SomeNewUser", "Some", "User", "some@user.com", "superSecretPassword");
	...
	{
		ObjectNode user = Users.json("AUserToSearch", "Searchable", "User", "searchable@user.com",
		...
		else
		{
			...
			Assert
				.assertEquals(error.get("error_description").textValue(), "Searching for users is no longer possible");
		}
	...
	private static void assertUser(JsonNode actualUser, JsonNode expectedUser)
	{
		...
		assertEquals(actualUser.get("emailAddress").asText(), expectedUser.get("emailAddress").asText());
