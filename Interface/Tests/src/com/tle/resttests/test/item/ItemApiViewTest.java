...
public class ItemApiViewTest extends AbstractItemApiTest {
  ...
  public void testFileList() throws IOException {
    ...
    final ArrayNode fileList =
        (ArrayNode) getItemSubResource(ITEM_UUID, 1, "file", getToken()).withArray("files");
    ...
    assertFile(
        fileNodes.get(conditionsIndex),
        "The vile vendor go figure.zip/ConditionsOfUse.html",
        12926,
        "\"9f8e81b1ef4c8e0b5bdc3438cd50cf0e\"");
    assertFile(
        fileNodes.get(navTopIndex),
        "The vile vendor go figure.zip/images/nav_top.png",
        331,
        "\"f691a28829c39b90bc0eca49f5fef98e\"");
    assertFile(
        fileNodes.get(avatarIndex), "avatar.png", 12627, "\"5a4e69eeae86aa557c4a27d52257b757\"");
    assertFile(
        fileNodes.get(resttodoIndex),
        "resttodo.zip/resttodo.txt",
        890,
  ...
  private void assertHistory(JsonNode history) {
    ...
    asserter.assertHistory(
        history.get(6), USERID_AUTOTEST, "2012-03-04T18:03:24.769-06:00", "edit", "live");
  ...
  private void assertWebPageAttachment(JsonNode webAttachment) {
    ...
    assertEquals(
        webAttachment.get("filename").textValue(),
  ...
  private void assertFlickrAttachment(JsonNode flickrAttachment) {
    ...
    assertEquals(
        flickrAttachment.get("viewUrl").textValue(),
        "http://flickr.com/photos/19017538@N00/277527331");
    assertEquals(
        flickrAttachment.get("thumbUrl").textValue(),
    ...
    assertEquals(
        flickrAttachment.get("mediumUrl").textValue(),
  ...
  private void assertITunesU(JsonNode itunesAttachment) {
    asserter.assertAttachmentBasics(
        ...
        "SGA Roundup 01-31-11");
    assertEquals(
        itunesAttachment.get("playUrl").textValue(),
  ...
  private void assertPackageResource(JsonNode pkgresAttachment) {
    asserter.assertAttachmentBasics(
        ...
        "index.html");
    assertEquals(
        pkgresAttachment.get("filename").textValue(), "The vile vendor go figure.zip/index.html");
