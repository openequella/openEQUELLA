...
public class FacetsApiTest extends AbstractEntityCreatorTest {
  ...
  public void twoDeep() throws Exception {
    List<FacetResult> bookClasses =
        doFacetSearch("/item/info/book_class,/item/store/price", null, false, true);
  ...
  public void branchLevel() throws Exception {
    ...
    List<FacetResult> results =
        doFacetSearch(
            "/item/author/full_name,/item/author/pets/pet/species,/item/author/pets/pet/name",
            null,
            false,
            true);
  ...
  public void nestedIndexingTest() throws Exception {
    ...
    ObjectNode schema =
        Schemas.json(context.getFullName("non nested schema"), "/item/name", "/item/description");
    ...
    String nestedCollection =
        collections.getId(collections.create(CollectionJson.json("nested", nestedUuid, null)));
    String nonNestedCollection =
        collections.getId(
            collections.create(CollectionJson.json("not nested", nonNestedUuid, null)));
    ...
    ItemId notNested =
        items.getId(items.create(Items.jsonXml(nonNestedCollection, xml.toString()), 45));

    searches.waitForIndex(nested, ITEM_NESTED);
    searches.waitForIndex(notNested, ITEM_NON_NESTED);

    List<FacetResult> nonNestedResult =
        doFacetSearch(
            "/item/nestornot/field1,/item/nestornot/field2",
            null,
            false,
            true,
            "\"" + ITEM_NON_NESTED + "\"");
    ...
    nestedResult =
        doFacetSearch(
            "/item/nestornot/field1,/item/nestornot/field2", "/item/nestornot", false, true, null);
  }

  public List<FacetResult> doFacetSearch(String nodes, String nest, boolean getItems, boolean auth)
      throws Exception {
    return doFacetSearch(nodes, nest, getItems, auth, null);
  }

  public List<FacetResult> doFacetSearch(
      String nodes, String nest, boolean getItems, boolean auth, String query) throws Exception {
