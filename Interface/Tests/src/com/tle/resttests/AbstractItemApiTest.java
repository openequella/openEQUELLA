...
public abstract class AbstractItemApiTest extends AbstractRestApiTest {
  ...
  protected String[] copyStaging(ItemId itemId, String token) throws IOException {
    HttpResponse stagingResponse =
        execute(
            new HttpPost(
                appendQueryString(
                    context.getBaseUrl() + "api/item/copy",
                    queryString(
                        "uuid",
                        itemId.getUuid(),
                        "version",
    ...
    ObjectNode stagingJson =
        (ObjectNode) getEntity(stagingResponse.getLastHeader("Location").getValue(), token);
  ...
  protected String[] createStaging(String token) throws IOException {
    HttpResponse stagingResponse =
        execute(new HttpPost(context.getBaseUrl() + "api/staging"), true, token);
    assertResponse(stagingResponse, 201, "201 not returned from staging creation");
    ObjectNode stagingJson =
        (ObjectNode) getEntity(stagingResponse.getLastHeader("Location").getValue(), token);
    String stagingUuid = stagingJson.get("uuid").asText();
  ...
  protected void uploadFile(String stagingDirUrl, String filename, URL resource, Object... params)
      throws IOException {
  ...
  protected HttpResponse taskAction(
      ItemId itemId, String taskId, String action, String token, Object... paramNameValues)
      throws ClientProtocolException, IOException {
    HttpPost request =
        new HttpPost(
            getItemUri(itemId)
  ...
  protected ObjectNode editItem(ObjectNode item, String token, Object... paramNameValues)
    ...
    assertResponse(
        putItem(itemUri, item.toString(), token, paramNameValues),
        200,
        "Should have been able to edit");
    return getItem(itemUri, null, token);
  }
  ...
  protected HttpResponse putItem(
      ItemId itemId, String json, String token, Object... paramNameValues) throws IOException {
    return putItem(getItemUri(itemId), json, token, paramNameValues);
  }

  protected HttpResponse putItem(
      String itemUri, String json, String token, Object... paramNameValues) throws IOException {
  ...
  protected ObjectNode postItemError(
      String json, String token, int responseCode, Object... paramNameValues) throws Exception {
    final HttpPost request =
        new HttpPost(
            appendQueryString(context.getBaseUrl() + "api/item", queryString(paramNameValues)));
    final StringEntity ent = new StringEntity(json, "UTF-8");
    ent.setContentType("application/json");
    request.setEntity(ent);
    HttpResponse response = execute(request, false, token);
  ...
  protected void waitForIndex(final String itemName, final String uuid, boolean all)
      throws IOException {
  ...
  protected JsonNode basicSearch(String query, String token, boolean showAll) throws Exception {
    ...
    HttpGet get =
        new HttpGet(context.getBaseUrl() + "api/search?showall=" + showAll + "&" + paramString);
  ...
  protected void waitForIndex(
      final Collection<String> itemNames, final Collection<String> uuids, final boolean all)
      throws IOException {
              ...
              HttpGet get =
                  new HttpGet(
                      context.getBaseUrl()
                          ...
                          + String.valueOf(all)
                          + "&length=50&q="
                          + URLUtils.urlEncode(combinedNames));
              HttpResponse response = execute(get, false, token);
