...
public class BlackboardUploadWebServicePage extends AbstractPage<BlackboardUploadWebServicePage> {
  private static String BB9_SERVICE_URL =
      "http://dev-builds.equella.com/job/EQUELLA-Build/lastSuccessfulBuild/artifact/Source/Integration/Blackboard/bb91/product/webservice.jar";
  ...
  public BlackboardWebServicesListPage uploadLatest() {
    try {
      ...
      throw new SkipException(
          "An error occurred downloading the webservice " + e.getLocalizedMessage());
    }
    ...
    waiter.until(
        new ExpectedCondition<Boolean>() {
          ...
          public Boolean apply(WebDriver d) {
            BlackboardWebServicesListPage listPage =
                new BlackboardWebServicesListPage(context).get();
  ...
  private File downloadFile(File file, String url) throws Exception {
    ...
    httpGet.addHeader(
        BasicScheme.authenticate(
            new UsernamePasswordCredentials("autotest", "cb7967ae9be763b30aeef126c775128a"),
            "UTF-8",
            false));
