...
public class CreateSearchFilterPage extends AbstractPage<CreateSearchFilterPage>
{
	...
	{
		save.click();
		waiter.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class, 'ctrlinvalid')]")));
	}
	...
	{
		return isPresent(By
			.xpath("//div[contains(@class, 'ctrlinvalid')]/input[@id='esf_name']"));
	}

	public boolean mimeTypeValidationExists()
	{
		return isPresent(By
			.xpath("//div[contains(@class, 'ctrlinvalid')]/div[contains(@class, 'mimeTypesContainer')]"));
	}
}