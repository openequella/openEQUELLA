...
{
	public static final String SELECTION_SESSIONS_HEADER = "Quick contribute"; // original property key: quickcontribute.title
	public static final String VERSION_VIEW_OPTIONS_PREFIX = "_versionViewOptions"; // individual ids: _versionViewOptions_0, etc 
	...
	/**
	 * getSelectableHyperlinks initiates a clickOn() but leaves the drop down open,
	 * so we close the dropdown again before returning.
	 * @return
	 */
	...
	/**
	 * We allow for the possibility that there is no checked view option selected, but
	 * there cannot be more than one.
	 * @return
	 */
	private WebElement getCheckedViewOption()
	{
		List<WebElement> allChecked = driver.findElements(By.xpath(".//input[@name='" + VERSION_VIEW_OPTIONS_PREFIX
			+ "' and @checked='checked']"));
		int allCheckedSize = allChecked.size();
		Assert.assertTrue(allCheckedSize == 0 || allCheckedSize == 1, "Expected either one or none checked, but there are "
			+ allCheckedSize + " View Options checked");
		if( allCheckedSize == 1 )
