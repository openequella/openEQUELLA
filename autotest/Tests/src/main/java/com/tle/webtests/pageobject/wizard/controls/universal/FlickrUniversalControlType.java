...
public class FlickrUniversalControlType
    extends AbstractUniversalControlType<FlickrUniversalControlType> {
  ...
  public WebElement performSearchGetOrdinalResult(String search, SearchType type, int index) {
    ...
    WebElement resultPage =
        driver.findElement(
            By.id(page.subComponentId(ctrlnum, "dialog_fhflfsr_results_" + (index - 1))));
  ...
  protected EquellaSelect getInstitutionSelector() {
    openFilters();
    WebElement selectedInstitution =
        context.getDriver().findElement(By.xpath("//select[contains(@id,'_fhflfbfi_fins')]"));
  ...
  /**
   * Ensure the text provided is selected in the selector (which will not necessarily involve a
   * click and new search.
   *
  ...
  private WaitingPageObject<ItemListPage> resultsUpdateWaiter() {
    ...
    return ExpectWaiter.waiter(
        ExpectedConditions2.updateOfElementLocated(
            firstChild,
            driver,
            By.xpath(
                "id('searchresults')[div[@class='itemlist'] or ul[contains(@class,'standard')]]")),
        itemListPage);
  }
