...
public class ScreenshotTaker {
  ...
  private static File chromeScreenie(WebDriver driver) {
    ...
    BufferedImage stitchedImage =
        new BufferedImage(totalWidth, totalHeight, BufferedImage.TYPE_3BYTE_BGR);
    ...
    for (Rectangle cr : rectangles) {
      ...
      if (pr != null) {
        ...
        // Scroll
        ((JavascriptExecutor) driver)
            .executeScript(MessageFormat.format("window.scrollBy({0}, {1})", xDiff, yDiff));
        try {
          Thread.sleep(200);
      ...
      try {
        ...
        Rectangle sr =
            new Rectangle(
                viewportWidth - cr.width, viewportHeight - cr.height, cr.width, cr.height);
        ...
        // System.out.println(MessageFormat.format("cr  x:{0} y:{1} x2:{2} y2:{3}, sr x:{4} y:{5}
        // x2:{6} y2:{7}",
