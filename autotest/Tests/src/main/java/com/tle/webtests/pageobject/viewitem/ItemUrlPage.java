...
public class ItemUrlPage extends AbstractPage<ItemUrlPage> {
  ...
  public WebElement getFolderLink(String folderName) {
    try {
      return getContext()
          .getDriver()
          .findElement(By.xpath("//a[@href=" + quoteXPath(encode(folderName) + "/~") + "]"));
  ...
  public WebElement getFileLink(String fileName) {
    try {
      return getContext()
          .getDriver()
          .findElement(By.xpath("//a[@href=" + quoteXPath(encode(fileName)) + "]"));
    } catch (NoSuchElementException nse) {
      return null;
    }
  }

  private static final Set<String> ILLEGAL_FILENAMES =
      new HashSet<String>(
          Arrays.asList(
