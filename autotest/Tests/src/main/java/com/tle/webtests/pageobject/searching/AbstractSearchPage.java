...
public abstract class AbstractSearchPage<T extends AbstractSearchPage<T, RL, SR>, RL extends AbstractResultList<RL, SR>, SR extends SearchResult<SR>>
	extends
		AbstractPage<T>
{
	...
	public final static String SORT_BUTTON_ID = "sra_sort";
	public final static String SHARE_BUTTON_ID = "sra_share";
	public final static String FILTER_BUTTON_ID = "sra_filter";
	public final static String FILTER_BOX_COLLECTION_ID = "in";
	public final static String FILTER_RUN_SEARCH_CONTROL_ID = "fbakw_s";
	...
	/**
	 * @param controlId the id of the inner control, to override the default of
	 *            "sort", so we can ask for "rs" (reverse sort checkbox) for
	 *            example.
	 * @return
	 */
	...
	/**
	 * @param controlId the id of the inner control, to override the default of
	 *            "sort", so we can ask for "in" (institution) or "dr" (date
	 *            range) for example.
	 * @return
	 */
	...
	{
		EquellaSelect resourceOptions = new EquellaSelect(context, getFilterControl(FILTER_BOX_COLLECTION_ID));
	...
	{
		WaitingPageObject<RL> waiter = resultsPageObject.getUpdateWaiter();
		driver.findElement(By.xpath("id('page')/ul/li/a[text()=" + quoteXPath(String.valueOf(page)) + "]")).click();
		return waitForResultsReload(waiter);
	}
	...
	public T setDateFilterBefore(String date) throws ParseException
	{
		...
		openFilters();
		new FilterByDateSectionPage(context, resultsPageObject).get().setDateFilter("BEFORE", new Calendar[]{d1});
		return get();
	}
