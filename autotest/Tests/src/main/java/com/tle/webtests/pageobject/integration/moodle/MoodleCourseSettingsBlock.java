...
public class MoodleCourseSettingsBlock extends MoodleBasePage<MoodleCourseSettingsBlock>
{
	...
	public MoodleCourseSettingsBlock(PageContext context)
	{
		super(context);
		settingsDiv = By
			.xpath("//div[contains(@class, 'block_settings') and ./div[@class='header' and .//h2[text()='Administration']]]");
	...
	{
		return "//span[contains(@class, 'backup_stage_current') and text() = " + quoteXPath(string) + "]";
	}
	...
	public <T extends PageObject> T restoreCourse(String filename, WaitingPageObject<T> page)
	{
		...
		WebElement row = backupTable.findElement(By.xpath(".//tr[td[text()=" + quoteXPath(filename) + "]]"));
		row.findElement(By.linkText("Restore")).click();
		waitForStage("1. Confirm");
		clickButton("Continue");
		waitForStage("2. Destination");
		WebElement form = driver.findElement(By.xpath("//form[.//h2[text() = 'Restore into this course']]"));
	...
	{
		driver.findElement(settingsDiv).findElement(By.xpath(".//a[text() = " + quoteXPath(string) + "]")).click();
	}
