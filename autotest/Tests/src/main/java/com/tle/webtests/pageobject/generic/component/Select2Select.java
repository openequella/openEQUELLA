...
public class Select2Select extends AbstractPage<Select2Select> {
  ...
  public Select2Select(PageContext context, WebElement select) {
    super(
        context,
        By.xpath("following-sibling::span[contains(@class, 'select2-container--default')]"));
    origSelect = select;
  ...
  public void checkLoaded() throws Error {
    ...
    textDiv =
        container.findElement(By.xpath(".//span[contains(@class, 'select2-selection__rendered')]"));
  }

  public void selectByVisibleText(String name) {
    WebElement dropDiv = clickOn();
    WebElement entry = dropDiv.findElement(By.xpath(".//li[text()=" + quoteXPath(name) + "]"));
  ...
  public WebElement clickOn() {
    ...
    return driver.findElement(
        By.xpath("/html/body/span/span[contains(@class, 'select2-dropdown')]"));
  }
