...
public class EditMetadataDialog extends AbstractPage<EditMetadataDialog> {
  ...
  public EditMetadataDialog addModification() {
    WaitingPageObject<EditMetadataDialog> ajaxUpdate =
        ajaxUpdateExpect(ajaxDiv, schemaSelectionDiv);
    addMod.click();
  ...
  public EditMetadataDialog selectNode(String node) {
    ...
    WebElement findElement =
        waitForElement(By.xpath("//li/div[text() = " + quoteXPath(node) + "]/a"));
    findElement.click();
    return ajaxUpdate.get();
  }
  ...
  private void selectAction(String action) {
    ...
    if (!actionSel.getSelectedValue().equals(action)) {
      WaitingPageObject<EditMetadataDialog> ajaxUpdate =
          ajaxUpdateExpect(actionAjaxDiv, firstActionSetting);
  ...
  public EditMetadataDialog changeActionOrder(int index, boolean moveUp) {
    ...
    modificationTable
        .findElement(By.xpath("tbody/tr[" + index + "]/td[3]/a[" + buttonPos + "]"))
        .click();
    return ajaxUpdate.get();
  }
