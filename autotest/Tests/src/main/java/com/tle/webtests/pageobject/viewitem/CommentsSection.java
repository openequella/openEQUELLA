...
public class CommentsSection extends AbstractPage<CommentsSection>
{
	...
	public void addComment(String comment, int rating, boolean doGet)
	{
		...
		{
			driver.findElement(
				By.xpath("//div[@class='rate-stars']//a[text()=" + quoteXPath(Integer.toString(rating)) + "]")).click();
		}
		addButton.click();
	...
	{
		driver.findElement(By.xpath(xpathForComment(comment) + "/div[@class='comment-delete']/a")).click();
		acceptConfirmation();
		...
		waiter.until(new ExpectedCondition<Boolean>()
		{
			...
			{
				return Boolean.valueOf(!isPresent(By
					.xpath(xpathForComment(comment) + "/div[@class='comment-delete']/a")));
			}
		});
	}
	...
	{
		WebElement starDiv = driver.findElement(By.xpath(xpathForComment(comment)
			+ "//div[contains(@class, 'star-rating-static-')]"));
		return Integer.parseInt(starDiv.getAttribute("class").substring("star-rating-static-".length()));

	}

	public String getCommentOwner(String comment)
	{
		return driver.findElement(By.xpath(xpathForComment(comment) + "/div[@class='comment-username']/span"))
			.getText();
	}
