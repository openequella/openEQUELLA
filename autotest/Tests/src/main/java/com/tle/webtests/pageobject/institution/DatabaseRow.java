...
public class DatabaseRow extends AbstractPage<DatabaseRow>
{
	...
	{
		longWaiter.until(new Function<WebDriver, Boolean>()
		{
			@Override
			public Boolean apply(WebDriver driver)
			{
	...
	private void assertStatusStartsWith(String statusCheck)
	{
		...
		{
			throw new AssertionError("Wrong status expected '" + statusCheck + "' but found '" + status + "'");
		}
	}
	...
	{
		String status = getStatus();
		Assert.assertEquals(status, statusCheck, "Wrong status expected '" + statusCheck + "' but found '" + status + "'");
	}
