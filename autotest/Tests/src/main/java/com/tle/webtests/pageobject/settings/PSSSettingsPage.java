...
public class PSSSettingsPage extends AbstractPage<PSSSettingsPage>
{
	...
	{
		return isPresent(By.xpath("//p[@class='ctrlinvalidmessage' and text()=" + quoteXPath(errorText) + "]"));
	}

	public boolean hasSuccess(String successText)
	{
		return isPresent(By.xpath("//div[@class='success']/span[text()=" + quoteXPath(successText) + "]"));
	}

	public PSSSettingsPage testFailValidation(String errorText)
	{
		WaitingPageObject<PSSSettingsPage> waiter;
		waiter = visibilityWaiter(driver,
	...
	{
		WaitingPageObject<PSSSettingsPage> ajaxUpdate = ajaxUpdateExpect(connectionStatusAjaxDiv, successDiv);
		testButton.click();
	...
	{
		WaitingPageObject<PSSSettingsPage> waiter;
		waiter = visibilityWaiter(
			driver,
			By.xpath("//p[@class='ctrlinvalidmessage' and text()='You must successfully test the Pearson SCORM Services settings before saving.']"));
		saveButton.click();
		return waiter.get();
	}
