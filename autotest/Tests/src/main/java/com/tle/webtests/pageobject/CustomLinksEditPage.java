...
public class CustomLinksEditPage extends AbstractPage<CustomLinksEditPage> {
  ...
  public CustomLinksPage save() {
    return save(
        customLinksPage.getNewExpectation(
            getTitleField().getCurrentString(), urlField.getAttribute("value")));
  }
  ...
  public void uploadIcon(URL icon, boolean replace) {
    ExpectedCondition<?> iconUpdate =
        replace
            ? ExpectedConditions.and(
                ExpectedConditions2.stalenessOrNonPresenceOf(getCurrentIcon()), getIconVisible())
            : getIconVisible();
