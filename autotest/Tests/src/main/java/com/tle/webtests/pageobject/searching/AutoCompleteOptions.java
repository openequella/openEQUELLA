...
public class AutoCompleteOptions extends AbstractPage<AutoCompleteOptions>
{
	...
	{
		waiter.until(ExpectedConditions2.elementAttributeToContain(qs.getLoadedElement(), "class", "ui-autocomplete"));
	...
	{
		String realTextEscaped = (String) ((JavascriptExecutor) driver).executeScript(
			"return $(arguments[0]).data(\"real\");", promptField);
		qs.typeKeys(keys);
	...
	private void listWait(String expected)
	{
		...
		{
			by = By.xpath("li/a[normalize-space(concat(descendant::text(), text()))=" + quoteXPath(expected.trim())
				+ "]");
		}
		acWaiter.until(new ExpectedCondition<Boolean>()
		{
			...
			public Boolean apply(WebDriver driver)
			{
				...
				if( Check.isEmpty(lastQuery) )
				{
					...
					((JavascriptExecutor) driver).executeScript("$(arguments[0]).keydown();", qs.getLoadedElement());
				}
				else
				{
