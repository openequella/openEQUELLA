...
public class RecipientSelectorPage extends AbstractPage<RecipientSelectorPage>
{
	...
	public void chooseGroupingOption(String groupingId, String option)
	{
		...
		rightSide.findElement(
			By.xpath("//li[@id=" + quoteXPath(groupingId) + "]/div/ul/li/a[text()=" + quoteXPath(option) + "]"))
			.click();
	}
	...
	{
		List<WebElement> findElements = driver.findElements(By.xpath("//ul[@class='search-result']/li"));
	...
	protected List<WebElement> getGroupElements(WebElement group)
	{
		...
		final List<WebElement> selectedGroupElements = groupParent.findElements(By
			.xpath("./li[div/@class='expression-text']"));
	...
	{
		int highestId = 0;
		final List<WebElement> findElements = rightSide.findElements(By.xpath("//li[starts-with(@id, 'grouping')]"));
	...
	{
		driver.findElement(By.xpath("//div[@id='tab2']/div/ul/li/label[text()=" + quoteXPath(option) + "]")).click();
	...
	{
		WebElement element = rightSide.findElement(By
			.xpath("//ul/li/div[text()=" + quoteXPath(selectedName) + "]/../a"));
	...
	{
		WebElement result = resultDiv.findElement(By.xpath("//ul[@class='search-result']/li[" + index + "]"));
		WebElement plusIcon = result.findElement(By.xpath("div[@class='add-user']/a"));
		plusIcon.click();
		waitForElement(By
			.xpath("//ul[contains(@class, 'grouping')]/li/div[@class='expression-text' and contains(text(), "
	...
	{
		return new ArrayList<>(Lists.transform(resultElements, new Function<WebElement, String>()
		{
			@Override
	...
	{
		return new ArrayList<>(Lists.transform(groupElements, new Function<WebElement, String>()
		{
			@Override
			public String apply(WebElement li)
			{
				WebElement div = li.findElement(By.xpath("./div[contains(@class, 'expression-text')]"));
				return div.getText();
			}
		}));
	}
	...
	{
		WaitingPageObject<T> disappearWaiter = ExpectWaiter.waiter(removalCondition(okButton), returnPage);
		okButton.click();
	...
	{
		return resultDiv.findElement(By.xpath("//ul[@class='search-result']/li[" + index + "]/div/span")).getText();
	}

	public String getGroups(int index)
	{
		return resultDiv.findElement(By.xpath("//ul[@class='search-result']/li[" + index + "]/label")).getText();
	}
