...
public class UniquenessTest extends AbstractWizardControlsTest {
  ...
  private final String DUPLICATE_CHECK_ITEM_DIFFERENT_NAME = "uniquenessTest - Duplicate Check different ";
  private final String DUPLICATE_WARNING_LINK_TEXT = "Click here to view existing items that contain duplicates";
  ...
  public void contributeSecond() {
	...
	private boolean findWarningLink(WizardPageTab wizardPage, String controlDivId,boolean isWarningDisplayed){
		By linkTextXpath = By.xpath("//div[@id='"+controlDivId+"']//a[text()=" + quoteXPath(DUPLICATE_WARNING_LINK_TEXT) + "]");
		if(isWarningDisplayed) {
			WebElement warningLink = wizardPage.getWaiter().until(ExpectedConditions.elementToBeClickable(linkTextXpath));
			// Check if the warning link text is correct
			Assert.assertEquals(warningLink.getText(),"Click here to view existing items that contain duplicates");
			warningLink.click();
			return true;
		}
		else {
      ...
      By duplicateContentXpath =
          By.xpath(
              "//div[starts-with(@class,'duplicate')]//*[contains(text(),"
                  ...
                  + ")]");
			WebElement duplicateContent = duplicateDataPage.getWaiter().until(ExpectedConditions.visibilityOfElementLocated(duplicateContentXpath));
			Assert.assertEquals(duplicateContent.getText(),"'"+ duplicateContentText + "' is also used by:");
		}

		By duplicatesFoundXpath = By.xpath("//a[text()='" + DUPLICATE_CHECK_ITEM_NAME + "']");
		WebElement duplicateItem = duplicateDataPage.getWaiter().until(ExpectedConditions.visibilityOfElementLocated(duplicatesFoundXpath));
