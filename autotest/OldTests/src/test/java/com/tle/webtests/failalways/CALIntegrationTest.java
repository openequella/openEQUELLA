...
public class CALIntegrationTest extends AbstractCALTest {
  ...
  public void contributeAndSelect() {
    ...
    ContributePage contributePage =
        DynamicUrlPage.load(
            integTester().getSignonUrl("contribute", "caladmin", "1234", "")
                + "&attachmentonly=true",
    ...
    CALActivatePage<SelectionCheckoutPage> activatePage =
        new CALActivatePage<SelectionCheckoutPage>(context, new SelectionCheckoutPage(context));
    SelectionCheckoutPage checkoutPage = activatePage.activate();
    IntegrationTesterReturnPage returnPage =
        checkoutPage.returnSelection(new IntegrationTesterReturnPage(context));
    CALAgreementPage agreementPage =
        DynamicUrlPage.load(returnPage.getReturnedUrl(), new CALAgreementPage(context));
  ...
  public void selectAttachment() {
    ...
    CALSummaryPage summaryPage =
        selectionSession.homeSearch('"' + bookFullname + '"').viewFromTitle(bookFullname).cal();
    CALActivatePage<SelectionCheckoutPage> activatePage =
        summaryPage.activate(1, ATTACH1_FILENAME, new SelectionCheckoutPage(context));
  ...
  private void verifyReturned(CALActivatePage<SelectionCheckoutPage> activatePage) {
    ...
    IntegrationTesterReturnPage returnPage =
        checkout.returnSelection(new IntegrationTesterReturnPage(context));
    CALAgreementPage agreementPage =
        DynamicUrlPage.load(returnPage.getReturnedUrl(), new CALAgreementPage(context));
  ...
  public void testAddActivatedPortion() {
    ...
    summaryPage =
        selectionSession.homeSearch('"' + bookFullname + '"').viewFromTitle(bookFullname).cal();
    assertTrue(
        summaryPage.canAdd(1, ATTACH1_FILENAME), "Course should have activation but cannot add");
    summaryPage.add(1, ATTACH1_FILENAME);
    IntegrationTesterReturnPage integReturn =
        selectionSession.finish().returnSelection(new IntegrationTesterReturnPage(context));
