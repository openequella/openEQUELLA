...
public class DynaCollectionApiTest extends AbstractRestApiTest
{
	...
	/**
	 * Note this test assumes the pre-existence of a Dynamic Collection with
	 * exactly this name, and it applicability restricted to search(Usage), with
	 * 4 items within its definition, 2 of those items with date_last_modified
	 * being before march 2013
	 */
	private static final String OAUTH_CLIENT_ID = "DynaCollectionApiTestClient";
	/**
	 * Test assumes existence of manual virtual value dyna collection with this
	 * UUID and 'giant' as one of its virtual values. (Also required by
	 * SearchApiTest)
	 */
	public static final String KNOWN_MANUAL_VIRTUAL_DYNACOLL_UUID = "637314db-75b6-4d8e-81ea-41a1c64a9e9a";
	public static final String GIANT = "giant";
	private static final String KNOWN_MANUAL_VIRTUAL_VALUE_GIANT = SearchApiTest.SEARCH_API_TEST + "ClientDynamic ("
		+ GIANT + ")"; // also "average", "miniature"
	...
	{
		String token = requestToken(OAUTH_CLIENT_ID);
		URI uri = new URI(context.getBaseUrl() + API_DYNACOLL_PATH + '/' + KNOWN_MANUAL_VIRTUAL_DYNACOLL_UUID);
		...
		assertEquals(expandedVirualisedName, KNOWN_MANUAL_VIRTUAL_VALUE_GIANT, "Bad result:" + result.toString());
