...
public class SearchApiTest extends AbstractRestApiTest {
  ...
  public Object[][] getSortData() {
    return new Object[][] {
      {"name", new Integer[] {0, 1, 2, 3, 4}},
      {"modified", new Integer[] {3, 4, 2, 1, 0}},
      {"rating", new Integer[] {4, 0, 2, 1, 3}},
    };
  }

  private JsonNode doSearch(String info, String query, Map<?, ?> otherParams, String token)
      throws Exception {
  ...
  public void testOrdering(String order, Integer[] itemIndexes) throws Exception {
    ...
    JsonNode resultsNode =
        doSearch(
            BASIC,
            SEARCH_API_TEST,
            ImmutableMap.of("order", order, "collections", "b28f1ffe-2008-4f5e-d559-83c8acd79316"),
            token);
  ...
  public void testStartAndLength() throws Exception {
    ...
    JsonNode resultsNode =
        doSearch(
            BASIC,
            SEARCH_API_TEST,
            ImmutableMap.of("order", "name", "start", 1, "length", 2),
            token);
  ...
  public void testShowall() throws Exception {
    ...
    JsonNode resultsNode =
        doSearch(
            BASIC,
            "\"" + SEARCH_API_TEST + " - Other collection\"",
    ...
    JsonNode itemsNode = resultsNode.get("results");
    asserter.assertBasic(
        (ObjectNode) itemsNode.get(0), SEARCH_API_TEST + " - Other collection", null);
    asserter.assertBasic(
        (ObjectNode) itemsNode.get(1), SEARCH_API_TEST + " - Other collection draft", null);
  }
    ...
    JsonNode resultsNode =
        doSearch(
            ...
            ImmutableMap.of(
                ...
                "true",
                "where",
                "/xml/item/name = '" + SEARCH_API_TEST + " - Other collection'"),
            token);
  ...
  /**
   * The pre-defined dynamic collection 'SearchApiTest' is expected, with 4 items, 2 modified before
   * 15th March 2013, and 2 after.
   *
   * @throws Exception
   */
  @Test
  public void testSearchDynaCollContents() throws Exception {
    ...
    String dynaCollUuid =
        confirmDynaCollectionByName(dynaCollQueryUri.toString(), OAUTH_CLIENT_ID, token);
    ...
    otherParams.clear();
    dynaCollUuid =
        DynaCollectionApiTest.KNOWN_MANUAL_VIRTUAL_DYNACOLL_UUID
            + ':'
            + DynaCollectionApiTest.GIANT;
    otherParams.put("dynacollection", dynaCollUuid);
  ...
  /**
   * The pre-defined dynamic collection 'SearchApiTestDynamic' is expected, 2 items with the virtual
   * value 'giant', none with 'average' and 1 with 'miniature'.
   *
   * @throws Exception
   */
  @Test
  public void testSearchDynaCollContentsWithVirtualValue() throws Exception {
    ...
    String dynaCollUuid =
        confirmDynaCollectionByName(uri.toString(), dynaBaseName + " (giant)", token);
    Map<String, String> otherParams = new HashMap<String, String>();
    otherParams.put("dynacollection", dynaCollUuid);
    ...
    dynaCollUuid =
        confirmDynaCollectionByName(uri.toString(), dynaBaseName + " (miniature)", token);
    otherParams.put("dynacollection", dynaCollUuid);
   ...
   * @throws Exception
   */
  private String confirmDynaCollectionByName(String uriStr, String target, String token)
      throws Exception {
    ...
    assertNotNull(
        dynaCollCompoundId,
        "Test precondition not met: couldn't find dynamic collection named " + target);
