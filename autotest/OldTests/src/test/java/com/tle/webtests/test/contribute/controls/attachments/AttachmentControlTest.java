...
public class AttachmentControlTest extends AbstractCleanupTest {
  ...
  public void url() {
    ...
    UrlAttachmentEditPage edit =
        universalControl.editResource(
            new UrlUniversalControlType(universalControl), "http://www.google.com/");
  ...
  public void selectResource() {
    ...
    WaitingPageObject<UniversalControl> newAttachWaiter =
        universalControl.attachNameWaiter(itemName, false);
    ...
    SummaryPage viewFromTitle =
        selectionSession.homeExactSearch(itemName).viewFromTitle(itemName, 1);
    ...
    WaitingPageObject<UniversalControl> selectedPageWaiter =
        universalControl.attachNameWaiter("page.html", false);
    selectionSession = resourceDialog.getSelectionSession();

    SelectionCheckoutPage checkout =
        selectionSession
            .homeExactSearch(itemName)
            .viewFromTitle(itemName, 1)
            .attachments()
            .selectSingleAttachment("page.html");
    checkout.returnSelection(selectedPageWaiter);

    GenericAttachmentEditPage editResource =
        wizard
            .universalControl(8)
            .editResource(new ResourceUniversalControlType(universalControl), "page.html");
    editResource.setDisplayName("A linked page").save();

    item = wizard.saveNoConfirm();

    assertTrue(
        item.attachments().attachmentExists("A linked page"),
        "Attachment has the wrong display name, should be 'A linked page'");
    assertTrue(
        item.attachments()
            .viewAttachment("A linked page", new VerifyableAttachment(context))
            .isVerified());
  ...
  public void selectionCreateAndSelect() {
    ...
    UniversalControl universalControl = wizard.universalControl(8);
    WaitingPageObject<UniversalControl> outerContribAttach =
        universalControl.attachNameWaiter(attachment, false);
    ResourceUniversalControlType resourceDialog =
        universalControl.addDefaultResource(new ResourceUniversalControlType(universalControl));
    ...
    WaitingPageObject<UniversalControl> innerContribAttach =
        universalControl.attachNameWaiter(attachment, false);
    ...
    innerSearch
        .homeExactSearch(selectItem)
        .viewFromTitle(selectItem)
        .attachments()
    ...
    assertTrue(
        attachments.viewAttachment(attachment, new VerifyableAttachment(context)).isVerified());
