...
public class ItemExportTest extends AbstractCleanupAutoTest {
  ...
  public void metsExportTest() throws Exception {
    ...
    DownloadFilePage downloadFilePage =
        new DownloadFilePage(context, IMS_PACKAGE_ITEM.replace(" ", "_") + "_METS.xml", 2600);
    ...
    downloadFilePage =
        new DownloadFilePage(context, IMS_PACKAGE_ITEM.replace(" ", "_") + "_METS.zip", 240000);
    downloadFilePage.deleteFile();
    ...
    assertEquals(
        wizard.universalControl(2).resourceCount(), 1, "There should only be one attachment!");
    ...
    assertTrue(
        packageViewer.selectedAttachmentContainsText(
  ...
  public void imsExportTest() throws Exception {
    ...
    DownloadFilePage downloadFilePage =
        new DownloadFilePage(context, IMS_PACKAGE_ZIP, IMS_PACKAGE_MD5);
    downloadFilePage.deleteFile();
  ...
  public void newItemImsExportTest() throws Exception {
    ...
    assertTrue(attachments.attachmentExists("Child 2"));
    assertTrue(
        attachments.viewAttachment("Child 2", new VerifyableAttachment(context)).isVerified());
