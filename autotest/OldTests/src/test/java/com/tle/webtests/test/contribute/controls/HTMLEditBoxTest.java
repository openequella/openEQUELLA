...
public class HTMLEditBoxTest extends AbstractWizardControlsTest
{
	...
	public void lazyLoadTest() throws Exception
	{
		...
		final String HTML = "<p>'...it wasn't all wung!'</p> <p>- Jolse Maginnis (after being accused of winging his SOAP presentation with Clemens)</p>";
		final String DISABLED_HTML = "'...it wasn't all wung!'\n- Jolse Maginnis (after being accused of winging his SOAP presentation with Clemens)";
		...
		Assert.assertTrue(lazyHtmlEditBox.hasMispeltWords(), "Expected spellchecker to find misspelt words");
		Assert.assertTrue(lazyHtmlEditBox.confirmMisspeltWord("wung"), "Expected spellchecker to highlight 'wung'");
	...
	private boolean checkItemXml(PropBagEx itemXml, String content, String html)
	{
		...
		String lazyHtmlEditBox = itemXml.getNode("item/controls/html/htmleditboxondemand").replace("\n", " ").trim();
