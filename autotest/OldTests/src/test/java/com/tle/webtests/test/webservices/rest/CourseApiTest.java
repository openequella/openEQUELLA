...
public class CourseApiTest extends AbstractRestApiTest
{
	...
	/**
	 * Using the OAUTH_CLIENT_ID as recognisable (and hence disposable)
	 * temporary course name prefix
	 */
	...
	public void testCreateCourse() throws Exception
	{
		...
		JsonNode newObj = createAndVerifyCourse(token, COURSE_CODE_RECOGNISABLE, "delete me if you see me", uri);
	...
	public void testCannotRePOSTSameCode() throws Exception
	{
		...
		JsonNode firstObj = createAndVerifyCourse(token, COURSE_CODE_RECOGNISABLE, "delete me if you see me", targetUri);

		assertTrue(firstObj != null && firstObj.get("code").asText().equals(COURSE_CODE_RECOGNISABLE));

		String jsonStr = buildJsonCourse(COURSE_CODE_RECOGNISABLE,
			"We expect it's a folorn hope for this object to be created", "2010-03-31", "2010-12-25", 31,
			KNOWN_USER_UUID, null);
		...
		assertTrue(postResponseCode == 400 || postResponseCode == 500, "unexpected (" + postResponseCode
			+ ") post response.");
	...
	*/
	private JsonNode createAndVerifyCourse(String token, String code, String name, URI uri) throws Exception
	{
		...
		assertTrue(postResponseCode == 200 || postResponseCode == 201, "nonsuccess (" + postResponseCode
			+ ") in post response.");
	...
	private String buildJsonCourse(String code, String name, String dateFromStr, String dateUntilStr, int students,
		String ownerUuid, String uuid) throws Exception
	{
		String jsonStr = "{ ";
		if( uuid != null )
		{
