...
public class CommentApiTest extends AbstractItemApiTest {
  ...
  private HttpResponse getComment(String uuid, int version, String commentUuid) throws IOException {
    String apiUrl =
        context.getBaseUrl() + "api/item/" + uuid + "/" + version + "/comment/" + commentUuid;

    final HttpGet request = new HttpGet(apiUrl);
    return execute(request, true, getToken());
  }

  private HttpResponse createComment(
      ItemId itemid, String commentText, int commentRating, boolean isAnonymous)
    ...
    String apiUrl =
        context.getBaseUrl()
            ...
            + "/comment";
    return postEntity(
        createCommentJson(commentText, commentRating, isAnonymous).toString(),
        apiUrl,
        getToken(),
        true);
  }

  private HttpResponse deleteComment(ItemId itemid, String uuid)
      throws ClientProtocolException, IOException {
    String apiUrl =
        context.getBaseUrl()
            + "api/item/"
