...
public class MyResourcesWebPageTest extends AbstractCleanupTest {
  ...
  public void addAndEditWebPageItemContent() {
    ...
    ItemListPage results =
        myResourcesPage.authorWebPages(scrapbookItem, pageTitles, pageBodies).results();
    ...
    for (int i = 0; i < newHtmlContent.length; ++i) {
      assertTrue(
          htmlAreaContent.contains(newHtmlContent[i]),
          "Failed to find '" + newHtmlContent[i] + "' in edit area content");
    }
  }
  ...
  public void addAndDeleteAuthoredWebPageItem() {
    ...
    results =
        results
            .getResultForTitle(scrapbookItem, presumedIndex)
            .clickActionConfirmAndRemove("Delete", true, results);
