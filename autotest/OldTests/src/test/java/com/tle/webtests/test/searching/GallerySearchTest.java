...
public class GallerySearchTest extends AbstractCleanupTest
{
	...
	{
		DEFAULT, NONE, CUSTOM_IMAGE, CUSTOM_PDF;
	}
	...
	public void testShowConditions(PrefixedName itemName, boolean image, ThumbOptions thumb, boolean shouldExist)
	{
		logon("admin", "``````");
		...
		assertEquals(shouldExist, search.hasResults(), shouldExist ? "Should have results for this item: " + itemName
	...
	private void testSuppressThumbnails()
	{
		...
		control.editResource(new FileUniversalControlType(control), "veronicas_wall1.jpg").setThubmnailSuppress(true)
			.save();
