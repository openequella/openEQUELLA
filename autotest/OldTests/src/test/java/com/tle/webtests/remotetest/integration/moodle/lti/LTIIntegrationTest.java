...
public class LTIIntegrationTest extends AbstractParallelMoodleTest
{
	...
	private final Map<Integer, String> LTI_OAUTH_MAP = ImmutableMap.<Integer, String> builder()
		.put(25, "b90fa6ea-15fe-4640-939a-b7341fc5a4bc").put(26, "8aeda259-b311-4c45-9e7e-075a1b041559")
		.put(27, "c9edb02e-4ab8-4ce0-9d34-350c86aaa57b").put(28, "a67e8529-89f5-471d-959f-c0f432e78714").build();
	...
	private final Map<Integer, String> QTU_OAUTH_MAP = ImmutableMap.<Integer, String> builder()
		.put(25, "ea71c193-4251-4121-a0a3-5170b874ba1c").put(26, "304ef014-fa0b-4a61-ae9d-923419b5a677")
		.put(27, "de85d31e-a65f-4b5c-9134-b8fb919973d2").put(28, "1ce5becd-a051-4962-a126-092a684daa84").build();
	private static final String QTI_QUIZ_URL = "items/5f63ad95-acc1-42e0-b1d2-81442ea485f9/1/?.vi=qtiTestViewer&attachment.uuid=3b22a99f-90a4-4269-ad3d-f67148394c59";
	...
	public void setupExternalTool()
	{
		...
		extToolPage.addExternalTool(EXTERNAL_TOOL_NAME, context.getBaseUrl(), LTI_OAUTH_ID,
			LTI_OAUTH_MAP.get(getMoodleVersion()), true);
	...
	public void basicLTITest()
	{
		logout();
		// Add link
		MoodleCoursePage mcp = new MoodleLoginPage(context).load().logon("tokenuser", "``````")
			.clickCourse(COURSE_NAME);
		mcp.setEditing(true);

		MoodleAddExternalToolPage aetp = mcp.addExternalTool(2);
		mcp = aetp.setupExternalTool(LTI_LINK_NAME, EXTERNAL_TOOL_NAME, context.getBaseUrl()
			+ "items/21ff03e5-ade5-432d-a938-5c59ab0260b0/1/", COURSE_NAME);

		// Check for link
		Assert.assertTrue(mcp.hasLTIResource(2, LTI_LINK_NAME), "Lti resource not found or too many found");
		...
		mrp.switchToItem(ep);
		Assert.assertEquals(ep.getDetail(), "Bad request: Signature is invalid. Please check key and secret");

	}

	@Test(dependsOnMethods = {"basicLTITest", "doQTIOverLTITest", "openToolViaEquellaTest"}, alwaysRun = true)
