...
{
	public static final String DISCARDABLE_URL = "http://purple-excess.dyndns.ws"; // Larry's sometime personal URL, try :8078
	...
	{
		ShortcutURLsSettingsPage susp = logonToShortcutURLsSettingsPage();
		AddShortcutURLPage dialog = susp.addShortcutTextAndURLFailure("", "", false, "Shortcut cannot be blank.");
		susp = dialog.close();
		susp.load();
		dialog = susp.addShortcutTextAndURLFailure("valid", "", true, "URL cannot be blank.");
		susp = dialog.close();
		susp.load();
		dialog = susp.addShortcutTextAndURLFailure("valid", "This has no chance of being a URL", true,
