...
public class DRMTest extends AbstractSessionTest
{
	...
	public void drmWithNoSummaryAcceptance(String name, boolean allowComp, boolean summary)
	{
		...
		SummaryPage item = new SearchPage(context).load().setSort("rank").exactQuery("Link to " + name).getResult(1)
			.viewSummary();
		PackageViewer pack = item.attachments().viewFullscreen(getAgree()).preview(new PackageViewer(context));
		testItem(pack, name, allowComp);

		logon("AutoTest", "automated");
		item = new SearchPage(context).load().setSort("rank").exactQuery("Link to " + name).getResult(1).viewSummary();
	...
	{
		logon("AutoTest", "automated");
		SummaryPage item = new SearchPage(context).load().setSort("rank").exactQuery("Summary DRM - Link to " + name)
			.getResult(1).viewSummary(getAgree()).preview(new SummaryPage(context));
		PackageViewer pack = item.attachments().viewFullscreen();

		testItem(pack, name, allowComp);

		logon("AutoTest", "automated");
		item = new SearchPage(context).load().setSort("rank").exactQuery("Summary DRM - Link to " + name).getResult(1)
			.viewSummary(getAgree()).preview(item);
	...
	{
		logon("AutoTest", "automated");
		SummaryPage item = new SearchPage(context).load().setSort("rank").exactQuery("Link to Summary of " + name)
		...
		{
			pack = attachments.viewAttachment("Music History - The Beatles", getDialogAgree()).preview(
				new PackageViewer(context));
		}
		else
		{
			pack = attachments.viewAttachment("Music History - The Beatles", new PackageViewer(context));
		}
		assertTrue(pack.selectedAttachmentContainsText("The Beatles were an English rock band, formed in"));
	}

	private void testItem(PackageViewer pack, String name, boolean allowComp)
	{
		assertEquals(!pack.clickAttachment("Music History - The Beatles")
			.selectedAttachmentContainsText("Terms of use"), allowComp, allowComp ? "DRM should not be show"
			: "DRM should be shown");

		assertTrue(pack.clickAttachment("Link page").selectedAttachmentContainsText("Music History - The Beatles"));
		...
		assertTrue(linkedPackage.clickAttachment("The Beatles").selectedAttachmentContainsText(
	...
	private void testItem2(PackageViewer pack, String name, boolean allowComp)
	{
		...
		assertFalse(pack.clickAttachment("Music History - The Beatles").selectedAttachmentContainsText("Terms of use"));
	...
	public void attachmentTest() throws UnsupportedEncodingException
	{
		...
		final String ATTACHMENT_URL = "file/677a4bbc-defc-4dc1-b68e-4e2473b66a6a/1/Biscuit factory complex ratios.zip/index.html";
		final String DIRECT_URL = "items/677a4bbc-defc-4dc1-b68e-4e2473b66a6a/1/?attachment.uuid=a159f050-15fc-4a95-a9f5-63a821a0426a";
		final String DIRECT_PREVIEW_URL = "items/677a4bbc-defc-4dc1-b68e-4e2473b66a6a/1/?attachment.uuid=a159f050-15fc-4a95-a9f5-63a821a0426a&preview=true";
		...
		assertTrue(summaryPage.attachments().attachmentExists("Start: Biscuit factory: complex ratios"));
		// Viewing attachments shows acceptance
		summaryPage.attachments().viewAttachment("Start: Biscuit factory: complex ratios", getDialogAgree())
			.preview(new VerifyableAttachment(context, "Curriculum Corporation, 2006, except where indicated"));
	...
	public void multiImageTest()
	{
		...
		SummaryPage summaryPage = searchPage.results().getResult(1).viewSummary();
		DRMAgreementDialogPage drmDialog = summaryPage.attachments().viewAttachment("cat2.png",
			new DRMAgreementDialogPage(context));
	...
	{
		// DTEC 14600
		final String IMS_DOWNLOAD_URL = "items/677a4bbc-defc-4dc1-b68e-4e2473b66a6a/1/viewims.jsp?viewMethod=download";

		logon("AutoTest", "automated");
		openUrl(IMS_DOWNLOAD_URL);

		DownloadFilePage dlfPage = getAgree().preview(
			new DownloadFilePage(context, "Biscuit factory complex ratios.zip"));
	...
	{
		assertTrue(new ErrorPage(context).get().getSubErrorMessage().toLowerCase().contains(error.toLowerCase()));
	}
