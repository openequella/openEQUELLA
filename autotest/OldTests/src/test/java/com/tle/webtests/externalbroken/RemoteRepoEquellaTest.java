...
public class RemoteRepoEquellaTest extends AbstractCleanupAutoTest {
  ...
  public void testEquellaSearchAndContribute() {
    ...
    RemoteRepoEquellaSearchPage rrEquella =
        rrp.clickRemoteRepository(remoteRepo, new RemoteRepoEquellaSearchPage(context));
    ...
    assertTrue(
        attachmentDetails.contains("Size: 434.38 KB"),
        "Attachment should have a size of 434.38 KB. " + attachmentDetails);

    ImageGalleryPage imgPage =
        attachments.viewAttachment(REMOTE_ATTACHMENT, new ImageGalleryPage(context));
