...
public class ItemApiEditTest extends AbstractItemApiTest
{
	...
	private void createItemFromJsonFile(String json, String token, String stagingUuid) throws IOException
	{
		createItemFromJsonFile(json, token, stagingUuid, 201, false, standardParams());
	}

	private ObjectNode createItemFromJsonFile(String json, String token, String stagingUuid, int responseCode,
		boolean returnError, Map<String, String> nameValues) throws IOException
	{
	...
	private ObjectNode createItemFromJsonObject(ObjectNode newItem, String token, String stagingUuid, int responseCode,
		boolean returnError) throws IOException
	{
	...
	public void testEditAttachments() throws Exception
	{
		...
		// get it again
		urlAttachment = (ObjectNode) getItem(ITEM_UUID, 1, "attachment", token).get("attachments").get(2);
	...
	public void testEditEverything() throws Exception
	{
		...
		try
		{
			assertResponse(response, 200, "Save should work, but didn't: " + superSerialResponse(response));
		}
		finally
		{
			EntityUtils.consume(response.getEntity());
	...
	public void testPOSTToExisting() throws Exception
	{
		...
		ObjectNode error = createItemFromJsonFile("newitem.json", token, null, 400, true, standardParams());

		// ensure error
		assertError(error, 400, "Bad Request", "Version 1 of the item '" + ITEM_UUID + "' already exists");
	...
	public void testThumbnailFieldsReadWrite() throws Exception
	{
		...
		ObjectNode item = (ObjectNode) mapper.readTree(readJsonFile("thumbitem.json", standardParams()));
		...
		assertEquals(attachment.get("thumbnail").asText(), "_THUMBS/shopimage.jpeg.jpeg", "Attachment thumb incorrect");
	...
	private HttpResponse lock(String uuid, int version, String token, int assertCode) throws Exception
	{
		final HttpPost request = new HttpPost(context.getBaseUrl() + "api/item/" + ITEM_UUID + "/" + 1 + "/lock");
	...
	{
		final HttpDelete request = new HttpDelete(context.getBaseUrl() + "api/item/" + ITEM_UUID + "/" + 1 + "/lock");
	...
	private HttpResponse getLock(String uuid, int version, String token, int assertCode) throws Exception
	{
		final HttpGet request = new HttpGet(context.getBaseUrl() + "api/item/" + ITEM_UUID + "/" + 1 + "/lock");
		final HttpResponse response = execute(request, false, token);
		return response;
	}
	...
	public void testLockAndSaveWithLock() throws Exception
	{
		...
		HttpResponse response = putItem(ITEM_UUID, 1, item.toString(), token, "lock", lock.get("uuid").asText());
