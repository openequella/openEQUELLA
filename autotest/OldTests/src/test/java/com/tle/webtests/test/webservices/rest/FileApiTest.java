...
public class FileApiTest extends AbstractRestApiTest
{
	...
	public void testUnzip() throws Exception
	{
		...
		response = execute(getPut(contentUrl + "/myzip.zip", file, "unzipto", "unzippedхцч/sub1"), true);
		...
		assertEquals("Wrong file size for culture.jpg", dlFile.length(), cultureNode.get("size").asInt());
	}
	...
	public void testFileCopyWithOAuthLogin() throws Exception
	{
		...
		HttpResponse response = execute(
			new HttpPost(context.getBaseUrl() + "api/file/copy?" + URLEncodedUtils.format(qparams, "UTF-8")), true,
			token);
	...
	public void testEverything() throws Exception
	{
		...
		response = execute(getPost(stagingDirUrl, "{\"filename\":\"" + Utils.jsescape(FOLDER_1) + "\"}"), true);
		...
		assertEquals("folder1 dir url not correct", folder1DirUrl,
		...
		response = execute(getPut(stagingDirUrl + "/folder4/folder4.1/folder4.1.1", folder411Node.toString()), true);
		...
		assertTrue("Couldn't find 1st folder at folder41", !folder41Node.path("folders").path(0).isMissingNode());
		...
		response = execute(getPut(folder1DirUrl, "{\"filename\":\"" + Utils.jsescape(FOLDER_1) + "\"}"), true);
	...
	public void testNestedFolders() throws Exception
	{
		...
		assertEquals("file1 self link wrong", stagingDirUrl + "/f1/" + URLUtils.urlEncode(TEST_FILE, false),
			myfile1LinkSelf);
		String myfile1LinkDir = getLink(myfile1DeepNode, "dir");
		assertEquals("file1 dir link wrong", stagingDirUrl + "/f1/" + URLUtils.urlEncode(TEST_FILE, false),
			myfile1LinkDir);
		String myfile1LinkContent = getLink(myfile1DeepNode, "content");
		assertTrue("file1 content link wrong",
