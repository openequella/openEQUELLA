...
public class CALSoapInterfaceTest extends AbstractCALTest {
  ...
  public void importCourses() throws Exception {
    ...
    for (String courseCode : courseCodes) {
      ...
      if (courseName != null) {
        assertEquals(
            course_prop.getNode("name/strings/entry/com.tle.beans.entity.LanguageString/text"),
            courseName);
      }
    }
  ...
  private String[] getItemAndAttachments(String itemUuid) throws Exception {
    ...
    ValueThoroughIterator iter =
        prop.iterateAllValues("item/copyright/portions/portion/sections/section/attachment");
    while (iter.hasNext()) {
  ...
  private String searchSoapItem(SoapInterfaceV1 soapService, String defaultPortion)
      throws Exception {
    String searchReq =
  ...
  public void initialSetup() {
    ...
    soapService =
        soapHelper.createSoap(
            SoapInterfaceV1.class,
            "services/SoapInterfaceV1",
            "http://remoting.core.tle.com",
            null);
    soapService.login("caladmin", "``````");
    calCourseService =
        soapHelper.createSoap(
            SoapCourseService.class,
            "services/calcourses.service",
            "http://soap.remoting.web.tle.com",
            soapService);
    calActivationService =
        soapHelper.createSoap(
            CALSoapActivationService.class,
            "services/calactivation.service",
            "http://service.web.cal.tle.com",
            soapService);
  }
}
