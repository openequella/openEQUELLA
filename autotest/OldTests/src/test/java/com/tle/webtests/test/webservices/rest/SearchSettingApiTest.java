...
public class SearchSettingApiTest extends AbstractRestApiTest {
  ...
  private static final String SHOW_NONE_LIVE = "showNoneLive";
  ...
  private static final String DEFAULT_RESULT_ORDER = "defaultResultOrder";
  ...
  public void testSeachPageSettings() throws Exception {
    ...
    assertFalse(initialSettings.get(SHOW_NONE_LIVE).asBoolean());
    ...
    assertNull(initialSettings.get(DEFAULT_RESULT_ORDER));
    ...
    newSearchPageSettings.put(SHOW_NONE_LIVE, true);
    ...
    HttpResponse response = postEntity(newSearchPageSettings.toString(), uri, getToken(), true);
    ...
    assertTrue(updatedSettings.get(SHOW_NONE_LIVE).asBoolean());
    ...
    assertEquals(updatedSettings.get(DEFAULT_RESULT_ORDER).asText(), "RATING");
  }
}
