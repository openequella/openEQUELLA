...
public class CancelWizardTest extends AbstractCleanupTest
{
	...
	{
		IntegrationTesterPage itp = new IntegrationTesterPage(context, SHAREDID, SECRET).load();
		itp.getSignonUrl(ACTION, USERNAME, "", "contributionCollectionIds=b28f1ffe-2008-4f5e-d559-83c8acd79316", true);
	...
	public void cancelFromEdit()
	{
		...
		IntegrationTesterPage itp = new IntegrationTesterPage(context, SHAREDID, SECRET).load();
		itp.getSignonUrl(ACTION, USERNAME, "", "", true);
		SummaryPage view = itp.clickPostToUrlButton(new SelectionSession(context)).homeExactSearch(fullName)
	...
	public void cancelFromNewVersion()
	{
		...
		SummaryPage view = itp.clickPostToUrlButton(new SelectionSession(context)).homeExactSearch(fullName)
			.viewFromTitle(fullName);
		ContributePage contributePage = view.adminTab().newVersion().cancel(new ContributePage(context));
	...
	public void addItem()
	{
		...
		WizardPageTab wizard = itp.clickPostToUrlButton(new SelectionSession(context)).contribute()
