...
/**
 * Test Reference: http://time/DTEC/test/editTest.aspx?testId=15123 Date control
 * assumed tested elsewhere. TODO flickr userid search, add to collection, edit
 * display name
 * 
 * @author larry
 */
...
{
	/**
	 * Contribution wizard for the collection assumed to have only 1 wizard
	 * page. Flickr attachment control presumed to be mandatory, hence we can
	 * save and publish if we have added a flickr attachment, otherwise only
	 * save to draft (or complete wizard / cancel)
	 */
	@Test
	public void basicSearchAndAddFlickrPhoto()
	{
		...
		ItemListPage searchResultsPage = flickrControl.performSearch(searchKeywords, SearchType.GENERAL_ANY_TAGS);
		int howManyAnyTags = searchResultsPage.getTotalAvailable();
		assertTrue(howManyAnyTags > 1000, " We're relying on a large (1000+) returned result from '" + searchKeywords
			+ "' but there's only " + howManyAnyTags);
	...
	/**
	 * Make use of the licence filter and ensure returned results conform to
	 * expectations. It would be nice if they did, but we cannot rely entirely
	 * on Flickr's result set. Cases are observed where an "unrestricted"
	 * licenced item is returned within a filtered licence query, which is
	 * something EQUELLA cannot be liable for. For a basic sanity check, we
	 * expect the majority of returned items to conform to the licence
	...
	public void creativeCommonsFilterTest()
	{
		...
		for( Pair<String, String> textAltPair : licencesTextAltPairs )
		{
			...
			assertTrue(asExpectedElements > failedElements,
				"Expected at least a majority of licence texts to match expected value '" + expectedLicenceText
					+ "', but only " + asExpectedElements + " out of " + sampleSize + " do so.");

			// Clear filters afterwards
			searchResultsPage = flickrControl.resetFilters();

		}
		assertTrue(howManyAnyTags >= sumLicencesAllOptions, "Expected unchecked total(" + howManyAnyTags
			+ ") to be greater than or possibly equal to sum of single options (" + sumLicencesAllOptions + ")");
		flickrControl.close();
	}
	...
	public void flickrInstitutionsTest()
	{
		...
		// To get a reliable sample, we use the widest search possible -
		// ANY_TAGS
		ItemListPage searchResultsPage = flickrControl.performSearch(searchKeywords, SearchType.GENERAL);
		int howManyAnyTags = searchResultsPage.getTotalAvailable();
		assertTrue(howManyAnyTags > 1000, " We're relying on a large (1000+) returned result from '" + searchKeywords
			+ "' but there's only " + howManyAnyTags);
		...
		searchResultsPage = flickrControl.useInstitution(availableInstitutions.get(availableInstitutions.size() - 1));
		...
		assertTrue(
			...
			"Expected at least one of our institutions (" + availableInstitutions.get(1) + ", "
				+ availableInstitutions.get(availableInstitutions.size() - 1) + ") to return a non-zero result");
		flickrControl.close();
	}
