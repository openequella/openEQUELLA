...
public class TasksApiTest extends AbstractRestApiTest
{
	...
	// @formatter:off
	private final String[][] MAP = { //The tasks call won't get item names so we need to match on UUID
		{ "assignedme", "d7f554ee-d50f-4567-a85e-5e62578d10ed" },
		{ "assignedothers", "9424a318-0c16-4080-a737-dc141cd46837" },
		{ "assignednone", "ed883bc3-0a7a-48c4-acac-4b1e3c038f47", "24b977ec-4df4-4a43-8922-8ca6f82a296a",
			"e48edefa-e1fc-4ebe-946f-ff65938c9fae", "d6518347-366b-46db-8b53-327e7381a1e6", "cc7a3e13-4f44-4481-8bc2-881cecfbed96" },
		{ "mustmoderate", "ed883bc3-0a7a-48c4-acac-4b1e3c038f47", "24b977ec-4df4-4a43-8922-8ca6f82a296a",
			"e48edefa-e1fc-4ebe-946f-ff65938c9fae", "d6518347-366b-46db-8b53-327e7381a1e6", "cc7a3e13-4f44-4481-8bc2-881cecfbed96" }
	};
	// @formatter:on
	...
	private JsonNode doTasksSearch(String query, String subsearch, Map<?, ?> otherParams, String token)
		throws Exception
	{
	...
	public void taskSubsearchTest() throws Exception
	{
		...
		for( String[] arr : MAP )
		{
			JsonNode resultsNode = doTasksSearch("TaskApiTest", arr[0], ImmutableMap.of("order", "name"), token);
		...
		JsonNode resultsNode = doTasksSearch("TaskApiTest", "all", ImmutableMap.of("order", "name"), token);
	...
	public void taskFilterTest() throws Exception
	{
		...
		JsonNode resultsNode = doTasksSearch("TaskApiTest", "all",
			ImmutableMap.of("collections", "6bf51943-9d20-4934-a228-8edcec6691ae"), token);
		// Complex workflow collection
		assertTrue(containsInAvailableResult(resultsNode, "24b977ec-4df4-4a43-8922-8ca6f82a296a", "uuid"));
		assertTrue(containsInAvailableResult(resultsNode, "cc7a3e13-4f44-4481-8bc2-881cecfbed96", "uuid"));
		assertFalse(containsInAvailableResult(resultsNode, "9424a318-0c16-4080-a737-dc141cd46837", "uuid"));
	}
	...
	public void taskFilterCountsTest() throws Exception
	{
		...
		Map<String, Pair<Integer, Boolean>> expectations = new HashMap<String, Pair<Integer, Boolean>>();
		...
		HttpResponse response = execute(get, false, token);
		assertEquals(response.getStatusLine().getStatusCode(), 200, "Did not receive successful response from api/task/filtername");
		...
		assertTrue(numTaskTypes >= 7, "test assumes at least 7 task types, but recognise only " + numTaskTypes);
		...
		for( int i = 0; i < numTaskTypes; ++i)
		{
			...
			if( expects != null )
			{
				...
				assertTrue(expects.getFirst().compareTo(count) <= 0, "expected at least " + expects.getFirst() + " for " + taskId +"' but got " + count);
	...
	private void assertInNameOrder(JsonNode jsonNode, String token) throws Exception
	{
		...
		{
			String name = getName(jsonNode.get("results").get(i).get("item").get("uuid").asText(),
				jsonNode.get("results").get(i).get("item").get("version").asInt(), token);
	...
	private void assertInDueDateOrder(JsonNode jsonNode) throws Exception
	{
		...
		for( int i = 0; i < results; i++ )
		{
			...
			assertTrue(due.after(first) || due.equals(first), "Results were not sorted by due date: "
				+ first + " before " + due);
			first = due;
		}
	}
