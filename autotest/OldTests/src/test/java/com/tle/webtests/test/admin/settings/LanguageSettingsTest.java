...
public class LanguageSettingsTest extends AbstractSessionTest
{
	...
	public void testAddContribLang()
	{
		...
		assertTrue((numContribLangsPrior + 1) == numContribLangsPost, "Expected number of langs to increase to "
			+ (numContribLangsPrior + 1) + ", but have " + numContribLangsPost + '.');
		logout();
		// return and find our entry
		String displayYiddishInYemen = DEMO_NAME;
		lsp = loginToLanguageSettingsPage();
		assertTrue(lsp.doesContribLangByDisplayNameExist(displayYiddishInYemen), "Expected to see "
			+ displayYiddishInYemen + ", but not found.");
	}
