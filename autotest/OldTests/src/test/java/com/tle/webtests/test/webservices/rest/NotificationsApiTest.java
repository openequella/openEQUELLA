...
public class NotificationsApiTest extends AbstractItemApiTest
{
	...
	private final String[][] MAP = {
		...
		{ "watchedwentlive", "NotificationsApiTest - Watched Collection", "NotificationsApiTest - Other User Notifying You" },
	...
	private void waitForIndex() throws IOException
	{
		...
		notificationsIndexedWaiter.until(new Function<WebDriver, Boolean>()
		{
			@Override
			public Boolean apply(WebDriver driver)
			{
				try
				{
					JsonNode resultsNode = doNotificationsSearch("NotificationsApiTest", "all",
						ImmutableMap.of("order", "name"), token);
	...
	public void setupNotifications()
	{
		...
		SummaryPage summaryPage = filterListPage.all().search("NotificationsApiTest").getResultForTitle(OTHER_USER, 1)
			.viewSummary();
	...
	private void testClearNotifications() throws Exception
	{
		...
		HttpDelete delete = new HttpDelete(context.getBaseUrl() + "api/notification/" + notificationIdToDelete
			+ "/?waitforindex=true");
	...
	private void testGetNotifications() throws Exception
	{
		...
		for( String[] arr : MAP )
		{
			...
			JsonNode resultsNode = doNotificationsSearch("NotificationsApiTest", arr[0],
				ImmutableMap.of("order", "name", "showall", "true"), token);
	...
	protected void cleanupAfterClass() throws Exception
	{
		...
		AbstractCleanupTest.deleteItemsWithPrefix(context, "autotest", "automated", "NotificationsApiTest");
	}
