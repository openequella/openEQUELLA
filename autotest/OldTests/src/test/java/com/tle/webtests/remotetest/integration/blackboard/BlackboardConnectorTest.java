...
public class BlackboardConnectorTest extends AbstractBlackboardTest {
  ...
  public void bulkMoveActionTest() {
    ...
    BlackboardMyInstitutionPage indexPage =
        new BlackboardLoginPage(context).load().logon("AutoTest", "``````");
  ...
  public void newVersionShowAllVersions() {
    ...
    LMSExportPage lms =
        connectorHelper
            .createTestItem(context.getFullName("New version version1"))
            .adminTab()
            .lmsPage();
    connectorHelper.selectConnector(lms);
    lms.selectSummary();
  ...
  public void addInBlackboardAppearsOnUses() {
    ...
    FindUsesPage uses =
        SearchPage.searchExact(context, "UnicodeItem").getResult(1).viewSummary().findUsesPage();
  ...
  public void addInArchivedBlackboardAppearsOnUsesWhenRelevant() {
    ...
    BlackboardContentPage content =
        coursePage.content().addFolder(FOLDER1).addFolder(FOLDER2).enterFolder(FOLDER1);
    SelectionSession selection = content.addEquellaResource();
    ItemListPage items = selection.getSearchPage().exactQuery("UnicodeItem");
    SummaryPage summary = items.viewFromTitle(WEIRD_ITEM);
    summary.selectItemSummary();
  ...
  public void setupBlackboard() {
    ...
    EditBlackboardConnectorPage editPage =
        page.editConnector(new EditBlackboardConnectorPage(page), BLACKBOARD_CONNECTOR_NAME);
    ...
    page = new ShowConnectorsPage(context).load();
    ShowBlackboardConnectorsPage.addBlackboardConnection(
        page, BLACKBOARD_CONNECTOR_NAME2, context.getIntegUrl(), "testuser", proxyPassword);

    editPage =
        page.editConnector(new EditBlackboardConnectorPage(page), BLACKBOARD_CONNECTOR_NAME2);
    editPage.exportableForAll();
    editPage.viewableForAll();
    editPage.save();
  }
