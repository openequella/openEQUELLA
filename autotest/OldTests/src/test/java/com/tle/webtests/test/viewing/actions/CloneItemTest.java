...
public class CloneItemTest extends BaseCloneMove {
  ...
  public void testCloneToDraft() {
    ...
    verifyItem(
        adminPage.load().exactQuery(origName, 1).viewFromTitle(origName),
        ORIGINAL_COLLECTION,
        true);
    verifyItem(
        adminPage.load().exactQuery(clonedName, 1).viewFromTitle(clonedName),
        "Simple 1 Step",
        true);
  }
  ...
  private void testBulk(
    ...
    for (int i = 1; i <= NUM_ITEMS; i++) {
      ...
      ItemSearchResult result =
          itemAdminPage.load().exactQuery(itemName, NUM_ITEMS).getResultForTitle(itemName, 1);
      ...
      assertTrue(
          (draftStatus && !submit)
              || (liveStatus && submit && expectLive)
              || (modStatus && submit && !expectLive));

      verifyItem(result.viewSummary(), newCollection, !noattachments);
      verifyItem(
          itemAdminPage.load().exactQuery(itemName).getResultForTitle(itemName, 2).viewSummary(),
          ORIGINAL_COLLECTION,
          true);
    }
  }
