...
{
	public OAuthRequests(URI uri, TokenProvider token, ObjectMapper mapper, PageContext context,
	...
	{
		// @formatter:off
		String token = com.jayway.restassured.path.json.JsonPath.from(
				given().
				param("grant_type", "client_credentials").
				param("client_id", clientId).
				param("redirect_uri", "default").
				param("client_secret", password).
				get(getBaseUri().resolve("oauth/access_token").toString()).asString())
				.get("access_token");
		// @formatter:on
