...
public class SearchRequests extends AuthorizedRequests {
  ...
  public ObjectNode waitForIndex(final ItemId expectedId, final String query)
  ...
  public <T> T waitUntil(RequestSpecification request, final Function<ObjectNode, T> until)
    ...
    Waiter<RequestSpecification> indexWaiter =
        new Waiter<RequestSpecification>(request).withTimeout(20, TimeUnit.SECONDS);
  ...
  public Boolean waitUntilIgnoreError(
      RequestSpecification request, final Function<ObjectNode, Boolean> until)
      throws TimeoutException {
    return requestWaiter(request)
        .until(
            new Function<RequestSpecification, Boolean>() {
              @Override
