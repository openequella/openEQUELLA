...
{
	public StagingRequests(URI baseUri, TokenProvider tokenProvider, ObjectMapper mapper, PageContext pageContext,
	...
	{
		return createRequest().expect().header("x-eps-stagingid", Matchers.notNullValue()).post(getResolvedPath())
			.getHeader("Location");
	}
	...
	public Response unzip(String stagingUrl, String path, File file, String zipDir) throws IOException
	{
		return successfulRequest().content(Files.toByteArray(file)).queryParam("unzipto", zipDir)
	...
	{
		Files.write(
			successfulRequest().get(URLDecoder.decode(getLink(fileNode, "self"), "UTF-8")).body().asByteArray(), file);
	}
	...
	{
		ObjectNode object = object(successfulRequest().queryParam("uploads", "").post(getFileUrl(stagingUrl, path)));
		return object.get("uploadId").asText();
	}

	public String uploadPart(String stagingUrl, String path, String uploadId, File file, int partNumber,
		long fileOffest, long length) throws IOException
	{
		try (FileInputStream finp = new FileInputStream(file))
		{
			...
			return successfulRequest().content(bytes).queryParam("partNumber", partNumber)
				.queryParam("uploadId", uploadId).put(getFileUrl(stagingUrl, path)).getHeader("ETag");
		}
	}
