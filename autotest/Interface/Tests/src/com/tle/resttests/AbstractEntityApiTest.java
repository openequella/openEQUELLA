...
public abstract class AbstractEntityApiTest extends AbstractItemApiTest {
  ...
  protected HttpResponse editWorkflow(ObjectNode json) throws IOException {
    ...
    return putEntity(
        json.toString(), context.getBaseUrl() + "api/workflow/" + uuid, getToken(), true);
  }
  ...
  protected HttpResponse lockEntityError(String token, String entityUri, Object... params)
      throws IOException {
    String lockSuffix = entityUri.endsWith("/") ? "lock" : "/lock";
    HttpPost lockPost =
        new HttpPost(appendQueryString(entityUri + lockSuffix, queryString(params)));
    return execute(lockPost, true, token);
  }

  protected HttpResponse getLockEntityError(String token, String entityUri, Object... params)
      throws IOException {
    String lockSuffix = entityUri.endsWith("/") ? "lock" : "/lock";
  ...
  protected JsonNode lockEntity(String token, String entityUri, Object... params)
      throws IOException {
    String lockSuffix = entityUri.endsWith("/") ? "lock" : "/lock";
    HttpPost lockPost =
        new HttpPost(appendQueryString(entityUri + lockSuffix, queryString(params)));
