...
public class WorkflowApiEditTest extends AbstractEntityApiEditTest
{
	...
	public void testModifyUserPath()
	{
		...
		String collUuid = createSimpleCollection(RestTestConstants.SCHEMA_BASIC, workflows.getId(workflow));

		String itemName = context.getFullName("Metadata item");
		ObjectNode item = items.create(Items.json(collUuid, "item/name", itemName, "item/moderators",
			RestTestConstants.USERID_MODERATOR1, "item/moderators2", RestTestConstants.USERID_MODERATOR2), true);
	...
	public void testPriorityChange()
	{
		...
		ObjectNode item = items.create(Items.json(collUuid, "item/name", itemName), true);
		ItemId itemId = items.getId(item);
		RequestSpecification searchRequest = mod1tasks.searchRequest('"' + itemName + '"', null, "priority", "50");
	...
	{
		ObjectNode collection = CollectionJson.json(context.getFullName("Simple collection"), schemaUuid, workflowUuid);
