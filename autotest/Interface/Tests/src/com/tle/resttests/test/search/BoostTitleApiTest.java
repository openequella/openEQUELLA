...
public class BoostTitleApiTest extends AbstractItemApiTest
{
	...
	{
		String fullName = context.getFullName("Boosted " + UUID.randomUUID().toString().substring(0, 8));
		ItemId unboostedId = addDeletable(createItem(
			createItemJsonWithValues(COLLECTION_BASIC, "/item/description", fullName).toString(), getToken(),
			"waitforindex", true));
		ItemId boostedId = addDeletable(createItem(createItemJsonWithValues(COLLECTION_BASIC, "/item/name", fullName)
			.toString(), getToken(), "waitforindex", true));

		List<ItemId> expected = ImmutableList.of(boostedId, unboostedId);

		List<ItemId> itemIds = Lists.newArrayList();
		JsonNode results = getEntity(context.getBaseUrl() + "api/search", getToken(), "q", '"' + fullName + '"',
			"order", "relevance");
