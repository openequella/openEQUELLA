...
public class ItemWorkflowAutoActionTest extends AbstractEntityCreatorTest
{
	...
	public void create()
	{
		...
		ObjectNode task1 = Workflows.task(STEP1, "Auto accept", false, RestTestConstants.USERID_MODERATOR1);
		...
		ObjectNode task3 = Workflows.task(STEP3, "Auto reject", false, RestTestConstants.USERID_MODERATOR1);
		...
		collectionUuid = collections.createId(CollectionJson.json("Auto action collection",
			RestTestConstants.SCHEMA_BASIC, workflows.createId(workflow)));
	...
	public void auto() throws InterruptedException
	{
		...
		ItemId itemId = items.createId(collectionUuid, true, "item/name", context.getFullName("Accept automatically"),
			"item/dueDate", ISO8601Utils.format(overdue));
		...
		ObjectNode step2Status = items.untilModeration(itemId, new Function<ObjectNode, ObjectNode>()
		{
			@Override
			public ObjectNode apply(ObjectNode moderation)
			{
				ObjectNode step2Status = findStatus(moderation, SERIAL, STEP2);
				if( step2Status.get("cause").get("comments").get(0).get("type").asText().equals("reject") )
				{
					return step2Status;
				}
				return null;
