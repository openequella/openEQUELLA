...
public class WorkflowApiTest extends AbstractEntityCreatorTest {
  ...
  public void testWorkflow() throws Exception {
    ...
    List<String> expectEvents =
        Lists.newArrayList("edit", "statechange", "resetworkflow", "approved", "statechange");
    for (String event : expectEvents) {
      assertTrue(
          historyEvents.contains(event),
          "Item history should contain '" + event + "', list: " + historyEvents);
    }
  ...
  private String createSimpleWorkflow(boolean movelive) throws IOException {
    ...
    Workflows.child(
        workflow.with("root"), Workflows.task("Task", true, RestTestConstants.USERID_AUTOTEST));
