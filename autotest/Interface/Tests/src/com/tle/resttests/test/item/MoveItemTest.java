...
public class MoveItemTest extends AbstractEntityCreatorTest {
  ...
  protected void customisePageContext() {
    ...
    collectionID =
        collections.getId(
            collections.create(CollectionJson.json("Very simple collection", schemaId, null)));
  }
  ...
  public void moveItem() {
    ...
    item.put(
        "metadata", "<xml><item><name>Item for moving</name></item><name>newName</name></xml>");
  ...
  public void testSecurity() {
    ...
    collection
        ...
        .putArray("rules")
        .addObject()
        .put("who", "R:TLE_LOGGED_IN_USER_ROLE ")
        .put("override", false)
        .put("granted", false)
        .put("privilege", "CREATE_ITEM");
    collection = collections.create(collection);
    String newCollection = collections.getId(collection);

    ObjectNode item =
        items.create(
            Items.jsonXml(collectionID, "<xml><name>MoveItemTest - SecurityTest</name></xml>"));
  ...
  public void testToAndFromModeration() {
    ObjectNode item =
        items.create(
            Items.jsonXml(
                RestTestConstants.COLLECTION_NOTIFICATIONS,
    ...
    Assert.assertEquals(
        moderation.get("nodes").get("children").get(0).get("status").asText(), "incomplete");
  ...
  public void testWorflowReset() {
    ObjectNode collection =
        CollectionJson.json(
            "MoveItemTest - TestWorlfowReset", schemaId, RestTestConstants.WORKLFOW_COMPLEX);
    String complexWFCollection = collections.getId(collections.create(collection));
    ObjectNode item =
        items.create(
            Items.jsonXml(
                complexWFCollection, "<xml><name>MoveItemTest - TestModeration</name></xml>"));
    ItemId id = items.getId(item);
    ...
    Assert.assertEquals(
        moderation.get("nodes").get("children").get(0).get("status").asText(), "incomplete");
    Assert.assertEquals(
        moderation.get("nodes").get("children").get(1).get("status").asText(), "waiting");

    tasks.accept(id, "3c4926a7-4b28-4ea2-88b8-42da1902a9ed", null);
    moderation = items.moderation(id);
    Assert.assertEquals(
        moderation.get("nodes").get("children").get(0).get("status").asText(), "complete");
    Assert.assertEquals(
        moderation.get("nodes").get("children").get(1).get("status").asText(), "incomplete");
