...
public class GroupApiTest extends AbstractRestAssuredTest {
  ...
  private void assertUsers(ObjectNode search, boolean autotest, boolean mod1, boolean mod2) {
    ...
    Assert.assertEquals(
        users.contains(RestTestConstants.USERID_AUTOTEST),
        autotest,
        "Group should" + (autotest ? "" : "n't") + " contain user autotest");
    Assert.assertEquals(
        users.contains(RestTestConstants.USERID_MODERATOR1),
        mod1,
        "Group should" + (mod1 ? "" : "n't") + " contain user moderator1");
    Assert.assertEquals(
        users.contains(RestTestConstants.USERID_MODERATOR2),
        mod2,
        "Group should" + (mod2 ? "" : "n't") + " contain user moderator2");
  }
  ...
  public void search() throws Exception {
    ...
    if (testConfig.isEquella()) {
      ...
      Assert.assertEquals(
          error.get("error_description").textValue(), "Searching for groups is no longer possible");
    }
  }
