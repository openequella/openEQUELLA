...
public class InstitutionApiTest extends AbstractEntityCreatorTest {
  ...
  protected void deleteByUrl(String url) {
    ObjectNode byUrl = institutions.getByUrl(url);
    if (byUrl != null) {
  ...
  public void createTest() {
    ...
    wrong.put("password", "tle010");
    wrong.remove("name");
    assertValidationError(wrong, "Institution name must not be left blank");
    wrong.put("name", "AutoTest");
    assertValidationError(
        wrong, "Institution name 'AutoTest' is already in use by another institution");
    wrong.put("name", INST_NAME);
    wrong.put("filestoreId", "autotest");
    assertValidationError(
        wrong, "Filestore ID 'autotest' is already in use by another institution");
  ...
  public void editTest() {
    ObjectNode inst =
        institutions.jsonAppendBaseUrl("new name", "newpass", "newfsid", "newurl", false);
