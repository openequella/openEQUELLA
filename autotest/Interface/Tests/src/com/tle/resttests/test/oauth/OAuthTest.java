/** This test needs Redis, if this test fails it's probably because Redis isn't right */
...
public class OAuthTest extends AbstractRestApiTest {
  ...
  private HttpResponse defaultClientTokenRequest(String... otherParams) throws Exception {
    String[] params =
        new String[] {
          "redirect_uri", REDIRECT_URI, "client_id", CLIENT_ID, "response_type", "token"
        };
    return getResponse(
        context.getBaseUrl() + "oauth/authorise",
        null,
        false,
  ...
  private HttpResponse defaultClientTokenRequestPost(String... otherParams) throws Exception {
    ...
    return postUri(
        context.getBaseUrl() + "oauth/authorise", null, ArrayUtils.addAll(params, otherParams));
  }
  ...
  public void testOAuthServletErrors(String type, String descritopn, String url)
      throws IOException {
  ...
  public void testOAuthServerSideFlow() throws Exception {
    ...
    final String tokenGetUrl =
        context.getBaseUrl()
            + "oauth/access_token?grant_type=authorization_code&client_id="
            + CLIENT_ID_SERVER_FLOW
            + "&redirect_uri="
            + home
            + "&client_secret="
    ...
    HttpResponse searchResp =
        execute(new HttpGet(context.getBaseUrl() + "api/search?showall=true"), false);
    Assert.assertEquals(
        searchResp.getStatusLine().getStatusCode(), 200, "Unexpected search result status");
  ...
  public void testUnknownClientId() throws Exception {
    ...
    String string = EntityUtils.toString(response.getEntity());
    Assert.assertTrue(
        string.contains("No OAuth client can be found with the supplied client_id"),
    ...
    response =
        postUri(
            ...
            null,
            "redirect_uri",
            context.getBaseUrl() + "logon.do",
            "client_id",
            "testUnknownRespTypeClient",
            "response_type",
            "whatthe");
    String location = getLocation(response);

    Assert.assertTrue(
        location.contains("unsupported_response_type"), "Wrong error message for invalid client");
  }
