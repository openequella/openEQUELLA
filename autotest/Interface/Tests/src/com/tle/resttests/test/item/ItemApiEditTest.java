...
public class ItemApiEditTest extends AbstractItemApiTest {
  ...
  protected void addOAuthClients(List<Pair<String, String>> clients) {
    ...
    clients.add(
        new Pair<String, String>(OAUTH_OTHER_CLIENT_ID, RestTestConstants.USERID_TOKENUSER));
    clients.add(
        new Pair<String, String>(OAUTH_NOPRIVS_CLIENT_ID, RestTestConstants.USERID_RESTNOPRIV));
  }
  ...
  private ItemId createItem(String token) throws Exception {
    ...
    uploadFile(
        stagingDirUrl + "/_mypages/78a2de74-96de-48de-8865-22856c22ae49",
        "page.html",
        Attachments.get("page.html"));
    uploadFile(
        stagingDirUrl + "/_IMS",
        "The vile vendor go figure.zip",
        Attachments.get("The vile vendor go figure.zip"),
        "unzipto",
        "The vile vendor go figure.zip");
  ...
  private ObjectNode createItemFromString(
      String newItemJson, String token, String stagingUuid, int responseCode, boolean returnError)
      throws Exception {
    if (!returnError) {
      if (stagingUuid != null) {
  ...
  private ObjectNode createItemFromFile(
      String jsonFile, String token, String stagingUuid, int responseCode, boolean returnError)
      throws Exception {
  ...
  public void testPOSTToExisting() throws Exception {
    ...
    ObjectNode error =
        createItemFromString(getItem(itemId, "all", token).toString(), token, null, 400, true);
    ...
    assertError(
        ...
        400,
        "Bad Request",
        "Version 1 of the item '" + itemId.getUuid() + "' already exists");
  }
  ...
  private HttpResponse lock(ItemId itemId, String token, int assertCode) throws Exception {
    final HttpPost request =
        new HttpPost(context.getBaseUrl() + "api/item/" + itemId.getUuid() + "/" + 1 + "/lock");
  ...
  private void unlock(ItemId itemId, String token, int assertCode) throws Exception {
    final HttpDelete request =
        new HttpDelete(context.getBaseUrl() + "api/item/" + itemId.getUuid() + "/" + 1 + "/lock");
  ...
  private HttpResponse getLock(ItemId itemId, String token, int assertCode) throws Exception {
    final HttpGet request =
        new HttpGet(context.getBaseUrl() + "api/item/" + itemId.getUuid() + "/" + 1 + "/lock");
    final HttpResponse response = execute(request, false, token);
    return response;
  }
  ...
  public void testLockAndSaveWithLock() throws Exception {
    ...
    HttpResponse response =
        putItem(itemId, item.toString(), token, "lock", lock.get("uuid").asText());
