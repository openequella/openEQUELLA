...
public class ItemWorkflowLiveTest extends AbstractEntityCreatorTest {
  ...
  public void create() {
    ...
    ObjectNode task1 =
        Workflows.task(STEP1, "Only step", false, RestTestConstants.USERID_MODERATOR1);
    task1.put("moveLive", "arrival");
    Workflows.rootChild(workflow, task1);
    collectionUuid1 =
        collections.createId(
            CollectionJson.json(
                "Move live collection1",
  ...
  public void arrival() {
    ItemId itemId =
        items.createId(
            collectionUuid1, true, "item/name", context.getFullName("Move live immediately"));
  ...
  public void accepted() {
    ItemId itemId =
        items.createId(
            collectionUuid2, true, "item/name", context.getFullName("Move live after acceptance"));
    ObjectNode moderation = items.moderation(itemId);
