...
public class SecurityTest extends AbstractEntityApiTest {
  ...
  protected void addOAuthClients(List<Pair<String, String>> clients) {
    ...
    clients.add(
        new Pair<String, String>(OAUTH_OTHER_USER_CLIENT_ID, RestTestConstants.USERID_TOKENUSER));
  }
  ...
  public void testOwnerSecurity() throws Exception {
    ...
    ObjectNode item =
        createItemJsonWithValues(collUuid, "item/name", context.getFullName("A Changed Item"));
    HttpResponse response =
        putItem(getItemUri(itemId), item.toString(), token1, "waitforindex", true);
    assertResponse(response, 200, "Should be ok");

    item =
        createItemJsonWithValues(
            collUuid, "item/name", context.getFullName("Another Changed Item"));
