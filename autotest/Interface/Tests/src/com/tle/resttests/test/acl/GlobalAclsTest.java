...
public class GlobalAclsTest extends AbstractRestAssuredTest {
  ...
  public void setup() {
    ...
    ObjectNode created =
        institutions.create(institutions.jsonAppendBaseUrl(name, "tle010", "blah", "acls", true));
    RequestsBuilder builder =
        new RequestsBuilder(
            this, OAuthTokenCache.ADMIN_TOKEN, URI.create(created.get("url").asText()));
    // acls = builder.acls();
    ...
    // RequestsBuilder autoTestBuilder =
    // builder.user(tokenCache.getProvider(RestTestConstants.USERID_AUTOTEST));
