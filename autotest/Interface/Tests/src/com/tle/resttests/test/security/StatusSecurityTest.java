...
public class StatusSecurityTest extends AbstractEntityCreatorTest
{
	...
	public void statusTest()
	{
		...
		ObjectNode coll = CollectionJson.json(context.getFullName("StatusCollection"), RestTestConstants.SCHEMA_BASIC,
			null);
		CollectionJson.addStatusRule(coll.with("security"), "draft", AclLists.rule("EDIT_ITEM", false, false, "*"));
		coll = collections.create(coll);
		String collId = collections.getId(coll);
		ObjectNode item = items.create(
			Items.jsonXml(collId, "<xml><item><name>" + context.getFullName(itemName) + "</name></item></xml>"), false,
			"draft", true);
