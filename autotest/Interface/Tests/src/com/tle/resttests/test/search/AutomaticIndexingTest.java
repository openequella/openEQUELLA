...
public class AutomaticIndexingTest extends AbstractItemApiTest {
  ...
  public Object[][] getData() {
    return new Object[][] {
      {"DOC.doc", "msword", 1},
      {"html test.html", "myhtmlTest", 2},
      {"text test.txt", "unique", 3},
      {"xl2007.xlsx", "ridiculousness", 4},
      {"XLS.xls", "msexcel", 5}
    };
  }
  ...
  public void autoIndexing(String filename, String query, int itemUpto) throws Exception {
    ...
    // Just having the item name discoverable is not enough.  The indexing is broken into slow and
    // fast, where the slow is extracting metadata from the attachment.
    waitForIndex(query, itemId.getUuid(), true);

    JsonNode basicResults = basicSearch(query, token, true);
    Assert.assertTrue(
        basicResults.get("length").asInt() > 0,
        "Zero results for " + query + " but expecting to find " + itemName);
