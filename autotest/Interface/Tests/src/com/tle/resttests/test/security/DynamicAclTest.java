...
public class DynamicAclTest extends AbstractEntityCreatorTest
{
	...
	{
		// As Autotest:
		ObjectNode item = Items.json(collectionUuid, "/item/name", USER_REVOKE_ITEM, "/item/userToRevoke", USER);
		...
		// As AutoTest:
		// Change item
		ObjectNode v2 = Items.json(collectionUuid, "/item/name", USER_REVOKE_ITEM, "/item/userToRevoke",
	...
	{
		// As Autotest:
		ObjectNode item = Items.json(collectionUuid, "/item/name", GROUP_REVOKE_ITEM, "/item/groupToRevoke", GROUP);
		ItemId itemID = items.getId(items.create(item, 45));
		// Wait for index, make sure it's there
		...
		getTokens().invalidate(USER);
		item = Items.json(collectionUuid, "/item/name", "Something different", "/item/groupToRevoke", GROUP);
