...
public class WebHostCallback implements Callback
{
	...
	public void task(Wizard installer)
	{
		...
		if( !m.matches() )
		{
			JOptionPane.showMessageDialog(parent, "You must specify the external URL of this"
				+ " server.  Some examples of this are:\n - http://my.domain/\n - http://my."
				+ "domain:8000\n - http://my.domain/some/context\n - http://my.domain:8000/" + "some/context\n",
				"Incorrect Server URL", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		{
			JOptionPane.showMessageDialog(parent, "Hostnames cannot contain underscore"
				+ " characters.  Please modify the hostname to remove this character" + " before continuing.", "Error",
				JOptionPane.ERROR_MESSAGE);
			return;
		}

		if( hostname.equalsIgnoreCase("localhost") || hostname.equals("127.0.0.1") ) //$NON-NLS-1$ //$NON-NLS-2$
		{
			int result = JOptionPane.showConfirmDialog(parent, "You have entered 'localhost'"
				...
				+ " using 'localhost' or '127.0.0.1'?", "Warning!", JOptionPane.YES_NO_OPTION,
		...
		{
			JOptionPane.showMessageDialog(parent, "The server port must be an number between 1 and"
				+ " 65536 inclusively.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	private void checkCanConnect(final Wizard installer, final String hostname, final int port)
	{
		...
		GlassSwingWorker<?> worker = new GlassSwingWorker<Object>()
		{
			...
			public void exception()
			{
				...
				{
					int result = JOptionPane.showConfirmDialog(parent, "The hostname you supplied"
				...
				{
					String message = "Your computer could not bind to the given hostname and port."
						+ "  Please ensure that\n no other services are running on this port.\n\n";
					if( port < 1024 )
					{
						message += "Please note that ports less than 1024 are restricted for" + " security.\n\n";
					}
					message += "Do you want to continue anyway?";

					int result = JOptionPane.showConfirmDialog(parent, message, "Warning", JOptionPane.YES_NO_OPTION,
						JOptionPane.WARNING_MESSAGE);
