...
public class FindJava implements Callback {
  ...
  private static final String[] SEARCH_PATHS = {
    "c:/", "/usr", "/usr/local", "/opt", "c:/devenv/sdk/", "c:/Program Files/Java", "/usr/lib/jvm"
  };
  ...
  public void task(Wizard installer) {
    ...
    // starts with solaris as it might be solaris-sparc or solaris-x86
        ...
        || platform.startsWith("solaris")) // $NON-NLS-1$ //$NON-NLS-2$
    {
      page += 1;
    }

    if (defaults == null
        || (defaults.getNode("java/jdk") == null
            || defaults.getNode("java/jdk").equals(""))) // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    {
      if (platform.equals("mac")) // $NON-NLS-1$
      {
        javaHome = "/Library/Java/Home"; // $NON-NLS-1$
      } else if (platform.startsWith("win")
          || platform.startsWith("linux") // $NON-NLS-1$ //$NON-NLS-2$
    ...
    if (defaults == null
        || (defaults.getNode("install.path") == null
            || defaults
                .getNode("install.path") // $NON-NLS-1$ //$NON-NLS-2$
                .equals(""))) // $NON-NLS-1$
    {
      if (platform.equals("mac")) // $NON-NLS-1$
      {
  ...
  /**
   * Looks for the SDK in the given path. If there are multiple matching directories, it will try
   * and grab the one with the highest revision. For example, j2sdk1.4.2_07 will be selected rather
   * than j2sdk1.4.2_04.
   */
