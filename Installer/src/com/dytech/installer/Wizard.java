...
public class Wizard {
  ...
  /**
   * Constructor for <code>Installer</code>. The given properties bag represents the XML installer
   * definition.
   */
  ...
  /**
   * Start the installation process. Extract all the information from the proporties bag, create the
   * pages, create the window, and go to the first page.
   */
  ...
  /**
   * Displays the page of the given page number. All the input values are stored in the output
   * properties bag.
   *
  ...
  /**
   * Move to page relative to the current one. For example, <code>gotoRelativePage(2)</code> is
   * equivalent to <code>gotoPage(getCurrentPage() + 2)</code>.
   */
  ...
  /**
   * Declare that the first phase of the install process is finshed. This will Move the installer on
   * to phase two; the actual installation.
   */
  ...
  /**
   * Extracts all of the "global" type information from the proporties bag. This includes
   * information such as product names, company names, images, background colours, and more.
   */
    ...
    if (file.exists()) {
      int confirm =
          JOptionPane.showConfirmDialog(
              getFrame(),
              "Values entered during a"
                  + " previous installation have been found.\nDo you wish you use these values?",
