...
lazy val equellaserver = (project in file("Source/Server/equellaserver"))
...
lazy val UpgradeInstallation =
  (project in file("Source/Tools/UpgradeInstallation"))
    .settings(legacyPaths)
    .dependsOn(
      platformCommon,
      platformEquella,
...
git.gitTagToVersionNumber := {
...

  {
    case TagRegex(_, _, v, sha) =>
      Some(
        EquellaVersion(majorMinor, s"$streamName.$buildName", v.toInt, sha).fullVersion)
    case _ => None
  }
}
...
updateLicenses := {
  ...
  val plugsinReports =
    updateLicenses.all(ScopeFilter(inAggregates(allPlugins))).value
  val allLicenses =
    (plugsinReports.flatMap(_.licenses) ++ serverReport.licenses)
      .groupBy(_.module)
      .values
      .map(_.head)
...
mergeJPF := {
  ...
  val _allPluginDirs = pluginAndLibs
    .all(ScopeFilter(inAggregates(allPlugins, includeRoot = false)))
    .value
  val extensionsOnly =
    (baseDirectory.value / "Source/Plugins/Extensions" * "*" / "plugin-jpf.xml").get
  val allPluginDirs = _allPluginDirs ++ (extensionsOnly.map(f =>
    (f.getParentFile, Seq.empty)))
  if (args.isEmpty) {
    val plugins = PluginRefactor.findPluginsToMerge(allPluginDirs,
...
val userBeans
  : FileFilter = ("GroupBean.java" || "UserBean.java" || "RoleBean.java") &&
...
sources in (Compile, doc) := {
  (javadocSources((baseDirectory in LocalProject("com_equella_base")).value)
    +++ javadocSources(
      (baseDirectory in LocalProject("com_equella_core")).value)).get
}
