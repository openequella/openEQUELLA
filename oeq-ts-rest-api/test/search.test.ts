...
describe("Search for items", () => {
  it("should be able to search without params", async () => {
    const searchResult = await Search.search(TC.API_PATH);
  ...
  it("should be able to search with params", async () => {
    ...
    const searchParams: Search.SearchParams = {
      query: "API",
      status: [Search.ItemStatus.LIVE],
      collections: [collection]
    };
    const searchResult = await Search.search(TC.API_PATH, searchParams);
    ...
    expect(status.toUpperCase()).toBe(Search.ItemStatus.LIVE);
    expect(collectionId).toBe(collection);
  });

  it("should get 404 response if search for a non-existing collection", async () => {
    await expect(Search.search(TC.API_PATH, {collections: ["testing collection"]}))
