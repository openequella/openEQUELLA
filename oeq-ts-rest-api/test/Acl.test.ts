...
import {
  ...
  ACL_EDIT_SYSTEM_SETTINGS,
  ACL_HIERARCHY_PAGE,
  ACL_SEARCH_PAGE,
  ACL_VIEW_HIERARCHY_TOPIC,
} from '../src/Acl';
...
describe('Check privileges', () => {
  it('returns privileges granted to the current user when given a list of privileges', async () => {
    ...
    const acls: string[] = [ACL_SEARCH_PAGE, ACL_HIERARCHY_PAGE, VIEW_APIDOCS];

    const result = await checkPrivilege(TC.API_PATH, acls);
    expect(result).toEqual([ACL_SEARCH_PAGE, ACL_HIERARCHY_PAGE]);
  });
