...
export const RestorePortletsTab = ({
  ...
  const renderPortlets = (portlets: OEQ.Dashboard.PortletClosed[]) =>
    A.isNonEmpty(portlets) ? (
      <ClosedPortletsList
        closedPortlets={portlets}
        onPortletRestore={onPortletRestore}
      />
    ) : (
      <PortletSearchResultNoneFound noneFoundMessage={noClosedPortletsLabel} />
    );

  return pipe(closedPortlets, (cp): React.JSX.Element => {
    switch (cp.state) {
      case "loading":
        return (
          <Skeleton
      ...
      case "success":
        return renderPortlets(cp.portlets);
      case "failed":
