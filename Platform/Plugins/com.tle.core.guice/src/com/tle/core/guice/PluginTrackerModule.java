...
public abstract class PluginTrackerModule extends AbstractModule {
  ...
  protected <T> TrackerProvider<T> bindTracker(
      Class<T> clazz, String extensionPoint, String beanParameter) {
    return bindTracker((java.lang.reflect.Type) clazz, null, extensionPoint, beanParameter);
  }

  protected <T> TrackerProvider<T> bindTracker(
      java.lang.reflect.Type typeParam, String extensionPoint, String beanParameter) {
    return bindTracker(typeParam, null, extensionPoint, beanParameter);
  }

  protected <T> TrackerProvider<T> bindTracker(
      Class<T> clazz, Annotation annotatedWith, String extensionPoint, String beanParameter) {
    return bindTracker(
        (java.lang.reflect.Type) clazz, annotatedWith, extensionPoint, beanParameter);
  }
  ...
  protected <T> TrackerProvider<T> bindTracker(
    ...
    TrackerProvider<T> trackerProvider =
        new TrackerProvider<T>(getPluginId(), extensionPoint, beanParameter);
    @SuppressWarnings("unchecked")
    TypeLiteral<PluginTracker<T>> typeLiteral =
        (TypeLiteral<PluginTracker<T>>) TypeLiteral.get(type);
    LinkedBindingBuilder<PluginTracker<T>> bindingBuilder = bind(typeLiteral);
    if (annotatedWith != null) {
      bindingBuilder =
          ((AnnotatedBindingBuilder<PluginTracker<T>>) bindingBuilder).annotatedWith(annotatedWith);
    }
