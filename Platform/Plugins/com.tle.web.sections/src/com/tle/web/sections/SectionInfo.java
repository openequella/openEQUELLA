...
/**
 * This is the request time instances by which {@link Section}s communicate with each other.
 *
 * <p>It could be considered analogous to {@link HttpServletRequest} but for the Sections library.
 * The responsibilities of the {@code SectionInfo} include:
 *
 * <ul>
 *   <li>Maintaining a list of {@link SectionTree}s, which can be thought of as a wrapping list,
 *       with the last {@code SectionTree} in the list wrapping the previous one.
 *   <li>Keeping a queue of {@link SectionEvent}s and ensuring that {@code SectionEvent}s are
 *       processed by their appropriate listeners (see {@link #processEvent(SectionEvent)})
 *   <li>Maintaining a cache of Model instances for any {@code Section}s contained in the list of
 *       {@code SectionTree}s.
 * </ul>
 *
 * Instances of {@code SectionInfo} are created by the {@link SectionsController}. To see how a
 * {@code HttpServletRequest} gets processed and ends up with a response written to the {@code
 * HttpServletResponse}, see the {@link SectionsController}.
 *
 * @author jmaginnis
 */
...
public interface SectionInfo extends InfoCreator {
  ...
  /**
   * Gets the data registered with the {@code SectionTree} for the given {@code SectionId}
   *
   * @param <T> The type of the tree data
   * @param id The {@code SectionId} containing the id of the {@code Section} to lookup data for
  ...
  /**
   * Check whether or not any registered {@code SectionTree}s contain a {@code Section} registered
   * with the given id
   *
   ...
   *
   * @param id The {@code SectionId} of a {@code Section} in the outer {@code SectionTree}
  ...
  /**
   * Get an attribute, safely. If the attribute doesn't exist yet, it creates an instance using the
   * Class passed in.
   *
   * @param <T> The type of the attribute
   * @param name The key for the attribute
   * @param clazz The Class to call {@code newInstance()} on if the attribute is null.
   * @return The attribute
   */
  ...
  /**
   * Get a list which is a contains values from all {@code SectionTree}s combined. If a tree has the
   * attribute defined, it must be a list.
   *
  ...
  /**
   * Scans trees for any section registered as clazz OR if specific class not found then the first
   * section which is a subclass of clazz
   *
   * @param clazz
   * @return
   */
  ...
  /**
   * Get the {@code HttpServletResponse} associated with this {@code SectionInfo}
   *
  ...
  /**
   * Get the {@code HttpServletRequest} associated with this {@code SectionInfo}
   *
   ...
   * This involves either getting the listener list via {@link #getListeners(String, Class)} and
   * calling {@link SectionEvent#fire(SectionId, SectionInfo, EventListener)} for each listener, or
   * in the case of Direct events calling {@link SectionEvent#fireDirect(SectionId, SectionInfo)}.
   *
   ...
   * This should really only be called by the {@code SectionsController} or a {@link SectionFilter}.
   *
  ...
  /**
   * Set the fact that this {@code SectionInfo} has been rendered, so the {@link SectionsController}
   * doesn't need to proceed with the render process. Also clears the event queue.
   */
  void setRendered();

  /** Removes any event's with priority higher than {@code SectionEvent#PRIORITY_RENDER}. */
  void renderNow();
  ...
  /**
   * Returns the "public" bookmark. Basically the url that will be displayed in the browser. When we
   * do the "GET" of PRG, this is the url we will send back.
   *
   * @return An Bookmark representing the current state of this {@code SectionInfo}
   */
