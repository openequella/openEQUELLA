...
public class AnnotatedBookmarkScanner {
  ...
  public AnnotatedBookmarkScanner(
      Class<?> clazz, BookmarkRegistrationHandler handler, Conversion conversion) {
    ...
    for (Field field : fields) {
      ...
      if (bookmark != null) {
        addProperty(
            cachedBeanInfo.getPropertyDescriptor(field.getName()), field, bookmark, classAnn);
      }
    }
  ...
  private void addProperty(
      PropertyDescriptor descriptor, Field field, Bookmarked bookmark, Bookmarked classAnn) {
      ...
      if (vals != null) {
        ...
        try {
          ...
          if (vals.length == 2 && vals[0].equals(vals[1])) {
            ...
            val =
                conversion.convertFromParameters(name, prop.writeAccessor.getType(), repairedVals);
          } else {
  ...
  public void document(SectionInfo info, String id, DocumentParamsEvent event) {
    ...
    for (BookmarkedProperties prop : properties) {
      if (prop.annotation.stateful()) {
        ...
        event.addParam(
            prop.contexts.getContexts().contains(BookmarkEvent.CONTEXT_SUPPORTED), paramId, type);
      }
    }
  }
