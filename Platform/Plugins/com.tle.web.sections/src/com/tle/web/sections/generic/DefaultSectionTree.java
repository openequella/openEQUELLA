...
public class DefaultSectionTree implements SectionTree {
  ...
  private static final Map<Class<?>, Set<Class<?>>> INTERFACE_CACHE =
  ...
  private final List<SectionEvent<? extends EventListener>> applicationEvents =
      new ArrayList<SectionEvent<? extends EventListener>>();
  ...
  private void addSection(
      String parent, String id, Section section, String insertRef, boolean after, boolean inner) {
    if (parent == null) {
      if (root != null) {
      ...
      parentNode.addChild(
          new InternalNode(parentNode, id, section, getClassToRegister(section)),
          insertRef,
          after,
          inner);
    }
  ...
  private String registerSectionsInternal(
      String preferred,
      Object node,
      String parentId,
      String insertId,
    ...
    if (parentId != null) {
      ...
      if (parent == null) {
        throw new SectionsRuntimeException(
            "Trying to register into an unknown parent: " + parentId);
      }
    }
    if (node instanceof List<?>) {
      ...
      String id =
          registerSectionsInternal(
              preferredId, section, parentId, insertId, after, inner, idForRoot);
  ...
  public <T extends SectionId, S extends T> S lookupSection(Class<T> key, SectionId reference) {
    ...
    throw new Error(
        "Multiple sections of class "
            ...
            + (reference == null
                ? ""
                : " when doing a lookup on " + reference + " " + (reference.getClass().getName())));
  }
  ...
  class InternalNode {
      ...
      if (lookForId != null) {
        ...
        if (!found) {
          throw new RuntimeException(
              "Cannot find insertion point " + lookForId + " for section id " + id);
        }
    ...
    public String toString() {
      return id
          + " ("
          + (classRegisteredAs == null ? "No direct registered class" : classRegisteredAs)
          + ")";
    }
  }
}
