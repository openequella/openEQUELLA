...
/**
 * keeps reading until we get a '&amp;' and then buffer the rest as 'suspect' until a ';' is read.
 * Only then can we determine if the suspect buffer is illegal and can be discarded, OR it's okay
 * and we can 'read' it in. If we read in an illegal unicode char, it can be discarded straight
 * away. This code is deliberately mad-hacks (eg unrolled loops and inline method code) for
 * performance reasons
 *
 * @author aholland
 */
public class BadCharacterFilterReader extends FilterReader {
  ...
  public BadCharacterFilterReader(final Reader reader, final int bufferSize) {
    ...
    if (bufferSize <= MAX_SUSPECTION_LENGTH) {
      throw new IllegalArgumentException(
          "bufferSize must be at least " + MAX_SUSPECTION_LENGTH); // $NON-NLS-1$
    }
  ...
  @Override
  public int read(final char[] outBuffer, final int originalWriteOffset, final int len)
      throws IOException {
    synchronized (lock) {
      if (in == null) {
      ...
      if (suspectStart > 0) {
        ...
        final int maxWrappedReadSize =
            (maxWriteSize > suspectLength ? maxWriteSize - suspectLength : 0);
        ...
        if (wrappedReadSize >= 0) {
          for (int i = 0; i < wrappedReadSize; ++i) {
            ...
            if ((next < 0x20 && next != 0x9 && next != 0xA && next != 0xD)
                || (next > 0xD7FF && next < 0xE000)
                || (next > 0xFFFD && next < 0x10000)
                || next > 0x10FFFF) {
              // discard
              didDiscard = true;
            } else {
  ...
  /**
   * A very apt name for this code methinks... Preconditions: suspectStart is ALWAYS zero here
   * suspect[0] == '&' ALWAYS suspect[suspectLength-1] == ';' ALWAYS
   *
   * @param suspect
   * @param suspectLength
   * @return
  ...
  protected boolean isDodgy() {
    ...
    if (suspect[1] == '#') {
      if (suspectLength > 2) {
        ...
        } else if (c3 == '0' || c3 == '1' || c3 == '2' || c3 == '3' || c3 == '4' || c3 == '5'
            || c3 == '6' || c3 == '7' || c3 == '8' || c3 == '9') {
