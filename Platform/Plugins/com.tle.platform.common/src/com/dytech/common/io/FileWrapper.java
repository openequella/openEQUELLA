    ...
    if (destination.exists()) {
      ...
      if (isFile() && !destination.isFile()) {
        throw new IOException(
            "Could not move: destination is not a file: " + destination.toString());
      }
      if (isDirectory() && !destination.isDirectory()) {
        throw new IOException(
            "Could not move: destination is not a directory: " + destination.toString());
      }
      try {
        destination.recursiveDelete();
      } catch (IOException e) {
        throw new IOException(
            "Could not move: destination(s) could not be removed: " + e.getMessage());
      }
    }
    boolean madeDirs = (new FileWrapper(destination.getParent())).mkdirs();
    if (!(madeDirs || destination.getParentFile().exists())) {
      throw new IOException(
          "Could not create/confirm directory " + destination.getParentFile().getAbsolutePath());
    }
    ...
    if (target.exists()) {
      if (!target.isDirectory()) {
        throw new IllegalArgumentException(
            "Existing destination is not a directory: " + toString());
      }
      if (!overwrite) {
        throw new IOException("Will not overwrite destination: " + target.toString());
      }
