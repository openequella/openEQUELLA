...
public final class FileUtils
{
	...
	public static boolean delete(Path f, FileCallback callback, boolean failOnError) throws IOException
	{
	...
	public static List<String> grep(final Path dirs, final String pattern, @Nullable GrepFunctor func,
		final boolean filesOnly)
	{
		...
		final PathMatcher pathMatcher = FileSystems.getDefault().getPathMatcher("glob:**/" + dirName + "/" + pattern);
		...
		try
		{
			Files.walkFileTree(dirs, EnumSet.noneOf(FileVisitOption.class), depth, new SimpleFileVisitor<Path>()
			{
				@Override
	...
	public static long countFiles(Path file)
	{
		...
		{
			throw new RuntimeException("Error counting files for " + file.toString());
		}
	}
