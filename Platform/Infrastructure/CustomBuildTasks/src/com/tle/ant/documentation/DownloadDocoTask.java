...
public class DownloadDocoTask extends Task
{
	...
	public static void main(String[] args)
	{
		...
		// MAESTRO
		// task.setOut("out");
		// task.setQuery("\"EQUELLA 6.1 Java REST Tutorial\"");
		// task.setFilename("EQUELLA 6.1 Java REST Tutorial.pdf");
		// task.setEquellaVersion("6.1");
		// task.setProxyHost("origliasso");
		// task.setProxyPort(8888);
					...
					if( attachments != null )
					{
						for( AttachmentBean attachmentBean : attachments )
						{
							...
							if( attachmentFilename.equalsIgnoreCase(getFilename()) )
							{
								...
								getProject().setNewProperty(out, absPath.getAbsolutePath());
								return;
							}
						}
					}
	...
	{
		return new URL(baseUrl, "file/" + uuid + "/" + version + "/" + URLEncoder.encode(attachmentFilename, "UTF-8"));
	}
