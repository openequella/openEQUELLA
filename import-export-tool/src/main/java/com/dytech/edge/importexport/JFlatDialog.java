...
class JFlatDialog extends JDialog {
  ...
  /** Returns the main container of the dialog. Overrides <code>getContentPane</code> in JDialog. */
  @Override
...
class Title extends JLabel {
  private static final Image TEXT_BACK =
      new ImageIcon(JFlatDialog.class.getResource("TextBackRep.jpeg")).getImage();
  private static final Image transition =
      new ImageIcon(JFlatDialog.class.getResource("Transition.jpeg")).getImage();
  ...
  public void paint(Graphics g) {
    ...
    int transX =
        (int) g.getFontMetrics().getStringBounds(getText(), g).getWidth() + getInsets().left;

    if (TEXT_BACK != null && TEXT_BACK.getWidth(null) > 0) {
      for (int i = 0; i < transX; i += TEXT_BACK.getWidth(null)) g.drawImage(TEXT_BACK, i, 0, null);
    }
