...
import com.tle.reporting.oda.ui.jdbc.ui.model.ChildrenAllowedNode;
import com.tle.reporting.oda.ui.jdbc.ui.model.DBNodeUtil;
import com.tle.reporting.oda.ui.jdbc.ui.model.FilterConfig;
import com.tle.reporting.oda.ui.jdbc.ui.model.FilterConfig.Type;
import com.tle.reporting.oda.ui.jdbc.ui.model.IDBNode;
import com.tle.reporting.oda.ui.jdbc.ui.model.RootNode;
...
public class SQLDataSetEditorPage extends DataSetWizardPage
{
	...
	private static String DEFAULT_MESSAGE = TLEOdaPlugin.getResourceString2("dataset.new.query");//$NON-NLS-1$	
	...
	{
		DEFAULT_MESSAGE = TLEOdaPlugin.getResourceString2("dataset.new.query");
	...
	private Control createDBMetaDataSelectionComposite(Composite parent)
	{
		...
		dataSourceLabel.setText(TLEOdaPlugin.getResourceString2("tablepage.label.availableItems"));//$NON-NLS-1$
		...
		Composite tablescomposite)
	{
		...
		schemaLabel.setText(TLEOdaPlugin.getResourceString2("tablepage.label.schema"));
		...
		FilterLabel.setText(TLEOdaPlugin.getResourceString2("tablepage.label.filter"));
		...
		selectTypeLabel.setText(TLEOdaPlugin.getResourceString2("tablepage.label.selecttype"));
		...
		findButton.setText(TLEOdaPlugin.getResourceString2("tablepage.button.filter"));//$NON-NLS-1$
			...
			public void widgetSelected(SelectionEvent event)
			{
				PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable()
				{
					public void run()
					{
						...
						DBNodeUtil.createTreeRoot(availableDbObjectsTree, new RootNode(dataSetDesign
							.getDataSourceDesign().getName()), fc);
					}
				});
			}
		});
		...
		if( supportsSchema )
		{
			String allFlag = TLEOdaPlugin.getResourceString2("tablepage.text.All");
		...
		{
			fc = populateFilterConfig();
			DBNodeUtil.createTreeRoot(availableDbObjectsTree, new RootNode(dataSetDesign.getDataSourceDesign()
				.getName(), allSchemaNames), fc);
		}
		else
		{
			DBNodeUtil.createRootTip(availableDbObjectsTree,
				new RootNode(dataSetDesign.getDataSourceDesign().getName()));
		}
	}
	...
	private FilterConfig populateFilterConfig()
	{
		...
		FilterConfig result = new FilterConfig(schemaName, type, namePattern, isShowSystemTable, maxSchemaCount,
	...
	private void setupIdentifierQuoteStringCheckBox(Group group)
	{
		...
		identifierQuoteStringCheckBox.setText(TLEOdaPlugin.getResourceString2("tablepage.button.dnd")); //$NON-NLS-1$
	...
	private void setupShowSystemTableCheckBox(Group group)
	{
		...
		showSystemTableCheckBox.setText(TLEOdaPlugin.getResourceString2("tablepage.button.showSystemTables")); //$NON-NLS-1$
	...
	private void setupIncludeSchemaCheckBox(Group group)
	{
		...
		includeSchemaCheckBox.setText(TLEOdaPlugin.getResourceString2("tablepage.button.includeSchemaInfo")); //$NON-NLS-1$
	...
	private String getTextToInsert()
	{
		...
		if( selection != null && selection.length > 0 )
		{
			...
			{
				IDBNode dbNode = (IDBNode) element.getData();
