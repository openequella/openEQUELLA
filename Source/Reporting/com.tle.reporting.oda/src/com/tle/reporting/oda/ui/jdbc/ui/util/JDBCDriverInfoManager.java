...
/**
 * The JDBCDriverInfoManager manage the information of jdbc drivers read from
 * the org.eclipse.birt.report.data.oda.jdbc.driverInfo extensions. The
...
public class JDBCDriverInfoManager
{
	...
	static public JDBCDriverInformation[] getDrivers()
	{
		...
		IConfigurationElement[] configElements = Platform.getExtensionRegistry().getConfigurationElementsFor(
			OdaJdbcDriver.Constants.DRIVER_INFO_EXTENSION);
		if( configElements != null )
		{
			for( IConfigurationElement configElement : configElements )
			{
				if( configElement.getName().equals(OdaJdbcDriver.Constants.DRIVER_INFO_ELEM_JDBCDRIVER) )
				{
					drivers.add(newJdbcDriverInfo(configElement));
				}
			}
		}
	...
	/**
	 * Creates a new JDBCDriverInformation instance based on a driverInfo
	 * extension element
	 * 
	 * @param driverClass
	 * @return
	...
	{
		JDBCDriverInformation driverInfo = JDBCDriverInformation.newInstance(configElement
			.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_DRIVERCLASS));
		driverInfo.setDisplayName(configElement.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_NAME));
		driverInfo.setUrlFormat(configElement.getAttribute(OdaJdbcDriver.Constants.DRIVER_INFO_ATTR_URLTEMPL));
		return driverInfo;
	}
