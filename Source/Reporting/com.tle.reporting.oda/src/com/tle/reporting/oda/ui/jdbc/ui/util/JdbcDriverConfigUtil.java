...
public class JdbcDriverConfigUtil
{
	...
	static
	{
		Map map = Utility.getPreferenceStoredMap(TLEOdaPlugin.DELETED_JAR_MAP_PREFERENCE_KEY);
		...
		Utility.setPreferenceStoredMap(TLEOdaPlugin.DELETED_JAR_MAP_PREFERENCE_KEY, new HashMap());
	}

	/**
	 * Gets a list of possible driver files under the oda.jdbc plugin's
	 * "drivers" directory Returned file list has been filtered by file type.
	 * Only JAR and ZIP files are expected
	 * 
	 * @return driverFiles
	 */
	public static List getDriverFiles()
	{
		try
		{
			...
			Map deletedJars = Utility.getPreferenceStoredMap(TLEOdaPlugin.DELETED_JAR_MAP_PREFERENCE_KEY);
