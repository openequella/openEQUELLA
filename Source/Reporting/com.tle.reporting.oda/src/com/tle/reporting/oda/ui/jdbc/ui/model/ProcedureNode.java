...
public class ProcedureNode extends ChildrenAllowedNode implements Comparable<ProcedureNode>
{
	...
	static
	{
		...
		reg.put(PROCEDURE_ICON, ImageDescriptor.createFromFile(TLEOdaPlugin.class, "icons/column.gif"));//$NON-NLS-1$
	}
	...
	{
		/**
		 * In our case, 2 <code>ProcedureNode</code> instances need to be
		 * compared
		 * <p>
		...
		if( rs != null )
		{
			try
			{
				...
				while( rs.next() )
				{
					...
					if( columnName == null )
					{
						// if the column name cannot retrieved ,give the unique
