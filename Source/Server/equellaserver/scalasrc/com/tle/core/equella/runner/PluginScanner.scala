...
object PluginScanner {
  ...
  case class JPFLibrary(id: String,
                        libType: String,
                        path: String,
  ...
  def parseJPF(f: URL, base: File): ParsedJPF = {
    ...
    val (extDeps, deps) = root
      ...
      .map { e =>
        (e.getAttributeValue("plugin-id"),
         e.getAttributeValue("exported", "false") == "true")
      }
    ...
    def convertOne(already: Set[String],
      ...
      if (already.contains(pId)) (already, processed)
        ...
        parsedMap
          ...
          .map {
            case pjpf =>
              val (a, l) = convertAll(parsedMap,
                                      already + pId,
                                      processed,
                                      pjpf.internalDeps.map(_._1))
              (a, pjpf :: l)
          }
          .getOrElse {
  ...
  def depsWithExports(d: Set[String],
    ...
    if (newDeps.isEmpty) added
      ...
      val exportedNew = newDeps.flatMap(
        s =>
          parsedMap
            .get(s)
            .map(_.internalDeps.filter(_._2).map(_._1))
            .getOrElse(Set.empty))
  ...
  def scanForPlugins(registry: PluginRegistry,
    ...
    if (devMode) {
      ...
      convertAll(manifestMap, Set.empty, List.empty, manifestMap.keys)._2
        .foreach { jpf =>
          val jpfBase = jpf.baseFile
          val classesDir = jpfBase / "target/scala-2.12/classes"
          val codeLibrary =
            JPFLibrary("code", "code", classesDir.toURI.toString, Some("*"))
          val resourcesDir = Option(jpfBase / "resources").filter(_.isDirectory)
          val resLibrary = resourcesDir.map(d =>
            JPFLibrary("resources", "resources", d.toURI.toString, None))
          val jpfJars = (jpfBase / "target/jpflibs" * "*.jar").get.map(j =>
            JPFLibrary(j.getName, "code", j.toURI.toString, Some("*")))
          val (id, manXml) =
            modifyManifestLibraries(jpfBase / "plugin-jpf.xml",
  ...
  def modifyManifestLibraries(f: File,
                              jars: Iterable[JPFLibrary]): (String, String) = {
    val x = saxBuilder().build(f)
    val root = x.getRootElement
    val pluginId = root.getAttribute("id").getValue
    Option(root.getChild("requires")).foreach { r =>
      val newchildren = r
        .getChildren()
        .filterNot(_.getAttribute("plugin-id").getValue.contains(":"))
      if (newchildren.isEmpty) root.removeChild("requires")
      else r.setContent(newchildren)
    }
    if (jars.nonEmpty) {
      ...
      jars.foreach { f =>
        ...
        f.export.foreach(p =>
          lib.addContent(new Element("export").setAttribute("prefix", p)))
