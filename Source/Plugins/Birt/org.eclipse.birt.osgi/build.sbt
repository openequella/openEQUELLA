...
libraryDependencies := Seq(
  "com.github.equella.reporting" % "birt-osgi" % "3.7.2" artifacts Artifact(
    "birt-osgi",
...
ivyConfigurations := overrideConfigs(BirtOsgi, CustomCompile)(
...
resourceGenerators in Compile += Def.task {
  ...
  val ur = update.value
  val pluginJars = Classpaths
    .managedJars(BirtOsgi, Set("jar"), ur)
    .files
    .filter(_.getName.endsWith(".jar"))
  val unzipped =
    IO.unzip(ur.select(artifact = artifactFilter(extension = "zip")).head,
             baseBirt)
