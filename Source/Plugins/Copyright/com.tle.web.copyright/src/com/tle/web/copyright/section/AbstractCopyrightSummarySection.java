		...
		AttachmentSelectorEventListener
{
		...
		PortionDisplay portionDisplay, boolean holdingIsThis, boolean canActivate)
	{
		...
		for( Section section : portion.getSections() )
		{
			...
			if( attachment != null )
			{
				...
				if( session != null && status != ActivateRequest.TYPE_INACTIVE )
				{
					if( activationService.attachmentIsSelectableForCourse(copyrightService.getActivationType(),
						attachUuid, getCourseCode(context, session, integrating)) )
					{
						HtmlComponentState addState = new HtmlComponentState();
						addState.setClickHandler(events.getNamedHandler("addAlreadyActivated", portionItem.getItemId(),
							attachUuid));
	...
	public void addAlreadyActivated(SectionInfo info, ItemId itemId, String attachmentId)
	{
		...
		if( session != null )
		{
			...
			final Item item = (Item) viewableItem.getItem();

			final String courseCode = getCourseCode(info, session, integrationService.isInIntegrationSession(info));
			final List<ActivateRequest> requests = activationService.getAllCurrentAndPendingActivations(
				getCopyrightServiceImpl().getActivationType(), attachmentId);

			// FIXME: we need some logic to pull out the best activate request
			// in the case there is > 1
			final ActivateRequest activateRequest = requests.stream()
				.filter(r -> r.getCourse().getCode().equals(courseCode)).findFirst().orElse(requests.get(0));

			final Attachment attachment = copyrightWebService.getAttachmentMap(info, item).get(
			...
			if( cls != null && cls.isApplicable(info) )
			{
				for( String folder : cls.getSelectedFolders(info) )
				{
					addResource(
						info,
						new SelectedResource(item.getItemId(), attachment, selectionService.findTargetFolder(info,
							folder), null), activateRequest);
