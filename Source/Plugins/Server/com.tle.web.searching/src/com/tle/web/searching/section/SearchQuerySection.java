		...
		BlueBarEventListener
{
	...
	public void treeFinished(String id, SectionTree tree)
	{
		...
		UpdateDomFunction editQuery = ajax.getAjaxUpdateDomFunction(tree, this, events.getEventHandler("editQuery"),
			ajax.getEffectFunction(EffectType.FADEOUTIN_ONRESULTS), DIV_QUERY);
		...
		onSearchFunctions.add(CallAndReferenceFunction.get(
			searchResults.getResultsUpdater(tree, events.getEventHandler("saveQuery"), DIV_QUERY), searchButton));
		...
		collectionList.addChangeEventHandler(new OverrideHandler(updateInterface, changedWhereUpdaters, collectionList
			.createGetExpression(), currentHidden.createGetExpression()));

		clearQueryButton.setClickHandler(new OverrideHandler(clearQuery));

		editQueryButton.setClickHandler(new StatementHandler(
	...
	public DefaultSearch createDefaultSearch(SectionInfo info, boolean includeQuery)
	{
		...
		if( session != null )
		{
			if( !session.isAllCollections() )
			{
				...
				// UUID return results
				final Collection<ItemDefinition> collections = collectionService.getMatchingSearchableUuid(session
					.getCollectionUuids());
	...
	public void setCollection(SectionInfo info, String collectionUuid)
	{
		...
		collectionList.setSelectedStringValue(info, whereModel.createWhere(collectionUuid, WithinType.COLLECTION)
			.getValue());
	...
	public void addBlueBarResults(RenderContext context, BlueBarEvent event)
	{
		...
		{
			event.addScreenOptions(new SettingsRenderer(LABEL_NONLIVE, renderSection(context, includeNonLive),
