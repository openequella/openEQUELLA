...
public class IpAddressInputRenderer extends AbstractInputRenderer implements JSDisableable, JSValueComponent
{
	private static final PluginResourceHelper helper = ResourcesService.getResourceHelper(IpAddressInputRenderer.class);

	private static final CssInclude CSS = CssInclude.include(helper.url("css/ipaddress.css")).make();

	private static final IncludeFile JS = new IncludeFile(helper.url("js/ipaddress.js"), JQueryCore.PRERENDER);
	...
	protected void writeEnd(SectionWriter writer) throws IOException
	{
		...
		while( fieldCount < 4 )
		{
			...
			ipLimit.addEventStatements("input", Js.call_s(SET_F, fieldCount, this, ipLimit.getElementId(writer)));
		...
		maskLimit.addEventStatements("input", Js.call_s(SET_F, 4, this, maskLimit.getElementId(writer)));
