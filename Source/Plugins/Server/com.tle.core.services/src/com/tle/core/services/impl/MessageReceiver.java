...
public class MessageReceiver implements Runnable
{
			...
			try( Socket socket = new Socket(host, port) )
			{
				...
				while( !die )
				{
					...
					if( messageOffset != -1 && cmo != messageOffset )
					{
						...
						{
							LOGGER.trace(MessageFormat.format("Expected offset: {0}, Current offset: {1}",
								messageOffset, cmo));
						}
						LOGGER
							.warn("Message offset greater than known offset. Messages may have been missed. Offset expected: "
	...
	{
		LOGGER.error(MessageFormat.format("Error communicating with NODE: {0}, Error message was: {1}", nodeId,
			e.getMessage()));
	}
