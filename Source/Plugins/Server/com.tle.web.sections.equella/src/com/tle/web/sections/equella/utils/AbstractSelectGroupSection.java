		...
		AbstractPrototypeSection<M> implements HtmlRenderer
{
	...
	public void registered(String id, SectionTree tree)
	{
		...
		{
			groupList = (MultiSelectionList<GroupBean>) componentFactory.createSingleSelectionList(id, "ul", tree);
		}

		final GroupSearchWithExclusionsListModel listModel = new GroupSearchWithExclusionsListModel(query, userService,
			groupFilter, SEARCH_LIMIT);
		groupList.setListModel(listModel);

		if( !Check.isEmpty(groupFilter) )
		{
			groupFilterTooltip.addReadyStatements(TooltipModule.getTooltipStatements(new JQuerySelector(Type.CLASS,
				"filterstooltip"), new JQuerySelector(Type.CLASS, "displayfilters"), 250, 110, 0, false));
	...
	protected abstract SelectedGroup createSelectedGroup(SectionInfo info, String uuid, String displayName);
	...
	{
		public GroupSearchWithExclusionsListModel(TextField query, UserService userService, Set<String> groupFilter,
			int limit)
		{
