...
public class RootRemoteCachingSection extends OneColumnLayout<RootRemoteCachingSection.RemoteCachingModel>
{
	...
	public void registered(String id, SectionTree tree)
	{
		...
		removeUserGroupButton.setClickHandler(Js.handler(Js.call_s(removeFunc)).addValidator(
			new Confirm(LABEL_CONFIRM_REMOVE)));
	...
	public class RemoteCachingTreeModel implements HtmlTreeModel
	{
		...
		public List<HtmlTreeNode> getChildNodes(SectionInfo info, String id)
		{
			...
			if( id == null )
			{
				...
				final RemoteCachingTreeNode nctn = new RemoteCachingTreeNode(uuid, getText(info, node), node.getNodes()
					.isEmpty(), model.getSelectedNode().equals(uuid), true);
				list.add(nctn);
			}
			else
			{
				...
				if( node != null )
				{
					for( Node n2 : node.getNodes() )
					{
						...
						final RemoteCachingTreeNode nctn = new RemoteCachingTreeNode(uuid, getText(info, n2), n2
							.getNodes().isEmpty(), model.getSelectedNode().equals(uuid), n2.getId() == null);
						list.add(nctn);
					}
				}
				else
	...
	public class RemoteCachingTreeNode implements HtmlTreeNode
	{
		...
		{
			final ImageRenderer icon = (group ? new ImageRenderer(URL_GROUP_ICON, LABEL_GROUP) : new ImageRenderer(
				URL_USER_ICON, LABEL_USER));
			...
			linkRenderer.setNestedRenderable(CombinedRenderer.combineMultipleResults(icon, new LabelRenderer(
				new TextLabel(textName))));
			...
			link.setClickHandler(new StatementHandler(Js.call_s(SELECT, Jq.$(usrGrpTree), Jq.$(divRenderer),
				selectNodeFunc, id)));
