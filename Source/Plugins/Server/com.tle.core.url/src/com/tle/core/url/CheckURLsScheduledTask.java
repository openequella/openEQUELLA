...
import static com.tle.core.url.URLCheckerPolicy.TRIES_UNTIL_DISABLED;
import static com.tle.core.url.URLCheckerPolicy.TRIES_UNTIL_WARNING;
		...
		while( bi.hasNext() || !workingset.isEmpty() )
		{
			...
			for( Iterator<ListenableFuture<ReferencedURL>> iter = workingset.iterator(); iter.hasNext(); )
			{
				...
				if( f.isDone() )
				{
					...
					switch( rurl.getTries() )
					{
						case TRIES_UNTIL_WARNING:
							eventType = URLEventType.URL_WARNING;
							break;
						case TRIES_UNTIL_DISABLED:
