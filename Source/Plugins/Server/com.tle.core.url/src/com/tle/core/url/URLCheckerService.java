...
public class URLCheckerService
{
	...
	public URLCheckerService()
	{
		...
		bc.setAllowPoolingConnection(false); // Avoid keep-alive
		bc.setCompressionEnabled(true);
		bc.setUseProxyProperties(true);
		bc.setFollowRedirects(true);
		bc.setMaximumConnectionsPerHost(2);
		bc.setMaximumConnectionsTotal(200);
		bc.setMaximumNumberOfRedirects(25);
		...
		bc.setRequestTimeoutInMs(60000);
	...
	private ListenableFuture<Pair<ReferencedURL, Boolean>> checkUrl(final ReferencedURL rurl, final boolean head)
	{
		...
		{
			throw new RuntimeException("Error checking URL " + rurl.getUrl());
		}
	}
