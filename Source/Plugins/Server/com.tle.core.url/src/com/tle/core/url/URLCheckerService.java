...
public class URLCheckerService
{
	...
	public static enum URLCheckMode
	{
		...
		/**
		 * Does not check the URL and relies entirely on past checks. Does not
		 * invoke the clustered task so that it can happen in the same
		 * transaction.
		 */
	...
	public boolean isUrlDisabled(String url)
	{
		...
		{
			return URLCheckerPolicy.isUrlDisabled(getUrlStatus(url, URLCheckMode.RECORDS_ONLY));
		}
		return true;
	}
	...
	public ReferencedURL getUrlStatus(final String url, final URLCheckMode mode)
	{
		...
		// not need checking.
		if( mode.useRecords && !URLCheckerPolicy.requiresChecking(rurl) )
		{
			return rurl;
		}
	...
	private ListenableFuture<Pair<ReferencedURL, Boolean>> checkUrl(final ReferencedURL rurl, final boolean head)
	{
		AsyncHandler<Pair<ReferencedURL, Boolean>> handler = new AsyncHandler<Pair<ReferencedURL, Boolean>>()
		{
			...
			public STATE onStatusReceived(HttpResponseStatus responseStatus)
			{
				...
				// urls returning 405 will be marked as good as per EQ-1044.
				if( (code >= HTTP_OK && code < HTTP_MULT_CHOICE) || code == HTTP_UNAUTHORIZED
					|| code == HTTP_PAYMENT_REQUIRED || code == HTTP_BAD_METHOD )
				{
					if( LOGGER.isDebugEnabled() )
					{
		...
		try
		{
			RequestBuilder requestBuilder = new RequestBuilder(head ? "HEAD" : "GET").setUrl(rurl.getUrl());

			return Futures.transform(
				new AsyncHttpToGuavaAdapter<Pair<ReferencedURL, Boolean>>(client.executeRequest(requestBuilder.build(),
					handler)), new AsyncFunction<Pair<ReferencedURL, Boolean>, Pair<ReferencedURL, Boolean>>()
				{
					@Override
