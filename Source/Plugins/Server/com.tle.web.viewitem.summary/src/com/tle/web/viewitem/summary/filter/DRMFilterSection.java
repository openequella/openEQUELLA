		...
		DRMFilter
{
	...
	/**
	 * unhide what we may already have hidden - reconsider whether to hide anon
	 * 
	 * @param info
	 */
	private void unhideActionSections(SectionInfo info)
	{
		if( majorActionsGroupSection.getChildSectionIds(info) != null )
		{
			for( SectionId renda : majorActionsGroupSection.getChildSectionIds(info) )
			{
				...
				{
					((HideableFromDRMSection) renda).unhideSection(info);
				}
			}
		}

		minorActionsGroupSection.unhideSection(info);
	}
	 ...
	 * @param info
	 */
	private void hideActionSections(SectionInfo info)
	{
		if( majorActionsGroupSection.getChildSectionIds(info) != null )
		{
			for( SectionId renda : majorActionsGroupSection.getChildSectionIds(info) )
			{
				if( renda instanceof HideableFromDRMSection )
				{
					((HideableFromDRMSection) renda).hideSection(info);
				}
			}
		}
		minorActionsGroupSection.hideSection(info);
	}
