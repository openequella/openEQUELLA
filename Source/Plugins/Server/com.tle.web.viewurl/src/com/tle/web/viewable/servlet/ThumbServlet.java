...
public class ThumbServlet extends HttpServlet
{
		...
		IOException
	{
		...
		if( "$".equals(path[1]) )
		{
			...
			{
				final FileContentStream stream = fileSystemService.getContentStream(staging, thumbPath, "image/jpeg");
		...
		else
		{
			...
			final ItemId itemId = new ItemId(uuid, version == 0 ? itemService.getLiveItemVersion(uuid) : version);
			...
			else
			{
				...
				final FileContentStream stream = fileSystemService.getContentStream(thumb.getHandle(),
					thumb.getLocalFile(), mimeType);
				contentStreamWriter.outputStream(request, response, stream);
			}
		}
	}
	...
	@Nullable
	private ThumbRef getThumbRef(HttpServletRequest request, ViewableItem<Item> vitem, @Nullable String attachmentUuid,
		@Nullable GalleryParameter gallery)
	{
		final SectionInfo info = sectionsController.createInfo("/viewitem/viewitem.do", request, null, null, null,
	...
	private void redirectToPlaceholder(HttpServletRequest request, HttpServletResponse response,
		@Nullable GalleryParameter gParam) throws IOException
	{
		final String path;
		if( gParam == null )
		{
