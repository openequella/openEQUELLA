 ...
 * virtualised, its value is encoded into the OAI Set identifier as
 * UUID:VirtualisedValue
 */
...
public class OAICatalog extends AbstractCatalog
{
	...
	@Override
	public Vector<?> getSchemaLocations(String identifier) throws IdDoesNotExistException, NoMetadataFormatsException, // NOSONAR
		OAIInternalServerError
	{
	...
	public Map<?, ?> listIdentifiers(String from, String until, String set, String metadataPrefix)
		throws BadArgumentException, CannotDisseminateFormatException, NoItemsMatchException, NoSetHierarchyException,
		OAIInternalServerError
	{
	...
	@Override
	public Map<?, ?> listIdentifiers(String resumption) throws BadResumptionTokenException, OAIInternalServerError
	{
	...
	{
		List<VirtualisableAndValue<DynaCollection>> dcs = dynaCollectionService.enumerateExpanded(OAI_USAGE);
		switch( dcs.size() )
		{
			case 0:
				return;
	...
	private Map<String, Map<?, ?>> list(DefaultSearch request, String format, int start, Handler handler)
		throws NoRecordsMatchException, OAIInternalServerError
	{
		try
		{
			...
			FreetextSearchResults<FreetextResult> results = freeTextService.search(request, start, MAX_RESULTS);
