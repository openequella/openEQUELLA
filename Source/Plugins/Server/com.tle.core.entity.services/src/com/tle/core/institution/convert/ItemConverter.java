...
public class ItemConverter extends AbstractConverter<ItemConverter.ItemConverterInfo>
{
	...
	@Override
	public void deleteIt(final TemporaryFileHandle staging, final Institution institution,
		final ConverterParams params, final String cid)
	{
		...
		message.setKey("institutions.converter.items.deletemsg"); //$NON-NLS-1$
		...
		String cid) throws IOException
	{
		...
		TransactionThreadPool threadPool = newThreadPool();
				...
				public void run()
				{
					try
					{
						...
						final SubTemporaryFile importItemFolder = new SubTemporaryFile(allImportItemsFolder,
							dataFolderPath);
						final ItemFile itemFolder = new ItemFile(item);
						...
						runExtras(info, getXStream(), new SubTemporaryFile(allImportItemsFolder, dataFolderPath
							+ "-extra"), true);
						itemDao.flush();
						itemDao.clear();
	...
	private TransactionThreadPool newThreadPool()
	{
		return new TransactionThreadPool(this, Math.max(4, Runtime.getRuntime().availableProcessors()));
	}
	...
	@Override
	public void exportIt(final TemporaryFileHandle staging, final Institution institution,
		final ConverterParams params, String cid) throws IOException
	{
		...
		TransactionThreadPool pool = newThreadPool();
		final Map<Object, Object> sharedState = Collections.synchronizedMap(Maps.newHashMap());
			...
			pool.doInTransaction(new Runnable()
			{
				...
				public void run()
				{
					try
					{
						...
						final SubTemporaryFile exportedItemFolder = new SubTemporaryFile(bucketFolder, Long
							.toString(itemId));
						...
						runExtras(info, getXStream(), new SubTemporaryFile(bucketFolder, Long.toString(itemId)
							+ "-extra"), false);
	...
	void exportXml(ConverterParams params, Item item, SubTemporaryFile exportedSystemItemFolder, Institution institution)
	{
		try
		{
	...
	public void setPluginService(PluginService pluginService)
	{
		...
		itemExtrasTracker = new PluginTracker<ItemExtrasConverter>(pluginService, ServiceConstants.SERVICE_PLUGIN_ID,
