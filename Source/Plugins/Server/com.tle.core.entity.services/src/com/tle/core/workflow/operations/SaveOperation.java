...
public class SaveOperation extends AbstractWorkflowOperation implements DeleteHandler
{
	...
	public boolean execute()
	{
		...
		if( unlock && pack != null )
		{
			lockingService.unlockItem(pack.getItem(), false);
	...
	{
		Wizard wizard = getItemdef().getWizard();
		if( wizard != null )
		{
			final String saveScript = wizard.getSaveScript();
			if( !Check.isEmpty(saveScript) )
			{
				workflowOpService.executeScript(saveScript, SAVE_SCRIPT_NAME, createScriptContext(null), true);
			}
		}
	}
	...
	private void saveToRepository(ItemPack<Item> itemPack, boolean doUpdate)
	{
		...
		if( !doUpdate
			&& aclManager.filterNonGrantedPrivileges(item.getItemDefinition(), Arrays.asList(CREATE_ITEM_PRIV))
				.isEmpty() )
		{
		...
		workflowOpService.updateMetadataBasedSecurity(getItemXml(), getItem());
		...
		{
			LOGGER.warn("There was an error updating referenced urls for submitted item " + getId(), e);
		}
	...
	protected void saveAttachments(Item item)
	{
		try
		{
			...
			if( staging != null )
			{
				...
				params.addAfterCommitHook(WorkflowParams.COMMIT_HOOK_PRIORITY_MEDIUM, new Runnable()
				{
					@Override
					public void run()
					{
