...
/**
 * This migration only applies to SQL Server. Our custom types were hardcoded to
 * varchar, but there should be no varchar columns, only nvarchar.
 * 
 * @author Aaron
 */
...
public class ConvertVarcharDatabaseMigration extends AbstractHibernateSchemaMigration
{
	...
	private static final String keyPrefix = PluginServiceImpl.getMyPluginId(ConvertVarcharDatabaseMigration.class)
		+ ".";
	...
	{
		return new MigrationInfo(keyPrefix + "migration.v52.convertvarchar");
	}
			...
			session.doWork(new Work()
			{
				...
				public void execute(Connection connection) throws SQLException
				{
					...
					for( String tableName : tableNames )
					{
						...
						{
							String sizeStr = ntextCol.size == Integer.MAX_VALUE ? "max" : Integer
								.toString(ntextCol.size);
