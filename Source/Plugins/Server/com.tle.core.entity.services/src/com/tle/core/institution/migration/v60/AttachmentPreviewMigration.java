...
public class AttachmentPreviewMigration extends AbstractHibernateSchemaMigration
{
	...
	private static String prefix = PluginServiceImpl.getMyPluginId(AttachmentPreviewMigration.class) + "."; //$NON-NLS-1$
	@Inject
	...
	{
		return new MigrationInfo(prefix + "migration.v60.prvattach.title");
	}
	...
	protected void executeDataMigration(HibernateMigrationHelper helper, MigrationResult result, Session session)
	{
		...
		ScrollableResults scroll = session.createQuery(
			"from Attachment where data like '%<string>" + KEY_PREVIEW + "</string>%'").scroll();
		while( scroll.next() )
		{
