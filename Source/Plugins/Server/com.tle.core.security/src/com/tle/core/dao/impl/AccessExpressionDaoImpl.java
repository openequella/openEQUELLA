...
public class AccessExpressionDaoImpl extends GenericDaoImpl<AccessExpression, Long> implements AccessExpressionDao
{
	...
	public AccessExpression retrieveOrCreate(String expression)
	{
		...
		List<AccessExpression> list = getHibernateTemplate().findByNamedParam(
	...
	public void deleteOrphanedExpressions()
	{
		getHibernateTemplate().execute(new HibernateCallback()
		{
			...
			{
				Query query = session.createQuery("from AccessExpression expression"
	...
	{
		return changedExpression(SecurityConstants.getRecipient(Recipient.USER, fromUserId),
	...
	{
		return changedExpression(SecurityConstants.getRecipient(Recipient.GROUP, fromGroupId),
	...
	private Map<Long, Long> changedExpression(final String findThis, final String replaceWithThis)
	{
		...
		getHibernateTemplate().execute(new TLEHibernateCallback()
		{
			...
			{
				// NOTE THAT THIS IS NOT HQL!!! IT IS PRETTY MUCH SQL!!!
				String sql = "SELECT {ae.*} FROM access_expression ae WHERE ae.id IN"
					+ " (SELECT access_expression_id FROM access_expression_expression_p"
				...
				{
					AccessExpression newAe = retrieveOrCreate(ae.getExpression().replace(findThis, replaceWithThis));
