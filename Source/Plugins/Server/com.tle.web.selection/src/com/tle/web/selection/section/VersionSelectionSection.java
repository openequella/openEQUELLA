...
public class VersionSelectionSection
	extends
		AbstractPrototypeSection<VersionSelectionSection.VersionSelectionTableModel> implements HtmlRenderer
{
		...
		if( folder.getResourceCount() > 0 )
		{
			...
			for( SelectedResource res : folder.getResources().values() )
			{
				...
				if( allowVersionChoice )
				{
					...
					versionCell.addContent(versionSelections.getListState(info, key,
						getVersionOptionsForSelectedResource(res), dvo));
					versionCell.addClasses("version");
				}

				final HtmlComponentState unselect = new HtmlComponentState();
				unselect.setClickHandler(new OverrideHandler(removeFunc, key).addValidator(Js
					.confirm(TABLE_CONFIRM_REMOVE_LABEL)));
				TableCell unselectCell = new TableCell(unselect);
				unselectCell.addClasses("unselect");

				tableState.addRow(res.getTitle(), versionCell, unselectCell);
			}
