...
public class TleSessionFilter extends AbstractWebFilter
{
		...
		throws ServletException, IOException
	{
		...
		try
		{
			...
			{
				LOGGER.info("No existing user state but received EXISTING result");
				resultType = Result.GUEST_NO_SESSION_CREATION;
			}

			switch( resultType )
			{
				case GUEST:
				case GUEST_NO_SESSION_CREATION:
					if( existingUserState == null || !existingUserState.isGuest() )
					{
						userService.logoutToGuest(userService.getWebAuthenticationDetails(request),
							resultType == Result.GUEST);
					}
					break;
				case LOGIN_NO_SESSION_CREATION:
				case LOGIN_SESSION:
