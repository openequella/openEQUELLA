...
public abstract class AbstractCacheSelectionAction extends TLEAction
{
	...
	public AbstractCacheSelectionAction(String titleKey, String descriptionKey, String errorTitleKey,
		String errorMsgKey, Backend backend, FileListPanel fileList)
	{
	...
	public final void actionPerformed(ActionEvent e)
	{
		...
		{
			GlassProgressWorker<?> worker = new GlassProgressWorker<File>(CurrentLocale.get(
				"action.abstractcache.progress", file.getName()), (int) file.getSize(), false) //$NON-NLS-1$
			{
				@Override
				public File construct() throws Exception
				{
					return backend.cacheFileForEditing(file, new ProgressMonitorCallback()
					{
						@Override
	...
	protected void handleException(Exception ex)
	{
		...
		JOptionPane.showMessageDialog(fileList, CurrentLocale.get(errorMsgKey), CurrentLocale.get(errorTitleKey),
