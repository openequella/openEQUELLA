...
public class DownloadAction extends TLEAction
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if( fc.showSaveDialog(fileList) == JFileChooser.APPROVE_OPTION )
		{
			...
			{
				JOptionPane.showMessageDialog(fileList, CurrentLocale.get("action.download.notdir.message"),
					CurrentLocale.get("action.download.notdir.title"), JOptionPane.WARNING_MESSAGE);
				return;
			}
				...
				CurrentLocale.get("action.download.progress.unknown"), -1, true)
			{
				...
				private void download(File destDir, List<FileInfo> files) throws IOException
				{
					for( FileInfo fileInfo : files )
					{
						...
						{
							setMessage(CurrentLocale.get("action.download.progress.downloading", destFile.getName()));
							backend.downloadFile(fileInfo, destFile,
								new InterruptAwareGlassPaneProgressMonitorCallback(this));
						}
					}
				}
				...
				private long getFileSize(List<FileInfo> files)
				{
					...
					{
						total += file.isDirectory() ? getFileSize(backend.listFiles(file.getFullPath())) : file
							.getSize();
					}
					return total;
				}
				...
				public void exception()
				{
					...
					{
						ex.printStackTrace();
						JOptionPane.showMessageDialog(fileList, CurrentLocale.get("action.download.error.message"),
							CurrentLocale.get("action.download.error.title"), JOptionPane.ERROR_MESSAGE);
					}
				}
			};
