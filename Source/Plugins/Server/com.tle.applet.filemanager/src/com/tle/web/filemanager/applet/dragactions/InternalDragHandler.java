...
public class InternalDragHandler implements DropHandler
{
	...
	{
		final FileInfo draggedFile = (FileInfo) e.getTransferable()
			.getTransferData(FileInfoTransferable.fileInfoFlavor);
		...
		{
			menu.add(new TLEAction(action.getDisplayName(), action.getIcon())
			{
				@Override
