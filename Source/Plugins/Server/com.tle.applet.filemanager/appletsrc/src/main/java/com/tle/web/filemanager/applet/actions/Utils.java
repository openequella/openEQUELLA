...
public final class Utils
{
	...
	{
		Object[] options = new Object[]{CurrentLocale.get("common.overwrite.yes"), //$NON-NLS-1$
				CurrentLocale.get("common.overwrite.no"),}; //$NON-NLS-1$
		if( CurrentLocale.isRightToLeft() )
		{
			parent.applyComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
		}
		return JOptionPane.showOptionDialog(parent, CurrentLocale.get(messageKey),
			CurrentLocale.get("common.overwrite.title"), JOptionPane.YES_NO_OPTION, //$NON-NLS-1$
	...
	public static boolean filenamesClash(File[] files, List<FileInfo> infos)
	{
		return filenamesClashP(
			Lists.newArrayList(Collections2.transform(Lists.newArrayList(files), new Function<File, String>()
			{
				@Override
	...
	{
		return filenamesClashP(Lists.transform(Arrays.asList(files), new Function<String, String>()
		{
			@Override
	...
	{
		return !Collections.disjoint(files, Lists.transform(infos, new Function<FileInfo, String>()
		{
			@Override
