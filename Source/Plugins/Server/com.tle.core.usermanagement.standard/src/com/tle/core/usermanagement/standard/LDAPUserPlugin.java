/*
 * Created on Feb 25, 2005 For "The Learning Edge"
 */
...
import com.tle.core.services.user.LDAPService;
...
public class LDAPUserPlugin extends AbstractUserDirectory
{
	...
	{
		this.ldap = new LDAP((LDAPSettings) settings);
	...
	{
		return new Pair<ChainResult, Collection<GroupBean>>(ChainResult.CONTINUE, ldapService.getGroupsContainingUser(
			ldap, userID));
	}
		...
		final boolean recursive)
	{
		...
		return new Pair<ChainResult, Collection<UserBean>>(ChainResult.CONTINUE, ldapService.getUsersInGroup(ldap,
			query, parentGroupID, recursive));
	}
