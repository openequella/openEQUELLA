...
public class LDAP
{
	...
	public LDAP(LDAPSettings config)
	{
		this.config = config;
		...
		this.adminPassword = getString(config.getAdminPassword(), Constants.BLANK);
	...
	public <T> T searchFirstResult(DirContext ctx, Name base, Filter filter, HitsCollector<T> collector, boolean recurse)
