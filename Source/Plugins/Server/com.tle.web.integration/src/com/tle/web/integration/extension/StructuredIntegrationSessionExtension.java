...
public class StructuredIntegrationSessionExtension implements IntegrationSessionExtension
{
	...
	public void setupSession(SectionInfo info, SelectionSession session, SingleSignonForm form)
	{
		...
		if( structure != null && structure.length() > 0 )
		{
			...
			final ObjectNode structureJson;
			try
			{
				structureJson = (ObjectNode) mapper.readTree(structure);
			}
			catch( IOException e )
			{
			...
			struct.putAttribute("courseCode", nodeValue(structureJson, "code", null));
			recurseFolders(structureJson, struct, session);
