		...
		AbstractIntegrationService<GenericIntegrationService.GenericIntegrationData>
{
	...
	@Override
	public void select(SectionInfo info, final GenericIntegrationData data, SelectionSession session)
	{
		if( session.isSelectMultiple() || session.isForcePost() )
		{
			...
			try
			{
				DivRenderer divTag = new DivRenderer(new HiddenInput(data.getPrefix() + "links",
