		...
		HtmlRenderer
{
	...
	@Override
	public SectionResult renderHtml(RenderEventContext info) throws Exception
	{
		SelectionSession session = selectionService.getCurrentSelectionSession(info);
		...
		if( pageId != null && !pageId.equals(HtmlEditorService.CONTENT_DIRECTORY) )
		{
			...
			addGroup(pageAttachments, getPageGroup(staging, item, info, page, null, null, true, wizardState));
		}
		...
		if( fileSystemService.fileExists(staging, HtmlEditorService.CONTENT_DIRECTORY) )
		{
			addGroup(
				pageAttachments,
				getPageGroup(staging, item, info, null, resources.getString("upload.htmlcontrolattachments"),
	...
	{
		return getPageGroup(staging, item, context, null,
			resources.getString("upload.itemattachments"), "", false, wizardState); //$NON-NLS-1$//$NON-NLS-2$
	}

	private PageGroup getPageGroup(FileHandle staging, Item item, SectionContext context, HtmlAttachment page,
		String pageName, String pageFolder, boolean recurse, WizardStateInterface wizardState)
	{
		try
		{
			if( page != null )
			...
			return new PageGroup(pageName, summarise(item, attachments, context, wizardState));
		}
	...
	private List<AttachmentView> summarise(Item item, List<Attachment> attachments, SectionInfo info,
		WizardStateInterface wizardState)
	{
	...
	{
		final SelectionSession session = selectionService.getCurrentSelectionSession(info);
	...
	@EventHandlerMethod
	public void uploadFile(SectionInfo info)
	{
		...
		if( hasFile )
		{
			...
			final FileAttachment newb = myPagesService.uploadStream(info, wizardSessionId, pageId,
				PathUtils.fileencode(filename), description, fileUpload.getInputStream(info));
