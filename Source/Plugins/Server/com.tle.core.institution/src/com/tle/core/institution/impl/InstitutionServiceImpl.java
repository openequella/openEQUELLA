		...
		TaskStatusListener
{
	...
	public long getSchemaIdForInstitution(Institution institution)
	{
		...
		if( schemaId == null )
		{
			throw new IllegalArgumentException("Institution with id " + institution.getUniqueId()
				+ " has no schema entry");
		}
		return schemaId;
	}
	...
	private synchronized void setAllInstitutions(Map<Long, InstitutionStatus> map)
	{
		...
		{
			eventService.publishApplicationEvent(new InstitutionEvent(InstitutionEventType.UNAVAILABLE,
				newlyUnavailable));
		}
	}
	...
	{
		institutionTaskId = taskService.getGlobalTask(
			new BeanClusteredTask("InstitutionKeeper", true, InstitutionService.class, "createTask"), DEFAULT_TIMEOUT)
			.getTaskId();
