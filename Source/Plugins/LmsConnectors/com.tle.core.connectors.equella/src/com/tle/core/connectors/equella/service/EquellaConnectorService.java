...
public class EquellaConnectorService implements ConnectorRepositoryImplementation
{
		...
		boolean reverseSort) throws LmsUserNotFoundException
	{
		...
		final List<CustomAttachment> resources = attachmentDao
			.findResourceAttachmentsByQuery(query, !archived, sortHql);
	...
	private ConnectorContent convertAttachment(Item item, CustomAttachment attachment, String name, String owner)
	{
		...
		final ViewItemUrl attachmentUrl = attachmentResourceService.getViewableResource(info,
			viewableItemFactory.createNewViewableItem(item.getItemId()/*
																	 ...
																	 * targetItemVersion
																	 * )
																	 */), attachment).createDefaultViewerUrl();
		...
		if( !Check.isEmpty(targetAttachmentUuid) )
		{
			if( targetAttachmentUuid.equalsIgnoreCase("viewims.jsp") )
			{
				...
				List<ImsAttachment> ims = new UnmodifiableAttachments(itemService.getUnsecure(new ItemId(
					targetItemUuid, targetItemVersion))).getList(AttachmentType.IMS);
