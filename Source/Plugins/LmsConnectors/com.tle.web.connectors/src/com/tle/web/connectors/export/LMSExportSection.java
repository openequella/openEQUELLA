...
public class LMSExportSection extends AbstractContentSection<LMSExportSection.LMSExporterModel>
{
		...
		if( !checkErrors(context) )
		{
			...
			for( AttachmentRowDisplay row : attachmentRowDisplays )
			{
				...
				if( attachmentView != null )
				{
					...
					CheckboxRenderer attachmentCheckbox = new CheckboxRenderer(attachmentSelections.getBooleanState(
						context, attachmentUuid));
					if( model.isCopyrighted() )
					{
						...
						{
							checkBoxState.setClickHandler(new OverrideHandler(ajax.getAjaxUpdateDomFunction(getTree(),
								this, events.getEventHandler("filterCourses"), "lms-tree-ajax")));
						}
					...
					rowLi.setNestedRenderable(CombinedRenderer.combineResults(attachmentCheckbox,
						rowLi.getNestedRenderable()));
				}
			}
			...
			final Connector connector = model.getConnector();
			if( connector != null )
			{
	...
	public void registered(String id, SectionTree tree)
	{
		...
		connectorsList.addChangeEventHandler(new OverrideHandler(ajax.getAjaxUpdateDomFunction(tree, this,
			events.getEventHandler("loadTree"), "lms-tree-ajax"), true));
		connectorsList.setAlwaysSelect(false);

		SimpleValidator locationValidator = new SimpleValidator(Js.call(FUNCTION_LOCATION_SELECTED,
			folderSelections.getElementsExpression()));
		...
		publishButton.addClickStatements(events.getNamedHandler("publish").addValidator(locationValidator)
		...
		final JSCallAndReference doFilterCall = CallAndReferenceFunction.get(
			...
			for( String folderAndCourseId : selectedFolders )
			{
				...
				if( !Check.isEmpty(folderId) )
				{
					...
					for( String attachmentUuid : selectedResources )
					{
						...
						{
							locations.add(new KeyLabel(KEY_LOCATION_ATTACHMENT, attachment.getDescription(), folder
								.getName(), folder.getCourse().getName()));
						}
					...
					if( selectedSummary )
					{
						...
						if( !useSummaryMessage )
						{
							locations.add(new KeyLabel(KEY_LOCATION_SUMMARY, folder.getName(), folder.getCourse()
								.getName()));
						}
						else if( numResources == 1 )
						{
					...
					if( selectedContentPackage )
					{
						...
						if( !useSummaryMessage )
						{
							locations.add(new KeyLabel(KEY_LOCATION_PACKAGE, folder.getName(), folder.getCourse()
								.getName()));
						}
						else if( numResources == 1 )
						{
		...
		public List<HtmlTreeNode> getChildNodes(SectionInfo info, String id)
		{
			...
			else if( id.contains("$") )
			{
				...
				return Lists.transform(
					repositoryService.getFoldersForFolder(connector, CurrentUser.getUsername(), courseId, folderId),
					new FolderToTreeNodeTransform(info));
			}

			return Lists.transform(repositoryService.getFoldersForCourse(connector, CurrentUser.getUsername(), id),
				new FolderToTreeNodeTransform(info));
		}
	}
