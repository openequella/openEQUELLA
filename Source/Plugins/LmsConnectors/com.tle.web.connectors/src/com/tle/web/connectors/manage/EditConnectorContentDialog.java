...
public class EditConnectorContentDialog extends AbstractOkayableDialog<EditConnectorModel>
{
	...
	{
		getModel(info).setConnectorItemKey(key);
	...
	public void saveContent(SectionInfo info)
	{
		...
		String description = descriptionField.getValue(info);

		Connector connector = querySection.getConnector(info);
		try
		{
			repositoryService.editContent(connector, CurrentUser.getUsername(), model.getConnectorItemKey()
				.getContentId(), title, description);
			info.getRootRenderContext().setRenderedBody(
				new CloseWindowResult(jscall(getCloseFunction()), jscall(getOkCallback(), getModel(info)
					.getConnectorItemKey())));
		}
