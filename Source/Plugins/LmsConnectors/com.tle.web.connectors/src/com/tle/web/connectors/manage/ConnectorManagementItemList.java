		...
		StandardListSection<ConnectorManagementListEntry, StandardListSection.Model<ConnectorManagementListEntry>>
{
	...
	protected void customiseListEntries(RenderContext context, List<ConnectorManagementListEntry> entries)
	{
		...
		for( ConnectorManagementListEntry entry : entries )
		{
			...
			{
				ConnectorContent content = entry.getContent();
				ConnectorItemKey itemId = new ConnectorItemKey(content, querySection.getConnectorList()
					.getSelectedValue(context).getId());
				entry.getTag().setElementId(new SimpleElementId(DIV_PFX + itemId.toKeyString()));

				if( !selectionSection.isSelected(context, itemId) )
				{
					HtmlLinkState link = new HtmlLinkState(LABEL_BUTTON_SELECT, new OverrideHandler(selectCall, itemId));
					entry.addRatingAction(new ButtonRenderer(link).showAs(ButtonType.SELECT));
				}
				else
				{
					HtmlLinkState link = new HtmlLinkState(LABEL_BUTTON_UNSELECT, new OverrideHandler(removeCall,
						itemId));
	...
	public static class ConnectorManagementListEntry extends AbstractItemListEntry
	{
		...
		protected void setupMetadata(RenderContext context)
		{
			...
			final HtmlLinkState equellaResourceLink = new HtmlLinkState(new BundleLabel(item.getName(), item.getUuid(),
				bundleCache), itemUrls.createItemUrl(context, item.getItemId()));
			...
			if( !Check.isEmpty(equellaAttachmentUuid) )
			{
				...
				if( attachment != null )
				{
					...
					{
						String viewerId = Check.isEmpty(attachment.getViewer()) ? viewItemService
							.getDefaultViewerId(viewableResource) : attachment.getViewer();
			...
			if( connectorService.canExport(connector) )
			{
				...
				{
					HtmlLinkState state = new HtmlLinkState(LABEL_BUTTON_EDIT, new OverrideHandler(editFunction,
						new ConnectorItemKey(content, connector.getId()), content.getExternalTitle(),
						getDescriptionText()));
