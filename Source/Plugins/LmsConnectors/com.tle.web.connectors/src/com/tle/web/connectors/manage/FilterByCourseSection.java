		...
		ResetFiltersListener
{
	...
	protected List<ConnectorCourse> getCourses(SectionInfo info) throws LmsUserNotFoundException
	{
		...
		if( connector != null && repositoryService.supportsExport(connector.getLmsType()) )
		{
			ExpiringValue<List<ConnectorCourse>> cachedCourses = userSessionService.getAttribute(SESSION_KEY
				+ connector.getUuid());
			if( cachedCourses == null || cachedCourses.isTimedOut() )
			{
				cachedCourses = ExpiringValue.expireAfter(
					repositoryService.getAllCourses(connector, CurrentUser.getUsername(), true), 30, TimeUnit.MINUTES);
				userSessionService.setAttribute(SESSION_KEY + connector.getUuid(), cachedCourses);
			}
			List<ConnectorCourse> courses = new ArrayList<ConnectorCourse>(cachedCourses.getValue());
			if( !filterByArchivedSection.isArchived(info) )
			{
				Iterator<ConnectorCourse> iterator = courses.iterator();
				while( iterator.hasNext() )
				{
					ConnectorCourse course = iterator.next();
