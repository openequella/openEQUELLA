		...
		AbstractSearchResultsSection<ConnectorManagementListEntry, ConnectorManagementSearchEvent, ConnectorManagmentSearchResultEvent, SearchResultsModel>
{
	...
	public void processResults(SectionInfo info, ConnectorManagmentSearchResultEvent results)
	{
		...
		{
			ItemKey itemKey = new ItemId(connectorContent.getUuid(), connectorContent.getVersion());
			Item item = itemLookop.get(ItemId.fromKey(itemKey));
			ConnectorManagementListEntry listEntry = entryProvider.get();
			listEntry.setContent(connectorContent);
			listEntry.setConnector(connector);
			if( item == null )
			{
				// FIXME just for now (eg if item has been deleted)
				item = new Item();
				item.setUuid(connectorContent.getUuid());
				item.setVersion(connectorContent.getVersion());
				item.setStatus(ItemStatus.DELETED);
				item.setDateModified(connectorContent.getDateModified());
			}
		...
		ConnectorManagementSearchEvent connectorSearchEvent)
	{
		try
		{
			...
			for( ConnectorContent content : allUsage )
			{
				...
				query.addQuery(new NodeInQuery(content.getUuid(), false, Collections
					.singleton(FreeTextQuery.FIELD_UUID), null));
				query.addQuery(new NodeInQuery(String.valueOf(version), false, Collections
					.singleton(FreeTextQuery.FIELD_VERSION), null));
