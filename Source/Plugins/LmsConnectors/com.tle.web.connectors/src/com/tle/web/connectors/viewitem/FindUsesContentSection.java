...
public class FindUsesContentSection extends AbstractContentSection<FindUsesContentSection.FindUsesModel>
{
	...
	@Component(name = "cl", ignoreForContext = BookmarkEvent.CONTEXT_BROWSERURL)
		...
		if( connector != null )
		{
			...
			if( !checkErrors(context) )
			{
				...
				if( usage != null && usage.size() > 0 )
				{
					...
					final TableHeaderRow header = tableState.setColumnHeadings(new KeyLabel(terms.getCourseHeading()),
						new KeyLabel(terms.getLocationHeading()), LABEL_DATE_ADDED, null);
					...
					for( ConnectorContent use : usage )
					{
						...
						{
							renderAttList.add(new ConnectorContentAttribute(attribute.getLabelKey(), SectionUtils
								.convertToRenderer(attribute.getValue())));
						}
						...
						if( !Check.isEmpty(attachmentUuid) )
						{
							...
							display.setAttachmentName(new TextLabel(attachment == null ? attachmentUuid : attachment
								.getDescription()));
						}
						SectionRenderable details = viewFactory.createResultWithModel("finduses/details.ftl", display);

						TableCell dropArrowCol = new TableCell(new ImageRenderer(URL_DROP_ARROW, LABEL_SHOW_DETAILS))
							.addClass("droparrow");
	...
	public void registered(String id, SectionTree tree)
	{
		...
		OverrideHandler findFunc = new OverrideHandler(ajax.getAjaxUpdateDomFunction(tree, this,
	...
	{
		FindUsesModel model = getModel(info);
		try
		{
			getUsage(info);
		}
