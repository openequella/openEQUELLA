...
public class BlackboardIntegration extends AbstractIntegrationService<BlackBoardSessionData>
{
	...
	@Override
	public void select(SectionInfo info, BlackBoardSessionData data, SelectionSession session)
	{
		try
		{
		...
		SelectedResource resource) throws IOException
	{
		...
		if( imsOrScorm != null && resourceItem instanceof Item )
		{
			...
			try (Reader in = new InputStreamReader(fileSystemService.read(new ItemFile(item), imsOrScorm.getUrl()
				+ "/imsmanifest.xml")))
			{
				CharStreams.copy(in, out);
			}

			PropBagEx resultXml = blackBoardCall.invoke(
				"manifest",
		...
		{
			itemxml.setNode("attachments/@selectedType", linkDetails.getResourceAttachment().getAttachmentType().name()
				.toLowerCase());
		}
