...
public abstract class AbstractAppletLauncher extends JApplet {
  ...
  public void init() {
    try {
      ...
      if (worker == null) {
        worker =
            new GlassProgressWorker<JComponent>("Loading...", -1, false) {
              @Override
              ...
              public void exception() {
                ...
                JLabel label =
                    new JLabel(
                        "Error starting application." + "  Please contact your administrator.");
  ...
  private void commonInitialise() throws Exception {
    ...
    if (isMac) {
      ...
      lch.put(
          ...
          Collections.singletonMap(
              "Set-Cookie", lch.splitCookieString(getParameter(COOKIE_PARAMETER))));
    ...
    if (endpoint != null) {
      CurrentLocale.initialise(
          new ClientLocaleImplementation(
              endpoint, getBundleGroups(), locale, getI18nKeyPrefix(), rtl));
    } else {
      try (InputStream in = new FileInputStream("../resources/lang/i18n.properties")) {
        ...
        CurrentLocale.initialise(
            new ClientLocaleImplementation(locale, props, getI18nKeyPrefix(), rtl));
      }
    }
  }
