...
public final class DialogUtils
{
	...
	public static DialogResult openDialog(Component parent, String title, FileFilter filter, File defaultFile)
	{
		return doDialog(parent, true, title, filter, true, defaultFile);
	}

	public static DialogResult openDialogStrictFilter(Component parent, String title, FileFilter filter,
		File defaultFile)
	{
	...
	public static DialogResult saveDialog(Component parent, String title, FileFilter filter, File defaultFile)
	{
		return doDialog(parent, false, title, filter, true, defaultFile);
	}

	public static DialogResult saveDialog(Component parent, String title, FileFilter filter, String defaultFileName)
	{
		return doDialog(parent, false, title, filter, true, getDefaultFile(defaultFileName));
	}

	public static DialogResult saveDialogStrictFilter(Component parent, String title, FileFilter filter,
		File defaultFile)
	{
	 ...
	 * @param loader
	 */
	public static void doOpenDialog(Component parent, String title, FileFilter filter, FileWorker loader)
	{
	...
	private static File getDefaultFile(String defaultFileName)
	{
		...
		{
			defaultFile = new File(getDirectory(), unextended.getFirst() + "(" + i + ")." + unextended.getSecond()); //$NON-NLS-1$ //$NON-NLS-2$
			i++;
		}
		return defaultFile;
	}
