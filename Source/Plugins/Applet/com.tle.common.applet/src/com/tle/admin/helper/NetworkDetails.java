...
public class NetworkDetails extends JDialog implements ActionListener
{
	...
	private void setup()
	{
		JLabel nameLabel = new JLabel(CurrentLocale.get("com.dytech.edge.admin.helper.networkdetails.rangename"));
		JLabel minLabel = new JLabel(CurrentLocale.get("com.dytech.edge.admin.helper.networkdetails.startaddress"));
		JLabel maxLabel = new JLabel(CurrentLocale.get("com.dytech.edge.admin.helper.networkdetails.endaddress"));
		...
		if( e.getSource() == ok )
		{
			...
			// gets focus.
			{
				JOptionPane.showMessageDialog(ok,
					CurrentLocale.get("com.dytech.edge.admin.helper.networkdetails.entername"));
				name.requestFocus();
			}
		}
