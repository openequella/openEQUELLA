...
public abstract class AbstractNamingAction extends TLEAction {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    final String lOriginalFilename =
        originalFilename == null ? null : originalFilename.trim().toLowerCase();

    JLabel label =
        new JLabel(
            CurrentLocale.get(
                keyPrefix + "dialog.instruction", // $NON-NLS-1$
                originalFilename));
    final JTextField textField = new JTextField();
    JButton okButton = new JButton(CurrentLocale.get(keyPrefix + "dialog.okbutton")); // $NON-NLS-1$
    JButton cancelButton =
        new JButton(CurrentLocale.get(keyPrefix + "dialog.cancelbutton")); // $NON-NLS-1$
    ...
    final Set<String> filenames =
        new HashSet<String>(
            Lists.transform(
                fileList.getCurrentDirectoryFiles(),
    ...
    final ActionListener okCallback =
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            ...
            if (newName.length() == 0) {
              ...
              JOptionPane.showMessageDialog(
                  dialog,
                  CurrentLocale.get(keyPrefix + "dialog.empty"),
                  CurrentLocale //$NON-NLS-1$
                      .get(keyPrefix + "dialog.popuptitle"),
            ...
            if (filenames.contains(lNewName)) {
              JOptionPane.showMessageDialog(
                  dialog,
                  CurrentLocale.get(keyPrefix + "dialog.duplicate"),
                  CurrentLocale //$NON-NLS-1$
                      .get(keyPrefix + "dialog.popuptitle"),
                  JOptionPane.ERROR_MESSAGE); // $NON-NLS-1$
              return;
    ...
    final ActionListener cancelCallback =
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            dialog.dispose();
