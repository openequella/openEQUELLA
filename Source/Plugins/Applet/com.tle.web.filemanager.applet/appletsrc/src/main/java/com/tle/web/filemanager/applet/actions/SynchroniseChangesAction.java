              ...
              CurrentLocale.get("action.sync.progress.unknown"), files.size(), true) // $NON-NLS-1$
          {
            ...
            public File construct() throws Exception {
              for (FileInfo info : files) {
                setMessage(
                    CurrentLocale.get(
                        "action.sync.progress.syncing", info.getName())); // $NON-NLS-1$
                backend.synchroniseFile(
                    info, new InterruptAwareGlassPaneProgressMonitorCallback(this));
              }
              return null;
            }
            ...
            public void exception() {
              ...
              if (!(ex instanceof UserCancelledException)) {
                ...
                JOptionPane.showMessageDialog(
                    fileList,
                    CurrentLocale.get("action.sync.error.message"),
                    CurrentLocale //$NON-NLS-1$
                        .get("action.sync.error.title"),
    ...
    private SelectDialog(Component parent, List<FileInfo> changes) {
      JLabel message =
          new JLabel(
              "<html>"
                  + CurrentLocale.get(
                      "action.sync.select.message",
                      changes //$NON-NLS-1$ //$NON-NLS-2$
                          .size()));

      final CheckboxList<FileInfo> list =
          new CheckboxList<FileInfo>() {
