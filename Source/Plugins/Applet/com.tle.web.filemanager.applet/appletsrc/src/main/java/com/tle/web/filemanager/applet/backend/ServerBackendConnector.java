...
public class ServerBackendConnector extends AbstractRemoteBackendImpl {
  ...
  public void writeFile(final String filename, long length, InputStream content) {
    InputStreamChucker.chunk(
        length,
        content,
        new InputStreamChucker.ChunkHandler() {
          @Override
          public void handleChunk(byte[] bytes, boolean append)
              throws IOException, BannedFileException {
  ...
  public boolean move(FileInfo sourceFile, FileInfo destinationFile) {
    return serverBackend.renameFile(
        wizardId, sourceFile.getFullPath(), destinationFile.getFullPath());
  }
