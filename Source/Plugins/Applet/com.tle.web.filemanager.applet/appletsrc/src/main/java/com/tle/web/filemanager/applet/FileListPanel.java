...
public class FileListPanel extends JPanel {
  ...
  public FileListPanel(Backend backend) {
    ...
    upFolderButton =
        new AddressBarButton(CurrentLocale.get("updir"), "back.gif", currentDirectory) // NOSONAR
        {
          @Override
          public void onClick() {
            FileInfo parent =
                currentDirectory != null ? currentDirectory.getParentFileInfo() : null;
            if (parent == null) {
              parent = new FileInfo();
    ...
    upFolderButton
        ...
        .put(KeyStroke.getKeyStroke("BACK_SPACE"), "clicked");
    upFolderButton
        .getActionMap()
        .put(
            "clicked",
    ...
    rootFolderButton =
        new AddressBarButton(CurrentLocale.get("rootpath"), "home.gif", currentDirectory) {
          @Override
          public void onClick() {
    ...
    listScroller.addComponentListener(
  ...
  private void setCwdForBreadcrumb(FileInfo directory) {
    ...
    while (directory != null) {
      ...
      if (!Check.isEmpty(name)) {
        ...
        AddressBarButton ab =
            new AddressBarButton(name, folderName, directory) {
  ...
  private void setCwdForFilePane(final FileInfo directory) {
    GlassProgressWorker<?> worker =
        new GlassProgressWorker<List<FileInfo>>("Loading...", -1, false) {
          @Override
  ...
  private static class Renderer implements ListCellRenderer {
    ...
    public Renderer() {
      ...
      MigLayout layout =
          new MigLayout(
              "",
              "[" + width1 + "][" + width2 + "][200 - " + (width2 + width1) + "]",
