  ...
  private void uploadFiles(final FileInfo directory, final File[] files) {
    ...
    GlassProgressWorker<?> worker =
        new GlassProgressWorker<Object>(
            CurrentLocale.get("action.upload.progress.unknown"),
            (int) FileSize.getFileSize(files),
            true) //$NON-NLS-1$
        {
          @Override
          ...
          private void uploadFiles(FileInfo cwd, File[] files) throws IOException {
            ...
            for (File file : files) {
              if (!file.isDirectory()) {
                setMessage(
                    CurrentLocale.get(
                        "action.upload.progress.uploading",
                        file //$NON-NLS-1$
                            .getName()));
                long size =
                    backend.uploadFile(
                        file, cwd, new InterruptAwareGlassPaneProgressMonitorCallback(this));
          ...
          public void exception() {
            ...
            if (!(ex instanceof UserCancelledException)) {
              ...
              JOptionPane.showMessageDialog(
                  fileList,
                  CurrentLocale.get("action.upload.error.message"),
                  CurrentLocale //$NON-NLS-1$
                      .get("action.upload.error.title"),
