...
/**
 * A Swing component allowing the user to select one or more elements from a list on the left-hand
 * side, by moving them to the right-hand side with the given controls. Note: There is an
 * implementation of a JFlatShuffleBox which has the same functionality, but using a Flat GUI. It
 * would probably be wise to one day combine the two, or implement JFlatShuffleBox using this, and
...
public class JShuffleBox<T> extends JPanel {
  ...
  /**
   * Sets whether or not to allow duplicates in the lists. The duplicates are tested with the <code>
   * equals</code> method.
   */
  ...
  private void setupGUI(String leftTitle, String rightTitle) {
    ...
    int[] rows =
        new int[] {height1, TableLayout.FILL, height2, height2, height2, height2, TableLayout.FILL};
  ...
  protected class MovementHandler implements ActionListener {
    ...
    public MovementHandler(
        JShuffleBox<T>.Group source, JShuffleBox<T>.Group destination, boolean moveAll) {
      this.source = source;
