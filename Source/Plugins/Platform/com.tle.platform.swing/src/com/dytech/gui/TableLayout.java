...
/**
 * If you've ever used Java's layout managers, you will know that they are
 * really good at what they do. Unfortunately, they do a real good job of
 * sucking badly and irritating the programmer. This layout manager has been
 * created to provide the simplicity of GridLayout, but with the flexibility of
 * the well-feared GridBagLayout. This basically provides the same functionality
 * of CellLayout (provided in this same directory), but allows for rows and
 * columns to be of different sizes. Best of all, you can specify the sizes
 * yourself, either by fixed pixels, relative sizes, or a mix of the two. <h1>
 * Usage</h1> The following is an example of a simple 3x4 fixed size grid:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * You can specify a relative row or column size by using -1. The following
 * example would have 3 equally spaced columns across the entire container:
 * 
 * <pre>
 * int rows[] = new int[]{10, 20, 25, 15};
 ...
 * </pre>
 * 
 * You can specify the magnitude of the relative size by increasing the
 * magnitude of the negative number (ie, decreasing the number). For example,
 * the following will create three columns, with the second being twice as large
 * as the first, and the third being four times larger than the first (and twice
 ...
 * </pre>
 * 
 * You can also use FILL, DOUBLE_FILL, TRIPLE_FILL, and QUADRUPLE_FILL to
 * replace those nasty negative numbers. You can even do mathematics on them.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * To add items to a layout, you use a rectangle to set the x and y position on
 * the grid, as well as the width (# of columns to span) and height (# of rows
 * to span). You need to use the two parameter method of <code>add</code>.
 * 
 * <pre>
 * container.add(someComponent, new Rectangle(0, 1, 3, 2));
 * </pre>
 * 
 * <h1>New Features</h1> A host of new features have been add to version 1.1 of
 * this layout manager, but still maintaining backwards compatibility. You can
 * now use PREFERRED for row sizes. This will use the largest preferred size of
 * all components on the row, except for components that also span one or more
 * variable fill rows. You can also use INVISIBLE for rows or columns, which is
 * equivalent to creating the row or column with a size of zero. Row and column
 * sizes can now be changed dynamically, making this layout perfect for
 * providing hiding/showing tabs or sections.
 * 
...
public class TableLayout implements LayoutManager2
{
	...
	private static final String ERROR_CONSTRAINT_NOT_RECTANGLE = "Constraint must be an instance of java.awt.Rectangle"; //$NON-NLS-1$
	private static final String ERROR_CONSTRAINT_OUT_OF_BOUNDS = "Constraint is placing the component outside of the layouts defined rows and columns"; //$NON-NLS-1$
	private static final String ERROR_ADD_BY_STRING_NOT_SUPPORTED = "You need to add the component with a Rectangle as a constraint, not a String!"; //$NON-NLS-1$
	...
	/**
	 * Constructs a <code>TableLayout</code> with the given row and column
	 * sizes. Please consult this classes JavaDoc for information on specifying
	 ...
	 *      java.awt.Component)
	 * @deprecated Do not use this method! It will throw a
	...
	/**
	 * Finds the largest height of all the preferred sizes of components that
	 * span the given row. Components that span a variable row are not counted.
	 * 
	...
	/**
	 * Returns the accumulated height of all the fixed rows that the given
	 * bounds span.
	 * 
	...
	/**
	 * Performs all the calculations to determine total size of fixed sections,
	 * the final size of preferred sections, and the number of variable
	 * sections.
	 */
