...
/**
 * The ChangeDetector provides a simple interface for registering graphical components and detecting
 * whether any of them have changed. This is great for cases where you want to display a <i>"Do you
 * want to save you work"</i> message when someone exits, but only if a change has taken place. <hr>
 *
 * <h2>Developers Notes</h2>
 *
 * If you want to detect other changes, then add a pair of <code>watch</code> and <code>ignore
 * </code> methods for the given type. For example, if you wish to add a new type listener to a
 * JSomeComponent, then you add <code>watch</code> and <code>ignore</code> methods such as the
 * following:
 *
 * <pre>
 ...
 * </pre>
 *
 * The middle section of this class file is devoted to <code>watch</code> and <code>ignore</code>
 * methods, so put yours there too. You now need to make the EventHandler inner class implement the
 * new listener type and implement any relavent methods.
 *
...
public class ChangeDetector implements Changeable {
  ...
  /**
   * Indicates whether a change has been detected since construction of the <code>ChangeDetector
   * </code>, or the invocation of <code>clearChanges</code>.
   *
  ...
  /**
   * Forces the change detector to register the given object as something that has changed. This is
   * useful in some situations where something is changed programmatically, but the component does
  ...
  private static class EventHandler
    ...
    /**
     * Forces the change detector to register the given object as something that has changed. This
     * is useful in some situations where something is changed programmatically, but the component
