...
public class LtiWrapper extends AbstractUserDirectory
{
	...
	public ModifiableUserState authenticateRequest(HttpServletRequest request)
	{
		...
		{
			LtiConsumer consumer = consumerService.findByConsumerKey(request
				.getParameter(LtiConsumerConstants.PARAM_CONSUMER_KEY));
			...
			setupData(request, ltiUserState);
			setupRoles(request, ltiUserState, consumer);
	...
	@Nullable
	private String getUsername(HttpServletRequest request, @Nullable String fallbackUsername, LtiConsumer consumer)
	{
	...
	private void setupRoles(HttpServletRequest request, UserState state, LtiConsumer consumer)
	{
		...
		state.getUsersRoles().addAll(extraRoles);
	...
	private void setupData(HttpServletRequest request, LtiUserState state)
	{
		...
		ltiData.setLaunchPresentation(request.getParameter("launch_presentation_document_target"));
