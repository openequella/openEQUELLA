...
public class CloudServiceImpl implements CloudService, ItemResolverExtension
{
	...
	// "http://epspqa.stg-openclass.com/cloud/api/";
	...
	// Set 'relevance' to be 'modified' on blank query!
	private void modifySearch(CloudSearch cloudSearch)
	{
		final SortField[] sortType = cloudSearch.getSortFields();
		final boolean reverse = sortType != null ? sortType[0].isReverse() : false;
		final String currentSort = sortType != null ? sortType[0].getField() : null;
		if( cloudSearch.getQuery().equals("") && Objects.equals(currentSort, "relevance") )
		{
			cloudSearch.setSortFields(new SortField[]{new SortField("modified", reverse)});
		}
	}
