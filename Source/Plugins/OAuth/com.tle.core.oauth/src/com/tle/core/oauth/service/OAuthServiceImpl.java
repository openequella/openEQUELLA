		...
		OAuthService
{
		...
		List<ValidationError> errors)
	{
		...
		if( redirect != null && !redirect.equals("default") )
		{
			...
			{
				errors.add(new ValidationError("redirectUrl", CurrentLocale
					.get(KEY_ERROR_VALIDATION_REDIRECTURL_INVALID)));
			}
		}
	...
	protected void populateEditingBean(OAuthClientEditingBean bean, OAuthClient entity)
	{
		...
		ocBean.setClientSecret(entity.getClientSecret());
	...
	protected void populateEntity(OAuthClientEditingBean bean, OAuthClient entity)
	{
		...
		entity.setClientSecret(ocBean.getClientSecret());
	...
	/**
	 * Locking is dealt with within this method to preserve Propagation
	 * mandates.
	 * 
	 * @param editingBean
	 * @param lockId if present must be the effective lock for an actually
	 *            locked resource
	 */
	@Transactional(propagation = Propagation.REQUIRED)
	@Override
	public OAuthClient saveWithEditingBean(OAuthClientEditingBean editingBean, String lockId, boolean keepLocked)
	{
		validateBean(editingBean, false);

		OAuthClient entity = null;
		// Is the bean pre-populated with a uuid?
		OAuthClient oldClientIfAny = Check.isEmpty(editingBean.getUuid()) ? null : clientDao.getByUuid(editingBean
			.getUuid());
		if( oldClientIfAny != null )
		{
			entity = oldClientIfAny;
			editingBean.setId(oldClientIfAny.getId());
			editingBean.setClientSecret(oldClientIfAny.getClientSecret());
		}
		else
		{
		...
		if( lockId != null )
		{
			if( oldClientIfAny == null )
			{
				throw new RuntimeException("Attempting to lock a non-existent OAuthClient");
			}
			// verify lock's validity
			lockingService.getLock(oldClientIfAny, lockId);
		}
		EntityPack<OAuthClient> entityPack = new EntityPack<OAuthClient>();
		entityPack.setEntity(entity);

		if( oldClientIfAny != null )
		{
			beforeStopEdit(entityPack, oldClientIfAny, keepLocked);
		}
		populateEntity(editingBean, entity);
		entity.setClientId(editingBean.getClientId());
		entity.setClientSecret(editingBean.getClientSecret());
		entity.setPermissions(editingBean.getPermissions());
		entity.setRedirectUrl(editingBean.getRedirectUrl());
		entity.setRequiresApproval(editingBean.isRequiresApproval());
		entity.setUserId(editingBean.getUserId());
		entity.setInstitution(CurrentInstitution.get());
		Long id = clientDao.save(entity);

		if( oldClientIfAny != null )
		{
			afterStopEdit(entityPack, oldClientIfAny);
		}
