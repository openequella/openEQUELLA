...
public class SruServiceImpl implements SruService {
  ...
  /**
   * @see
   *     com.tle.core.remoterepo.sru.service.SrwService#search(com.tle.beans.entity.FederatedSearch,
  ...
  public SruSearchResults search(FederatedSearch sruSearch, String query, int offset, int perpage) {
    ...
    if (responseInputStream != null) {
      ...
      if (responseXmlDoc != null) {
        ...
        try {
          String numRecordsStr =
              responseXmlDoc.nodeValue("/searchRetrieveResponse/numberOfRecords");
  ...
  /**
   * Get a single record, presumably as identified in the list of results in search ...
   *
   * @see
   *     com.tle.core.remoterepo.sru.service.SruService#getRecord(com.tle.beans.entity.FederatedSearch,
  ...
  public GenericRecord getRecord(FederatedSearch sruSearch, String qs, int index) {
    ...
    if (theOneResponse != null) {
      ...
      if (matchesEither(MARCXML, recordSchema, null)) {
        ...
        basic.setUrl(
            theOneResponse.getNode("institutionUrl")
                + "items/"
                + theOneResponse.getNode("item/@uuid")
                + "/"
  ...
  private HttpMethod executeQuery(
      FederatedSearch sruSearch, String query, SRUSettings settings, int offset, int perpage)
      throws IOException {
    HttpMethod httpMethod = null;
    ...
    httpClient
        ...
        .getParams()
        .setConnectionTimeout(sruSearch.getTimeout() * 1000);
  ...
  private NameValuePair[] populateNameValuePairs(
      String query, SRUSettings settings, int offset, int perpage) {
  ...
  private List<SruSearchResult> buildResults(
      XmlDocument responseXmlDoc, int offset, String responseAsString, String schemaInSettings) {
    List<SruSearchResult> results = new ArrayList<SruSearchResult>();
    int index = offset;

    for (Node singleRecord :
        responseXmlDoc.nodeList("/searchRetrieveResponse/records/record/recordData")) {
      if (singleRecord != null) // surely not
      {
        results.add(
            convertNodeRecordToSearchResult(
                singleRecord, index, responseAsString, schemaInSettings));
      }
      index++;
    }
    return results;
  ...
  private SruSearchResult convertNodeRecordToSearchResult(
      Node singleRecordNode, int index, String responseAsString, String schemaInSettings) {
    ...
    if (singleRecordPropBag != null) {
      if (singleRecordPropBag.getRootElement().getNodeName().endsWith("diagnostic")) {
        ...
        surl =
            singleRecordPropBag.getNode("institutionUrl")
                + "items/"
                + singleRecordPropBag.getNode("item/@uuid")
                + "/"
                + singleRecordPropBag.getNode("item/@version");
      }
    }
   ...
   * long-form matches the recordSchema (expressed as a full URI) returned in the SRU/W server
   * response data, failing that the EQUELLA short-form matches, or failing that, the EQUELLA known
   * long- form matches the namespaceURI in the SRU/W server response data.<br>
  ...
  private boolean matchesEither(
    ...
    // @formatter:off
    return ((!Check.isEmpty(recordSchema)
  ...
  /**
   * If the returned xml for the SRU server doesn't conveniently state the record schema in a
   * dedicated text node, then the only way to determine the response schema is to look for a
   * namespaceUri affixed to the record nodes within the recordData elements. Depending on the
   * schema, the child of a recordData node could have any sort of name.
   *
   * @param rawXml
   * @return
   */
  private String findNamespaceUriFromRecordNode(String rawXml) {
    ...
    outhere:
    for (int i = 0; i < firstGen.getLength(); ++i) {
