...
public class DCCrosswalk extends XML2oai_dc
{
	 ...
	 * 
	 * @param nativeItem the native "item". In this case, it is already
	 *            formatted as an OAI <record> element, with the possible
	 *            exception that multiple metadataFormats are present in the
	 *            <metadata> element.
	 * @return a String containing the XML to be stored within the <metadata>
	 *         element.
	 * @exception CannotDisseminateFormatException nativeItem doesn't support
	 *                this format.
	 */
	@Override
	public String createMetadata(Object nativeItem) throws CannotDisseminateFormatException
	{
		...
		xml.addNamespace("xmlns:oai_dc", "http://www.openarchives.org/OAI/2.0/oai_dc/"); //$NON-NLS-1$ //$NON-NLS-2$
		xml.addNamespace("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance"); //$NON-NLS-1$ //$NON-NLS-2$
		xml.addNamespace("xsi:schemaLocation", "http://www.openarchives.org/OAI/2.0/" //$NON-NLS-1$ //$NON-NLS-2$
		...
		xml.addValue("dc:title", CurrentLocale.get(fullItem.getName(), fullItem.getUuid())); //$NON-NLS-1$
		xml.addValue("dc:description", CurrentLocale.get(fullItem.getDescription(), "")); //$NON-NLS-1$ //$NON-NLS-2$
		xml.addValue("dc:identifier", fullItem.getIdString()); //$NON-NLS-1$
		xml.addValue("dc:date", new UtcDate(fullItem //$NON-NLS-1$
