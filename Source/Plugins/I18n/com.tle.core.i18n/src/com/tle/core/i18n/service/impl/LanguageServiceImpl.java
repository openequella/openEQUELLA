...
public class LanguageServiceImpl implements LanguageService
{
	...
	public void importLanguagePack(final String stagingId, final String filename) throws IOException
	{
		...
		if( fileSystemService.fileExists(staging, LOCALE_PROPERTIES_FILE) )
		{
			...
			try
			{
				...
				locale = new Locale(getLocalePart(props, LOCALE_PROPERTIES_LANGUAGE, 2),
	...
	private final Control resourceBundleControl = new Control()
	{
		...
		{
			for( Extension ext : bundleFileTracker.getExtensions(new AndFilter<Extension>(
				new LocaleParamFilter("locale", locale.toString()), new ParamFilter("group", baseName))) )
			{
