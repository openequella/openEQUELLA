		...
		ModalEchoServerSection
{
	...
	private EchoServer loadDetailsFromForm(SectionInfo info, EchoServer es)
	{
		...
		String appUrl = checkURL(applicationUrl.getValue(info).trim());
		applicationUrl.setValue(info, appUrl);
		es.setApplicationUrl(appUrl);
		String contUrl = checkURL(contentUrl.getValue(info).trim());
		contentUrl.setValue(info, contUrl);
		es.setContentUrl(contUrl);
		es.setConsumerKey(consumerKey.getValue(info).trim());
		es.setConsumerSecret(consumerSecret.getValue(info).trim());
		es.setEchoSystemID(echoSystemID.getValue(info).trim());
		return es;
	}
