...
public class CompareDateCollector extends AbstractCompareDateCollector {
  ...
  public CompareDateCollector(
      Collection<ItemIndexDate> modifiedTimes,
      IndexedItemFactory indexedItemProvider,
      Map<Long, Institution> insts,
      List<ItemIndexDelete> toDelete,
      Date ignoreMissingAfter) {
  ...
  public void compareDate(long itemId, long instId, long time) {
    ...
    if (itemDate == null || itemDate.getLastIndexed().getTime() != time) {
      ...
      if (itemDate == null) {
        ...
        indItem =
            indexedItemProvider.create(itemDate.getKey(), instMap.get(itemDate.getInstitutionId()));
        indItem.setAdd(true);
      }
      if (indItem != null) {
