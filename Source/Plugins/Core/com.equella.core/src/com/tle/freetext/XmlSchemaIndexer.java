...
public class XmlSchemaIndexer {
  ...
  public void indexOneValue(
      SchemaNode schemaNode, String itemNamePath, String fullPath, String xpath, String value) {
    ...
    if (schemaNode.isIndexedInBody()) {
      ...
      if (value.length() > 0 && !itemNamePath.trim().equals(fullPath)) {
        indexedFields.add(
            AbstractIndexingExtension.unstoredAndVectored(FreeTextQuery.FIELD_BODY, value));
        indexedFields.add(
            AbstractIndexingExtension.unstoredAndVectored(FreeTextQuery.FIELD_BODY_NOSTEM, value));
      }
    }
  }

  public void indexChildNodes(
      SchemaNode parentNode, String itemNamePath, String fullPath, String xpath, PropBagEx xml) {
    ...
    for (SchemaNode schemaNode : childNodes) {
      ...
      if (schemaNode.isAttribute()) {
        indexOneValue(
            schemaNode, itemNamePath, thisFullPath, thisXPath, xml.getNode(schemaNode.getName()));
      } else {
