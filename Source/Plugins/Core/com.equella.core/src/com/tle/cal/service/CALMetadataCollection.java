...
public class CALMetadataCollection implements ItemMetadataListener, RelationListener {
  ...
  public void metadataChanged(Item item, PropBagEx itemXml) {
    ...
    if (holding != null) {
      ...
      Collection<Relation> portionRelations =
          relationService.getAllByToItemAndType(item, CALConstants.CAL_HOLDING);
    ...
    if (holding == null) {
      Collection<Relation> relation =
          relationService.getAllByFromItemAndType(item, CALConstants.CAL_HOLDING);
      if (!relation.isEmpty()) {
  ...
  private List<CALPortion> extractPortions(PropBagEx copyright) {
    ...
    while (iter.hasNext()) {
      ...
      while (sectIter.hasNext()) {
        ...
        section.setCopyrightStatus(
            Check.isEmpty(copyrightStatus) ? CALConstants.CAL_COPYRIGHTSTATUS : copyrightStatus);
