...
public class CalItemXmlConvert extends AbstractItemXmlMigrator {
  ...
  private Map<String, String> getAttachmentMap(PropBagEx xml) {
    ...
    while (iter.hasNext()) {
      ...
      map.put(
          attach.getNode("url").trim(), attach.getNode("uuid").trim()); // $NON-NLS-1$ //$NON-NLS-2$
    }
    return map;
  }

  private void processActivateRequests(
      PropBagEx xml, Map<String, String> attachmentMap, SubTemporaryFile file, String id)
      throws IOException {
    ...
    while (iter.hasNext()) {
      ...
      request.setAttachment(
          convertAttachment(attachmentMap, xml, actXml.getNode("attachment"))); // $NON-NLS-1$
    ...
    fileSystemService.write(
        file,
        id + "-extra/" + ActivationsConverter.ACTIVATIONS_XML,
        inp, //$NON-NLS-1$
        false);
  }

  private void processMetadata(
      Map<String, String> attachmentMap, SubTemporaryFile file, String id, PropBagEx dbXml)
      throws IOException {
    ...
    try (InputStream in = fileSystemService.read(file, metadataPath)) {
      ...
      Iterator<PropBagEx> iter =
          metadata.iterateAll(
              "item/copyright/portions/portion/sections/section/attachment"); //$NON-NLS-1$
      while (iter.hasNext()) {
      ...
      if (changesMade) {
        try (OutputStreamWriter outStream =
            new OutputStreamWriter(fileSystemService.getOutputStream(file, metadataPath, false))) {
  ...
  private String convertAttachment(
      Map<String, String> attachmentMap, PropBagEx dbXml, String filename) {
