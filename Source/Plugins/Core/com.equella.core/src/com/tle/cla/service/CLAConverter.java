...
public class CLAConverter extends AbstractConverter<Object> {
  ...
  private static String KPFX =
      PluginServiceImpl.getMyPluginId(CLAConverter.class) + "."; // $NON-NLS-1$

  @Inject private ItemService itemService;
  @Inject private Provider<CLACollectFilter> filterFactory;
  ...
  public void addTasks(ConvertType type, ConverterTasks tasks, ConverterParams params) {
    if (!params.hasFlag(ConverterParams.NO_ITEMS)) {
      if (type == ConvertType.IMPORT || type == ConvertType.CLONE) {
        tasks.addAfter(
            new NameValue(CurrentLocale.get(KPFX + "cla.collecttask"), CLA_CID)); // $NON-NLS-1$
      }
    }
  }
  ...
  public static class CLACollectFilter extends BaseFilter {
    @Inject private Provider<CLACollectOperation> collectFactory;
    ...
    public String getJoinClause() {
      return "join i.itemDefinition d left join d.attributes a with a.key = '"
          + CLAConstants.ENABLED
          + "' "; //$NON-NLS-1$//$NON-NLS-2$
    }
