  ...
  public void metadataChanged(Item item, PropBagEx itemXml) {
    ...
    if (holding != null) {
      ...
      Collection<Relation> portionRelations =
          relationService.getAllByToItemAndType(item, CLAConstants.HOLDING);
    ...
    if (holding == null) {
      Collection<Relation> relation =
          relationService.getAllByFromItemAndType(item, CLAConstants.HOLDING);
      if (!relation.isEmpty()) {
    ...
    while (iter.hasNext()) {
      ...
      while (sectIter.hasNext()) {
        ...
        section.setCopyrightStatus(
            Check.isEmpty(copyrightStatus) ? CLAConstants.COPYRIGHTSTATUS : copyrightStatus);
