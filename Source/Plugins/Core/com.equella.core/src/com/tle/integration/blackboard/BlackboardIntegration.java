 ...
 * Blackboard now uses the generic integration.
 */
@Bind
@Singleton
...
public class BlackboardIntegration extends AbstractIntegrationService<BlackBoardSessionData> {
  ...
  public void setupSingleSignOn(SectionInfo info, SingleSignonForm form) {
    ...
    data.setCourseInfoCode(
        integrationService.getCourseInfoCode(form.getCourseId(), form.getCourseCode()));
    initSession(data);

    IntegrationActionInfo actionInfo =
        integrationService.getActionInfo(data.getEntryUrl(), form.getOptions());
  ...
  private PropBagEx addResourceToBlackboard(
      SectionInfo info,
      SelectionSession session,
      BlackBoardSessionData data,
    ...
    final LmsLinkInfo linkDetails =
        getLinkForResource(
            info,
            createViewableItem(itemService.get(itemId), resource),
            resource,
            true,
            session != null && session.isAttachmentUuidUrls());
      ...
      try (Reader in =
          new InputStreamReader(
              fileSystemService.read(
                  itemFileService.getItemFile(item), imsOrScorm.getUrl() + "/imsmanifest.xml"))) {
        CharStreams.copy(in, out);
      }

      PropBagEx resultXml =
          blackBoardCall.invoke(
              "manifest",
              new NameValue[] {
  ...
  public String getClose(BlackBoardSessionData data) {
    ...
    if (data.canSelect()) {
      ...
      try {
        closeURL =
            blackBoardCall.redirContent(data.getCourseId(), data.getContentId(), true).toString();
      } catch (IOException e) {
  ...
  public <I extends IItem<?>> ViewableItem<I> createViewableItem(
    ...
    final ViewableItem<I> vitem =
        viewableItemResolver.createIntegrationViewableItem(
            itemId, latest, ViewableItemType.BLACKBOARD, itemExtensionType);
    return vitem;
  }
}
