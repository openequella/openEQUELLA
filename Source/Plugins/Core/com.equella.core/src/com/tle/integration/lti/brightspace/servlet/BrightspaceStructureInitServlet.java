...
public class BrightspaceStructureInitServlet extends HttpServlet {
  ...
  @Inject private StructuredIntegrationSessionExtension structuredInteg;
  ...
  public void setupCache() {
    courseStructureCache =
        cacheService.getCache("BrightspaceSignonCourseStructure", 100, 2, TimeUnit.MINUTES);
  }
  ...
  private String getStructure(String courseId, String connectorUuid, String selected) {
    try {
      ...
      final String cacheKey =
          connectorUuid + ":" + courseId + (!isDefaultApplicable ? "" : ":" + selected);
      ...
      if (structure == null) {
        ...
        final List<ConnectorFolder> folders =
            brightspaceService.getFoldersForCourse(connector, null, courseId, false);
        boolean first = true;
        for (ConnectorFolder folder : folders) {
          ...
          folderNode.put(
              "selected",
              (!isDefaultApplicable && first)
                  || (isDefaultApplicable && selected.equals(folder.getId())));
  ...
  private void recurseFolder(
    ...
    try {
      final boolean isDefaultApplicable = !Strings.isNullOrEmpty(selected);
      final List<ConnectorFolder> subFolders =
          brightspaceService.getFoldersForFolder(connector, null, courseId, folder.getId(), false);
