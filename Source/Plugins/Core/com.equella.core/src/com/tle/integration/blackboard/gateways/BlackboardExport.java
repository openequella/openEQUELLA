...
public class BlackboardExport extends Blackboard {
  ...
  /** Exports a plan to Blackboard. This creates a link back to a plan/item in the repository. */
  public PropBagEx exportItems(PropBagEx itemxml, boolean isPlan, boolean create)
      throws IOException {
  ...
  protected PropBagEx export(
      PropBagEx xml, Collection<Content> contents, boolean isPlan, boolean isNew)
      throws IOException {
    if (contents.isEmpty()) {
      return null;
    }
  ...
  public String getRedirectURL(String id, String itemdefid, int version1, String path) {
    String url1 =
        "item/" + itemdefid + "/" + id + "/" + version1 + "/" + path + "?hasPoppedUp=true&";
    url1 = encode(url1);
  ...
  public URL previewContent(String courseId, String contentId, boolean modify) throws IOException {
    NameValue[] parameters =
        new NameValue[] {
          new NameValue(courseId, "course_id"),
          new NameValue(contentId, "content_id"),
          new NameValue(Boolean.toString(modify), "modify"),
        };
  ...
  public String getHome() {
    try {
      return new URL(
              url.getProtocol(), url.getHost(), url.getPort(), "/webapps/portal/tab/_1_1/index.jsp")
          .toString();
  ...
  public String getTopHome() {
    try {
      return new URL(
              url.getProtocol(), url.getHost(), url.getPort(), "/webapps/portal/frameset.jsp")
          .toString();
    } catch (MalformedURLException e) {
      // Never happen
    }
