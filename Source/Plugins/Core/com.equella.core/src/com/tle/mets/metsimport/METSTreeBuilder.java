...
public class METSTreeBuilder implements PackageTreeBuilder {
  ...
  public PackageInfo createTree(
    ...
    try (InputStream manifest =
        imsService.getMetsManifestAsStream(
            staging,
            nonZip ? "" : packageExtractedFolder,
            nonZip ? originalPackagePath : METS_FILENAME,
            true)) {
      if (manifest != null) {
        Mets mets;
        try {
          ...
          METSTreeHelper treeHelper =
              new METSTreeHelper(
                  item, staging, packageExtractedFolder, packageName, mets.getLABEL());
          ...
          result.setError(invalidManifest());
          LOGGER.warn(invalidManifest() + ": " + e.getMessage());
          return result;
        }
  ...
  private String notLoaded() {
    return r.getString("metsimport.notloaded");
  }

  private String invalidManifest() {
    return r.getString("metsimport.notvalidmanifest");
  }
  ...
  public PackageInfo getInfo(SectionInfo info, FileHandle staging, String packageExtractedFolder) {
    ...
    try (InputStream manifest =
        imsService.getMetsManifestAsStream(
            staging, packageExtractedFolder, nonZip ? null : METS_FILENAME, true)) {
      if (manifest != null) {
        ...
        mets.validate(new MetsValidator());
        METSTreeHelper treeHelper =
            new METSTreeHelper(new Item(), staging, packageExtractedFolder, null, mets.getLABEL());
        // treeHelper.populateTree(findStructMaps(mets));
  ...
  public boolean canHandle(SectionInfo info, FileHandle staging, String packageExtractedFolder) {
    ...
    try {
      ArchiveEntry entry =
          fileSystem.findZipEntry(staging, packageExtractedFolder, METS_FILENAME, false);
      return (entry != null);
    } catch (Exception e) {
      return false;
    }
  ...
  @Override
  public List<String> determinePackageTypes(
      SectionInfo info, FileHandle staging, String packageFilepath) {
  ...
  protected class METSTreeHelper implements AttachmentAdder {
    ...
    @Nullable protected final String targetFolder;
    protected String title;
    protected Collection<Attachment> createdAttachments;

    public METSTreeHelper(
        Item item,
        FileHandle staging,
        String packageExtractedFolder,
    ...
    @SuppressWarnings("unchecked")
    protected void populateStructMap(ItemNavigationNode parent, StructMap struct)
      ...
      for (MetsElement elem : elements) {
        if (elem instanceof Div) // edu.harvard.hul.ois.mets.Div
        {
          ...
          throw new MetsException(invalidManifest());
        }
      }
    }
      ...
      for (MetsElement elem : content) {
        ...
        if (!imported) {
          if (elem instanceof FLocat) {
            ...
            String nodetitle =
                (loc.getXlinkTitle() != null ? loc.getXlinkTitle() : loc.getXlinkHref());
          ...
          {
            // TODO:
            HELPER_LOGGER.warn(r.getString("treebuilder.streamnotsupported"));
          } else if (elem instanceof TransformFile) // edu.harvard.hul.ois.mets.TransformFile
          {
            // TODO:
            HELPER_LOGGER.warn(r.getString("treebuilder.transformfilenotsupported"));
    ...
    @SuppressWarnings("unchecked")
    protected void populateFContent(
        ItemNavigationNode parent, FContent fcontent, String mimeType, String filename)
        throws MetsException {
    ...
    protected void populateBinData(
        ItemNavigationNode parent, BinData bin, String mimeType, String filename)
        throws MetsException {
      for (Object elem : bin.getContent()) {
        if (elem instanceof PCData) {
          ...
          try {
            FileInfo fileInfo =
                fileSystem.write(staging, filename, new ByteArrayInputStream(bytes), true);
            filename = fileInfo.getFilename();
            if (fileInfo.getLength() == 0) {
              fileSystem.removeFile(staging, filename);
              throw new MetsException(r.getString("treebuilder.notzero"));
            }
            ...
            throw new MetsException(r.getString("treebuilder.couldntwrite"));
          }
        }
  ...
  public void setPluginService(PluginService pluginService) {
    attachmentExporters =
        new PluginTracker<MetsAttachmentImporterExporter>(
            pluginService, "com.tle.mets", "exporterimporter", "id");
