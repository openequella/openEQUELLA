...
/**
 * An abstract {@link AttachmentHandler} which has an "add" page and a "details" page. The "details"
 * page will only show when editing an attachment.
 *
 * <p>Attachments themselves are represented by a generic type UA extends {@link
 * UniversalAttachment}, which can have more state than just the {@link Attachment} object itself.
 * Most handlers don't need the extra state ability and should extend {@link
 * BasicAbstractAttachmentHandler}.
 *
 * @author jolz
 * @param <M>
 * @param <UA>
...
@NonNullByDefault
public abstract class AbstractDetailsAttachmentHandler<
        M extends AbstractDetailsAttachmentHandler.AbstractAttachmentHandlerModel,
        UA extends UniversalAttachment>
    extends AbstractAttachmentHandler<M> {
  public static final String THIS_PID =
      AbstractPluginService.getMyPluginId(AbstractAttachmentHandler.class);
  public static final String COMMON_PREFIX = THIS_PID + ".";
  public static final String COMMON_INCLUDE_PATH = "/" + THIS_PID + "@/common-edit-handler.ftl";
  ...
  @Component private TextField displayName;
  @Component private Checkbox previewCheckBox;
  @Component private Checkbox restrictCheckbox;
  @Component private SingleSelectionList<NameValue> viewers;
  ...
  /**
   * Get the attachment type for the attachment being handled. Used by viewer list
   *
   * @param info TODO
  ...
  public void loadForEdit(SectionInfo info, Attachment attachment) {
    dialogState.setAttribute(
        info,
        this,
        Collections.singletonList(
            createUniversalAttachmentForEdit(info, (Attachment) attachment.clone())));
    setupDetailEditing(info);
  }

  /**
   * Create a {@link UniversalAttachment} for the given attachment which is about to be edited.
   *
   * @param info
   * @param attachment
   * @return
   ...
   * Before calling this method the {@code showSave} flag of the {@code DialogRenderOptions} is set
   * to true.
   *
   * @param context
   * @param renderOptions
   * @return
   */
  protected abstract SectionRenderable renderDetails(
      RenderContext context, DialogRenderOptions renderOptions);

  /**
   * Render the "add" page. <br>
   * If {@link #isOnePageAdd()} returns true, the showSave and showAddReplace flags are set to true
   * on the {@code DialogRenderOptions}.
   *
   * @param context
   * @param renderOptions
   * @return
   */
  protected abstract SectionRenderable renderAdd(
      RenderContext context, DialogRenderOptions renderOptions);

  /**
   * Create a list of new {@link UniversalAttachment} objects after validating the values on the
   * "add" page. <br>
   * If only 1 {@code UniversalAttachment} is returned from this method the handler will go to
   * "details editing" page to edit the details for the single {@code UniversalAttachment}.
   *
   * @param info
   * @return
   */
  ...
  /**
   * Returns true if the handler has a single "add" page which always needs the add/replace button
   * to show.
   *
   * @return
   */
  ...
  /**
   * Override this if your "details" page needs other UI components to be set up.
   *
   * @param info
   */
  ...
  protected void saveDetailsToAttachment(SectionInfo info, Attachment attachment) {
    ...
    ViewableResource viewableResource =
        attachmentResourceService.getViewableResource(
            info, dialogState.getViewableItem(info), attachment);
  ...
  /**
   * Modify the item and itemxml with the new {@link UniversalAttachment}. Only needs to be
   * overridden if the Attachment has to make other changes beside the {@code Attachment} and UUID
   * in the item xml.
   *
   * @param info
   * @param attachment
  ...
  public SectionRenderable getThumbnailUrlForAttachment(SectionInfo info, Attachment attachment) {
    return attachmentResourceService
        .getViewableResource(info, dialogState.getViewableItem(info), attachment)
        .createStandardThumbnailRenderer(new TextLabel(attachment.getDescription()))
        .addClass("file-thumbnail");
  }

  public SectionRenderable getThumbnailUrlForFile(
      SectionInfo info, String filename, String mimeType) {
    return attachmentResourceService
        .createPathResource(
            info, dialogState.getViewableItem(info), filename, filename, mimeType, null)
        .createStandardThumbnailRenderer(new TextLabel(mimeType))
        .addClass("file-thumbnail");
  }

  /**
   * Prepare the model for rendering the "details" page. Called before {@link
   * #renderDetails(RenderContext, DialogRenderOptions)}.
   *
   * @param info
   */
