...
public class ShowHtmlEditorPluginsSection
  ...
  private final Cache<String, UploadResult> UPLOADS =
      CacheBuilder.newBuilder().expireAfterWrite(15, TimeUnit.SECONDS).concurrencyLevel(4).build();
  ...
  protected SectionRenderable renderTop(RenderEventContext context) {
    ...
    model.setCanAdd(
        !aclService
            .filterNonGrantedPrivileges(HtmlEditorPluginConstants.PRIV_CREATE_HTMLEDITOR_PLUGIN)
            .isEmpty());
  ...
  public void registered(String id, SectionTree tree) {
    ...
    updateFunction =
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("checkUpload"), getAjaxId());
  }
  ...
  /**
   * This method ensures uploadPlugin doesn't complete before the file is there to display.
   *
   * @param info
   * @param newFileUuid
   */
  ...
  @Override
  protected AbstractEntityService<HtmlEditorPluginEditingBean, HtmlEditorPlugin>
      getEntityService() {
  ...
  @Override
  protected void addDynamicColumnData(
      SectionInfo info, HtmlEditorPlugin ent, SelectionsTableSelection row) {
  ...
  public SettingInfo getSettingInfo(SectionInfo info) {
    if (!aclService
        .filterNonGrantedPrivileges(
            HtmlEditorPluginConstants.PRIV_CREATE_HTMLEDITOR_PLUGIN,
            HtmlEditorPluginConstants.PRIV_EDIT_HTMLEDITOR_PLUGIN,
            HtmlEditorPluginConstants.PRIV_DELETE_HTMLEDITOR_PLUGIN)
        .isEmpty()) {
  ...
  @NonNullByDefault(false)
  public static class ShowHtmlEditorPluginsModel
