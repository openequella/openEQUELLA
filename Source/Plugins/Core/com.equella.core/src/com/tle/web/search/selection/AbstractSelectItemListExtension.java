...
@NonNullByDefault
public abstract class AbstractSelectItemListExtension<
  ...
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(AbstractSelectItemListExtension.class);
  private static final IncludeFile INCLUDE =
      new IncludeFile(resources.url("scripts/selectitemlist.js"));
  private static final JSCallable SELECT_ITEM =
      new ExternallyDefinedFunction("selectItemList", 4, INCLUDE);
  ...
  public ProcessEntryCallback<I, LE> processEntries(
      final RenderContext context, List<LE> entries, ListSettings<LE> settings) {
    ...
    return new ProcessEntryCallback<I, LE>() {
      ...
      public void processEntry(LE entry) {
        ...
        if (session.containsResource(new SelectedResourceKey(itemId, extensionType), false)) {
          ...
          hls.setClickHandler(
              new OverrideHandler(
                  session.isSelectMultiple() ? removeCall : removeCallOne, itemId, extensionType));

          entry.addRatingAction(
              new ButtonRenderer(hls)
                  .showAs(ButtonType.MINUS)
                  .addClass("button-expandable unselect"));
          entry.setSelected(true);
        } else {
          final JSCallable selectItemFunction =
              selectionService.getSelectItemFunction(context, getViewableItem(context, item));
  ...
  private boolean selectButtonDisable(SectionInfo info, SelectionSession session) {
    ...
    final boolean disableSelectionButton =
