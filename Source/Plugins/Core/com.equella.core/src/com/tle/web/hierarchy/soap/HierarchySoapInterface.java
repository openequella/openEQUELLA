...
public interface HierarchySoapInterface {
  ...
  /**
   * Deletes the Hierarchy topic specified by topicUuid. This will also delete all child topics of
  ...
  /**
   * Allows the creation of a Hierarchy topic with the parent being the topic specified by
   * parentUuid. If no parent UUID is specified the topic shall be created as a root level topic.
   *
   * @param parentUuid The unique ID of the parent Hierarchy topic
   * @param topicXml The XML of the topic to be created - <em>See the getTopic() method for the
   *     format of the topic xml</em>
   * @param index The index the topic should be inserted into in relation to its siblings. Zero
   *     inserts it as the first sibling, one as the second sibling, and so on. If the index is less
   *     than zero or greater than the number of siblings minus one, then the topic will be added as
   *     the last sibling.
  ...
  /**
   * Updates the Hierarchy topic specified by topicUuid with the information included in topicXml.
   *
   * @param topicUuid The unique ID of the Hierarchy topic to be edited
   * @param topicXml The XML from which the Hierarchy topic should be updated - <em>See the
   *     getTopic() method for the format of the topic xml</em>
   */
  void editTopic(String topicUuid, String topicXml);

  /**
   * Moves the Hierarchy topic specified by childId to the location specified by index within the
   * parent Hierarchy topic specified by parentUuid. The only time parentUuid may be omitted is when
   * moving root level Hierarchy topics
   *
   * @param childId The unique ID of the Hierarchy topic to move
   * @param parentUuid The unique ID of the parent Hierarchy topic
   * @param index The location that the child Hierarchy topic should be moved to in relation to the
   *     other children
   */
