...
public class VersionSelectionSection
    extends AbstractPrototypeSection<VersionSelectionSection.VersionSelectionTableModel>
  ...
  public SectionResult renderHtml(RenderEventContext context) {
    ...
    final String folderId =
        (Check.isEmpty(model.getParentFolderId()) ? null : model.getParentFolderId());
  ...
  public void registered(String id, SectionTree tree) {
    ...
    removeFunc =
        ajax.getAjaxUpdateDomFunction(
            tree,
            null,
            events.getEventHandler("removeSelection"),
  ...
  private void doFolder(
      ...
      TargetFolder folder,
      ObjectMapper mapper,
      boolean allowVersionChoice,
      String defaultVersionOption,
      boolean showAll,
      List<Pair<Label, TableState>> tables) {
    if (folder.getResourceCount() > 0) {
      TableState tableState = new TableState();
      tableState.setColumnHeadings(
          TABLE_NAME_LABEL, allowVersionChoice ? TABLE_VERSION_LABEL : null, null);
      ...
      for (SelectedResource res : folder.getResources().values()) {
        ...
        if (allowVersionChoice) {
          ...
          versionCell.addContent(
              versionSelections.getListState(
                  info, key, getVersionOptionsForSelectedResource(res), dvo));
          versionCell.addClasses("version");
        }

        final HtmlComponentState unselect = new HtmlComponentState();
        unselect.setClickHandler(
            new OverrideHandler(removeFunc, key)
                .addValidator(Js.confirm(TABLE_CONFIRM_REMOVE_LABEL)));
  ...
  private List<Option<?>> getVersionOptionsForSelectedResource(SelectedResource res) {
    ...
    rv.add(
        new VoidKeyOption(
            SPECIFIC_VERSION_KEY, new Object[] {res.getVersion()}, SPECIFIC_VERSION_VALUE));
    return rv;
  }
  ...
  public void setPluginService(PluginService pluginService) {
    versionChoiceDeciders =
        new PluginTracker<VersionChoiceDecider>(
            pluginService, "com.tle.web.selection", "versionChoiceDecider", null);
