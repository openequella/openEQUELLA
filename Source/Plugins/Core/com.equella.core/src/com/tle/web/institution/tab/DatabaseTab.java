...
public class DatabaseTab extends AbstractInstitutionTab<DatabaseTab.Model> {
  ...
  @Inject @Component private ShowErrorDialog errorsDialog;
  @Inject @Component private ProgressDialog progressDialog;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    migrateSelectedButton.setClickHandler(
        events.getNamedHandler("migrateSelected").addValidator(migrateConfirm));
    ...
    table
        .setColumnHeadings(LABEL_COL_NAME, LABEL_COL_STATUS, null, LABEL_COL_ACTIONS)
        .getCells()
        .get(3)
    ...
    disableHandler =
        new StatementHandler(
            Js.call_s(
                migrateSelectedButton.createDisableFunction(),
                new NotExpression(
                    Js.call(FormValuesLibrary.IS_SOME_CHECKED, bulkBoxes.getParameterId()))));
  }
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    for (SchemaInfo schemaInfo : schemas) {
      ...
      if (model.isPartial()) {
        row.setAjaxCaptureOptions(
            new AjaxCaptureOptions(
                "schemaRow", true, true, ImmutableMap.of("id", (Object) schemaId)));
      }
      ...
      if (schemaInfo.isChecking()) {
        ...
        createOption(
            ...
            handlers,
            "action_takedown",
            new OverrideHandler(setOnlineCallable, schemaId, false)
                .addValidator(TAKE_OFFLINE_CONFIRM),
            true);
        ...
        if (!Check.isEmpty(finishedTaskId)) {
          ...
          if (taskStatus != null) {
            createOption(
                ...
                handlers,
                "action_review",
                new OverrideHandler(progressDialog.getOpenFunction(), finishedTaskId),
                false);
          }
        }
      } else {
        ...
        if (schemaInfo.getErrorMessage() != null) {
          ...
          hasMigrations = true;
          processMigrationRequired(
              context, schemaInfo, drow, actions, handlers, updates, schemas.size() > 2);
        } else {
          if (schemaInfo.getDuplicateWith() != null) {
            ...
            if (!schemaInfo.getDatabaseSchema().isOnline()) {
              createOption(
                  ...
                  handlers,
                  "action_online",
                  new OverrideHandler(setOnlineCallable, schemaId, true),
                  true);
            }
          }
        }
  ...
  private void addCells(
      TableRow row,
      DatabaseRow drow,
      HtmlListState actions,
      ObjectExpression handlers,
      long schemaId) {
    if (drow.editDelete) {
      createOption(
          EDIT_LABEL,
          ...
          handlers,
          "action_edit",
          new OverrideHandler(editDialog.getOpenFunction(), schemaId),
          false);
      createOption(
          DELETE_LABEL,
          actions,
          handlers,
          "action_delete",
          new OverrideHandler(deleteCallable, schemaId).addValidator(DELETE_CONFIRM),
          false);
    }
    ...
    createOption(
        ...
        handlers,
        "action_refresh",
        events.getNamedHandler("refreshSchema", schemaId),
        Check.isEmpty(actions.getSelectedValues()));
  ...
  private Option<String> createOption(
      Label label,
      HtmlListState actions,
      ObjectExpression handlers,
      String key,
  ...
  private LinkRenderer createErrorLink(SchemaInfo schemaInfo, Label label) {
    ...
    state.setClickHandler(
        new OverrideHandler(
            errorsDialog.getOpenFunction(), schemaInfo.getDatabaseSchema().getId()));
    return new LinkRenderer(state);
  }

  private void processMigrationRequired(
    ...
    if (taskId != null) {
      ...
      createOption(
          ...
          handlers,
          "action_progress",
          new OverrideHandler(progressDialog.getOpenFunction(), taskId),
          true);
      ...
      if (statusKey != null) {
        drow.progress =
            new KeyLabel(
                taskStatus.getStatusKey(),
                taskStatus.getDoneWork(),
                taskStatus.getMaxWork(),
                taskStatus.getPercentage());
      }
  ...
  @AjaxMethod
  public JSONResponseCallback updatePage(
      final AjaxRenderContext info, Map<Long, Long> updateTimes) {
