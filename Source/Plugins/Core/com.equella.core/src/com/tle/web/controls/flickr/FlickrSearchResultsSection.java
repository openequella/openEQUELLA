...
public class FlickrSearchResultsSection
  ...
  @Inject private FlickrListSection itemList;
  @Inject private DateRendererFactory dateRendererFactory;

  @ViewFactory private FreemarkerFactory searchViewFactory;

  @Component private MultiSelectionList<Photo> results;

  @TreeLookup private FlickrLayoutSection flickrLayoutSection;
  @TreeLookup private FlickrQuerySection flickrQuerySection;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    results.setListModel(
        new DynamicHtmlListModel<Photo>() {
          ...
          protected Iterable<Option<Photo>> populateOptions(SectionInfo info) {
            ...
            for (Iterator<FlickrListEntry> iter = flist.iterator(); iter.hasNext(); ) {
              ...
              ImageRenderer thumbnail =
                  new ImageRenderer(photo.getThumbnailUrl(), new TextLabel(photoTitle));
              ...
              String displayLicenceVal =
                  filterByCreativeCommonsLicencesSection.getDisplayLicenceForValue(photoLicence);
  ...
  @Override
  protected FlickrSearchResultsEvent createResultsEvent(
      SectionInfo info, FlickrSearchEvent searchEvent) {
  ...
  protected SearchResults<Photo> doSearch(SectionInfo info, FlickrSearchEvent searchEvent) {
    ...
    try {
      ...
      if (photoList != null) {
        ...
        searchResults =
            new SimpleSearchResults<Photo>(
                photoList, photoListSize, searchEvent.getOffset(), queryTotal);
      }
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    // populated
    if (Check.isEmpty(searchEvent.getQuery())
        && Check.isEmpty(searchEvent.getParams().getUserRawText())
    ...
    model.setActions(
        SectionUtils.renderChildren(context, this, new ResultListCollector(true)).getFirstResult());
  ...
  List<NameValueExtra> getAllLicenceValues() {
    return filterByCreativeCommonsLicencesSection != null
        ? filterByCreativeCommonsLicencesSection.getAllLicenceValues()
        : null;
  }

  private PhotoList<Photo> invokeService(SectionInfo info, FlickrSearchEvent searchEvent)
    ...
    photoList =
        flickrService.searchOnParams(
            params,
            pagerCurrentPage,
            FlickrUtils.PAGER_PER_PAGE,
            flickrSettings.getApiKey(),
            flickrSettings.getApiSharedSecret());
  ...
  public static class FlickrResultOption implements Option<Photo> {
    ...
    /**
     * Photo title appears as the link value: there's nothing we want from calling getName per se.
     */
    @Override
