...
public abstract class AbstractConnectorEditorSection<
  ...
  private static final IncludeFile INCLUDE =
      new IncludeFile(
          ResourcesService.getResourceHelper(AbstractConnectorEditorSection.class)
              .url("scripts/editcommon.js"));
  private static final ExternallyDefinedFunction FUNCTION_MODIFY_USERNAME_CHANGED =
      new ExternallyDefinedFunction("modifyUsernameChanged", 2, INCLUDE);
  ...
  @Component private Div modifyUsernameDiv;
  @Inject private ExpressionSelectorDialog exportableSelector;
  @Inject private ExpressionSelectorDialog viewableSelector;
  @Component private Div exportDiv;
  @Component private Div viewDiv;
  @Component private Div usernameDiv;
  ...
  public final SectionResult renderHtml(RenderEventContext context) {
    ...
    modifyUsername.setLabel(
        context, new KeyLabel(KEY_MODIFY_USERNAME, getConnectorLmsName(context, connector)));
    ...
    if (!DebugSettings.isAutoTestMode()) {
      context
          .getBody()
          .addEventStatements(
              JSHandler.EVENT_BEFOREUNLOAD, new ReturnStatement(getWarningNavigateAway()));
    }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    testUrlButton.setClickHandler(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("testUrl"), getAjaxDivId()));

    exportableSelector.setStateful(false);
    exportableSelector.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("expression"), "exportable"));
    ...
    viewableSelector.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("expression"), "viewable"));
    ...
    modifyUsername.addReadyStatements(
        Js.call_s(
            FUNCTION_MODIFY_USERNAME_CHANGED,
            Jq.$(modifyUsername),
            Jq.$(modifyUsernameDiv),
            false));
    modifyUsername.addEventStatements(
        "change",
        Js.call_s(
            FUNCTION_MODIFY_USERNAME_CHANGED, Jq.$(modifyUsername), Jq.$(modifyUsernameDiv), true));
  ...
  public void expression(SectionInfo info, String selectorId, String expression) {
    if (selectorId != null) {
      final ConnectorEditingSession session =
          connectorService.loadSession(getModel(info).getSessionId());
  ...
  public void testUrl(SectionInfo info) {
    ...
    try (Response resp =
        httpService.getWebContent(new Request(url), configService.getProxyDetails())) {
  ...
  protected void validate(
      SectionInfo info, EntityEditingSession<ConnectorEditingBean, Connector> session) {
    final ConnectorEditingBean connector = session.getBean();
    ...
    if (Check.isEmpty(url)) {
      errors.put(
          "url", new KeyLabel(KEY_ERROR_URL, getConnectorLmsName(info, connector)).getText());
      onTestUrlFail(info, connector);
    } else {
      ...
      if (Check.isEmpty(testedUrl) || !testedUrl.equals(url)) {
        errors.put(
            "url", new KeyLabel(KEY_ERROR_TESTURL, getConnectorLmsName(info, connector)).getText());
        onTestUrlFail(info, connector);
      }
    }
    ...
    if (exportExpression != null || viewExpression != null) {
      ...
      if (exportExpression != null) {
        list.getEntries()
            .add(
                new TargetListEntry(
                    true, false, ConnectorConstants.PRIV_EXPORT_VIA_CONNECTOR, exportExpression));
      }
        ...
        list.getEntries()
            .add(
                new TargetListEntry(
                    true,
                    false,
                    ConnectorConstants.PRIV_VIEWCONTENT_VIA_CONNECTOR,
                    viewExpression));
      }
  ...
  protected void loadFromSession(
    ...
    exportSummary.setChecked(
        info, connector.getAttribute(ConnectorConstants.SHOW_SUMMARY_KEY, true));
    ...
    final String username =
        (connector.getId() == 0 ? "return username;" : connector.getUsernameScript());
  ...
  public class AbstractConnectorEditorModel
      extends AbstractEntityEditor<ConnectorEditingBean, Connector, M>.AbstractEntityEditorModel {
