...
/**
 * This class handles Webdav requests and allows access to Learning Edge attachments. Only one
 * attachment can be accessed at a time, through the UUID value of the attachment.
 *
...
public class WebdavServlet extends HttpServlet {
  ...
  private String[] decodeParts(HttpServletRequest request, String szURL)
  ...
  /**
   * Services an HTTP request from the client, and determines how to handle it. In particular it
   * examines the request method and decides whether the request is a WebDav or normal request.
   *
   * <p>The following are the supported WebDav methods:
   *
   * <ul>
   *   <li>GET
   *   <li>POST
  ...
  public void handleMethod(
    ...
    if (methodName.equalsIgnoreCase(HttpMethod.GET)
        || methodName.equalsIgnoreCase(HttpMethod.POST)) {
  ...
  /**
   * Handles the webdav PROPFIND method, which retrieves the list of available files, and their
   * properties.
   *
   * <p>The request from a browser uses a body as shown here, which details which properties to
   * return.
   *
   * <p>Microsoft Windows wants this: <code>
   * <pre>
   ...
   * </code>
   *
   * <p>Here is an example of a correctly formatted response. The "HTTP/1.1 404 Not Found"
   * <propstat>'s are to descibe what properties haven't been listed in the response.
   *
   * <p><code>
   * <pre>
   * <?xml version = '1.0' encoding = 'utf-8'?>
  ...
  public void propFindMethod(
    ...
    if (isDir) {
      ...
      recurseDirectory(
          requestedProperties, uri, propBag, staging, filename, dir, getRequestedDepth(req));
    } else {
      final WebdavProps madProps = new WebdavProps(mimeTypeService, requestedProperties);
  ...
  /**
   * Handles the OPTIONS method, which asks the server what WebDav version and methods are
   * supported.
   *
   * @param req HTTP Request from client
   * @param resp HTTP Response to client
  ...
  /**
   * Handles the MKCOL method, which makes a new "Collection" (ie Folder) on the server. This is
   * supported, but doesn't actually work because currently the Learning Edge doesn't support such
   * an action. However, a CREATED status is sent to fool the client into thinking it has been
   * created. If any files are then dragged into the folder then it WILL be created.
   *
   * @param req HTTP Request from client
   * @param resp HTTP Response to client
   * @throws RemoteException
   */
  public void mkcolMethod(
      HttpServletRequest req, HttpServletResponse resp, StagingFile staging, String filename)
      throws RemoteException {
  ...
  /**
   * Handles the COPY method, which copies a file from one location to another.
   *
   * @param req HTTP Request from client
   * @param resp HTTP Response to client
  ...
  /**
   * Handles the MOVE method, which moves a file from one location to another, which is also used
   * for renaming a file.
   *
   * @param req HTTP Request from client
   * @param resp HTTP Response to client
  ...
  private void copyOrMove(
    ...
    final boolean success =
        (copy
            ? fileSystemService.copy(staging, filename, newname) != null
  ...
  private void recurseDirectory(
    ...
    if (dir.isFolder()) {
      for (FileEntry file : dir.getFiles()) {
        ...
        String newuri =
            uri + URLEncoder.encode(name, Utils.CHARSET_ENCODING).replaceAll("\\+", "%20");
        ...
        // Recurse directory
        recurseDirectory(
            requestedProperties, newuri + '/', propBag, staging, file.getName(), file, depth - 1);
      }
    }
  }

  private void addResponse(
      final PropBagEx multiStatusBag, final WebdavProps props, final String href) {
