    ...
    if (webControlExtensions == null) {
      ...
      Collection<Extension> extensions =
          pluginService.getConnectedExtensions(
              "com.tle.web.wizard", "webControl"); // $NON-NLS-1$ //$NON-NLS-2$
      for (Extension extension : extensions) {
        webControlExtensions.put(
            extension.getParameter("type").valueAsString(), extension); // $NON-NLS-1$
      }
    }
      ...
      WebControl webControl =
          (WebControl)
              pluginService.getBean(
                  plugin, extension.getParameter("class").valueAsString()); // $NON-NLS-1$
