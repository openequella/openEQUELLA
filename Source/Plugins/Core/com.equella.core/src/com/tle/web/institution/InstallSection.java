...
public class InstallSection extends AbstractPrototypeSection<InstallSection.Model> {
  ...
  public void registered(String id, SectionTree tree) {
    ...
    installButton.setClickHandler(
        events
            .getNamedHandler("runInstall")
            .addValidator(
                new SimpleValidator(
                        new EqualityExpression(
                            password.createGetExpression(), passwordConfirm.createGetExpression()))
                    .setFailureStatements(Js.alert_s(LABEL_PASSWORDCONFIRM))));
  }
  ...
  public void runInstall(SectionInfo info) {
    ...
    if (model.getErrors().isEmpty()) {
      InstallSettings installSettings =
          new InstallSettings(
              Hash.hashPassword(passwordText),
              emailsText,
              smtpText,
              mailUsername,
              mailPassword,
