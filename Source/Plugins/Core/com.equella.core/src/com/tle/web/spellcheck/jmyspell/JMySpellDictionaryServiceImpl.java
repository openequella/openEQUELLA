...
public class JMySpellDictionaryServiceImpl implements DictionaryService {
  private Map<String, OpenOfficeSpellDictionary> dictionaries =
      new HashMap<String, OpenOfficeSpellDictionary>();
  private Map<String, TLEDictionary> dictMap;
  private Map<String, Extension> extMap;
  @Inject private PluginTracker<TLEDictionary> tracker;
  ...
  public synchronized OpenOfficeSpellDictionary getDictionary(String code) {
    if (!dictionaries.containsKey(code)) {
      ...
      /**
       * for OpenOfficeSpellDictionary(InputStream zippedStream, File personalDict): It seems that
       * personalDict is a personal dictionary file where users can add words to their ignore list,
       * add new words etc. Since we don't use that, we can leave it empty. It also seems that the
       * constructor method is not thread safe, which means since we do this in a loop the
       * dictionaries arne't being built properly. The results will be NullPointerExceptions all
       * over the place when we try to use the dictionary. Thus, run in background is set to false.
       */
