  ...
  public void execute(TemporaryFileHandle staging, InstitutionInfo instInfo, ConverterParams params)
    ...
    try (InputStream in = getClass().getResourceAsStream("/entities/mycontentcollection.xml")) {
      ...
      final FileHandle handle =
          isBucketed(itemdefFolder)
              ? new BucketFile(itemdefFolder, MyContentConstants.MY_CONTENT_UUID)
              : itemdefFolder;
    ...
    try (InputStream in = getClass().getResourceAsStream("/entities/mycontentschema.xml")) {
      ...
      final FileHandle handle =
          isBucketed(schemaFolder)
              ? new BucketFile(schemaFolder, MyContentConstants.MY_CONTENT_SCHEMA_UUID)
              : schemaFolder;
      fileSystemService.write(
          handle, MyContentConstants.MY_CONTENT_SCHEMA_UUID + ".xml", in, false);
    }
  }
}
