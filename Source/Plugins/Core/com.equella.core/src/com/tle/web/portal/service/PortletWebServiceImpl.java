...
public class PortletWebServiceImpl
  ...
  @Inject private Provider<PortletRendererWrapper> rendererFactory;
    ...
    return !CurrentUser.wasAutoLoggedIn()
        && !aclService
            .filterNonGrantedPrivileges(
                Arrays.asList(new String[] {PortletConstants.CREATE_PORTLET}), true)
            .isEmpty();
  }
  ...
  public boolean canAdminister() {
    return !aclService
        .filterNonGrantedPrivileges(
            Collections.singleton(PortletConstants.PRIV_ADMINISTER_PORTLETS), true)
        .isEmpty();
  }
  ...
  private PortletContentRenderer<?> getRendererForType(String type) {
    ...
    if (extension == null) {
      throw new RuntimeApplicationException(
          "Unsupported portlet rendering type " + type); // $NON-NLS-1$
    }
  ...
  public void close(SectionInfo info, String portletUuid) {
    firePortletsChanged(
        info, CurrentUser.getUserID(), portletUuid, false, PortletUpdateEventType.REMOVED);
  ...
  public void restore(SectionInfo info, String portletUuid) {
    ...
    firePortletsChanged(
        info, CurrentUser.getUserID(), portletUuid, false, PortletUpdateEventType.CREATED);
  }
  ...
  public void move(SectionInfo info, Portlet prev, Portlet toMove, int position) {
    ...
    final Triple<
            List<Entry<Portlet, PortletPreference>>,
            List<Entry<Portlet, PortletPreference>>,
            List<Entry<Portlet, PortletPreference>>>
        columns = reorderColumns(null, prefs);
    ...
    firePortletsChanged(
        info, userId, toMove.getUuid(), toMove.isInstitutional(), PortletUpdateEventType.MOVED);
  }
  ...
  protected DefaultSectionTree buildRendererTree(String userId) {
    ...
    final DefaultSectionTree renderTree =
        new DefaultSectionTree(controller, new SectionNode("p")); // $NON-NLS-1$
    final Map<Portlet, PortletPreference> prefs =
        portletService.getPreferences(portletService.getViewablePortletsForDisplay());
  ...
  /**
   * @param tree If not specified then you will get the two columns back in the return result ,
   *     otherwise you won't (saves on unneccessary processing)
   * @param prefs
   * @return The left and right column preferences (only if tree not specified)
   */
  private Triple<
          List<Entry<Portlet, PortletPreference>>,
          List<Entry<Portlet, PortletPreference>>,
          List<Entry<Portlet, PortletPreference>>>
      reorderColumns(DefaultSectionTree tree, Map<Portlet, PortletPreference> prefs) {
    final List<Entry<Portlet, PortletPreference>> top =
        new ArrayList<Entry<Portlet, PortletPreference>>();
    final List<Entry<Portlet, PortletPreference>> left =
        new ArrayList<Entry<Portlet, PortletPreference>>();
    final List<Entry<Portlet, PortletPreference>> right =
        new ArrayList<Entry<Portlet, PortletPreference>>();
    ...
    if (tree != null) {
      final Map<String, SectionId> portletUuidToSectionId =
    ...
    return new Triple<
        List<Entry<Portlet, PortletPreference>>,
        List<Entry<Portlet, PortletPreference>>,
        List<Entry<Portlet, PortletPreference>>>(top, left, right);
  }
  ...
  protected void firePortletsChanged(
    ...
    PortletsUpdatedEvent event =
        new PortletsUpdatedEvent(userId, portletUuid, institutional, eventType);
  ...
  protected static class PortletPrefComparator
      implements Comparator<Entry<Portlet, PortletPreference>>, Serializable {
    @Override
  ...
  private static class PortletSectionIdTransform
      implements Function<Entry<Portlet, PortletPreference>, SectionId> {
  ...
  public void setPluginService(PluginService pluginService) {
    editorTracker =
        new PluginTracker<PortletEditor>(
            pluginService, "com.tle.web.portal", "portletEditor", "id");
    editorTracker.setBeanKey("class");

    rendererTracker =
        new PluginTracker<PortletContentRenderer<?>>(
            pluginService, "com.tle.web.portal", "portletRenderer", "id");
