...
public class BooleanListRenderer implements JSDisableable, SectionRenderable, JSListComponent {
  ...
  @Nullable private JSHandler changeHandler;
  @Nullable private SimpleFunction changeFunc;
  ...
  public <T> List<ListOption<T>> renderOptionList(RenderContext context) {
    ...
    for (Option<?> option : rawOptions) {
      ...
      bstate.setRendererType(
          state.isMultiple() ? RendererConstants.CHECKBOX : RendererConstants.RADIO);
  ...
  public JSExpression createGetExpression() {
    return new FunctionCallExpression(
        state.isMultiple()
            ? FormValuesLibrary.GET_CHECKED_VALUES
            : FormValuesLibrary.GET_CHECK_VALUE,
