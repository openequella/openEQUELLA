...
@NonNullByDefault
@Bind
public class TreeNavigationSection
    extends AbstractTreeViewerSection<TreeNavigationSection.TreeNavigationModel>
  ...
  @Inject private ScormAPIHandler scormAPIHandler;
  ...
  protected List<ItemNavigationNode> getTreeNodes(
      SectionInfo info, List<? extends IAttachment> attachments, boolean scorm) {
    ...
    for (IAttachment attachment : attachments) {
      try {
        ...
        IMSManifest imsManifest =
            imsService.getImsManifest(vitem.getFileHandle(), packageZip, true);
        Item item2 = new Item();
      ...
      if (ims == null) {
        ...
        if (fileSystem.fileIsDir(handle, FileSystemService.SCORM_FOLDER)
            && fileSystem.fileExists(
                handle, PathUtils.filePath(FileSystemService.SCORM_FOLDER, unadornedFilename))) {
  ...
  public static class TreeNavigationModel extends AbstractTreeViewerModel {
    ...
    @Bookmarked private boolean download;
