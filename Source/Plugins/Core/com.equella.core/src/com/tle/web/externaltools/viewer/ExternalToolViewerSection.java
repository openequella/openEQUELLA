...
public class ExternalToolViewerSection
  ...
  @TreeLookup private RootItemFileSection rootItemFileSection;
  ...
  public SectionResult view(RenderContext context, ViewItemResource resource) throws IOException {
    ...
    String ltiProviderUUID =
        (String) attachment.getData(ExternalToolConstants.EXTERNAL_TOOL_PROVIDER_UUID);
    ...
    boolean shareEmail =
        deriveBoolean(
            attachmentData.get(ExternalToolConstants.SHARE_EMAIL),
            toolProvider != null ? toolProvider.getShareEmail() : null);
    boolean shareName =
        deriveBoolean(
            attachmentData.get(ExternalToolConstants.SHARE_NAME),
  ...
  /**
   * In the case where an external consumer (such as an LMS) is using EQUELLA as a front for a
   * actual tool provider.
   */
  private void equellaAsProvider(
  ...
  private void addDynamicLtiParameters(
    ...
    formParams.put(
        ExternalToolConstants.LAUNCH_PRESENTATION_LOCALE, CurrentLocale.getLocale().toString());
    ...
    for (NameValue customParam : customParams) {
      if (!customParam.getFirst().startsWith(ExternalToolConstants.CUSTOM_PARAMS_POST_PREFIX)) {
        formParams.put(
            ExternalToolConstants.CUSTOM_PARAMS_POST_PREFIX + customParam.getFirst(),
    ...
    formParams.put(
        ExternalToolConstants.CONTEXT_TITLE, CurrentLocale.get(item.getName(), item.getUuid()));
  ...
  private void addDefaultLtiParameters(Map<String, String> formParams) {
    LOGGER.debug(
        "Using the optional-config external.tool.contact.email of ["
            + externalToolContactEmail
            + "]");
    ...
    formParams.put(
        ExternalToolConstants.CONTEXT_TYPE, ExternalToolConstants.CONTEXT_TYPE_EQUELLA_ITEM);
  ...
  /**
   * For the purposes of imsglobal certification, we needed to be able to send with Learner role (or
   * none). During testing this was done with a one-off by-hand 'variation' by changing this Boolean
   * variable in a debugging session to null or TRUE.
   */
  ...
  /**
   * overwrite or create these parameters with values from lms: resource_link_id,
   * tool_consumer_info_*, lis_outcome_service_url, lis_result_sourcedid,
   * launch_presentation_return_url, context_*
   */
  private void overwriteOrAddWithRequestParameters(
      Map<String, String> formParams,
    ...
    final String[] rqParams = {
      ExternalToolConstants.LAUNCHER_ROLES,
      ExternalToolConstants.ROLE_SCOPE_MENTOR,
      ExternalToolConstants.RESOURCE_LINK_ID,
      ExternalToolConstants.RESOURCE_LINK_TITLE,
      ExternalToolConstants.RESOURCE_LINK_DESCRIPTION,
      ExternalToolConstants.USER_ID,
      ExternalToolConstants.LAUNCH_PRESENTATION_RETURN_URL,
      ExternalToolConstants.LAUNCH_PRESENTATION_DOCUMENT_TARGET,
      ExternalToolConstants.LAUNCH_PRESENTATION_LOCALE,
      ExternalToolConstants.CONTEXT_ID,
      ExternalToolConstants.CONTEXT_TYPE,
      ExternalToolConstants.CONTEXT_TITLE,
      ExternalToolConstants.CONTEXT_LABEL
    };
    // @formatter:on
    ...
    if (!Check.isEmpty(request.getParameter(ExternalToolConstants.TOOL_CONSUMER_INFO_VERSION))) {
      String productVersion =
          request.getParameter(ExternalToolConstants.TOOL_CONSUMER_INFO_VERSION);
    ...
    if (shareEmail) {
      if (!Check.isEmpty(
          request.getParameter(ExternalToolConstants.LIS_PERSON_CONTACT_EMAIL_PRIMARY))) {
        String personEmail =
            request.getParameter(ExternalToolConstants.LIS_PERSON_CONTACT_EMAIL_PRIMARY);
    ...
    for (Enumeration<String> paramNames = request.getParameterNames();
  ...
  /**
   * To enable the external tool to send gradebook information back to the LMS (via EQUELLA), we
   * substitute the LMS's outcome_service_url for EUQLLA's own, meanwhile preserving the original
   * LMS outcome_service_url as an appended item of information when EQUELLA onsends to LMS's
   * lis_result_sourcedid.<br>
   * We extend the original lis_result_sourcedid with such information as EQUELLA can use to
   * identify where to onsend the gradebook results back to.
   *
   * @param formParams
   * @param request
   */
  protected void addResultOutcomeParameters(
      Map<String, String> formParams, HttpServletRequest request) {
    String lmsOutcomeServiceUrl =
        request.getParameter(ExternalToolConstants.LIS_OUTCOME_SERVICE_URL);
    // If there's an outcome service url, we want to replace it with our own
    if (!Check.isEmpty(lmsOutcomeServiceUrl)) {
      String instiUrlExtended =
          institutionService.institutionalise(ExternalToolConstants.OUTCOME_SERVICE_URL_PATH);
    ...
    SourcedIdPackage sourcedIdPackage =
        new SourcedIdPackage(lmsResultSourcedid, lmsOutcomeServiceUrl, oAuthConsumerKeyForLms);

    Gson gson =
        new GsonBuilder()
            .registerTypeAdapter(SourcedIdPackage.class, new SrcdIdPkgSerializer())
            .create();
  ...
  /**
   * If EQUELLA is provider, any resource_link_id sent by consumer is irrelevant, but conformance to
   * imsglobal certifications requires that we throw an error if resource_link_id is absent.
   * Consumer could send any random stuff and we'd be happy.
   *
  ...
  /**
   * The error that Moodle showed us how to do: where URL query parameters exist, add them to (a
   * copy of) the form for the purposes of calculating the OAuth signature, but ensure that the
   * original URL including its query string, and params (without the added query-string pairs) are
