...
public class ConnectorManagementResultsSection
    extends AbstractSearchResultsSection<
        ConnectorManagementListEntry,
        ConnectorManagementSearchEvent,
        ConnectorManagmentSearchResultEvent,
        SearchResultsModel> {
  @Inject private Provider<ConnectorManagementListEntry> entryProvider;
  ...
  public void treeFinished(String id, SectionTree tree) {
    ...
    editDialog.setOkCallback(
        selectionSection.getUpdateSelection(
            tree, events.getEventHandler("refreshContent"))); // $NON-NLS-1$
  }
    ...
    try {
      success =
          repositoryService.deleteContent(
              connector, CurrentUser.getUsername(), itemId.getContentId());
    } catch (Exception e) {
  ...
  public void processResults(SectionInfo info, ConnectorManagmentSearchResultEvent results) {
    ...
    Collection<String> words =
        new DefaultSearch.QueryParser(results.getEvent().getSearchedText()).getHilightedList();
    ListSettings<ConnectorManagementListEntry> listSettings =
        connectorManagementItemList.getListSettings(info);
  ...
  protected Label getNoResultsTitle(
      SectionInfo info,
      ConnectorManagementSearchEvent searchEvent,
  ...
  @Override
  protected Label getSuggestions(
      SectionInfo info,
      ConnectorManagementSearchEvent searchEvent,
      ConnectorManagmentSearchResultEvent resultsEvent) {
    if (querySection.getConnectorList().getSelectedValue(info) == null) {
  ...
  protected ConnectorManagmentSearchResultEvent createResultsEvent(
    ...
    try {
      ...
      if (connectorSearchEvent.isUserFiltered()) {
        unfilteredCount =
            repositoryService.getUnfilteredAllUsagesCount(
                connector, CurrentUser.getUsername(), connectorSearchEvent.getQuery(), true);
      }
      ...
      for (ConnectorContent content : allUsage) {
        ...
        if (uuid != null) {
          ...
          query.addQuery(
              new NodeInQuery(uuid, false, Collections.singleton(FreeTextQuery.FIELD_UUID), null));
          query.addQuery(
              new NodeInQuery(
                  String.valueOf(version),
                  false,
                  Collections.singleton(FreeTextQuery.FIELD_VERSION),
                  null));
      ...
      FreetextSearchResults<FreetextResult> searchResults =
          freeTextService.search(rawSearch, 0, count);

      SimpleSearchResults<ConnectorContent> simpleResults =
          new SimpleSearchResults<ConnectorContent>(
              allUsage, allUsage.size(), offset, findAllUsages.getAvailable());

      return new ConnectorManagmentSearchResultEvent(
          connectorSearchEvent,
          simpleResults,
          searchResults,
          unfilteredCount - findAllUsages.getAvailable(),
          connector);
