...
public class EquellaRepoSessionDialog
    extends AbstractOkayableDialog<EquellaRepoSessionDialog.EquellaRepoDoSessionModel> {
  ...
  protected SectionRenderable getRenderableContents(RenderContext info) {
    ...
    final String user =
        (equellaSettings.isUseLoggedInUser()
            ? CurrentUser.getUsername()
            : equellaSettings.getUsername());
    ...
    try {
      params.put(
          ...
          TokenGenerator.createSecureToken(
              user,
              equellaSettings.getSharedSecretId(),
              equellaSettings.getSharedSecretValue(),
              null));
    } catch (IOException e) {
