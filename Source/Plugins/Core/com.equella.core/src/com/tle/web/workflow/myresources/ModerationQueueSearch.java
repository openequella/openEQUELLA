...
public class ModerationQueueSearch extends AbstractPrototypeSection<Object>
    implements MyResourcesSubSearch {
  static {
    PluginResourceHandler.init(ModerationQueueSearch.class);
  }

  @Inject private ModerationQueueItemList itemList;
  @Inject private ModerationQueueSort sort;
  @Inject private ModerationQueueStatusFilter statusFilter;
  @Inject private FilterByWorkflowTaskSection taskFilter;
  @Inject private WorkflowFromCollectionSection workflowSelector;
  private final List<MyResourcesSubSubSearch> subSearches =
      new ArrayList<MyResourcesSubSubSearch>();
    ...
    MyResourcesSearch search =
        new MyResourcesSearch() {
          ...
          public void addExtraMusts(List<List<Field>> musts) {
            musts.add(
                Arrays.asList(
                    new Field(FreeTextQuery.FIELD_MODERATING, "true"),
                    new Field(FreeTextQuery.FIELD_ITEMSTATUS, ItemStatus.REJECTED.toString())));
          }
    ...
    tree.addDelayedRegistration(
        new DelayedRegistration() {
          ...
          public void register(SectionTree tree) {
            ...
            FilterByCollectionSection collectionSection =
                tree.lookupSection(FilterByCollectionSection.class, null);

            // Can't TreeLookup this?
            FilterByItemStatusSection statusSection =
                tree.lookupSection(FilterByItemStatusSection.class, null);
