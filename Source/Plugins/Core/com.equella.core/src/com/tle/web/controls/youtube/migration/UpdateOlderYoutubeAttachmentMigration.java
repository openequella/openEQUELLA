 ...
 * videoId as a discrete value. The youtube player now looks for the videoId saved in the
 * Attachments metadata, and composes a new URL on the fly, no longer restricted to the 'old-style'
 * playUrl value. Accordingly, for older youtube attachments which lack the videoId, it is necessary
 * to parse the playUrl value, extract and preserve the video Id substring as the videoId value.<br>
 * Looking for URL values of the form
 * http://www.youtube.com/x/y/z/AA00BB11CC3?foo=whatever&bar=whateverelse, where we're indifferent
 * as to the path segments (x/y/z) if any between the server and the 11-character identifier, and
 * equally indifferent as to the param key-values pairs if any. What we're looking for is the
 * vidoeId string which is the last path segment before the key-value parameters. (In practice this
 * videoId appears to be always 11 characters long). We look for an explicit videoId key-value in
 * the attachments attribute Map, and having established the absence thereof, update the attachments
 * data map with an explicit vidoeId extracted from the full URL.<br>
 * NB - some youtube URLs in the system lack a '?' dividing the URL path from the query key-value
 * parameters eg http://www.youtube.com/x/y/z/AA00BB11CC3&foo=woteva&bar=wotevaelse, hence we allow
 * our parse of the URL to accept this oddity.
 *
 * @author larry
 */
...
public class UpdateOlderYoutubeAttachmentMigration extends AbstractHibernateDataMigration {
  ...
  public MigrationInfo createMigrationInfo() {
    return new MigrationInfo(
        keyPrefix + "older.youtube.title", keyPrefix + "older.youtube.description");
  }
  ...
  protected void executeDataMigration(
    ...
    String query =
        "From Attachment a "
            + " where a.data like '%<string>"
            + PLAY_URL_KEY
            + "</string>%'"
            + " and a.data like '%<string>"
            + YOUTUBE_URL
            + "%'";
