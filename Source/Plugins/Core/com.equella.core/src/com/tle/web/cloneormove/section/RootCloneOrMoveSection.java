...
public class RootCloneOrMoveSection extends AbstractContentSection<RootCloneOrMoveModel> {
  ...
  public SectionResult renderHtml(final RenderEventContext context) throws Exception {
    ...
    if (isMove(context)) {
      addDefaultBreadcrumbs(
          context, ParentViewItemSectionUtils.getItemInfo(context), MOVE_TITLE_LABEL);
      cloneOrMoveSection.setMove(context, true);
    } else if (isClone(context)) {
      addDefaultBreadcrumbs(
          context, ParentViewItemSectionUtils.getItemInfo(context), CLONE_TITLE_LABEL);
      cloneOrMoveSection.setMove(context, false);
    } else {
      throw new RuntimeException(
          "User does not have sufficient privileges for the requested operation"); //$NON-NLS-1$
    }
  ...
  public void registered(final String id, final SectionTree tree) {
    ...
    cloneOrMoveSection.setClientSideCallbacks(
        events.getSubmitValuesFunction("proceed")); // $NON-NLS-1$
  ...
  public void proceed(
      ...
      final int cloneOption,
      final String newCollectionUuid,
    ...
    webWizardService.forwardToCloneItemWizard(
        info,
        newCollectionUuid,
        model.getUuid(),
        model.getVersion(),
    ...
    return model.getIsMove()
        && !aclManager
            .filterNonGrantedPrivileges(
                srcItem, Collections.singletonList(CloneOrMoveSection.MOVE_ITEM))
            .isEmpty();
  }
  ...
  protected boolean isClone(final SectionInfo info) {
    ...
    return !model.getIsMove()
        && !aclManager
            .filterNonGrantedPrivileges(
                srcItem, Collections.singletonList(CloneOrMoveSection.CLONE_ITEM))
            .isEmpty();
  }
