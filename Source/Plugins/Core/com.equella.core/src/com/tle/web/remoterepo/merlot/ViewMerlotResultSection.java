...
public class ViewMerlotResultSection
    extends RemoteRepoViewResultSection<
        ViewMerlotResultSection.ViewMerlotResultModel, MerlotListEntry, MerlotSearchResult> {

  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(ViewMerlotResultSection.class);
  ...
  protected void getContents(
      SectionInfo info, ViewMerlotResultModel model, List<SectionRenderable> contents) {
    ...
    if (result.getPublishedDate() != null) {
      addField(
          contents,
          "result.creationdate",
          dateRendererFactory.createDateRenderer(result.getPublishedDate()));
    }
    if (result.getModifiedDate() != null) {
      addField(
          contents,
          "result.modifieddate",
          dateRendererFactory.createDateRenderer(result.getModifiedDate()));
    }
    addField(
        contents,
        "result.categories",
        result.getCategories() == null ? null : result.getCategories().toArray());
    addField(contents, "result.community", result.getCommunity());
    addField(contents, "result.creativecommons", result.getCreativeCommons());
    addField(
        contents,
        "result.audiences",
        result.getAudiences() == null ? null : result.getAudiences().toArray());
  ...
  private MerlotSearchResult resolveRecord(SectionInfo info, FederatedSearch search, int index) {
    final MerlotRemoteRepoSearchEvent merlot =
        new MerlotRemoteRepoSearchEvent(getRootRemoteRepoSection(), search);
  ...
  public static class ViewMerlotResultModel
      extends RemoteRepoViewResultSection.RemoteRepoViewResultModel<MerlotSearchResult> {
    @Bookmarked(name = "v")
