...
public class QtiPlayViewerSection extends AbstractViewerSection<QTIPlayViewerModel>
    implements ResourceViewerAware, HtmlRenderer {
  ...
  @Component private Link returnLink;
  ...
  public SectionResult view(RenderContext info, ViewItemResource resource) throws IOException {
    ...
    if (currentTestPartKey != null) {
      ...
      final TestPartSessionState testPartSessionState =
          testSessionState.getTestPartSessionStates().get(testPart.getKey());
      ...
      if (testPartSessionState.isEntered() || testPartSessionState.isEnded()) {
        ...
        model.setSections(
            populateSectionModels(info, resource, quiz, testPart, testSessionState, ended));
  ...
  private void setupCloseLinks(RenderContext info) {
    ...
    if (ltiData != null) {
      ...
      if (courseTitle != null) {
        returnLink.setLabel(
            info, new KeyLabel(KEY_RETURN_TO_NAMED_COURSE, new TextLabel(courseTitle)));
      } else {
  ...
  private List<TestSectionModel> populateSectionModels(
      RenderContext context,
      ViewItemResource resource,
      ResolvedAssessmentTest resolvedAssessmentTest,
      TestPlanNode testPartNode,
    ...
    final TestSessionController testSessionController =
        qtiWebService.getTestSessionController(context, resource);

    final TestProcessingMap testProcessingMap = testSessionController.getTestProcessingMap();
    List<TestPlanNode> sectionsOrRoots =
        testPartNode.searchDescendants(TestNodeType.ASSESSMENT_SECTION);
    if (sectionsOrRoots.size() == 0) {
      sectionsOrRoots = Lists.newArrayList(testPartNode);
    }

    final String testRootPath =
        PathUtils.getParentFolderFromFilepath(
            resolvedAssessmentTest.getTestLookup().getSystemId().toString());
    final Map<URI, ResolvedAssessmentItem> resolvedAssessmentItemBySystemIdMap =
        resolvedAssessmentTest.getResolvedAssessmentItemBySystemIdMap();
    ...
    for (TestPlanNode sectionOrRootNode : sectionsOrRoots) {
      ...
      if (sectionOrRootNode.getTestNodeType() == TestNodeType.ASSESSMENT_SECTION) {
        final AssessmentSection section =
            (AssessmentSection) testProcessingMap.resolveAbstractPart(sectionOrRootNode);
      ...
      final List<TestPlanNode> questions =
          sectionOrRootNode.searchDescendants(TestNodeType.ASSESSMENT_ITEM_REF);

      int qIndex = 0;
      for (TestPlanNode questionNode : questions) {
        qIndex++;
        boolean allowReviewForQuestion =
            questionNode.getEffectiveItemSessionControl().isAllowReview();

        final AssessmentItemRef assessmentItemRef =
            (AssessmentItemRef) testProcessingMap.resolveAbstractPart(questionNode);
        ...
        final ResolvedAssessmentItem resolvedAssessmentItem =
            resolvedAssessmentItemBySystemIdMap.get(URI.create(fullItemPath));
        final AssessmentItem assessmentItem =
            resolvedAssessmentItem.getItemLookup().extractIfSuccessful();
        ...
        final HtmlLinkState qlink =
            new HtmlLinkState(new TextLabel(assessmentItem.getTitle()), handler);
        ...
        if (selected) {
          if (!ended || (ended && allowReviewForQuestion)) {
            ...
            final QtiNodeRenderer questionRenderable =
                questionRenderers.chooseRenderer(
                    assessmentItem, new PlayerContext(context, resource));
            model.setQuestionRenderable(questionRenderable);
          }
        }

        final ItemSessionState itemSessionState =
            testSessionState.getItemSessionStates().get(questionKey);
  ...
  private SectionRenderable renderEndSummary(
      RenderContext info,
      ViewItemResource resource,
      ResolvedAssessmentTest resolvedAssessmentTest,
      TestSessionState testSessionState) {
    ...
    final List<TestFeedback> feedbacks =
        resolvedAssessmentTest.getTestLookup().extractIfSuccessful().getTestFeedbacks();
  ...
  public void registered(String id, SectionTree tree) {
    ...
    submitButton.setClickHandler(
        events.getNamedHandler("onSubmitTest").addValidator(new Confirm(LABEL_CONFIRM_SUBMIT)));
  ...
  /**
   * Serve up the image (or whatever) relative to the current question (if any)
   *
   * @param info
   * @param href
   */
  ...
  public void viewResource(SectionInfo info, String href) {
    ...
    final FileContentStream stream =
        fileSystemService.getInsecureContentStream(
            fileHandle, finalPath, mimeService.getMimeTypeForFilename(finalPath));
  ...
  private boolean matchesProtectedFile(
      TestPlan testPlan, FileHandle fileHandle, String testXmlPath, String path) {
  ...
  public class PlayerContext implements QtiViewerContext {
    ...
    @Nullable private ItemSessionController itemSessionController;
    ...
    public List<String> getValues(Identifier responseIdentifier) {
      ...
      final ItemSessionState currentItemSessionState =
          testSessionState.getCurrentItemSessionState();
      final ResponseData responseValue =
          currentItemSessionState.getRawResponseData(responseIdentifier);
      ...
      if (!hasResolvedItemSessionController) {
        ...
        if (currentItemKey != null) {
          ...
          itemSessionController =
              (ItemSessionController) testController.getItemProcessingContext(itemNode);
