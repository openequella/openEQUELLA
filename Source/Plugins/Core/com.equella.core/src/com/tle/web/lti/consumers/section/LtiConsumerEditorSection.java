...
public class LtiConsumerEditorSection
    extends AbstractEntityEditor<LtiConsumerEditingBean, LtiConsumer, LtiConsumerEditorModel> {
  ...
  @Component private TextField consumerKeyField;
  ...
  @Component private SelectionsTable otherRolesTable;

  @Component
  @PlugKey("editor.table.roles.add")
  private Link addOtherRole;

  @Component private SelectionsTable unknownUserGroupsTable;

  @Component
  @PlugKey("editor.table.groups.add")
  private Link addUnknownUserGroup;

  @Component private TextField customRoleField;
  @Component private SelectionsTable customRolesTable;
  @Inject private SelectRoleDialog customRoleDialog;

  @Inject private ExpressionSelectorDialog allowedSelector;
  @Inject private SelectRoleDialog selectInstructorDialog;
  @Inject private SelectRoleDialog selectOtherDialog;
  @Inject private SelectGroupDialog unknowUserGroupDialog;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    allowedSelector.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("expressionCallback"), "allowed"));
    ...
    selectInstructorDialog.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("instructorRoleCallback"), "instructorrole"));
    ...
    removeInstructorRoleFunc =
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("removeRole"), "instructorrole");

    selectOtherDialog.setAjax(true);
    selectOtherDialog.setMultipleRoles(true);
    selectOtherDialog.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("otherRoleCallback"), "otherrole"));
    ...
    removeOtherRoleFunc =
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("removeRole"), "otherrole");

    unknownUserList.setListModel(
        new EnumListModel<LtiConsumerConstants.UnknownUser>(
            UKNOWN_USER_PFX, true, UnknownUser.values()));
    unknownUserList.addChangeEventHandler(
        ajax.getAjaxUpdateDomFunction(tree, this, null, "unknownusergroup"));

    unknowUserGroupDialog.setAjax(true);
    unknowUserGroupDialog.setMultipleGroups(true);
    unknowUserGroupDialog.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("unknownUserGroupCallback"), "unknownusergroup"));
    ...
    removeGroupFunc =
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("removeGroup"), "unknownusergroup");

    customRoleDialog.setAjax(true);
    customRoleDialog.setMultipleRoles(true);
    customRoleDialog.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("customRolesCallback"), "customrole"));
  ...
  public void expressionCallback(SectionInfo info, String selectorId, String expression) {
    LtiConsumerEditingBean bean =
        getEntityService().loadSession(getModel(info).getSessionId()).getBean();
  ...
  public void instructorRoleCallback(SectionInfo info, String rolesJson) {
    ...
    Set<String> selectedRoles =
        SelectRoleDialog.rolesFromJsonString(rolesJson).stream()
            .map(sr -> sr.getUuid())
  ...
  public void unknownUserGroupCallback(SectionInfo info, String groupsJson) {
    ...
    Set<String> selectedGroups =
        SelectGroupDialog.groupsFromJsonString(groupsJson).stream()
            .map(sg -> sg.getUuid())
            .collect(Collectors.toSet());
  ...
  public void customRolesCallback(SectionInfo info, String rolesJson) {
    EntityEditingSession<LtiConsumerEditingBean, LtiConsumer> session =
        getEntityService().loadSession(getModel(info).getSessionId());
    ...
    Set<Pair<String, String>> roleMappings =
        SelectGroupDialog.groupsFromJsonString(rolesJson).stream()
            .map(sg -> new Pair<String, String>(ltiRole, sg.getUuid()))
            .collect(Collectors.toSet());
  ...
  public void removeCustom(SectionInfo info, String ltiRole, String roleUuid) {
    ...
    if (customRoles.size() > 1) {
      Pair<String, String> filtered =
          customRoles.stream()
              .filter(crm -> crm.getFirst().equals(ltiRole) && crm.getSecond().equals(roleUuid))
              .findFirst()
              .get();
  ...
  protected void loadFromSession(
      SectionInfo info, EntityEditingSession<LtiConsumerEditingBean, LtiConsumer> session) {
    final LtiConsumerEditingBean bean = session.getBean();
    consumerKeyField.setValue(
        info, bean.getConsumerKey() == null ? UUID.randomUUID().toString() : bean.getConsumerKey());
    consumerSecretField.setValue(
        info,
    ...
    if (!Check.isEmpty(bean.getCustomRoles())) {
      ...
      for (Pair<String, String> entry : bean.getCustomRoles()) {
        ...
        customRoles.add(
            new CustomRoleMapping(
                entry.getFirst(), new SelectedRole(roleInfo.getUniqueID(), roleInfo.getName())));
      }
      ...
      switch (type) {
        case INSTRUCTOR:
          selection.setViewAction(
              new LabelRenderer(new KeyLabel(INSTRUCTOR_ARROW, role.getDisplayName())));
          actions.add(
              makeRemoveAction(
                  TABLE_REMOVE_ACTION, new OverrideHandler(removeInstructorRoleFunc, role, type)));
          break;
        case UNKNOWN:
          selection.setViewAction(
              new LabelRenderer(new KeyLabel(UNKNOWN_ARROW, role.getDisplayName())));
          actions.add(
              makeRemoveAction(
                  TABLE_REMOVE_ACTION, new OverrideHandler(removeOtherRoleFunc, role, type)));
      }
    }
  }
  ...
  private class CustomRoleTableModel extends DynamicSelectionsTableModel<CustomRoleMapping> {
    ...
    protected void transform(
        SectionInfo info,
        SelectionsTableSelection selection,
        CustomRoleMapping thing,
        List<SectionRenderable> actions,
        int index) {
      selection.setViewAction(
          new LabelRenderer(
              new TextLabel(thing.getLtiRole() + " ⇨ " + thing.getEquellaRole().getDisplayName())));
      actions.add(
          makeRemoveAction(
              TABLE_REMOVE_ACTION,
              new OverrideHandler(
                  removeCustomRolesFunc, thing.getLtiRole(), thing.getEquellaRole().getUuid())));
    }
  }
  ...
  private class GroupsTableModel extends DynamicSelectionsTableModel<SelectedGroup> {
    ...
    protected void transform(
      ...
      actions.add(
          makeRemoveAction(TABLE_REMOVE_ACTION, new OverrideHandler(removeGroupFunc, group)));
    }
  }
