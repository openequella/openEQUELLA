...
public abstract class AbstractSoapService {
  ...
  /**
   * client does not _need_ to pass in a session id, it's just used to verify their cookies are
   * working properly.
   *
   * @param session The current session id as returned from login() or loginWithToken()
   */
    ...
    if (!Check.isEmpty(session)) {
      ...
      if (!userSession.equals(session)) {
        throw new RuntimeException(
            CurrentLocale.get(
                "com.tle.web.remoting.soap.error.nocookies", userSession)); // $NON-NLS-1$
      }
    }
  }
}
