...
/**
 * Stolen from EPS - allow OAuth token to set system user for Institution manipulation
 *
 * @author Dustin
 */
@Bind
...
public class SystemUserStateHook implements UserStateHook {
  @Inject private SystemConfigService sysConfig;
  ...
  public UserStateResult getUserState(HttpServletRequest request, UserState userState)
    ...
    if (xauth != null) {
      ...
      if (tokenType.equals("system_token")) {
        ...
        if (Hash.checkPasswordMatch(
            CurrentInstitution.get().getAdminPassword(), ensurePassword(token))) {
  ...
  private String ensurePassword(String[] token) {
    if (token.length != 2) {
      throw new WebException(
          403, "access_denied", "Invalid access_token format in X-Authorization header");
    }
    return token[1];
  }
