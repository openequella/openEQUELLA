...
public class UniversalResourcesDialog
    extends AbstractOkayableDialog<UniversalResourcesDialog.UniversalResourcesDialogModel>
    implements UniversalControlState {
  private static final String WIDTH = "810px";
  private static final CssInclude CSS =
      CssInclude.include(
              ResourcesService.getResourceHelper(UniversalResourcesDialog.class)
                  .url("css/universalresource.css"))
          .hasRtl()
          .make();
  ...
  @Inject private PluginTracker<AttachmentHandler> handlersTracker;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    resourceTypePicked.addReadyStatements(
        new ExternallyDefinedFunction(
            "setupPickResourcesType", new IncludeFile(SCRIPT_URL, JQueryUICore.PRERENDER)),
        Jq.$(pickResourceTypeTable),
        resourceTypePicked);
  }
                ...
                new RuntimeFunction() {
                  ...
                  protected JSCallable createFunction(RenderContext info) {
                    return new PassThroughFunction(
                        "cl", events.getSubmitValuesFunction("cancelled"));
                  }
                })));
      ...
      if (!Check.isEmpty(orderedHandlerIds)) {
        for (String handlerId : orderedHandlerIds) {
          ...
          if (attachmentHandler.show()) {
            ...
            tableRow.setClickHandler(
                new OverrideHandler(selectedHandler.createSetFunction(), handlerId));
          }
        }
  ...
  public void openDialog(SectionInfo info, String replaceUuid, String editUuid) {
    ...
    if (!Check.isEmpty(editUuid)) {
      final Attachment attachment =
          (Attachment) repository.getAttachments().getAttachmentByUuid(editUuid);
      for (Entry<String, AttachmentHandler> entry : handlers.entrySet()) {
        AttachmentHandler handler = entry.getValue();
        if (handler.supports(attachment)) {
  ...
  public static class UniversalResourcesDialogModel extends DialogModel {
    ...
    @Nullable private List<Button> actions;
