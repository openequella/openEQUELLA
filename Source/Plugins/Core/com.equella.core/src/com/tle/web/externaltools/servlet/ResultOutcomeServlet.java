 ...
 *
 * <p><imsx_POXEnvelopeRequest> <imsx_POXHeader> <imsx_POXRequestHeaderInfo>
 * <imsx_version>V1.0</imsx_version> <imsx_messageIdentifier>---some UUID
 * ---</imsx_messageIdentifier> </imsx_POXRequestHeaderInfo> </imsx_POXHeader> <imsx_POXBody>
 * <replaceResultRequest> <resultRecord> <sourcedGUID> <sourcedId>--- the arbitrary string data as
 * sent to Icodeon launch---</sourcedId> </sourcedGUID> <result> <resultScore>
 * <language>en</language> <textString>--the numeric score 0.0 -- 1.0---</textString> </resultScore>
 * </result> </resultRecord> </replaceResultRequest> </imsx_POXBody> </imsx_POXEnvelopeRequest>
 *
 * <p>The first version of this class submitted for testing is built on the premise that EQUELLA
 * needs to work around Icodeon's limitations in that when Icodeon firstly receives the LTI launch
 * request, including the all-important lis_result_sourced parameter:<br>
 ...
 * - Base64 encoding no longer engaged - assuming that Icodeon has fixed its JSON indigestion. The
 * truncation over 512 characters remains a hazard. This code is submitted on the hypothesis that
 * neither Moodle nor any other LMS will put together a lis_result_sourcedid value that (when
 * extended by EQUELLA) won't push the 512 limit.
 *
 * @author larry
 */
// @formatter:on
...
public class ResultOutcomeServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      final JAXBContext jc =
          JAXBContext.newInstance(
              "com.tle.web.lti.imsx", ResultOutcomeServlet.class.getClassLoader());
      final Unmarshaller unmarshaller = jc.createUnmarshaller();
      Object unmarshalled = unmarshaller.unmarshal(req.getReader());
      if (!(unmarshalled instanceof JAXBElement)) {
        throw new ServletException(
            "Unexpected object in unmarshalled request data: "
    ...
    ltiService.sendGrade(
        env, lmsoauthkey, clientSecret, lmsOutcomeServiceURL.toExternalForm(), resp);
