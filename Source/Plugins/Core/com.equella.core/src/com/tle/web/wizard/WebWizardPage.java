...
public class WebWizardPage extends WizardPage implements com.tle.web.wizard.page.WizardPage {
  ...
  private static final Map<String, Class<? extends HTMLControl>> CONTROLMAP =
      new ImmutableMap.Builder<String, Class<? extends HTMLControl>>()
          .put(Button.CLASS, CButton.class)
          .put(Calendar.CLASS, CCalendar.class)
          .put(CheckBoxGroup.CLASS, CCheckBoxGroup.class)
          .put(Date.CLASS, CCalendar.class)
          .put(EditBox.CLASS, CEditBox.class)
          .put(Group.CLASS, CGroupCtrl.class)
          .put(ListBox.CLASS, CListBox.class)
          .put(Multi.CLASS, CMultiCtrl.class)
          .put(RadioGroup.CLASS, CCheckBoxGroup.class)
          .put(Repeater.REPEATER_CLASS, CRepeater.class)
          .put(ShuffleBox.CLASS, CShuffleBox.class)
          .put(ShuffleList.CLASS1, CShuffleList.class)
          .put(Html.CLASS, CStaticHTML.class)
          .put(TreeNav.CLASS, CTreeNav.class)
          .put(MultiEditBox.CLASS, CMultiEditBox.class)
          .put(Hidden.CLASS, CHidden.class)
          .build();

  private static final Class<?>[] CONSTRUCTCLASSES =
      new Class<?>[] {WizardPage.class, Integer.TYPE, Integer.TYPE, WizardControl.class};
  ...
  private final Map<Object, DefaultSectionTree> sectionTrees =
      new IdentityHashMap<Object, DefaultSectionTree>();

  private LERepository webRepository;
  private WebWizardPageState state;
  @Inject private WizardControlFactory factory;
  ...
  public void loadFromDocument(final SectionInfo info) {
    super.loadFromDocument(
        pageControls,
        new GetItemsInterface() {
          @Override
    ...
    if (list.size() > 0) {
      DefaultSectionTree sectionTree =
          new DefaultSectionTree(
              ...
              new SectionNode("p" + getPageNumber() + ((treeCount++ == 0) ? "" : "t" + treeCount)));
    ...
    for (DefaultSectionTree sectionTree : sectionTrees.values()) {
      if (!registered.containsKey(sectionTree)) {
        ...
        if (Boolean.TRUE.equals(sectionTree.getAttribute(PAGESREG_NEWLYADDED))) {
          Map<SectionTree, SectionTree> newTrees =
              info.getAttributeSafe(PAGESREG_NEW, IdentityHashMap.class);
  ...
  @Override
  protected boolean execScriptWithContext(String script, ScriptContext context)
  ...
  @Override
  public Map<String, List<ControlResult>> renderPage(
      RenderContext context, AjaxUpdateData updateData, String rootId) {
  ...
  public List<ControlResult> renderChildren(
      RenderContext context, List<WebControl> controls, String parentId, boolean forceDisable) {
  ...
  public List<ControlResult> renderChildren(
      RenderContext context, List<WebControl> controls, String parentId) {
    return renderChildren(context, controls, parentId, false);
  }

  private List<ControlResult> renderChildren(
      ...
      if (ctrl.isViewable()) {
        ...
        if (!ignored) {
          ...
          if (forceDisable || (disable && !ctrl.isEnabled())) {
            tagState.addReadyStatements(
                new FunctionCallStatement(ctrl.getDisabler(context).createDisableFunction(), true));
          }
        }
  ...
  public static class RenderData {
    ...
    private final Map<String, List<ControlResult>> parentMap =
        new HashMap<String, List<ControlResult>>();
