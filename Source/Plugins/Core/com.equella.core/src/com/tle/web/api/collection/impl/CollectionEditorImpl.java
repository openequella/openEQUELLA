...
public class CollectionEditorImpl extends AbstractBaseEntityEditor<ItemDefinition, CollectionBean>
  ...
  @Nullable private Collection<String> drmPageIds;
  ...
  @AssistedInject
  public CollectionEditorImpl(
      @Assisted ItemDefinition collection,
  ...
  protected void afterFinishedEditing() {
      ...
      publishEventAfterCommit(
          new ItemOperationEvent(
              new FactoryMethodLocator<BaseFilter>(
                  FilterFactory.class, "drmUpdate", entity.getId(), (Serializable) drmPageIds)));
    }
    ...
    if (editing && workflowChanged) {
      ...
      batchEvent.addEvent(
          new ItemOperationEvent(
              new FactoryMethodLocator<BaseFilter>(
                  FilterFactory.class, "workflowChanged", entity.getId())));
    }

    if (editing
        && (itemMetadataRulesChanged || dynamicMetadataRuleChanged || searchDisplayNodesChanged)) {
      fireEvent = true;
      batchEvent.addEvent(
          new ItemOperationEvent(
              new FactoryMethodLocator<BaseFilter>(
                  FilterFactory.class, "refreshCollectionItems", entity.getId())));
    }
  ...
  public interface CollectionEditorFactory {
    CollectionEditorImpl createExistingEditor(
        @Assisted ItemDefinition collection,
        @Assisted("stagingUuid") @Nullable String stagingUuid,
        @Assisted("lockId") @Nullable String lockId,
        @Assisted("editing") boolean editing,
    ...
    CollectionEditorImpl createNewEditor(
