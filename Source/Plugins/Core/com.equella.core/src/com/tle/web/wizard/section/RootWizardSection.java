...
public class RootWizardSection extends TwoColumnLayout<WizardForm>
    implements BeforeEventsListener, RespondingListener, CourseListVetoSection {
  protected static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(WizardCommand.class);
  ...
  @Inject private Provider<DefaultWizardSectionInfo> infoProvider;
    ...
    if (moderationService.isModerating(info)) {
      ...
      if (oldTaskId != null) {
        moderationService.moderate(
            info, ModerationService.VIEW_METADATA, new ItemTaskId(key, oldTaskId), 0, 1);
        return null;
      }
    }
    ...
    if (model.getException() != null) {
      ...
      for (final Extension extension : extensions) {
        if (extension.getParameter("exceptionClass").valueAsString().equals(exceptionName)) {
          final WizardExceptionHandler handler =
              exceptionTracker.getBeanByParameter(extension, "bean");
  ...
  protected void addBreadcrumbsAndTitle(
    ...
    if (state.isEntryThroughEdit()) {
      crumbs.add(
          breadcrumbService.getSearchCollectionCrumb(info, state.getItemDefinition().getUuid()));

      crumbs.add(breadcrumbService.getViewItemCrumb(info, state.getItem()));

      decorations.setTitle(
          new KeyLabel(
              resources.key("breadcrumb." + state.getOperation().toString().toLowerCase())));
    } else {
  ...
  public void setPluginService(final PluginService pluginService) {
    exceptionTracker =
        new PluginTracker<WizardExceptionHandler>(
            pluginService, "com.tle.web.wizard", "exceptionHandler", null);
  }
