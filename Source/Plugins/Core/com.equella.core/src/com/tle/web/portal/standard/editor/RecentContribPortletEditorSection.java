...
public class RecentContribPortletEditorSection
  ...
  private static final PluginResourceHelper RESOURCES =
      ResourcesService.getResourceHelper(RecentContribPortletEditorSection.class);
  ...
  public void registered(String id, SectionTree tree) {
    ...
    // checked/unchecked
    allCollections.addEventStatements(
        JSHandler.EVENT_CHANGE,
        new FunctionCallStatement(
            collection.createDisableFunction(), allCollections.createGetExpression()));

    // Disable "All Collections" if one or more collections have been picked
    collection.addEventStatements(
        JSHandler.EVENT_CHANGE,
        new FunctionCallStatement(
            allCollections.createDisableFunction(),
    ...
    DynamicHtmlListModel<BaseEntityLabel> listModel =
        new DynamicHtmlListModel<BaseEntityLabel>() {
          @Override
          ...
          @Override
          protected Option<BaseEntityLabel> convertToOption(
              SectionInfo info, BaseEntityLabel bent) {
            return new NameValueOption<BaseEntityLabel>(
                new BundleNameValue(bent.getBundleId(), Long.toString(bent.getId()), bundleCache),
                bent);
          }
        };
    ...
    displayTypeList.setListModel(
        new SimpleHtmlListModel<BundleNameValue>(
            new BundleNameValue(titleAndDescription, null),
            new BundleNameValue(titleOnly, KEY_TITLEONLY)));
  }
  ...
  protected void customLoad(SectionInfo info, PortletEditingBean portlet) {
    ...
    if (Check.isEmpty(collections)) {
      ...
      if (!Check.isEmpty(sessionId)) {
        allCollections.setChecked(
            info,
            !portletService.loadSession(sessionId).getValidationErrors().containsKey("collection"));
      }
      collection.addReadyStatements(
          info, collection.createDisableFunction(), allCollections.createGetExpression());
    } else {
      allCollections.addReadyStatements(
          info,
          allCollections.createDisableFunction(),
          new CombinedExpression(
              collection.createGetExpression(), new PropertyExpression("length != 0")));
      ...
      collection.setSelectedStringValues(
          info,
          Collections2.transform(
              collections,
              new Function<ItemDefinition, String>() {
                @Override
