...
public abstract class AbstractPortletEditorSection<
    ...
    extends AbstractPrototypeSection<M> implements HtmlRenderer, PortletEditor {
  ...
  protected abstract SectionRenderable customRender(
      RenderEventContext context, M model, PortletEditingBean portlet) throws Exception;
  ...
  public final SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (LangUtils.isEmpty(titleBundle)) {
      ...
      if (!Check.isEmpty(languages)) {
        ...
        title.setLanguageBundle(
            context,
            LangUtils.convertBundleToBean(
                LangUtils.createTextTempLangugageBundle(
                    portletName, closest != null ? closest : locales.get(0))));
      }
    }
    ...
    model.setPageTitle(
        CurrentLocale.get(
            portlet.getId() == 0 ? CREATE_PAGETITLE_KEY : EDIT_PAGETITLE_KEY, portletName));
    ...
    institutional.setEventHandler(
        ...
        new StatementHandler(
            ajaxFactory.getAjaxUpdateDomFunction(
                ...
                ajaxFactory.getEffectFunction(EffectType.REPLACE_IN_PLACE),
                "institutionWideSettings")));
  ...
  public final void save(SectionInfo info) {
    ...
    if (session.isValid()) {
      ...
      portletWebService.returnFromEdit(
          info, false, portletBean.getUuid(), portletBean.isInstitutional());
  ...
  private void saveFields(SectionInfo info, EntityPack<Portlet> pack, PortletEditingBean portlet) {
    ...
    if (admin) {
      ...
      if (expression != null) {
        ...
        list.getEntries()
            .add(new TargetListEntry(true, false, PortletConstants.VIEW_PORTLET, expression));
