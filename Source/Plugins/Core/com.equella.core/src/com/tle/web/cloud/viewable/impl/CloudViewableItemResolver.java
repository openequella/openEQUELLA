...
public class CloudViewableItemResolver implements ViewableItemResolverExtension {
  ...
  @Inject private CloudViewItemUrlFactory viewItemUrlFactory;
  ...
  public <I extends IItem<?>> ViewableItem<I> createIntegrationViewableItem(
    ...
    return (ViewableItem<I>)
        new CloudViewableItem(
            cloudService.getItem(itemKey.getUuid(), latest ? 0 : itemKey.getVersion()), true);
  }
  ...
  public <I extends IItem<?>> Bookmark createThumbnailAttachmentLink(
    ...
    if (attachmentUuid != null && !Strings.isNullOrEmpty(attachmentUuid)) {
      final CloudAttachment attachment =
          (CloudAttachment)
              new UnmodifiableAttachments(cloudItem).getAttachmentByUuid(attachmentUuid);
