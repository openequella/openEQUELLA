...
public abstract class AbstractSelectionAddon extends AbstractTleTinyMceAddOn {
  ...
  public SectionResult execute(
      ...
      @Nullable Set<String> collectionUuids,
      boolean restrictedDynacolls,
      @Nullable Set<String> dynaCollUuids,
      boolean restrictedSearches,
      @Nullable Set<String> searchUuids,
      boolean restrictedContributables,
      @Nullable Set<String> contributableUuids) {
    SelectionSession session =
        new SelectionSession(new BeanLookupSelectionCallback(getCallbackClass()));
    ...
    if (restrictedDynacolls) {
      ...
      session.setDynamicCollectionIds(
          dynaCollUuids != null ? dynaCollUuids : new HashSet<String>());
    } else {
    ...
    if (restrictedContributables) {
      ...
      session.setContributionCollectionIds(
          contributableUuids != null ? contributableUuids : new HashSet<String>());
    } else {
  ...
  /**
   * Add/remove selectables, banned types and other selection session initialisation
   *
   * @param info
   * @param session
   */
  ...
  /**
   * When the selection session is finished, it will create an instance of this class and invoke the
