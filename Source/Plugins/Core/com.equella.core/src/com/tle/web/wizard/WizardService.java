...
public interface WizardService extends ScriptEvaluator {
  ...
  /**
   * Merges the values from each target/value pair in to the document bag. This method has not been
   * completely tested! The new format for the fixed meta data element should look something like
   * the following: <wizard> ... <fixedmetadata> <data target="/item/somewhere" value="some value">
   * <data target="/workflow/registered" value="true"> ... </fixedmetadata> </wizard>
   *
  ...
  boolean checkDuplicateXpathValue(
   ...
   */
  ScriptContext createScriptContext(
      ItemPack pack, WizardPage page, HTMLControl control, Map<String, Object> attributes);
   ...
   * @param staging Not required. If you wish to use a pre-existing staging area then by all means
   *     supply one.
   * @return A wizard state to be stored in the session (use addToSession(WizardStateInterface) ) @
   */
   ...
   * @param newItemDefUuid The new item definition to move the item to. (doesn't have to be
   *     different, or leave it null to remain unchanged)
   * @param transform Optional. This is the name of the schema import transform on the newItemDef's
   *     schema.
   * @param copyAttachments Bring the attachments across as well as the metadata.
   * @return A new wizard state initialised with a clone of the item with the new itemDef. The
   *     unified XML will have been transformed if a transform was supplied.
   */
  WizardState cloneItem(
   ...
   * @param transform Optional. This is the name of the schema import transform on the newItemDef's
   *     schema.
   * @return A new wizard state initialised with the item with the new itemDef. The unified XML will
   *     have been transformed if a transform was supplied. @
   */
