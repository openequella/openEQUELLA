...
public class HtmlPluginServlet extends HttpServlet {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(HtmlPluginServlet.class);
    ...
    if (firstSlash < 0) {
      ...
      final FileContentStream stream =
          fileSystemService.getContentStream(
              entFile, relPath, mimeService.getMimeTypeForFilename(relPath));
      contentStreamWriter.outputStream(request, response, stream);
    }
  }

  private void redirectToTinyMceFiles(HttpServletResponse response, String path)
      throws IOException {
    response.sendRedirect(
        institutionService.institutionalise(
            resources.url(PathUtils.urlPath("scripts/tinymce/", path))));
  }
}
