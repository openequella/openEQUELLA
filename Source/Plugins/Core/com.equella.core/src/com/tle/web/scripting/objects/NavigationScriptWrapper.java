...
import com.tle.common.scripting.objects.NavigationScriptObject;
...
public class NavigationScriptWrapper extends AbstractScriptWrapper
  ...
  @Inject
  protected NavigationScriptWrapper(
  ...
  @Override
  public NavigationTabScriptType addTab(
      String description, AttachmentScriptType attachment, NavigationNodeScriptType node) {
    if (attachment == null) {
      throw new RuntimeException(
          "com.tle.web.scripting.advanced.navigation.error.nullattach"); //$NON-NLS-1$
    }
  ...
  public void autocreate() {
    itemNavService.populateTreeNavigationFromAttachments(
        item, item.getTreeNodes(), item.getAttachments(), null);
    rebuildTree();
  }
  ...
  public List<NavigationNodeScriptType> listChildNodes(NavigationNodeScriptType parentNode) {
    ...
    final List<ItemNavigationNode> realChildren =
        (realNode == null ? tree.getRootNodes() : tree.getChildMap().get(realNode));
  ...
  protected NavigationNodeScriptType getScriptNode(ItemNavigationNode realNode) {
    ...
    if (scriptNode == null) {
      throw new RuntimeException(
          CurrentLocale.get(
              "com.tle.web.scripting.advanced.navigation.error.noscriptnode",
              realNode //$NON-NLS-1$
                  .getName()));
    }
    return scriptNode;
    ...
    protected NavigationNodeScriptTypeImpl(ItemNavigationNode wrapped) {
      ...
      for (ItemNavigationTab tab : wrapped.ensureTabs()) {
        if (tab.getAttachment() != null) {
          tabs.add(
              new NavigationTabScriptTypeImpl(
                  tab, scriptTypeFactory.createAttachment(tab.getAttachment(), null), this));
        } else {
  ...
  public class NavigationTabScriptTypeImpl implements NavigationTabScriptType {
    ...
    protected NavigationTabScriptTypeImpl(
        ItemNavigationTab wrapped, NavigationNodeScriptType parentNode) {
      this(wrapped, null, parentNode);
    }

    protected NavigationTabScriptTypeImpl(
        ItemNavigationTab wrapped,
