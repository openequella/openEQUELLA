...
public class BrowsePortletRenderer extends PortletContentRenderer<TopicDisplayModel> {
    ...
    if (!Check.isEmpty(rootTopics)) {
      ...
      for (VirtualisableAndValue<HierarchyTopic> rootTopic : rootTopics) {
        ...
        if (searchCount == GeneralConstants.UNCALCULATED) {
          ...
          int[] countsOfOne =
              freeTextService.countsFromFilters(Collections.singletonList(subSearch));
  ...
  private LiveItemSearch getSubSearch(
      SectionInfo info,
      VirtualisableAndValue<HierarchyTopic> vTopic,
    ...
    if (topic.isShowResults()) {
      ...
      Map<String, String> vs =
          virtValue == null ? null : Collections.singletonMap(topic.getUuid(), virtValue);
