...
public class InstitutionResourceImpl implements InstitutionResource {
  ...
  @Inject private RunAsUser runAs;
  ...
  private static final String KEY_PREFIX =
      PluginServiceImpl.getMyPluginId(InstitutionService.class) + '.';
  ...
  private void validateEssentialFields(InstitutionBean institutionBean) {
    ...
    if (Check.isEmpty(institutionBean.getFilestoreId())) {
      throw new BadRequestException(
          VALDATION_PREAMBLE + "Institution Filestore Id must not be left blank");
    }
    ...
    for (InstitutionStatus instati : institutionService.getAllInstitutions()) {
      if (Objects.equals(institutionBean.getName(), instati.getInstitution().getName())) {
        KeyString messg =
            new KeyString(
                KEY_PREFIX + "institution.validate.use",
                VALDATION_PREAMBLE + "Institution name",
                institutionBean.getName());
        throw new BadRequestException(messg.toString());
      }
      if (Objects.equals(
          institutionBean.getFilestoreId(), instati.getInstitution().getFilestoreId())) {
        KeyString messg =
            new KeyString(
                KEY_PREFIX + "institution.validate.use",
                VALDATION_PREAMBLE + "Filestore ID",
                institutionBean.getFilestoreId());
        throw new BadRequestException(messg.toString());
      }
      if (Objects.equals(institutionBean.getUrl(), instati.getInstitution().getUrl())) {
        KeyString messg =
            new KeyString(
                KEY_PREFIX + "institution.validate.use",
                VALDATION_PREAMBLE + "Institution URL",
                institutionBean.getFilestoreId());
        throw new BadRequestException(messg.toString());
      }
    }
  ...
  @Transactional
  private Institution saveNewInstitution(
      final InstitutionBean institutionBean, final long schemaId) {
    final long uniqueId = systemConfigService.createUniqueInstitutionId();
    final long resultantId =
              ...
              public Long call() throws Exception {
                ...
                if (!fileSystemService.fileExists(handle)) {
                  ...
                  throw new FileAlreadyExistsException(
                      handle.getAbsolutePath(), null, "non-directory file exists");
                }
                try {
  ...
  private Institution deserialize(
      InstitutionBean bean, final long uniqueId, @Nullable Institution originalInstitution) {
    Institution newInstitution =
        originalInstitution != null ? originalInstitution : new Institution();
