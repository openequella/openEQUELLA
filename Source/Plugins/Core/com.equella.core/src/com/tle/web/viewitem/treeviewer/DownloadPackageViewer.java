...
public class DownloadPackageViewer extends AbstractResourceViewer {
  ...
  public void setPluginService(PluginService pluginService) {
    downloadLink =
        new PluginTracker<ItemUrlExtender>(
            pluginService, "com.tle.web.viewitem.treeviewer", "downloadLink", "id");
  ...
  public boolean supports(SectionInfo info, ViewableResource resource) {
    ...
    return (mimeType.equals(MimeTypeConstants.MIME_IMS)
        || mimeType.equals(MimeTypeConstants.MIME_SCORM));
  }
  ...
  public ViewItemUrl createViewItemUrl(SectionInfo info, ViewableResource resource) {
    UrlEncodedString urlFilepath =
        !Check.isEmpty(resource.getFilepath())
            ? UrlEncodedString.createFromValue(resource.getFilepath())
            : null;
    ViewItemUrl vurl =
        urlFactory.createItemUrl(resource.getInfo(), resource.getViewableItem(), urlFilepath, 0);
