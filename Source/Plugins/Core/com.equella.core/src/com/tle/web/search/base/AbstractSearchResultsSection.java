...
public abstract class AbstractSearchResultsSection<
        LE extends ListEntry,
        SE extends AbstractSearchEvent<SE>,
        RE extends AbstractSearchResultsEvent<RE>,
        M extends AbstractSearchResultsSection.SearchResultsModel>
    extends AbstractPrototypeSection<M> implements HtmlRenderer, SearchResultsListener<RE> {
  ...
  @TreeLookup protected PagingSection<SE, RE> paging;
    ...
    resultsEffect =
        new PrependedParameterFunction(
            new ExternallyDefinedFunction(
                "showNewResults",
                new IncludeFile(URL_RESULTSJS),
                AjaxGenerator.AJAX_LIBRARY,
                JQueryScrollTo.INCLUDE),
  ...
  private JSCallable getResultsUpdater(
      SectionTree tree, ParameterizedEvent event, boolean restart, String... ajaxIds) {
    if (!tree.isFinished()) {
    ...
    return ajax.getAjaxUpdateDomFunction(
        tree, null, event, resultsEffect, ajaxList.toArray(new String[ajaxList.size()]));
  }
  ...
  private JSCallable getCheckBoxUpdater(SectionTree tree) {
    ...
    return ajax.getAjaxUpdateDomFunction(
        tree, null, checkBoxEvent, resultsEffect, DIV_SEARCHRESULTS);
  }
  ...
  public JSCallable getResultsUpdater(
      SectionTree tree, ParameterizedEvent event, String... ajaxIds) {
  ...
  public void treeFinished(String id, SectionTree tree) {
    ...
    paging.setChangeHandlers(
        getResultsUpdater(tree, null, false),
        getResultsUpdater(tree, null, true),
