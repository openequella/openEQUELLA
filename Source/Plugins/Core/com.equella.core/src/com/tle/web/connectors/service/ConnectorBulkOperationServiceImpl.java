...
public class ConnectorBulkOperationServiceImpl implements ConnectorBulkOperationService {
  ...
  public class ConnectorBulkOperationTask extends SingleShotTask {
    ...
    public ConnectorBulkOperationTask(
        UserState userState, Collection<? extends ItemKey> items, BulkOperationExecutor executor) {
    ...
    public void runTask() throws Exception {
      final ConnectorBulkWorkflowFilter filter =
          new ConnectorBulkWorkflowFilter(this, items, executor);
      runAs.execute(
          userState,
          new Callable<Void>() {
            @Override
  ...
  public static class ConnectorBulkWorkflowFilter
      implements ItemOperationFilter, FilterResultListener {
    private final Collection<? extends ItemKey> items;
    private final BulkOperationExecutor executor;
    private final Task task;

    public ConnectorBulkWorkflowFilter(
        Task task, Collection<? extends ItemKey> items, BulkOperationExecutor executor) {
      this.task = task;
      this.items = items;
      this.executor = executor;
    }
