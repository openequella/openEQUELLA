...
{
	private static final IncludeFile INCLUDE = new IncludeFile(ResourcesService.getResourceHelper(LinkRenderer.class)
		.url("scripts/render/link.js"));
	private static final ExternallyDefinedFunction FUNCTION_IS_ENABLED = new ExternallyDefinedFunction("isEnabled",
		INCLUDE);
	private static final ExternallyDefinedFunction FUNCTION_SET_DISABLED = new ExternallyDefinedFunction("setDisabled",
		INCLUDE);
	...
	protected void prepareLastAttributes(SectionWriter writer, Map<String, String> attrs)
	{
		if( ensureClickable && !attrs.containsKey("on" + JSHandler.EVENT_CLICK) )//$NON-NLS-1$
		{
			...
			{
				attrs.put("onclick", "document.location.href=this.href; return false;"); //$NON-NLS-1$//$NON-NLS-2$
			}
		}
	}
	...
	public JSCallable createDisableFunction()
	{
		if( disableFunc == null )
		{
			...
			disableFunc = new SimpleFunction("dis", this, Js.call_s(FUNCTION_SET_DISABLED, Jq.$(this), dis), dis);
		}
