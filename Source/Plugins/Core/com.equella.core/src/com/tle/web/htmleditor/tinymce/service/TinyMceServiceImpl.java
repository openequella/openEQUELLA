...
public class TinyMceServiceImpl implements TinyMceService {
  ...
  private static final CssInclude TLE_SKIN =
      CssInclude.include(RESOURCES.url("scripts/tinymce/themes/advanced/skins/tle/ui.css")).make();
  private static final CssInclude TLE_SILVER_SKIN =
      CssInclude.include(RESOURCES.url("scripts/tinymce/themes/advanced/skins/tle/ui_silver.css"))
          .prerender(TLE_SKIN)
          .make();

  private static final IncludeFile TINY_MCE_JS =
      new IncludeFile(RESOURCES.url("scripts/tinymce/tiny_mce.js"), JQueryCore.PRERENDER);
  private static final IncludeFile TINY_MCE_DEV_JS =
      new IncludeFile(RESOURCES.url("scripts/tinymce/tiny_mce_src.js"), JQueryCore.PRERENDER);

  private static final String DEFAULT_PLUGINS = "inlinepopups,paste,wordcount,fullscreen";

  private final IncludeFile htmlEditorJs =
      new IncludeFile(
          RESOURCES.url("scripts/htmleditor.js"),
          new PreRenderable() {
            @Override
  ...
  private final JSCallAndReference initialiseMce =
      new ExternallyDefinedFunction("initialiseMce", 4, htmlEditorJs);
  private final JSCallAndReference registerEditor =
      new ExternallyDefinedFunction("registerEditor", 1, htmlEditorJs);
  private final JSCallAndReference saveContents =
      new ExternallyDefinedFunction("saveContents", 1, htmlEditorJs);
  private final JSCallAndReference setDisabled =
      new ExternallyDefinedFunction("setTinyMceDisabled", 3, htmlEditorJs);
  private final JSCallAndReference toggleTinyMceFullscreen =
      new ExternallyDefinedFunction("toggleTinyMceFullscreen", 2, htmlEditorJs);
  ...
  @Inject private Provider<TinyMceControl> controlProvider;
  @Inject private Provider<TinyMceEditorSection> editorProvider;
  ...
  public void preRender(
      ...
      AbstractRenderedComponent<?> textAreaComponent,
      TinyMceModel model) {
    if (!context.getBooleanAttribute("TinyMceService.initialiseMce")) {
      ...
      final String directionality =
          (model.getDirectionality() == null ? "ltr" : model.getDirectionality());
      ...
      final Map<String, HtmlEditorButtonDefinition> buttonDefs =
          Maps.newHashMap(getStandardButtons());
      ...
      for (TinyMceAddOn addOn : addOns) {
        ...
        if (usedPluginIds.contains(addOn.getId()) || addOn.getButtons(context).isEmpty()) {
          ...
          scriptContext.addScriptObject(
              "institutionUrl", institutionService.getInstitutionUrl().toString());
    ...
    textAreaComponent.addReadyStatements(
        context, Js.call_s(registerEditor, Jq.$(textAreaComponent)));
  }
  ...
  public JSHandler getToggleFullscreeenHandler(ElementId textAreaComponent, ElementId link) {
    return new OverrideHandler(
        Js.call_s(toggleTinyMceFullscreen, Jq.$(textAreaComponent), Jq.$(link)));
  }
  ...
  private ObjectExpression getEditorOptions(HtmlEditorConfiguration config) {
    ...
    try {
      final ObjectMapper jsonMapper =
          objectMapperService.createObjectMapper(LenientMapperExtension.NAME);
  ...
  public void populateModel(
      ...
      Map<Class<?>, Set<String>> collectionUuids,
    ...
    final List<String> exclusions =
        !Check.isEmpty(excludedAddOns) ? Arrays.asList(excludedAddOns.split(",")) : null;
    ...
    actionSection.setSearchableUuids(
        actionInfo,
  ...
  public JSCallable getDisableFunction(ElementId element, ElementId fullscreenLinkElement) {
    ...
    final JSAssignable setDisabledThis =
        Js.function(Js.call_s(setDisabled, Jq.$(element), Jq.$(fullscreenLinkElement), dis), dis);
  ...
  public LinkedHashMap<String, HtmlEditorButtonDefinition> getButtons(SectionInfo info) {
    final LinkedHashMap<String, HtmlEditorButtonDefinition> buttons =
        Maps.newLinkedHashMap(getStandardButtons());
    // customs
    ...
    if (STANDARD_BUTTONS == null) {
      synchronized (BUTTON_LOCK) {
        if (STANDARD_BUTTONS == null) {
          ...
          addDdButton(
              buttons, "formatselect", null, "mce_formatselect", LABEL_FORMAT_SELECT, 0, true);
          ...
          addDdButton(
              buttons, "fontselect", null, "mce_fontselect", LABEL_FONT_FAMILY_SELECT, 2, true);
          addDdButton(
              buttons,
              "fontsizeselect",
              null,
              "mce_fontsizeselect",
              LABEL_FONT_SIZE_SELECT,
              2,
              true);
          addButton(buttons, "spellchecker", "spellchecker", 2);

          DivRenderer renderer =
              new DivRenderer(
                  "tleSkin",
                  new SpanRenderer(
                      "mceSeparator", new LabelRenderer(new TextLabel("&nbsp;", true))));
          HtmlEditorButtonDefinition seppo =
              new HtmlEditorButtonDefinition(
                  "|", null, renderer, new KeyLabel(KEY_BUTTON_PREFIX + "separator"), 2, false);
          buttons.put("|", seppo);
          ...
          addDdButton(
              buttons, "styleselect", null, "mce_styleselect", LABEL_STYLE_SELECT, -1, true);
  ...
  private void addButton(
      Map<String, HtmlEditorButtonDefinition> buttons, String id, String pluginId, int row) {
    addButton(buttons, id, pluginId, row, true);
  }

  private void addButton(
      Map<String, HtmlEditorButtonDefinition> buttons,
      String id,
      String pluginId,
      int row,
      boolean singleton) {
    HtmlEditorButtonDefinition bold =
        new HtmlEditorButtonDefinition(
            id,
            pluginId,
            new StandardButtonRenderer("mce_" + id),
            new KeyLabel(KEY_BUTTON_PREFIX + id),
            row,
            singleton);
    buttons.put(id, bold);
  }

  private void addDdButton(
      Map<String, HtmlEditorButtonDefinition> buttons,
      String id,
      String pluginId,
      String cssClass,
    ...
    HtmlEditorButtonDefinition bold =
        new HtmlEditorButtonDefinition(
            id,
            pluginId,
            new DropdownButtonRenderer(cssClass, text),
            new KeyLabel(KEY_BUTTON_PREFIX + id),
            row,
            singleton);
    buttons.put(id, bold);
  ...
  public void setPluginService(PluginService pluginService) {
    addOnTracker =
        new PluginTracker<TinyMceAddonProvider>(
            pluginService, "com.tle.web.htmleditor.tinymce", "addons", "id");
  ...
  protected static TableState selectButtonState(String buttonCss, Label buttonText) {
    ...
    DivRenderer aopen =
        new DivRenderer(
            ...
            "mceOpen",
            new SpanRenderer(
                new SpanRenderer("mceIconOnly", new LabelRenderer(new TextLabel("&nbsp;", true)))));
  ...
  public class DropdownButtonRenderer extends DivRenderer {
    public DropdownButtonRenderer(String buttonCss, Label buttonText) {
      super(
          ...
          new DivRenderer(
              "span",
              new TableRenderer(selectButtonState(buttonCss, buttonText), rendererFactory) {
