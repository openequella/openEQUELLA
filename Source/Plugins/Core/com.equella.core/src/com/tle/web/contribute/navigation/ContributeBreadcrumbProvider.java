...
public class ContributeBreadcrumbProvider implements BreadcrumbProvider {
  @PlugKey("contribute.breadcrumb.title")
  private static Label BREADCRUMB_TITLE;

  @SuppressWarnings("nls")
  private static Label LABEL =
      new KeyLabel(
          ResourcesService.getResourceHelper(ContributeBreadcrumbProvider.class)
              .key("breadcrumbForContributionSelector"));

  @Override
  public TagState getBreadcrumb(SectionInfo info, Map<String, ?> params) {
    HtmlLinkState con =
        new HtmlLinkState((info.createForward("/access/contribute.do").getPublicBookmark()));
    con.setLabel(LABEL);
    con.setRel("parent");
