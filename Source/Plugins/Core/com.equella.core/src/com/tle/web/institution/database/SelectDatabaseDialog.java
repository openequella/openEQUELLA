...
public class SelectDatabaseDialog extends AbstractOkayableDialog<DialogModel> {
  ...
  @Component private SingleSelectionList<Void> list;

  public Iterable<DatabaseSchema> getOnlineSchemas() {
    return Iterables.transform(
        Iterables.filter(
            migrationService.getMigrationsStatus().getSchemas().values(),
            new Predicate<SchemaInfo>() {
              @Override
    ...
    list.setListModel(
        new DynamicHtmlListModel<Void>() {
          ...
          protected Iterable<Option<Void>> populateOptions(SectionInfo info) {
            return Iterables.transform(
                getOnlineSchemas(),
                new Function<DatabaseSchema, Option<Void>>() {
                  @Override
                  public Option<Void> apply(DatabaseSchema ds) {
                    return new LabelOption<Void>(
                        databaseTabUtils.getNameLabel(ds), Long.toString(ds.getId()), null);
                  }
                });
          }
  ...
  protected JSHandler createOkHandler(SectionTree tree) {
    return new OverrideHandler(
        jscall(getOkCallback(), list.createGetExpression()), jscall(getCloseFunction()));
  }
