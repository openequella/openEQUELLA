...
public class SearchSettingsSection
    extends OneColumnLayout<SearchSettingsSection.SearchSettingsSectionModel> {
  ...
  @Inject private SearchPrivilegeTreeProvider securityProvider;

  @TreeLookup private EditSearchFilterSection editSection;

  @EventFactory private EventGenerator events;

  @Component private SingleSelectionList<VoidKeyOption> defaultSortType;
  @Component private Checkbox showNonLiveCheckbox;
  @Component private Checkbox authenticateByDefault;
  @Component private Checkbox disableGalleryView;

  @Component(name = "dicc")
  private Checkbox disableFileCountCheckbox;

  @Component private Checkbox disableVideosView;

  @Component private SingleSelectionList<NameValue> harvestOptions;
  ...
  @Component private TextField titleBoost;
  @Component private TextField descriptionBoost;
  @Component private TextField attachmentBoost;

  @Inject private PluginTracker<SearchSettingsExtension> extensionsTracker;
  ...
  private static final PluginResourceHelper RESOURCES =
      ResourcesService.getResourceHelper(SearchSettingsSection.class);
  private static final IncludeFile INCLUDE = new IncludeFile(RESOURCES.url("scripts/slider.js"));
  private static final JSCallable SHOW_SLIDER =
      new ExternallyDefinedFunction("showSlider", JQuerySlider.PRERENDER, INCLUDE);
  ...
  protected TemplateResult setupTemplate(RenderEventContext context) {
    ...
    descriptionBoost.addReadyStatements(
        context, SHOW_SLIDER, "description-slider", descriptionBoostValue);

    attachmentBoost.setValue(context, String.valueOf(attachmentBoostValue));
    attachmentBoost.addReadyStatements(
        context, SHOW_SLIDER, "attachment-slider", attachmentBoostValue);
    ...
    templateResult.addNamedResult(
        BODY, viewFactory.createResult("settings/searchsettings.ftl", this));
    HelpAndScreenOptionsSection.addHelp(
        context, viewFactory.createResult("settings/searchsettingshelp.ftl", this));
  ...
  public void registered(String id, SectionTree tree) {
    ...
    SimpleHtmlListModel<NameValue> harvestOptionsList =
        new SimpleHtmlListModel<NameValue>(
            new NameValue(
                CurrentLocale.get(INDEX_NONE_LABEL),
                Integer.toString(SearchSettings.URL_DEPTH_LEVEL_NONE)),
            new NameValue(
                CurrentLocale.get(INDEX_REFERENCED_LABEL),
                Integer.toString(SearchSettings.URL_DEPTH_LEVEL_REFERENCED)),
            new NameValue(
                CurrentLocale.get(INDEX_REFLINKED_LABEL),
  ...
  private boolean isAllBoostValuesZero(SectionInfo info) {
    if (titleBoost.getValue(info).equals("0")
        && descriptionBoost.getValue(info).equals("0")
  ...
  private class SearchFilterModel extends DynamicSelectionsTableModel<SearchFilter> {
    ...
    protected void transform(
      ...
      selection.setViewAction(
          new LinkRenderer(new HtmlLinkState(new TextLabel(filter.getName()), editHandler)));

      actions.add(makeAction(EDIT_SEARCH_FILTER_LABEL, editHandler));
      actions.add(
          makeAction(
              REMOVE_SEARCH_FILTER_LABEL,
              new OverrideHandler(removeFilterFunc, filterId)
                  .addValidator(REMOVE_SEARCH_FILTER_CONFIRM)));
    }
  }
  ...
  public static class SearchSettingsSectionModel extends OneColumnLayout.OneColumnLayoutModel {
    @Bookmarked private boolean notSuccessful;
