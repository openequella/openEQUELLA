...
public abstract class AbstractItemlikeListAttachmentDisplaySection<
        I extends IItem<?>, LE extends ItemlikeListEntry<I>>
    extends AbstractPrototypeSection<
        AbstractItemlikeListAttachmentDisplaySection.AttachmentDisplayModel<I>>
    implements ItemlikeListEntryExtension<I, LE>, HtmlRenderer, AttachmentSelectorEventListener {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(AbstractItemlikeListAttachmentDisplaySection.class);
  private static final IncludeFile JS =
      new IncludeFile(resources.url("scripts/itemlistattachments.js"));
  private static final JSCallAndReference ATTACHMENT_LIST_CLASS =
      new ExternallyDefinedFunction("ItemListAttachments", JS);
  private static final JSCallable TOGGLE =
      new ExternallyDefinedFunction(ATTACHMENT_LIST_CLASS, "toggle", 5);
  private static final JSCallable END_TOGGLE =
      new ExternallyDefinedFunction(ATTACHMENT_LIST_CLASS, "endToggle", 0);
  ...
  @Nullable
  @Override
  public ProcessEntryCallback<I, LE> processEntries(
      final RenderContext context, List<LE> entries, ListSettings<LE> listSettings) {
    for (LE entry : entries) {
      if (!Check.isEmpty(entry.getAttachments().getList())
        ...
        final SimpleElementId attId =
            new SimpleElementId(ATTACHMENTS_ID_PREFIX + itemUuidAndVersion);
        attId.registerUse();
          ...
          if (searchDetails != null) {
            defaultOpen =
                selectionService.getCurrentSession(context) != null
                    ? searchDetails.isIntegrationOpen()
                    : searchDetails.isStandardOpen();

            String attDisplay = searchDetails.getAttDisplay();
            attdisplay =
                (Strings.isNullOrEmpty(attDisplay)
                    ? AttachmentDisplay.STRUCTURED
          ...
          if (defaultOpen) {
            entry.addExtras(
                createAttachmentsList(
                    context, item, structured, attId, itemUuid, itemVersion, defaultOpen));
          } else {
  ...
  private SectionRenderable createToggler(
    ...
    final UpdateDomFunction updateFunc =
        new UpdateDomFunction(
            updateEvent,
            toggleIdStr,
            ajax.getEffectFunction(EffectType.ACTIVITY, AjaxGenerator.URL_SPINNER_INLINE),
            END_TOGGLE);
    final JSCallAndReference updateCarf = CallAndReferenceFunction.get(updateFunc, toggleId);
    divToggle.setClickHandler(
        new OverrideHandler(
            TOGGLE, Jq.$(divToggle), Jq.$(attId), updateCarf, itemUuid, itemVersion));
  ...
  private SectionRenderable createAttachmentsList(
      RenderContext context,
      I item,
      boolean structured,
      SimpleElementId attId,
    ...
    DivRenderer atts =
        new DivRenderer(
            attDivState,
            new AjaxCaptureRenderer(
                attId.getElementId(context),
                new DivRenderer(
                    captureDiv, viewFactory.createResultWithModel("attachmentdisplay.ftl", model)),
                defaultOpen ? null : getEffectMap()));

    atts.addClass("opened");
    return atts;
  }

  private List<AttachmentRowDisplay> buildAttachmentRowDisplay(
      RenderContext context, I item, boolean structured, SimpleElementId attId) {
    final List<AttachmentRowDisplay> attachments =
        viewAttachmentWebService.createViewsForItem(
            context, getViewableItem(item), attId, true, false, !structured, false);
    viewAttachmentWebService.filterAttachmentDisplays(context, attachments);
    return attachments;
  }

  private TagState createCaptureDiv(
      RenderContext context,
      I item,
      SimpleElementId attId,
      final String itemUuid,
    ...
    if (selectFunction != null) {
      ...
      captureDiv.addReadyStatements(
          viewAttachmentWebService.setupSelectButtonsFunction(
              selectFunction,
              new ItemId(itemUuid, itemVersion),
              itemExtensionType,
              "#" + attId.getElementId(context)));
    }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    updateEvent =
        UpdateDomEvent.register(tree, this, events.getEventHandler("toggleAttachments"), "");
    selectCallOne = events.getSubmitValuesFunction("selectAttachment");

    final JSCallable effectFunction =
        ajax.getEffectFunction(EffectType.ACTIVITY, AjaxGenerator.URL_SPINNER_INLINE);
    selectCall =
        ajax.getAjaxUpdateDomFunction(
            tree, null, events.getEventHandler("selectAttachment"), effectFunction, "placeholder1");
        ...
        new DivRenderer(
            ...
            new AjaxCaptureRenderer(
                ...
                new DivRenderer(
                    captureDiv,
                    model.isShow()
                        ? viewFactory.createResult("attachmentdisplay.ftl", this)
                        : null),
                getEffectMap()));
  ...
  public void toggleAttachments(
      SectionInfo info, String itemUuid, int itemVersion, boolean opened) {
