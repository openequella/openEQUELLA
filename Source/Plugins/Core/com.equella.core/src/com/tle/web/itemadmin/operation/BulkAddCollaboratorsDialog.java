...
@Bind
public class BulkAddCollaboratorsDialog
  ...
  public void addOptions(SectionInfo info, List<Option<OperationInfo>> options) {
    options.add(
        new KeyOption<OperationInfo>(
            StandardOperations.getNameKey() + BULK_VALUE,
            BULK_VALUE,
  ...
  public BeanLocator<? extends BulkOperationExecutor> getExecutor(
    ...
    if (users.size() > 0) {
      ...
      HashSet<String> selections =
          (HashSet<String>) users.stream().map(u -> u.getUuid()).collect(Collectors.toSet());
      return new FactoryMethodLocator<AddCollaboratorExecutor>(
          AddCollaboratorExecutorFactory.class, "create", selections);
    }
  ...
  public static class AddCollaboratorExecutor implements BulkOperationExecutor {
    ...
    public WorkflowOperation[] getOperations() {
      return new WorkflowOperation[] {
        workflowFactory.addCollaborators(collaborators), workflowFactory.save()
      };
    }
  ...
  public Label getStatusTitleLabel(SectionInfo info, String operationId) {
    return new KeyLabel(
        StandardOperations.getStatusKey(),
