...
public class ContributeSection extends AbstractPrototypeSection<ContributeSection.ContributeModel>
    implements HtmlRenderer, CourseListVetoSection {
  @PlugKey("contribute.title")
  ...
  @Inject private QuickUploadSection quickUploadSection;
  ...
  public SectionResult renderHtml(RenderEventContext context) {
    final Multimap<String, ItemDefinition> wizardHash =
        TreeMultimap.create(
            Format.STRING_COMPARATOR,
    ...
    if (resumableWizards.size() > 0) {
      for (WizardInfo wizInfo : resumableWizards) {
        ...
        if (wizInfo.isNewItem()) {
          ...
          final HtmlLinkState resumeLink =
              new HtmlLinkState(events.getNamedHandler("resume", uuid));

          reWizard.setResumeLink(resumeLink);
          reWizard.setCollectionName(new TextLabel(wizInfo.getCollectionName()));

          final HtmlLinkState removeLink =
              new HtmlLinkState(LABEL_REMOVE, events.getNamedHandler("removeWizard", uuid));
    ...
    if (selectionSession != null
        && selectionSession.getLayout() == Layout.COURSE
    ...
    for (Map.Entry<String, Collection<ItemDefinition>> entry : wizardHash.asMap().entrySet()) {
      ...
      for (ItemDefinition itemdef : collections) {
        final List<FederatedSearch> fedSearches =
            federatedSearchService.getForCollectionUuid(itemdef.getUuid());
        ...
        if (!fedSearches.isEmpty()) {
          ...
          Collections.sort(
              repos,
              new NumberStringComparator<RemoteRepository>() {
                private static final long serialVersionUID = 1L;
        ...
        if (singleCol && Check.isEmpty(wizard.getRemoteRepos())) {
          webWizardService.forwardToNewItemWizard(
              context, contributableCollections.get(0).getUuid(), null, null, true);
          return null;
        }
        ...
        if (!itemdef.isDenyDirectContribution()) // Allows Direct
        {
          ...
          category.addRow(
              viewFactory.createResultWithModel("contribute/collectionCell.ftl", wizard));
    ...
    temp.addNamedResult(
        OneColumnLayout.BODY, viewFactory.createResult("contribute/contribute.ftl", context));
    HelpAndScreenOptionsSection.addHelp(
        context, viewFactory.createResult("contribute/help.ftl", context));
    return temp;
  }
