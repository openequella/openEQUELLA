...
public class SearchQuerySection
    extends AbstractResetFiltersQuerySection<
        SearchQuerySection.SearchQueryModel, FreetextSearchEvent>
    implements BookmarkEventListener,
        SearchResultsListener<FreetextSearchResultEvent>,
        BlueBarEventListener {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(SearchQuerySection.class);
  ...
  @Component private TextField currentHidden;

  @Component(name = "inl", parameter = "nonlive", supported = true)
  private Checkbox includeNonLive;

  @TreeLookup private SearchResultsSection searchResults;
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (isPowerSelected(context)) {
      ...
      doAdvancedButton.addClickStatements(
          context, new FunctionCallStatement(restoreSkinnySearchActions));
      searchButton.addClickStatements(
          context, new FunctionCallStatement(restoreSkinnySearchActions));
    ...
    editQueryButton.setLabel(
        context, Check.isEmpty(model.getCriteria()) ? ADD_QUERY_LABEL : EDIT_QUERY_LABEL);
  ...
  private void showControls(RenderEventContext context, WizardPage powerPage) {
    ...
    final Map<String, List<ControlResult>> results =
        powerPage.renderPage(context, model.getUpdateData(), "wizard-controls");
  ...
  public void treeFinished(String id, SectionTree tree) {
    ...
    JSCallable resultsUpdater =
        searchResults.getResultsUpdater(tree, events.getEventHandler("changedWhere"), DIV_QUERY);

    UpdateDomFunction changedWhere =
        ajax.getAjaxUpdateDomFunction(
            tree,
            this,
            events.getEventHandler("changedWhere"),
            ajax.getEffectFunction(EffectType.REPLACE_IN_PLACE),
            DIV_QUERY);

    UpdateDomFunction editQuery =
        ajax.getAjaxUpdateDomFunction(
            tree,
            this,
            events.getEventHandler("editQuery"),
            ajax.getEffectFunction(EffectType.FADEOUTIN_ONRESULTS),
            "wherecontainer",
            "wizardcontainer");

    JSCallable clearQuery =
        searchResults.getResultsUpdater(tree, events.getEventHandler("clearQuery"), DIV_QUERY);

    restoreSkinnySearchActions =
        new ExternallyDefinedFunction("restoreSkinnySearchActions", UPDATE_INCLUDE);
    ...
    JSHandler searchHandler =
        new OverrideHandler(searchFunction, new ArrayExpression(onSearchFunctions.toArray()));
    ...
    collectionList.addChangeEventHandler(
        new OverrideHandler(
            updateInterface,
            changedWhereUpdaters,
            collectionList.createGetExpression(),
            currentHidden.createGetExpression()));

    clearQueryButton.setClickHandler(new OverrideHandler(clearQuery));

    editQueryButton.setClickHandler(
        new StatementHandler(
            new ExternallyDefinedFunction("editQuery", UPDATE_INCLUDE), editQuery));

    includeNonLive.setClickHandler(
        new StatementHandler(searchResults.getResultsUpdater(tree, null)));
  }
  ...
  public void forwardToRemote(SectionInfo info) {
    if (isRemoteSelected(info)) {
      changedWhere(info);
      remoteRepoWebService.forwardToSearch(
          info,
          federatedSearchService.getByUuid(collectionList.getSelectedValue(info).getUuid()),
          true);
  ...
  public void editQuery(SectionInfo info) {
    ...
    doAdvancedButton.addClickStatements(
        info, new FunctionCallStatement(restoreSkinnySearchActions));
  ...
  public DefaultSearch createDefaultSearch(SectionInfo info, boolean includeQuery) {
    ...
    if (session != null) {
      if (!session.isAllCollections()) {
        ...
        // UUID return results
        final Collection<ItemDefinition> collections =
            collectionService.getMatchingSearchableUuid(session.getCollectionUuids());
  ...
  public void setCollection(SectionInfo info, String collectionUuid) {
    ...
    collectionList.setSelectedStringValue(
        info, whereModel.createWhere(collectionUuid, WithinType.COLLECTION).getValue());
  ...
  public void setPowerSearch(SectionInfo info, String uuid, String xml) {
    SearchWhereModel whereModel = (SearchWhereModel) collectionList.getListModel();
    collectionList.setSelectedStringValue(
        info, whereModel.createWhere(uuid, WithinType.POWER).getValue());
    ...
    public void loadPowerPage() {
      ...
      powerPage =
          wizardPageService.createSimplePage(
              powerSearch.getWizard(), itemXml, new WebWizardPageState(), true);
    ...
    if (getSearchSettings().isSearchingShowNonLiveCheckbox()) {
      event.addScreenOptions(
          new SettingsRenderer(
              LABEL_NONLIVE,
              renderSection(context, includeNonLive),
              "screen-option")); //$NON-NLS-1$
    }
  }
