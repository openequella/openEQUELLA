...
public class CollectionBeanSerializer
    extends AbstractEquellaBaseEntitySerializer<ItemDefinition, CollectionBean, CollectionEditor> {
  @Inject private ItemDefinitionService collectionService;
  @Inject private CollectionEditorFactory editorFactory;
  ...
  @Override
  protected CollectionEditor createNewEditor(
      ItemDefinition entity, @Nullable String stagingUuid, boolean importing) {
    return editorFactory.createNewEditor(entity, stagingUuid, importing);
  }

  @Override
  protected CollectionEditor createExistingEditor(
  ...
  private List<DynamicRuleBean> getDynamicRules(ItemDefinition entity) {
    ...
    if (dynamicMetadataRules != null) {
      for (DynamicMetadataRule rule : dynamicMetadataRules) {
        ...
        List<TargetListEntryBean> targetListEntryBean =
            convertToTargetListEntryBean(rule.getTargetList());
    ...
    if (itemMetadataRules != null) {
      for (ItemMetadataRule rule : itemMetadataRules) {
        ...
        TargetList targetList =
            aclManager.getTargetList(
                Node.ITEM_METADATA, new ItemMetadataTarget(rule.getId(), entity));
  ...
  private Map<String, List<TargetListEntryBean>> getItemStatusesRules(ItemDefinition entity) {
    ...
    for (ItemStatus status : ItemStatus.values()) {
      TargetList targetList =
          aclManager.getTargetList(Node.ITEM_STATUS, new ItemStatusTarget(status, entity));
