...
public class ImageTiler {
  ...
  /**
   * We split the work up so that we can 'start' tiling as many images as the contributor wants, and
   * the viewer will see that an image is "currently being processed". @See
   * com.tle.web.plugin.image.LargeImageViewer.view(SectionContext context)
   */
  private boolean tileInit(File originalImage, File destFolder) throws IOException {
    ...
    if (image.exists()
        && image.lastModified() >= originalImage.lastModified()
  ...
  private File createRowStrip(
      final File src, int imageWidth, final int zoomLevel, final int row, final File basePath)
      throws IOException {
