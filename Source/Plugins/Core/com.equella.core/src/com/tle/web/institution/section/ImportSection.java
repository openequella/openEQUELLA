...
public class ImportSection extends AbstractEditSection<ImportSection.ImportModel> {
  ...
  private InstitutionInfo getInstitutionInfo(SectionInfo info) {
    ...
    return model
        ...
        .get(
            info,
            new CacheFiller<InstitutionInfo>() {
              @Override
  ...
  public void doAction(SectionInfo info) {
    ...
    if (validate(info, i)) {
      ...
      progressSection.setupProgress(
          info,
          instImportService.getConverterTasks(ConvertType.IMPORT, impInstInfo),
          "institutions.import.importing",
          i,
          new ProgressRunnable() {
            @Override
            public void run(ListProgressCallback callback) {
              instImportService.importInstitution(
                  copyStaging, targetSchemaId, newInstInfo, callback);
            }
