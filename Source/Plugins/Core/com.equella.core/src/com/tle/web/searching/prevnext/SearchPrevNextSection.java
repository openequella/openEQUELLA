...
public class SearchPrevNextSection
  ...
  public void nav(SectionInfo info, int direction) {
    ...
    if (targettedItemId != null) {
      ...
      MappedSearchIndexValues indexMap =
          mapAttrObj != null ? (MappedSearchIndexValues) mapAttrObj : null;
      ...
      if (itemKeyToView == null) {
        FreetextSearchResults<FreetextResult> singleSearchResultForAdjacent =
            freetextService.search(search, effectiveRelativeOffset, 1);
  ...
  /**
   * Visibility is set in the admin console for collections. Specific visibility here depends on
   * whether the search result set had more than one result.
   *
  ...
  /**
   * When called from nav (click of either forward/back button) we don't want to reset the model
   * state.
   *
   * @param info
   * @return
  ...
  private int verifyStateSetup(SectionInfo info) {
    ...
    if (!model.isStateSet()) {
      Object mapAttrObj =
          sessionService.getAttribute(MappedSearchIndexValues.MAPPED_SEARCH_ATTR_KEY);
      if (mapAttrObj != null) {
  ...
  public static class SearchPrevNextModel {
    /**
     * this is a reference to an entity held in the session, hence changes to it will persist beyond
    ...
    public int getIndexIntoSearch() {
      return mappedSearchIndexValues != null
          ? mappedSearchIndexValues.getIndexForItemId(currentItemKey)
          : -1;
    }
