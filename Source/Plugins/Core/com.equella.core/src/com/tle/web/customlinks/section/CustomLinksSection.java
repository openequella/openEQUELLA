...
public class CustomLinksSection extends OneColumnLayout<CustomLinksModel> {
  ...
  private static final PluginResourceHelper R =
      ResourcesService.getResourceHelper(CustomLinksSection.class);
  private static final JSCallable SETUP_LINKS =
      new ExternallyDefinedFunction(
          "setupLinks", JQuerySortable.PRERENDER, new IncludeFile(R.url("js/setuplinks.js")));
  ...
  @EventFactory private EventGenerator eventFactory;
  @AjaxFactory private AjaxGenerator ajax;

  @Inject private CustomLinkService linkService;
  @Inject private CustomLinksMenuContributor menuContributor;
  ...
  @Component private FileUpload file;
  ...
  @Inject private ExpressionSelectorDialog selector;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    displayNameField.addReadyStatements(
        new JQueryStatement(displayNameField, new FunctionCallExpression("focus")));

    TagState tag = new TagState();
    tag.addReadyStatements(
        SETUP_LINKS,
        new ObjectExpression("movedCallback", ajax.getAjaxFunction("urlMoved")),
        "#cls_us");
    linkDiv = new DivRenderer(tag);

    deleteIconButton.setClickHandler(eventFactory.getNamedHandler("removeIcon"));
    downloadButton.setClickHandler(
        new OverrideHandler(
            ajax.getAjaxUpdateDomFunction(
                tree,
                this,
                eventFactory.getEventHandler("downloadFavicon"),
                "currentIcon",
                "downloadIcon")));
    deleteUrlFunc =
        ajax.getAjaxUpdateDomFunction(
            tree,
            this,
            eventFactory.getEventHandler("deleteUrl"),
  ...
  protected TemplateResult setupTemplate(RenderEventContext context) {
    ...
    if (!Check.isEmpty(sessionId)) {
      ...
      image =
          new ImageRenderer(getIconUrl(context, bean.getUuid()), new TextLabel(bean.getFileName()));
  ...
  public void downloadFavicon(SectionInfo info) throws IOException {
    ...
    try {
      ...
      session
          .getValidationErrors()
          .put("urlField", R.getString("edit.download.error.invalid", ex.getLocalizedMessage()));
      return;
    } catch (ConnectException ex) {
      session
          .getValidationErrors()
          .put("urlField", R.getString("edit.download.error.connect", ex.getLocalizedMessage()));
      return;
    } catch (Exception ex) {
    ...
    if (method.getStatusCode() != HttpStatus.SC_OK) {
      session.getValidationErrors().put("urlField", R.getString("edit.download.error.notfound"));
      return;
    }

    removeIcon(info);
    try (ByteLimitedInputStream remoteStream =
        new ByteLimitedInputStream(method.getResponseBodyAsStream(), MAX_BYTES)) {
      ...
      if (!imageMagickService.supported(mimeTypeService.getMimeTypeForFilename(filename))) {
        session.getValidationErrors().put("urlField", R.getString("edit.download.error.process"));
        return;
      }
  ...
  public void getIcon(SectionInfo info, String uuid, String filename) throws IOException {
    ...
    if (fileSystemService.fileExists(stagingFile, filename)
        && !fileSystemService.fileIsDir(stagingFile, filename)) {
  ...
  private CustomLinkEditingSession saveInternal(
      SectionInfo info, CustomLinkEditingSession session) {
    ...
    if (expression != null) {
      ...
      if (Check.isEmpty(expression)) {
        ...
        list.getEntries()
            .add(new TargetListEntry(true, false, "VIEW_CUSTOM_LINK", expression)); // $NON-NLS-1$
      }
  ...
  public String getIconUrl(SectionInfo info, String uuid) {
    ...
    final String href =
        new BookmarkAndModify(
                info, eventFactory.getNamedModifier("getIcon", session.getStagingId(), fileName))
            .getHref();
    ...
    for (CustomLink link : linkService.enumerateInOrder()) {
      ...
      if (linkService.canDelete(link)) {
        ...
        delete.setClickHandler(
            new OverrideHandler(deleteUrlFunc, uuid).addValidator(DELETE_CONFIRM));
      }
