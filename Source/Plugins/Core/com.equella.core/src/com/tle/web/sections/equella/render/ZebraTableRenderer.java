...
public class ZebraTableRenderer extends TableRenderer implements JSTableComponent
{
	...
	private static final ExternallyDefinedFunction SORT_FUNCTION = new ExternallyDefinedFunction("sortColumn", 4,
		INCLUDE);
	private static final ExternallyDefinedFunction FILTER_FUNCTION = new ExternallyDefinedFunction("filterTable", 2,
		INCLUDE);

	private static final ExternallyDefinedFunction SWALLOW_ENTER_FUNCTION = new ExternallyDefinedFunction(
	...
	public void realRender(SectionWriter writer) throws IOException
	{
		...
		if( tableState.isFilterable() && tableState.getRows().size() >= filter )
		{
			...
			IfElseStatement ife = new IfElseStatement(new NotExpression(Jq.methodCall(ts, Js.function("hasClass"),
				"blur")), Js.call_s(FILTER_FUNCTION, Jq.$(state), Jq.$val(ts)), Js.call_s(FILTER_FUNCTION, Jq.$(state),
				Js.str("")));
	...
	public void preRender(PreRenderContext info)
	{
		...
		if( !info.getBooleanAttribute(state) )
		{
			...
			if( headerRow != null )
			{
				...
				{
					state.addReadyStatements(Js.call_s(SORT_FUNCTION, Jq.$(state), Jq.$(primarySortColumn),
						primarySortColumnIndex, true));
				}
			}
	...
	@Override
	protected void renderHeaderRow(SectionWriter writer, TableHeaderRow headerRow) throws IOException
	{
	...
	@Override
	protected void renderHeaderCell(SectionWriter writer, TableHeaderCell cell, int columnIndex) throws IOException
	{
	...
	private abstract class BaseZebraRowRenderer<R extends BaseTableRow<C>, C extends TableCell> extends AjaxTagRenderer
	{
		...
		public SectionRenderable getNestedRenderable()
		{
			...
			{
				result = CombinedRenderer.combineMultipleResults(result,
					createCellRenderer(cells.get(columnIndex), columnIndex));
			}
			return result;
		}
	...
	private class ZebraHeaderRowRenderer extends BaseZebraRowRenderer<TableHeaderRow, TableHeaderCell>
	{
	...
	private class ZebraCellRenderer extends TagRenderer
	{
		...
		protected ZebraCellRenderer(String tag, TableState tableState, TableCell state, int columnIndex)
		{
			super(tag, state);
			this.tableState = tableState;
		...
		public SectionRenderable getNestedRenderable()
		{
			...
			if( filterData != null )
			{
				...
				content = CombinedRenderer.combineMultipleResults(content,
					SectionUtils.convertToRenderer(filterDataRenderer));
			}
	...
	private class ZebraHeaderCellRenderer extends ZebraCellRenderer
	{
		...
		public ZebraHeaderCellRenderer(TableState tableState, TableHeaderCell state, int columnIndex)
		{
			...
			if( sortable )
			{
				...
				headerCellState.setClickHandler(Js.handler(Js.call_s(SORT_FUNCTION, Jq.$(tableState), Jq.$(cellState),
					columnIndex, false)));
			}
		}
