...
public class MerlotWebServiceImpl implements MerlotWebService {
  ...
  private static final Cache<String, Collection<NameValue>> OPTION_CACHE =
      CacheBuilder.newBuilder().softValues().expireAfterWrite(14, TimeUnit.DAYS).build();

  private static final Cache<String, Map<String, NameValue>> LANGUAGES_CACHE =
      CacheBuilder.newBuilder().softValues().expireAfterWrite(14, TimeUnit.DAYS).build();

  private static final Cache<String, Map<String, Collection<NameValue>>> CATEGORIES_CACHE =
      CacheBuilder.newBuilder().softValues().expireAfterWrite(14, TimeUnit.DAYS).build();
  ...
  public Collection<NameValue> getCategories(SectionInfo info, String categoryId) {
    ...
    if (categories == null) {
      ...
      for (Map.Entry<String, Collection<NameValue>> entry :
          merlotService.getCategories(repoWebService.getRemoteRepository(info)).entrySet()) {
        categories.put("categories." + entry.getKey(), entry.getValue());
      }
      CATEGORIES_CACHE.put("categories", categories);
    }

    Collection<NameValue> rv =
        categories.get("categories." + (Check.isEmpty(categoryId) ? "root" : categoryId));
    if (rv == null) {
  ...
  public Collection<NameValue> getTechnicalFormats(SectionInfo info) {
    ...
    if (technicalFormats == null) {
      technicalFormats =
          merlotService.getTechnicalFormats(repoWebService.getRemoteRepository(info));
