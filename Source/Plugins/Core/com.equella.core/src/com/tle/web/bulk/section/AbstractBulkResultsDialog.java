...
public abstract class AbstractBulkResultsDialog<T extends ItemKey>
    extends EquellaDialog<AbstractBulkResultsDialog.Model> implements OkayableDialog {
  protected static final int PER_PAGE = 8;

  private static final ExternallyDefinedFunction POLLING_CALL =
      new ExternallyDefinedFunction("setupPolling", AbstractBulkSelectionSection.BULKINCLUDE);
  ...
  @TreeLookup private AbstractBulkSelectionSection<T> selectionSection;
  ...
  @Component private Table selectionsTable;
  @Component private Table bulkResultsTable;
  @Component private Tree previewTree;

  private JSCallable updateCall;
  @Nullable private JSCallable cancelCallback;

  private Collection<Button> extraNavigation;

  protected abstract DynamicHtmlListModel<OperationInfo> getBulkOperationList(
  ...
  public void registered(String id, SectionTree tree) {
    ...
    operationList.addChangeEventHandler(
        getFooterUpdate(tree, events.getEventHandler("changeOperation")));
    ...
    okButton.setClickHandler(
        events.getNamedHandler("execute").addValidator(new Confirm(LABEL_CONFIRM)));
  ...
  protected SectionRenderable renderConfirmation(RenderContext context) {
    ...
    model.setShowingLabel(
        new KeyLabel(KEY_SHOWING, offset + 1, offset + pageOfIds.size(), totalSelections));
  ...
  protected Collection<Button> collectFooterActions(RenderContext context) {
    ArrayList<Button> footerActions =
        Lists.newArrayList(prevButton, okButton, nextButton, previewButton);
  ...
  public void execute(SectionInfo info) {
    ...
    if (opInfo == null) {
      throw new SectionsRuntimeException(
          "Can't execute the operation without appropriate privileges");
    }
  ...
  private SectionRenderable manifestNextPrevForTask(
      RenderContext context,
      Model model,
      String taskId,
      @Nullable BulkOperationExtension op,
    ...
    context
        .getForm()
        .addReadyStatements(
            POLLING_CALL, updateCall, Jq.$(bulkResultsTable), LABEL_HIDECONFIRM.getText());

    return viewFactory.createResult("bulkresults.ftl", this);
  }

  private void manifestNextPrevForOp(
      RenderContext context,
      Model model,
      @Nullable BulkOperationExtension op,
      @Nullable String opId) {
    if (opId == null) {
      throw new SectionsRuntimeException(
          "Can't show execute buttons without an operation selected");
    }
    nextButton.setDisplayed(
        context, !model.isShowOptions() && !(op == null || !op.hasExtraOptions(context, opId)));
    ...
    if (op == null) {
      ...
      if (op.hasPreview(context, opId) && !model.isShowPreview()) {
        previewButton.setDisplayed(
            context, op.areOptionsFinished(context, opId) && model.isShowOptions());
        okButton.hide(context);
  ...
  public static class Model extends DialogModel {
    @Bookmarked private String taskId;
    @Bookmarked private boolean showOptions;
    @Bookmarked private boolean forExecute;
    @Bookmarked private boolean showPreview;
    ...
    @Bookmarked private long previewItemId;
        ...
        nodeDiv =
            new DivRenderer(
                new CombinedRenderer(
                    new LabelRenderer(getLabel()), new LabelRenderer(new TextLabel(content))));
      } else {
