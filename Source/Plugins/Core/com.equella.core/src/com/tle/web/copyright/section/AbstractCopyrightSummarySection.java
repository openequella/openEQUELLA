...
@NonNullByDefault
public abstract class AbstractCopyrightSummarySection<
        H extends Holding, P extends Portion, S extends Section>
    extends AbstractPrototypeSection<AbstractCopyrightSummarySection.Model>
    implements HtmlRenderer, AttachmentSelectorEventListener {
  static {
  ...
  @Component private Button activateSelected;
  @Component private MappedBooleans selections;
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (isVisible(context)) {
      ...
      if (!holdingDisplay.getOtherPortions().isEmpty()
          && !aclService
              .filterNonGrantedPrivileges(ActivationConstants.VIEW_LINKED_PORTIONS)
              .isEmpty()) {
  ...
  private void processSections(
    ...
    NewDefaultViewableItem vitem =
        viewableItemFactory.createNewViewableItem(portionItem.getItemId());
    ...
    for (Section section : portion.getSections()) {
      ...
      if (attachment != null) {
        ...
        if (status == ActivateRequest.TYPE_ACTIVE) {
          ...
          holdingDisplay.setTotalInactivePercent(
              percent + holdingDisplay.getTotalInactivePercent());
        }
        ...
        if (canActivate && (session == null || session.isSelectAttachments()) && !restricted) {
          ...
          activateButton.setClickHandler(
              events.getNamedHandler(
                  "activateOne", //$NON-NLS-1$
                  jsonAttachId));
          ButtonRenderer activateOneButton =
              new ButtonRenderer(activateButton).addClass("activate-one");
          if (inIntegration) {
          ...
          boolean canAdd =
              !integrating
                  || activationService.attachmentIsSelectableForCourse(
                      copyrightService.getActivationType(), attachUuid, courseCode);
          if (canAdd) {
            ...
            sectionDisplay.setAddButton(
                new ButtonRenderer(addState).showAs(ButtonType.PLUS).addClass("add"));
    ...
    if (session != null) {
      ...
      if (integrating) {
        final String courseCode =
            getCourseCode(info, session, integrationService.isInIntegrationSession(info));
        if (activationService.attachmentIsSelectableForCourse(
            copyrightService.getActivationType(), attachmentId, courseCode)) {
          final List<ActivateRequest> requests =
              activationService.getAllCurrentAndPendingActivations(
                  getCopyrightServiceImpl().getActivationType(), attachmentId);

          // FIXME: we need some logic to pull out the best activate request in the case there is >
          // 1
          // Pull out the first activate request with matching course if there is one, otherwise
          // just whatever is first.
          activateRequest =
              requests.stream()
                  .sorted(
                      (o1, o2) -> {
                  ...
                  .get();
          attachment =
              copyrightWebService.getAttachmentMap(info, item).get(activateRequest.getAttachment());
        } else {
          throw new RuntimeException(
              "Attachment " + attachmentId + " is not selectable for the current course.");
        }
      } else {
  ...
  private void addResource(
      ...
      @Nullable ActivateRequest activateRequest) {
    ...
    if (cls != null && cls.isApplicable(info)) {
      ...
      addResource(
          info, new SelectedResource(item.getItemId(), attachment, null, null), activateRequest);
    }
  }

  private void addResource(
      SectionInfo info, SelectedResource resource, @Nullable ActivateRequest activateRequest) {
  ...
  protected double setupPageRange(
      HoldingDisplay holdingDisplay, SectionDisplay sectionDisplay, Section section) {
    double percent = 0;
  ...
  private HtmlLinkState getViewItemCommand(
      RenderEventContext context, Item item, @Nullable Attachment attachment) {
  ...
  private void sortPortions(
      HoldingDisplay holdingDisplay,
      List<PortionDisplay> portions,
      List<LinkRenderer> otherPortions) {
    if (holdingDisplay.isBook()) {
      Collections.sort(
          portions,
          new NumberStringComparator<PortionDisplay>() {
    ...
    Collections.sort(
        otherPortions,
        new NumberStringComparator<LinkRenderer>() {
          private static final long serialVersionUID = 1L;
