...
public class OAuthClientEditorSection
  ...
  public final SectionResult renderHtml(RenderEventContext context) {
    ...
    model.setDefaultRedirectUrl(
        institutionService.institutionalise(OAuthWebConstants.OAUTH_DEFAULT_REDIRECT_URL));
  ...
  public void registered(String id, SectionTree tree) {
    ...
    selectFlow.setListModel(
        new DynamicHtmlListModel<OAuthFlowDefinition>() {
          @Override
          ...
          @Override
          protected Option<OAuthFlowDefinition> convertToOption(
              SectionInfo info, OAuthFlowDefinition obj) {
    ...
    chooseUrl.setListModel(
        new SimpleHtmlListModel<NameValue>(
            new NameValue(CurrentLocale.get(CHOOSE_URL_FALSE), "false"),
    ...
    selectUserDialog.setOkCallback(
        ajax.getAjaxUpdateDomFunction(
            tree, null, events.getEventHandler("selectUser"), inplace, "userAjaxDiv"));

    selectFlow.addChangeEventHandler(
        ajax.getAjaxUpdateDomFunction(
            tree, null, events.getEventHandler("typeSelected"), inplace, "flowAjaxDiv"));
    chooseUrl.addChangeEventHandler(
        ajax.getAjaxUpdateDomFunction(
            tree, null, events.getEventHandler("chooseUrlSelected"), inplace, "flowAjaxDiv"));
  ...
  private boolean validate(
      SectionInfo info, OAuthClientEditingBean oauth, Map<String, Object> errors) {
    errors.clear();
    ...
    if (Check.isEmpty(redirectUrl)
        && selectFlow.getSelectedValue(info).isSetUrl()
  ...
  private OAuthClientEditingSession saveToSession(
      SectionInfo info, OAuthClientEditingSession session, boolean validate) {
    ...
    if (selectFlow.getSelectedValue(info) != null
        && selectFlow.getSelectedValue(info).isSetUser()) {
