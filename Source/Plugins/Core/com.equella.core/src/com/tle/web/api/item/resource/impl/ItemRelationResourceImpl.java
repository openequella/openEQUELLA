...
public class ItemRelationResourceImpl {
  ...
  public List<RelationBean> getAll(
      @PathParam("uuid") String uuid, @PathParam("version") int version) {
  ...
  public Response create(
      @PathParam("uuid") String uuid, @PathParam("version") int version, RelationBean relation) {
    ...
    long relationId =
        relationService.createRelation(
            fromId,
            toId,
            fromId.equals(thisId),
            relation.getRelation(),
            relation.getFromResource(),
            relation.getToResource());
    return Response.created(
            UriBuilder.fromResource(getClass())
                .path(getClass(), "get")
                .build(uuid, version, relationId))
        .build();
  }
  ...
  private RelationBean convertBean(Relation relation, ItemKey itemId) {
    ...
    links.put(
        "self",
        urlLinkService
            .getMethodUriBuilder(getClass(), "get")
            .build(itemId.getUuid(), itemId.getVersion(), relation.getId())
            .toString());
    return relationBean;
  }
