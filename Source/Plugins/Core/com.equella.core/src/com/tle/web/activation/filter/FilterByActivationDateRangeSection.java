...
public class FilterByActivationDateRangeSection
    extends AbstractPrototypeSection<FilterByActivationDateRangeSection.Model>
  ...
  public void treeFinished(String id, SectionTree tree) {
    ...
    OverrideHandler handler =
        new OverrideHandler(searchResults.getResultsUpdater(tree, null, "activation-date-clear"));
    ...
    clearButton.setClickHandler(
        new OverrideHandler(
            searchResults.getResultsUpdater(
                tree, events.getEventHandler("clear"), "activation-date-range-filter")));
  }
  ...
  private void setDates(
      ...
      TleDate[] startDateRange,
      TleDate[] endDateRange,
      TleDate[] activateddateRange) {
    if (startDateRange != null) {
      event.addDateFilter(
          new DateFilter(FreeTextQuery.FIELD_ACTIVATION_FROM, convertDates(startDateRange)));
    }

    if (endDateRange != null) {
      event.addDateFilter(
          new DateFilter(FreeTextQuery.FIELD_ACTIVATION_UNTIL, convertDates(endDateRange)));
    }
