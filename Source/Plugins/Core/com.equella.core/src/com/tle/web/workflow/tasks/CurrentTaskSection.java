...
public class CurrentTaskSection extends AbstractPrototypeSection<CurrentTaskSection.TasksModel>
  ...
  @Component @Inject private ApproveDialog approveDialog;

  @Component @Inject private RejectDialog rejectDialog;

  @Component @Inject private CommentDialog commentDialog;
  ...
  @Component private Button assignButton;
  ...
  public void assign(SectionInfo info) throws Exception {
    ...
    itemService.operation(
        taskState.getItemTaskId(),
        workflowFactory.assign(taskState.getTaskId()),
  ...
  public void doComment(
      SectionInfo info, CommentType commentType, String step, String comment, String messageUuid) {
    TasksModel model = getModel(info);
    TaskListState taskState = model.getTaskState();
  ...
  private void tryNavigation(SectionInfo info, int dir) {
    ...
    if (selectedTaskList != null) {
      ...
      moderationService.moderate(
          info, ModerationService.VIEW_METADATA, itemTaskId, newIndex, totalResults);
    }
    return;
  }
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (taskListSize != 1) {
      if (selectedTaskList != null) {
        listButton.setLabel(
            context, new KeyLabel(KEY_TASK_LISTOF, (taskState.getIndex() + 1), taskListSize));
      } else {
        listButton.setLabel(
            context, new KeyLabel(KEY_LISTOF, (taskState.getIndex() + 1), taskListSize));
      }
    }
    ...
    GenericTemplateResult template =
        new GenericTemplateResult(
            viewFactory.createNamedResult(OneColumnLayout.UPPERBODY, "currenttask.ftl", this));
    if (model.getModalSection() != null) {
      return new CombinedTemplateResult(
          renderToTemplate(context, model.getModalSection().getSectionId()), template);
    }
