...
/**
 * A file viewing utility to generate a URL which invokes google's doc viewer to render various mime
 * types of the MS office suite and other proprietary mime types:
 *
 * <ul>
 *   <li>Microsoft Word (.DOC and .DOCX)
 *   <li>Microsoft Excel (.XLS and .XLSX)
 *   <li>Microsoft PowerPoint 2007 / 2010 (.PPTX)
...
public class GoogleDocViewer extends FileViewer implements ViewItemViewer {
  /**
   * The string which acts as a link between specified mime types and the GoogleDoc action (in
   * attributes.conf), and also serves as the key for the dialogue title.
   */
  public static final String MIME_ACTION_KEY = "googledocviewer";

  /**
   * There probably wont be any locale-specific alternatives to docs.google.com, but it makes more
   * sense to preserve the URL In a properties file than hard-coded into java. Expected to evaluate
   * to<br>
  ...
  private static final PluginResourceHelper RESOURCES =
      ResourcesService.getResourceHelper(GoogleDocViewer.class);
  ...
  public ResourceViewerConfigDialog createConfigDialog(
    ...
    GoogleDocViewerConfigDialog gdvcd =
        componentFactory.createComponent(
            parentId, "dfcd", tree, GoogleDocViewerConfigDialog.class, true);
    gdvcd.setTemplate(dialogTemplate);
    return gdvcd;
  }

  /**
   * Gets the canonical string for the Equella resource. Converts that string to its URL encoded
   * variant, so "http://mumble.com" becomes "http%3A%2F%2Fmumble.com" Prefix with the Google URL
   * and package into a Bookmark we can return.
   */
  @Override
  ...
  private String getViewerUrl(String localFileUrl) {
    ...
    // single string ...
    String fullEquellaURLWithTokenParam =
        localFileUrl + (localFileUrl.contains("?") ? "&" : "?") + "token=" + encodedToken;

    // ... all of which is then encoded as a suffix to the plain google URL)
    UrlEncodedString urlAllEncodedStringPlusToken =
        UrlEncodedString.createFromValue(fullEquellaURLWithTokenParam);
    ...
    if (!Strings.isNullOrEmpty(resource.getFilepath())) {
      ...
      ViewableResource viewableResource =
          attachmentResourceService.getViewableResource(
              info,
              vitem,
              new UnmodifiableAttachments(vitem.getItem())
                  .getAttachmentByFilename(resource.getFilepath()));
