...
public class LtiConsumerUserStateHook extends AbstractOAuthV1UserStateHook {
  ...
  protected String getSecretFromKey(String consumerKey) {
    ...
    if (consumer == null || consumer.isDisabled()) {
      String error =
          CurrentLocale.get(
              LTI_ERROR_PREFIX + (consumer == null ? "client.missing" : "client.disabled"));
      error +=
          MessageFormat.format(
              " ({0} = {1})", LtiConsumerConstants.PARAM_CONSUMER_KEY, consumerKey);
      LOGGER.error(error);
  ...
  protected UserStateResult getUserStateResult(HttpServletRequest request) {
    try {
      ...
      throw new WebException(
          401, error, CurrentLocale.get(LTI_ERROR_PREFIX + "nousername", e.getUsername()));
    } catch (AuthenticationException e) {
      throw new WebException(
          400, e.getMessage(), CurrentLocale.get(LTI_ERROR_PREFIX + e.getMessage()));
    }
  }

  /**
   * The workaround for Moodle and Canvas OAuth.<br>
   * If we have a duplicate, and it came from Moodle, it's worth presuming a different reality
   * applies. Hopefully by version moodle-3 they'll have fixed this. ext_lms for moodle 2.3, 2.4,
   * 2.5 was literally "moodle-2". Use startsWith in case future moodle 2.x has an extended string.
   * Read: Dodgical hax
   *
   * @param request
   * @return
    ...
    if ("canvas".equalsIgnoreCase(product)
      ...
      if (dupe) {
        return new OAuthMessage(
            request.getMethod(),
            urlService.getUriForRequest(request, null).toString(),
            newParams.entrySet());
      }
    }
