...
public class ItemLinkServiceImpl implements ItemLinkService {
  ...
  public RootFolderBean addLinks(RootFolderBean stagingBean) {
    ...
    final String selfUrl =
        institutionService.institutionalise(
            PathUtils.filePath(PATH_FILE_API, stagingBean.getUuid(), CONTEXT_FILE_DIR));
    links.put(REL_SELF, selfUrl);
    links.put(REL_DIR, selfUrl);
    links.put(
        REL_CONTENT,
        institutionService.institutionalise(
            PathUtils.filePath(PATH_FILE_API, stagingBean.getUuid(), CONTEXT_FILE_CONTENT)));
  ...
  private void addGenericFileLinks(StagingFile staging, GenericFileBean fileBean, String fullPath) {
    ...
    final String selfUrl =
        institutionService.institutionalise(
            URLUtils.urlEncode(
                PathUtils.filePath(PATH_FILE_API, staging.getUuid(), CONTEXT_FILE_DIR, fullPath),
                false));
    links.put(REL_SELF, selfUrl);
    links.put(REL_DIR, selfUrl);
    links.put(
        ...
        institutionService.institutionalise(
            URLUtils.urlEncode(
                PathUtils.filePath(
                    PATH_FILE_API, staging.getUuid(), CONTEXT_FILE_CONTENT, fullPath),
                false)));
