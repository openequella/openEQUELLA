...
@NonNullByDefault
public abstract class AbstractTitleAndDescriptionSection<
        I extends IItem<?>, M extends AbstractTitleAndDescriptionSection.TitleAndDescriptionModel>
    extends AbstractPrototypeSection<AbstractTitleAndDescriptionSection.TitleAndDescriptionModel>
    implements HtmlRenderer {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(AbstractTitleAndDescriptionSection.class);
  private static final IncludeFile JS = new IncludeFile(resources.url("scripts/itemsummary.js"));
  private static final JSCallAndReference SUMMARY_CLASS =
      new ExternallyDefinedFunction("ItemSummary", JS);
  private static final JSCallable SELECT_ITEM_FUNCTION =
      new ExternallyDefinedFunction(SUMMARY_CLASS, "selectItem", 3);
  ...
  public SectionResult renderHtml(RenderEventContext context) {
    ...
    if (selectItemFunction != null) {
      ...
      final JSHandler selectHandler =
          new OverrideHandler(
              SELECT_ITEM_FUNCTION,
              Jq.$(button),
              selectItemFunction,
              itemId,
      ...
      final SectionRenderable dr =
          new DivRenderer(
              "selectitem",
              new ButtonRenderer(button).showAs(ButtonType.PLUS).addClasses("button-expandable"));
      model.setSelectDiv(dr);
      model.setSelectable(true);
    }

    return viewFactory.createNamedResult(
        "section_nameowner", "viewitem/title_and_description.ftl", context);
  }
