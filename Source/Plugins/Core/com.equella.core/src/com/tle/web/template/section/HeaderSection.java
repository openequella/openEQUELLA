...
@Bind
public class HeaderSection extends AbstractPrototypeSection<HeaderSection.HeaderModel>
  ...
  private void setupTitle(RenderContext context, HeaderModel model) {
    ...
    model.setTitle(
        new LabelRenderer(
            AppendedLabel.get(decorations.getTitle(), LABEL_WINDOWTITLE, TextLabel.SPACE)));
  }

  private void setupPostMarkupScripts(
      RenderContext context, HeaderModel model, StandardRenderContext helper) {
    ...
    model.setExternalPostScripts(
        jsFileList.subList(model.getExternalScripts().size(), jsFileList.size()));
  }

  private void setupHeaderScripts(
      RenderContext context, HeaderModel model, StandardRenderContext helper) {
  ...
  private List<JSStatements> preRenderPageScripts(
      RenderContext context, StandardRenderContext helper) {
