...
public class ItemModerationResourceImpl implements ItemModerationResource {
  ...
  private void convertTree(
      Map<String, TreeNode> treeNodeMap, TreeNode treeNode, WorkflowNode workflowNode) {
  ...
  @Nullable
  private ItemNodeStatusBean serializeNodeStatus(
      Set<WorkflowNodeStatus> nodeStatuses, WorkflowNode rootWorkflowNode) {
  ...
  private ItemNodeStatusBean serializeNodeStatus(TreeNode treeNode) {
    ...
    if (node.getType() == WorkflowNode.ITEM_TYPE) {
      ...
      if (task.getDescription() != null) {
        taskStatusBean.setDescription(
            new SimpleI18NStrings(task.getDescription().getStrings()).asI18NString(null));
      }
    ...
    List<ItemNodeStatusBean> childBeans =
        Lists.transform(
  ...
  private void addMessages(ItemNodeStatusBean statusBean, List<WorkflowMessage> messages) {
    statusBean.setComments(
        new ArrayList<>(
            Lists.transform(
                messages,
                new Function<WorkflowMessage, ItemNodeStatusMessageBean>() {
                  @Override
                  public ItemNodeStatusMessageBean apply(WorkflowMessage msg) {
                    return new ItemNodeStatusMessageBean(
                        MSGTYPE_MAP.get(msg.getType()),
                        new UserBean(msg.getUser()),
                        msg.getMessage(),
                        msg.getDate());
                  }
                })));
