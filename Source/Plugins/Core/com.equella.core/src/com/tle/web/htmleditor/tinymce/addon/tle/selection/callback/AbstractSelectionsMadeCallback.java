...
public abstract class AbstractSelectionsMadeCallback implements SelectionsMadeCallback {
  ...
  private static final PluginResourceHelper urlHelper =
      ResourcesService.getResourceHelper(AbstractSelectionsMadeCallback.class);
  private static final ExternallyDefinedFunction DO_SELECTION_FUNCTION =
      new ExternallyDefinedFunction(
          "DoSelection.selectionsMade",
          new IncludeFile(
              urlHelper.plugUrl(
                  "com.tle.web.htmleditor.tinymce", "scripts/tinymce/tiny_mce_popup.js")),
  ...
  public boolean executeSelectionsMade(SectionInfo info, SelectionSession session) {
    ...
    final FunctionCallStatement selectionsMadeCall =
        Js.call_s(
            DO_SELECTION_FUNCTION,
            sessionId,
            massageSelectedResources(sessionId, pageId, session, info));

    final FunctionCallStatement selectionsMadeSetupCall =
        Js.call_s(new ExternallyDefinedFunction("DoSelection.sm"), Js.function(selectionsMadeCall));
  ...
  public void executeModalFinished(SectionInfo info, ModalSession session) {
    ...
    CloseWindowResult closeMe =
        new CloseWindowResult(new FunctionCallStatement(DO_SELECTION_FUNCTION, sessionId));
    info.getRootRenderContext().setRenderedBody(closeMe);
  }

  protected ObjectExpression[] massageSelectedResources(
      String sessionId, String pageId, SelectionSession session, SectionInfo info) {
    ...
    for (SelectedResource res : session.getSelectedResources()) {
      ...
      final ViewableResource vres =
          createViewableResourceFromSelection(info, res, sessionId, pageId);
  ...
  protected void populateSelection(
      SectionInfo info, ObjectExpression jsobj, ViewableResource vres, String title) {
    final String vhref =
