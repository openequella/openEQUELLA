...
public class FindUsesContentSection
  ...
  private static final JSCallable SETUP_DETAILS =
      new ExternallyDefinedFunction("setupDetails", INCLUDE);
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (connector != null) {
      if (repositoryService.isRequiresAuthentication(connector)) {
        ...
        final ConnectorTerminology terms =
        ...
        if (!checkErrors(context)) {
          ...
          if (usage != null && usage.size() > 0) {
            ...
            final UnmodifiableAttachments attachments =
                new UnmodifiableAttachments(itemInfo.getItem());

            // column headings
            final TableState tableState = usageTable.getState(context);
            final TableHeaderRow header =
                tableState.setColumnHeadings(
                    new KeyLabel(terms.getCourseHeading()),
                    new KeyLabel(terms.getLocationHeading()),
                    LABEL_DATE_ADDED,
                    null);
            ...
            for (ConnectorContent use : usage) {
              ...
              List<ConnectorContentAttribute> renderAttList =
                  new ArrayList<ConnectorContentAttribute>();
              for (ConnectorContentAttribute attribute : attributeList) {
                renderAttList.add(
                    new ConnectorContentAttribute(
                        attribute.getLabelKey(),
              ...
              if (!Check.isEmpty(attachmentUuid)) {
                ...
                display.setAttachmentName(
                    new TextLabel(
                        attachment == null ? attachmentUuid : attachment.getDescription()));
              }
              SectionRenderable details =
                  viewFactory.createResultWithModel("finduses/details.ftl", display);

              TableCell dropArrowCol =
                  new TableCell(new ImageRenderer(URL_DROP_ARROW, LABEL_SHOW_DETAILS))
                      .addClass("droparrow");
  ...
  public void registered(String id, SectionTree tree) {
    ...
    UpdateDomFunction ajaxUpdateDomFunction =
        ajax.getAjaxUpdateDomFunction(
            tree, this, events.getEventHandler("loadUsage"), "lms-table-ajax");
  ...
  private boolean checkErrors(SectionInfo info) {
    ...
    try {
      ...
      if (t instanceof LmsUserNotFoundException) {
        model.setError(
            CurrentLocale.get(KEY_NO_USER, ((LmsUserNotFoundException) t).getUsername()));
      } else {
  ...
  protected List<ConnectorContent> getUsage(SectionInfo info) throws LmsUserNotFoundException {
    ...
    if (content == null) {
      ...
      if (connector != null) {
        ...
        content =
            repositoryService.findUsages(
                connector,
                CurrentUser.getUsername(),
                item,
                showArchived.isChecked(info),
                showAllVersion.isChecked(info));
