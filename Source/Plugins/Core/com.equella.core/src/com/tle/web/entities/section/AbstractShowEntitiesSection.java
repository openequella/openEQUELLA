...
/**
 * Make sure your section tree contains a TreeIndexed subclass of AbstractEntityContributeSection if
 * you wish to add or edit entities.
 *
 * @author Aaron
 * @param <E> The entity type. E.g. Connector
 * @param <M> Your section model. Must extend AbstractShowEntitiesSection.AbstractShowEntitiesModel
 */
@SuppressWarnings("nls")
public abstract class AbstractShowEntitiesSection<
        E extends BaseEntity, M extends AbstractShowEntitiesSection.AbstractShowEntitiesModel>
  ...
  public void registered(String id, SectionTree tree) {
    ...
    deleteFunction =
        ajax.getAjaxUpdateDomFunction(
            tree,
            this,
            events.getEventHandler("deleteEntity"),
    ...
    toggleEnabledFunction =
        ajax.getAjaxUpdateDomFunction(
            tree,
            this,
            events.getEventHandler("toggleEnabled"),
            ajax.getEffectFunction(EffectType.REPLACE_IN_PLACE),
            getAjaxId());
  ...
  /**
   * Assumed yes (because you can get to the page) unless you want to disallow editing.
   *
   * @param info
   * @param ent
   * @return
  ...
  protected SectionRenderable createToggleEnabledLink(SectionInfo info, E ent) {
    final HtmlLinkState actionLink =
        new HtmlLinkState(new OverrideHandler(toggleEnabledFunction, ent.getUuid()));
  ...
  protected SectionRenderable createEditLink(SectionInfo info, E ent) {
    final LinkRenderer link =
        new LinkRenderer(new HtmlLinkState(new OverrideHandler(editFunction, ent.getUuid())));
  ...
  protected SectionRenderable createCloneLink(SectionInfo info, E ent) {
    final LinkRenderer link =
        new LinkRenderer(new HtmlLinkState(new OverrideHandler(cloneFunction, ent.getUuid())));
  ...
  protected SectionRenderable createDeleteLink(SectionInfo info, E ent) {
    ...
    if (isInUse(info, ent)) {
      link =
          new LinkRenderer(
              new HtmlLinkState(
                  new OverrideHandler(Js.alert_s(getInUseLabel(info, ent).getText()))));
      link.setLabel(getDeleteLabel(info, ent));
    } else {
      link =
          new LinkRenderer(
              new HtmlLinkState(
                  new OverrideHandler(deleteFunction, ent.getUuid())
                      .addValidator(getDeleteValidator(info, ent))));
      link.setLabel(getDeleteLabel(info, ent));
    }
    return link;
  }
  ...
  /**
   * By default, a validator on delete is a simple confirm dialog, drawing on a customisable
   * message. Override where a more elaborate validator is called for.
   *
   * @param info
   * @param ent
   * @return
