...
public abstract class AbstractWizardViewItemActionSection extends GenericMinorActionSection {
  ...
  protected void forwardToWizard(
      SectionInfo info, boolean edit, boolean redraft, boolean newVersion) throws Exception {
    final Item item = getItemInfo(info).getItem();
    webWizardService.forwardToLoadItemWizard(
        info, item.getUuid(), item.getVersion(), edit, redraft, newVersion);
  }
}
