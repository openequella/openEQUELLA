...
public class RendererFactoryImpl implements RendererFactory {
  private final Map<RendererKey, RendererFactoryExtension> mappings =
      new HashMap<RendererKey, RendererFactoryExtension>();
  ...
  private SectionRenderable getRendererInternal(
      SectionInfo info, HtmlComponentState state, String renderer) {
  ...
  public void setPluginService(PluginService pluginService) {
    rendererTracker =
        new PluginTracker<RendererFactoryExtension>(
            pluginService,
            "com.tle.web.sections.standard",
            "rendererFactory",
            null,
  ...
  protected void addMapping(
      String rendererType, Class<?> stateClass, Class<? extends SectionRenderable> renderClass) {
      ...
      if (rendererTracker.needsUpdate()) {
        ...
        for (Extension extension : rendererTracker.getExtensions()) {
          ...
          if (rendererParam == null) {
            ...
            renderClazz =
                (Class<? extends SectionRenderable>)
                    rendererTracker.getClassForName(extension, rendererParam.valueAsString());
          }
          Collection<Parameter> renderIds = extension.getParameters("rendererId"); // $NON-NLS-1$
          for (Parameter renderIdParam : renderIds) {
            Collection<Parameter> stateClasses =
                extension.getParameters("stateClassName"); // $NON-NLS-1$
            ...
            for (Parameter stateClassParam : stateClasses) {
              try {
                ...
                final Class<? extends HtmlComponentState> clazz =
                    (Class<? extends HtmlComponentState>)
                        rendererTracker.getClassForName(extension, stateClass);
