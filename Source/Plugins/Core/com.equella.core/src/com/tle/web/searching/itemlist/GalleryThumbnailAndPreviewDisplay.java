      ...
      public void processEntry(StandardItemListEntry entry) {
        ...
        if (thumb.startsWith("custom:")) {
          ...
          final ViewableResource viewableResource =
              attachmentResourceService.getViewableResource(context, viewItem, attachment);
          entry.addThumbnail(
  ...
  private DivRenderer attachPreview(IAttachment att, ViewableResource viewableResource) {
    ...
    String previewPath =
        new StringBuilder(thumbPath)
            .replace(lastIndex, thumbPath.length(), FileSystemService.GALLERY_PREVIEW_EXTENSION)
            .toString();
