 ...
 *
 * <p>When Equella is acting as an OAuth client, this is the redirect URL you should specify.
 * Basically, it's a copy of the sample REST client LoginServlet
 *
 * @author Aaron
 */
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (error != null) {
      ...
      request.setAttribute(
          WebConstants.KEY_LOGIN_EXCEPTION,
          new Exception(
              CurrentLocale.get("com.tle.web.usermanagement.canvas.error.canvaserror", error)));
      LogonSection.forwardToLogon(
          sectionsController, request, response, null, LogonSection.STANDARD_LOGON_PATH);
      return;
    }
    ...
    if (code == null) {
      ...
      final Request codeRequest =
          new Request(PathUtils.urlPath(settings.getCanvasUrl(), "login/oauth2/auth"));
    ...
    final Request tokenRequest =
        new Request(PathUtils.urlPath(settings.getCanvasUrl(), "login/oauth2/token"));
    ...
    final String token;
    try (final Response tokenResponse =
        httpService.getWebContent(tokenRequest, configService.getProxyDetails())) {
      if (!tokenResponse.isOk()) {
        throw new RuntimeException(
            "Problem retrieving token from Canvas ("
    ...
    final Request apiRequest =
        new Request(PathUtils.urlPath(settings.getCanvasUrl(), "api/v1/users/self/profile"));
