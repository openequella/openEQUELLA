...
public class HtmlEditorSettingsFrontPageSection
    extends AbstractPrototypeSection<
        HtmlEditorSettingsFrontPageSection.HtmlEditorSettingsFrontPageModel>
    implements ReadyToRespondListener, HtmlRenderer {

  @PlugKey("settings.front.table.heading")
  private static Label LABEL_TABLE_HEADING;

  @TreeLookup private RootHtmlEditorSettingsSection root;
    ...
    for (ModalHtmlEditorSettingsSection subPage : subPages) {
      ...
      if (settingInfo != null) {
        final HtmlLinkState link =
            new HtmlLinkState(
                settingInfo.getLinkTitle(),
                events.getNamedHandler("linkClicked", settingInfo.getId()));
        settingsTable.addRow(
            context,
            CombinedRenderer.combineMultipleResults(
                new LinkRenderer(link),
                new BrRenderer(),
                new LabelRenderer(settingInfo.getBlurb())));
      }
    }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    subSettings =
        new CollectInterfaceHandler<ModalHtmlEditorSettingsSection>(
            ModalHtmlEditorSettingsSection.class);
  ...
  public static class HtmlEditorSettingsFrontPageModel {
    @Bookmarked private String link;
