...
public class RemoveViewPackageContentXmlMigration extends XmlMigrator {
  /**
   * TODO: it seems a lot of overhead to be reading in these XML files multiple times for the same
   * entity type... e.g. see DisplayTemplateLocationXmlMigrator which also works on ItemDefinitions
   * The entity post read migrator should probably have an additional parameter for the entity type
   */
  ...
  public void execute(
    ...
    for (String entry : xmlHelper.getXmlFileList(itemDefFolder)) {
      ...
      Iterator<PropBagEx> iter =
          summarySections.iterator("configList/com.tle.beans.entity.itemdef.SummarySectionsConfig");
      while (iter.hasNext()) {
