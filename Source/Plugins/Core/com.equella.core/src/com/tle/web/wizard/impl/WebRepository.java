...
public class WebRepository implements LERepository {
  ...
  @Inject private FileSystemService fsys;
  ...
  @Inject private PluginTracker<PackageTreeBuilder> builderTracker;
   ...
   * @throws IOException
   */
  public PackageInfo createPackageNavigation(
      SectionInfo info,
  ...
  private void dfsNodeTraversal(
      @Nullable List<ItemNavigationNode> nodes,
    ...
    for (PackageTreeBuilder builder : getTreeBuilders()) {
      if (builder.canHandle(info, stagingHandle, packageFilepath)) {
        final List<String> packageTypes =
            builder.determinePackageTypes(info, stagingHandle, packageFilepath);
  ...
  public ScriptContext getScriptContext(
    ...
    final ScriptContext context =
        wizardService.createScriptContext(state, page, control, attributes);
  ...
  @Override
  public boolean checkDataUniqueness(
      String xpath, ImmutableCollection<String> values, boolean canAccept) {
