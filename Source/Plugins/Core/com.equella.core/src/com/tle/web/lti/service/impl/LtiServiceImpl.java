...
public class LtiServiceImpl implements LtiService {
  ...
  public void sendGrade(LtiData ltiData, String grade) {
    ...
    if (outcomeServiceUrl != null) {
      try {
        ...
        final Request req =
            createLtiOutcomesRequest(
                buildEnvelope(lisData, grade),
                outcomeServiceUrl,
                ltiData.getOAuthData().getConsumerKey(),
  ...
  private void parseError(Response resp, String url) throws Exception {
    ...
    if (!httpService.isError(resp)) {
      ...
      final Node statusInfoNode =
          xml.node(
              "/imsx_POXEnvelopeResponse/imsx_POXHeader/imsx_POXResponseHeaderInfo/imsx_statusInfo");
      ...
      if (!codeMajor.equals("success")) {
        throw new RuntimeException(
            "Error returned from gradebook (" + codeMajor + ") " + description);
      }
    } else {
  ...
  private Request createLtiOutcomesRequest(
      ImsxPOXEnvelopeType imsxEnvelope, String url, String consumerKey, String consumerSecret)
      throws Exception {
    ...
    final JAXBElement<ImsxPOXEnvelopeType> createImsxPOXEnvelopeRequest =
        of.createImsxPOXEnvelopeRequest(imsxEnvelope);

    final JAXBContext jc =
        JAXBContext.newInstance("com.tle.web.lti.imsx", LtiServiceImpl.class.getClassLoader());
  ...
  private OAuthMessage createLaunchParameters(
      String consumerKey, String secret, String url, String bodyHash) {
