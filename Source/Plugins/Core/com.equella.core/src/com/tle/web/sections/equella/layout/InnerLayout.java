...
/**
 * The reality is, there is only one inner layout. The dialog inner layout is set using the
 * Decorations.layoutSelectors TODO: it would be nice to roll these into the same thing
 *
 * @author aholland
 */
public class InnerLayout implements LayoutSelector {
  ...
  public static final InnerLayout STANDARD =
      new InnerLayout("layouts/inner/standard.ftl"); // $NON-NLS-1$
  public static final InnerLayout DIALOG =
      new InnerLayout("layouts/inner/dialog.ftl"); // $NON-NLS-1$
  ...
  public TemplateResult getLayout(
      Decorations decorations, RenderContext info, TemplateResult templateResult) throws Exception {
    return null;
  }
