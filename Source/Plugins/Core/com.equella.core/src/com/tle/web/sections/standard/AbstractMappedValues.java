...
public abstract class AbstractMappedValues<T> extends AbstractHtmlComponent<AbstractMappedValues.MappedValuesModel<T>>
	implements
	...
	public void bookmark(SectionInfo info, BookmarkEvent event)
	{
		if( addToThisBookmark(info, event) )
		{
			...
			for( Map.Entry<String, T> entry : model.getMap().entrySet() )
			{
				...
				{
					event.setParam(getParameterId() + '(' + keyEscape(key) + ')', convertBack(entry.getValue()));
				}
			}
		}
	}
