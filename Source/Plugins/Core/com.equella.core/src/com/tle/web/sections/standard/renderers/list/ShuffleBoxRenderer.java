...
public class ShuffleBoxRenderer extends AbstractElementRenderer
	implements
		JSListComponent,
		JSDisableable,
		RendererFactoryAware
{
	private static final PluginResourceHelper URL_HELPER = ResourcesService.getResourceHelper(ShuffleBoxRenderer.class);
	private static final IncludeFile SHUFFLE_JS = new IncludeFile(URL_HELPER.url("js/shufflebox.js"),
		SelectModule.INCLUDE);
	private static final String CSS_URL = URL_HELPER.url("css/shufflebox.css");

	private static final JSCallable SETUP_FUNC = new ExternallyDefinedFunction("setupShuffleBox", 4, SHUFFLE_JS);
	private static final JSCallable DISABLE_FUNC = new ExternallyDefinedFunction("disableShuffleBox", SHUFFLE_JS);
	...
	protected void writeMiddle(SectionWriter writer) throws IOException
	{
		...
		ArrayExpression buttons = new ArrayExpression(allRightButton, rightButton, leftButton, allLeftButton);

		if( changeHandler == null )
		{
			changeHandler = new FunctionCallStatement("updateButtons", dropDownLeft, dropDownRight, buttons);
		}

		writer.addReadyStatements(new FunctionCallStatement(SETUP_FUNC, dropDownLeft, dropDownRight, buttons,
