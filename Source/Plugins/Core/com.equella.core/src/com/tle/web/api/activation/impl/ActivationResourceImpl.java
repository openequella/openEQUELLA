...
public class ActivationResourceImpl implements ActivationResource {
  ...
  private final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(ActivationResourceImpl.class);

  @Inject private CourseInfoService courseInfoService;
  @Inject private ActivationService activationService;
  @Inject private ActivationSerializer activationSerializer;
  @Inject private ItemService itemService;
  @Inject private UrlLinkService urlLinkService;
  @Inject private TLEAclManager aclService;

  /**
   * Retrieve all activations, or if ...?course={courseUuid} param appended, retrieve just those
   * activations associated with the specified course.
   *
   * @param courseUuid
   * @return
   */
    ...
    if (!Strings.isNullOrEmpty(courseUuid)) {
      ...
      if (course == null) {
        throw new NotFoundException(
            resources.getString("activation.notfound.course.uuid", courseUuid));
      }
  ...
  public Response create(ActivationBean bean) {
    ...
    final Item item =
        itemService
            .getItemPack(new ItemId(bean.getItem().getUuid(), bean.getItem().getVersion()))
            .getItem();
    final CourseInfo course = findCourseFromBean(bean);
    final ActivateRequest activateRequest =
        composeActivateRequest(new ActivateRequest(), item, course, bean);
  ...
  public Response edit(String requestUuid, boolean disable, ActivationBean bean) {
    ...
    if (disable
        && (oldStatus == ActivateRequest.TYPE_ACTIVE
            || oldStatus == ActivateRequest.TYPE_PENDING)) {
  ...
  private ActivationBean serialize(ActivateRequest request) {
    ...
    Map<String, String> links =
        Collections.singletonMap("self", getSelfLink(activationBean.getUuid()).toString());
  ...
  private void validateBean(ActivationBean bean) {
    ...
    if (Check.isEmpty(type)
        || !(ACTIVATION_TYPE_CLA.equals(type) || ACTIVATION_TYPE_CAL.equals(type))) {
      ...
      try {
        requests =
            activationService.activate(
                type, activateRequest.getItem(), Collections.singletonList(activateRequest), false);
      } catch (CopyrightViolationException we) {
        if (we.isCALBookPercentageException()) {
          if (aclService
              .filterNonGrantedPrivileges(ActivationConstants.COPYRIGHT_OVERRIDE)
              .isEmpty()) {
            throw new AccessDeniedException(
                resources.getString("activation.access.denied.percent"));
          }
          if (Check.isEmpty(activateRequest.getOverrideReason())) {
            throw new AccessDeniedException(
                resources.getString("activation.access.denied.message"));
          }
          requests =
              activationService.activate(
                  type,
                  activateRequest.getItem(),
                  Collections.singletonList(activateRequest),
                  true);
        } else {
  ...
  private ActivateRequest composeActivateRequest(
      ActivateRequest activation, Item item, CourseInfo course, ActivationBean bean) {
    ...
    activation.setCitation(
        !Check.isEmpty(bean.getCitation()) ? bean.getCitation() : course.getCitation());
  ...
  private CourseInfo findCourseFromBean(ActivationBean bean) {
    CourseInfo course =
        bean.getCourse().getUuid() != null
            ? courseInfoService.getByUuid(bean.getCourse().getUuid())
            : (bean.getCourse().getCode() != null
                ? courseInfoService.getByCode(bean.getCourse().getCode())
                : null);
    if (course == null) {
