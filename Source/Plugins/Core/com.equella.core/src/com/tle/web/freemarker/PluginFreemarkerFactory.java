...
@NonNullByDefault
public class PluginFreemarkerFactory extends AbstractFreemarkerFactory
    implements FreemarkerFactory {
  private String name = "";
  private URL rootUrl;
  ...
  public void registerFactory() {
    ((CustomTemplateLoader) configuration.getTemplateLoader())
        .addFactory(pluginId + '@' + name, this);
  ...
  protected void addRootObjects(
    ...
    if (writer instanceof SectionWriter) {
      ...
      map.put("p", ResourcesService.getResourceHelper(pluginId));
    }
  }
  ...
  @Override
  public SectionRenderable createResult(
      String template, SectionId sectionId, PreRenderable preRenderer) {
  ...
  public TemplateResult createTemplateResult(String template, SectionId sectionId) {
    return new FreemarkerTemplateResult(
        this, (FreemarkerSectionResult) createResult(template, sectionId));
  }
  ...
  @Override
  protected void finishedRender(
      Writer writer, FreemarkerSectionResult result, Environment environment) {
    if (writer instanceof SectionWriter) {
      SectionWriter sWriter = (SectionWriter) writer;
      try {
        ...
        if (var != null) {
          sWriter
              .getBody()
              .addEventStatements(JSHandler.EVENT_LOAD, new ScriptStatement(var.toString()));
        }
