...
public class FancyBoxDialogRenderer extends AbstractComponentRenderer implements DialogRenderer {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(JQueryFancyBox.class);
  private static final PreRenderable INCLUDE = new IncludeFile(resources.url("js/fancybox.js"));

  public static final JSCallable RESIZE =
      new ExternallyDefinedFunction("$.fancybox.resize", 0, JQueryFancyBox.PRERENDER);
  ...
  public FancyBoxDialogRenderer(DialogState state) {
    ...
    JSFunction openCallback =
        !DebugSettings.isAutoTestMode()
            ? Js.function(Js.call_s(FOCUS, state.getDialogOpenedCallback()))
            : state.getDialogOpenedCallback();
    ...
    if (diagState.isAjax()) {
      ...
      return new JSCallable() {
        ...
        public String getExpressionForCall(RenderContext info, JSExpression... params) {
          ...
          newparams[4] =
              new AnonymousFunction(JQueryFancyBox.FANCYBOX_STATIC, (Object[]) fancyOptions);
        ...
        public void preRender(PreRenderContext info) {
          info.preRender(
              AjaxGenerator.OPEN_AJAX_DIALOG,
              JQueryFancyBox.FANCYBOX_STATIC,
  ...
  public void setupOpener(HtmlLinkState opener) {
    ...
    if (clickHandler != null) {
      clickHandler =
          new StatementHandler(clickHandler, new FunctionCallStatement(createOpenFunction())) {
            @Override
