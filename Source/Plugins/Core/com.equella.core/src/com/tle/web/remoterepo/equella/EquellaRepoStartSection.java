...
public class EquellaRepoStartSection
    extends AbstractPrototypeSection<EquellaRepoStartSection.EquellaRepoSelectionSessionModel>
    implements HtmlRenderer, CourseListVetoSection {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(EquellaRepoStartSection.class);
  ...
  @TreeLookup private EquellaRepoDownloadProgressSection downloadSection;

  @Inject @Component private EquellaRepoSessionDialog selectionDialog;
  @Component private SingleSelectionList<NameValue> downloadOptions;
  @Component private Button startDownloadButton;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    options.add(
        new BundleNameValue(
            resources.key("start.downloadoption.attachments"), OPTION_ATTACHMENTS, bundleCache));
    options.add(
        new BundleNameValue(
            resources.key("start.downloadoption.metadata"), OPTION_METADATA, bundleCache));
    ...
    selectionsMadeFunction =
        CallAndReferenceFunction.get(
            events.getSubmitValuesFunction("selectionMade"), selectionDialog);
  }
  ...
  public void startDownload(SectionInfo info) {
    ...
    if (selection != null) {
      ...
      downloadSection.setDownloadOptions(
          info, selection.getUuid(), selection.getVersion(), downloadAttachments);
    }
  }
