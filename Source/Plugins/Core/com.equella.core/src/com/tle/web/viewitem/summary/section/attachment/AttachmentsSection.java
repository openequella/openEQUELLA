...
public class AttachmentsSection extends AbstractAttachmentsSection<Item, AttachmentsSection.Model>
  ...
  private static final JSCallable SETUP_REORDER =
  ...
  protected void customRender(
    ...
    if (!tleACLManager
            .filterNonGrantedPrivileges(item, Collections.singleton(SecurityConstants.EDIT_ITEM))
            .isEmpty()
      ...
      reorderAttachments.setClickHandler(
          ...
          new OverrideHandler(
              ...
              ajax.getAjaxFunction("attachmentsReordered"),
              MODAL_CLOSE_WARNING_LABEL.getText(),
              MODAL_SAVE_LABEL.getText(),
              MODAL_CANCEL_LABEL.getText(),
              isShowStructuredView()));
      reorderAttachments.setLabel(
          context, new IconLabel(Icon.MOVE, reorderAttachments.getLabel(context), false));
    } else {
    ...
    if (!movedItems.isEmpty()) {
      ...
      itemService.operation(
          ...
          new WorkflowOperation[] {
            itemOperationFactory.editMetadata(itemService.getItemPack(itemKey)),
            itemOperationFactory.saveNoSaveScript(true)
          });
    }
  }

  private boolean attachmentStructureReorderable(
      List<AttachmentRowDisplay> attachmentDisplays, Item item) {
    return attachmentDisplays.size() > 1
        && isFlatHierachy(attachmentDisplays)
        && (item.getTreeNodes() == null || item.getTreeNodes().isEmpty())
        && !containsHiddenZip(item);
  }
  ...
  protected Bookmark getFullscreenBookmark(SectionInfo info, ViewableItem<Item> vitem) {
    return urlFactory.createItemUrl(
        info,
        vitem,
  ...
  /** Only render the fullscreen link if there are more than 0 enabled attachments. */
  @Override
  ...
  protected AttachmentViewFilter getCustomFilter(
      SectionInfo info, ViewableItem<Item> vitem, boolean filtered) {
    return new AttachmentViewFilter() {
      ...
      public boolean shouldBeDisplayed(SectionInfo info, AttachmentView attachmentView) {
        ...
        if (!filtered
            && attachmentView.getAttachment().getAttachmentType() == AttachmentType.IMSRES) {
          return true;
        }
