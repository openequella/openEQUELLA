...
public class ChoiceInteractionRenderer extends BlockInteractionRenderer {
  ...
  @AssistedInject
  public ChoiceInteractionRenderer(
      @Assisted ChoiceInteraction model, @Assisted QtiViewerContext context) {
    super(model, context);
    this.model = model;
  }
  ...
  public void preProcess() {
    ...
    for (SimpleChoice choice : model.getSimpleChoices()) {
      final SimpleChoiceRenderer simpleChoiceRenderer =
          qfac.getRendererFactory()
              .createSimpleChoiceRenderer(
                  choice,
                  getContext(),
                  QtiViewerConstants.CONTROL_PREFIX + id(responseId),
