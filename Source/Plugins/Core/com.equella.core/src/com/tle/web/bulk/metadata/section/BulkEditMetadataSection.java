...
public class BulkEditMetadataSection extends AbstractPrototypeSection<BulkEditMetadataModel>
  ...
  private static final String AJAX_DIV = "metadata-edit";
  private static final String BUTTON_FOOTER_DIV = "bss_bulkDialogfooter";
  ...
  private void hideButtons(RenderEventContext context) {
    ...
    if (model.isSchemaSelection()) {
      chooseActionButton.setClickHandler(
          context,
          new StatementHandler(
                  getAjaxUpate(getTree(), "chooseAction", false, AJAX_DIV, BUTTON_FOOTER_DIV))
              .addValidator(
                  pathValues
                      .createNotBlankValidator()
                      .setFailureStatements(Js.alert_s(BLANK_TERMS_LABEL))));
    }
  }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    UpdateDomFunction delete =
        getAjaxUpate(tree, "deleteModification", false, AJAX_DIV, BUTTON_FOOTER_DIV);
    UpdateDomFunction edit =
        getAjaxUpate(tree, "editModification", false, AJAX_DIV, BUTTON_FOOTER_DIV);
    ...
    addModification.setClickHandler(
        new StatementHandler(getAjaxUpate(tree, "addMod", false, AJAX_DIV, BUTTON_FOOTER_DIV)));
    editNodesButton.setClickHandler(
        new StatementHandler(getAjaxUpate(tree, "addMod", false, AJAX_DIV, BUTTON_FOOTER_DIV)));
    editNodesButton.setComponentAttribute(ButtonType.class, ButtonType.EDIT);

    schemaList.setListModel(new SchemaListModel());
    StatementHandler treeUpdate =
        new StatementHandler(getAjaxUpate(tree, "schemaChange", true, AJAX_DIV));
    ...
    addNodeFunc =
        new SimpleFunction(
            ...
            new FunctionCallStatement(
                getAjaxUpate(tree, "addNode", false, "selected-nodes"), nodeVar),
            nodeVar);

    chooseActionButton.setLabel(new IconLabel(Icon.COG, CHOOSE_ACTION_LABEL));

    StatementHandler actionUpdate =
        new StatementHandler(
    ...
    actionList.setListModel(
        new EnumListModel<ActionTypes>(KEY_ACTIONS_PFX, true, ActionTypes.values()));

    SimpleHtmlListModel<NameValue> setTextOptionsList =
        new SimpleHtmlListModel<NameValue>(
            new BundleNameValue(SET_OPTION_ALWAYS, "always"),
            new BundleNameValue(SET_OPTION_EXISTS, "exists"),
    ...
    modificationReturnButton.setClickHandler(
        new StatementHandler(getAjaxUpate(tree, "modReturn", false, AJAX_DIV, BUTTON_FOOTER_DIV)));
    modificationReturnButton.setComponentAttribute(ButtonType.class, ButtonType.PREV);

    pathValues.setEventHandler(
  ...
  private void setUpValidators(SectionInfo context, ActionTypes selection) {
    if (selection.equals(ActionTypes.SET)) {
      saveActionButton.setClickHandler(
          context,
          new StatementHandler(
              getAjaxUpate(getTree(), "saveAction", true, AJAX_DIV, BUTTON_FOOTER_DIV)));
      ...
      saveActionButton.setClickHandler(
          ...
          new StatementHandler(
              ...
              .addValidator(
                  findTextField
                      .createNotBlankValidator()
                      .setFailureStatements(Js.alert_s(BLANK_FIND_LABEL))));
      ...
      saveActionButton.setClickHandler(
          ...
          new StatementHandler(
              ...
              .addValidator(
                  addXMLTextArea
                      .createNotBlankValidator()
                      .setFailureStatements(Js.alert_s(BLANK_XML_LABEL))));
    }
  }

  private UpdateDomFunction getAjaxUpate(
      SectionTree tree, String eventHandlerName, boolean useLoad, String... ajaxIds) {
    ...
    return ajax.getAjaxUpdateDomFunction(
        tree, this, events.getEventHandler(eventHandlerName), effectFunction, ajaxIds);
  }
  ...
  public void chooseAction(SectionInfo info) {
    ...
    // reset
    if (actionList.getSelectedValue(info) == null
        || actionList.getSelectedValue(info).equals(ActionTypes.NONE)) {
  ...
  public class SchemaListModel extends DynamicHtmlListModel<Schema> {
    ...
    protected Option<Schema> convertToOption(SectionInfo info, Schema schema) {
      return new NameValueOption<Schema>(
          new BundleNameValue(schema.getName(), String.valueOf(schema.getId()), bundleCache),
          schema);
    }
  ...
  public class SchemaTreeModel implements HtmlTreeModel {
    ...
    public List<HtmlTreeNode> getChildNodes(SectionInfo info, String xpath) {
      ...
      for (PropBagEx child : schemaXml.iterator(xpath + "/*")) {
        ...
        String fullpath =
            Check.isEmpty(xpath) ? name : MessageFormat.format("{0}/{1}", xpath, name);
  ...
  public class SchemaTreeNode implements HtmlTreeNode {
    ...
    public SectionRenderable getRenderer() {
      DivRenderer nodeDiv;
      if (!selected) {
        HtmlComponentState selectLink =
            new HtmlComponentState(new OverrideHandler(addNodeFunc, id));
        ...
        HtmlComponentState deselectLink =
            new HtmlComponentState(
                new OverrideHandler(
                    getAjaxUpate(getTree(), "removeNode", false, "selected-nodes"), id));
        deselectLink.setLabel(REMOVE_NODE_LABEL);
        deselectLink.addClass("unselect");
        nodeDiv =
            new DivRenderer(
                new CombinedRenderer(
                    new LabelRenderer(getLabel()), new LinkRenderer(deselectLink)));
  ...
  private class ModificationTableModel extends DynamicSelectionsTableModel<Modification> {
    ...
    public ModificationTableModel(
        UpdateDomFunction moveFunction,
        UpdateDomFunction deleteFunction,
    ...
    protected void transform(
      ...
      actions.add(
          makeAction(
              DELETE_MOD_LABEL,
              new OverrideHandler(deleteFunction, index)
                  .addValidator(new Confirm(ACTION_DELTE_CONFIRM))));

      final HtmlLinkState upLinkState =
          new HtmlLinkState(new OverrideHandler(moveFunction, index, true));
      ...
      final HtmlLinkState downLinkState =
          new HtmlLinkState(new OverrideHandler(moveFunction, index, false));
  ...
  public void editModification(SectionInfo info, int index) {
    ...
    if (action.equalsIgnoreCase(ActionTypes.REPLACE.toString().toLowerCase())) {
      ...
      setTextOptions.setSelectedStringValue(
          info, actionOptions.get(ModificationKeys.SET_TEXT_OPTION));
      model.setActionSet();
