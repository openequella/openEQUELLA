...
public class ShowSelectionHomePortalsSection extends AbstractPrototypeSection<Object>
  ...
  private synchronized Pair<List<SectionId>, List<SectionId>> getPortalLayout(
    ...
    if (portletLayout == null) {
      ...
      for (SectionId childId : childIds) {
        ...
        try {
          ...
          if (json.get("column").equals("dynamic")) // $NON-NLS-1$//$NON-NLS-2$
          {
            ...
            (json.get("column").equals("left") ? left : right)
                .add(data); // $NON-NLS-1$ //$NON-NLS-2$
          }
        } catch (RuntimeException ex) {
          throw new SectionsRuntimeException(
              "The layout property for "
                  + childId
                  + " is missing or incorrect.  Must be in JSON format with properties"
                  + " \"{order: 1234, column: 'left'|'right'}\", but it was \""
                  + layout
                  + "\"",
              ex);
        }
      ...
      portletLayout =
          new Pair<List<SectionId>, List<SectionId>>(
              Lists.transform(left, DATA_TO_SECTION_ID),
  ...
  private static final Function<LayoutData, SectionId> DATA_TO_SECTION_ID =
      new Function<LayoutData, SectionId>() {
        @Override
