...
public class SoapInterfaceV2Impl extends AbstractSoapService
    implements com.tle.core.remoting.SoapInterfaceV2 {
  ...
  public String acceptTask(
      String ssid, String itemUuid, int itemVersion, String taskId, boolean unlock)
      throws Exception {
    authenticate(ssid);
  ...
  private String acceptTask(String ssid, ItemTaskId itemTaskId, boolean unlock) throws Exception {
    ...
    itemService.operation(
        itemTaskId,
        operationFactory.accept(itemTaskId.getTaskId(), null, null),
        operationFactory.status(),
        operationFactory.saveUnlock(unlock));
  ...
  public String rejectTask(
    ...
    authenticate(ssid);
    return rejectTask(
        ssid, new ItemTaskId(itemUuid, itemVersion, taskId), rejectMessage, toStep, unlock);
  }

  private String rejectTask(
      String ssid, ItemTaskId itemTaskId, String rejectMessage, String toStep, boolean unlock)
      throws Exception {
    authenticate(ssid);
    itemService.operation(
        itemTaskId,
        operationFactory.reject(itemTaskId.getTaskId(), rejectMessage, toStep, null),
        operationFactory.status(),
        operationFactory.saveUnlock(unlock));
    return itemTaskId.getTaskId();
  }
