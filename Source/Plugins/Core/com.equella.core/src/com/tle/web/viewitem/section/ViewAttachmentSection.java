...
public class ViewAttachmentSection
    extends AbstractPrototypeSection<ViewAttachmentSection.ViewAttachmentModel>
    implements ViewAttachmentInterface, ViewItemViewer, ViewItemFilter {
  ...
  public IAttachment getAttachment(SectionInfo info, ViewItemResource resource) {
    ...
    if (attachUuid != null) {
      ...
      if (attachment == null) {
        SectionUtils.throwRuntime(
            new IllegalArgumentException("Attachment " + attachUuid + " could not be found."));
      }
      return attachment;
    }
    return null;
  ...
  public SectionResult view(RenderContext info, ViewItemResource resource) {
    ...
    contentStreamWriter.outputStream(
        info.getRequest(), info.getResponse(), resource.getContentStream());
    return null;
  }
  ...
  public static class ViewAttachmentModel {
    ...
    @Bookmarked private boolean stream;
    ...
    public ViewItemResource filter(SectionInfo info, ViewItemResource resource) {
      ...
      if (Check.isEmpty(viewerId)) {
        IAttachment attachment =
            resource.getViewableItem().getAttachmentByFilepath(resource.getFilepath());
        if (attachment != null) {
          ViewableResource viewableResource =
              attachmentResourceService.getViewableResource(
                  info, resource.getViewableItem(), attachment);
  ...
  public class ViewAttachmentViewItemResource extends AbstractAttachmentViewItemResource {
    public ViewAttachmentViewItemResource(
