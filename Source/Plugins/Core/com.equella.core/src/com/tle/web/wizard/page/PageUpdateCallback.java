...
public abstract class PageUpdateCallback implements JSONResponseCallback {
  private static final IncludeFile INCJS =
      new IncludeFile(
          ResourcesService.getResourceHelper(PageUpdateCallback.class)
              .url("scripts/updatecontrols.js"),
          AjaxEffects.EFFECTS_LIB,
          JQueryUIEffects.HIGHLIGHT,
          JQueryUIEffects.BLIND);
  private static final JSCallable UPDATE_CONTROLS =
      new ExternallyDefinedFunction("updateControls", 4, INCJS);
  ...
  public Object getResponseObject(AjaxRenderContext context) {
    ...
    return new UpdateControlsObj(
        fullDOMResult, html, htmlMap, visibleControls, fullDOMResult.getLists(), getHiddenState());
  }
  ...
  public static class UpdateControlsObj extends AbstractDOMResult {
    ...
    public UpdateControlsObj(
        AbstractDOMResult result,
        Map<String, AjaxCaptureResult> contents,
        Map<String, FullAjaxCaptureResult> updates,
        Map<String, List<String>> controlIds,
        Map<String, List<FullAjaxCaptureResult>> listUpdates,
        Map<String, String> hiddenState) {
      super(result);
