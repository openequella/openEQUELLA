...
public class DirListViewer extends AbstractPrototypeSection<Object> implements ViewItemViewer {
  ...
  @Inject private ItemXsltService itemXsltService;
  ...
  public void treeFinished(String id, SectionTree tree) {
    rootSection.addViewerMapping(
        com.tle.web.viewitem.section.PathMapper.Type.FILENAME,
        this,
        LIST_NAME,
  ...
  private void viewAttachmentList(
      ViewItemResource resource, FileHandle fileHandle, HttpServletResponse response)
      throws IOException {
    response.setContentType("text/plain");

    try (PrintWriter writer = new PrintWriter(new PrintStream(response.getOutputStream()))) {
      final Collection<String> list =
          fileSystemService.grep(fileHandle, resource.getFileDirectoryPath(), "**");
  ...
  private void viewDirList(
    ...
    try (PrintWriter writer = new PrintWriter(new PrintStream(response.getOutputStream()))) {
      ...
      writer.print(
          CurrentLocale.get(
              "viewitem.section.dirlistsection.listing",
  ...
  private void viewXml(RenderContext info, HttpServletResponse response) throws IOException {
    ...
    outputStream.println(
        itemXsltService
            .getXmlForXslt(info, AbstractParentViewItemSection.getItemInfo(info))
            .toString());
    outputStream.flush();
    outputStream.close();
  }
