...
public abstract class RemoteRepoResultsSection<
        E extends RemoteRepoSearchEvent<E>,
        R extends RemoteRepoSearchResult,
        M extends RemoteRepoResultsSection.RemoteRepoResultsModel>
    extends AbstractSearchResultsSection<
        RemoteRepoListEntry<R>, E, RemoteRepoSearchResultEvent<R>, M> {
  @TreeLookup
  private RemoteRepoListItemViewHandlerCreator<RemoteRepoListEntry<R>> viewResultCreator;

  @TreeLookup private AbstractRootRemoteRepoSection rootRemoteRepoSection;

  @Inject private RemoteRepoListSection<R> resultList;
  ...
  protected void setupTitle(SectionInfo info) {
    Decorations.getDecorations(info)
        .setTitle(new BundleLabel(repoWebService.getRemoteRepository(info).getName(), bundleCache));
  }
  ...
  public void processResults(SectionInfo info, RemoteRepoSearchResultEvent<R> results) {
    for (R result : results.getResults().getResults()) {
      resultList.addListItem(
          info, populateListItem(info, getEntryFactory().createListEntry(info, result)));
    }
  }

  @Override
  protected boolean showResults(
      RenderContext context, E searchEvent, RemoteRepoSearchResultEvent<R> resultsEvent) {
  ...
  protected RemoteRepoListEntry<R> populateListItem(
      SectionInfo info, RemoteRepoListEntry<R> listItem) {
  ...
  public static class RemoteRepoResultsModel
      extends AbstractSearchResultsSection.SearchResultsModel {
    @Bookmarked private boolean search;
