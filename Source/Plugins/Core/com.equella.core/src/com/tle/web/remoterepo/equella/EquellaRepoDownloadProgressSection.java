...
public class EquellaRepoDownloadProgressSection
    extends AbstractPrototypeSection<
        EquellaRepoDownloadProgressSection.EquellaRepoDownloadProgressModel>
    implements HtmlRenderer, CourseListVetoSection {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(EquellaRepoDownloadProgressSection.class);

  private static final ExternallyDefinedFunction SHOW_PROGRESS =
      new ExternallyDefinedFunction(
          "showProgress",
          new IncludeFile(resources.url("scripts/download.js"), JQueryProgression.PRERENDER));

  @Inject private EquellaRepoService equellaRepoService;
  ...
  @Component private Div progress;

  @Override
  public SectionResult renderHtml(RenderEventContext context) {
    final EquellaRepoDownloadProgressModel model = getModel(context);
    progress.addReadyStatements(
        context,
        new FunctionCallStatement(
            SHOW_PROGRESS, resources.instUrl("progress/?id=" + model.getSessionId())));
  ...
  public void startDownload(SectionInfo info) {
    ...
    if (model.getUuid() != null) {
      ...
      if (model.isAttachments() && model.getSessionId() == null) {
        ...
        sessionId =
            equellaRepoService.downloadAttachments(
                settings, model.getUuid(), model.getVersion(), callback);
        model.setSessionId(sessionId);

        callback.setForwardUrl(
            new BookmarkAndModify(info, events.getNamedModifier("finished")).getHref());
  ...
  public void finished(SectionInfo info) {
    ...
    if (session.isFinished()) {
      ...
      try {
        ...
        settings.load(search);
        final PropBagEx xml =
            equellaRepoService.getItemXml(
                settings, model.getUuid(), model.getVersion(), !model.isAttachments());
  ...
  public static class EquellaRepoDownloadProgressModel {
    @Bookmarked private boolean attachments;
    @Bookmarked private String uuid;
    @Bookmarked private int version;
    @Bookmarked private String sessionId;
