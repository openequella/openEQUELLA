...
public class BoxRenderer extends TagRenderer {
  private static final Set<String> INTERNAL_EVENTS =
      new HashSet<String>(Arrays.asList("edit", "close", "minimise"));

  private static final PluginResourceHelper RESOURCES =
      ResourcesService.getResourceHelper(BoxRenderer.class);

  private static final PreRenderable INCLUDE =
      new IncludeFile(RESOURCES.url("scripts/component/boxrenderer.js"), JQueryUIEffects.BLIND);
  public static final ExternallyDefinedFunction MIN_MAX_EFFECT =
      new ExternallyDefinedFunction("toggleMinimise", INCLUDE);
  private static final CssInclude CSS =
      include(RESOURCES.url("css/component/box.css")).hasRtl().make();

  private static final Label EXPAND_LABEL = new KeyLabel(RESOURCES.key("component.box.expand"));
  private static final String EXPAND_LTR_IMAGE_URL =
      RESOURCES.url("images/component/box_head_closed.ltr.gif");
  private static final String EXPAND_RTL_IMAGE_URL =
      RESOURCES.url("images/component/box_head_closed.rtl.gif");

  private static final Label CONTRACT_LABEL = new KeyLabel(RESOURCES.key("component.box.contract"));
  private static final String CONTRACT_IMAGE_URL =
      RESOURCES.url("images/component/box_head_open.gif");
    ...
    if (minimise == null) {
      ...
      if (minState != null) {
        ...
        String url =
            !isMin
                ? CONTRACT_IMAGE_URL
                : CurrentLocale.isRightToLeft() ? EXPAND_RTL_IMAGE_URL : EXPAND_LTR_IMAGE_URL;
