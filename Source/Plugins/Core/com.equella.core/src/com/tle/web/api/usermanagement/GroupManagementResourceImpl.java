...
public class GroupManagementResourceImpl implements EquellaGroupResource {
  ...
  /**
   * QueryString q will be fitted with leading and trailing wildcard and matched against Group name.
   * <br>
   * An optional query param is userId, in which case the include all parents flag is referred to in
   * compiling results. If queried on both name and user, and allParents flag is true, the result
   * set may include parent groups which themselves may lie outside the query filter. If queried on
   * both name and user, explicit setting of allparents to false will confine results to groups
   * which match the query filter and directly contain the nominated user.
   *
   * @param q
   * @param userId
   ...
   */
  @Override
  public SearchBean<GroupBean> list(
      UriInfo uriInfo, String q, String userId, Boolean allParents, String name) {
    ensurePriv();
    ...
    for (String memberId : members) {
      ...
      Map<String, String> links =
          Collections.singletonMap(
              "self",
              urlLinkService
                  .getMethodUriBuilder(EquellaUserResource.class, "getUser")
                  .build(uuid)
                  .toString());
  ...
  public Response deleteGroup(
      // @formatter:off
      @ApiParam(APIDOC_GROUPUUID) @PathParam("uuid") String uuid,
      @ApiParam(
              value = "also delete subgroups",
