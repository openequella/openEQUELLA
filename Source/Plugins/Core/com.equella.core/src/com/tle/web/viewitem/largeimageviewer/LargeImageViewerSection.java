...
public class LargeImageViewerSection
  ...
  @Component private Button startButton;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    JSCallable function =
        new ExternallyDefinedFunction(
            "initializeGraphic", JQueryDraggable.PRERENDER, JQueryDroppable.PRERENDER);
    title.addReadyStatements(
        new ScriptStatement(
            new FunctionCallExpression(
                function,
                new ElementIdExpression(zoomIn),
                new ElementIdExpression(zoomOut),
                new ElementIdExpression(rotateLeft),
                new ElementIdExpression(rotateRight),
                new ElementIdExpression(hideThumb),
                CurrentLocale.get(HIDE_THUMB_KEY),
                CurrentLocale.get(SHOW_THUMB_KEY))));
  }
  ...
  public ViewAuditEntry getAuditEntry(SectionInfo info, ViewItemResource resource) {
    ...
    if (!Objects.equals(model.getMethod(), "tile")) {
      FileHandle tileBaseHandle =
          largeImageViewer.getTileBaseHandle(resource.getViewableItem(), resource.getFilepath());
  ...
  public SectionResult view(RenderContext info, ViewItemResource resource) {
    ...
    final FileHandle tileBaseHandle =
        largeImageViewer.getTileBaseHandle(viewableItem, resource.getFilepath());
    ...
    if (tiledImageProps != null) {
      ...
      model.setTileBaseUri(
          URLUtils.urlEncode(
              viewableItem.getItemdir()
                  + largeImageViewer.getTileBasePath(resource.getFilepath())
                  + '/',
              false));
      ...
      title.setLabel(
          info,
          new IconLabel(Icon.BACK, new BundleLabel(item.getName(), item.getUuid(), bundleCache)));
      title.setBookmark(
          info,
          itemUrls.createItemUrl(
              info,
              resource.getViewableItem().getItem().getItemId(),
    ...
    if (!confirmStart) {
      ...
      JQueryCore.appendReady(
          ...
          new FunctionCallStatement(
              StandardModule.SET_TIMEOUT, new ReloadFunction(false), RELOAD_INTERVAL));
    }
  ...
  public SectionResult tile(SectionInfo info, ViewItemResource resource) {
    ...
    try {
      ...
      File rotatedFile =
          fileSystemService.getExternalFile(resource.getViewableItem().getFileHandle(), filepath);
  ...
  private Properties getTiledImageProperties(SectionInfo info, FileHandle tileBaseHandle) {
    ...
    if (tiledImageProperties == null) {
      tiledImageProperties =
          largeImageViewer.getTileProperties(
              fileSystemService.getExternalFile(tileBaseHandle, null));
  ...
  private void startTileProcessor(ViewItemResource resource, FileHandle tileBaseHandle) {
    ...
    largeImageViewer.startTileProcessor(
        Collections.singleton(new Pair<File, File>(originalImage, destFolder)));
  }
  ...
  public static class LargeImageViewerModel {
    @Bookmarked private String method;
