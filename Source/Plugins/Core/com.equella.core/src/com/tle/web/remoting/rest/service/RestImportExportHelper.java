...
public abstract class RestImportExportHelper {
  ...
  /**
   * Check the query string to see if the import/export flag is present, and if so, does it parse as
   * true?
   *
   * @param uriInfo
   * @param isImportOp Are we looking for an import operation, or export?
   * @return true if sought for directive is both present, and parses as true. Otherwise false.
   * @throws AccessDeniedException if directive is present and true, but user does not have System
   *     credentials.
   */
  private static boolean verifyAccessIfParamFound(UriInfo uriInfo, boolean isImportOp)
