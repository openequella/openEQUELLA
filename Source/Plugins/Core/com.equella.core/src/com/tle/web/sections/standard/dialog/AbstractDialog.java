 ...
 *
 * <p>A dialog has the following properties:
 *
 * <ul>
 *   <li>Contents
 *   <li>An open and close javascript function
 *   <li>An optional "opener" link/button
 * </ul>
 *
 * It can be rendered as an inline DIV, ajax loaded inline DIV and IFrame
 *
 * <p>Details such as width and height of the dialog is stored here, but the renderer can override
 * those sizes.
 *
 * @see DialogRenderer
...
public abstract class AbstractDialog<S extends DialogModel> extends AbstractRenderedComponent<S>
    implements Dialog, ReadyToRespondListener {
  private final List<JSCallAndReference> parentCalls = new ArrayList<JSCallAndReference>();

  @Nullable private JSCallable openFunction;
  @Nullable private JSCallable closeFunction;
  @Nullable private DialogRenderer dialogRenderer;
  ...
  @Component private DialogLink opener;
  ...
  public void treeFinished(String id, SectionTree tree) {
    if (ajax) {
      ...
      showContents =
          ajaxEvents.getUpdateDomModifier(
              tree, this, AjaxGenerator.AJAXID_BODY, getAjaxShowEvent());
    } else {
  ...
  public void closeDialog(SectionInfo info, JSStatements... statements) {
    getModel(info)
        .setAfterCloseStatements(
            StatementBlock.get(
                new FunctionCallStatement(getCloseFunction()), StatementBlock.get(statements)));
  }
  ...
  public JSCallable getOpenFunction() {
    if (openFunction == null) {
      openFunction =
          new DelayedFunction<DialogRenderer>(
              this, "open", this, getNumberOfOpenParameters()) // $NON-NLS-1$
          {
            @Override
  ...
  public JSCallable getCloseFunction() {
    if (closeFunction == null) {
      closeFunction =
          new DelayedFunction<DialogRenderer>(this, "close", this, 1) // $NON-NLS-1$
          {
            @Override
            protected JSCallable createRealFunction(RenderContext info, DialogRenderer renderer) {
  ...
  public DialogRenderer getSelectedRenderer(RenderContext info) {
    ...
    if (diagRenderer == null) {
      throw new SectionsRuntimeException(
          "Section ID:" + displayDialog.getSectionId() + " didn't render the dialog");
    }
    return diagRenderer;
  }
    ...
    if (!redirect) {
      ...
      if (model.isShowing()) {
        ...
        if (ajax) {
          ...
          PrependedParameterFunction submit =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, true, true);
          PrependedParameterFunction submitNoBlock =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, false, false);
          PrependedParameterFunction submitEvent =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, true, true);
          PrependedParameterFunction submitNoValidation =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, false, true);
          PrependedParameterFunction submitEventNoValidation =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, false, true);
          PrependedParameterFunction submitEventNoBlock =
              new PrependedParameterFunction(
                  AjaxGenerator.SUBMIT_BODY, CurrentForm.EXPR, false, false);
