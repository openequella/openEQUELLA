...
public abstract class AbstractBulkApproveRejectSection
    extends AbstractPrototypeSection<AbstractBulkApproveRejectSection.BulkApproveRejectModel>
    implements HtmlRenderer, BulkOperationExtension {
  ...
  @TreeLookup private TaskResultsDialog taskResultsDialog;

  private static final PluginResourceHelper URL_HELPER =
      ResourcesService.getResourceHelper(AbstractBulkApproveRejectSection.class);
  private static final IncludeFile INCLUDE =
      new IncludeFile(URL_HELPER.url("scripts/bulkop.dnd.js"));
  private static final JSCallAndReference JS_CLASS =
      new ExternallyDefinedFunction("WorkflowBulkOpDnd", INCLUDE);
  private static final ExternallyDefinedFunction DONE_UPLOAD =
      new ExternallyDefinedFunction(JS_CLASS, "dndUploadFinishedCallback", 0);

  private static final ExternallyDefinedFunction VALIDATOR =
      new ExternallyDefinedFunction(JS_CLASS, "validateMessage", 2);
    ...
    getDialog()
        ...
        .addValidator(
            new FunctionCallValidator(
                VALIDATOR, Jq.$(commentField), LABEL_MANDATORY_MESSAGE.getText()));
