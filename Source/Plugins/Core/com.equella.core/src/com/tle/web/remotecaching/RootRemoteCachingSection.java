...
public class RootRemoteCachingSection
    extends OneColumnLayout<RootRemoteCachingSection.RemoteCachingModel> {
  ...
  private static final PluginResourceHelper urlHelper =
      ResourcesService.getResourceHelper(RootRemoteCachingSection.class);
  private static final IncludeFile INCLUDE =
      new IncludeFile(urlHelper.url("scripts/usrgrptree.js"));
  private static final ExternallyDefinedFunction SELECT =
      new ExternallyDefinedFunction("select", 3, INCLUDE);
  ...
  @Inject private RemoteCachingPrivilegeTreeProvider securityProvider;
  @Inject private BundleCache bundleCache;
  @Inject private RemoteCachingWebService remoteCachingWebService;
  ...
  public void registered(String id, SectionTree tree) {
    ...
    removeUserGroupButton.setClickHandler(
        Js.handler(Js.call_s(removeFunc)).addValidator(new Confirm(LABEL_CONFIRM_REMOVE)));
  ...
  private void saveQueries(
      SectionInfo info, MultiSelectionList<ItemDefinition> collections, List<Query> queries) {
    queries.clear();
  ...
  public class RemoteCachingTreeModel implements HtmlTreeModel {
    ...
    public List<HtmlTreeNode> getChildNodes(SectionInfo info, String id) {
      ...
      if (id == null) {
        ...
        final RemoteCachingTreeNode nctn =
            new RemoteCachingTreeNode(
                uuid,
                getText(info, node),
                node.getNodes().isEmpty(),
                model.getSelectedNode().equals(uuid),
                true);
        list.add(nctn);
      } else {
  ...
  public class RemoteCachingTreeNode implements HtmlTreeNode {
    ...
    public RemoteCachingTreeNode(
        String id, String textName, boolean leaf, boolean selected, boolean group) {
      this.id = id;
    ...
    /**
     * A little manip to ensure the link id's are unique. The Js call is to enable javascript to set
     * a class identifiable by the stylesheet as a highlight colour
     */
    @Override
    public SectionRenderable getRenderer() {
      final ImageRenderer icon =
          (group
              ? new ImageRenderer(URL_GROUP_ICON, LABEL_GROUP)
      ...
      linkRenderer.setNestedRenderable(
          CombinedRenderer.combineMultipleResults(
              icon, new LabelRenderer(new TextLabel(textName))));
      ...
      link.setClickHandler(
          new StatementHandler(
              Js.call_s(SELECT, Jq.$(usrGrpTree), Jq.$(divRenderer), selectNodeFunc, id)));
