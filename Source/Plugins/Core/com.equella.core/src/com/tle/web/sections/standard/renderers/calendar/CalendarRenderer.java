...
public class CalendarRenderer extends AbstractInputRenderer implements JSDisableable {
  ...
  private static String KEY_PFX = AbstractPluginService.getMyPluginId(CalendarRenderer.class) + ".";

  private final HtmlCalendarState state;
  private final AppendedElementId visibleElement;
  @Nullable private ElementId notBefore;
  @Nullable private ElementId notAfter;
  @Nullable private SimpleFunction disableFunc;
  ...
  public void preRender(PreRenderContext info) {
    ...
    info.addReadyStatements(
        JQueryDatepicker.setupPicker(
            Jq.$(getVisibleElementId()),
            Jq.$(this),
            tzOff,
            changeExpr,
            otherDateSelector,
            ...
            pickerFormat,
            time,
            CurrentLocale.get(KEY_PFX + "renderers.calendar.button")));
  ...
  public JSCallable createDisableFunction() {
    if (disableFunc == null) {
      return new PrependedParameterFunction(
          JQueryDatepicker.DISABLE_PICKER_FUNCTION, Jq.$(getVisibleElementId()));
    }
    return disableFunc;
  }
}
