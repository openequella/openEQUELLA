...
public class SelectNewPortletTypeSection
  ...
  public SectionResult renderHtml(RenderEventContext context) throws Exception {
    ...
    if (!CurrentUser.getUserState().isSystem()) {
      HelpAndScreenOptionsSection.addHelp(
          context, view.createResult("common/create/help.ftl", this));
  ...
  public SectionRenderable renderOptions(RenderContext context) {
    if (!portletWebService.canCreate() && !isAdmin()) {
      ...
      final List<Pair<HtmlLinkState, String>> portletTypes =
          new ArrayList<Pair<HtmlLinkState, String>>();
      ...
      for (PortletTypeDescriptor portletType : portletService.listContributableTypes(isAdmin())) {
        ...
        HtmlLinkState link =
            new HtmlLinkState(
                new KeyLabel(portletType.getNameKey()),
                events.getNamedHandler("typeSelected", type));
        portletTypes.add(new Pair<HtmlLinkState, String>(link, portletType.getDescriptionKey()));
      }

      Collections.sort(
          portletTypes,
          new Comparator<Pair<HtmlLinkState, String>>() {
            @Override
            public int compare(
                Pair<HtmlLinkState, String> type1, Pair<HtmlLinkState, String> type2) {
