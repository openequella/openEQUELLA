...
public class ResetFiltersSection<RE extends AbstractSearchResultsEvent<RE>>
    extends AbstractPrototypeSection<ResetFiltersSection.Model>
    implements SearchResultsListener<RE>, HtmlRenderer {
  ...
  @TreeLookup private SearchResultsActionsSection searchResultsActionsSection;

  @SuppressWarnings("nls")
  @Override
  public void treeFinished(String id, SectionTree tree) {
    resetButton.setClickHandler(
        searchResultsSection.getResultsUpdater(
            tree,
            events.getEventHandler("resetFilters"),
            searchResultsActionsSection.getResetFilterAjaxIds()));
  }
  ...
  /**
   * In the typical case, this class applies to an internal search where the search can determine
   * how many elements would be returned for an unfiltered search, and thus the filteredOut value,
   * where non-zero, can be used to inform the user of the actual number excluded by the filters.
   * Where an external search service (such a the Flickr service) is being queried, we content
   * ourselves with setting the filteredOut value as Integer.MIN_VALUE, where a filter has been
   * applied to the search (ie in addition to keyword search), to serve as a simple pseudo-boolean
   * alerting the user to the fact that a filter is in place and therefore the result set ~may~ have
   * been reduced by some unspecified number. Some SearchResult structures may return small
   * negatives with an identical intent to returning zero (eg Hierarchy search), so -1 is an
   * unsatisfactory choice for 'non-zero but unspecified', which is the intent of passing
   * Integer.MIN_VALUE.
   */
  @Override
