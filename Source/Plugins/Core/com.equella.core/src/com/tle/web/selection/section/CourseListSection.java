...
public class CourseListSection extends AbstractPrototypeSection<CourseListSection.CourseListModel>
  ...
  @Inject @Component private SelectionsDialog versionDialog;
  ...
  public SectionResult renderHtml(RenderEventContext context) {
    if (isVisible(context)) {
      ...
      if (structure.isNoTargets()) {
        ...
        saveButton.addReadyStatements(
            context,
            SETUP_FUNCTION,
            onDropAjaxUpdateFunction,
            ajax.getAjaxFunction("selectFolder"),
            ss.isSelectItem(),
            ss.isSelectAttachments());
        ...
        viewSelections.setClickHandler(
            context, new OverrideHandler(versionDialog.getOpenFunction(), "", true));
      }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    selectItemFunction =
        getReloadFunction(false, events.getEventHandler("selectItem"), COURSE_LIST_AJAX);
    selectPackageFunction =
        getReloadFunction(false, events.getEventHandler("selectPackage"), COURSE_LIST_AJAX);
  ...
  private CourseFolderModel convertStructure(SectionInfo info, TargetFolder folder) {
    ...
    nameLink.setClickHandler(
        new OverrideHandler(versionDialog.getOpenFunction(), folder.getId(), false));
  ...
  public void onDrop(SectionInfo info, SelectedResourceKey resourceKey, String folderId) {
    ...
    if (resourceKey.getType() == 'a') {
      addAttachment(
          info,
          resourceKey.getItemKey(),
          resourceKey.getAttachmentUuid(),
          folder,
          resourceKey.getExtensionType());
    } else {
      selectionService.addSelectedResource(
          info,
          selectionService.createItemSelection(
              info,
              itemResolver.getItem(
                  new ItemId(resourceKey.getUuid(), resourceKey.getVersion()),
                  resourceKey.getExtensionType()),
              folder,
              resourceKey.getExtensionType()),
          false);
  ...
  public void selectAttachment(
    ...
    addAttachment(
        ...
        itemId,
        attachmentUuid,
        selectionService.findTargetFolder(info, folderId),
        extensionType);
  }
  ...
  public void selectItem(SectionInfo info, ItemId itemId, String extensionType) {
    ...
    selectionService.addSelectedResource(
        info,
        selectionService.createItemSelection(info, itemResolver.getItem(itemId, et), folder, et),
        false);
  }
    ...
    if (attachment != null) {
      selectionService.addSelectedResource(
          ...
          selectionService.createAttachmentSelection(
              info, itemId, attachment, folder, extensionType),
          false);
    }
  }
