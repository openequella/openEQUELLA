...
public class QTIServiceImpl implements QTIService {
  ...
  public QTIQuiz parseQuizXml(PropBagEx quizXml, String resource) {
    ...
    while (quizIterator.hasNext()) {
      ...
      qtiItem
          .getAnswers()
          .addAll(
              parseResponseConditionsXml(
                  resProcXml, qtiItem.getResponseFeedback(), qtiItem.getResponses()));
  ...
  @SuppressWarnings("nls")
  private List<QTIAnswer> parseResponseConditionsXml(
      PropBagEx resProcXml,
      Map<String, QTIMaterial> responseFeedback,
  ...
  private Map<String, QTIMaterial> parseResponseFeedbackXml(String resource, PropBagEx itemXml) {
    ...
    while (feedIterator.hasNext()) {
      ...
      QTIMaterial feedDisplay =
          parseDisplayElementsXml(resource, feedbackXml.aquireSubtree("material"));
      ...
      if (respXml.getNodeName().startsWith("response_")) {
        ...
        while (choiceIterator.hasNext()) {
          ...
          if (nodeName.startsWith("render_")) {
            ...
            QTIResponse qtiResponse =
                new QTIResponse(respId, renderChoice, display, respCard, lookupId);

            PropBagIterator rendIterator =
                respXml.iterator("render_" + renderChoice + "/response_label");
            while (rendIterator.hasNext()) {
              PropBagEx rendXml = rendIterator.next();

              QTIMaterial rendStr =
                  parseDisplayElementsXml(resource, rendXml.aquireSubtree("material"));
