...
public class QTIItemRenderer {
  ...
  protected PluginResourceHelper resources =
      ResourcesService.getResourceHelper(QTIItemRenderer.class);
    ...
    if (matching && !responseValues.isEmpty()) {
      ...
      for (QTIResponse response : responseValues) {
        ...
        if (ansRenderer == null) {
          ...
          for (QTIResponseElement value : entries) {
            ansRenderer.addRenderer(
                new DivRenderer(
                    "li",
                    response.getType(), // $NON-NLS-1$
      ...
      renderer.addRenderer(
          new DivRenderer(
              DESC,
              new SimpleSectionResult(
                  CurrentLocale.get(resources.key("matching.text"))))); // $NON-NLS-1$
      renderer.addRenderer(
          new DivRenderer("ol", "matching-left", respRenderer)); // $NON-NLS-1$ //$NON-NLS-2$
      renderer.addRenderer(
          new DivRenderer(
              "ul", "matching-right standard", ansRenderer)); // $NON-NLS-1$ //$NON-NLS-2$
    } else {
      for (QTIResponse response : responseValues) {
        CombinedRenderer respRenderer = new CombinedRenderer();
        ...
        respRenderer.addRenderer(
            new DivRenderer(
                "span",
                "response-text", //$NON-NLS-1$//$NON-NLS-2$
        ...
        renderer.addRenderer(
            new DivRenderer(
                DESC,
                new SimpleSectionResult(
                    CurrentLocale.get(resources.key(key), response.getType()))));
  ...
  public SectionRenderable getAnswers() {
    ...
    for (QTIAnswer qtiAnswer : answers) {
      ...
      SpanRenderer span =
          new SpanRenderer(
              score > 0 ? "correct" : "incorrect", new SimpleSectionResult(answer + " : " + score));
      ...
      itemRenderer.addRenderer(
          new SpanRenderer("feedback", new SimpleSectionResult(html.toString())));
