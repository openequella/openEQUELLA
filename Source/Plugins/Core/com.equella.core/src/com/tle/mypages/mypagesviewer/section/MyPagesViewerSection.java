...
/**
 * Need only be used in the event that the item in question has page attachments, but no navigation
 * tree of it's own. This class will whip up a temporary tree on each page request. Usage: point to
 * the URL /item/xxxxxxx/vv/viewpages.jsp
 *
 * @author aholland
 */
@Bind
...
public class MyPagesViewerSection extends AbstractTreeViewerSection<AbstractTreeViewerModel> {
  ...
  protected List<ItemNavigationNode> getTreeNodes(SectionInfo info) {
    ...
    itemNavService.populateTreeNavigationFromAttachments(
        item,
        nodes,
        myPagesService.getPageAttachments(info, null, item.getItemId().toString()),
