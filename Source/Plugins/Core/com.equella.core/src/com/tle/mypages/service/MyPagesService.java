...
public interface MyPagesService {
  ...
  /** Add the given attachment to the state and uploads a blank page to the repository */
  HtmlAttachment newPage(SectionInfo info, String sessionId, String pageName);

  /**
   * Retrieves the text of the given attachment and converts it to draft urls. Also converts any
   * urls which point to itemId, to point to preview urls.
   */
  String getDraftHtml(
      WizardStateInterface state, SectionInfo info, HtmlAttachment attachment, ItemKey itemId);
   ...
   */
  HtmlAttachment getPageAttachment(
  ...
  /**
   * If this is not the final page then get the next page. If it is the final page, get the previous
   * page. If there are no other pages return null.
   *
   * @param sessionId
   * @param pageUuid
   * @return
  ...
  /**
   * Commits HtmlAttachments that are in draft. Changes draft URLs into preview URLs OR item urls,
   * dependent on the value of leaveAsPreview
   */
  ...
  /**
   * Goes through HtmlAttachments and if it is a draft item, delete its files and set it to
   * non-draft
   */
  ...
  /**
   * Scans the html and removes all /preview/uuid/etc URLs and replaces them with /items/uuid/etc
   *
   * @param state
   */
   ...
   * @return
   */
  HtmlAttachment clonePage(
      WizardStateInterface state, Item sourceItem, HtmlAttachment sourcePage, boolean draft);
