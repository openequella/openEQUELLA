...
/* package protected */ class ThumbnailRequestServiceImpl implements ThumbnailRequestService {
  ...
  public void newRequest(
    ...
    // (TODO: this could be a large video... we'd want to avoid this copy if it were possible.
    // COWFS anyone?)
  ...
  /**
   * You can directly use this method if you know that it definitely still exists. The Nullable is
   * due to the string parameter version using this method.
   *
   * @param tr
   */
  @Transactional
  protected void delete(@Nullable ThumbnailRequest tr, String requestUuid) {
    try {
      ...
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug(
            "Cleaning up files for thumbnail request "
                + (tr == null ? requestUuid : tr.toString()));
      }
      fileSystemService.removeFile(thumbQueueFile);
    } catch (Exception e) {
      LOGGER.warn(
          "Error cleaning up thumbnail request files " + (tr == null ? requestUuid : tr.toString()),
          e);
    } finally {
  ...
  public List<ThumbnailRequest> listForFile(ItemKey itemId, FileHandle handle, String filename) {
    final List<ThumbnailRequest> forFile =
        new ArrayList<>(
            thumbRequestDao.listForFile(
                CurrentInstitution.get(), itemId, new Md5(filename).getStringDigest()));
  ...
  public boolean exists(ItemKey itemId, FileHandle handle, String filename) {
    ...
    return thumbRequestDao.exists(
        itemId, superSerial, filename, new Md5(filename).getStringDigest());
  }
  ...
  public void cleanThumbQueue() {
    ...
    for (InstitutionStatus inst : allInstitutions) {
      fileSystemService.apply(
          new InstitutionFile(inst.getInstitution()),
          "ThumbQueue",
          "*/*",
          new GrepFunctor() {
            @Override
