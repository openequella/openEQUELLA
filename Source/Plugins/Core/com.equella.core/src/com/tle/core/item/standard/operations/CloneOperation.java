...
public final class CloneOperation extends AbstractCloneOperation // NOSONAR
{
  ...
  private CloneOperation(
      @Assisted("copyAttachments") boolean copyAttachments, @Assisted("submit") boolean submit) {
  ...
  @AssistedInject
  private CloneOperation(
      @Assisted String newItemDefUuid,
      @Assisted("copyAttachments") boolean copyAttachments,
  ...
  protected void finalProcessing(Item origItem, Item item) {
    ...
    if (!Check.isEmpty(transform)) {
      ...
      ItemPack<Item> oldPack =
          new ItemPack<>(oldItem, itemService.getItemXmlPropBag(oldItem), null);
        ...
        PropBagEx newXml =
            new PropBagEx(
                schemaService.transformForImport(
                    newItem.getItemDefinition().getSchema().getId(), transform, oldXml));
        pack.setXml(newXml);
      } catch (Exception ex) {
