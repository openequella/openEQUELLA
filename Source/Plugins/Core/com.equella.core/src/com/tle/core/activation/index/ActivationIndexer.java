  ...
  public void indexFast(IndexedItem indexedItem) {
    ...
    for (ActivateRequest request : activateRequests) {
      if (request.getCourse() != null) {
        ...
        doc.add(
            keyword(FreeTextQuery.FIELD_ACTIVATION_STATUS, String.valueOf(request.getStatus())));
        ...
        doc.add(
            keyword(
                ...
                new UtcDate(request.getTime()).format(Dates.ISO)));

        doc.add(
            indexed(
                FreeTextQuery.FIELD_ACTIVATION_FROM,
                new UtcDate(request.getFrom()).format(Dates.ISO)));
        doc.add(
            indexed(
                FreeTextQuery.FIELD_ACTIVATION_UNTIL,
                new UtcDate(request.getUntil()).format(Dates.ISO)));
