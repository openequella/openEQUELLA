...
public class EntityDisabledFieldMigrationPart2 extends AbstractHibernateSchemaMigration {
  ...
  public static class CheckColumnWork implements Work {
    ...
    /**
     * When running on Oracle, we actually need to uppercase the table name. Specifying the column
     * name as the fourth parameter to getColumns is optional but again, if on Oracle, the column
     * name if present must also be uppercased (we could otherwise pass null as the column name
     * filter, and then search through all the returned columns, being sure to do a case-insensitive
     * match on the column name as String)
     */
    @Override
    public void execute(Connection connection) throws SQLException {
      ...
      final String metadataFriendlyTableName =
          helper.getExtDialect().getNameForMetadataQuery(TABLE_FED, false);
      final String metadataFriendlyColumnName =
          helper.getExtDialect().getNameForMetadataQuery(COL_DISABLED, false);
      final ResultSet columnSet =
          metaData.getColumns(
              defaultCatalog, defaultSchema, metadataFriendlyTableName, metadataFriendlyColumnName);
