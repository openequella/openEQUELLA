...
public class HttpServiceImpl implements HttpService {
  ...
  @Override
  public Response getWebContent(
      Request request, @Nullable ProxyDetails proxy, boolean followRedirects) {
    return getWebContentPrivate(request, proxy, followRedirects);
  }

  private ResponseImpl getWebContentPrivate(
      Request request, @Nullable ProxyDetails proxy, boolean followRedirects) {
    ...
    try {
      ...
      final DefaultHttpClient client =
          createClient(httpMethod.getURI().getScheme().equals("https"));
  ...
  private DefaultHttpClient createClient(boolean https) {
    final DefaultHttpClient client =
        (https ? new DefaultHttpClient(conMan) : new DefaultHttpClient());
  ...
  public static class ACookie {
    private final DateFormat expiresFormat1 =
        new SimpleDateFormat("E, dd MMM yyyy k:m:s 'GMT'", Locale.US);
    private final DateFormat expiresFormat2 =
        new SimpleDateFormat("E, dd-MMM-yyyy k:m:s 'GMT'", Locale.US);
  ...
  private HttpRequestBase getHttpMethod(Request request) {
    ...
    final List<NameValuePair> reqParams =
        Lists.transform(request.getParams(), new ParamConverter());
