...
public class ReplaceDeletedControlsItemXmlMigrationFixer
    extends ReplaceDeletedControlsItemXmlMigration {
  @Inject private XmlHelper xmlHelper;

  @Override
  public boolean migrate(
      ConverterParams params, PropBagEx xml, SubTemporaryFile itemsHandle, String filename) {
    ...
    while (iter.hasNext()) {
      ...
      if (axml.getNodeName().equals("com.tle.beans.item.attachments.FileAttachment")
        ...
        if (Check.isEmpty(itemIdString)) {
          ...
          itemId =
              new ItemId(
                  metadata.getNode(file + "/@uuid"), metadata.getIntNode(file + "/@version"));
        } else {
      ...
      if (handler != null) {
        ...
        if (xMetadata.node(
                "xml/"
                    + ReplaceDeletedControlsXmlMigration.getXpathForHandler(handler)
                    + "[text()='"
                    + uuid
                    + "']")
            == null) {
    ...
    if (!itemAttachments.isEmpty()) {
      ...
      for (Node node :
          xXml.nodeList(
              "//com.tle.beans.item.attachments.ItemNavigationTab"
