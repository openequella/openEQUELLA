...
public class ItemDeserializerServiceImpl implements ItemDeserializerService {
  @Inject private ItemEditorService itemEditorService;
  @Inject private PluginTracker<ItemDeserializerEditor> editorsTracker;
  @Inject private ItemService itemService;
  @Inject private ItemOperationFactory workflowFactory;

  @Override
  @Transactional
  public ItemIdKey edit(
    ...
    ItemEditor editor =
        itemEditorService.getItemEditor(itemId, stagingUuid, lockId, editorsTracker.getBeanList());
  ...
  public ItemIdKey newItem(
    ...
    ItemEditor editor =
        itemEditorService.newItemEditor(
            collectionUuid, itemId, stagingUuid, editorsTracker.getBeanList());
    editor.doEdits(itemBean);
    if (!dontSubmit) {
  ...
  @Transactional
  @Override
  public ItemIdKey importItem(
      EquellaItemBean itemBean, String stagingUuid, boolean ensureOnIndexList) {
    BaseEntityReference collectionRef = itemBean.getCollection();
    if (collectionRef == null) {
      throw new ItemEditingException("No collection specified");
    }
    String collectionUuid = collectionRef.getUuid();
    ItemEditor editor =
        itemEditorService.importItemEditor(
            collectionUuid,
            new ItemId(itemBean.getUuid(), itemBean.getVersion()),
            stagingUuid,
