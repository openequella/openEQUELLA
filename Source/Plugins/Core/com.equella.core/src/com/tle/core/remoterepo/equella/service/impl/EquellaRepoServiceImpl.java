...
// Will be needed as we upgrade tika
...
public class EquellaRepoServiceImpl implements EquellaRepoService {
  ...
  // This is not ideal...
  private static final Set<String> FILE_BASED_ATTACHMENT_TYPES =
      new HashSet<String>(Arrays.asList(new String[] {"local", "zip", "imsres", "html"}));

  private final Cache<String, AttachmentDownloadSessionImpl> sessions =
    ...
    try {
      ...
      if (session == null
        ...
        final String username =
            (settings.isUseLoggedInUser() ? CurrentUser.getUsername() : settings.getUsername());
        taskService.getGlobalTask(
            new BeanClusteredTask(
                key,
                EquellaRepoService.class,
                "createDownloadTask",
                CurrentUser.getUserState(),
                username,
                uuid,
                version,
                settings.getInstitutionUrl(),
                settings.getSharedSecretId(),
                settings.getSharedSecretValue(),
                key),
  ...
  public PropBagEx getItemXml(
      TLESettings settings, String uuid, int version, boolean stripFileBasedAttachments) {
    try {
      final String username =
          (settings.isUseLoggedInUser() ? CurrentUser.getUsername() : settings.getUsername());
      final SoapHarvesterService client =
          getNewClient(
              settings.getInstitutionUrl(),
              settings.getSharedSecretId(),
              settings.getSharedSecretValue(),
  ...
  protected SoapHarvesterService getNewClient(
      String url, String sharedId, String sharedValue, String username) {
    try {
      ...
      factory.setServiceName(
          new QName(
              "http://soap.harvester.core.tle.com", SoapHarvesterService.class.getSimpleName()));
      ...
      soapClient.loginWithToken(
          TokenGenerator.createSecureToken(username, sharedId, sharedValue, null));
      return soapClient;
    } catch (Exception x) {
    ...
    public void doDownload() {
      try {
        ...
        try {
          ...
          UploadCallbackInputStream inputStream =
              new UploadCallbackInputStream(conn.getInputStream(), callback);
