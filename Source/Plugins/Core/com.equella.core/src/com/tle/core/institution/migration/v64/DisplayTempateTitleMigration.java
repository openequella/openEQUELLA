...
{
	private static PluginResourceHelper r = ResourcesService.getResourceHelper(DisplayTempateTitleMigration.class);
				...
				while( iter.hasNext() )
				{
					...
					switch( configValue )
					{
						case "attachmentsSection":
							InternalI18NString attachmentTitle = new KeyString(r.key("summary.content.attachments.title"));
							addNewBundleTitle(attachmentTitle.toString(), config);
							break;
						case "commentsSection":
							InternalI18NString commentTitle = new KeyString(
								r.key("comments.addnew"));
