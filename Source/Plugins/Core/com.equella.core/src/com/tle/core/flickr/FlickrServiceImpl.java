...
public class FlickrServiceImpl implements FlickrService {
  ...
  /**
   * A string to place in the search params in order to flag that we want this icon_server,
   * original_format, last_update, geo, media, path_alias,
   */
  private static final Set<String> EXTRAS_SET =
      new HashSet<String>(
          Arrays.asList(
              "description",
              "license",
  ...
  /**
   * We allow that where tags are sent by caller, that either the totality of tags is the search
   * criterion (hence "all"), or just "any".
   */
  ...
  /**
   * the expanded class FlickrSearchParameters is queried to determine what fine-tuning of the
   * search parameters is required before submitting to the flickr service API. If a tag search has
   * been requested, the tags will be tokenised on commas if any are present, otherwise on spaces.
   * If a value has been set for a flickr user string, that string is examined to see if it is an
   * email, a flick user-id or (failing either) a flickr user-name.
   */
  @Override
  public PhotoList<Photo> searchOnParams(
      FlickrSearchParameters params,
      int pageToGet,
      int queryLimit,
      String apiKey,
    ...
    // query
    /* boolean sendingFlickrUserId = */ extractUserSearchString(flickr, params);
  ...
  /**
   * Determine if there is non-whitespace content in the "user" field, and if to to decide if it's a
   * flickr userid (not so likely, but more efficient) or a flickr user's email address. If neither,
   * we assume it's a flickr username. Assign the resultant value in the searchParams.
   *
   ...
   * @throws FlickrException Flickr throws with a user not found message if such applies
   */
   ...
   * @throws FlickrException Flickr throws with a user not found message if such applies
   */
  private String queryFlickrOnUsername(Flickr flickr, String presumedUsername)
      throws FlickrException {
    String flickrUserId = null;
    PeopleInterface pi = flickr.getPeopleInterface();
