...
public class BrightspaceConnectorServiceImpl extends AbstractIntegrationConnectorRespository
  ...
  private static final Pattern UUID_VERSION_REGEX =
      Pattern.compile(
          ".*/(" + UUID_REGEX + ")/([0-9])+/?(?:\\?attachment\\.uuid=(" + UUID_REGEX + "))?",
          Pattern.CASE_INSENSITIVE);
  private static final String KEY_PFX =
      AbstractPluginService.getMyPluginId(BrightspaceConnectorServiceImpl.class) + ".";
  ...
  private Cache<String, CourseOffering> courseCache =
      CacheBuilder.newBuilder().expireAfterAccess(30, TimeUnit.MINUTES).build();
  private Cache<String, ContentObject> moduleCache =
      CacheBuilder.newBuilder().expireAfterAccess(30, TimeUnit.MINUTES).build();
  ...
  public String whoAmI(
      String appId, String appKey, String brightspaceServerUrl, String userId, String userKey) {
  ...
  public String testApplication(String appId, String appKey, String brightspaceServerUrl) {
    BrightspaceAppContext brightspaceAppContext =
        new BrightspaceAppContext(appId, appKey, brightspaceServerUrl);
    URI uri =
        brightspaceAppContext
            .createAnonymousUserContext()
            .createAuthenticatedUri(API_ROOT + "/versions/", "GET");

    Response webContent =
        httpService.getWebContent(new Request(uri.toString()), configService.getProxyDetails());
  ...
  private String getAuthorisationUrl(
      BrightspaceAppContext appContext, String forwardUrl, @Nullable String postfixKey) {
  ...
  public List<ConnectorCourse> getCourses(
    ...
    final BrightspaceUserContext userContext =
        (management ? getAdminContext(connector) : getUserContext(connector));
    ...
    while (hasMore && giveUp < 10) {
      ...
      final EnrollmentsPagedResults results =
          sendBrightspaceData(
              connector, pagedUrl, EnrollmentsPagedResults.class, null, userContext, Method.GET);
  ...
  public List<ConnectorFolder> getFoldersForCourse(
    ...
    final BrightspaceUserContext userContext =
        (management ? getAdminContext(connector) : getUserContext(connector));
    final URI modules =
        userContext.createAuthenticatedUri(leApi + "/" + courseId + "/content/root/", "GET");
    final Request request = new Request(modules.toString());
    try (Response webContent =
      ...
      final List<ContentObject> contents =
          jsonMapper.readValue(stringed, new TypeReference<List<ContentObject>>() {});
  ...
  public List<ConnectorFolder> getFoldersForFolder(
    ...
    final BrightspaceUserContext userContext =
        (management ? getAdminContext(connector) : getUserContext(connector));
    final ModuleTransformer transformer =
        new ModuleTransformer(
            courseOfferingTransformer.apply(
                getBrightspaceCourse(connector, courseId, userContext)));
    final List<ConnectorFolder> folders = new ArrayList<>();
  ...
  private void appendFindUsages(
      BrightspaceUserContext userContext,
      List<ConnectorContent> usages,
      Connector connector,
      String uuid,
      int version,
      boolean archived) {
    final StringBuilder url =
        new StringBuilder(CUSTOM_API + "/linkinfo/" + uuid + "/" + version + "/?limit=1000");
    if (archived) {
      url.append("&showInactive=" + archived);
    }

    final BrightspaceUsageResults results =
        sendBrightspaceData(
            connector,
            url.toString(),
            BrightspaceUsageResults.class,
            null,
            userContext,
            Method.GET);
  ...
  public SearchResults<ConnectorContent> findAllUsages(
    ...
    return new SimpleSearchResults<ConnectorContent>(
        usages, usages.size(), offset, results.getResultSetCount());
  }
  ...
  private ConnectorContent convertUsage(BrightspaceEquellaLink link) {
    ...
    final ConnectorContent usage =
        new ConnectorContent(
            new CompositeId(
                    Integer.toString(link.getOrgUnitId()),
                    Integer.toString(link.getParentModuleId()),
                    Integer.toString(link.getTopicId()),
                    Integer.toString(link.getLtiLinkId()))
                .toString());
    ...
    final UtcDate modDate =
        DateHelper.parseOrNull(link.getLastModifiedDate(), Dates.ISO_WITH_MILLIS_NO_TIMEZONE);
    if (modDate != null) {
        ...
        if (linkTarget.contains("?")) {
          Map<String, String> params =
              URLUtils.parseQueryString(
                  Utils.safeSubstring(linkTarget, linkTarget.indexOf("?")), true);
    ...
    usage.setAttribute(
        "contentAvailable",
        getKey("bspace.finduses.label.visible"),
        CurrentLocale.get(
            getKey("bspace.finduses.value.visible." + (link.getVisible() ? "yes" : "no"))));
  ...
  public boolean editContent(
    ...
    final ContentObject contentObject =
        sendBrightspaceData(
            connector,
            leApi + "/" + compId.getCourseId() + "/content/topics/" + compId.getTopicId(),
            ContentObject.class,
            null,
            userContext,
            Method.GET);
  ...
  public boolean moveContent(
    ...
    final ContentObject contentObject =
        sendBrightspaceData(
            ...
            userContext,
            Method.GET);
    final BrightspaceLtiLink ltiLink =
        sendBrightspaceData(
            connector,
            leApi + "/lti/link/" + compId.getCourseId() + "/" + compId.getLtiLinkId(),
            BrightspaceLtiLink.class,
            null,
            userContext,
            Method.GET);
  ...
  private void delete(BrightspaceUserContext userContext, Connector connector, CompositeId compId) {
    ...
    sendBrightspaceData(
        connector,
        leApi + "/lti/link/" + compId.getLtiLinkId(),
        null,
        null,
        userContext,
        Method.DELETE);
  ...
  @Override
  public BrightspaceQuicklink addQuicklink(
      Connector connector,
      String courseId,
      String moduleId,
      LmsLink link,
  ...
  private BrightspaceQuicklink addQuicklink(
      Connector connector,
      String courseId,
      String moduleId,
      String name,
    ...
    final BrightspaceLtiLink retLink =
        sendBrightspaceData(
            connector,
            leApi + "/lti/link/" + courseId,
            BrightspaceLtiLink.class,
            ltiLink,
            getAdminContext(connector),
            Method.POST);

    final BrightspaceQuicklink quicklink =
        sendBrightspaceData(
            connector,
            leApi + "/lti/quicklink/" + courseId + "/" + retLink.getLtiLinkId(),
            BrightspaceQuicklink.class,
            null,
            userContext,
            Method.POST);
    ...
    if (topicOption == TopicCreationOption.CREATE) {
      ...
      sendBrightspaceData(
          connector,
          leApi + "/" + courseId + "/content/modules/" + moduleId + "/structure/",
          null,
          topic,
          userContext,
          Method.POST);
  ...
  private BrightspaceUserContext getUserContext(Connector connector) {
    BrightspaceUserContext userContext =
        userSessionService.getAttribute(BrightspaceConnectorConstants.SESSION_KEY_USER_CONTEXT);
    if (userContext == null) {
      final String userId =
          userSessionService.getAttribute(BrightspaceConnectorConstants.SESSION_KEY_USER_ID);
      if (userId == null) {
        throw new LmsRequiresAuthenticationException(
            "No Brightspace user ID associated with session");
      }

      final String userKey =
          userSessionService.getAttribute(BrightspaceConnectorConstants.SESSION_KEY_USER_KEY);

      final BrightspaceAppContext appContext = getAppContext(connector);
      userContext = appContext.createUserContext(userId, userKey);
      userSessionService.setAttribute(
          BrightspaceConnectorConstants.SESSION_KEY_USER_CONTEXT, userContext);
  ...
  private BrightspaceUserContext getAdminContext(Connector connector) {
    ...
    final String userKey =
        connector.getAttribute(BrightspaceConnectorConstants.FIELD_ADMIN_USER_KEY);
  ...
  @Nullable
  private <T> T sendBrightspaceData(
      Connector connector,
      String path,
      ...
      try (Response response =
        ...
        if (code >= 300) {
          throw new RuntimeException(
              "Received " + code + " from Brightspace. Body = " + responseBody);
        }
        if (returnType != null) {
  ...
  private ContentObject getModule(
      Connector connector, String courseId, String folderId, BrightspaceUserContext userContext) {
    final String key =
    ...
    if (module == null) {
      module =
          sendBrightspaceData(
              connector,
              leApi + "/" + courseId + "/content/modules/" + folderId,
              ContentObject.class,
              null,
              userContext,
              Method.GET);
  ...
  private CourseOffering getBrightspaceCourse(
      Connector connector, String courseId, BrightspaceUserContext userContext) {
    final String key =
        CurrentInstitution.get().getUniqueId() + ":" + connector.getUuid() + ":" + courseId;
    CourseOffering course = courseCache.getIfPresent(key);
    if (course == null) {
      course =
          sendBrightspaceData(
              connector,
              lpApi + "/courses/" + courseId,
