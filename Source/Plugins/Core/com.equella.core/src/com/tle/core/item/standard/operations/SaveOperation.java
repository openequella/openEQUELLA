...
public class SaveOperation extends AbstractStandardWorkflowOperation implements DeleteHandler {
  ...
  @Inject private Provider<InsecureArchiveOperation> archiveProvider;
  ...
  @AssistedInject
  protected SaveOperation(
      @Assisted boolean unlock,
      @Assisted("pre") List<WorkflowOperation> preSaveOperations,
  ...
  public boolean execute() {
    ...
    if (params.isModified()) {
      ...
      if (wentlive) {
        addNotifications(
            item.getItemId(), item.getNotifications(), Notification.REASON_WENTLIVE, true);
        if (getWorkflow() != null) {
  ...
  private void saveToRepository(ItemPack<Item> itemPack, boolean doUpdate) {
    ...
    if (!doUpdate
        && aclManager
            .filterNonGrantedPrivileges(item.getItemDefinition(), Arrays.asList(CREATE_ITEM_PRIV))
            .isEmpty()) {
      throw new AccessDeniedException(
          "You do not have the required privileges to" //$NON-NLS-1$
    ...
    try {
      ...
      LOGGER.warn(
          "There was an error updating referenced urls for submitted item " + getItemId(), e);
    }
    ...
    try {
      ...
      if (staging != null) {
        ...
        params.addAfterCommitHook(
            ItemOperationParams.COMMIT_HOOK_PRIORITY_MEDIUM,
            new Runnable() {
              @Override
              public void run() {
                try {
