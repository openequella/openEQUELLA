...
public class URLCheckerDaoImpl extends GenericDaoImpl<ReferencedURL, Long>
  ...
  @Override
  @Transactional
  public ReferencedURL retrieveOrCreate(
      final String url, final boolean httpUrl, final boolean forImport, boolean persist) {
    ReferencedURL rurl =
        (ReferencedURL)
  ...
  private ReferencedURL loadOrCreate(
      Session session, String url, boolean httpUrl, boolean forImport, boolean persist) {
    ...
    if (rurl == null) {
      if (httpUrl && !forImport && persist) {
        taskService.postSynchronousMessage(
            getTaskId(),
            new CreateUrlMessage(url, CurrentInstitution.get().getUniqueId()),
            MSGTIMEOUT);
  ...
  public Collection<ReferencedURL> getRecheckingBatch(final long startId, final int batchSize) {
    return (Collection<ReferencedURL>)
        getHibernateTemplate()
            .execute(
                new HibernateCallback() {
                  @Override
  ...
  private ReferencedURL getFromDb(Session session, String url) {
    return (ReferencedURL)
        session
            .createQuery("FROM ReferencedURL WHERE urlHash = :urlHash")
            .setString("urlHash", DigestUtils.md5Hex(url))
            .setReadOnly(true)
            .uniqueResult();
  }
          ...
          taskService
              .getGlobalTask(
                  new BeanClusteredTask(
                      "ReferencedURLCreator",
                      URLCheckerDao.class,
                      "createReferencedURLCreatorTask"),
                  MSGTIMEOUT)
              .getTaskId();
    }
    return taskId;
    ...
    return new AlwaysRunningTask<SimpleMessage>() {
      ...
      public void runTask(final SimpleMessage msg) {
        ...
        runAs.executeAsSystem(
            institutionService.getInstitution(content.getInstitutionId()),
            new Runnable() {
              @Override
              public void run() {
