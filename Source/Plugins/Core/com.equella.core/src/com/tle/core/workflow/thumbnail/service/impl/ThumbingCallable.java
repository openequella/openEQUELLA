...
public class ThumbingCallable implements Callable<ThumbingCallableResult> {
  ...
  @Inject private PluginTracker<ThumbnailGenerator> thumbnailTracker;
  ...
  public ThumbingCallableResult call() {
    try {
      return runAs.executeAsSystem(
          institution,
          new Callable<ThumbingCallableResult>() {
            @Override
            public ThumbingCallableResult call() {
              return makeThumbs();
            }
  ...
  public ThumbingCallableResult makeThumbs() {
    ...
    final ThumbingCallableResult result =
        new ThumbingCallableResult(
            thumbnailRequest, CurrentInstitution.get(), requestUuid, itemId, serialHandle);
    try {
      ...
      final ItemId itemId =
          new ItemId(thumbnailRequest.getItemUuid(), thumbnailRequest.getItemVersion());
      ...
      final String thumbFile =
          PathUtils.filePath(
              FileSystemService.THUMBS_FOLDER, filename + FileSystemService.THUMBNAIL_EXTENSION);
      final String galleryThumb =
          PathUtils.filePath(
              FileSystemService.THUMBS_FOLDER,
              filename + FileSystemService.GALLERY_THUMBNAIL_EXTENSION);
      final String galleryPreview =
          PathUtils.filePath(
              FileSystemService.THUMBS_FOLDER,
      ...
      if (ThumbnailType.TYPE_GALLERY_PREVIEW.enabled(flags)) {
        final Dimension dimensions =
            thumbGen.getImageDimensions(
                fileSystemService.getExternalFile(thumbQueueFile, filename));
      ...
      for (String prodThumb : result.getBuiltThumbnails()) {
        ...
        // This is re-evaluated each time because it may change disappear at any time (ie item is
        // saved)
        ...
        if (destHandle instanceof StagingFile
          ...
          realDestHandle =
              (fileSystemService.fileExists(destHandle)
                  ? destHandle
      ...
      LOGGER.info(
          "Finished thumbnailing "
              + thumbnailRequest.toString()
              + " into "
              + result.getBuiltThumbnails().size()
              + " thumbnails in "
              + (now - result.getStartTime())
              + "ms.");
    } catch (Throwable ex) {
  ...
  private void genThumb(
      ThumbnailQueueFile thumbQueueFile,
      ThumbingCallableResult result,
      String destinationFilename,
      @Nullable ThumbnailOptions options,
      String... sources)
      throws Exception {
    ...
    for (String source : sources) {
      if (fileSystemService.fileExists(thumbQueueFile, source)) {
        ...
        if (options == null) {
          thumbGen.generateThumbnail(
              fileSystemService.getExternalFile(thumbQueueFile, source), dstFile);
        } else {
          thumbGen.generateThumbnailAdvanced(
              fileSystemService.getExternalFile(thumbQueueFile, source), dstFile, options);
        }
