...
public final class ItemEditorImpl implements ItemEditor, DeleteHandler, ItemEditorChangeTracker {
  ...
  @AssistedInject
  private ItemEditorImpl(
      TLEAclManager aclManager,
      @Assisted Item item,
      @Assisted @Nullable ItemLock lock,
    ...
    privileges =
        aclManager.filterNonGrantedPrivileges(
            item,
            ImmutableSet.of(
                ItemSecurityConstants.EDIT_ITEM, ItemSecurityConstants.REASSIGN_OWNERSHIP_ITEM));
  ...
  @AssistedInject
  private ItemEditorImpl(
      @Assisted Item item, @Assisted List<ItemDeserializerEditor> deserializerEditors) {
    this.item = item;
    this.newItem = true;
    this.itemLock = null;
    this.deserializerEditors = deserializerEditors;
      ...
      for (String attachmentUuid : attachmentUuids) {
        ...
        if (newAttach == null) {
          throw new ItemEditingException(
              "Attachment with uuid '" + attachmentUuid + "' does not exist");
        }
      ...
      for (ItemNodeStatusExportBean insexb : nodeStatii) {
        ...
        if (!Check.isEmpty(insexb.getStatus())) {
          ...
          if (!WorkflowNodeStatus.isValidMessageType(c)) {
            throw new ItemEditingException(
                "Unrecognised node status expression: " + insexb.getStatus());
          }
      ...
      for (ItemNodeStatusExportBean insexb : nodeStatii) {
        if (insexb.getCauseIndex() >= 0) {
          ...
          if (workflowNodeStatus instanceof WorkflowItemStatus) {
            ((WorkflowItemStatus) workflowNodeStatus)
                .setCause(indexedNewNodes.get(insexb.getCauseIndex()));
          }
        }
        indexMe++;
      }
  ...
  public ItemIdKey finishedEditing(boolean ensureOnIndexList) {
    ...
    TransactionSynchronizationManager.registerSynchronization(
        new TransactionSync(ensureOnIndexList));
    ...
    public void afterCommit() {
      ...
      if (!indexingChanges.isEmpty()) {
        ...
        eventService.publishApplicationEvent(
            new IndexItemBackgroundEvent(itemId, !ensureOnIndexList));
  ...
  public interface ItemEditorFactory {
    ItemEditorImpl createExistingEditor(
        Item item, ItemLock lock, List<ItemDeserializerEditor> deserializerEditors);

    ItemEditorImpl createNewEditor(Item item, List<ItemDeserializerEditor> deserializerEditors);

    ItemEditorImpl createImportEditor(
        Item item, List<ItemDeserializerEditor> deserializerEditors, boolean importing);
  }
