...
public interface FileSystemService extends RemoteFileSystemService {
  ...
  /**
   * Gets a Java File object if you for the associated Staging/Institution/Item file
   *
   * @param handle
   * @param path The path cannot be at a level above the handle path, ie. no ".." allowed! Can be
   *     null or "" to indicate to use the handle only.
   * @return
   */
  File getExternalFile(FileHandle handle, String path);

  /**
   * Builds a flat file listing of the folder represented by the path. Does not recurse
   * subdirectories
   *
  ...
  /**
   * Builds a tree of FileEntry objects for the given file path. The root of the tree represents the
   * path specified. If the path is a file, a single FileEntry object is returned.
   *
   * @param handle The base folder handle. E.g. a StagingFile object
   * @param path Path relative to handle, may be a folder or an actual file.
   * @param filter Custom filter to ignore certain files
   * @return A tree representing the folder structure of path, or single FileEntry if path is a
   *     file.
   * @throws IOException
   */
  ...
  OutputStream getOutputStream(FileHandle handle, String filename, boolean append)
      throws IOException;
   ...
   */
  FileInfo write(
      FileHandle handle, String filename, InputStream content, boolean append, boolean md5)
      throws IOException;
   ...
   * @throws IOException
   */
  FileInfo write(FileHandle handle, String filename, InputStream content, boolean append)
      throws IOException;
  ...
  void commitFiles(TemporaryFileHandle staging, String filename, FileHandle destination)
      throws IOException;
  ...
  /**
   * Test if two paths point to the same file. Eg. in Windows path1 = "MyFile.jpg" and path2 =
   * "myfile.JPG" don't string compare and yet refer to the same file. There will be other reasons
   * the handles and paths don't match and yet refer to the same file.
   *
   * @param handle1
   * @param path1
   * @param handle2
  ...
  void copyToStaging(
      ...
      String newname,
      boolean ignoreInteralFiles)
      throws IOException;

  void copyToStaging(FileHandle handle, TemporaryFileHandle staging, boolean ignoreInteralFiles)
      throws IOException;
  ...
  void unzipFile(
      FileHandle handle, InputStream in, ArchiveType method, final ArchiveProgress progress)
      throws IOException;
  ...
  FileInfo unzipFile(
      FileHandle handle, String zipfile, String outpath, final ArchiveProgress progress)
      throws IOException;

  FileInfo unzipFile(FileHandle handle, String zipfile, String outpath) throws IOException;

  ZipProgress unzipWithProgress(FileHandle handle, String zipFile, String targetPath)
      throws IOException;
   ...
   */
  ByteArrayOutputStream extractNamedZipEntryAsStream(
      FileHandle handle, String packageZipName, String entryToFind);
   ...
   */
  void extractNamedZipEntryAsStream(
  ...
  /**
   * Deletes a file, or recursively remove a folder. File or folder need not exist
   *
   * @param handle
   * @return
   */
  ...
  void zipFile(
      FileHandle handle, String path, FileHandle out, String outFilename, ArchiveType archiveType)
      throws IOException;

  /**
   * Returns a list of paths matching the given pattern. The pattern is modelled on Ant's file
   * patterns, and a complex example can be seen if you view the source for this interface. The
   * example could not be included in Javadoc comments, as it contains characters that cannot be
   * escaped.
   */
  //
  ...
  /**
   * Applies a functor to a list of paths matching the given pattern. See matchFiles() for basic
   * pattern usage, and an example of returning a list of match file names.
   */
