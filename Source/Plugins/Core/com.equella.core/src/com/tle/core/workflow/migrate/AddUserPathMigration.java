...
public class AddUserPathMigration extends AbstractHibernateSchemaMigration {
  private static final String keyPrefix =
      PluginServiceImpl.getMyPluginId(AddUserPathMigration.class) + "."; // $NON-NLS-1$
  ...
  public MigrationInfo createMigrationInfo() {
    return new MigrationInfo(
        keyPrefix + "migratepath.title", keyPrefix + "migratepath.description");
  }
  ...
  protected List<String> getDropModifySql(HibernateMigrationHelper helper) {
    return helper.getAddIndexesAndConstraintsForColumns(
        "workflow_node", "user_path", "user_schema_uuid");
  }
}
