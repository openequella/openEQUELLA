...
public class ReplaceDeletedControlsXmlMigration extends XmlMigrator {
  ...
  private static final String CUSTOM_CONTROL_CLASS =
  ...
  private static final String RESOURCE_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"resource\"]";
  private static final String YOUTUBE_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"youtube\"]";
  private static final String FLICKR_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"flickr\"]";
  private static final String GOOGLEBOOK_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"googlebook\"]";
  private static final String ITUNESU_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"itunesu\"]";
  private static final String MYPAGES_XPATH =
      "com.dytech.edge.wizard.beans.control.CustomControl[classType=\"mypages\"]";
    ...
    if (fileSystemService.fileExists(idefFolder)) {
      ...
      for (String entry : xmlHelper.getXmlFileList(idefFolder)) {
        final XmlDocument xml =
            new XmlDocument(xmlHelper.readToPropBagEx(idefFolder, entry).toString());
  ...
  public static XStream createXStream(XmlHelper xmlHelper) {
    final XStream x =
        xmlHelper.createXStream(ReplaceDeletedControlsXmlMigration.class.getClassLoader());
  ...
  private static boolean replaceResourceSelector(XmlDocument xml, Node wizardNode, XStream x) {
    return replaceGeneric(
            ...
            new DefaultHandlerAttributesCallback<FakeCustomControl>("resourceHandler") {
              @Override
              public void mapHandlerAttributes(
                  CustomControl newControl, FakeCustomControl oldControl) {
  ...
  private static boolean replaceGoogleBooks(XmlDocument xml, Node wizardNode, XStream x) {
    return replaceGeneric(
            ...
            x,
            GOOGLEBOOK_XPATH,
            new DefaultHandlerAttributesCallback<FakeCustomControl>("googleBookHandler"),
            0)
        != 0;
  }
  ...
  private static boolean replaceYouTube(XmlDocument xml, Node wizardNode, XStream x) {
    return replaceGeneric(
            ...
            x,
            YOUTUBE_XPATH,
            new DefaultHandlerAttributesCallback<FakeCustomControl>("youTubeHandler"),
            0)
        != 0;
  }
  ...
  private static boolean replaceFlickr(XmlDocument xml, Node wizardNode, XStream x) {
    return replaceGeneric(
            ...
            x,
            FLICKR_XPATH,
            new DefaultHandlerAttributesCallback<FakeCustomControl>("flickrHandler"),
            0)
        != 0;
  }
  ...
  private static boolean replaceFileUpload(XmlDocument xml, Node wizardNode, XStream x) {
    final DefaultHandlerAttributesCallback<FakeFileControl> cb =
        new DefaultHandlerAttributesCallback<FakeFileControl>("fileHandler") {
          @Override
  ...
  private static boolean replaceLinks(XmlDocument xml, Node wizardNode, XStream x) {
    ...
    idx =
        replaceGeneric(
            ...
            x,
            LINKS_XPATH,
            new DefaultHandlerAttributesCallback<FakeMultiControl>("urlHandler", true),
            prevIdx);
    modified |= (idx != prevIdx);
    prevIdx = idx;
    ...
    idx =
        replaceGeneric(
            ...
            x,
            LINK_XPATH,
            new DefaultHandlerAttributesCallback<FakeEditBox>("urlHandler", false),
            prevIdx);
    modified |= (idx != prevIdx);
    prevIdx = idx;
    ...
    idx =
        replaceGeneric(
            ...
            x,
            SINGLE_LINK_XPATH,
            new DefaultHandlerAttributesCallback<FakeEmptyControl>("urlHandler", false),
            prevIdx);
    modified |= (idx != prevIdx);
  ...
  private static boolean replaceMyPages(XmlDocument xml, Node wizardNode, XStream x) {
    return replaceGeneric(
            ...
            x,
            MYPAGES_XPATH,
            new DefaultHandlerAttributesCallback<FakeCustomControl>("mypagesHandler"),
            0)
        != 0;
  }
  ...
  private static <T extends FakeEmptyControl> int replaceGeneric(
      XmlDocument xml,
      Node wizardNode,
      XStream x,
      String controlXpath,
      HandlerAttributesCallback<T> callback,
      int schemaNodeUniqueIndexStart) {
    ...
    while (it.hasNext()) {
      ...
      final FakeEmptyControl oldControl =
          (FakeEmptyControl) x.fromXML(XmlDocument.xmlToString(oldControlXml));
      ...
      final Node newControlXml =
          parentXml
              .getOwnerDocument()
              .importNode(
                  new XmlDocument(x.toXML(newControl))
                      .node("com.dytech.edge.wizard.beans.control.CustomControl"),
                  true);
  ...
  private static class DefaultHandlerAttributesCallback<T extends FakeEmptyControl>
      implements HandlerAttributesCallback<T> {
    protected final String handlerType;
    protected final String xpathModifier;
    protected boolean multiple;

    protected DefaultHandlerAttributesCallback(
        String handlerType, String xpathModifier, boolean multiple) {
  ...
  private static void switcheroo(
      XmlDocument xml, String nodeToSwitch, Node newParent, Node oldParent) {
