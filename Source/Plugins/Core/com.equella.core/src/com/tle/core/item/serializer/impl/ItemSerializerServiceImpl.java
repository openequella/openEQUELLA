...
public class ItemSerializerServiceImpl implements ItemSerializerService {
  ...
  @Inject private PluginTracker<ItemSerializerProvider> providerTracker;
  ...
  public ItemSerializerXml createXmlSerializer(
    ...
    ItemSerializerState state =
        createState(new ItemIdsWhereClause(itemIds), categories, privileges, false, false);
  ...
  public ItemSerializerItemBean createItemBeanSerializer(
    ...
    return new SimpleItemSerializerItemBean(
        createState(where, categories, privileges, false, export));
  }
      ...
      for (Object collectionObj : itemStatuses) {
        ...
        if (!Check.isEmpty(itemStatus.getComments())) {
          ...
          for (WorkflowMessage wrkflwMsg : itemStatus.getComments()) {
            ...
            ItemNodeStatusMessageBean insmb =
                new ItemNodeStatusMessageBean(
                    msgType, ub, wrkflwMsg.getMessage(), wrkflwMsg.getDate());
            insmbs.add(insmb);
          }
  ...
  protected ItemSerializerState createState(
    ...
    List<Map<String, Object>> itemQueryResults =
        itemService.queryItems(state.getItemQuery(), state.getFirstResult(), state.getMaxResults());
    ...
    if (!privs.isEmpty()) {
      if (state.isIgnorePrivileges()) {
        ...
        for (Long itemId : state.getItemKeys()) {
          ...
          if (!owner) {
            Collection<String> collabs =
                (Collection<String>) itemData.get(ItemSerializerState.COLLAB_ALIAS);
            owner = collabs != null && collabs.contains(currentUserId);
          }
          Collection<String> metadataTargets =
              (Collection<String>) itemData.get(ItemSerializerState.SECURITY_ALIAS);
          SimpleItemSecurity itemSecurity =
              new SimpleItemSecurity(itemId, status, collectionId, metadataTargets, owner);
          securityObjects.add(itemSecurity);
        }
        Builder<String, Long> privMultimap = ImmutableSetMultimap.builder();
        Map<SimpleItemSecurity, Map<String, Boolean>> privMap =
            aclManager.getPrivilegesForObjects(privs, securityObjects);
        for (Entry<SimpleItemSecurity, Map<String, Boolean>> securityPrivEntry :
            privMap.entrySet()) {
  ...
  private final ItemSerializerXml EMPTY_ITEM_SERIALIZER_XML =
      new ItemSerializerXml() {
        @Override
        public void writeXmlResult(XMLStreamer xml, long itemId) {
  ...
  private final ItemSerializerItemBean EMPTY_ITEM_SERIALIZER_ITEM_BEAN =
      new ItemSerializerItemBean() {
        @Override
        public void writeItemBeanResult(EquellaItemBean equellaItemBean, long itemId) {
          throw new UnsupportedOperationException("This serialiser assumes there are no items");
        }
