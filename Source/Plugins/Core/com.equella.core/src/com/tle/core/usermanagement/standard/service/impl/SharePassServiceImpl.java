...
public class SharePassServiceImpl implements SharePassService {
  @Inject private SharePassDao dao;
  @Inject private TLEAclManager aclManager;
  @Inject private UserService userService;

  @Override
  public boolean isEnabled() {
    return userService.isWrapperEnabled(
        "com.tle.beans.usermanagement.standard.wrapper.SharePassSettings");
  }
  ...
  public String activatePasses(String token) {
    ...
    for (SharePass other : dao.findAllByCriteria(c1, c2, c3)) {
      if (!other.isActivated() && isValidDateRange(other)) {
        ...
        aclManager.addAccessEntry(
            other.getItem(),
            Node.ITEM,
            true,
            false,
            other.getPrivilege().getPriv(),
            expression,
            other.getExpiry());
