...
public class WorkflowDaoImpl extends AbstractEntityDaoImpl<Workflow> implements WorkflowDao {
  ...
  public int getCommentCount(ItemKey itemKey) {
    return ((Number)
            getHibernateTemplate()
                .findByNamedParam(
                    "select count(*) " + getMessageHQL(),
                    new String[] {"uuid", "version", "inst"},
                    new Object[] {
                      itemKey.getUuid(), itemKey.getVersion(), CurrentInstitution.get()
                    })
                .get(0))
        .intValue();
  }
  ...
  public int getItemCountForWorkflow(String uuid) {
    return ((Number)
            getHibernateTemplate()
                .findByNamedParam(
                    "SELECT count(*) FROM Item i WHERE i.itemDefinition.workflow.uuid = :uuid "
                        + "AND i.moderating = true "
                        + "AND i.institution = :inst ",
                    new String[] {"uuid", "inst"},
