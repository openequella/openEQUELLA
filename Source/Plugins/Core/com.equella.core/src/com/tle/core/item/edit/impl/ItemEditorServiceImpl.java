...
public class ItemEditorServiceImpl implements ItemEditorService {
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public ItemEditor newItemEditor(
      ...
      if (version != 0) {
        if (existingVersions.contains(version)) {
          throw new ItemEditingException(
              "Version " + version + " of the item '" + uuid + "' already exists");
        }
        if (version != latestVersion + 1) {
          throw new ItemEditingException(
              "Item '"
                  ...
                  + "' has "
                  + latestVersion
                  + " version(s), you must create version "
                  + (latestVersion + 1)
                  + " or use 0");
        }
      } else {
  ...
  @Transactional(propagation = Propagation.MANDATORY)
  @Override
  public ItemEditor importItemEditor(
      String collectionUuid,
      ItemKey itemKey,
      String stagingUuid,
      List<ItemDeserializerEditor> deserializerEditors) {
