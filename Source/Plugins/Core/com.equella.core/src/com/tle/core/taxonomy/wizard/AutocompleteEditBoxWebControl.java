...
public class AutocompleteEditBoxWebControl
    extends BaseTermSelectorDisplayDelegate<
        AutocompleteEditBoxWebControl.AutocompleteEditBoxModel> {
  ...
  public AutoCompleteTermResult[] autocomplete(SectionInfo info) {
    ...
    final Pair<Long, List<TermResult>> res =
        taxonomyService.searchTerms(
            definitionControl.getSelectedTaxonomy(),
            query,
            selection,
            RESULT_LIMIT,
            searchFullTerm);
    ...
    Collections.sort(
        terms,
        new NumberStringComparator<TermResult>() {
    ...
    for (TermResult term : terms) {
      ...
      suggestTerms.add(
          new AutoCompleteTermResult(
              term.getTerm() + " - " + result.toString(), result.toString()));
    }
  ...
  public void termSelected(SectionInfo info) {
    ...
    if (!Check.isEmpty(fullTermPath)) {
      ...
      if (term == null) {
        if (definitionControl.isAllowAddTerms()
            && taxonomyService.supportsTermAddition(taxonomyUuid)) {
          try {
          ...
          && definitionControl.getSelectionRestriction() == SelectionRestriction.LEAF_ONLY) {
      ...
      model.setSelectedTerms(
          Collections.singleton(
              definitionControl.getTermStorageFormat() == TermStorageFormat.LEAF_ONLY
                  ? termName
                  : fullTermPath));
    }
  }
  ...
  public void registered(String id, SectionTree tree) {
    ...
    if (definitionControl.getBooleanAttribute(
      ...
      selectTerm.setClickHandler(
          termWebControl.getReloadFunction(true, events.getEventHandler("termSelected")));
    } else {
      selectTerm.setClickHandler(
          ajax.getAjaxUpdateDomFunction(
              tree,
              new SimpleSectionId(id),
              events.getEventHandler("termSelected"),
