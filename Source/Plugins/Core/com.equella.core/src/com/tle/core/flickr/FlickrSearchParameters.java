...
/**
 * @author larry A minor extension to the flickr searchParameters class, wherein we provide a
 *     general purpose string for the user identifier, and determine later if the identifier is a
 *     flickr userid (which is a prerequisite for the flickr search. or a flickr username or flickr
 *     user's email address. If either of the latter, a preliminary flickr query is required to get
 *     the flickr userid.
 */
public class FlickrSearchParameters extends SearchParameters {
  ...
  /**
   * Unvalidated text which identifies the flickr user (either by flickrid, email or username)
   *
   * @return
   */
  ...
  /**
   * Raw search text which will either be set as the general text, or as tags only.
   *
  ...
  /**
   * By default tagsNotText and tagsAll are false, requires that user explicitly set (and that
   * EQUELLA correctly records) that a tags-only search is requested.
   *
   * @return the tags
   */
