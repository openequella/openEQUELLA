...
public class MerlotServiceImpl implements MerlotService {
  ...
  /**
   * Fixed at 10 since the advanced API only supports pages of 10 (whereas the Basic API supports
   * offsets and results counts, go figure)
   */
  ...
  private static final String MERLOT_TECHNICALFORMATS_URL =
      MERLOT_BASE_URL + "technicalFormats.rest";
  ...
  @Override
  public SearchResults<MerlotSearchResult> search(
      MerlotSearchParams search, int offset, int perpage) {
    ...
    final StringBuilder buildUrl =
        new StringBuilder(advanced ? MERLOT_ADVANCED_URL : MERLOT_BASIC_URL);
    ...
    try (Response response =
      ...
      final int available =
          (advanced ? Integer.parseInt(xml.getNode("nummaterialstotal")) : resultCount);
      ...
      int counter = 0;
      final PropBagThoroughIterator pti =
          (advanced ? xml.iterateAll("material") : xml.iterateAll("results/material"));
      for (PropBagEx r : pti) {
  ...
  private void appendParam(StringBuilder buildUrl, String paramName, String paramValue) {
    if (!Check.isEmpty(paramValue)) {
      buildUrl
          ...
          .append(paramName)
          .append("=")
          .append(URLUtils.basicUrlEncode(paramValue));
    }
  }
    ...
    if (advanced) {
      if (xml.getNodeName().equals("invalid")) {
        throw new RuntimeException(
            CurrentLocale.get("com.tle.core.remoterepo.merlot.error.search", xml.getNode("")));
      }
    } else {
      ...
      if (!STATUS_OK.equals(status)) {
        ...
        throw new RuntimeException(
            CurrentLocale.get(
                "com.tle.core.remoterepo.merlot.error.search", code + " - " + errorMessage));
      }
    }
  }
  ...
  private List<NameValue> getSimpleNameValues(
      String endpoint, String objectNode, String nameNode, String valueNode) {
    try (Response response =
        httpService.getWebContent(new Request(endpoint), configService.getProxyDetails())) {
      final PropBagEx xml = new PropBagEx(response.getBody());
  ...
  public List<NameValue> getCommunities(FederatedSearch merlotSearch) {
    return getSimpleNameValues(
        serviceUrl(merlotSearch, MERLOT_COMMUNITIES_URL), "community", "", "@id");
  }

  @Override
  public List<NameValue> getLanguages(FederatedSearch merlotSearch) {
    return getSimpleNameValues(
        serviceUrl(merlotSearch, MERLOT_LANGUAGES_URL), "language", "", "@code");
  }

  @Override
  public List<NameValue> getMaterialTypes(FederatedSearch merlotSearch) {
    return getSimpleNameValues(
        serviceUrl(merlotSearch, MERLOT_MATERIALTYPES_URL), "materialtype", "", "");
  }

  @Override
  public List<NameValue> getTechnicalFormats(FederatedSearch merlotSearch) {
    return getSimpleNameValues(
        serviceUrl(merlotSearch, MERLOT_TECHNICALFORMATS_URL), "technicalformat", "", "");
  }

  @Override
  public List<NameValue> getAudiences(FederatedSearch merlotSearch) {
    return getSimpleNameValues(
        serviceUrl(merlotSearch, MERLOT_MATERIALAUDIENCES_URL), "materialaudience", "", "");
  }
}
