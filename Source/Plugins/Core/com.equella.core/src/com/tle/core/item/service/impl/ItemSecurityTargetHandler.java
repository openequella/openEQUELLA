...
public class ItemSecurityTargetHandler implements SecurityTargetHandler {
  @Inject private ItemStatusAndMetadataSecurityTargetHandler itemStatusAndMetadataHandler;
  ...
  public void gatherAllLabels(Set<String> labels, Object target) {
    ...
    itemStatusAndMetadataHandler.gatherAllLabels(
        labels, new ItemStatusTarget(item.getStatus(), itemDef));

    if (item.getMetadataSecurityTargets() != null) {
      for (String extra : item.getMetadataSecurityTargets()) {
        itemStatusAndMetadataHandler.gatherAllLabels(
            labels, new ItemMetadataTarget(extra, itemDef));
      }
    }
  }

  private String getDynamicItemMetadataLabel(Item item) {
    return TARGET_DYNAMIC_ITEM_METADATA
        + ":"
        + item.getId()
        + ":"
        + item.getItemDefinition().getId();
  }
