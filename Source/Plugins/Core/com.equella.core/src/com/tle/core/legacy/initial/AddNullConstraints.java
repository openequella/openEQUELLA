...
public class AddNullConstraints extends AbstractHibernateMigration {
  private static final String keyPrefix =
      PluginServiceImpl.getMyPluginId(AddNullConstraints.class) + ".addnulls."; // $NON-NLS-1$
  ...
  public void migrate(MigrationResult status) throws Exception {
    ...
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "access_entry", "expression_id", "institution_id"));
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session,
            "activate_request",
    ...
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "base_entity", "date_created", "date_modified", "uuid", "institution_id"));
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "bookmark", "owner", "institution_id", "item_id"));
    ...
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(session, "hierarchy_topic", "uuid", "institution_id"));
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "institution", "admin_password", "badge_url", "short_name", "name"));
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "item", "owner", "institution_id", "item_definition_id"));
    ...
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session, "language", "country", "language", "variant", "institution_id"));
    ...
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(session, "relation", "first_item_id", "second_item_id"));
    sql.addAll(
        helper.getAddNotNullSQLIfRequired(
            session,
            "share_pass",
    ...
    if (extDialect.requiresNoConstraintsForModify()) {
      ...
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns("base_entity", "uuid", "institution_id"));
      sql.addAll(helper.getAddIndexesAndConstraintsForColumns("bookmark", "institution_id"));
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns(
              "hierarchy_topic", "uuid", "institution_id"));
      ...
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns("mime_entry", "type", "institution_id"));
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns("tlegroup", "uuid", "institution_id"));
