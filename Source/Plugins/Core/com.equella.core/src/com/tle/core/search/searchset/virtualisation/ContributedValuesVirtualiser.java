...
public class ContributedValuesVirtualiser implements SearchSetVirtualiser {
  @Inject private FreetextIndex freetext;
  @Inject private SearchSetService searchSetService;

  @Override
  public <T> void expandSearchSet(
    ...
    final LocalSearch ls =
        new LocalSearch(
            searchSetService.getFreetextQuery(set),
    ...
    for (Pair<String, Integer> valueCount :
        freetext.facetCount(ls, Collections.singletonList(virtPath)).get(virtPath)) {
  ...
  /**
   * Only if the searchSet is related to hierarchy topics is it possible to have parents (ie
   * excludes DynamicCollections) and if this hierarchy does in fact have parents, and those parents
   * have mapped values (which if extant will correspond to that parent's virtualisation path) add
   * them as key/values to the search. In other words, the search must remain confined by the
   * virtualisation path/values of its hierarchical parents
   */
  private <T> void checkAncestorHierarchyVirtualisation(
      LocalSearch ls, SearchSet set, Map<String, String> mappedValues) {
