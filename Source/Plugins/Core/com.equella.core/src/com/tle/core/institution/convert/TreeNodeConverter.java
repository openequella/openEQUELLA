...
public abstract class TreeNodeConverter<T extends TreeNodeInterface<T>>
    extends AbstractConverter<Object> implements TreeNodeCreator<T> {
  ...
  public void exportIt(
    ...
    final DefaultMessageCallback message =
        new DefaultMessageCallback("institutions.converter.generic.genericmsg"); // $NON-NLS-1$
    message.setType(getNodeClass().getSimpleName().toLowerCase());
  ...
  public void importIt(
    ...
    final ImportHandler<T> importHandler =
        fileSystemService.fileExists(nodesFolder, oldSingleFilename)
            ? new SingleTreeNodeFileImportHandler<T>(
                nodesFolder, oldSingleFilename, xmlHelper, this, getXStream())
  ...
  protected static class TreeInitialiserCallback<T extends TreeNodeInterface<T>>
      implements InitialiserCallback {
    @Override
