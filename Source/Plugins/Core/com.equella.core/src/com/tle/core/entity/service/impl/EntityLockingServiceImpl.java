...
@Singleton
public class EntityLockingServiceImpl
    implements EntityLockingService, UserChangeListener, UserSessionLogoutListener {
  @Inject private EntityLockingDao entityLockDao;
  ...
  public EntityLock lockEntity(BaseEntity entity) {
    ...
    if (lock != null) {
      throw new LockedException(
          "Entity is already locked", lock.getUserID(), lock.getUserSession(), entity.getId());
    } else {
  ...
  public void unlockEntity(BaseEntity entity, boolean force) {
    ...
    if (lock != null) {
      if (force || CurrentUser.getSessionID().equals(lock.getUserSession())) {
        ...
        throw new LockedException(
            "Entity is locked by another session",
            lock.getUserID(),
            lock.getUserSession(),
            entity.getId());
      }
  ...
  public EntityLock getLock(BaseEntity entity) {
    ...
    if (lock != null) {
      if (!CurrentUser.getSessionID().equals(lock.getUserSession())) {
        throw new LockedException(
            CurrentLocale.get(entity.getName()) + " is locked by another session",
            lock.getUserID(),
            lock.getUserSession(),
            entity.getId());
      }
  ...
  public EntityLock getLock(BaseEntity entity, String lockId) {
    ...
    if (lock != null) {
      if (lockId == null) {
        throw new LockedException(
            "Entity is locked by another user '" + lock.getUserID() + "'",
            lock.getUserID(),
            lock.getUserSession(),
            entity.getId());
      }
      if (!lockId.equals(lock.getUserSession())) {
        throw new LockedException(
            "Wrong lock id. Entity is locked by user.",
            lock.getUserID(),
            lock.getUserSession(),
            entity.getId());
      }
  ...
  /**
   * From REST calls, we may not care about who or how a lock was set- just return it if it exists
   */
  @Override
  @Transactional
