...
public class TermServiceImpl implements TermService {
  ...
  @Inject private XmlHelper xmlHelper;

  @Override
  public List<String> listTerms(Taxonomy taxonomy, String parentFullPath) {
    return Check.isEmpty(parentFullPath)
        ? termDao.getRootTermValues(taxonomy)
  ...
  public List<TermResult> listTermResults(Taxonomy taxonomy, String parentFullPath) {
    return Check.isEmpty(parentFullPath)
        ? termDao.getRootTermResults(taxonomy)
  ...
  @Override
  @SecureOnCall(priv = SecurityConstants.EDIT_TAXONOMY)
  @Transactional(propagation = Propagation.REQUIRED)
    ...
    if (parent != null || root) {
      ...
      final String fullPathNoRoot =
          (parentFullPath.startsWith(TaxonomyConstants.TERM_SEPARATOR)
                  ? parentFullPath.substring(TaxonomyConstants.TERM_SEPARATOR.length())
                  : parentFullPath)
  ...
  public void validateTerm(
      Taxonomy taxonomy, String parentFullPath, String term, boolean requireParent)
      throws InvalidDataException {
    validateTerm(
        taxonomy, termDao.getTerm(taxonomy, parentFullPath), parentFullPath, term, requireParent);
  }

  private void validateTerm(
      Taxonomy taxonomy, Term parent, String parentFullPath, String term, boolean requireParent)
      throws InvalidDataException {
  ...
  public void move(Taxonomy taxonomy, TermResult termToMove, TermResult parentTerm, int index) {
    ensureLocked(taxonomy);
    termDao.move(
        getTermByUuid(taxonomy, termToMove.getUuid()),
        getTermByUuid(taxonomy, parentTerm.getUuid()),
        index);
  }
  ...
  public void doExport(
    ...
    message.setType(resources.getString("type.term"));
  ...
  public void doImport(
      ...
      final SubTemporaryFile termFolder,
      Institution institution,
      ConverterParams params) {
    final List<String> termFiles = xmlHelper.getXmlFileListOrdered(termFolder);

    final DefaultMessageCallback part1message =
        new DefaultMessageCallback(resources.key("converter.readingterms.progressmessage"));
    ...
    final DefaultMessageCallback part2message =
        new DefaultMessageCallback(resources.key("converter.savingterms.progressmessage"));
  ...
  protected void addToChildTermMap(
      Multimap<WrappedTerm, WrappedTerm> childTermMap,
      WrappedTerm parentWrappedTerm,
      WrappedTerm wrappedTerm) {
    parentWrappedTerm.incTotalChildCount();
    // trickle up the tree
    for (WrappedTerm p = parentWrappedTerm.getParentWrappedTerm();
        p != null;
        p = p.getParentWrappedTerm()) {
  ...
  protected void processTermChildren(
      final WrappedTerm parentWrappedTerm,
      final Multimap<WrappedTerm, WrappedTerm> childTermMap,
  ...
  /**
   * For storing in the HashMap. When the id of the term changes upon save, don't want the hash code
   * to bugger up.
   */
