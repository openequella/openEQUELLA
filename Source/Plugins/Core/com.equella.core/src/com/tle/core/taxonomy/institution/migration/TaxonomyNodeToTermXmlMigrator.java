...
import com.tle.web.viewurl.attachments.impl.ItemNavigationServiceImpl;

/**
 * Changes com.tle.beans.tree.TaxonomyNode to com.tle.core.taxonomy.schema.TaxonomyNode within the
 * taxonomy XML file(s), then it converts TaxonomyNodes into Terms and writes the XML out into the
 * taxonomy2/terms/TAXUUID/ folder where they will be expected to be found in the future.
 *
 * @author aholland
 */
@Bind
...
public class TaxonomyNodeToTermXmlMigrator extends XmlMigrator
    implements TreeNodeCreator<TaxonomyNode> {
  private static final String OLD_TAXONOMY_NODE_CLASS =
      "com.tle.beans.tree.TaxonomyNode"; //$NON-NLS-1$

  private static PluginResourceHelper r =
      ResourcesService.getResourceHelper(TaxonomyNodeToTermXmlMigrator.class);
  ...
  @Override
  public void execute(
      TemporaryFileHandle staging, InstitutionInfo instInfo, final ConverterParams params)
      throws IOException {
    final SubTemporaryFile sourceFolder =
        new SubTemporaryFile(staging, PRE41_TAXONOMY_EXPORT_FOLDER);
    final SubTemporaryFile taxonomiesFolder = new SubTemporaryFile(staging, TAXONOMY_EXPORT_FOLDER);

    final ImportHandler<TaxonomyNode> importHandler =
        (fileSystemService.fileExists(sourceFolder, TaxonomyConstants.SINGLE_TAXONOMIES_FILE)
            ? new SingleTreeNodeFileImportHandler<TaxonomyNode>(
                sourceFolder,
    ...
    DefaultMessageCallback message =
        new DefaultMessageCallback(r.key("converter.nodetoterm.progressmessage"));
    params.setMessageCallback(message);
    message.setType(r.getString("type.taxonomynode"));
    ...
    for (final Iterator<TaxonomyNode> iter = importHandler.iterateNodes(); iter.hasNext(); ) {
      ...
      if (node.getParent() == null) {
        writeTaxonomyXmlFileFromRootNode(
            taxonomiesFolder, instInfo.getInstitution(), node, nodeId2Taxonomy);
      } else {
        writeTermXmlFileFromNode(
            taxonomiesFolder,
            node,
            instInfo.getInstitution(),
            params,
            nodeId2Term,
            nodeId2Taxonomy,
            p);
  ...
  protected void writeTaxonomyXmlFileFromRootNode(
      final SubTemporaryFile taxonomiesFolder,
      final Institution institution,
      final TaxonomyNode node,
      Map<Long, Taxonomy> nodeId2Taxonomy) {
    ...
    final SubTemporaryFile taxonomyTermsFolder =
        new SubTemporaryFile(actualTaxonomyFolder, TERMS_EXPORT_FOLDER);
    xmlHelper.writeExportFormatXmlFile(taxonomyTermsFolder, true);
    nodeId2Taxonomy.put(node.getId(), taxonomy);
  }

  protected void writeTermXmlFileFromNode(
      final SubTemporaryFile taxonomiesFolder,
      final TaxonomyNode node,
      Institution institution,
      ConverterParams params,
      final Map<Long, Term> nodeId2Term,
      Map<Long, Taxonomy> nodeId2Taxonomy,
      Progress progress) {
