...
public class ItemConverter extends AbstractConverter<ItemConverter.ItemConverterInfo> {
  ...
  public void importIt(
      ...
      String cid)
      throws IOException {
    final SubTemporaryFile allImportItemsFolder =
        new SubTemporaryFile(staging, ITEMS_EXPORT_IMPORT_FOLDER);
          ...
          new Runnable() {
            ...
            public void run() {
              try {
                ...
                // data folder
                final SubTemporaryFile importItemFolder =
                    new SubTemporaryFile(allImportItemsFolder, dataFolderPath);
                ...
                ItemConverterInfo info =
                    new ItemConverterInfo(item, itemFolder, sharedState, params);
                ...
                runExtras(
                    info,
                    getXStream(),
                    new SubTemporaryFile(allImportItemsFolder, dataFolderPath + "-extra"),
                    true);
                itemDao.flush();
                itemDao.clear();
  ...
  void runExtras(
      ItemConverterInfo info, XStream xs, SubTemporaryFile extrasFolder, boolean doImport)
      throws IOException {
  ...
  private void changeHardcodedUrls(ConverterParams params, FileHandle folder) throws IOException {
    ...
    for (final String filePath : paths) {
      ...
      try (UnicodeReader reader =
          new UnicodeReader(fileSystemService.read(folder, filePath), Constants.UTF8)) {
      ...
      final String newHtml =
          changeHardcodedUrls(infile, params.getOldServerURL(), params.getCurrentServerURL());
      if (newHtml != null) {
  ...
  protected void storeXml(
      ConverterParams params, Item item, ItemFile file, Institution institution) {
  ...
  public void exportIt(
      ...
      String cid)
      throws IOException {
    final SubTemporaryFile allExportedItemsFolder =
        new SubTemporaryFile(staging, ITEMS_EXPORT_IMPORT_FOLDER);
          ...
          new Runnable() {
            ...
            public void run() {
              try {
                ...
                final SubTemporaryFile exportedItemFolder =
                    new SubTemporaryFile(bucketFolder, Long.toString(itemId));
                // _ITEM folder (item specific)
                final SubTemporaryFile exportedSystemItemFolder =
                    new SubTemporaryFile(exportedItemFolder, SYSTEM_ITEM_FOLDER);
                ...
                if (fileSystemService.fileExists(itemFile)) {
                  if (attachments) {
                    ...
                    fileSystemService.copyToStaging(
                        new SubItemFile(itemFile, SYSTEM_ITEM_FOLDER),
                        exportedSystemItemFolder,
                        false);
                  }
                }

                xmlHelper.writeXmlFile(
                    bucketFolder, Long.toString(itemId) + ".xml", item, getXStream());
                runExtras(
                    info,
                    getXStream(),
                    new SubTemporaryFile(bucketFolder, Long.toString(itemId) + "-extra"),
                    false);
  ...
  void exportXml(
      ...
      Item item,
      SubTemporaryFile exportedSystemItemFolder,
  ...
  public static class ItemConverterInfo {
    public ItemConverterInfo(
        Item item, FileHandle itemFolder, Map<Object, Object> state, ConverterParams params) {
      this.item = item;
  ...
  public void setPluginService(PluginService pluginService) {
    ...
    itemExtrasTracker =
        new PluginTracker<ItemExtrasConverter>(
            pluginService,
            "com.tle.core.item.convert",
            "itemExtrasConverter",
            null,
  ...
  public interface ItemExtrasConverter {
    void importExtras(ItemConverterInfo info, XStream xstream, SubTemporaryFile extrasFolder)
        throws IOException;

    void exportExtras(ItemConverterInfo info, XStream xstream, SubTemporaryFile extrasFolder)
        throws IOException;
  }
