...
public class DrmServiceImpl implements DrmService, ItemDeletedListener, UserChangeListener {
  ...
  private final Cache<String, Boolean> needsRights =
      CacheBuilder.newBuilder().softValues().expireAfterAccess(30, TimeUnit.SECONDS).build();

  @Inject private DrmAcceptanceDao dao;
  ...
  @Override
  public Pair<Long, List<DrmAcceptance>> enumerateAgreements(
  ...
  @Override
  public boolean requiresAcceptanceCheck(
      ItemKey key, boolean isSummaryPage, boolean viewedInComposition) {
  ...
  @Override
  public DrmSettings requiresAcceptance(
  ...
  private DrmSettings realAcceptanceCheck(
      Item item, boolean isSummaryPage, boolean viewedInComposition) {
    ...
    try {
      ...
      if (rights.getMaximumUsageCount() > 0
          && countAgreements(item) >= rights.getMaximumUsageCount()) {
        throw new DRMException(
            "This item has been licenced the maximum number" + " of allowable times.");
      }
      return rights;
    }
  }
  ...
  @Override
  public boolean hasAcceptedOrRequiresNoAcceptance(
      Item item, boolean isSummaryPage, boolean viewedInComposition) {
  ...
  public void isAuthorised(Item item, String ipaddress) {
    ...
    if (!checkForUserOrGroup(rights)) {
      throw new DRMException(
          "User is not a member of the individuals or groups" + " allowed to view this item.");
    }
    ...
    if (!checkForNetwork(rights, ipaddress)) {
      throw new DRMException(
          "User is not coming from an IP address that this" + " item is restricted to.");
    }
  }
  ...
  public void mergeSettings(DrmSettings settings, DRMPage page) {
    ...
    if (!contributor.isDatetime()) {
      ...
      if (acceptStart != null) {
        settings.setRestrictedToDateRange(
            new Pair<Date, Date>(acceptStart, container.getAcceptEnd()));
      } else {
  ...
  public boolean isReferredFromDifferentItem(HttpServletRequest request, ItemKey itemId) {
    ...
    if (refer.indexOf("/file/") < 0
        && refer.indexOf("/integ/") < 0
        && refer.indexOf("/preview/") < 0
        && refer.indexOf("/item/") < 0
        && refer.indexOf("/items/") < 0) {
      return false;
    }
  ...
  public boolean isReferredFromSamePackage(HttpServletRequest request, ItemKey itemId) {
    ...
    if (refer != null) {
      ...
      if (refer.indexOf("/file/") >= 0
        ...
        if (refer.indexOf(urlPart + "/viewims.jsp") >= 0
            || refer.indexOf(urlPart + "/treenav.jsp") >= 0
