...
public class RecentContribPortletService implements PortletServiceExtension {
  private static final String KEY_ERROR_VALIDATION_AGE =
      "com.tle.core.portal.recent.age.error"; //$NON-NLS-1$

  @Inject private PortletDao portletDao;

  @Override
  public void loadExtra(Portlet portlet) {
    PortletRecentContrib contrib =
        portletDao.findAnyById(PortletRecentContrib.class, portlet.getId());
  ...
  public void doValidation(PortletEditingBean newPortlet, List<ValidationError> errors) {
    ...
    if (recent.getAgeDays() < 0) {
      errors.add(
          new ValidationError("age", CurrentLocale.get(KEY_ERROR_VALIDATION_AGE))); // $NON-NLS-1$
    }
  }
}
