...
public class EnsureForeignKeyIndexes2 extends AbstractHibernateMigration {
  private static final String keyPrefix =
      PluginServiceImpl.getMyPluginId(EnsureForeignKeyIndexes2.class)
  ...
  public void migrate(MigrationResult status) throws Exception {
    ...
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "bookmark_keywords", "bookkeywords", "bookmark_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "power_search_itemdefs",
    ...
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "hierarchy_topic_inh_schemas",
            new String[] {"htis_schema", "entity_id"},
            new String[] {"htis_topic", "hierarchy_topic_id"}));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "hierarchy_topic_all_parents", "htap_parent", "all_parents_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "hierarchy_topic_inh_idefs",
            new String[] {"htii_entity", "entity_id"},
            new String[] {"htii_topic", "hierarchy_topic_id"}));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "hierarchy_topic_add_idefs",
            new String[] {"htai_topic", "hierarchy_topic_id"},
            new String[] {"htai_entity", "entity_id"}));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
    ...
    sql.addAll(
        helper.getAddIndexesRawIfRequired(session, "tlegroup_users", "tleguGroup", "tlegroup_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "tlegroup_all_parents", "tlegap_parent", "all_parents_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(session, "item_collaborators", "ic_item", "item_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(session, "item_notifications", "in_item", "item_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "base_entity_attributes", "bea_entity", "base_entity_id"));
    ...
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "dyna_collection_item_defs",
            new String[] {"dcid_entity", "entity_id"},
            new String[] {"dcid_dyna", "dyna_collection_id"}));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "dyna_collection_schemas",
            new String[] {"dcs_dyna", "dyna_collection_id"},
            new String[] {"dcs_entity", "entity_id"}));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "dyna_collection_usage_ids", "dcui_dyna", "dyna_collection_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "schema_exp_transforms", "set_schema", "schema_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "schema_imp_transforms", "sit_schema", "schema_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(session, "schema_citations", "sc_schema", "schema_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "access_expression_expression_p", "aeep_aexp", "access_expression_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "workflow_node_users", "wnu_node", "workflow_node_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "workflow_node_groups", "wng_node", "workflow_node_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "workflow_node_roles", "wnr_node", "workflow_node_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "workflow_node_auto_assigns", "wnaa_node", "workflow_node_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
    ...
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "mime_entry_extensions", "mee_mime", "mime_entry_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session,
            "portlet_recent_contrib_collect",
            new String[] {"portrc_contrib", "portlet_recent_contrib_id"},
            new String[] {"portrc_collect", "collections_id"}));
    sql.addAll(helper.getAddIndexesIfRequired(session, "portlet_recent_contrib", "portrc_portlet"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cla_holding_ids", "clahi_holding", "cla_holding_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cla_holding_authors", "claha_holding", "cla_holding_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cla_portion_topics", "clapt_portion", "cla_portion_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cla_portion_authors", "clapa_portion", "cla_portion_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cal_holding_ids", "calhi_holding", "cal_holding_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cal_holding_authors", "calha_holding", "cal_holding_id"));

    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cal_portion_topics", "calpt_portion", "cal_portion_id"));
    sql.addAll(
        helper.getAddIndexesRawIfRequired(
            session, "cal_portion_authors", "calpa_portion", "cal_portion_id"));
  ...
  protected Class<?>[] getDomainClasses() {
    return new Class<?>[] {
      ...
      Relation.class,
      Bookmark.class,
      FakePortletRecentContrib.class,
      FakePortlet.class,
      FakeCLAHolding.class,
      DynaCollection.class,
      FakeCLAPortion.class,
      FakeCALPortion.class,
      FakeCALHolding.class,
      MimeEntry.class,
  ...
  public static class FakeCLAHolding {
    ...
    @Column(name = "element")
    @CollectionTable(
        name = "cla_holding_authors",
        joinColumns = @JoinColumn(name = "cla_holding_id"))
    List<String> authors;
  }
  ...
  public static class FakeCLAPortion {
    ...
    @Column(name = "element")
    @CollectionTable(
        name = "cla_portion_authors",
        joinColumns = @JoinColumn(name = "cla_portion_id"))
    List<String> authors;

    @ElementCollection
    @Column(name = "element")
    @CollectionTable(
        name = "cla_portion_topics",
        joinColumns = @JoinColumn(name = "cla_portion_id"))
    List<String> topics;
  }
  ...
  public static class FakeCALHolding {
    ...
    @ElementCollection
    @CollectionTable(
        name = "cal_holding_authors",
        joinColumns = @JoinColumn(name = "cal_holding_id"))
    @Column(name = "element")
    List<String> authors;
  }
  ...
  public static class FakeCALPortion {
    ...
    @Column(name = "element")
    @CollectionTable(
        name = "cal_portion_authors",
        joinColumns = @JoinColumn(name = "cal_portion_id"))
    List<String> authors;

    @ElementCollection
    @Column(name = "element")
    @CollectionTable(
        name = "cal_portion_topics",
        joinColumns = @JoinColumn(name = "cal_portion_id"))
    List<String> topics;
  }
}
