...
public abstract class AbstractEntityPrivilegeTreeProvider<T extends BaseEntity>
    implements PrivilegeTreeProvider {
   ...
   * @param allTargetsKey Key for ACL viewer target matching all of this entity.
   */
  protected AbstractEntityPrivilegeTreeProvider(
      AbstractEntityService<?, T> service,
      Node allNode,
      String allNodeKey,
      Node singularNode,
      String allTargetsKey) {
  ...
  public void gatherChildTargets(List<SecurityTarget> childTargets, SecurityTarget target) {
    if (target == null) {
      ...
      for (BaseEntityLabel bel : ts) {
        ...
        childTargets.add(
            new SecurityTarget(
                TextBundle.getLocalString(bel.getBundleId(), bundleCache, null, null),
                singularNode,
                entity,
                false));
      }
