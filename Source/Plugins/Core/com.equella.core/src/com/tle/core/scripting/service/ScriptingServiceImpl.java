...
@Singleton
public class ScriptingServiceImpl implements ScriptingService, RemoteScriptingService {
  @Inject
  @Named("scripting.safe")
  private boolean safeScripting;
  ...
  public ScriptContext createScriptContext(ScriptContextCreationParams params) {
    ...
    return new DefaultScriptContext(scriptObjects, userSriptObjects, xml, params.getSafeContext());
  }
  ...
  private Object executeInternal(
    ...
    if (!Check.isEmpty(script)) {
      if (safeScripting) {
        try {
          new BasicParser(new SafeScriptOptions(context.getSafeContext()), new StringReader(script))
              .importScript();
        } catch (InvalidScriptException ise) {
          throw new UnsafeScriptException("Advanced script not allowed");
        }
      }
