...
public class SearchSetServiceImpl implements SearchSetService {
  ...
  public void setPluginService(PluginService pluginService) {
    virtualisers =
        new PluginTracker<SearchSetVirtualiser>(
            pluginService,
            "com.tle.core.search",
            "searchSetVirtualiser",
            PluginTracker.LOCAL_ID_FOR_KEY);
    virtualisers.setBeanKey("class");
  }

  @Override
  public <T> List<VirtualisableAndValue<T>> expandSearchSets(
      Collection<T> objs,
    ...
    for (T ht : objs) {
      ...
      if (virtualiser == null) {
        ...
        virtualiser.expandSearchSet(
            virtualisedPathList, ht, searchSet, mappedValues, collectionUuids, helper);
      }
    }
  ...
  @Override
  public FreeTextBooleanQuery getSearchClauses(
      final SearchSet searchSet, final Map<String, String> virtualisationValues) {
  ...
  private void applyVirtualiser(
      FreeTextBooleanQuery andQuery, SearchSet searchSet, Map<String, String> values) {
  ...
  private FreeTextBooleanQuery getSearchClauses(
      final SearchSet searchSet, Gatherer itemDefGatherer, Gatherer schemaGatherer) {
  ...
  /** Gathers the schemas and where clauses for the topic, and recurses on the parent. */
  ...
  /** Gathers the item definitions and where clauses for the topic, and recurses on the parent. */
