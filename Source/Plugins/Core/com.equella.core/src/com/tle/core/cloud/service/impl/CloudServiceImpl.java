...
public class CloudServiceImpl implements CloudService, ItemResolverExtension {
  ...
  @Nullable private ObjectMapper jsonMapper;
  private final ExecutorService executor =
      new ThreadPoolExecutor(
          ...
          new NamedThreadFactory("CloudService"));
  ...
  private LoadingCache<String, CloudFilterInfo> filterCache =
      CacheBuilder.newBuilder()
          .expireAfterAccess(30, TimeUnit.MINUTES)
          .build(
              new CacheLoader<String, CloudFilterInfo>() {
                @Override
  ...
  private LoadingCache<String, Integer> resultCountCache =
      CacheBuilder.newBuilder()
  ...
  private LoadingCache<PagedCloudSearch, CloudSearchResultsInternal> resultCache =
      CacheBuilder.newBuilder()
          .expireAfterWrite(30, TimeUnit.MINUTES)
          .softValues()
          .build(
              new CacheLoader<PagedCloudSearch, CloudSearchResultsInternal>() {
                @Override
                public CloudSearchResultsInternal load(PagedCloudSearch pcs) throws Exception {
                  ...
                  final String response =
                      doQuery(
                          ...
                          sort,
                          reverse,
                          buildWhereClause(cloudSearch),
                          "basic,metadata,attachment,detail",
                          offset,
                          pcs.getLength());
                  final CloudSearchResultsBean csr =
                      getObjectMapper().readValue(response, CloudSearchResultsBean.class);

                  final List<CloudItem> itemResults =
                      Lists.newArrayList(
                          Lists.transform(
                              csr.getResults(),
                  ...
                  return new CloudSearchResultsInternal(
                      itemResults, csr.getLength(), offset, csr.getAvailable());
                }
              });
  ...
  public boolean isCloudy() {
    return httpService.canAccessInternet()
        && !configService.getProperties(new CloudSettings()).isDisabled();
  }
  ...
  @Override
  public CloudSearchResults search(
      final CloudSearch cloudSearch, final int offset, final int length) {
    try {
      ...
      if (results == null) {
        resultsFuture =
            executor.submit(
                new Callable<CloudSearchResultsInternal>() {
                  @Override
      ...
      return new CloudSearchResults(
          results.getResults(), results.getCount(), results.getOffset(), available, filtered);
    } catch (Throwable t) {
      throw Throwables.propagate(t);
    }
  }
  ...
  private CloudFilterInfo buildFiltersCache() throws Throwable {
    ...
    final FacetSearchWorkerThread lang =
        new FacetSearchWorkerThread(CloudConstants.LANGUAGE_PATH) {
          @Override
    ...
    final FacetSearchWorkerThread edu =
        new FacetSearchWorkerThread(CloudConstants.EDUCATION_LEVEL_PATH);
  ...
  private CloudFacetSearchResults facetSearch(String... schemaNodes) {
    try {
      ...
      final CloudFacetSearchResultsBean cfsr =
          getObjectMapper().readValue(response, CloudFacetSearchResultsBean.class);
  ...
  private class FacetSearchWorkerThread implements Callable<List<NameValue>> {
    ...
    public List<NameValue> call() {
      ...
      final List<NameValue> nameVals =
          new ArrayList<>(Lists.transform(searchResults.getResults(), getTransformer()));
      clean(nameVals);
  ...
  private static class CloudSearchResultsInternal extends SimpleSearchResults<CloudItem> {
    public CloudSearchResultsInternal(
        List<CloudItem> results, int count, int offset, int available) {
    ...
    public boolean equals(@Nullable Object obj) {
      if (obj == null || !(obj instanceof PagedCloudSearch)) {
        ...
        return offset == rhs.offset
            && length == rhs.length
            && Objects.equals(cloudSearch, rhs.cloudSearch);
      }
    }
  }
