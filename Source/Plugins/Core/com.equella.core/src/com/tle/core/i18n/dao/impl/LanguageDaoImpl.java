...
@Singleton
public class LanguageDaoImpl extends GenericInstitionalDaoImpl<Language, Long> implements LanguageDao
{
	private enum QueryType
	{
		CLOSEST("getClosest"), MORE_SPECIFIC("getMoreSpecific"), LEAST_SPECIFIC("getLeastSpecific"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	public Map<Long, String> getNames(final Collection<Long> bundleRefs)
	{
		...
		return (Map<Long, String>) getHibernateTemplate().execute(new HibernateCallback()
		{
			@Override
	...
	private Collection<Long> getMissingBundleRefs(Collection<Long> bundleRefs, Map<Long, String> nameMap)
	{
	...
	private void populateNameMap(final Session session, Collection<Long> bundleRefs, final QueryType queryType,
		final Map<Long, String> nameMap)
	{
		new CollectionPartitioner<Long, Object>(bundleRefs)
		{
			...
			public List<Object> doQuery(Session session, Collection<Long> collection)
			{
				...
				{
					case CLOSEST:
						query.setParameterList("locales", LocaleUtils.getAllPossibleKeys(locale)); //$NON-NLS-1$
						break;
	...
	public void deleteBundles(Collection<Long> bundles)
	{
		if( !bundles.isEmpty() )
		{
			...
			query.append(')');
			getHibernateTemplate().bulkUpdate("delete LanguageString where bundle.id in " + query.toString(), //$NON-NLS-1$
				bundles.toArray());
			getHibernateTemplate().bulkUpdate("delete LanguageBundle where id in " + query.toString(), //$NON-NLS-1$
				bundles.toArray());
		}
	}
}
