...
public class NotificationDaoImpl extends GenericInstitionalDaoImpl<Notification, Long>
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public int deleteAllForUserKeyReasons(
      final ItemKey itemKey, final String userId, final Collection<String> reasons) {
    return ((Number)
            getHibernateTemplate()
                .execute(
                    new HibernateCallback() {
  ...
  public boolean userIdChanged(
      final ItemKey itemKey, final String fromUserId, final String toUserId) {
    return (Boolean)
        getHibernateTemplate()
            .execute(
                new HibernateCallback() {
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public NotifiedUser getUserToNotify(
      final Date notAfter, final String attemptId, final boolean batched) {
    return (NotifiedUser)
        getHibernateTemplate()
            .execute(
                new HibernateCallback() {
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public List<Notification> getNewestNotificationsForUser(
      final int maximum,
      final String user,
      final Collection<String> reasons,
  ...
  public int markProcessed(
      final String user, final Collection<String> reasons, final String attemptId) {
    if (reasons.isEmpty()) {
      return 0;
    }
    return (Integer)
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public int markProcessedById(
      final String user, final Collection<Long> notifications, final String attemptId) {
    if (notifications.isEmpty()) {
      return 0;
    }
    return (Integer)
  ...
  @Override
  @Transactional(propagation = Propagation.MANDATORY)
  public int deleteUnindexedById(
      final String user, final Collection<Long> notifications, final String attemptId) {
    if (notifications.isEmpty()) {
      return 0;
    }
