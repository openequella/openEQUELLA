...
public interface EntityLockingService {
  ...
  /**
   * Does not attempt to perform a lock or see if we own it the existing one, just gets an existing
   * lock (if any)
   *
   * @param entity
   * @return
   ...
   * @param sessionId Optional. Is the entity locked by the supplied session ID?
   * @return
   */
