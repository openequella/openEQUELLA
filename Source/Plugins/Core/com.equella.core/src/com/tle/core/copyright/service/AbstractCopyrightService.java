...
public abstract class AbstractCopyrightService<
        H extends Holding, P extends Portion, S extends Section>
    implements CopyrightService<H, P, S>, ActivationImplementation {
  ...
  public AbstractCopyrightService(CopyrightDao<H, P, S> dao) {
    ...
    copyrightCache =
        CacheBuilder.newBuilder()
            ...
            .softValues()
            .build(
                new CacheLoader<ItemIdInst, Boolean>() {
                  @Override
  ...
  public Item getCopyrightedItem(ItemId itemId) {
    if (itemId != null
        && copyrightCache.getUnchecked(new ItemIdInst(itemId, CurrentInstitution.get()))) {
  ...
  public AgreementStatus getAgreementStatus(Item item, IAttachment attachment) {
    ...
    if (holding == null) {
      ...
      // really.
      // throw new IllegalStateException("This portion has no holding " +
  ...
  protected boolean isNeedsAgreement(
      ItemDefinition holdingCollection, Item item, IAttachment attachment) {
    if (Boolean.parseBoolean(holdingCollection.getAttributes().get(getHasAgreementAttribute()))) {
      if (!Boolean.TRUE.equals(
          userSessionService.getAttribute(getKeyForAttachment(item, attachment)))) {
        return true;
      }
    }
    return false;
  ...
  private String findAttachmentDescription(Item item, String attachmentUuid) {
    ...
    CombinedCollection<IAttachment> linksAndFiles =
        new CombinedCollection<>(
            attachments.getList(AttachmentType.FILE), attachments.getList(AttachmentType.LINK));
    Map<String, IAttachment> uuid2Attachment =
        UnmodifiableAttachments.convertToMapUuid(linksAndFiles);
  ...
  private PropBagEx getUnifiedXml(Item item) {
    ...
    return itemHelper.convertToXml(
        new ItemPack(item, xml, ""), // $NON-NLS-1$
