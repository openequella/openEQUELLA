...
public class EntityConverter extends AbstractConverter<BaseEntity> {
  ...
  public void doExport(
      TemporaryFileHandle staging, Institution institution, ConverterParams params) {
    ...
    for (AbstractEntityService<?, BaseEntity> service : services) {
      try {
        ...
        final List<BaseEntity> allEntities =
            dao.enumerateAll(new EnumerateOptions().setIncludeSystem(true));
  ...
  public void doDelete(Institution institution, ConverterParams params) {
    ...
    final List<AbstractEntityService<?, BaseEntity>> services = registry.getAllEntityServices();
    for (ListIterator<AbstractEntityService<?, BaseEntity>> iter =
    ...
    for (AbstractEntityService<?, BaseEntity> service : registry.getAllEntityServices()) {
      ...
      for (String entry : entries) {
        try {
          ...
          // data folder
          final SubTemporaryFile thisEntitiesImportFolder =
              new SubTemporaryFile(entityImportFolder, entry.replace(".xml", Constants.BLANK));
