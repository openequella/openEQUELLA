...
public class TaxonomyServiceImpl
    extends AbstractEntityServiceImpl<EntityEditingBean, Taxonomy, TaxonomyService>
    implements TaxonomyService,
        TaxonomyModifiedListener,
        ScriptObjectContributor,
        InstitutionListener {
  private final Cache<Institution, Cache<String, TaxonomyDataSource>> dataSourceCache =
      CacheBuilder.newBuilder().softValues().expireAfterAccess(1, TimeUnit.HOURS).build();
  ...
  protected void doValidation(
      EntityEditingSession<EntityEditingBean, Taxonomy> session,
      Taxonomy entity,
  ...
  private TaxonomyDataSource getDataSourceNoCache(final Taxonomy taxonomy) {
    final TaxonomyDataSourceFactory factory =
        dataSourceFactoryTracker.getBeanMap().get(taxonomy.getDataSourcePluginId());
  ...
  public void setPluginService(PluginService pluginService) {
    dataSourceFactoryTracker =
        new PluginTracker<TaxonomyDataSourceFactory>(
            pluginService,
  ...
  public TermResult addTerm(
    ...
    throw new IllegalOperationException(
        CurrentLocale.get("com.tle.core.taxonomy.service.error.cantinsert"));
  }
  ...
  public void addScriptObjects(Map<String, Object> objects, ScriptContextCreationParams params) {
    objects.put(
        TaxonomyServiceScriptObject.DEFAULT_VARIABLE, new TaxonomyServiceScriptWrapper(this));
  }
  ...
  protected void doAfterImport(
      TemporaryFileHandle taxonomyImportFolder,
  ...
  public void prepareExport(
      TemporaryFileHandle exportFolder, Taxonomy taxonomy, ConverterParams params) {
    ...
    final SubTemporaryFile termsFolder =
        new SubTemporaryFile(exportFolder, TaxonomyConstants.TERMS_EXPORT_FOLDER);

    termService.doExport(
        taxonomy, termsFolder, params.getInstituionInfo().getInstitution(), params);
  ...
  public void taxonomyModifiedEvent(TaxonomyModifiedEvent event) {
    synchronized (cacheLock) {
      final Cache<String, TaxonomyDataSource> instEntry =
          dataSourceCache.getIfPresent(CurrentInstitution.get());
      if (instEntry != null) {
