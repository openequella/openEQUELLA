...
public class DynaCollectionServiceImpl
    extends AbstractEntityServiceImpl<EntityEditingBean, DynaCollection, DynaCollectionService>
    implements DynaCollectionService, ItemDefinitionDeletionListener, SchemaDeletionListener {
  private final DynaCollectionDao dao;

  private static PluginResourceHelper helper =
      ResourcesService.getResourceHelper(DynaCollectionServiceImpl.class);
  private static final String ASSERT_USAGE_ERR_MSG =
      helper.key("dynacollection.assertusage"); // $NON-NLS-1$
  ...
  public List<VirtualisableAndValue<DynaCollection>> enumerateExpanded(String usage) {
    List<DynaCollection> unexpanded =
        Check.isEmpty(usage) ? dao.enumerateAll() : dao.enumerateForUsage(usage);
  ...
  public VirtualisableAndValue<DynaCollection> getByCompoundId(String compoundId) {
    ...
    if (deconstructedCompound.length == 1) {
      ...
      List<VirtualisableAndValue<DynaCollection>> expanded =
          searchSetService.expandSearchSets(
  ...
  protected void doValidation(
      EntityEditingSession<EntityEditingBean, DynaCollection> session,
      DynaCollection entity,
      List<ValidationError> errors) {
    // Nothing to validate
  }

  private final VirtualisationHelper<DynaCollection> virtualisationHelper =
      new VirtualisationHelper<DynaCollection>() {
        @Override
