...
public class ThumbnailRequestDaoImpl extends GenericInstitionalDaoImpl<ThumbnailRequest, Long>
    implements ThumbnailRequestDao {
  ...
  public List<ThumbnailRequest> list(Institution institution) {
    return findAllByCriteria(
        Order.desc("dateRequested"), -1, Restrictions.eq("institution", institution));
  }
  ...
  public List<ThumbnailRequest> list(Institution institution, ItemKey itemId) {
    return findAllByCriteria(
        Order.desc("dateRequested"),
        -1,
        Restrictions.eq("institution", institution),
        Restrictions.eq("itemUuid", itemId.getUuid()),
        Restrictions.eq("itemVersion", itemId.getVersion()));
  }

  @Transactional
  @Override
  public List<ThumbnailRequest> listForFile(
      Institution institution, ItemKey itemId, String filenameHash) {
    return findAllByCriteria(
        Order.desc("dateRequested"),
        -1,
        Restrictions.eq("institution", institution),
  ...
  @Transactional
  @Override
  public List<ThumbnailRequest> listForHandle(
  ...
  public ThumbnailRequest getByUuid(String requestUuid) {
    return findByCriteria(
        Restrictions.eq("uuid", requestUuid),
                  ...
                  public Object doInHibernate(Session session) throws HibernateException {
                    Query query =
                        session
                            ...
                            .setParameter("serialHandle", serialHandle)
  ...
  public void delete(Item item) {
    List<ThumbnailRequest> thumbsForItem =
        findAllByCriteria(
            Restrictions.eq("itemUuid", item.getUuid()),
            Restrictions.eq("itemVersion", item.getVersion()),
            Restrictions.eq("institution", CurrentInstitution.get()));
