...
public class ScriptNotificationMigration extends AbstractHibernateSchemaMigration {
  private static final String keyPrefix =
      PluginServiceImpl.getMyPluginId(ScriptNotificationMigration.class) + ".";
    ...
    List<String> creationSql =
        helper.getCreationSql(
            new TablesOnlyFilter(
                "wf_script_notify_completion_u",
                "wf_script_notify_completion_g",
                "wf_script_notify_error_u",
                "wf_script_notify_error_g"));
    creationSql.addAll(
        helper.getAddColumnsSQL("workflow_node", "notify_on_completion", "notify_on_error"));
    return creationSql;
  }
