...
public class ThumbnailServiceImpl implements ThumbnailService, ItemDeletedListener {
  ...
  @Inject private ThumbingCallableTracker taskTracker;

  @Transactional(propagation = Propagation.REQUIRED)
  @Override
  public String submitThumbnailRequest(
      ItemKey itemId, FileHandle handle, String filename, boolean forceIt, boolean clearPending) {
    ...
    if (thumbnailRequestService.exists(itemId, handle, filename)) {
      ...
      for (ThumbnailType thumbType : ThumbnailType.values()) {
        if (thumbGen.supportsThumbType(thumbType)
            && (forceIt
                || !fileSystemService.fileExists(handle, thumbFilename(filename, thumbType)))) {
      ...
      if (thumbFlags == 0) {
        ...
        thumbnailRequestService.newRequest(
            filename, itemId, handle, thumbFlags, forceIt, clearPending);
      }
    }
   ...
   * @return
   */
  public Task createThumbnailerTask(
      String requestUuid, long institutionId, ItemId itemId, String serialHandle) {
  ...
  public void itemDeletedEvent(ItemDeletedEvent event) {
    ...
    for (ThumbnailRequest request :
        thumbnailRequestService.list(CurrentInstitution.get(), itemId)) {
      deleteRequest(request);
    }
  }
