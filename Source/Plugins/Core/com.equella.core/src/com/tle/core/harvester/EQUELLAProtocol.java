...
public class EQUELLAProtocol extends AbstractHarvesterProtocol {
  ...
  public int setupAndRun(HarvesterProfile aProfile, boolean testOnly) throws Exception {
    ...
    soapService =
        clientFactory.createSoapClient(
            SoapHarvesterService.class, endpointUrl, "http://soap.harvester.core.tle.com");
  ...
  public Collection<LearningObject> getUpdatedLearningObjects(Date since) throws Exception {
    ...
    if (length == 1) {
      resultsXml =
          new PropBagEx(
              soapService.searchItemsSince(split, liveOnly, date.format(Dates.ISO_WITH_TIMEZONE)));
    } else if (length >= 2) {
      String dynaCol = split[1];
      String virtVal = (length == 2 ? "" : split[2]);
      resultsXml =
          new PropBagEx(
              soapService.searchDynamicCollection(
                  dynaCol, virtVal, date.format(Dates.ISO_WITH_TIMEZONE), liveOnly));
    } else
  ...
  public void downloadLO(LearningObject lobject, String stagingID) throws Exception {
    ...
    fileSystemService.unzipFile(
        new StagingFile(stagingID), downloadUrl.openStream(), ArchiveType.TAR_GZ);
  }
  ...
  public void postProcessing(PropBagEx xml, LearningObject lobject) throws Exception {
    ...
    PropBagEx harvestXml =
        new PropBagEx(
            soapService.getItemXml(
                oldXml.getNode("@id"), Integer.parseInt(oldXml.getNode("@version"))));
  ...
  public static final class EQUELLALearningObject extends LearningObject {
    ...
    public EQUELLALearningObject(
        String identifier, String title, Date created, boolean attachment) {
