...
public class CourseInfoServiceImpl
  ...
  protected void doValidation(
      EntityEditingSession<EntityEditingBean, CourseInfo> session,
      CourseInfo entity,
  ...
  public void prepareImport(
      TemporaryFileHandle importFolder, CourseInfo entity, ConverterParams params) {
  ...
  public List<CourseInfo> bulkImport(CsvReader reader, boolean override) {
    CourseDefaultsSettings persistedSettings =
        configurationService.getProperties(new CourseDefaultsSettings());
    final Date defaultFromDate =
        DateHelper.parseOrNullDate(persistedSettings.getStartDate(), Dates.CALENDAR_CONTROL_FORM);
    final Date defaultToDate =
        DateHelper.parseOrNullDate(persistedSettings.getEndDate(), Dates.CALENDAR_CONTROL_FORM);
    ...
    return new BulkImport<CourseInfo>() {
      ...
      protected void processOne(
          CsvReader reader, boolean override, java.util.List<CourseInfo> objects)
      ...
      public void update(CsvReader reader, CourseInfo course, boolean create) throws Exception {
        ...
        Date fromDate =
            DateHelper.parseOrNullDate(reader.get(START_COLUMN), Dates.CALENDAR_CONTROL_FORM);
        if (fromDate == null) {
          fromDate = defaultFromDate;
        }
        course.setFrom(fromDate);
        Date toDate =
            DateHelper.parseOrNullDate(reader.get(END_COLUMN), Dates.CALENDAR_CONTROL_FORM);
        if (toDate == null) {
