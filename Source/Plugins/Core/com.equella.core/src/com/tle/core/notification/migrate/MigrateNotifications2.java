...
public class MigrateNotifications2 extends AbstractHibernateSchemaMigration {
  ...
  private static final String KEY_PREFIX =
      PluginServiceImpl.getMyPluginId(MigrateNotifications2.class) + '.';
    ...
    if (drop) {
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns(
              TABLE, "processed", "batched", "itemid_only"));
    } else {
      sql.addAll(
          helper.getAddIndexesAndConstraintsForColumns(
              TABLE, "processed", "batched", "itemid_only", "itemid"));
    }
    return sql;
  }
