...
public class QtiAssessmentResultDaoImpl extends GenericDaoImpl<QtiAssessmentResult, Long>
    implements QtiAssessmentResultDao {
  ...
  private Criterion[] getBaseCriteria(
    ...
    return new Criterion[] {
      ...
      Restrictions.eq("resourceLinkId", resourceLinkId),
      Restrictions.eq("userId", userId),
      Restrictions.eq("lmsInstanceId", toolConsumerInstanceGuid)
    };
  }
  ...
  public int countAttemptsByResourceLink(
      ...
      String toolConsumerInstanceGuid) {
    return (int)
        countByCriteria(getBaseCriteria(test, resourceLinkId, userId, toolConsumerInstanceGuid));
  }
   ...
   */
  @Override
  public List<QtiAssessmentResult> findByResourceLink(
      QtiAssessmentTest test,
      String resourceLinkId,
      String userId,
      String toolConsumerInstanceGuid) {
    return findAllByCriteria(
        Order.desc("datestamp"),
        -1,
        getBaseCriteria(test, resourceLinkId, userId, toolConsumerInstanceGuid));
  }

  @Override
  public QtiAssessmentResult getCurrentByResourceLink(
      QtiAssessmentTest test,
      String resourceLinkId,
      String userId,
      String toolConsumerInstanceGuid) {
    List<QtiAssessmentResult> findByResourceLink =
        findByResourceLink(test, resourceLinkId, userId, toolConsumerInstanceGuid);
  ...
  private Query getAllQuery(Session session) {
    final Query query =
        session.createQuery(
            "FROM QtiAssessmentResult result WHERE result.test.institution = :institution");
