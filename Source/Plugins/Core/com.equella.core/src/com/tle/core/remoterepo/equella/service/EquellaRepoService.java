...
public interface EquellaRepoService {
  ...
  /**
   * Only call this once for any completed session (it MUST be completed). The session is removed
   ...
   * @param uuid
   * @param version
   * @param stripFileBasedAttachments Removes all attachments/attachment nodes of type
   *     "local","zip","imsres","html" (basically anything that relies on a physical local file)
   * @return
   */
