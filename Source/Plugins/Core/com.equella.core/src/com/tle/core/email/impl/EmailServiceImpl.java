...
public class EmailServiceImpl implements EmailService {
  ...
  @Override
  public <T> Callable<EmailResult<T>> createEmailer(
      String subject,
    ...
    try {
      ...
      if (Check.isEmpty(server)) {
        throw new RuntimeException(
            "Incorrect mail settings - No server set on institution: "
  ...
  @Override
  public <T> Callable<EmailResult<T>> createEmailer(
      String subject, List<String> emailAddresses, String message, final T key) {
  ...
  public Future<EmailResult<String>> sendSystemEmail(String subject, String message) {
    ...
    return emailThread.submit(
        createEmailer(subject, Lists.newArrayList(emails), message, message, mailSettings));
  }
}
