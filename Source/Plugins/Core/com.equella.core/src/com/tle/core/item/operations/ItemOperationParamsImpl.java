...
public final class ItemOperationParamsImpl implements ItemOperationParams {
  ...
  private final PriorityQueue<AfterCommitHook> afterCommit =
      new PriorityQueue<AfterCommitHook>(
          11, Ordering.from(new AfterCommitHookOrdering()).nullsLast());

  private final Map<String, String> attributes = new HashMap<String, String>();

  private final List<ApplicationEvent<?>> afterCommitEvents = new ArrayList<ApplicationEvent<?>>();
  private final List<ApplicationEvent<?>> afterCommitAllEvents =
  ...
  public List<Runnable> getAfterSave() {
    return Lists.transform(
        afterSave,
        new Function<AfterSaveHook, Runnable>() {
          @Override
