...
public class AddLastActionDate extends AbstractHibernateSchemaMigration {
  private static String keyPrefix =
      PluginServiceImpl.getMyPluginId(AddLastActionDate.class) + "."; // $NON-NLS-1$

  @Override
  protected int countDataMigrations(HibernateMigrationHelper helper, Session session) {
    return count(
        session.createQuery(
            "select count(*) from Item where moderating = true or status = 'rejected'"));
  }
  ...
  public static class FakeItem {
    ...
    boolean moderating;
    String status;
    @ManyToOne NewWorkflowStatus moderation;
    @OneToMany List<FakeHistoryEvent> history;
  }
