...
public class ReplicatedUserPlugin extends AbstractUserDirectory {
  ...
  public ModifiableUserState authenticateUser(final String username, final String password) {
    ...
    if (pass == null
        || (isNotPlainText ? pass.equalsIgnoreCase(innerPass) : pass.equals(innerPass))) {
  ...
  @Override
  public ModifiableUserState authenticateUserFromUsername(
      final String username, String privateData) {
  ...
  /**
   * This is the final fall-back in case all the other "search groups" queries are empty. It just
   * gets all the users in the group. It is not correct, just a last resort.
   */
  ...
  public Collection<RoleBean> searchRoles(final String query) {
    ...
    executeSql(
        sql,
        getParamValues(sql, query.replace('*', '%')),
        new RowCallbackHandler() {
          @Override
          public void processRow(ResultSet set) throws SQLException {
            String id = set.getString(1);
  ...
  private void handleIn(String sql, Collection<?> values, RowCallbackHandler handler) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      final int thisChunkSize =
          (total - thisChunkStart > MAX_IN_PARAMETERS ? MAX_IN_PARAMETERS : total - thisChunkStart);
      final List<Object> chunkValues =
          newVals.subList(thisChunkStart, thisChunkStart + thisChunkSize);
  ...
  public Collection<GroupBean> searchGroups(String query, String parentId) {
    ...
    if (!Check.isEmpty(sql)) {
      return getGroupInfo(getIds(sql, getParamValues(sql, parentId, query.replace('*', '%'))))
          .values();
    }
