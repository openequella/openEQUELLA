  ...
  public void edit(EquellaItemBean itemBean, ItemEditor editor, boolean importing) {
    ...
    if (attachments != null) {
      ...
      for (AttachmentBean attachmentBean : attachments) {
        ...
        AttachmentSerializer attachmentDeserializer =
            deserializerMap.get(attachmentBean.getRawAttachmentType());
    ...
    if (drm != null) {
      ...
      if (options != null) {
        ...
        drmEditor.editStudentsMustAcceptIfInCompilation(
            options.isStudentsMustAcceptIfInCompilation());
        drmEditor.editTermsOfAgreement(options.getTermsOfAgreement());
        drmEditor.editUsages(
            Lists.transform(
                options.getUsages(),
        ...
        if (partyBeans != null) {
          final List<Party> parties =
              Lists.transform(
                  partyBeans,
                  new Function<DrmPartyBean, Party>() {
                    @Override
        ...
        if (drmRestrictions != null) {
          ...
          if (networks != null) {
            final List<Triple<String, String, String>> networkTrips =
                Lists.transform(
                    networks,
                    new Function<DrmNetworkBean, Triple<String, String, String>>() {
                      ...
                      public Triple<String, String, String> apply(DrmNetworkBean input) {
                        return new Triple<String, String, String>(
                            input.getName(), input.getStartAddress(), input.getEndAddress());
                      }
                    });
