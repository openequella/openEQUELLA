...
public class FindHrefHandler extends AbstractHtmlContentHandler {
  ...
  private static final Set<String> RECOGNISED_ATTRIBUTES =
      newHashSet("href", "src", "background", "value");

  /**
   * If not outputting full html (as defined by property fullHtmlOutput) then do not output these
   * tags
   */
  @SuppressWarnings("nls")
   ...
   * @param w
   * @param callback
   * @param parseText You might want to set this to false since it's not terribly efficient.
   */
  public FindHrefHandler(
      StringWriter w, HrefCallback callback, boolean parseText, boolean scanAllAttributes) {
    super(w);
