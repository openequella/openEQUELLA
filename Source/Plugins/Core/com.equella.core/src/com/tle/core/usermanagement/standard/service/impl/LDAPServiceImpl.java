...
public class LDAPServiceImpl implements LDAPService {
  ...
  public List<? extends Attribute> getAttributes(
      LDAPSettings settings, final String base, final String[] attributes) {
    return new LDAP(settings, encryptionService)
        .doAsAdmin(
            new InContext<List<? extends Attribute>>() {
              @Override
  ...
  public List<SearchResult> search(
      LDAPSettings settings, final String base, final String filter, final SearchControls ctls) {
    return new LDAP(settings, encryptionService)
        .doAsAdmin(
            new InContext<List<SearchResult>>() {
              @Override
          ...
          public String execute(DirContext ctx) {
            ...
            Name results =
                ldap.searchFirstResultAllBases(
                    ctx, ldap.getUsernameFilter(username), new FullNameHitsCollector(), true);
            if (results != null) {
  ...
  public UserBean getUserBean(final LDAP ldap, final String userID) {
    return ldap.doAsAdmin(
        new InContext<UserBean>() {
          @Override
          public UserBean execute(DirContext ctx) throws NamingException {
            return ldap.searchFirstResultAllBases(
                ctx, ldap.getUserIDFilter(userID), new UserBeanHitsCollector(), true);
          }
        });
  }
  ...
  @Override
  public Collection<UserBean> getUsersInGroup(
      final LDAP ldap, final String query, final String parentGroupID, final boolean recursive) {
    return ldap.doAsAdmin(
        new InContext<List<UserBean>>() {
          @Override
  ...
  public Collection<UserBean> searchUsers(final LDAP ldap, final String query) {
    return ldap.doAsAdmin(
        new InContext<Collection<UserBean>>() {
          @Override
          public Collection<UserBean> execute(DirContext ctx) {
            return ldap.searchAllBases(
                ctx, ldap.getUserSearchFilter(query), new UserBeanHitsCollector(), true);
          }
        });
  }
  ...
  public GroupBean getGroupBean(final LDAP ldap, final String groupID) {
    return ldap.doAsAdmin(
        new InContext<GroupBean>() {
          @Override
          public GroupBean execute(DirContext ctx) throws NamingException {
            return ldap.getGroupBeanFromResult(
                ldap.getGroupResult(ctx, groupID, ldap.getGroupAttributes()));
          }
        });
  }
  ...
  @Override
  public Collection<GroupBean> searchGroups(
      final LDAP ldap, final String query, final String parentGroupId) {
    return ldap.doAsAdmin(
        new InContext<List<GroupBean>>() {
          @Override
          public List<GroupBean> execute(DirContext ctx) {
            return ldap.getGroupSearch()
                .searchGroupsInGroup(ctx, ldap.checkQuery(query), parentGroupId, true);
          }
        });
  }
