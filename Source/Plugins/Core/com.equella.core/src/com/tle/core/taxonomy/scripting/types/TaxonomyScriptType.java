...
/**
 * Retrieve one of these objects with the TaxonomyServiceScriptObject ('data') E.g. var myTaxonomy =
 * data.getTaxonomyByName('myTaxonomy')
 *
 * @author aholland
 */
public interface TaxonomyScriptType extends Serializable {
   ...
   * @return Gets all immediate child terms of the given term. (That is: not grandchild terms)
   */
   ...
   *
   * @param query The query to match terms against. You may use a wildcard character '*', but there
   *     will always be an implied wildcard at the end of the query.
   * @return A list of full term paths matching the query.
   */
  List<TermScriptType> searchTerms(String query);

  /**
   * Add a new term to a taxonomy. If the taxonomy does not support term addition an exception will
   * be thrown. Use supportsTermAddition to check this first.
   *
  ...
  /**
   * @return true if this taxonomy supports the addition of new terms. This is dependent on the data
   *     source of the taxonomy.
   */
  boolean supportsTermAddition();

  /**
   * @return true if this taxonomy supports term searching. This is dependent on the data source of
   *     the taxonomy (although this will almost always return true).
   */
  boolean supportsTermSearching();

  /**
   * @return true if this taxonomy supports term browsing. This is dependent on the data source of
   *     the taxonomy.
   */
