...
public class ExternalToolsServiceImpl
    ...
    implements ExternalToolsService {
  private static final PluginResourceHelper resources =
      ResourcesService.getResourceHelper(ExternalToolsServiceImpl.class);
  ...
  @Inject private ExternalToolsDao toolsDao;
  @Inject private WebMimeTypeService webMimeService;
  ...
  @Override
  protected <SESSION extends EntityEditingSession<ExternalToolEditingBean, ExternalTool>>
  ...
  protected void doAfterImport(
      TemporaryFileHandle importFolder,
      ExternalToolEditingBean bean,
      ExternalTool entity,
      ConverterParams params) {
    super.doAfterImport(importFolder, bean, entity, params);
    if (bean != null) {
      final ExternalTool dbExternalTool =
          bean.getId() != 0 ? toolsDao.findById(bean.getId()) : entity;
  ...
  protected void doValidationBean(ExternalToolEditingBean bean, List<ValidationError> errors) {
    if (Check.isEmpty(bean.getBaseURL())) {
      errors.add(
          new ValidationError("errors.baseurl", resources.getString(KEY_VALIDATION_NOBASEURL)));
    } else {
  ...
  /**
   * Ignoring the protocol, check known providers for matching host. If only one host matches,
   * return that provider. If multiple providers have a matching host with the launch url, attempt a
   * match on path. (the Moodle implementation for example only attempts a server-comparison and
   * returns first match. Moodle does not examine URL paths where servers match).
   */
  @Override
      ...
      if (!Check.isEmpty(providerHosts)) {
        if (providerHosts.size() > 1) {
          ...
          for (ExternalTool hostedP : providerHosts) {
            ...
            if (Objects.equal(
                Strings.nullToEmpty(launchFilePath), Strings.nullToEmpty(baseFilePath))) {
              return hostedP;
            }
          }
        }
  ...
  /**
   * To quote http://www.imsglobal.org/lti/v2p0pd/ltiIMGv2p0pd.html#_Toc340229529<br>
   * &lt;quote&gt;<br>
   * Note: In earlier versions of LTI, custom parameter names had to be converted to lower case, and
   * all punctuation or special characters in the name had to be replaced with underscores (before
   * adding the “custom_” prefix). This naming rule has been removed in LTI 2.0. To preserve
   * backward compatibility with launches from LTI 1 Tool Providers, it is recommended that when
   * Tool Consumers are sending a basic-lti-launch-request message, they check to see if the LTI 1.x
   * rule would result in a different POST parameter name. In this case, the Tool Consumer should,
   * as a best practice, render the parameter twice – once using the LTI 1 convention and a second
   * time using the LTI 2.0 convention.; for example, both as “custom_chapter ” and as
   * “custom_Chapter ”.<br>
   * &lt;/quote&gt;<br>
   * Accordingly, we will follow that 'best practice' here.<br>
   * Noting that the LTI 1 rule applies to parameter names (& not values), and also that the
   * "custom_" prefix is prepended by the ExternalToolViewerSection when composing the POST request
   */
  @Override
  ...
  /**
   * Because we may have munged and unmunged parameter names, and intend to included both, we may
   * create duplicates if the user has over-zealously explicitly provided their parameters in both
   * munged and unmunged form, or if we are re-saving a pList for which we have already provided a
   * munge/unmunged pair.<br>
   * Note also that the NameValue class has an equals(..) method which compares value (second) only,
   * which we ignore here by having an explicit iterator instead of relying on
   * Container.contains(...)
   *
   * @param pList
   * @param nameValue
  ...
  private NameValue addToPListIfUnique(List<NameValue> pList, NameValue nameValue) {
    ...
    for (NameValue nv : pList) {
      if (nv.getFirst().equals(nameValue.getFirst())
          && nv.getSecond().equals(nameValue.getSecond())) {
        matchExists = true;
        break;
      }
    }
    ...
    if (Check.isEmpty(iconUrl)) {
      ...
      if (!Check.isEmpty(externalToolUuid)) {
        ...
        if (externalToolUuid.equals(ExternalToolConstants.AUTOMATIC_UUID)) {
          lti =
              this.findMatchingBaseURL(
                  (String) attachment.getData(ExternalToolConstants.LAUNCH_URL));
        } else {
