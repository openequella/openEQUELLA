...
/**
 * convenience class for placing search information into the session where it can be referred to by
 * the SearchPrevNextSection, namely a map of index numbers for a search set keyed by the itemIds.
 * The class also includes a miscellaneous data such as the search total available, search offset
 * and the size of the keyResources from a Browse search which serves as an offsetHandicap.
 *
 * @author larry
 */
...
public class MappedSearchIndexValues implements Serializable {
  ...
  /**
   * The index we wish to map is the absolute index across all pages that may exist in the search
   * results, hence the fourth item on the third page maps to index value [33] (0-based, and
   * assuming 10 items per page). When adding items, looping through the items from a page-based
   * search, we'll want to combine offset and index to get a 'true' index. When adding individual
   * items when the true index is already known, we'll ignore the stored offset value and add as
   * given.
   *
   * @param itemId
   * @param index index which may be relative to a search result page
   * @param useOffset - whether the index is to be compounded with known offset or not
   */
