...
public class StatusOperation extends TaskOperation {
  ...
  public boolean execute() {
    try {
      ...
      Item item = getItem();
      if (item != null) {
        state.setModerationAllowed(
            params.getSecurityStatus().getAllowedPrivileges().contains(MODERATE_ITEM));
  ...
  private void recurseWorkflow(
      WorkflowStatus state, WorkflowNode node, Map<String, WorkflowStep> map) {
    if (!node.isLeafNode()) {
