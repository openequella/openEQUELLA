...
public class ItemLockingServiceImpl implements ItemLockingService {
  ...
  @Inject private ItemLockingDao dao;
  ...
  public ItemLock useLock(Item item, String lockId) {
    ...
    if (!itemLock.getUserSession().equals(lockId)) {
      throw new LockedException(
          "Wrong lock id '" + lockId + "'",
          itemLock.getUserID(),
          itemLock.getUserSession(),
          item.getId());
    }
    return itemLock;
  }
  ...
  public ItemLock lock(Item item) {
    ...
    if (lock != null) {
      throw new LockedException(
          "Item is already locked", lock.getUserID(), lock.getUserSession(), item.getId());
    }
  ...
  public void unlock(Item item, boolean force) {
    ...
    if (lock != null) {
      if (force || CurrentUser.getSessionID().equals(lock.getUserSession())) {
        ...
        throw new LockedException(
            "Item is locked by another session",
            lock.getUserID(),
            lock.getUserSession(),
            item.getId());
      }
