...
public class SqlTaxonomyDataSource implements TaxonomyDataSource {
  ...
  public boolean supportsTermSearching() {
    ...
    return !(Check.isEmpty(querySearchTermsLeaves)
        && Check.isEmpty(querySearchTermsTopLevel)
        && Check.isEmpty(querySearchTermsAny));
  }

  @Override
  public TermResult getTerm(String fullTermPath) {
    List<TermResult> trs =
        executeListQuery(queryGetTerm, Collections.singletonMap("term", fullTermPath));
  ...
  public List<TermResult> getChildTerms(String parentTerm) {
    return executeListQuery(
        queryGetChildTerms, Collections.singletonMap("parentTerm", Check.nullToEmpty(parentTerm)));
  }

  /** NOTE: is not using the searchFullTerm parameter! */
  @Override
  public Pair<Long, List<TermResult>> searchTerms(
      String searchQuery, SelectionRestriction restriction, int limit, boolean searchFullTerm) {
    String countSql;
    String searchSql;
    switch (restriction) {
      case LEAF_ONLY:
    ...
    if (Check.isEmpty(searchSql)) {
      throw new RuntimeException(
          "SQL query for restriction type '" + restriction.name() + "' has not been defined");
    }
  ...
  public TermResult getTermByUuid(String termUuid) {
    List<TermResult> trs =
        executeListQuery(queryGetTerm, Collections.singletonMap("uuid", termUuid));
    if (Check.isEmpty(trs)) {
      return null;
    } else if (trs.size() == 1) {
      return trs.get(0);
