...
public class ItemHelper {
  ...
  @Inject private ItemDetailsHelper detailsHelper;
  @Inject private AttachmentHelper attachmentHelper;
  @Inject private BadUrlHelper badUrlHelper;
  @Inject private HistoryEventHelper historyEventHelper;
  @Inject private CollaboratorsHelper collaboratorsHelper;
  @Inject private DRMHelper drmHelper;
  @Inject private ModerationHelper moderationHelper;
  ...
  @Inject private PluginTracker<AbstractHelper> tracker;
  ...
  public static List<Pair<LanguageBundle, LanguageBundle>> getValuesForDisplayNodes(
    ...
    final List<Pair<LanguageBundle, LanguageBundle>> results =
        new ArrayList<Pair<LanguageBundle, LanguageBundle>>();
    for (final DisplayNode displayNode : displayNodes) {
      final LanguageBundle values =
          getDisplayNodeValues(
              displayNode,
              itemxml,
              new DisplayNodeFilter() {
                @Override
  ...
  public static LanguageBundle getDisplayNodeValues(
      final DisplayNode displayNode, final PropBagEx xml, final DisplayNodeFilter filter) {
  ...
  /** Append values to a mapping of locales to existing concatenated display node values. */
  private static void appendToValues(
      final Map<String, StringBuilder> values,
      final PropBagEx xml,
      final Locale locale,
  ...
  public void updateItemFromXml(
      final ItemPack<Item> itemPack, DeleteHandler deleteHandler, boolean cleanMetadata) {
    ...
    item.setName(
        LanguageBundle.edit(
            item.getName(),
            LangUtils.getBundleFromXml(itemXml.getSubtree(namePath)),
            deleteHandler));
    item.setDescription(
        LanguageBundle.edit(
            item.getDescription(),
            LangUtils.getBundleFromXml(itemXml.getSubtree(descPath)),
            deleteHandler));
