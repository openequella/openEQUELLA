...
public class LORAXProtocol extends AbstractTLFProtocol {
  ...
  private static final String LORAX_QUERY_SYNTAX =
      LORAX_NAMESPACE_PATH + "/Definitions/QuerySyntax";
  ...
  private static final String LORAX_SOAP_ENVELOPE_TAG_VALUE =
  ...
  /** Differs from SHEX & MEX, so overrides the the version they derive from AbstractTLFProtocol */
  @Override
    ...
    for (PropBagEx result : queryXml.iterator(getQueryContentMethod() + "Result")) {
      ...
      try {
        ...
        getLogger()
            .error(
                CurrentLocale.get(
                    "com.tle.core.harvester.learning.badformat", "VersionDateTime", created));
        throw ex;
      }
