...
public class FreeTextServiceImpl
  ...
  @Inject private BackgroundIndexerFactory backgroundProvider;

  private Date lastSync = new Date();
  private final Timer timer = new Timer();
  private final LoadingCache<Long, BackgroundIndexer> backgroundIndexers =
      CacheBuilder.newBuilder().build(CacheLoader.from(new CreateBackgroundIndexer()));
    ...
    timer.schedule(
        new TimerTask() {
          ...
          public void run() {
            ...
            lastSync = new Date();
            Collection<Entry<Long, BackgroundIndexer>> indexers =
                backgroundIndexers.asMap().entrySet();
  ...
  private BackgroundIndexer getBackgroundIndexer() {
    return backgroundIndexers.getUnchecked(
        institutionService.getSchemaIdForInstitution(CurrentInstitution.get()));
  }
  ...
  public SearchResults<ItemIdKey> searchIds(Search searchreq, int nStart, int nCount) {
    ...
    return new ItemIdKeySearchResults(
        StdFreetextResults.convertToKeys(results.getResults()),
        results.getCount(),
        results.getOffset(),
  ...
  public int[] countsFromFilters(Collection<? extends Search> filters) {
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          "Total time taken: " + (t2.getTime() - t1.getTime()) + "ms"); // $NON-NLS-1$ //$NON-NLS-2$
    }
  ...
  @Override
  public List<ItemIdKey> getKeysForNodeValue(
  ...
  public MatrixResults matrixSearch(
      Search searchRequest, List<String> fields, boolean countOnly, int width, boolean searchAttachments) {
