...
/**
 * This NumberFormat converts long integers to and from Roman Numeral notation. Once an instance has
 * been created, the format and parse methods may be used as defined in java.text.NumberFormat.
 *
 * <p>The <a href="../benno/applets/RomanNumeralConverter.html">RomanNumeralConverter applet</a>
...
public class RomanFormat extends NumberFormat {
  ...
  /**
   * This method returns null. I have found no meaningful translation of floating point numbers to
   * Roman numerals, however the NumberFormat method requires that it is implemented. Perhaps should
   * just cast the double to a long and format accordingly.
   */
  @Override
  ...
  /**
   * This nested class is used to map Roman symbols onto their numerical values. Used in the Roman
   * class.
   *
   ...
   * Unfortunately, JavaDoc JDK 1.1 does not create documentation for the inner class Roman.SymTab,
   * so the reader cannot see the definition.
   */
  protected static final List<RomanFormat.SymTab> syms =
  ...
  /**
   * This method converts a Roman Numeral string to a long integer. It does not check that the
   * string is in the correct format - for some incorrectly formatted numbers, i.e. iix, it will
   * produce a number. For others, it will throw an exception.
   *
   ...
   * BUG: the method does not take account of the <code>FieldPosition p</code> parameter.
   *
    ...
    while (n > 0) {
      for (i = 0; i < syms.size(); i++) {
        if (syms.get(i).value <= n) {
          ...
          if (i > 0
              && shift < syms.size()
              && (syms.get(i - 1).value - syms.get(shift).value) <= n) {
