...
/** Ensure we have a taxonomy lock before doing any operations that modify the term tree. */
@Singleton
...
public class TermSoapServiceImpl implements TermSoapService {
  ...
  public void lockTaxonomyForEditing(String taxonomyUuid) {
    try {
      ...
      if (CurrentUser.getUserID().equals(ex.getUserID())) {
        ...
        throw new LockedException(
            "Taxonomy is locked by another user: " + ex.getUserID(),
            ex.getUserID(),
            ex.getSessionID(),
            ex.getEntityId());
      }
  ...
  private Taxonomy t(String taxonomyUuid) {
    ...
    if (tax == null) {
      throw new RuntimeException(
          "The taxonomy with UUID of " + taxonomyUuid + " could not be found");
    }
    return tax;
  }
}
