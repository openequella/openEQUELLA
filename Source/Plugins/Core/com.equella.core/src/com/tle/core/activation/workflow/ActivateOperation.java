...
public class ActivateOperation extends AbstractStandardWorkflowOperation {
  ...
  private static final PluginResourceHelper urlHelper =
      ResourcesService.getResourceHelper(ActivateOperation.class);
  ...
  private void validateDates(List<ValidationError> errors, ActivateRequest request) {
    if (request.getFrom() == null) {
      errors.add(
          new ValidationError(
              "from", CurrentLocale.get(urlHelper.key("activate.error.invalidfrom"))));
    } else if (request.getUntil() == null) {
      errors.add(
          new ValidationError(
              "until", CurrentLocale.get(urlHelper.key("activate.error.invaliduntil"))));
    } else if (request.getFrom().after(request.getUntil())) {
      errors.add(
          new ValidationError(
              "fromuntil", CurrentLocale.get(urlHelper.key("activate.error.invalidrange"))));
    } else if (request.getUntil().before(params.getDateNow())) {
      errors.add(
          new ValidationError(
              "fromuntil", CurrentLocale.get(urlHelper.key("activate.error.invalidtoday"))));
    }
  }
