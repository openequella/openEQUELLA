...
public class ItemResolverImpl implements ItemResolver {
  @Inject private PluginTracker<ItemResolverExtension> resolverTracker;
  ...
  @Override
  public IAttachment getAttachmentForUuid(
      ItemKey itemKey, String attachmentUuid, @Nullable String extensionType) {
    final IAttachment attachment =
        getResolver(extensionType).resolveAttachment(itemKey, attachmentUuid);
    if (attachment == null) {
      throw new NotFoundException(
          "Attachment with UUID " + attachmentUuid + " not found on item " + itemKey.toString());
    }
    return attachment;
  }
  ...
  @Override
  public boolean checkRestrictedAttachment(
      IItem<?> item, IAttachment attachment, @Nullable String extensionType) {
