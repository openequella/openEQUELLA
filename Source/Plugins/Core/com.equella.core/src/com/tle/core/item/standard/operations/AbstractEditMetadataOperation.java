...
public abstract class AbstractEditMetadataOperation extends AbstractStandardWorkflowOperation {
  ...
  private void ensureNavigationNodes(Item item, String stagingId) {
    ...
    try {
      ...
      LOGGER.error(
          CurrentLocale.get(
              "com.tle.core.workflow.operations.editmeta.error.readingmanifest",
              szPackage)); //$NON-NLS-1$
    }
  }
  ...
  private void ensureDrm(Item item) {
    ...
    if (drmSettings == null
        || !DrmSettings.CUSTOM_SCRIPTED_DRMSETTINGS_PAGE_PLACEHOLDER.equals(
            drmSettings.getDrmPageUuid())) {
