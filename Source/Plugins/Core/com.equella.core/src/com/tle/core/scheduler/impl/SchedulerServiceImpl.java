...
public class SchedulerServiceImpl implements SchedulerService, SchemaListener {
  ...
  public void setPluginService(PluginService pluginService) {
    ...
    taskTracker =
        new PluginTracker<ScheduledTask>(
            pluginService, "com.tle.core.scheduler", "scheduledTask", "id");
  ...
  public Task createSupervisorTask() {
    return new AlwaysRunningTask<Void>() {
      ...
      private final LoadingCache<Institution, TargetDates> targetDates =
          CacheBuilder.newBuilder()
              .build(
                  new CacheLoader<Institution, TargetDates>() {
                    @Override
      ...
      public void runTask(Void ignore) {
        ...
        if (targetDates.size() > institutions.size() + 1) {
          ...
          Set<Institution> inactiveInsts =
              Sets.difference(targetDates.asMap().keySet(), activeInsts);
      ...
      private List<Extension> getExtensionsToRun(Institution inst) {
        ...
        boolean notNowThankYou =
            now.before(td.dailyTarget)
                && now.before(td.weeklyTarget)
      ...
      /**
       * Waits for any of the given tasks to finish and returns that extension ID.
       *
        ...
        while (true) {
          ...
          if (delta > TASK_WAIT_WARN_TIME) {
            ...
            for (Map.Entry<String, String> runningTask : runningTasks.entrySet()) {
              rt.append(runningTask.getKey())
                  .append("=")
                  .append(runningTask.getValue())
                  .append(" (");
              String taskId = runningTask.getValue();
              TaskStatus status =
                  taskService.waitForTaskStatus(taskId, TimeUnit.SECONDS.toMillis(1));
              if (status == null) {
                rt.append("no status) ");
              } else {
      ...
      private RuntimeException logDependencyError(
          Set<String> finishedTasks, ListMultimap<String, String> unstartedTasks) {
        ...
        for (Iterator<Map.Entry<String, String>> iter = unstartedTasks.entries().iterator();
            iter.hasNext(); ) {
        ...
        for (String extId : unstartedTasks.keySet()) {
          LOGGER.error(
              "Task "
                  + extId
                  + " has following dependencies that don't seem to exist "
  ...
  public Task createScheduledTask(
      final String pluginId, final String beanName, final long institutionId) {
    return new SingleShotTask() {
      @Override
      public void runTask() {
        final Institution inst =
            institutionId == Institution.FAKE.getUniqueId()
                ? Institution.FAKE
        ...
        if (inst != Institution.FAKE) {
          ...
          for (Long schemaId : institutionService.getAvailableMap().keySet()) {
            databaseSchemaService.executeWithSchema(
                schemaId,
                new Callable<Void>() {
                  @Override
                  public Void call() {
  ...
  private void ensureGlobalTaskStarted() {
    taskService.getGlobalTask(
        new BeanClusteredTask(
            "Scheduler-Supervisor", true, SchedulerService.class, "createSupervisorTask"),
  ...
  private String startGlobalTask(Extension extension, String extId, Institution inst) {
    ...
    BeanClusteredTask bct =
        new BeanClusteredTask(
            "Scheduled-Task-" + extId + '-' + uniqueId,
