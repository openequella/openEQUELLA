...
public class FixAccessExpressionsMigration extends AbstractHibernateDataMigration {
  private static final String KEY_PREFIX =
      PluginServiceImpl.getMyPluginId(FixAccessExpressionsMigration.class) + ".";
  ...
  protected void executeDataMigration(
    ...
    for (FakeAccessExpression expr : exprs) {
      ...
      if (split.length == 1 && e.endsWith(" ")) {
        ...
        // NOTE!  We use 'like' instead of '=' since trailing whitespace is ignored with = (by SQL
        // Server at least)
        final FakeAccessExpression existing =
            (FakeAccessExpression)
                session
                    .createQuery("from AccessExpression where expression like :expression")
                    .setParameter("expression", trimExpression)
                    .uniqueResult();
        if (existing != null) {
          ...
          final List<FakeAccessEntry> entriesWithCrapExpression =
              session
                  .createQuery("from AccessEntry where expression = :expression")
  ...
  public static class FakeAccessExpression {
    ...
    @CollectionTable(
        name = "access_expression_expression_p",
        joinColumns = @JoinColumn(name = "access_expression_id"))
    @Column(name = "element")
