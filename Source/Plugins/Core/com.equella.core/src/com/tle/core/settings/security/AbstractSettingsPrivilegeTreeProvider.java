...
public abstract class AbstractSettingsPrivilegeTreeProvider implements PrivilegeTreeProvider {
  protected static PluginResourceHelper r =
      ResourcesService.getResourceHelper(AbstractSettingsPrivilegeTreeProvider.class);

  public enum Type {
    SYSTEM_SETTING(Node.ALL_SYSTEM_SETTINGS, Node.SYSTEM_SETTING, "EDIT_SYSTEM_SETTINGS"),
    MANAGEMENT_PAGE(Node.ALL_MANAGING, Node.MANAGING, "VIEW_MANAGEMENT_PAGE");
  ...
  @Inject private SettingsSecurityTargetHandler handler;
  ...
  protected AbstractSettingsPrivilegeTreeProvider(
      Type type, String nameKey, SettingsTarget settingTarget) {
    this.type = type;
  ...
  public void gatherChildTargets(List<SecurityTarget> childTargets, SecurityTarget target) {
    if (target != null && target.getTargetType() == type.getAllNode()) {
      childTargets.add(
          new SecurityTarget(
              CurrentLocale.get(nameKey), type.getSingularNode(), settingTarget, false));
    }
  }
