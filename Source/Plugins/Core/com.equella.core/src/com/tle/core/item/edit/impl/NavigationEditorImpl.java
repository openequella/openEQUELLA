...
public class NavigationEditorImpl implements NavigationEditor {
  ...
  public NavigationEditorImpl(
      ItemEditorChangeTracker changeTracker, Map<String, Attachment> attachmentMap, Item item) {
  ...
  public class NavigationNodeEditorImpl implements NavigationNodeEditor {
    ...
    public void editTab(int index, String name, String attachmentUuid, String viewer) {
      ...
      if (tabAdded
          || changeTracker.hasBeenEdited(
              origAttachment == null ? null : origAttachment.getUuid(), attachmentUuid)) {
    ...
    public void editTabCount(int numTabs) {
      ...
      if (tabSize < numTabs) {
        throw new ItemEditingException(
            "Requested size of tab list too many: wanted "
                + numTabs
                + " but only "
                + tabSize
                + " available");
  ...
  private void recurseNode(
      ...
      List<ItemNavigationNode> toRetain,
          ...
          if (attachment != null) {
            ...
            if (attachment == null) {
              throw new ItemEditingException(
                  "Navigation node '"
                      + navNode.getUuid()
                      + "' references non-existent attachment '"
                      + attachUuid
                      + "'");
            }
            tab.setAttachment(attachment);
