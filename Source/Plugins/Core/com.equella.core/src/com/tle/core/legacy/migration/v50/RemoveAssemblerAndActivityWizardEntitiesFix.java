...
public class RemoveAssemblerAndActivityWizardEntitiesFix extends XmlMigrator {
  public static final Set<String> ASSEM_COLLECTIONS =
      new HashSet<String>(
          Arrays.asList(
              ...
              "2f6bd1b8-6ddb-3b7c-554c-646617b1dad7"));
  ...
  private void removeInvalidSecurityRules(TemporaryFileHandle staging) {
    if (fileSystemService.fileExists(staging, "acls/entries.xml")) {
      ...
      for (Iterator<PropBagEx> iter = xml.iterator(); iter.hasNext(); ) {
        ...
        if (priv.endsWith("_ACTIVITY_WIZARD")
            || target.equals("C:assemblerDrmReference")
            || target.equals("C:assesmblerFileTypes")
            || target.equals("C:assemblerLinks")
            || target.equals("C:lmsexport")
            || target.equals("C:taxonomies")
            || target.equals("C:webct")) {
          iter.remove();
        }
      }
  ...
  private void removeInvalidConfigProps(TemporaryFileHandle staging) {
    if (fileSystemService.fileExists(staging, "properties/properties.xml")) {
      ...
      for (Iterator<PropBagEx> iter = xml.iterator(); iter.hasNext(); ) {
        ...
        if (prop.startsWith("assembler.")
            || prop.startsWith("webct.")
            || prop.startsWith("blackboard.")
            || prop.equals("soap.timeout")
            || prop.startsWith("activity-wizard.")) {
          iter.remove();
        }
      }
