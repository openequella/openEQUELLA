...
public class QtiItemVariable implements Serializable, IdCloneable {
  ...
  /**
   * We are not supporting the record type, so each value can be a simple string instead of a
   * "value" type. ( http ://www.imsglobal.org/question/qtiv2p1/imsqti_infov2p1.html#element10042 )
   *
   * <p>Note: OutcomeVariable defines a list of values and ResponseVariable defines a single
   * candidateResponse which itself contains only a list of values, hence the candidateResponse is
   * rolled into this value list and put into the base class for OutcomeVariable and
   * ResponseVariable to use.
   */
  ...
  @CollectionTable(
      name = "qti_item_variable_value",
      joinColumns = @JoinColumn(name = "qti_item_variable_id"))
