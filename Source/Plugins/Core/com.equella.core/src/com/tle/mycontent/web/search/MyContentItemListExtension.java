        ...
        if (entry instanceof MyContentItemListEntry) {
          if (settings.isEditable()) {
            if (item.getStatus() == ItemStatus.DELETED) {
              HtmlLinkState state =
                  new HtmlLinkState(
                      LABEL_RESTORE, events.getNamedHandler("restore", item.getItemId()));
              entry.addRatingAction(new ButtonRenderer(state).showAs(ButtonType.ADD));
            } else {
              HtmlLinkState edit =
                  new HtmlLinkState(LABEL_EDIT, events.getNamedHandler("edit", item.getItemId()));
              entry.addRatingAction(new ButtonRenderer(edit).showAs(ButtonType.EDIT));

              HtmlLinkState delete =
                  new HtmlLinkState(
                      LABEL_DELETE,
                      events
                          .getNamedHandler("delete", item.getItemId())
                          .addValidator(new Confirm(LABEL_DELETECONFIRM)));
