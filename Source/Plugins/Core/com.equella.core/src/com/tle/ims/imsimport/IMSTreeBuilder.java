...
public class IMSTreeBuilder implements PackageTreeBuilder {
      ...
      if (imsManifest != null) {
        ...
        Collection<Attachment> createdAttachments =
            navHelper.createTree(
                ...
                actualStaging,
                packageName,
                !Check.isEmpty(scormVersion),
                expand);
        for (IMSFileExporter fileImporter : fileImporters.getBeanList()) {
          fileImporter.importFiles(
              item, actualStaging, packageExtractedFolder, packageName, createdAttachments);
        }
  ...
  public boolean canHandle(SectionInfo info, FileHandle staging, String packageFilepath) {
    try {
      ArchiveEntry entry =
          fileSystem.findZipEntry(staging, packageFilepath, IMSUtilities.IMS_MANIFEST, false);
      return (entry != null);
    } catch (Exception e) {
      return false;
    }
