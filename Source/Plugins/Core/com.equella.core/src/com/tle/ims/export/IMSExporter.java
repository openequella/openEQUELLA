...
@Bind
public class IMSExporter extends AbstractPrototypeSection<IMSExporterModel>
  ...
  @Inject private DownloadPackageViewer viewer;
  ...
  public SectionResult renderHtml(RenderEventContext context) {
    ...
    if (ims != null) {
      ViewableResource resource =
          attachmentResourceService.getViewableResource(context, itemInfo.getViewableItem(), ims);
  ...
  private void createManifest(
      ItemSectionInfo itemSectionInfo, ItemFile itemFile, StagingFile stagingFile)
      throws IOException {
    final Item item = itemSectionInfo.getItem();
    ...
    if (transform != null) {
      ...
      String transformed =
          schemaService.transformForExport(
              schemaId, transform, itemSectionInfo.getItemxml(), false);
    ...
    OutputStream outputStream =
        fileSystemService.getOutputStream(stagingFile, "imsmanifest.xml", false);
    outputStream.write(manbag.toString().getBytes("UTF-8"));
    outputStream.close();
  }

  private void noNavNodes(
      Item item,
      Attachments attachments,
      List<IMSResource> resources,
      List<IMSItem> items,
      List<RootItemInfo> rootItemInfos,
  ...
  private void navTreeManifest(
    ...
    recurseTree(
        item, rootNodes, childMap, items, rootItemInfos, resources, attachmentSet, stagingFile, 0);
  }

  private void recurseTree(
      Item item,
      List<ItemNavigationNode> rootNodes,
      Map<ItemNavigationNode, List<ItemNavigationNode>> childMap,
      List<IMSItem> items,
      List<RootItemInfo> rootItemInfos,
      List<IMSResource> resources,
      Set<String> attachmentSet,
      StagingFile stagingFile,
      int level) {
    if (rootNodes == null) {
      return;
    ...
    for (ItemNavigationNode node : rootNodes) {
      ...
      for (ItemNavigationTab tab : tabs) {
        ...
        if (attachment != null) {
          ...
          String ident =
              getIMSResource(item, tab.getAttachment(), resources, attachmentSet, stagingFile);
          tabNode.setIdentifier(ident);
          tabNode.setIdentifierRef(ident);
          imsTabs.add(tabNode);
        }
  ...
  public void setPluginService(PluginService pluginService) {
    fileExporters =
        new PluginTracker<IMSFileExporter>(
            pluginService, "com.tle.web.ims", "imsFileExporter", "id");
    fileExporters.setBeanKey("class");

    manifestExporters =
        new PluginTracker<IMSManifestExporter>(
            pluginService, "com.tle.ims", "imsManifestExporter", "id");
    manifestExporters.setBeanKey("class");

    attachmentExporters =
        new PluginTracker<IMSAttachmentExporter>(
            pluginService, "com.tle.ims", "imsAttachmentExporter", "id");
