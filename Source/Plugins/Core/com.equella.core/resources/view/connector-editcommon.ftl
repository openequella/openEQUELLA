...
<@css "connector-editcommon.css" />

<@setting label=b.key('entity.uuid') >
	<input type="text" readonly="readonly" value="${m.entityUuid?html}" />
</@setting>

<@setting label=b.key('connector.editor.label.title') section=s.title error=m.errors["title"] mandatory=true />

<@setting label=b.key('connector.editor.label.description') section=s.description />
