...
object CurrentUpload {
  def validatedUploads(u: Iterable[CurrentUpload]): Iterable[ValidatedUpload] =
    u collect {
...
sealed trait CurrentUpload {
  ...
  def temporaryPath(path: String): String =
    s"${FileUploadState.UPLOADS_FOLDER}/${id}_files/$path"
}
...
case class ValidatedUpload(s: SuccessfulUpload, detected: Seq[PackageType])
    extends CurrentUpload {
  def finished = true
  def id = s.id
...
case class FailedUpload(id: UUID,
                        started: Instant,
                        originalFilename: String,
                        reason: UploadResult)
    extends CurrentUpload {
...
case class WrongPackageType(allowed: Seq[PackageType],
                            detected: Seq[PackageType])
