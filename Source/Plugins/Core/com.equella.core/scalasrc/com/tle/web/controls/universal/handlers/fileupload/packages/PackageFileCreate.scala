...
object PackageFileCreate {
  ...
  lazy val tracker = new PluginTracker[PackageAttachmentExtension](
  ...
  def extensionForAttachment(
      a: Attachment): Option[PackageAttachmentExtension] =
...
object IMSPackageExtension extends PackageAttachmentExtension {
  ...
  def commitFiles(stg: StagingContext,
                  pkgUnzip: String,
                  upload: SuccessfulUpload): Unit = {
    val pkgFolder = upload.originalFilename
    ...
    stg.moveFile(
      upload.uploadPath,
      s"${FileSystemConstants.IMS_FOLDER}/${upload.originalFilename}")
  ...
  val treatAsLabel =
    WebFileUploads.label("handlers.file.packageoptions.aspackage")
}
...
object ScormPackageExtension extends PackageAttachmentExtension {
  ...
  val treatAsLabel =
    WebFileUploads.label("handlers.file.packageoptions.asscorm")
