...
object SettingsList {
  ...
  def asLinkOrNull(settings: EditableSettings): HtmlLinkState = {
    new HtmlLinkState(new TextLabel(settings.name),
                      new SimpleBookmark(settings.uri))
  }
  ...
  val uiSettings = CoreSettingsRest(
    ...
    "uisettings.desc",
    "api/settings/ui",
    AclChecks.filterNonGrantedPrivileges(Iterable("EDIT_SYSTEM_SETTINGS"),
                                         false))
  ...
  val echoSettings = CoreSettingsPage(
    ...
    () =>
      !aclManager
        .filterNonGrantedPrivileges(EchoConstants.PRIV_CREATE_ECHO,
                                    EchoConstants.PRIV_EDIT_ECHO)
        .isEmpty
  )
  ...
  val connectorSettings = CoreSettingsPage(
    ...
    () =>
      !aclManager
        .filterNonGrantedPrivileges(PRIV_CREATE_CONNECTOR, PRIV_EDIT_CONNECTOR)
        .isEmpty
  )
