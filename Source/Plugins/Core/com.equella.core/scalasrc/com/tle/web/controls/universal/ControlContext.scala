...
class FileStagingContext(stgId: Option[String],
  ...
  def moveFile(from: String, to: String): Unit =
    fileSystemService.move(stgFile, from, to)

  def listRootFilenames(): Set[String] =
    fileSystemService
      .enumerate(stgFile, "", null)
  ...
  def fileSize(fullPath: String): Long =
    fileSystemService.fileLength(stgFile, fullPath)

  def deregisterFilename(id: UUID): Unit = repo.unregisterFilename(id)

  def thumbRequest(filePath: String): String = {
    val thumbnail = thumbnailService.submitThumbnailRequest(itemId,
                                                            stgFile,
                                                            filePath,
                                                            true,
                                                            false)
       ...
       Try(fileSystemService.read(stgFile, filepath))
         .flatMap { inp =>
           val tried = Try {
             ...
             val parser =
               new AutoDetectParser(new TikaConfig(getClass.getClassLoader))
         ...
         .map { m =>
           tikaMapping.flatMap {
             case (HttpHeaders.LAST_MODIFIED, "lastmodified") =>
               Option(m.getDate(HttpHeaders.LAST_MODIFIED))
                 .map(("lastmodified", _))
