...
trait NotificationLookup {
  ...
  trait TaskNotification extends ItemNotification {
    ...
    def linkToTask(taskId: String) = {
      RootTaskListSection.createModerateBookmark(
        controller,
        new ItemTaskId(item.getItemId, taskId))
    }
  }

  def createItemNotification(tName: String)(n: Notification,
                                            i: Item): ItemNotification =
