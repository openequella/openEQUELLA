...
import com.tle.web.api.interfaces.beans.security.{
  TargetListBean,
  TargetListEntryBean
}
...
class AclResource {
  ...
  @Path("/privileges") def getAllowedPrivileges(
      @QueryParam("node") node: Node) = {
    PrivilegeTree.getAllPrivilegesForNode(node).keySet().asScala.toVector.sorted
  }

  @GET
  @ApiOperation(
    value = "Determine if you have non-entity specific privilege(s)")
  ...
  def setEntries(@ApiParam bean: TargetListBean): Response = {
    ...
    val tle = bean.getEntries.asScala.map { eb =>
      new TargetListEntry(eb.isGranted,
                          eb.isOverride,
                          eb.getPrivilege,
                          eb.getWho)
    }
  ...
  @POST
  @ApiOperation(
    value = "Add/Remove an expression target from the recently used list")
  @Path("/recent")
