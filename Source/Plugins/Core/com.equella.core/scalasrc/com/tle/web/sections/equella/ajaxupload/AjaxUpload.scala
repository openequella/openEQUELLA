...
case class NewUploadResponse(uploadUrl: String, id: String, name: String)
    extends AjaxUploadResponse
case class UploadFailed(reason: String) extends AjaxUploadResponse
case class AddEntries(entries: Iterable[AjaxFileEntry])
    extends AjaxUploadResponse
...
object AjaxUpload {
  ...
  private val INCLUDE = new IncludeFile(r.url("scripts/render/ajaxupload.js"),
                                        CSS_INCLUDE,
                                        JQueryProgression.PRERENDER)
  private val FILE_UPLOAD_HANDLER_CLASS =
    new ExternallyDefinedFunction("AjaxUploads", INCLUDE)
  private val VALIDATE_FUNC =
    new ExternallyDefinedFunction(FILE_UPLOAD_HANDLER_CLASS, "validateFile", 5)
  private val ADD_UPLOAD_FUNC = new ExternallyDefinedFunction(
    FILE_UPLOAD_HANDLER_CLASS,
    "addUploadEntry",
    4)
  private val PROGESSONLY_UPLOAD_FUNC = new ExternallyDefinedFunction(
    FILE_UPLOAD_HANDLER_CLASS,
    "simpleUploadEntry",
    4)
  ...
  def simpleUploadValidator(id: String,
                            completed: JSExpression): JSAssignable = {
    val startedUpload = createSimpleProgressFunc(
      new JQuerySelector(JQuerySelector.Type.ID, id),
      null)

    Js.functionValue(
      AjaxUpload.createValidate(0,
