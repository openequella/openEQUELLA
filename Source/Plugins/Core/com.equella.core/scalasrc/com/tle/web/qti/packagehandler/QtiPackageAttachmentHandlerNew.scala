...
class QtiPackageAttachmentHandlerNew extends PackageAttachmentExtension {
  ...
  def handlesPackage(upload: SuccessfulUpload, d: Seq[PackageType]) =
    d.contains(QTIPackage)

  def create(info: SectionInfo,
             ctx: ControlContext,
             upload: SuccessfulUpload): AttachmentCreate = {
    ...
    AttachmentCreate(
      { stg =>
        ...
        qti.setUrl(
          PathUtils.filePath(QtiConstants.QTI_FOLDER_PATH, "imsmanifest.xml"))
  ...
  def handles(a: Attachment): Boolean = a match {
    case ca: CustomAttachment
        if ca.getType == QtiConstants.TEST_CUSTOM_ATTACHMENT_TYPE =>
      true
    case _ => false
  }
  ...
  private def populateDetails(stg: StagingContext,
    ...
    val manifest = Option(
      imsService.getImsManifest(stg.stgFile, baseExtractedPath, false))
    manifest
      .flatMap(m =>
        m.getAllResources.asScala.find(_.getType.startsWith("imsqti_test_xml")))
      .foreach { res =>
        val xmlRelLoc = res.getHref
        val quiz =
          qtiService.loadV2Test(stg.stgFile, baseExtractedPath, xmlRelLoc)
        ...
        if (toolName != null) {
          ...
          attachment.setData(QtiConstants.KEY_TOOL_VERSION,
                             details.getToolVersion)
        }
        attachment.setData(QtiConstants.KEY_MAX_TIME, details.getMaxTime)
        attachment.setData(QtiConstants.KEY_QUESTION_COUNT,
                           details.getQuestionCount)
        attachment.setData(QtiConstants.KEY_SECTION_COUNT,
                           details.getSectionCount)
        attachment.setData(QtiConstants.KEY_NAVIGATION_MODE,
                           details.getNavigationMode.toQtiString)
      }
  }
