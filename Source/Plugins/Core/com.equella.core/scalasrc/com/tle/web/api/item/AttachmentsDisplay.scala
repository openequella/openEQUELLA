...
object AttachmentsDisplay {
  ...
  def createAttachment(rc: RenderContext,
    ...
    val vr = LegacyGuice.attachmentResourceService.getViewableResource(
      rc,
      vi,
      attachment)
    val uuid = attachment.getUuid
    val deets = (vr.getCommonAttachmentDetails.asScala ++
      Option(vr.getExtraAttachmentDetails)
        .getOrElse(Collections.emptyList())
        .asScala).map { ad =>
      val descHtml = SectionUtils.renderToString(rc, ad.getDescription)
      MetaDisplay(ad.getName.getText, descHtml, true, "html")
    }
    val attachmentUrl = LegacyGuice.institutionService.institutionalise(
      vr.createDefaultViewerUrl().getHref)
    AttachmentSummary(
      vr.getDescription,
      uuid,
      attachmentUrl,
  ...
  def create(info: SectionInfo,
    ...
    val showStructuredView =
      !(xml.getNode(DISPLAY_MODE_KEY) == DISPLAY_MODE_THUMBNAIL)
    val metadataTargets = xml.getNodeList(METADATA_TARGET).asScala

    val allUuids = metadataTargets
      .flatMap(n => itemXml.iterateAllValues(n).iterator().asScala.toBuffer)
      .toSet
    val filterAttach =
      if (metadataTargets.nonEmpty) allUuids else (u: String) => true
