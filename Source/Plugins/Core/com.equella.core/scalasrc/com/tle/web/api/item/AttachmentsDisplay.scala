...
import com.tle.web.api.item.interfaces.beans.{AttachmentSummary, AttachmentsSummarySection, MetaDisplay}
...
object AttachmentsDisplay {
  ...
  def createAttachment(rc: RenderContext, vi: NewDefaultViewableItem, attachment: Attachment): AttachmentSummary = {
  ...
  def create(info: SectionInfo, vi: NewDefaultViewableItem, itemXml: PropBagEx, stitle: String, config: String): Option[AttachmentsSummarySection] = {
    ...
    val allUuids = metadataTargets.flatMap(n => itemXml.iterateAllValues(n).iterator().asScala.toBuffer).toSet
    val filterAttach = if (metadataTargets.nonEmpty) allUuids else (u:String) => true
    val attachDisplays = vi.getItem.getAttachmentsUnmodifiable.iterator()
      .asScala.filter(a => filterAttach(a.getUuid)).map(a => createAttachment(rc, vi, a)).toBuffer
