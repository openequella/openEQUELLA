...
object NewSelectionPage {
  ...
  def setupSelection(req: HttpServletRequest): Unit = {
    ...
    req.setAttribute(
      RenderNewTemplate.SetupJSKey, { oe: ObjectExpression =>
        ...
        integId
          .flatMap(i =>
            Option(uss.getAttribute(i).asInstanceOf[IntegrationSessionData]))
          .foreach { isd =>
  ...
  def selectionUrl(info: SectionInfo, integId: String): String = {
    ...
    val baseUri = AbsoluteUrl.parse(
      LegacyGuice.urlService.getBaseUriFromRequest(request).toString)
    val baseParts = baseUri.path.parts.filter(_.nonEmpty)
    baseUri
      .withPath(
        AbsolutePath(baseParts).addParts("selection", s"$sessionid:$integId"))
      .toString()
  }
}
