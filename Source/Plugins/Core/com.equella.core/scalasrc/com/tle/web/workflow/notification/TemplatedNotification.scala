...
case class StdNotificationGroup(templateName: String, reason: String)
    extends NotificationGroup {
  def headerHello(user: UserBean): Label =
    NotificationLangStrings.userHeaderLabel(user)
  def headerReason(total: Int): Label =
    NotificationLangStrings.headerLabel(reason, total)
...
trait TemplatedNotification {
  ...
  class StandardEmailModel(val getHello: Label,
                           val getReason: Label,
                           notifications: Iterable[N]) {
      ...
      case (group, notes) =>
        ...
        viewFactory.render(
          viewFactory.createResultWithModel(
            group.templateName,
            new StandardEmailModel(group.headerHello(user),
                                   group.headerReason(notes.size),
                                   notes)),
          writer)
        NotificationEmail(group.subjectLabel.getText,
                          writer.toString,
                          notes.map(_.note))
    }
  }
