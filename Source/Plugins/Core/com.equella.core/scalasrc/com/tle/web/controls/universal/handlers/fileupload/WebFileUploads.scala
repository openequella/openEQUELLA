...
object WebFileUploads {
  ...
  val KEY_ERROR_NOTALLOWEDPACKAGE =
    r.key("handlers.file.error.notallowedpackage")

  private val INCLUDE = new IncludeFile(
    r.url("scripts/file/fileuploadhandler.js"))
  private val FILE_UPLOAD_HANDLER_CLASS =
    new ExternallyDefinedFunction("FileUploadHandler", INCLUDE)
  val ZIP_PROGRESS_FUNC = new ExternallyDefinedFunction(
    FILE_UPLOAD_HANDLER_CLASS,
    "setupZipProgress",
    2)

  val SUPPRESS_THUMB_VALUE = "suppress"
  def isSuppressThumbnail(et: Attachment): Boolean =
    SUPPRESS_THUMB_VALUE == et.getThumbnail

  def validateBeforeUpload(
      mimeType: String,
      size: Long,
      fileSettings: FileUploadSettings): Option[IllegalFileReason] = {
    if (fileSettings.isRestrictByMime && !fileSettings.getMimeTypes.asScala.toSet
          .contains(mimeType))
      Some(WrongType)
  ...
  def writeStream(uf: UploadingFile,
                  ctx: ControlContext,
                  stream: InputStream): UploadResult = {
  ...
  def uploadStream(uploadId: UUID,
    ...
    val currentUpload =
      validateBeforeUpload(mimeType, fileSize, ctx.controlSettings) match {
        case Some(failReason) =>
          FailedUpload(uploadId,
                       Instant.now(),
                       filename,
                       IllegalFile(failReason))
        case None =>
  ...
  def allowedPackageTypes(
      fileSettings: FileUploadSettings): Seq[PackageType] = {
  ...
  def validateContent(
    ...
    val detected = ctx.repo
      .determinePackageTypes(info, uploadPath)
      .asScala
      .map(PackageType.fromString)
    val allowed = allowedPackageTypes(settings)
    if (settings.isPackagesOnly && detected.isEmpty)
      Left(NotAPackage)
    else if (settings.isPackagesOnly && allowed.nonEmpty && !allowed.exists(
               detected.toSet))
  ...
  def validateAllFinished(info: SectionInfo, ctx: ControlContext): Unit = {
    val allFinished = ctx.state.allCurrentUploads.collect {
      case s: SuccessfulUpload =>
        validateContent(info, ctx, s.uploadPath).fold(
          ifr => failedValidation(ctx, s, ifr),
          dpt => ValidatedUpload(s, dpt))
    }
    allFinished.foreach(u => ctx.state.finishedUpload(u))
  }

  def removeFailedUploads(ctx: ControlContext,
                          apartFrom: Option[UUID]): Unit = {
  ...
  def attachmentCreatorForUpload(info: SectionInfo,
    ...
    if (v.detected.nonEmpty)
      PackageFileCreate.createForUpload(info, ctx, v.s, v.detected)
    else
      StandardFileCreate.fileAttachmentFromUpload(
        v.s,
        ctx.controlSettings.isSuppressThumbnails)
  }
  ...
  def uniqueName(filename: String,
    ...
    val rootNames = ctx.stagingContext.listRootFilenames() -- replacing.map(
      _.toLowerCase())
    ...
    def registerName(fname: (String, String), count: Int): String = {
      if (count >= 100)
        sys.error("Failed to find unique filename after 100 tries")
    ...
    registerName(
      (p.getFirst, if (p.getSecond.isEmpty) "" else "." + p.getSecond),
      1)
  }

  def errorMessage(allUploads: Iterable[CurrentUpload]): Option[Label] = {
    allUploads.toBuffer[CurrentUpload].reverse.collectFirst {
      case FailedUpload(_, _, fn, IllegalFile(ifr)) =>
        labelForIllegalReason(ifr, fn)
    }
  }

  def labelForIllegalReason(reason: IllegalFileReason,
  ...
  def deleteAttachment(ctx: ControlContext, a: Attachment): AttachmentDelete =
    a match {
      case fa: FileAttachment =>
        AttachmentDelete(
          Iterable(fa),
          stg => if (!isSelectedInAZip(fa)) stg.delete(fa.getFilename))
      case za: ZipAttachment =>
        AttachmentDelete(a +: findAttachments(ctx.repo, isSelectedInZip(za)), {
          stg =>
            stg.delete(removeZipPath(za.getUrl))
            stg.delete(za.getUrl)
        })
      case a: Attachment =>
        PackageFileCreate.extensionForAttachment(a).get.delete(ctx, a)
    }
  ...
  def cleanupForUpload(ctx: ControlContext,
                       cu: CurrentUpload): StagingContext => Unit = {
  ...
  def findAttachments(repo: WebRepository,
                      f: IAttachment => Boolean): List[Attachment] = {
  ...
  def zipAttachment(attachment: IAttachment): Option[ZipAttachment] =
    attachment match {
  ...
  def isSelectedInAZip(a: IAttachment): Boolean = a match {
    case fa: FileAttachment =>
      fa.getData(ZipAttachment.KEY_ZIP_ATTACHMENT_UUID) != null
    case _ => false
  }

  def isSelectedInZip(za: ZipAttachment)(a: IAttachment): Boolean = a match {
    case fa: FileAttachment =>
      fa.getData(ZipAttachment.KEY_ZIP_ATTACHMENT_UUID) == za.getUuid
    case _ => false
  }
