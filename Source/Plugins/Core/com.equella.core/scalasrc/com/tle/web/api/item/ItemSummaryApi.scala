...
object ItemSummaryApi {
  ...
  def getItemSummary(uuid: String, version: Int): ItemSummary = {
    val vi = LegacyGuice.viewableItemFactory.createNewViewableItem(
    ...
    val converted =
      item.getItemDefinition.getItemSummaryDisplayTemplate.getConfigList.asScala
        .flatMap(
          c =>
            convert(info,
                    vi,
