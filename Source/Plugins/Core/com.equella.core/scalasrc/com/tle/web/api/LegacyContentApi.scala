...
import com.tle.web.sections.js.generic.statement.{
...
object LegacyContentController
    extends AbstractSectionsController
    with SectionFilter {
  ...
  override val getSectionFilters: util.List[SectionFilter] = {
    ...
    sf.add(this)
    sf.addAll(
      LegacyGuice.sectionsController
        .asInstanceOf[SectionsControllerImpl]
        .getSectionFilters)
    ...
    info.setAttribute(
      ...
      new EventAuthoriser {
        ...
        override def addToBookmark(
          ...
          bookmarkState.put(RenderTemplate.XSRF_PARAM,
                            Array[String](CurrentUser.getSessionID))
      }
    )
  }

  def baseUri(req: HttpServletRequest): URI =
    urlService.getBaseUriFromRequest(req)
  ...
  def getBookmarkState(info: SectionInfo,
                       event: BookmarkEvent): Map[String, Array[String]] = {
  ...
  def prepareJSContext(info: MutableSectionInfo): StandardRenderContext = {
    ...
    val standardContext =
      context.getAttributeForClass(classOf[StandardRenderContext])
  ...
  override def forwardToUrl(info: SectionInfo,
                            link: String,
                            code: Int): Unit = {
    info.setRendered()
...
class LegacyContentApi {
  ...
  def parsePath(
      path: String): (String, MutableSectionInfo => MutableSectionInfo) = {
    ...
    path match {
      ...
      case p if p.startsWith("items/") =>
        itemViewer(p.substring("items/".length), (_, vi) => vi)
      ...
      case Some(tree) => {
        ...
        val info = setupInfo(
          LegacyContentController
            .createInfo(tree, path, req, resp, null, params, null))
  ...
  def menuOptions(@Context req: HttpServletRequest,
    ...
    LegacyGuice.userSessionService.reenableSessionUse()
    val context = LegacyGuice.sectionsController.createInfo("/home.do",
                                                            req,
                                                            resp,
                                                            null,
                                                            null,
    ...
    val prefsEditable = !(cu.isSystem || cu.isGuest) && !(cu.wasAutoLoggedIn &&
      LegacyGuice.configService
        .getProperties(new AutoLogin)
        .isEditDetailsDisallowed)
    val menuGroups = {
      contributors
        .getExtensions(
          new PluginTracker.ParamFilter("enabledFor", noParam, filterName))
        .asScala
        .flatMap { ext =>
          contributors
            .getBeanByExtension(ext)
            .getMenuContributions(context)
            .asScala
        }
          ...
          case (_, links) =>
            links.sortBy(_.getLinkPriority).map { mc =>
              ...
              val href = Option(menuLink.getBookmark)
                ...
                .getHref
              val relativized = LegacyContentController
                .relativeURI(href)
                .filter(_.path.parts.last.endsWith(".do"))
              val route = Option(mc.getRoute)
              val iconUrl =
                if (mc.isCustomImage) Some(mc.getBackgroundImagePath) else None
              MenuItem(
    ...
    withTreePath(
      ...
      req.getParameterMap, { info =>
        ...
        renderedResponse(info).getOrElse {
          ajaxResponse(info,
                       info.getAttributeForClass(classOf[AjaxRenderContext]))
        }
      }
    )
  }
  ...
  def submit(@PathParam("path") _path: String,
    ...
    withTreePath(
      ...
      params.asJava, { info =>
        info.preventGET()
        info.getRootRenderContext.setRootResultListener(
          new LegacyResponseListener(info))
        LegacyContentController.execute(info)
        redirectResponse(info)
          .orElse(renderedResponse(info))
          .orElse(Option(info.getAttributeForClass(classOf[AjaxRenderContext]))
  ...
  def redirectResponse(info: MutableSectionInfo): Option[ResponseBuilder] = {
    ...
    Option(
      req
        .getAttribute(LegacyContentController.RedirectedAttr)
        .asInstanceOf[String]).map { url =>
      Response.ok {
        LegacyContentController.relativeURI(url) match {
          case None => ExternalRedirect(url)
          case Some(relative) =>
            InternalRedirect(relative.toString, userChanged(req))
        }
      }
    }
  }

  class LegacyResponseListener(info: MutableSectionInfo)
      extends RenderResultListener {

    override def returnResult(result: SectionResult, fromId: String): Unit = {
      val context =
        info.getRootRenderContext.asInstanceOf[StandardRenderContext]
      val decs = Decorations.getDecorations(info)
      val html = result match {
        case tr: TemplateResult =>
          ...
          val upperbody = SectionUtils.renderToString(
            context,
            tr.getNamedResult(context, "upperbody"))
          ...
          val crumbs = renderCrumbs(context, decs).map(
            SectionUtils.renderToString(context, _))
          Iterable(
            Some("body" -> body),
        ...
        case sr: SectionRenderable =>
          Map(
            "body" -> SectionUtils.renderToString(context,
                                                  wrapBody(context, sr)))
        case pr: PreRenderable =>
          Map(
            "body" -> SectionUtils.renderToString(context,
                                                  new PreRenderOnly(pr)))
      }
      ...
      val scripts =
        preRenderPageScripts(context, context).map(_.getStatements(context))
      ...
      val preventUnload = context.getBody.getHandler(
        JSHandler.EVENT_BEFOREUNLOAD) != null
  ...
  private def preRenderPageScripts(
    ...
    while ({
      ...
      val statements: util.List[JSStatements] =
        new util.ArrayList[JSStatements](origStatements)
  ...
  def wrapBody(context: RenderContext,
               body: SectionRenderable): SectionRenderable = {
    ...
    if (decs.isBanner || !decs.isMenuHidden || decs.isContent) {
      ...
      val citag = new TagState("content-inner")
        .addClass[TagState](decs.getPageLayoutDisplayClass)
  ...
  def renderCrumbs(context: RenderContext,
                   d: Decorations): Option[SectionRenderable] = {
    val bc = Breadcrumbs.get(context)
    if (d.isForceBreadcrumbsOn || (d.isBreadcrumbs && !bc.getLinks.isEmpty))
      Option {
  ...
  def renderedResponse(info: MutableSectionInfo) = {
    Option(info.getRootRenderContext.getRenderedResponse).map { sr =>
      ...
      Response.ok(
        SectionUtils
          .renderToString(LegacyContentController.prepareJSContext(info), sr))
    }
  }
  ...
  def ajaxResponse(info: MutableSectionInfo, arc: AjaxRenderContext) = {
    ...
    def renderAjaxBody(sr: SectionRenderable): Unit = {
      ...
      if (formTag.getAction == null) {
        ...
        formTag.setAction(
          new InfoFormAction(new InfoBookmark(context, bookmarkEvent)))
      }
      ...
      context.processEvent(
        new RenderEvent(
          ...
          new RenderResultListener {
            override def returnResult(result: SectionResult,
                                      fromId: String): Unit =
              bodySR = result
          }
        ))
