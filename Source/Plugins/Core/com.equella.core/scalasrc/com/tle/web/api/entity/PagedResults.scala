...
object PagedResults {
  ...
  def pagedResults[BE <: BaseEntity, BEB <: BaseEntityBean](
    ...
    val privilege =
      if (_privilege.isEmpty) Set("LIST_" + res.getPrivilegeType)
      else _privilege.asScala.toSet
    val forFull =
      Set("VIEW_" + res.getPrivilegeType, "EDIT_" + res.getPrivilegeType)

    val allReqPriv = if (full) forFull ++ privilege else privilege

    @tailrec
    def collectMore(len: Int,
                    offset: Int,
                    tried: Int,
                    vec: Vector[(BE, Boolean, Set[String])])
      : (Int, Vector[(BE, Boolean, Set[String])]) = {
    ...
    val (nextOffset, results) =
      collectMore(length, firstOffset, 0, Vector.empty)
    ...
    pb.setResults(results.map {
      case (be, canFull, privs) =>
        addPrivs(privs, res.serialize(be, null, canFull))
    }.asJava)
    pb
  }
}
