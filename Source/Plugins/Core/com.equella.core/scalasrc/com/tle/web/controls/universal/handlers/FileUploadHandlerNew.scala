...
import com.tle.mycontent.web.selection.{
  MyContentSelectable,
  MyContentSelectionSettings
}
...
import com.tle.web.sections.standard.renderers.{
  DivRenderer,
  FileDropRenderer,
  LinkRenderer
}
...
@Bind
class FileUploadHandlerNew
    extends AbstractAttachmentHandler[FileUploadHandlerModel] {
  ...
  def getLabel: AttachmentHandlerLabel =
    new AttachmentHandlerLabel(LABEL_NAME, LABEL_DESCRIPTION)
  ...
  def render(context: RenderContext,
             renderOptions: DialogRenderOptions): SectionRenderable = {
  ...
  def remove(info: SectionInfo,
             attachment: Attachment,
             willBeReplaced: Boolean): Unit = {
  ...
  @EventHandlerMethod def startSelection(info: SectionInfo) =
    getModel(info).selecting = true
  ...
  @AjaxMethod def uploadCommand(info: SectionInfo): SectionResult =
    ctx.processUploadCommand(info)

  class AfterRegister(id: String,
                      tree: SectionTree,
                      val controlState: UniversalControlState)
    ...
    val scrapBookOnClick =
      Option(new AnonymousFunction(events.getNamedHandler("startSelection")))
    ...
    val controlSettings = new FileUploadSettings(
      controlState.getControlConfiguration)

    val _resultsCallback = new PassThroughFunction(
      "r" + id,
      events.getSubmitValuesFunction("selectionsMade"))

    val showRestrict = hasInstitutionPrivilege(
      AttachmentConfigConstants.RESTRICT_ATTACHMENTS)
    ...
    val packageEditDetails = new PackageEditDetails(id,
                                                    tree,
                                                    this,
                                                    this,
                                                    showRestrict,
                                                    getEditingAttachment)
    val fileOptions = new FileOptions(id,
                                      tree,
                                      controlSettings,
                                      this,
                                      _ => singleUpload,
    ...
    def detailsEditorForAttachment(a: Attachment): DetailsPage =
      if (WebFileUploads.isPackageAttachment(a)) packageEditDetails
      else fileEditDetails
    ...
    case class DetailsEditState(
      ...
      def commit(): Unit = (a, zipAttachment) match {
        case (fa: FileAttachment, true) =>
          stagingContext.moveFile(fa.getFilename,
                                  WebFileUploads.zipPath(fa.getFilename))
          commitUnzipPaths.foreach(p =>
            stagingContext.moveFile(p._1, fa.getFilename))
    ...
    def getEditState =
      editStateO.getOrElse(
        sys.error("Trying to use edit state when none exists"))

    def setEditState(d: DetailsEditState) = editStateO = Some(d)

    def allValidatedUploads =
      CurrentUpload.validatedUploads(state.allCurrentUploads)
    ...
    class DetailsModel(info: SectionInfo) {
      ...
      val getThumbnail = resource.createStandardThumbnailRenderer(
        new TextLabel(resource.getDescription))
        ...
        if (m.selecting) renderSelection(context)
          ...
          if (m.resolved) {
            ...
            (CombinedRenderer.combineResults(
               renderModel("file/file-editheader.ftl",
                           new DetailsModel(context)),
               _r),
             _f)
    ...
    def renderFileUpload(context: RenderContext)
    ...
    def resolvedType(info: SectionInfo, vu: ValidatedUpload): Unit = {
      ...
      state.remove(vu.id)
      setEditState(
        DetailsEditState(a, ed, creator.commit, creator.cancel, false))
      ed.prepareUI(info)
    }
    ...
    def renderEditing(context: RenderContext)
      : (SectionRenderable, DialogRenderOptions => Unit) = {
      val s = getEditState
    ...
    def editAttachment(info: SectionInfo, attachment: Attachment): Unit = {
      ...
      editState.commit()
      val (newAttach, delattach) = detailsEditorForAttachment(attachment)
        .editAttachment(info, attachment, this)
      ...
      delattach.foreach { ad =>
        ...
        ad.attachments.foreach(a =>
          controlState.removeMetadataUuid(info, a.getUuid))
    ...
    def prepareOptions(context: RenderContext,
                       uploads: Iterable[CurrentUpload])(
      ...
      if (uploads.forall(_.finished)) {
        val allValidated = allValidatedUploads
        if (allValidated.size == 1 && fileOptions
              .defaultResolve(singleUpload)
              .isEmpty) {
          optionsButton.setClickHandler(context,
                                        events.getNamedHandler("nextPage"))
    ...
    def mimeTypeForFilename(name: String): String =
      mimeTypeService.getMimeTypeForFilename(name)
    ...
    def renderSelection(context: RenderContext)
      : (SectionRenderable, DialogRenderOptions => Unit) = {
      val forward: SectionInfo = selectionService.getSelectionSessionForward(
        context,
        initSession,
        myContentSelectable)
      (renderModel("file/file-selection.ftl",
                   SelectionModel(new InfoBookmark(forward).getHref)),
    ...
    def selectionsMade(info: SectionInfo,
                       selectedResources: Iterable[SelectedResourceDetails]) = {
      selectedResources.foreach { resource =>
        ...
        files.asScala.foreach { fa =>
          ...
          WebFileUploads.uploadStream(
            ...
            () =>
              fileSystemService.read(itemFileService.getItemFile(item),
                                     fa.getFilename),
            this)
        }
      }
        ...
        val (commitUnzipPaths, zp) = eds.a match {
          case fa: FileAttachment =>
            if (eds.commitUnzipPaths.isEmpty) {
              ...
              (Some((target, fa.getFilename)),
               stagingContext.unzip(srcZip, target))
            } else
              (eds.commitUnzipPaths,
               eds.zipProgress.getOrElse(EmptyZipProgress))
    ...
    def removeUnzipped: Unit =
      setEditState(getEditState.copy(zipAttachment = false))
    ...
    def processUploadCommand(info: SectionInfo): SectionResult = {
        ...
        state.uploadForId(uploadId) match {
          case Some(uf: UploadingFile) =>
            def illegal(reason: IllegalFileReason)
              : (CurrentUpload, AjaxUploadResponse) = {
              val u = uf.failed(IllegalFile(reason))
              (u,
               UploadFailed(
                 WebFileUploads
                   .labelForIllegalReason(reason, uf.originalFilename)
                   .getText))
            }
            val mimeType = mimeTypeForFilename(uf.originalFilename)
            val (fu, r) = WebFileUploads
              .validateBeforeUpload(mimeType,
      ...
      def replaceFilename =
        Option(dialogState.getDialog.getReplacedAttachment(info)).collect {
          case fa: FileAttachment => fa.getFilename
        }

      val response = Option(request.getParameter("uploadId"))
        .map(UUID.fromString)
        .map(uploadStream)
        .getOrElse {
          ...
          decode[AjaxUploadCommand](jsonSource).fold(throw _, identity) match {
            ...
            case NewUpload(filename, size) =>
              ...
              val uniqueName = WebFileUploads.uniqueName(filename,
                                                         replaceFilename,
                                                         uploadId,
                                                         this)
              WebFileUploads
                .validateBeforeUpload(mimeTypeForFilename(filename),
                                      size,
                                      controlSettings)
                .map { reason =>
                  UploadFailed(
                    WebFileUploads
                      .labelForIllegalReason(reason, filename)
                      .getText)
                }
                .getOrElse {
