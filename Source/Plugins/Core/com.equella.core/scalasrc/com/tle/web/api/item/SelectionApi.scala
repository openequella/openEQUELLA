...
class SelectionApi {
  ...
  def returnSelections(@QueryParam("integid") integid: String,
    ...
    val output =
      SectionUtils.renderToString(rc, SectionUtils.renderSection(rc, "temp"))
  ...
  def toSRK(resKey: SelectionKey): SelectedResourceKey = {
    val key =
      new SelectedResourceKey(new ItemId(resKey.uuid, resKey.version), null)
...
object SelectionApi {
  ...
  val blankTree = new DefaultSectionTree(
    LegacyGuice.treeRegistry,
    new SectionNode("", new AbstractScalaSection {
      override type M = Int
