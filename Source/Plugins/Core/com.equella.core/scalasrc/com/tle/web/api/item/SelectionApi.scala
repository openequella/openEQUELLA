...
class SelectionApi {
  ...
  @POST
  @Path("{sessid}/return")
  def returnSelections(@QueryParam("integid") integid: String, @PathParam("sessid") sessid: String,
                       @Context request: HttpServletRequest, @Context response: HttpServletResponse): Response = {
...
object SelectionApi {
  ...
  val blankTree = new DefaultSectionTree(LegacyGuice.treeRegistry, new SectionNode("", new AbstractScalaSection {
    override type M = Int

    override def newModel: SectionInfo => Int = _ => 1
  }))

  def setupSession(sessid: String, integid: Option[String], request: HttpServletRequest,
                   response: HttpServletResponse) = {
    val paramMap = Iterable(Some("_sl.stateId" -> Array(sessid)), integid.map("_int.id" -> Array(_))).flatten.toMap
    val info = LegacyGuice.sectionsController.createInfo(blankTree, "/", request,
      response, null, paramMap.asJava, null)
