...
import com.tle.web.wizard.controls.{
  AbstractWebControl,
  CCustomControl,
  WebControlModel
}
...
object UniversalWebControlNew {
  ...
  val KEY_HIDDEN_FROM_SUMMARY_NOTE =
    WebFileUploads.r.key("list.hidden.from.summary")
  val uploadListSrc = new IncludeFile(
    WebFileUploads.r.url("reactjs/uploadlist.js"),
    JQueryProgression.PRERENDER,
    FileDropRenderer.CSS,
    ZebraTableRenderer.CSS)
  val uploadListFunc =
    new ExternallyDefinedFunction("PS.inlineUpload", uploadListSrc)
}

@Bind
class UniversalWebControlNew
    extends AbstractWebControl[UniversalWebControlModel] {
  def getModelClass = classOf[UniversalWebControlModel]

  @ViewFactory(name = "wizardFreemarkerFactory") var wizardViewFactory
    : WizardFreemarkerFactory = _
  ...
  def renderHtml(context: RenderEventContext): SectionResult =
    ctx.renderControl(context)
  ...
  override def registered(id: String, tree: SectionTree): Unit = {
    ...
    ctx = new AfterRegister(id,
                            tree,
                            getWrappedControl.asInstanceOf[CCustomControl])
  }
  ...
  class AfterRegister(id: String,
                      tree: SectionTree,
                      storageControl: CCustomControl)
      extends ControlContext
      with RenderHelper {
    val state = new FileUploadState
    val definition = new UniversalSettings(
      control.getControlBean.asInstanceOf[CustomControl])
...
//    val commandModifier = ajax.getModifier("uploadCommand")
    ...
    dialog.setReloadFunction(
      getReloadFunction(true, events.getEventHandler("reloaded")))
...
//      sys.error("metadatamapping")
      ...
      def getDivTag = {
        ...
        val ts = new TagState()
        ts.setId(id + "_r")
        val topLevelAttachments = attachmentTreeService
          .getTreeStructure(dialog.getAttachments, false)
          .asScala
        val uploadArgs = new ObjectExpression(
          ...
          (if (definition.isMaxFilesEnabled) Some(definition.getMaxFiles)
           else Some(1).filterNot(_ => definition.isMultipleSelection))
            .map(Integer.valueOf)
            .orNull,
          "entries",
    ...
    def entryForAttachment(info: SectionInfo,
      ...
      val viewableResource = attachmentResourceService.getViewableResource(
        info,
        repository.getViewableItem,
        a)
      val desc =
        if (Option(dialog.findHandlerForAttachment(a))
              .exists(_.isHiddenFromSummary(a))) {
                  ...
                  case Successful(fileInfo) =>
                    validateContent(info, this, uf.uploadPath) match {
                      ...
                      case Right(detected) =>
                        stateAction(info) {
                          val v =
                            ValidatedUpload(uf.success(fileInfo), detected)
          ...
          decode[AjaxUploadCommand](jsonSource).fold(throw _, identity) match {
            ...
            case NewUpload(filename, size) =>
              stateAction(info) {
                ...
                val uniqueName =
                  WebFileUploads.uniqueName(filename, None, uploadId, this)
                WebFileUploads
                  ...
                  .getOrElse {
                    ...
                    val uploadUrl = ajax
                      .getModifiedAjaxUrl(
                        info,
                        new SimpleBookmarkModifier("uploadId",
                                                   uploadId.toString),
                        "uploadCommand")
                      .getHref
