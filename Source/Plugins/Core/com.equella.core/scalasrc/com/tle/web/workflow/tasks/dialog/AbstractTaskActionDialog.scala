...
import com.tle.web.sections.equella.render.{
  ButtonRenderer,
  UnselectLinkRenderer
}
...
abstract class AbstractTaskActionDialog
    extends AbstractOkayableDialog[TaskActionDialogModel]
    with ReadyToRespondListener {
  ...
  override def instantiateDialogModel(info: SectionInfo) =
    new TaskActionDialogModel
  ...
  private val currentUploads = new ConcurrentHashMap[UUID,
                                                     (String,
                                                      AtomicReference[Boolean],
                                                      Option[Label])]().asScala
  ...
  protected def setupModelForRender(context: RenderContext): Unit = {
    ...
    val fileEntries = fileSystemService.enumerate(
      new StagingFile(stagingFolderUuid),
      null,
      null)
        ...
        val shouldShow = currentUploads.find(_._2._1 == filename) match {
          case None =>
            Some(
              (index.toString,
               true,
               new StatementHandler(removeCallback, filename)))
        ...
        shouldShow.map {
          case (id, finished, removeHandler) =>
            val link = new HtmlLinkState(
              ...
              new SimpleBookmark(
                WorkflowMessageServlet.stagingUrl(stagingFolderUuid, filename)))
    ...
    _fileDrop.setAjaxUploadUrl(context,
                               ajaxEvents.getAjaxUrl(context, "dndUpload"))
    _fileDrop.setValidateFile(context, validateFile)
  }

  override protected def getRenderableContents(
      context: RenderContext): SectionRenderable = {
    setupModelForRender(context)
    viewFactory.createResult("dialog/taskaction.ftl", this)
  }

  override protected def getContentBodyClass(context: RenderContext) =
    "taskactiondialog"
  ...
  def setSuccessCallable(successCallable: JSCallable) =
  ...
  @EventHandlerMethod def ok(info: SectionInfo): Unit = {
    ...
    if (errorMessage == null) {
      val currentTaskSection: CurrentTaskSection =
        info.lookupSection(classOf[CurrentTaskSection])
  ...
  def dndUpload(info: SectionInfo): Boolean = {
    ...
    AjaxUpload.writeCancellableStream(
      ...
      case Failure(f) =>
        fileSystemService.removeFile(staging, fn)
        f match {
          case bfe: BannedFileException =>
            currentUploads.put(uploadId, (fn, uploadRef, Some(error("banned"))))
  ...
  override def treeFinished(id: String, tree: SectionTree): Unit = {
    ...
    setCancelHandler(new StatementHandler(Js.call_s(new RuntimeFunction() {
      override protected def createFunction(info: RenderContext) =
        new PassThroughFunction("cl",
                                events.getSubmitValuesFunction("cancelled"))
    })))
    ...
    val errorCallback =
      PartiallyApply.partial(events.getSubmitValuesFunction("illegalFile"), 2)
  ...
  override protected def collectFooterActions(
      context: RenderContext): util.Collection[Button] = {
