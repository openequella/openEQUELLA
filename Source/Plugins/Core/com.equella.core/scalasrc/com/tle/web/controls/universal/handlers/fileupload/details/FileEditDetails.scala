...
import com.tle.web.sections.annotations.{
  Bookmarked,
  EventFactory,
  EventHandlerMethod
}
...
object FileEditDetails {
  ...
  private val INCLUDE = new IncludeFile(
    WebFileUploads.r.url("scripts/file/zip.js"))
  private val SELECT_ALL =
    new ExternallyDefinedFunction("zipSelectAll", INCLUDE)
  private val SELECT_FUNCTION =
    new ExternallyDefinedFunction("zipSelect", INCLUDE)
...
class FileEditDetails(parentId: String,
  ...
  @Component
  @PlugKey("handlers.file.details.link.editfilewith") var editFileWithLink
    : Link = _
  @Component
  ...
  @EventHandlerMethod def editFile(info: SectionInfo,
                                   openWith: Boolean): Unit = {
  ...
  @AjaxMethod def zipProgress(info: SectionInfo): ZipJson = {
    zipHandler.zipProgress
      .map(zp =>
        new ZipJson(zp.getTotalFiles, zp.getCurrentFile, zp.isFinished))
  ...
  override def registered(id: String, tree: SectionTree): Unit = {
    ...
    editFileLink.setClickHandler(
      inplaceEditorService.createOpenHandler(
        ...
        false,
        Js.function(
          Js.call_s(editFileAjaxFunction, false.asInstanceOf[AnyRef]))))
    editFileWithLink.setClickHandler(
      inplaceEditorService.createOpenHandler(
        INPLACE_APPLET_ID,
        true,
        Js.function(
          Js.call_s(editFileAjaxFunction, true.asInstanceOf[AnyRef]))))

    editFileLink.addReadyStatements(
      inplaceEditorService.createHideLinksStatements(
        Jq.$(Type.CLASS, "editLinks"),
        Jq.$(editFileWithLink)))
    ...
    selectAll.setClickHandler(
      new OverrideHandler(Js.call_s(SELECT_ALL, true.asInstanceOf[Object])))
  ...
  case class FileEditDetailsModel(info: SectionInfo) {
    ...
    lazy val getErrors =
      (if (validate) validateDisplayName(info).toMap
       else Map.empty[String, Label]).asJava
    ...
    def isZipFile =
      isUnzipped || (!settings.isNoUnzip && WebFileUploads.isZipFile(a))
    ...
    lazy val getFileListDiv = {
      ...
      def mkNodeChildren(path: String): ObjectExpression =
        new ObjectExpression("folder",
                             false.asInstanceOf[Object],
                             "children",
                             pa.getOrElse(path, Seq.empty)
                               .filter(!_.isFolder)
                               .map(_.id)
                               .asJavaCollection)
      ...
      fileListDiv.addReadyStatements(info,
                                     new AssignStatement(SELECTION_TREE, map))
      fileListDiv
    }
  ...
  def prepareUI(info: SectionInfo): Unit = {
    ...
    et match {
      case za: ZipAttachment =>
        selections.setCheckedSet(info,
                                 zipHandler.selectedAttachments.keySet.asJava)
  ...
  def editAttachment(
    ...
    val (a, delete) = (_a, unzipped) match {
      ...
      case (za: ZipAttachment, false) =>
        ...
        (fa,
         Some(
           AttachmentDelete(ctx.controlState.getAttachments.asScala
                              .filter(WebFileUploads.isSelectedInZip(za)),
                            _ => ())))
      case _ => (_a, None)
    }
    if (unzipped) {
  ...
  def prependParent(parent: String,
                    filename: String,
                    sep: String = "/"): String =
    if (parent.isEmpty) filename else parent + sep + filename

  case class EntryFullPath(parentPath: String,
                           level: Int,
                           entry: FileEntry,
                           displayPath: String) {
  ...
  class EntryDisplay(fp: EntryFullPath,
                     check: HtmlBooleanState,
                     val id: String) {
  ...
  def convertFileList(
    ...
    val (files, _folders) = entries
      .map(
        e =>
          EntryFullPath(parentPath,
                        level,
                        e,
                        prependParent(displayParent, e.getName, " / ")))
      .partition(_.file)
    val folders = _folders.flatMap { fed =>
      ...
      if (childFiles.isEmpty) childFolders
      else fed +: (childFiles ++ childFolders)
    }
    (files, folders)
  }
