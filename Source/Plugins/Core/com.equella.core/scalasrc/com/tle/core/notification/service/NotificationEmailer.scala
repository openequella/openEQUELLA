...
class NotificationEmailer(batched: Boolean,
  ...
  override def call: java.lang.Iterable[Callable[EmailResult[EmailKey]]] = {
    ...
    val reasonMap =
      dao.getReasonCounts(user, attemptId).asScala.mapValues(_.intValue()).toMap
    val canSend = emailService.hasMailSettings && Option(
      userBean.getEmailAddress).exists(_.nonEmpty)
        ...
        (extO, canSend) match {
          case (Some(ext), true) =>
            ...
            ext.emails(userBean, newestNotes, reasonMap).map { ne =>
              ...
              def successCB(): Unit = {
                val (p, d) =
                  ne.pertainsTo.partition(n => ext.isIndexed(n.getReason))
