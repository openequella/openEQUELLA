...
object AclChecks {
  ...
  def ensureOnePriv[A](privs: String*)(db: DB[A]): DB[A] =
    for {
      ...
      r <- if (actualPrivs.isEmpty)
        throw new PrivilegeRequiredException(privs.asJavaCollection)
      else db
    } yield r
  ...
  def filterNonGrantedPrivileges(
      ...
      if (currentUser.isSystem || privileges.isEmpty)
        ...
        val exps =
          if (includePossibleOwnerAcls)
            _exps ++ currentUser.getOwnerAclExpressions.asScala
          else _exps
        ...
        def processPriv(s: PrivState, entry: PrivEntry): PrivState = {
          ...
          if (s.granted(priv) || s
                .revokedOverall(priv) || (objEntry && s.revokedPerObj
                .get(target)
                .exists(_(priv)))) s
          else if (entry.agg.endsWith(Character.toString(GRANT)))
            s.copy(granted = s.granted + priv)
          else if (!objEntry) s.copy(revokedOverall = s.revokedOverall + priv)
          else
            s.copy(
              revokedPerObj = s.revokedPerObj.updated(
                target,
          ...
          .rowsStream(
            StateT.inspectF { con =>
              IO {
                ...
                privileges.zipWithIndex.foreach {
                  case (p, i) => ps.setString(2 + i, p)
                }
                exps.zipWithIndex.foreach {
                  case (e, i) => ps.setLong(2 + privileges.size + i, e)
                }
                ps.executeQuery()
              }
            }
