...
object AclChecks {
  ...
  private val OBJECT_PRIORITY_DB_FORMAT = f"${SecurityConstants.PRIORITY_OBJECT_INSTANCE + SecurityConstants.PRIORITY_MAX}%04d"
  ...
  def ensureOnePriv[A](privs: String*)(db: DB[A]): DB[A] = for {
    actualPrivs <- filterNonGrantedPrivileges(privs, true)
    r <- if (actualPrivs.isEmpty) throw new PrivilegeRequiredException(privs.asJavaCollection) else db
  } yield r

  case class PrivEntry(agg: String, priv: String, target: String)

  case class PrivState(granted: Set[String] = Set.empty, revokedOverall: Set[String] = Set.empty,
    ...
    uc: UserContext =>
      ...
      if (currentUser.isSystem || privileges.isEmpty) privileges.toSet.pure[JDBCIO]
        ...
        def processPriv(s: PrivState, entry: PrivEntry): PrivState = {
          ...
          if (s.granted(priv) || s.revokedOverall(priv) || (objEntry && s.revokedPerObj.get(target).exists(_ (priv)))) s
          ...
          else s.copy(revokedPerObj = s.revokedPerObj.updated(target,
        ...
        JDBCQueries.rowsStream(
          ...
          PrivEntry(rs.getString(1),
            rs.getString(2),
            rs.getString(3))
        }.fold(PrivState())(processPriv).compile.last.map {
