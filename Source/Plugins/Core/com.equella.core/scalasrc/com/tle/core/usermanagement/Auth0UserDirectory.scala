...
class Auth0UserDirectory extends ApiUserDirectory {
  ...
  override protected implicit val userDecoder: Decoder[IdPUser] = Decoder.instance { cursor =>
    for {
      ...
      username    <- cursor.downField("username").as[Option[String]]
      firstName   <- cursor.downField("given_name").as[Option[String]]
      lastName    <- cursor.downField("family_name").as[Option[String]]
  ...
  /** Auth0 also supports using query parameter `q` to search against user attributes, with each
    * condition in the format of `attr:"value"`, but wildcard searches against custom attributes are
    * not supported.
    *
    * Double quotes around the value are required to prevent syntax errors when the value contains
    * special characters like `*`.
    *
    * Multiple criteria can be combined using `OR` or `AND`.
    */
  override protected def userListByAttrsEndpoint(
    ...
    val filters = attrs.map(attr => s"$attr:\"$value\"").mkString(" OR ")
    Uri(idp.apiUrl.toURI).addPath("users").addParam("q", filters)
  }
