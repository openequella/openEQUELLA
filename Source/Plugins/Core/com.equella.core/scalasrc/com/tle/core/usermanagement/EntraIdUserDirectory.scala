...
/**
  ...
  */
final case class EntraIdUser(id: String,
...
class EntraIdUserDirectory extends ApiUserDirectory {
  ...
  /**
    ...
    * search result is using their 'search' parameter;
    * 2. The 'search' parameter does NOT query multiple fields so we need to specify the fields;
    * 3. The query passed to this method ALWAYS has a prefix of '*' and a suffix of '*'. Check
    * [[com.tle.core.services.user.impl.UserServiceImpl#fixQuery(java.lang.String)]] for the reason.
    *
    * As a result, this method will remove the prefix and suffix and then build a search param
    * to support querying 'displayName', 'mail', 'userPrincipalName and maybe more if required.
    *
    * References:
  ...
  /**
    * According to the doco of OIDC scope, the scope `https://graph.microsoft.com/.default`
    * must be included in the request in order to obtain an access token that is permitted
    * to use Graph REST API.
    *
  ...
  /**
    * According to the doco of Advanced query, the header `ConsistencyLevel` must be included with value being `eventual`.
    *
