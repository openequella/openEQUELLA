...
object ViewCountJavaDao {
  ...
  def incrementSummaryViews(itemKey: ItemKey): Unit =
    RunWithDB.executeWithHibernate {
      Kleisli { uc =>
        queries
          ...
          .flatMap {
            case Some(c) =>
              queries.writeItemCounts.update(
                c,
                c.copy(count = c.count + 1, last_viewed = Instant.now()))
            case _ =>
              val newCount = ItemViewCount(uc.inst,
                                           itemKey.getUuid,
                                           itemKey.getVersion,
                                           1,
                                           Instant.now())
  ...
  def incrementAttachmentViews(itemKey: ItemKey, attachment: String): Unit =
    RunWithDB.executeWithHibernate {
      Kleisli { uc =>
        queries
          .attachmentCount(
            (uc.inst, itemKey.getUuid, itemKey.getVersion, attachment))
          .last
          .flatMap {
            case Some(c) =>
              queries.writeAttachmentCounts.update(
                c,
                c.copy(count = c.count + 1, last_viewed = Instant.now()))
            case _ =>
  ...
  def getSummaryViewCount(itemKey: ItemKey): Int =
    RunWithDB.executeWithHibernate {
  ...
  def getSummaryViewsForCollection(col: ItemDefinition): Int =
    RunWithDB.executeWithHibernate {
      Kleisli { uc: UserContext =>
  ...
  def getAttachmentViewsForCollection(col: ItemDefinition): Int =
    RunWithDB.executeWithHibernate {
      Kleisli { uc: UserContext =>
  ...
  def deleteForItem(item: ItemKey): Unit = RunWithDB.executeWithHibernate {
    Kleisli { uc: UserContext =>
      queries
        .deleteForItemId(uc.inst, item.getUuid, item.getVersion)
        .flush
        .compile
        .drain
    }
