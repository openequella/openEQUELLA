...
public class LinuxOpener extends AbstractListDialogOpener {
  ...
  protected List<App> getAppList(String filepath, String mimetype) throws IOException {
    ...
    if (Check.isEmpty(dirs)) {
      throw new UnsupportedOperationException(
          "Linux variation is unsupported - environment variable " + XDG_DATA_DIR + " was blank");
    }
  ...
  private String[] getDesktopsForMimeType(
      File fd, String mimeType, boolean haveTriedAndWarnedInOtherDir) {
    String rv =
        getValueForFile(new File(fd, "mimeinfo.cache"), mimeType, haveTriedAndWarnedInOtherDir);
    return rv == null ? null : rv.split(";");
  }

  private String getValueForFile(File file, String key, boolean haveTriedAndWarnedInOtherDir) {
    ValueReaderFromFile valueReaderFromFile =
        new ValueReaderFromFile(file, key, haveTriedAndWarnedInOtherDir);
  ...
  /**
   * An alternative to an inline anonymous class - this way we can get the returned property value.
   */
  ...
  private static class ValueReaderFromFile implements PrivilegedAction<Object> {
    ...
    protected ValueReaderFromFile(
        File fileToRead, String key, boolean haveTriedAndWarnedInOtherDir) {
