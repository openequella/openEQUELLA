 ...
 * java.net/projects/rococoa/ is its home, with some useful if minimal chatter on
 * java.net/projects/rococoa/lists/users/archive For Cocoa Objective-C generally, www.cocoadev.com
 * www.stackoverflow.com has some useful tagged queries.
 *
 * @author larry
 */
...
public class MacOpener extends AbstractListDialogOpener {
  ...
  public interface CCarbonWrapper extends Library {
    ...
    int LSCopyApplicationForMIMEType(
        Object cfstrMIMETYPE, int rolesMask, PointerByReference cfurlReturned);

    int LSGetApplicationForURL(
        Object cfURL, int rolesMask, Void fileRef, PointerByReference cfurlReturned);
    ...
    Pointer CFURLCreateWithFileSystemPath(
        Void alloc, Object cfstr, int pathStyle, boolean isDirectory);
  ...
  /**
   * To quote from http://lopica.sourceforge.net/faq.html ("Unofficial Java Web Start/JNLP FAQ")
   * "Note, that if you load classes with your own ClassLoader, you should also install your own
   * SecurityManager (or none) by calling System.setSecurityManager( null ), otherwise you might run
   * into security access violations even if you signed all your jars. Web Start's built-in security
   * manager only assigns all permissions to the classes loaded by its own JNLPClassLoader." A
   * reference to JavaWebStartSecurityManager if such a thing exists ...? And furthermore ... "Q:
   * How can I turn off the sandbox? If your app is signed, you can call
   * System.setSecurityManager(null) to turn off the sandbox. If you get rid off the security
   * manager, your app should speed up as it no longer goes through security layers. Don't expect
   * miracles, though." Roger that: miracles not expected ... ?
   */
  @Override
        ...
        new PrivilegedAction<Object>() {
          ...
          public Object run() {
            try {
              ...
              LOGGER.info(
                  "Attempting Native.loadLibrary('"
                      + MAC_CARBON_PATH
                      + ", ...') where 'jna.library.path' is "
                      + jnaLibPath
                      + ", and 'java.library.path' is "
                      + javaLibPath);
              ...
              LOGGER.info(
                  "Progressed past loadLibrary(\"" + ROCOCOA + "\"), about to call executeApp");
  ...
  protected List<App> getAppList(String filepath, String mimetype) throws IOException {
    ...
    LOGGER.info(
        "About to call LSGetApplicationForURL with url ("
            + curlstr
            + ") from filepath - "
            + filepath);
    ...
    if (mimetype != null && mimetype.length() > 0) {
      ...
      int retFromMimeType =
          ccarbon.LSCopyApplicationForMIMEType(cfstrMimetype, kLSRolesAll, cfurlFromMimeType);
      String preferredAppNameFromMimeType =
          processLSResult(cfurlFromMimeType, retFromMimeType, "LSCopyApplicationForMIMEType");
  ...
  protected void executeApp(App app, String filepath, String mimetype) throws IOException {
    ...
    try {
      ...
      boolean isRunning =
          nsWorkspace.openFile_withApplication(
              NSString.stringWithString(filepath), NSString.stringWithString(app.getExec()));
      LOGGER.info(
          "Called "
              + app.getExec()
              + " to open "
  ...
  private String processLSResult(PointerByReference cfURLreturn, int ret, String functionName) {
    ...
    switch (ret) {
        ...
        if (retptr != null) {
          ...
          if (cfstr != null) {
            ...
            LOGGER.info(
                "From call to "
                    + functionName
                    + ", preferred app from filepath is "
                    + preferredApp);
          }
      ...
      case -10814:
        LOGGER.info(
            "Called " + functionName + " with result (" + ret + ") kLSApplicationNotFoundErr");
        break;
      default:
        LOGGER.info(
            "Called "
  ...
  /**
   * Calls LSCopyApplicationURLsForURL to get all LaunchServices registered applications which claim
   * to be able to handle the named file.
   *
   * @param filepath local url filepath
   * @param appList may be null, if so it is initialised and populated. If not null, it is appended
   *     to with such discovered app names not already present.
   * @return appList
   */
  private List<App> getAllApplicationsForFilepath(
      Pointer curlref, List<App> appList, int rolesMask) {
    if (appList == null) {
  ...
  /**
   * This is here as a proof-of-concept pilot, but it may have a use eventually.
   *
   * @param mimetype eg "text/xml
   * @return what LaunchServices treats as the readable kind type, (eg "Text document"), null if
   *     failure or empty string if no result.
   */
  ...
  /**
   * The executeApp method takes application identifiers as strings, not URL's. Apple's inquiry
   * returns file:// URLs, which we decode (so 'Microsoft%20Word' becomes 'Microsoft Word', we strip
   * off the file://host preamble and strip any trailing spaces.
   */
  ...
  /**
   * utility string manip function Apple Applications are usually executable directories (eg
   * /Applications/TextEdit.app/ ) and thus are quite likely to be returned as having a trailing
   * separator. We remove it both for neatness sake and to facilitate extracting the app name from
   * the file path, in that the last occurrence of '/' will precede the app name.
   */
   ...
   * @return cfstr contents converted to java String. Never null, but may be zero length
   */
  ...
  /**
   * Some of these permissions are probably superfluous, but most, including nonsensical ones, are
   * required by jni at some point.
   *
   * @param permissions
   */
  private static void loadUpPermissions(Permissions permissions) {
    ...
    permissions.add(
        new FilePermission("/System/Library/Java/Extensions/libFoundation.dylib", "read"));
    LOGGER.info(
        "Added read file permission for /System/Library/Java/Extensions/libFoundation.dylib");
    ...
    permissions.add(
        new FilePermission("/System/Library/Java/Extensions/libFoundation.jnilib", "read"));
    LOGGER.info(
        "Added read file permission for /System/Library/Java/Extensions/libFoundation.jnilib");
    ...
    permissions.add(
        new FilePermission("/System/Library/Java/Extensions/" + LIBROCOCOA_JNILIB, "read"));
    LOGGER.info(
        "Added read file permission for /System/Library/Java/Extensions/" + LIBROCOCOA_JNILIB);
  ...
  /**
   * All my attempts to make use of CFURLGetFSRef result in crashing the browser, even after the
   * function appears to have been called successfully, and has logged the fact.
   *
   * @param originalAppName TODO - Get the locale display name from the Application as Bundle
   * @return
   */
