...
public class InstitutionGlobalTask extends AlwaysRunningTask<SimpleMessage> {
  ...
  private static final String KEY_PREFIX =
      PluginServiceImpl.getMyPluginId(InstitutionGlobalTask.class) + '.';
  @Inject private MigrationService migrationService;
  @Inject private SchemaDataSourceService dataSourceService;
  @Inject private InstitutionDao institutionDao;
  ...
  private Institution processCreate(
      CreateInstitutionMessage msg, InstitutionMessageResponse response) {
    long schemaId = msg.getSchemaId();
    final Institution institution = msg.getInstitution();
    Institution newInst =
        dataSourceService.executeWithSchema(
            schemaId,
            new Callable<Institution>() {
              @Override
  ...
  private InstitutionStatus createStatus(Institution institution, long schemaId) {
    ...
    for (InstitutionStatus status : institutionMap.values()) {
      if (status.isValid()) {
        ...
        if (inst.getUniqueId() != institution.getUniqueId()
            && containsDuplicate(institution, inst)) {
  ...
  private boolean containsDuplicate(Institution institution, Institution existing) {
    return existing.getName().equals(institution.getName())
        || existing.getUrl().equals(institution.getUrl())
  ...
  private void processDelete(DeleteInstitutionMessage msg) {
    ...
    changer.add(
        new InstitutionEvent(
            InstitutionEvent.InstitutionEventType.DELETED,
            ImmutableMultimap.of(schemaId, institution)));
  ...
  private void updateSingleInst(long schemaId, Institution institution) {
    ...
    changer.add(
        new InstitutionEvent(
            InstitutionEvent.InstitutionEventType.EDITED,
            ImmutableMultimap.of(schemaId, institution)));
    mergeChanges(changer);
  }
  ...
  protected String getTitleKey() {
    return KEY_PREFIX + "institutionglobaltask";
  }
  ...
  private void processValidate(
      ValidateInstitutionMessage valMsg, InstitutionMessageResponse response) {
  ...
  private void validateField(
      List<InstitutionValidationError> errors,
      String id,
      String displayKey,
      String value,
      int maxLength,
      boolean ensureLettersOrDigits) {
    if (Check.isEmpty(value)) {
      errors.add(
          new InstitutionValidationError(id, validateString("blank", validateString(displayKey))));
    } else if (value.length() > maxLength) {
      errors.add(
          new InstitutionValidationError(
              id, validateString("long", validateString(displayKey), maxLength)));
    } else if (ensureLettersOrDigits && !value.matches("^[A-Za-z0-9]+$")) {
      errors.add(
          new InstitutionValidationError(
              id, validateString("letters", validateString(displayKey))));
    }
  }
  ...
  private List<InstitutionValidationError> validate(Institution institution) {
    ...
    for (InstitutionStatus status : institutionMap.values()) {
      ...
      if (existing.getFilestoreId().equals(filestoreId)) {
        errors.add(
            new InstitutionValidationError(FIELD_FILESTORE, inUseString("filestore", filestoreId)));
      }
      ...
      if (uri != null) {
        ...
        if (uri.getHost().equals(otherUri.getHost()) && uri.getPort() == otherUri.getPort()) {
          ...
          if (msg != null) {
            errors.add(
                new InstitutionValidationError(
                    FIELD_URL, validateString("url." + msg, otherUri.toString())));
          }
        }
      }
      if (quota < 0) {
