...
/**
 * Caches changes in-memory only, and only pushes them to the "global" cache when you call <code>
 * flush()</code>. This works really well when you have a cache that is updated very frequently and
 * you don't care that the data isn't replicated immediately, or that you only care about the last
 * value of a key that's been updated multiple times since the last flush. For example, this is used
 * when keeping track of user sessions access timestamps so that we don't hit the DB for every
 * access, but instead get batched up and push in one hit.
 *
 * @author Nick Read
 */
public class BatchedCache<V extends Serializable> implements ReplicatedCache<V> {
  ...
  public BatchedCache(InstitutionService service, ReplicatedCache<V> delegate) {
    ...
    additions =
        service.newInstitutionAwareCache(
            new CacheLoader<Institution, Map<String, V>>() {
              @Override
  ...
  /**
   * Automatically flush when putting or invalidating if it's been more than the given time period.
   */
  ...
  public Iterable<Pair<String, V>> iterate(final String keyPrefixFilter) {
    ...
    Iterable<Pair<String, V>> addIter =
        Iterables.transform(
            entrySet,
