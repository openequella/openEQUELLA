...
public class SchemaDataSourceServiceImpl implements SchemaDataSourceService, SchemaListener {
  ...
  private final LoadingCache<Long, DataSourceHolder> schemaDatasources =
              ...
              new CacheLoader<Long, DataSourceHolder>() {
                ...
                public DataSourceHolder load(Long id) {
                  ...
                  return dataSourceService.getDataSource(
                      schema.getUrl(), schema.getUsername(), schema.getPassword());
                }
              });

  private final LoadingCache<Long, DataSourceHolder> reportingSchemaDatasources =
      CacheBuilder.newBuilder()
          .build(
              new CacheLoader<Long, DataSourceHolder>() {
                @Override
                public DataSourceHolder load(Long id) {
                  ...
                  return dataSourceService.getDataSource(
                      tryElse(schema.getReportingUrl(), connectionDetails[0]),
  ...
  public void removeSchemaDataSource(long schemaId) {
    ...
    if (!schema.isUseSystem() && !isSystemSchemaEquivalent(schema)) {
      ...
      dataSourceService.removeDataSource(
          schema.getUrl(), schema.getUsername(), schema.getPassword());
    }
  }
  ...
  public String[] getConnectionDetails(DatabaseSchema schema) {
    if (schema.isUseSystem()) {
      return new String[] {
        dataSourceService.getSystemUrl(),
        dataSourceService.getSystemUsername(),
  ...
  @Override
  public <V> Future<V> executeWithSchema(
      ExecutorService executor, final long schemaId, final Callable<V> code) {
