...
public class TransactionModule extends AbstractModule {
  private static AnnotationTransactionAttributeSource attributeSource =
  ...
  protected void configure() {
    ...
    bindInterceptor(
        Matchers.not(Matchers.annotatedWith(SystemDatabase.class)),
        transactionMatcher,
        getMainInterceptor());
    bindInterceptor(
        Matchers.annotatedWith(SystemDatabase.class), transactionMatcher, getSystemInterceptor());
  }
  ...
  private synchronized MethodInterceptor getMainInterceptor() {
    if (mainInterceptor == null) {
      mainInterceptor =
          new TLETransactionInterceptor(
              attributeSource, getProvider(TLETransactionManager.class), "main", false);
    }
  ...
  private synchronized MethodInterceptor getSystemInterceptor() {
    if (systemInterceptor == null) {
      systemInterceptor =
          new TLETransactionInterceptor(
              attributeSource, getProvider(TLETransactionManager.class), "system", true);
    }
