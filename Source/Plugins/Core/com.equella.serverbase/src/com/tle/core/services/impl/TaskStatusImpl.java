...
public class TaskStatusImpl implements TaskStatus, Serializable {
  ...
  @Override
  public synchronized <T extends Serializable> Pair<Integer, List<T>> getTaskLog(
      int askedOffset, int max) {
    ...
    return new Pair<Integer, List<T>>(
        askedOffset, Lists.newArrayList(taskLog.subList(offset, offset + max)));
  }
