...
public interface AbstractTreeDao<T extends TreeNodeInterface<T>> extends GenericDao<T, Long> {
  ...
  /**
   * Lists node IDs in top-down order. The only ordering guarantee is that a node is guaranteed to
   * show before any node with more parents. For example, all the root nodes are guaranteed to be
   * before any of the first level nodes, which are all guaranteed to be before any second level
   * nodes, and so on. Nodes at the same depth but from different parents may be mixed together.
   */
