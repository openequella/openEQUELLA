...
public abstract class Task implements Callable<Void> {
  /**
   * Enum values should be specified from highest to lower priority since the natural enum ordering
   * (ordinal value) is used. An example of INTERACTIVE priority is a bulk item change action kicked
   * off by a user. NORMAL is for tasks that aren't interactive per se, but should happen before any
   * background tasks, like the background search indexer for example. BACKGROUND is for proper
   * background tasks that have no user interaction and the lowest possible priority, like scheduled
   * tasks, etc..
   */
  ...
  private StandardStatusChange ensureChange() {
    ...
    StandardStatusChange taskStatusChange =
        (StandardStatusChange) changes.get(StandardStatusChange.class);
    ...
    synchronized (changes) {
      if (!changes.isEmpty()) {
        ...
        taskService.updateTaskStatus(
            this, Lists.newArrayList(changes.values()), statusVersion, becomes);
