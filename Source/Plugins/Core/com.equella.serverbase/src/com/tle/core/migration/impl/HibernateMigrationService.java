...
public class HibernateMigrationService {
  ...
  @Inject private HibernateFactoryService hibernateFactoryService;
  ...
  public synchronized HibernateFactory getHibernateFactory() {
    if (configuration == null) {
      configuration =
          hibernateFactoryService.createConfiguration(
              new DataSourceHolder(
                  new DynamicDataSource(), datasourceService.getSystemDataSource().getDialect()),
              MigrationLog.class,
              MigrationLog.LogStatus.class,
  ...
  public MigrationLog ensureLogEntry(MigrationState ext, Session session) {
    ...
    if (log == null) {
      ...
      if (!log.isCanRetry()) {
        throw new Error(
            "Trying to execute a non-retry migration:" //$NON-NLS-1$
                + log.getMigrationId());
      }
  ...
  public String getSystemPassword() {
    ...
    try {
      HibernateMigrationHelper helper =
          new HibernateMigrationHelper(
              getHibernateFactory(), datasourceService.getSystemDataSource().getDefaultSchema());
      if (helper.tableExists(session, SystemConfig.TABLE_NAME)) {
        SystemConfig configRow =
            (SystemConfig) session.get(SystemConfig.class, SystemConfig.ADMIN_PASSWORD);
        if (configRow != null) {
          return configRow.getValue();
        }
      } else if (helper.tableExists(session, ConfigurationProperty.TABLE_NAME)) {
        ConfigurationProperty configRow =
