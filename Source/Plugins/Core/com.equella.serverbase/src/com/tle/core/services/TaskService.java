...
/**
 * The service responsible for managing tasks which are to be run once through out the cluster.
 *
 * @author jolz
 */
@NonNullByDefault
public interface TaskService {
  ...
  /**
   * Return the UUID of a global clustered task, which may have already been started. If the task
   * has already started (or is suspended), it will return the UUID of that task. If the task hasn't
   * been started, it starts it and waits to find out what UUID it has.
   *
   * @param globalTask The global clustered task.
   * @param millis The maximum time to wait for the task to start. NOTE: ClusteredTaskService does
   *     not use this.
