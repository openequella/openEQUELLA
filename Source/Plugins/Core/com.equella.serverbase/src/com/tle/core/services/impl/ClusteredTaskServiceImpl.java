...
public class ClusteredTaskServiceImpl extends AbstractTaskServiceImpl
    implements PathChildrenCacheListener, ClusterMessageHandler {
  ...
  public void init() {
    ...
    priorityTaskRunnerExecutor =
        new ExecutorCompletionService<TaskResult>(createPriorityTaskExecutor());
    ...
    taskCache =
        zookeeperService.createPathCache(
            ZK_TASKPATH, false, this, StartMode.POST_INITIALIZED_EVENT);
  ...
  private boolean processResponse(SimpleMessage message) {
    BlockingQueue<SimpleMessage> waitingQueue =
        messageResponses.getIfPresent(message.getMessageId());
  ...
  public void updateTaskStatus(
    ...
    clusterMessagingService.postMessage(
        new StatusChangesMessage(taskId, statusChanges, appliesTo, becomes));
  }
  ...
  public <T> T waitForTaskSubStatus(String taskId, String subStatus, long millis) {
    ...
    while (true) {
      synchronized (statusHandler) {
        ...
        Serializable subStatusObj =
            taskStatus != null ? (Serializable) taskStatus.getTaskSubStatus(subStatus) : null;
        if (subStatus != null) {
  ...
  private BlockingQueue<SimpleMessage> postMessageInternal(
      String taskId, Serializable message, boolean withResponse) {
  ...
  private void addRunner(String taskId) {
    ...
    try {
      ...
      clusteredTaskWithArgs =
          (ClusteredTaskWithArgs) PluginAwareObjectInputStream.fromBytes(taskBytes);
  ...
  public class TaskStatusHandler {
    private final Logger LOGGER_STATUSES =
        LoggerFactory.getLogger(ClusteredTaskServiceImpl.class.getName() + ".TaskStatusHandler");
    private final Map<String, TaskStatusImpl> taskStatuses =
        Collections.synchronizedMap(new HashMap<String, TaskStatusImpl>());
    private final Cache<String, TaskStatusImpl> finishedStatuses =
    ...
    public synchronized void setStatus(String taskId, TaskStatusImpl status) {
      LOGGER_STATUSES.debug(
          "Setting status for taskId: " + taskId + " finished=" + status.isFinished());
    ...
    public synchronized boolean updateStatus(
        final String taskId,
        Collection<TaskStatusChange<?>> changes,
        String appliesTo,
        String becomes) {
    ...
    private void runListeners(final String taskId, final TaskStatusImpl taskStatus) {
      ...
      if (!listeners.isEmpty()) {
        final TaskStatusListener[] listenerArray =
            listeners.toArray(new TaskStatusListener[listeners.size()]);
  ...
  public class TaskRunnerHandler {
    private final Logger LOGGER_RUNNER =
        LoggerFactory.getLogger(ClusteredTaskServiceImpl.class.getName() + ".TaskRunnerHandler");
    ...
    public class TaskRunner implements Callable<TaskResult>, CuratorWatcher {
      ...
      public TaskResult call() throws Exception {
            ...
            if (nextLowest != null) {
              ...
              if (stat != null) {
                if (stat.getEphemeralOwner()
                  ...
                  LOGGER_RUNNER.warn(
                      "Appears that owner of next lock is us, deleting: " + nextLowest);
                  curator.delete().forPath(nextLowestFullPath);
                } else {
                  if (LOGGER_RUNNER.isTraceEnabled()) {
                    LOGGER_RUNNER.trace(
                        "Waiting on previous lock for: " + taskId + " : " + nextLowest);
                  }
                  synchronized (this) {
                    wait(10000);
                  }
        ...
        try {
          task =
              clusteredTask.createTask(getPluginService(), getArgsForTask(clusteredTaskWithArgs));
          ...
          clusterMessagingService.postMessage(
              new FullStatusResponseMessage(Collections.singletonMap(taskId, initialStatus)));
  ...
  public static class TaskResult {
    ...
    public TaskResult(
        TaskRunner runner, boolean taskCompleted, Throwable error, Task completedTask) {
      this.runner = runner;
  ...
  public abstract static class TaskMessage implements Serializable {
    enum MsgType {
      ASK,
      UPDATES,
      FULL_STATUS,
  ...
  public static class StatusChangesMessage extends TaskMessage {
    ...
    public StatusChangesMessage(
        String taskId, Collection<TaskStatusChange<?>> changes, String appliesTo, String becomes) {
  ...
  protected void handleMessage(TaskMessage msg) {
    ...
    switch (msg.getType()) {
      ...
      case UPDATES:
        ...
        if (!statusHandler.updateStatus(
            taskId, smsg.getChanges(), smsg.getAppliesTo(), smsg.getBecomes())) {
          requestFullStatus(Collections.singleton(taskId));
        }
        break;
      case MSG:
      ...
      case FULL_STATUS:
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
              "Being asked by "
                  + fMsg.getRequestingId()
                  + " for full statuses of tasks: "
                  + fMsg.getTaskIds());
        }
        if (!fMsg.getRequestingId().equals(ourNodeId)) {
          ...
          if (!fullStatuses.isEmpty()) {
            clusterMessagingService.postMessage(
                fMsg.getRequestingId(), new FullStatusResponseMessage(fullStatuses));
          }
        }
        break;
      case FULL_STATUS_RESPONSE:
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
              "Received full status responses for taskIds: " + rMsg.getStatuses().keySet());
        }
      ...
      case ARGS:
        ...
        if (argsId != null) {
          ...
          taskArgs.invalidate(argsId);
          clusterMessagingService.postMessage(
              argsMsg.getRequestingId(), new ArgsMessage(argsId, args));
        } else {
  ...
  public static class ClusteredTaskWithArgs implements Serializable {
    ...
    public ClusteredTaskWithArgs(
        ClusteredTask task, Serializable[] args, String argsId, String nodeId) {
      this.task = task;
      this.args = args;
      this.argsId = argsId;
      this.nodeId = nodeId;
          ...
          try {
            ...
            if (taskResult.isTaskCompleted()) {
              ...
              if (task != null) {
                try {
                  Collection<TaskStatusChange<?>> changes =
                      Lists.newArrayList(task.getCurrentChanges());
                  changes.add(new FinishedStatusChange(error));
                  updateTaskStatus(
                      task, changes, task.getStatusVersion(), UUID.randomUUID().toString());
                } catch (Exception e) {
