 ...
 * automatically converted to NULL values by the database, should they be empty. Note that this
 * class will not allow you to use NULL value strings when they are not allowed by Hibernate (such
 * as in Maps). Version for Hibernate 3 that does not add quotes to non-empty strings but escapes by
 * a keyword. This seems more economic in cases where empty strings are rare.
 *
