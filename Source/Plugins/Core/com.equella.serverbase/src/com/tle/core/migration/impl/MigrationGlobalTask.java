...
public class MigrationGlobalTask extends AlwaysRunningTask<SimpleMessage> {
  ...
  private static final String KEY_PFX =
      PluginServiceImpl.getMyPluginId(MigrationGlobalTask.class) + '.';

  @Inject private DataSourceService dataSourceService;
  @Inject private DatabaseSchemaDao dao;
  ...
  protected String getTitleKey() {
    return KEY_PFX + "migrationglobaltask.title";
  }
  ...
  private void processChecked(SchemaMessage migMessage) {
    ...
    if (callable != null) {
      ...
      try {
        ...
        Collection<SchemaInfoImpl> duplicates =
            getDuplicates(schemaInfo, checkResult.getUniqueId());
        if (duplicates.isEmpty()) {
          ...
          if (!schemaInfo.isTriedUpdatingId()) {
            ...
            schemaInfo.setUniqueId(
                hibernateMigrationService.updateUniqueId(
                    schemaDataSourceService.getDataSourceForId(dbSchema.getId())));
    ...
    for (MigrationInfo minfo : migrations) {
      if (minfo.isFailed()) {
        DataSourceHolder dataSource =
            schemaId == DatabaseSchema.SYSTEM_SCHEMA.getId()
                ? dataSourceService.getSystemDataSource()
                : schemaDataSourceService.getDataSourceForId(schemaId);
  ...
  private void checkSystemSchema() {
    ...
    if (!schemaInfo.isUp()) {
      try {
        ...
        MigrationsToRun systemMigrations =
            hibernateMigrationService.checkSchemaForMigrations(systemDataSource, true);
  ...
  private void migrateSchema(final long schemaId, final SchemaInfoImpl schemaInfo) {
    ...
    BeanClusteredTask task =
        new BeanClusteredTask(
            TASK_ID + "-" + schemaId, true, MigrateTaskFactory.class, "create", schemaId, system);
    String taskId = taskService.getGlobalTask(task, TimeUnit.MINUTES.toMillis(1)).getTaskId();
    taskService.addTaskStatusListener(
        taskId,
  ...
  public class CheckCallable implements Callable<CheckResult> {
    ...
    public CheckResult call() throws Exception {
      ...
      MigrationDataSource migDataSource =
          new MigrationDataSource(connectionDetails[0], connectionDetails[1], connectionDetails[2]);
      try {
        schemaDataSourceService.removeFromCache(schemaId);
        DataSourceHolder dataSource = new DataSourceHolder(migDataSource, dialect);
        MigrationsToRun toRun =
