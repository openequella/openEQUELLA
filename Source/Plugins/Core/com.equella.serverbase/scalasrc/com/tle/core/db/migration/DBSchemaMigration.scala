...
trait DBSchemaMigration {
  ...
  def addTables(tables: Seq[TableDefinition],
                progress: MigrationResult): JDBCIO[Unit] = {
    progress.setCanRetry(true)
    implicit val c = withSQLLog(progress)
    Stream
      .emits(tables.map(schemaSQL.createTable).map(rawSQL))
      .covary[JDBCIO]
      .flush
      .compile
      .drain
