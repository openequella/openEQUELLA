...
public class EditorTab extends BaseEntityTab<Schema> {
  ...
  private void setupGUI() {
    editor =
        new AbstractSchemaEditor(schemaModel) {
          @Override
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!names.add((childAt.isAttribute() ? "@" : "") + name)) {
        ...
        throw new EditorException(
            CurrentLocale.get(
                "com.tle.admin.schema.manager.editortab.warning",
                new Object[] {
                  type,
                  name, //$NON-NLS-1$
                  node.toString()
    ...
    public void setup() {
      JLabel titleLabel =
          new JLabel(
              "<html><b>" //$NON-NLS-1$
                  + CurrentLocale.get(
                      "com.tle.admin.schema.manager.editortab.target")); //$NON-NLS-1$
      JLabel nameLabel =
          new JLabel(
              CurrentLocale.get("com.tle.admin.schema.manager.editortab.name")); // $NON-NLS-1$
      JLabel typeLabel =
          new JLabel(
              CurrentLocale.get("com.tle.admin.schema.manager.editortab.type")); // $NON-NLS-1$

      targetName = new JSmartTextField();
      targetName.setStartWithAlphaCharacter(true);
      targetName.setInputType(
          JSmartTextField.I_ALPHANUMERIC
              | JSmartTextField.I_PERIOD
              | JSmartTextField.I_UNDERSCORE
              | JSmartTextField.I_HYPHEN);

      targetType = new JComboBox(new String[] {"text", "html"}); // $NON-NLS-1$  //$NON-NLS-2$
      targetSearch =
          new JCheckBox(
              CurrentLocale.get("com.tle.admin.schema.manager.editortab.free")); // $NON-NLS-1$
      targetField =
          new JCheckBox(
              CurrentLocale.get("com.tle.admin.schema.manager.editortab.index")); // $NON-NLS-1$
      ...
      final int[] rows = {
        height1, height2, height2, height2, height2, TableLayout.FILL,
      };
      final int[] columns = {
