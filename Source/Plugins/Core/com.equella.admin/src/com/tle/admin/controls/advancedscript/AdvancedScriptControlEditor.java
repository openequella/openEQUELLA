...
public class AdvancedScriptControlEditor extends AbstractControlEditor<AdvancedScriptControl>
  ...
  static {
    try {
      DEFAULT_TEMPLATE =
          Resources.toString(
              AdvancedScriptControlEditor.class.getResource("defaultDisplayTemplate.txt"),
              Charsets.UTF_8);
      HIGHLIGHTER_KEYWORDS =
          Resources.toString(
                  AdvancedScriptControlEditor.class.getResource("highlighterKeywords.txt"),
                  Charsets.UTF_8)
              .split("\\s");
  ...
  public AdvancedScriptControlEditor(
  ...
  private EquellaSyntaxTextArea createJavascriptHighlightingPane() {
    EquellaSyntaxTextArea pane =
        new EquellaSyntaxTextArea(CODE_HEIGHT_LINES, CODE_WIDTH_COLUMNS, "attributes", "request");
  ...
  private EquellaSyntaxTextArea createHtmlHighlightingPane() {
    EquellaSyntaxTextArea pane =
        new EquellaSyntaxTextArea(CODE_HEIGHT_LINES, CODE_WIDTH_COLUMNS, "attributes");
    pane.addCaretListener(this);
    pane.addFocusListener(this);
    return pane;
  }
  ...
  public boolean hasDetectedChanges() {
    ...
    return syntaxHighlightingPaneDetectedChanges(onloadJs)
        || syntaxHighlightingPaneDetectedChanges(onsubmitJs)
        || syntaxHighlightingPaneDetectedChanges(readJs)
        || syntaxHighlightingPaneDetectedChanges(storeJs)
        || syntaxHighlightingPaneDetectedChanges(freemarker);
  }

  private void loadSyntaxHighlightedPane(
      final EquellaSyntaxTextArea pane, final String value, final String defaultValue) {
  ...
  public static class AdvancedScriptConfigFileFilter extends FileFilterAdapter {
    public AdvancedScriptConfigFileFilter() {
      super(
          new FileExtensionFilter(EXPORT_FILE_EXTENSION),
          strings.text("file.description", EXPORT_FILE_EXTENSION));
    }
  }
    ...
    if (focus != null) {
      getEntityEditor()
          ...
          .setMessage(
              strings.text(
                  "lineandcolumn",
                  focus.getCaretLineNumber() + 1,
                  focus.getCaretOffsetFromLineStart())); // $NON-NLS-1$
    }
  }
}
