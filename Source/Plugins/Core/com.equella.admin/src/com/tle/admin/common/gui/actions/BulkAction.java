...
public abstract class BulkAction<T> extends TLEAction {
  ...
  public BulkAction() {
    ...
    putValue(
        ...
        CurrentLocale.get("com.tle.admin.gui.common.actions.bulkaction.description"));

    successMessage = CurrentLocale.get("com.tle.admin.gui.common.actions.bulkaction.success");
    confirmOverrideMessage =
        CurrentLocale.get("com.tle.admin.gui.common.actions.bulkaction.override");
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    FileFilterAdapter csvFilter =
        new FileFilterAdapter(
            new FileExtensionFilter("csv"),
            CurrentLocale.get("com.tle.admin.gui.common.actions.bulkaction.csvfile"));

    final DialogResult result =
        DialogUtils.openDialog(parentFrame, importDialogTitle, csvFilter, null);
    if (result.isOkayed()) {
      final File selectedFile = result.getFile();

      GlassSwingWorker<Void> worker =
          new GlassSwingWorker<Void>() {
            @Override
