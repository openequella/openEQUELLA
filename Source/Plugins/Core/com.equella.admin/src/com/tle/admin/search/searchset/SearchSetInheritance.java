...
public class SearchSetInheritance extends JPanel implements Changeable {
  ...
  public SearchSetInheritance(final EntityCache cache, final ClientService clientService) {
    ...
    final int[] rows = {
      height1, height2, TableLayout.FILL, height2, TableLayout.FILL,
    };
    final int[] cols = {
      TableLayout.FILL,
    };
  ...
  public void load(
      SearchSet searchSet, List<Schema> allParentSchemas, List<ItemDefinition> allParentItemDefs) {
  ...
  private class InheritedTableModel<T extends BaseEntity, U extends EntityScript<T>>
    ...
    public InheritedTableModel(
        Map<Long, NameId> cache, Class<U> protoClass, String entityDisplayName) {
      this.cache = cache;
