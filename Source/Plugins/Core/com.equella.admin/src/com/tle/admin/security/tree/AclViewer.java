...
public class AclViewer extends JPanel implements ActionListener, SecurityTreeTab {
  ...
  private void setupGui(ClientService clientService) {
    ...
    for (String privilege :
        new TreeSet<String>(PrivilegeTree.getPrivilegesForNode(privilegeNode))) {
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == privSelector) {
      ...
      GlassSwingWorker<List<ACLEntryMapping>> worker =
          new GlassSwingWorker<List<ACLEntryMapping>>() {
            @Override
  ...
  private static class MyTableModel extends AbstractTableModel {
    ...
    private static final String FIRST =
        CurrentLocale.get("security.editor.advanced.columnname.target"); // $NON-NLS-1$
