...
public class HtmlEditMceModel extends CustomControlModel<HtmlEditMceControl> {
  ...
  public String doValidation(ClientService clientService) {
    ...
    if (control.isRestrictCollections()
        && control.isRestrictDynacolls()
        && control.isRestrictSearches()
        && control.isRestrictContributables()) {
      // ... there must therefore be something, anything to restrict to.
      if (Check.isEmpty(control.getCollectionsUuids())
          && Check.isEmpty(control.getDynaCollectionsUuids())
          && Check.isEmpty(control.getSearchUuids())
          && Check.isEmpty(control.getContributableUuids())) {
        return getString("validate");
      }
    }
