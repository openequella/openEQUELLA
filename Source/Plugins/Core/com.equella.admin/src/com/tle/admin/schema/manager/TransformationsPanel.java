...
public class TransformationsPanel extends JPanel implements ActionListener, ListSelectionListener {
  ...
  public TransformationsPanel(
      RemoteAdminService adminService, EditorState<Schema> state, boolean handleImports) {
    ...
    final int[] rows = {
      height1, height1, height1, TableLayout.FILL,
    };
    final int[] cols = {
  ...
  private void onAdd() {
    ...
    GlassSwingWorker<SchemaTransform> worker =
        new GlassSwingWorker<SchemaTransform>() {
          @Override
  ...
  private void onDelete() {
    final int result =
        JOptionPane.showConfirmDialog(
            this,
            CurrentLocale.get(
                "com.tle.admin.schema.manager.transformationstab.delete")); //$NON-NLS-1$
