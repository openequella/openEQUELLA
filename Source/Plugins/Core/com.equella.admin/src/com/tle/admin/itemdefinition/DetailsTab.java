...
public class DetailsTab extends AbstractItemdefTab
    implements ActionListener, AbstractDetailsTab<ItemDefinition> {
  ...
  public void save() {
    ...
    itemDef.setReviewperiod(
        Integer.valueOf(((NameValue) reviewPeriod.getSelectedItem()).getValue()).intValue());
  ...
  public void load() {
    ...
    AppletGuiUtils.selectInJCombo(
        reviewPeriod, new NameValue(null, Integer.toString(itemDef.getReviewperiod())), 0);
  ...
  protected Collection<NameValue> getWorkflows() {
    try {
      List<BaseEntityLabel> results =
          clientService.getService(RemoteWorkflowService.class).listAll();
      ...
      while (true) {
        String result =
            JOptionPane.showInputDialog(
                getComponent(), s("enter"), s("newwiz"), JOptionPane.QUESTION_MESSAGE);

        if (result == null) {
          return;
        } else if (Check.isEmpty(result)) {
          Driver.displayInformation(
              getComponent(), "You must enter a name for the new wizard category");
        } else if (result.trim().length() > 100) {
          Driver.displayInformation(
              getComponent(), "The name of a wizard category must not exceed 100 characters");
        } else {
