...
public class DetailsTab extends BaseEntityTab<Schema> implements AbstractDetailsTab<Schema> {
  ...
  private void setupGUI() {
    JLabel nameLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.schema.manager.detailstab.name"));
    JLabel descriptionLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.schema.manager.detailstab.desc"));
    JLabel ownerLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.schema.manager.detailstab.owner"));
    JLabel nameXpathLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.schema.manager.detailstab.path"));
    JLabel descriptionXpathLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.schema.manager.detailstab.descpath"));
    ...
    final int[] rows = {
      height1, height1 * 3, ownerHeight, height1, height1, TableLayout.FILL,
    };
    final int[] cols = {
  ...
  public void validation() throws EditorException {
    if (name.isCompletelyEmpty()) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.supplyname"));
    }

    if (nameXpathChooser.getTarget().length() == 0) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.chooseforname"));
    }

    if (descriptionXpathChooser.getTarget().length() == 0) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.choosefordesc"));
    }

    if (model.getNode(nameXpathChooser.getTarget()) == null) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.choosenewname"));
    }

    if (model.getNode(descriptionXpathChooser.getTarget()) == null) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.choosenewdesc"));
    }

    if (owner.getUser() == null) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.schema.manager.detailstab.noowner"));
    }
  }
}
