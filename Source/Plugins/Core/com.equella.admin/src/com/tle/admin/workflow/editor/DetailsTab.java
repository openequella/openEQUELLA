...
public class DetailsTab extends JPanel {
  ...
  private void setupGui() {
    ...
    definedDateCheck.addItemListener(
        new ItemListener() {
          @Override
          public void itemStateChanged(ItemEvent e) {
  ...
  public void save(WorkflowItem item) {
    ...
    item.setMovelive(
        moveToLive.isSelected()
            ? (moveToLiveArrival.isSelected() ? MoveLive.ARRIVAL : MoveLive.ACCEPTED)
            : MoveLive.NO);
    item.setAutoAction(
        autoCheck.isSelected()
            ? (autoAccept.isSelected() ? AutoAction.ACCEPT : AutoAction.REJECT)
            : AutoAction.NONE);
