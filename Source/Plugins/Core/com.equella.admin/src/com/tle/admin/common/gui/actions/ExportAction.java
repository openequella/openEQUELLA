...
public abstract class ExportAction extends TLEAction {
  public ExportAction() {
    ...
    putValue(
        Action.NAME,
        CurrentLocale.get("com.tle.admin.gui.common.actions.exportaction.name")); // $NON-NLS-1$
    putValue(
        Action.SHORT_DESCRIPTION,
        CurrentLocale.get("com.tle.admin.gui.common.actions.exportaction.desc")); // $NON-NLS-1$
  }
  ...
  public File askForDestinationFile(Component parent, String objectName, String extension) {
    ...
    final DialogResult result =
        DialogUtils.saveDialog(
            ...
            null,
            DialogUtils.getSuggestedFileName(objectName, extension));
    if (result.isOkayed()) {
      file = result.getFile();
      if (file.exists()) {
        final int result2 =
            JOptionPane.showConfirmDialog(
                parent,
                CurrentLocale.get("com.tle.admin.gui.common.actions.exportaction.confirm"),
  ...
  public <T> void writeFile(
      Component parent, final File destination, final ExportSerialiser getter) {
    GlassSwingWorker<?> worker =
        new GlassSwingWorker<Object>() {
          ...
          public Object construct() throws Exception {
            try (Writer out =
                new BufferedWriter(
                    new OutputStreamWriter(new FileOutputStream(destination), Constants.UTF8))) {
          ...
          public void finished() {
            Driver.displayInformation(
                getComponent(),
                CurrentLocale.get(
                    "com.tle.admin.gui.common.actions.exportaction.successful")); //$NON-NLS-1$
          }
          ...
          public void exception() {
            Driver.displayInformation(
                getComponent(),
                CurrentLocale.get(
                    "com.tle.admin.gui.common.actions.exportaction.error")); //$NON-NLS-1$
