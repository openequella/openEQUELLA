...
public class SearchTab extends BaseEntityTab<FederatedSearch>
    implements AbstractDetailsTab<FederatedSearch> {
  ...
  protected Map<NameValue, List<String>> transformsPerCollection =
      new HashMap<NameValue, List<String>>();
  ...
  public final void init(Component parent) {
    ...
    panel =
        new JPanel(new MigLayout("insets 5px 5px 0 0, wrap 2", "[fill][:640:800, fill]", "top"));
  ...
  public void validation() throws EditorException {
    if (nameField.isCompletelyEmpty()) {
      throw new EditorException(getString("searchtab.entername"));
    }
    if (owner.getUser() == null) {
      throw new EditorException(getString("searchtab.noowner"));
    }
    plugin.validation();
  }
