...
public abstract class BaseEntityEditor<T extends BaseEntity>
  ...
  protected List<? extends BaseEntityTab<T>> getTabLoadOrder(
      List<? extends BaseEntityTab<T>> btabs2) {
    return btabs;
  }
  ...
  public void refreshWindowTitle(String newTitle) {
    if (dialog != null) {
      ...
      final String key =
          "com.tle.admin.baseentity.baseentityeditor.title-for-"
              + (Check.isEmpty(newTitle) ? "unnamed" : "named")
              + "-entity"; //$NON-NLS-2$ //$NON-NLS-3$
      dialog.setTitle(
          CurrentLocale.get(key, getWindowTitle(), driver.getInstitutionName(), newTitle));
    }
  }
  ...
  public final void save() throws EditorException {
    ...
    try {
      ...
      for (ValidationError error : ex.getErrors()) {
        LOGGER.error(
            "Validation error in " + error.getField() + ": " + error.getMessage()); // $NON-NLS-2$
      }
      ValidationError error = ex.getErrors().get(0);
      throw new EditorException(
          "Validation error in " + error.getField() + ": " + error.getMessage());
    } catch (Exception ex) {
