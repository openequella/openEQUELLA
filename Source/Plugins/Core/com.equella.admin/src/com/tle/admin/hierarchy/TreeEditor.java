...
/**
 * The <code>TreeEditor</code> provides a self contained component for displaying and editing a
 * hierarchy of <code>Topic</code> nodes. This allows for moving Topics up and down the tree, plus
 * creating new topics (children or siblings), and removing them.
 *
...
public class TreeEditor extends AbstractTreeEditorTree<HierarchyTreeNode> {
  ...
  protected boolean preAddNewNode(
      HierarchyTreeNode parent, HierarchyTreeNode newNode, Map<Object, Object> params) {
    if (!parent.isRoot()
  ...
  private final TLEAction exportAction =
      new ExportAction() {
        @Override
  ...
  private final TLEAction importAction =
      new ImportAction() {
        @Override
        public void actionPerformed(ActionEvent e) {
        ...
        public void update() {
          setEnabled(
              tree.getSelectionCount() == 1 && canEdit(getSelectedNode()) || canAddRootNodes());
        }
      };

  private void importTopic(
      final String content,
      final HierarchyTreeNode parent,
      final boolean newids,
      final boolean importSecurity) {
    final ProgressDialog pd = ProgressDialog.showProgress(this, "Importing...");

    GlassSwingWorker<ValidationError> worker =
        new GlassSwingWorker<ValidationError>() {
          @Override
            ...
            if (error == null) {
              ...
              int i =
                  JOptionPane.showConfirmDialog(
                      ...
                      CurrentLocale.get(
                          "com.tle.admin.gui.common.tree.editor.importsametopictitle"),
                      JOptionPane.YES_NO_OPTION,
