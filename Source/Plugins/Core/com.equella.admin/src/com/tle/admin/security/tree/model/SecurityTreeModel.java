...
public class SecurityTreeModel implements TreeModel {
  ...
  public SecurityTreeModel(final ClientService services, final PluginService pluginService) {
    ...
    root.addChild(
        new EntityGroupNode(
            CurrentLocale.get("com.tle.admin.security.tree.model.securitytreemodel.powersearches"),
            ALL_POWER_SEARCHES,
            POWER_SEARCH,
            services.getService(RemotePowerSearchService.class)));
    ...
    root.addChild(
        new EntityGroupNode(
            CurrentLocale.get("com.tle.admin.security.tree.model.securitytreemodel.fedsearches"),
            ALL_FEDERATED_SEARCHES,
            FEDERATED_SEARCH,
            services.getService(RemoteFederatedSearchService.class)));

    root.addChild(
        new EntityGroupNode(
            CurrentLocale.get("com.tle.admin.security.tree.model.securitytreemodel.courses"),
            ALL_COURSE_INFO,
            COURSE_INFO,
            services.getService(RemoteCourseInfoService.class)));

    // The following is the way of the future! Everything above should be
    // deleted in favour of the following approach.
    final RemotePrivilegeTreeService remotePrivilegeTreeService =
        services.getService(RemotePrivilegeTreeService.class);
    final List<SecurityTreeNode> nodes =
        DynamicPrivilegeTreeNode.getSecurityTargetsAsTreeNodes(
            pluginService, remotePrivilegeTreeService, null);
