...
public class ManagementDialog extends JFrame implements ActionListener, WindowListener {
  ...
  protected void loadConfig() {
    ...
    final Map<String, Set<String>> allowedToolsWithPrivs =
        clientService.getService(RemoteAdminService.class).getAllowedTools();
    final PluginTracker<?> tracker =
  ...
  private void setup() {
    ...
    exitButton =
        new JHoverButton(CurrentLocale.get("com.tle.admin.gui.managementdialog.exit"), icon);
  ...
  private void registerTool(Extension extension, Set<String> grantedPrivileges) {
    ...
    JButton button =
        new JHoverButton(
            CurrentLocale.get(extension.getParameter("name").valueAsString()), bullets);
    setupButton(button);
    buttons.add(button);
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (e.getSource() == b) {
          ...
          GlassSwingWorker<?> worker =
              new GlassSwingWorker<AdminTool>() {
                @Override
                public AdminTool construct() throws Exception {
                  ...
                  if (tool == null) {
                    ...
                    tool.setup(
                        privs, CurrentLocale.get(extension.getParameter("name").valueAsString()));
    ...
    if (show) {
      if (pluginFailedToLoad) {
        JOptionPane.showMessageDialog(
            this,
            CurrentLocale.get("com.tle.admin.gui.managementdialog.failedload"),
            CurrentLocale.get("com.tle.admin.gui.managementdialog.failed"),
