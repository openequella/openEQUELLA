...
public class KeyResourcesTab extends AbstractTopicEditorTab implements ActionListener {
  ...
  public void setup(ChangeDetector changeDetector) {
    ...
    final int width1 = 20;
    final int[] rows = {
      height1, TableLayout.FILL, height1, TableLayout.FILL,
    };
    final int[] cols = {
  ...
  private JComponent setupSelectedPanel(ChangeDetector changeDetector) {
    ...
    selectedTable.addMouseListener(
        new TablePopupListener(selectedTable, removeAction, upAction, downAction));
    selectedTable
        .getSelectionModel()
  ...
  private JComponent setupSearchPanel() {
    ...
    final int[] rows = {
      height1, TableLayout.FILL, height1, height1,
    };
    final int[] cols = {
      width1, TableLayout.FILL, width2, TableLayout.FILL, width1,
    };
  ...
  private JComponent setupResultsNavigator() {
    ...
    final int[] cols = {
      TableLayout.FILL, width1, width1, width1, width1,
    };
    ...
    if (e.getSource() == addAsKeyResource) {
      ...
      if (versions.contains(entry.getVersion())) {
        ...
        if (JOptionPane.showConfirmDialog(
                this,
                message,
                CurrentLocale.get("com.dytech.edge.admin.gui.confirm"),
      ...
      startResult =
          totalResults
              - (totalResults % RESULTS_PER_PAGE == 0
                  ? RESULTS_PER_PAGE
  ...
  private void doSearch() {
    ...
    GlassSwingWorker<SearchResults<Item>> worker =
        new GlassSwingWorker<SearchResults<Item>>() {
          @Override
          public SearchResults<Item> construct() throws Exception {
            ...
            SearchResults<Item> results =
                adminService.searchReducedItems(search, startResult, RESULTS_PER_PAGE);
