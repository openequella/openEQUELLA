...
public class ExtensionsTab extends AbstractItemdefTab {
  private static class ExtensionHandle {
    ...
    public ExtensionHandle(PluginServiceImpl service, Extension ext) throws ClassNotFoundException {
      ...
      this.configPanelClass =
          param == null
              ? null
              : (Class<? extends AbstractExtensionConfigPanel>)
                  service.getBeanClass(ext.getDeclaringPluginDescriptor(), param.valueAsString());
  ...
  public void init(final Component parent) {
    for (Extension ext :
        pluginService.getConnectedExtensions(
            "com.tle.admin.collection.tool", //$NON-NLS-1$
            "extra")) //$NON-NLS-1$
    {
      try {
      ...
      for (final ExtensionHandle extension : extensions) {
        ...
        if (extension.getConfigPanelClass() == null) {
          ...
          final JButton button =
              new JButton(
                  CurrentLocale.get(
                      "com.tle.admin.itemdefinition.extensionstab.config")); //$NON-NLS-1$
          button.addActionListener(
              new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                  ...
                  final int[] rows = {
                    TableLayout.FILL, size.height,
                  };
                  final int[] cols = {
                    TableLayout.FILL, size.width, size.width,
                  };
