...
public class StepsTab extends BaseEntityTab<Workflow> implements AbstractDetailsTab<Workflow> {
  ...
  public void validation() throws EditorException {
    if (nameField.isCompletelyEmpty()) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.workflow.stepstab.supplyname")); // $NON-NLS-1$
    }

    if (owner.getUser() == null) {
      throw new EditorException(
          CurrentLocale.get("com.tle.admin.workflow.stepstab.noowner")); // $NON-NLS-1$
    }
  ...
  private void recurseValidate(final WorkflowTreeNode pNode) throws EditorException {
    ...
    while (i.hasNext()) {
      ...
      if (LangUtils.isEmpty(node.getName())) {
        ...
        throw new EditorException(
            CurrentLocale.get("com.tle.admin.workflow.stepstab.supplystepname")); // $NON-NLS-1$
      }
      ...
      if (node.getType() == WorkflowNode.ITEM_TYPE) {
        ...
        if (!item.isModeratorsSpecified()) {
          ...
          throw new EditorException(
              CurrentLocale.get("com.tle.admin.workflow.stepstab.selected")); // $NON-NLS-1$
        }
      }
      ...
      if (node.getType() == WorkflowNode.SCRIPT_TYPE) {
        ...
        if (scriptNode.isNotifyOnCompletion() && !scriptNode.isNotifyOnCompletionSpecified()) {
          ...
          throw new EditorException(
              CurrentLocale.get(
                  "com.tle.admin.workflow.script.scripteditor.nofificationstab.completion.notify.nobody"));
        }
        ...
        if (!scriptNode.isNotifyNoErrorSpecified()) {
          ...
          throw new EditorException(
              CurrentLocale.get(
                  "com.tle.admin.workflow.script.scripteditor.nofificationstab.error.notify.nobody"));
        }
      }
  ...
  private JComponent createNorth() {
    final JLabel nameLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.workflow.stepstab.name")); // $NON-NLS-1$
    final JLabel ownerLabel =
        new JLabel(CurrentLocale.get("com.tle.admin.workflow.stepstab.owner")); // $NON-NLS-1$
    ...
    final int[] cols = {
      width1, TableLayout.FILL, TableLayout.FILL,
    };
  ...
  private JComponent createCentre() {
    ...
    final int[] cols = {
      width1, TableLayout.FILL, width2, width2, width2, width2, TableLayout.FILL,
    };
  ...
  private final TLEAction modifyAction =
      new EditAction() {
        @Override
        public void actionPerformed(final ActionEvent e) {
          tree.modify();
        ...
        private boolean showWarning() {
          ...
          final int result =
              JOptionPane.showOptionDialog(
                  panel,
                  CurrentLocale.get(prefix + "message"), // $NON-NLS-1$
                  CurrentLocale.get(prefix + "title"),
                  JOptionPane.YES_NO_CANCEL_OPTION, // $NON-NLS-1$
                  JOptionPane.QUESTION_MESSAGE,
                  null,
                  new Object[] {yes, yesDnwa, cancel},
                  cancel);
  ...
  private final TLEAction previewAction =
      new PreviewAction() {
        @Override
        public void actionPerformed(final ActionEvent e) {
          try {
          ...
          final WorkflowVisualiser visualiser =
              new WorkflowVisualiser(model.getRootNode(), new Dimension(500, 500));
          ...
          dialog.setTitle(
              CurrentLocale.get("com.tle.admin.workflow.stepstab.visualiser")); // $NON-NLS-1$
          ...
          try {
            ...
            JOptionPane.showMessageDialog(
                panel,
                CurrentLocale.get("com.tle.admin.workflow.stepstab.notcomplete")); // $NON-NLS-1$
          }
        }
