...
public class TermSelectorEditor extends AbstractPowerSearchControlEditor<TermSelectorControl> {
  ...
  protected void saveControl() {
    ...
    control.setAllowAddTerms(
        getWizardType() == WizardHelper.WIZARD_TYPE_CONTRIBUTION
            && addterms.isEnabled()
            && addterms.isSelected());
  ...
  private JPanel setupDetailsSection() {
    ...
    termStorageGroup =
        new BetterGroup<JRadioButton, TermStorageFormat>(
            true,
            storageFull,
            TermStorageFormat.FULL_PATH,
            storageLeaf,
            TermStorageFormat.LEAF_ONLY);
    ...
    restrictionGroup =
        new BetterGroup<JRadioButton, SelectionRestriction>(
            true,
            restrictionTopLevel,
            SelectionRestriction.TOP_LEVEL_ONLY,
            restrictionLeaf,
            SelectionRestriction.LEAF_ONLY,
            restrictionNone,
            SelectionRestriction.UNRESTRICTED);

    displayTypes =
        new RadioButtonChoiceList<TermSelectorControl, Extension>() {
          @Override
          ...
          public DynamicChoicePanel<TermSelectorControl> getChoicePanel(Extension choice) {
            ...
            final String configPanelClass =
                configPanelParam == null ? null : configPanelParam.valueAsString();

            if (Check.isEmpty(configPanelClass)) {
              return new EmptyDynamicChoicePanel<TermSelectorControl>();
            } else {
              DynamicChoicePanel<TermSelectorControl> dcp =
                  (DynamicChoicePanel<TermSelectorControl>)
                      getPluginService()
  ...
  private void taxonomySelectionEvent() {
    ...
    if (Check.isEmpty(uuid)) {
      ...
      GlassSwingWorker<?> worker =
          new GlassSwingWorker<List<Extension>>() {
            ...
            public List<Extension> construct() throws Exception {
              ...
              for (Extension ext :
                  getPluginService()
                      .getConnectedExtensions("com.tle.admin.taxonomy.tool", "displayType")) {
                if ((ext.getParameter("supportsBrowsing").valueAsBoolean() && supportsTermBrowsing)
                    || (ext.getParameter("supportsSearching").valueAsBoolean()
