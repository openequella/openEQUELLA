...
public class UniversalControlEditor extends AbstractControlEditor<CustomControl> {
  ...
  private void setupGUI() {
    ...
    GlassSwingWorker<?> worker =
        new GlassSwingWorker<Collection<Extension>>() {
          @Override
          public Collection<Extension> construct() throws Exception {
            return getPluginService()
                .getConnectedExtensions("com.tle.admin.controls.universal", "editor");
          }
    ...
    types =
        new CheckboxChoiceList<UniversalSettings, Extension>(getString("selecttypes"), 2) {
          ...
          public DynamicChoicePanel<UniversalSettings> getChoicePanel(Extension choice) {
            ...
            UniversalControlSettingPanel dcp =
                (UniversalControlSettingPanel)
                    getPluginService()
                        .getBean(choice.getDeclaringPluginDescriptor(), configPanelClass);
            dcp.init(
                (UniversalControl) getWizardControl(),
                getWizardType(),
                getSchema(),
                getClientService());
            return dcp;
