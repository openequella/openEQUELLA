...
public class TransformDialog implements ActionListener {
  ...
  private void setupGui() {
    JLabel typeLabel =
        new JLabel(
            CurrentLocale.get("com.tle.admin.schema.manager.transformdialog.name")); // $NON-NLS-1$
    JLabel fileLabel =
        new JLabel(
            CurrentLocale.get("com.tle.admin.schema.manager.transformdialog.xsl")); // $NON-NLS-1$
    ...
    fileSelector =
        new FileSelector(
            CurrentLocale.get(
                "com.tle.admin.schema.manager.transformdialog.browse")); //$NON-NLS-1$
    ...
    final int[] rows = {
      height1, height2, height1, height3, height4,
    };
    final int[] cols = {
  ...
  public boolean showEditDialog(Component parent, SchemaTransform transform) {
    ...
    dialog.setTitle(
        CurrentLocale.get("com.tle.admin.schema.manager.transformdialog.schema")); // $NON-NLS-1$
    ...
    if (okSelected) {
      if (Check.isEmpty(getSchemaType())) {
        ...
        Driver.displayInformation(
            dialog,
            CurrentLocale.get(
                "com.tle.admin.schema.manager.transformdialog.select")); //$NON-NLS-1$
      } else {
        dialog.dispose();
      }
    } else {
