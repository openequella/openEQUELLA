  ...
  public class WorkflowItemPanel extends WorkflowNodePanel implements ActionListener {
    ...
    private void updateScriptMessage() {
      if (script == null || script.trim().length() == 0) {
        scriptingLabel.setText(
            CurrentLocale.get(
                "com.tle.admin.workflow.editor.decisioneditor.noscripting")); //$NON-NLS-1$
      } else {
        scriptingLabel.setText(
            CurrentLocale.get(
                "com.tle.admin.workflow.editor.decisioneditor.notexecuted")); //$NON-NLS-1$
      }
    }
    ...
    private JComponent createSouth() {
      ...
      scriptingButton =
          new JButton(CurrentLocale.get("com.tle.admin.workflow.editor.decisioneditor.open"));
