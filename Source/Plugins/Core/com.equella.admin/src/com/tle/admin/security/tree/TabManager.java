...
public class TabManager extends JPanel {
  ...
  public void updateTabs(SecurityTreeNode node) {
    if (node == null) {
      tabs.removeAll();
      tabs.addTab(
          CurrentLocale.get("com.tle.admin.security.tree.tabmanager.instructions"),
          new MessagePanel(CurrentLocale.get("com.tle.admin.security.tree.tabmanager.select")));
  ...
  private void updateTabsForNoSelection() {
    ...
    tabs.addTab(
        CurrentLocale.get("com.tle.admin.security.tree.tabmanager.instructions"),
        new MessagePanel(CurrentLocale.get("com.tle.admin.security.tree.tabmanager.search")));
  ...
  private void updateTabs(Node privilegeNode, Object target) {
    ...
    if (privilegeNode != null) {
      tabs.addTab(
          CurrentLocale.get("com.tle.admin.security.tree.tabmanager.view"),
          new AclEditor(services, privilegeNode, target, allowEditing));

      if (!privilegeNode.isVirtual()) {
        tabs.addTab(
            CurrentLocale.get("com.tle.admin.security.tree.tabmanager.all"),
            new AclViewer(services, privilegeNode, target));
      }
    } else {
      tabs.addTab(
          "No Options",
