...
public class SchemaList extends JPanel implements ListSelectionListener, ActionListener {
  ...
  private void createGUI() {
    JLabel title =
        new JLabel(
            CurrentLocale.get(
                "com.tle.admin.itemdefinition.mapping.schemalist.title")); //$NON-NLS-1$
  ...
  /**
   * Updates the state of the buttons to ensure that only the correct operations for the given state
   * can occur.
   */
    ...
    if (index >= 0) {
      ...
      int result =
          JOptionPane.showConfirmDialog(
              this,
              CurrentLocale.get(
                  "com.tle.admin.itemdefinition.mapping.schemalist.confirm",
  ...
  private class SchemaPicker implements TargetListener, ActionListener {
    ...
    public SchemaNode showSchemaPicker(Component parent, String target) {
      ...
      dialog.setTitle(
          CurrentLocale.get(
              "com.tle.admin.itemdefinition.mapping.schemalist.picker")); //$NON-NLS-1$
    ...
    private void setupGui() {
      JLabel instruction =
          new JLabel(
              CurrentLocale.get(
                  "com.tle.admin.itemdefinition.mapping.schemalist.choose")); //$NON-NLS-1$

      alreadyChosen =
          new JLabel(
              CurrentLocale.get(
                  "com.tle.admin.itemdefinition.mapping.schemalist.selected")); //$NON-NLS-1$
      ...
      final int[] rows = {
        height1, height2, height1, height3,
      };
      final int[] cols = {
