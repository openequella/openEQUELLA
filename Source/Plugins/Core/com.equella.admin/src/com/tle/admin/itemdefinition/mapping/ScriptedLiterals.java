...
public class ScriptedLiterals extends JComponent implements ActionListener, ListSelectionListener {
  ...
  private void createGUI(SchemaModel schema) {
    ...
    final int[] rows = {
      height1, TableLayout.FILL, height2, height2, TableLayout.FILL, height3, height3,
    };
    final int[] cols = {
      width1, width2, width2, TableLayout.FILL,
    };
  ...
  /** This ensures that all buttons are properly enalbed or disabled for the current state. */
  ...
  private void setTitle(ScriptedTarget target) {
    if (target == null) {
      title.setText(
          CurrentLocale.get(
              "com.tle.admin.itemdefinition.mapping.scriptedliterals.noschema")); //$NON-NLS-1$
    } else {
      title.setText(
          CurrentLocale.get(
              "com.tle.admin.itemdefinition.mapping.scriptedliterals.literals",
              target //$NON-NLS-1$
                  .getTarget()));
    }
  }
  ...
  private void onAdd() {
    String literal =
        JOptionPane.showInputDialog(
            this,
            CurrentLocale.get(
                "com.tle.admin.itemdefinition.mapping.scriptedliterals.enter")); //$NON-NLS-1$
    if (literal != null) {
    ...
    if (index >= 0) {
      ...
      int result =
          JOptionPane.showConfirmDialog(
              this,
              CurrentLocale.get(
                  "com.tle.admin.itemdefinition.mapping.scriptedliterals.confirm",
                  literal), //$NON-NLS-1$
              "Remove?",
  ...
  /**
   * Provides our customised table model for viewing Collections of ScriptedRules.
   *
  ...
  private static class MyTableModel extends AbstractTableModel {
    ...
    private static final String[] COLUMNS = {
      CurrentLocale.get(
          "com.tle.admin.itemdefinition.mapping.scriptedliterals.literal"), //$NON-NLS-1$
      CurrentLocale.get("com.tle.admin.itemdefinition.mapping.scriptedliterals.script")
    }; //$NON-NLS-1$
    ...
    /**
     * Sets a new list of literals for the table. Modifications to this model will be reflected in
     * the given list.
     *
  ...
  private static class MyCellRenderer extends JButton implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (script != null && script.length() > 0) {
        setText(
            CurrentLocale.get(
                "com.tle.admin.itemdefinition.mapping.scriptedliterals.edit")); //$NON-NLS-1$
      } else {
        setText(
            CurrentLocale.get(
                "com.tle.admin.itemdefinition.mapping.scriptedliterals.add")); //$NON-NLS-1$
      }
  ...
  private static class MyCellEditor extends AbstractCellEditor
    ...
    public MyCellEditor(Component parent, SchemaModel schema) {
      ...
      button.setText(
          CurrentLocale.get(
              "com.tle.admin.itemdefinition.mapping.scriptedliterals.open")); //$NON-NLS-1$
    }
