...
public class SecurityTree implements TreeSelectionListener {
  ...
  public SecurityTree(
      ClientService clientService, PluginService pluginService, boolean allowEditing) {
  ...
  private void setupGui(ClientService clientService, PluginService pluginService) {
    ...
    final int[] rows = {
      TableLayout.FILL, closeButton.getPreferredSize().height,
    };
    final int[] cols = {
      TableLayout.FILL, closeButton.getPreferredSize().width,
    };
  ...
  int trySave(GlassSwingWorker<?> worker) {
    ...
    if (allowEditing && currentSelection != null && tabManager.hasDetectedChanges()) {
      ...
      Object[] buttons = {
        CurrentLocale.get("prompts.save"), // $NON-NLS-1$
        CurrentLocale.get("prompts.discard"),
  ...
  public void showDialog(Component parent) {
    ...
    dialog.setTitle(
        CurrentLocale.get(
            "com.tle.admin.security.tree.securitytree.title",
            Driver.instance().getInstitutionName())); // $NON-NLS-1$
