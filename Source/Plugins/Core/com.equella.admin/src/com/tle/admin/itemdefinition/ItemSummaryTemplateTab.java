...
public class ItemSummaryTemplateTab extends AbstractItemdefTab {
  ...
  public void init(Component parent) {
    tracker =
        new PluginTracker<SummaryDisplayConfig>(
            pluginService, "com.tle.admin.collection.tool", "summaryDisplay", "id");

    displayNodeList =
        new ListWithView<SummarySectionsConfig, SummaryDisplayView>(true) {
          @Override
          protected SummarySectionsConfig createElement() {
            NewDisplaySummarySectionDialog ndssd =
                new NewDisplaySummarySectionDialog(pluginService);
            return ndssd.showDialog(this);
          }

          @Override
          protected ListWithViewInterface<SummarySectionsConfig> getEditor(
              SummarySectionsConfig currentSelection) {
            return currentSelection == null
                ? null
                : new SummaryDisplayView(tracker, currentSelection.getValue());
          }
        };

    displayNodeList.setListCellRenderer(
        new DefaultListCellRenderer() {
          private final String UNTITLED =
              CurrentLocale.get("com.tle.admin.itemdefinition.abstracttemplatetab.untitlednode");
          ...
          public Component getListCellRendererComponent(
            ...
            String label =
                CurrentLocale.get(((SummarySectionsConfig) value).getBundleTitle(), null);
    ...
    hideOwner = new JCheckBox(getString("summarysections.hideowner"));
    hideCollaborators = new JCheckBox(getString("summarysections.hidecollaborators"));
  ...
  public void validation() throws EditorException {
    ...
    if (sections == null || sections.size() == 0) {
      throw new EditorException(getString("itemsummarytemplatetab.validation.nodisplaynodes"));
    } else {
      for (SummarySectionsConfig section : sections) {
        LanguageBundle bundleTitle = section.getBundleTitle();
        if (bundleTitle == null) {
          throw new EditorException(getString("itemsummarytemplatetab.validation.emptytitle"));
        }
        ...
        if (section.getValue().equals("displayNodes")) {
          ...
          for (DisplayNode displayNode : displayNodes) {
            ...
            boolean notTheCaseTheyAllEmpty =
                !(titleBundle == null && Check.isEmpty(node) && Check.isEmpty(type));
            ...
            if (notTheCaseTheyAllEmpty && incomplete) {
              throw new EditorException(
                  CurrentLocale.get(
                      "com.tle.admin.collection.tool.itemsummarytemplatetab.validation.incomplete"));
            }
          }
        }
      }
  ...
  public class SummaryDisplayView extends JPanel
      implements ListWithViewInterface<SummarySectionsConfig> {
