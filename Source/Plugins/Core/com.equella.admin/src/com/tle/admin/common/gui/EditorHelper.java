...
public final class EditorHelper {
  ...
  public static final String DEFAULT_SAVE_TEXT =
      CurrentLocale.get("com.dytech.edge.admin.gui.save"); // $NON-NLS-1$

  /** The default text for an editors "close" button. */
  public static final String DEFAULT_CLOSE_TEXT =
      CurrentLocale.get("com.dytech.edge.admin.gui.close"); // $NON-NLS-1$
  ...
  /**
   * This should be called by an editor when the close button is selected. Returns true if closed
   */
  public static boolean onClose(EditorInterface editor, Integer confirmOption) {
    if (editor.isReadOnly()) {
      ...
      if (confirmOption == null) {
        String message =
            CurrentLocale.get(
                "com.dytech.edge.admin.gui.editorhelper.confirm", //$NON-NLS-1$
        ...
        confirm =
            JOptionPane.showOptionDialog(
                editor.getParentWindow(),
                message,
                CurrentLocale.get(
                    "com.dytech.edge.admin.gui.editorhelper.savebefore"), //$NON-NLS-1$
        ...
        new GlassSwingWorker<Object>() {
          ...
          public Object construct() throws EditorException {
            statusbar.setMessage(
                CurrentLocale.get(
                    "com.dytech.edge.admin.gui.editorhelper.validating")); //$NON-NLS-1$
            editor.validation();

            statusbar.setMessage(
                CurrentLocale.get("com.dytech.edge.admin.gui.editorhelper.saving")); // $NON-NLS-1$
            editor.save();
          ...
          public void finished() {
            if (closeAfterSave) {
              ...
              Driver.displayInformation(
                  editor.getParentWindow(),
                  CurrentLocale.get(
                      "com.dytech.edge.admin.gui.editorhelper.success",
                      editor //$NON-NLS-1$
                          .getDocumentName()));
            }
          }
