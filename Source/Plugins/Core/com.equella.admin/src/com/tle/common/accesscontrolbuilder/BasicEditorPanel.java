...
public class BasicEditorPanel extends JComponent implements ItemListener, ActionListener {
  ...
  private void setupGUI(Node privNode) {
    ...
    select =
        new JButton(CurrentLocale.get("com.tle.admin.security.editors.basiceditorpanel.select"));
    ...
    final int[] cols = {
      20, TableLayout.FILL, width1, TableLayout.FILL,
    };
  ...
  private JPanel createOptions(Node privNode) {
    ...
    if (!PrivilegeTree.isOverrideDefault(privNode, privList.getPrivilege())) {
      justTheOwner =
          createButton(options, buttonGroup, "security.editor.basic.justTheOwner"); // $NON-NLS-1$
    }
    everyone = createButton(options, buttonGroup, "security.editor.basic.everyone"); // $NON-NLS-1$
    everyoneButGuests =
        createButton(
            options, buttonGroup, "security.editor.basic.everyoneButGuests"); // $NON-NLS-1$
    limitedSet =
        createButton(options, buttonGroup, "security.editor.basic.limitedSet"); // $NON-NLS-1$
  ...
  /**
   * The last entry is special - it's the "revoke everyone" entry. All entries before it must be
   * grants.
   *
