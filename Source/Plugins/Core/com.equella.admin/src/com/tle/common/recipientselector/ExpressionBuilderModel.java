...
public class ExpressionBuilderModel extends DefaultTreeModel {
  ...
  private ExpressionTreeNode cloneExpressionAndAddTemporaryNots(ExpressionTreeNode original) {
    ...
    for (Enumeration<?> e = original.children(); e.hasMoreElements(); ) {
      nodeToAddChildrenTo.add(
          cloneExpressionAndAddTemporaryNots((ExpressionTreeNode) e.nextElement()));
    }
    ...
    for (int i = node.getChildCount() - 1; i >= 0; i--) {
      ...
      if (childGroup != null) {
        if (childChildCount == 0) {
          node.remove(i);
        } else if (childChildCount == 1
            && (childGroup == Grouping.MATCH_ALL || childGroup == Grouping.MATCH_ANY)) {
          node.remove(i);
  ...
  protected static class Parser extends PostfixExpressionParser<ExpressionTreeNode> {
    ...
    protected void doBinaryOperator(
        Stack<Pair<ExpressionTreeNode, Integer>> operands, Grouping grouping) {
