...
public class AdvancedEditorPanel extends JComponent
  ...
  private void setupGUI(Node privNode, PrivilegeList accessModel) {
    ...
    final int[] rows = {
      height1, height1, height1, height1, TableLayout.FILL,
    };
    final int[] cols = {
      TableLayout.FILL, width1,
    };
    ...
    showDefaults =
        new JCheckBox(
            CurrentLocale.get("com.tle.admin.security.editors.advancededitorpanel.default"));
    ...
    final int[] mainRows = {
      TableLayout.INVISIBLE, TableLayout.FILL, TableLayout.INVISIBLE, mainHeight1,
    };
    final int[] mainCols = {
      TableLayout.FILL, TableLayout.FILL, width1,
    };
    ...
    add(
        new JLabel(
            CurrentLocale.get("com.tle.admin.security.editors.advancededitorpanel.overrides")),
        new Rectangle(2, 0, 1, 1));
    add(all, new Rectangle(0, 1, 3, 1));
    add(
        new JLabel(
            CurrentLocale.get("com.tle.admin.security.editors.advancededitorpanel.defaults")),
  ...
  private void setupFilters(Node privNode) {
    final int priority =
        PrivilegeTree.isOverrideDefault(privNode, privilege)
            ? privNode.getOverridePriority()
            : SecurityConstants.PRIORITY_OBJECT_INSTANCE;

    overridesFilter =
        new OverrideDefaultAclViewer.Filter() {
          @Override
  ...
  private static class MyTableModel extends AbstractTableModel {
    ...
    private static final String SECOND =
        CurrentLocale.get("security.editor.advanced.columnname.who"); // $NON-NLS-1$
    private static final String THIRD =
        CurrentLocale.get("security.editor.advanced.columnname.override"); // $NON-NLS-1$
  ...
  private abstract class ViewerChanger extends GlassSwingWorker<OverrideDefaultAclViewer> {
    ...
    public ViewerChanger(
        OverrideDefaultAclViewer viewer, int row, OverrideDefaultAclViewer.Filter filter) {
