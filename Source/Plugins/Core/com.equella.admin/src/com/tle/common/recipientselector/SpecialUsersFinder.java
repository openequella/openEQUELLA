...
public class SpecialUsersFinder extends JPanel implements UserGroupRoleFinder {
    ...
    if (everyone.isSelected()) {
      ...
      value =
          SecurityConstants.getRecipient(
              Recipient.TOKEN_SECRET_ID, (String) sharedSecretIds.getSelectedItem());
    } else {
  ...
  private void setupGUI(boolean hasOwner) {
    everyone =
        new JRadioButton(
            CurrentLocale.get("com.tle.admin.recipients.specialusersfinder.everyone"), true);
    owner =
        new JRadioButton(CurrentLocale.get("com.tle.admin.recipients.specialusersfinder.owner"));
    loggedIn =
        new JRadioButton(CurrentLocale.get("com.tle.admin.recipients.specialusersfinder.loggedin"));
    guest =
        new JRadioButton(CurrentLocale.get("com.tle.admin.recipients.specialusersfinder.guest"));
    sharedSecretId =
        new JRadioButton(CurrentLocale.get("com.tle.admin.recipients.specialusersfinder.tokenId"));

    sharedSecretIds = new JComboBox();
    sharedSecretIds.setEnabled(false);
    sharedSecretId.addActionListener(
        new ActionListener() {
          @Override
    ...
    final int[] rows = {
      height1, height1, height1, height1, height1, height1, TableLayout.FILL,
    };
    final int[] cols = {
      15, TableLayout.FILL,
    };
