...
/**
 * COMPARISON ::= XPATH_COMPARISON | CONTAINS_COMPARISON | STATUS_COMPARISON | TYPE_COMPARISON
 * XPATH_COMPARISON ::= "xml.get('" XPATH "')" EQUALITY "'" VALUE "'" CONTAINS_COMPARISON ::=
 * "xml.contains('" XPATH "', '" VALUE "')" STATUS_COMPARISON ::= "status" EQUALITY "'" VALUE "'"
 * TYPE_COMPARISON ::= "user." METHOD "('" VALUE "')"
 */
public class BasicParser extends IfParser {
  ...
  /**
   * COMPARISON ::= XPATH_COMPARISON | CONTAINS_COMPARISON | MODERATION_COMPARISON |
   * STATUS_COMPARISON | TYPE_COMPARISON
   */
  @Override
  ...
  /**
   * XPATH_COMPARISON ::= "xml.get('" XPATH "')" EQUALITY "'" VALUE "'" There is now the special
   * case, that if an "equals" is read in, it actually creates a "comparison" instead. This should
   * allow scripts to change to using contains without the need to fix everything in the database.
   */
  ...
  protected ContainsComparison getContainsComparison()
      throws InvalidScriptException, ParseException {
    match("contains(");
    match("'");
    String xpath = getXpath();
    match("'");
  ...
  protected ModerationComparison getModerationComparison()
      throws InvalidScriptException, ParseException {
  ...
  protected WorkflowStepComparison getWorkflowStepComparison()
      throws InvalidScriptException, ParseException {
    match("workflowstep");
    Equality op = getEquality();
    String value = getValue();
  ...
  protected UserMethodMethod getUserMethod() throws ParseException {
    ...
    if (look == 'h') {
      ...
      throw new ParseException(
          "Unknown user method starting with '" + look + "'", getCurrentOffset());
    }
    match(m.toScript());
    return m;
  }
  ...
  protected String getXpath() throws InvalidScriptException, ParseException {
    ...
    while (Character.isLetter(look)
        || Character.isDigit(look)
        || look == '/'
        || look == '.'
        || look == ':'
        || look == '_'
