...
public class TripleShuffleList extends JPanel implements ActionListener {
  ...
  protected void setupGUI(String t1, String t2) {
    ...
    add =
        new JButton(
            CurrentLocale.get("com.dytech.edge.admin.wizard.tripleshufflelist.add")); // $NON-NLS-1$
    remove =
        new JButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.tripleshufflelist.remove")); //$NON-NLS-1$
    ...
    int width2 =
        Math.max(firstText.getPreferredSize().width, secondText.getPreferredSize().width) - width1;
    if (width2 < 0) {
      width2 = 0;
    }
    ...
    if (BundleCache.getLanguages().size() == 1) {
      sort =
          new JButton(
              CurrentLocale.get(
                  "com.dytech.edge.admin.wizard.tripleshufflelist.sort")); //$NON-NLS-1$
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == add) {
      ...
      Collections.sort(
          items,
          new Comparator<WizardControlItem>() {
            @Override
  ...
  private static String[] getColumnNames() {
    return new String[] {
      CurrentLocale.get("com.dytech.edge.admin.wizard.tripleshufflelist.name"),
