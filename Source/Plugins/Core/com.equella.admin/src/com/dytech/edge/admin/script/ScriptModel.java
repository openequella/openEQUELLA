...
/**
 * A <code>ScriptModel</code> provides an abstract interface for manipulating script models. A
 * script model is a representation of a type of script. For example, the <code>ifmodel</code>
 * package provides and absract representation of an "if" statement from which other script models
 * can derive and provide extended functionality.
 *
...
public interface ScriptModel extends ListModel {
  ...
  /**
   * Provides a graphical editor for creating and changing rows in the script editor.
   *
   * @return A <code>JPanel</code> inserting in to the script editor which provides editing
   *     capabilities.
   */
