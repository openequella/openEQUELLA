...
public class DRMConfigTab extends JPanel {
  ...
  private static final String plugPrefix =
      "com.dytech.edge.admin.wizard.editor.drm."; //$NON-NLS-1$
  ...
  public static final String CUSTOM_PERMISSIONS_KEY =
      "drmconfigtab.custompermissions"; //$NON-NLS-1$

  private static final String[] USAGE_OPTIONS_KEYS = {
    BASIC_USAGE_KEY, USE_AND_ADAPT_KEY, CUSTOM_PERMISSIONS_KEY
  };
  ...
  private void createGUI() {
    ...
    usageOptions =
        new JShuffleBox<String>(
            usageOptionsStrings,
            CurrentLocale.get(plugPrefix + "drmconfigtab.available"), // $NON-NLS-1$
            CurrentLocale.get(plugPrefix + "drmconfigtab.selected")); // $NON-NLS-1$

    acceptBeforeSummary =
        new JCheckBox(CurrentLocale.get(plugPrefix + "drmconfigtab.requireusers")); // $NON-NLS-1$
    ownerMustAlsoAccept =
        new JCheckBox(CurrentLocale.get(plugPrefix + "drmconfigtab.requireowner")); // $NON-NLS-1$
    showLicenceInComposition =
        new JCheckBox(
            CurrentLocale.get(plugPrefix + "drmconfigtab.requirestudents")); // $NON-NLS-1$
    previewBeforeAccepting =
        new JCheckBox(CurrentLocale.get(plugPrefix + "drmconfigtab.allow")); // $NON-NLS-1$

    final int height1 = title.getPreferredSize().height;
    final int height2 = acceptBeforeSummary.getPreferredSize().height;
    final int[] rows = {
      height1, TableLayout.PREFERRED, height2, height2, height2, height2, TableLayout.FILL,
    };
    final int[] cols = {
      TableLayout.FILL,
    };
