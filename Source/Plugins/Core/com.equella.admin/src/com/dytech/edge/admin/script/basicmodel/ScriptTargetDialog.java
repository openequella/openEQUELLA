...
public class ScriptTargetDialog extends GeneralDialog {
  ...
  private Component createGUI() {
    ...
    status =
        new JRadioButton(
            CurrentLocale.get("com.dytech.edge.admin.script.schemadialog.status"),
            options.hasItemStatus()); // $NON-NLS-1$
    ...
    moderation =
        new JRadioButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.script.schemadialog.moderator")); //$NON-NLS-1$
    moderation.addActionListener(disabler);
    group.add(moderation);

    usertype =
        new JRadioButton(
            CurrentLocale.get("com.dytech.edge.admin.script.schemadialog.role")); // $NON-NLS-1$
    usertype.addActionListener(disabler);
    group.add(usertype);

    workflow =
        new JRadioButton(
            CurrentLocale.get("com.dytech.edge.admin.script.schemadialog.workflow")); // $NON-NLS-1$
    workflow.addActionListener(disabler);
    group.add(workflow);

    schema =
        new JRadioButton(
            CurrentLocale.get("com.dytech.edge.admin.script.schemadialog.schema")); // $NON-NLS-1$
