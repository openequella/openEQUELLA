...
public class RawHtmlEditor extends AbstractControlEditor<Html> {
  ...
  private static final NameValue[] ELEMENTS =
      new NameValue[] {
        new NameValue(
            CurrentLocale.get("com.dytech.edge.admin.wizard.editor.rawhtmleditor.horiz"), "<hr>")
      }; //$NON-NLS-1$ //$NON-NLS-2$
    ...
    if (preRadio.isSelected()) {
      html =
          LangUtils.createTextTempLangugageBundle(
              ((NameValue) preList.getSelectedItem()).getValue());
    } else {
      html = userText.save();
    }
  ...
  protected void createTop() {
    preRadio =
        new JRadioButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.editor.rawhtmleditor.predefined")); //$NON-NLS-1$
  ...
  protected void createBottom() {
    userHelp =
        new JLabel(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.editor.rawhtmleditor.select",
                "\"{/xpath/to/my/data}\"")); //$NON-NLS-1$ //$NON-NLS-2$
    userRadio =
        new JRadioButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.editor.rawhtmleditor.userdefined")); //$NON-NLS-1$
