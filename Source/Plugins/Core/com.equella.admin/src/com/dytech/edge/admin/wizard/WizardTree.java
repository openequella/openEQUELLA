 ...
 */
public class WizardTree extends JPanel
    implements TreeSelectionListener, TreeModelListener, Changeable {
  ...
  private void setup(int wizardType) {
    ...
    final int[] cols = {
      width1, TableLayout.FILL, width2, width2, TableLayout.FILL,
    };
   ...
   * @dytech.jira see Jira Defect TLE-600 : http://apps.dytech.com.au/jira/browse/TLE-600
   */
  ...
  private FileWorker importer() {
    return new AbstractFileWorker<Control>(null, "wizard/controlImport") // $NON-NLS-1$
    {
      ...
      public Control construct() throws Exception {
        ...
        return addExportedControl(
            (ExportedControl) xstream.fromXML(controlXml), getLastSelectedPathComponent(), true);
      }
  ...
  protected Set<String> getPluginIds(PropBagEx exportedControlSubtree) {
    ...
    for (PropBagEx subControl :
        exportedControlSubtree.iterateAll(
            "children/com.tle.admin.controls.ExportedControl")) //$NON-NLS-1$
    {
  ...
  protected Control addExportedControl(ExportedControl ctl, Control parent, boolean selectIt) {
    ...
    Control newControl =
        doAddControl(repository.getDefinition(ctl.getControlTypeId()), parent, selectIt);
    if (newControl != null) {
  ...
  public Control doAddControl(
      ControlDefinition newControlDef, Control parentControl, boolean selectIt) {
  ...
  protected ExportedControl getExportedControl(
      Control control, final PluginService pluginService, final String version) {
  ...
  private static class CellEditor extends DefaultTreeCellEditor implements CellEditorListener {
    ...
    public Component getTreeCellEditorComponent(
      ...
      editingIcon =
          repository.getIcon(currentControl.getControlClass(), currentControl.isScripted());
  ...
  protected class ControlTreeRemoveAction extends RemoveAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (def.hasContext(Contexts.CONTEXT_PAGES)) {
        message.append(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.wizardtree.selectedpage")); //$NON-NLS-1$
        if (hasChildren) {
          message.append(
              CurrentLocale.get(
                  "com.dytech.edge.admin.wizard.wizardtree.allcontrolspage")); //$NON-NLS-1$
        }
      } else if (WizardHelper.isGroup(control)) {
        message.append(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.wizardtree.selectedgroup")); //$NON-NLS-1$
        if (hasChildren) {
          message.append(
              CurrentLocale.get(
                  "com.dytech.edge.admin.wizard.wizardtree.allcontrolgroups")); //$NON-NLS-1$
        }
      } else if (def.hasContext(Contexts.CONTEXT_GROUP)) {
        message.append(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.wizardtree.selecteditem")); //$NON-NLS-1$
        if (hasChildren) {
          message.append(
              CurrentLocale.get(
                  "com.dytech.edge.admin.wizard.wizardtree.allcontrolitem")); //$NON-NLS-1$
        }
      } else {
        message.append(
            CurrentLocale.get(
                "com.dytech.edge.admin.wizard.wizardtree.selectedcontrol")); //$NON-NLS-1$
      }
