...
public class EditBoxEditor extends AbstractPowerSearchControlEditor<EditBox> {
  ...
  private JComponent createDetails() {
    ...
    JLabel defaultValueLabel =
        new JLabel(CurrentLocale.get("wizard.controls.initialvalue")); // $NON-NLS-1$
    ...
    checkDuplicates =
        new JCheckBox(CurrentLocale.get("wizard.controls.checkDuplicates")); // $NON-NLS-1$
    enforceUniqueness =
        new JCheckBox(CurrentLocale.get("wizard.controls.enforceUniqueness")); // $NON-NLS-1$

    checkDuplicates.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            if (checkDuplicates.isSelected() && showWarning) {
              Object[] options =
                  new String[] {
                    CurrentLocale.get("wizard.controls.uniqueness.ok"), // $NON-NLS-1$
                    CurrentLocale.get("wizard.controls.uniqueness.dontshowagain")
                  }; //$NON-NLS-1$

              if (JOptionPane.showOptionDialog(
                      checkDuplicates,
                      CurrentLocale.get("wizard.controls.uniqueness.message"),
                      CurrentLocale //$NON-NLS-1$
                          .get("wizard.controls.uniqueness.title"),
    ...
    final int[] rows = {
      height1, height1, height1, height1, height1, height1, height1, TableLayout.PREFERRED,
    };
    final int[] cols = {
