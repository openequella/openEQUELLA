...
public abstract class AbstractBasicModel extends IfModel implements ActionListener {
  ...
  public AbstractBasicModel(SchemaModel schema, ScriptOptions options, String script)
  ...
  protected Comparison constructComparison() {
    ...
    switch (xpathField.getType()) {
      ...
      case SCHEMA_ITEM:
        ...
        if (op == NOT_EQUALS) {
          return new XpathComparison(
              new NotEquals(), xpathField.getSchemaXpath(), value.getValue());
        } else if (op == CONTAINS) {
  ...
  protected void addStatuses() {
    ...
    for (String status : statuses) {
      valueSelection.addItem(
          new NameValue(
              CurrentLocale.get("com.dytech.edge.admin.script.status." + status), status));
    }
  }
