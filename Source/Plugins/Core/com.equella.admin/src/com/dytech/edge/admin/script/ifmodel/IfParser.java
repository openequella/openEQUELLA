...
/**
 * "If" Statement BNF: STATEMENT ::= PRE_TEXT BLOCK ELSE* POST_TEXT BLOCK ::= "if (" CLAUSE ")" BODY
 * ELSE ::= "else" BLOCK CLAUSE ::= TERM (OPERATOR TERM)* TERM ::= BRACKETS | COMPARISON BRACKETS
 * ::= "(" CLAUSE ")" COMPARISON ::= ??? XPATH ::= (ALPHA | NUMBER | "/" | "." | "_" | "@")+
 * EQUALITY ::= "!=" | "==" | "<" | "<=" | ">" | ">=" VALUE ::= "'" STRING "'" OPERATOR ::= "&&" |
 * "||" PRE_TEXT ::= "var bRet = false;" POST_TEXT ::= "return bRet;" BODY ::= "{ bRet = true; }"
 * JAVA_IDENTIFIER ::= ALPHA [STRING]* STRING ::= (ALPHA | [0-9] | ...)* ALPHA ::= [a-zA-Z] NUMBER
 * ::= [0-9]
 */
public abstract class IfParser extends AbstractTopDownParser {
  ...
  /**
   * Imports a statement. STATEMENT ::= PRE_TEXT BLOCK ELSE* POST_TEXT ELSE ::= "else" BLOCK
   * PRE_TEXT ::= "var bRet = false;" POST_TEXT ::= "return bRet;"
   */
