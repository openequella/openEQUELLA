...
public class ScriptPanel extends JTabbedPane
  ...
  private JPanel createAdvanced() {
    JLabel titleLabel =
        new JLabel(
            CurrentLocale.get(
                "com.dytech.edge.admin.script.scriptpanel.javascript")); //$NON-NLS-1$
  ...
  private JPanel createBasic() {
    JLabel statementLabel =
        new JLabel(
            CurrentLocale.get("com.dytech.edge.admin.script.scriptpanel.statement")); // $NON-NLS-1$
    JLabel scriptLabel =
        new JLabel(
            CurrentLocale.get("com.dytech.edge.admin.script.scriptpanel.script")); // $NON-NLS-1$

    deleteButton =
        new JButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.script.scriptpanel.deleteline")); //$NON-NLS-1$
    importButton =
        new JButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.script.scriptpanel.importscript")); //$NON-NLS-1$
    exportButton =
        new JButton(
            CurrentLocale.get(
                "com.dytech.edge.admin.script.scriptpanel.exportscript")); //$NON-NLS-1$
    ...
    final int width1 =
        Math.max(exportButton.getPreferredSize().width, importButton.getPreferredSize().width);
      ...
      if (index == BASIC_TAB) {
        try {
          ...
          Driver.displayInformation(
              this,
              CurrentLocale.get(
                  "com.dytech.edge.admin.script.scriptpanel.customised")); //$NON-NLS-1$
        ...
        if (f.exists()) {
          int confirm =
              JOptionPane.showConfirmDialog(
                  this,
                  CurrentLocale.get(
                      "com.dytech.edge.admin.script.scriptpanel.overwrite"), //$NON-NLS-1$
  ...
  public void caretUpdate(CaretEvent event) {
    statusHolder
        ...
        .setMessage(
            CurrentLocale.get(
                "com.tle.admin.scripting.editor.lineandcolumn",
                advanced.getCaretLineNumber() + 1,
                advanced.getCaretOffsetFromLineStart())); // $NON-NLS-1$
  }
}
