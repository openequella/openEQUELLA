...
public class ScriptTargetChooser extends TargetChooser {
  ...
  public void setType(SelectionType newType, Object... extras) {
    ...
    switch (type) {
      case ITEM_STATUS:
        target.setText(
            CurrentLocale.get("com.dytech.edge.admin.script.target.status")); // $NON-NLS-1$
        break;

      case MODERATION:
        target.setText(
            CurrentLocale.get("com.dytech.edge.admin.script.target.moderator")); // $NON-NLS-1$
        break;

      case USER_TYPE:
        target.setText(
            CurrentLocale.get("com.dytech.edge.admin.script.target.role")); // $NON-NLS-1$
        break;

      case WORKFLOW_STEP:
        target.setText(
            CurrentLocale.get("com.dytech.edge.admin.script.target.workflow")); // $NON-NLS-1$
        break;
      ...
      case SCHEMA_ITEM:
        ...
        if (extras[0] instanceof SchemaNode) {
          ...
          throw new IllegalArgumentException(
              "Requires extra parameter of type SchemaNode or String");
        }
  ...
  protected class SearchHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      if (dialog == null) {
        dialog =
            new ScriptTargetDialog(
                ScriptTargetChooser.this, getSchemaModel(), getScriptOptionModel());
      }
