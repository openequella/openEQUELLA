...
public class JdbcQueryDelegate implements QueryDelegate {
  ...
  @Inject
  public JdbcQueryDelegate(
      InstitutionService institutionService, SchemaDataSourceService dataSourceService) {
  ...
  public Map<String, ?> getDatasourceMetadata() throws OdaException {
    ...
    try {
      new JdbcTemplate(dataSourceHolder.getDataSource())
          .execute(
              new ConnectionCallback() {
                ...
                public Object doInConnection(Connection connection)
                  ...
                  results.put(
                      Constants.JDBC_IDENTIFIER_QUOTE_STRING, metaData.getIdentifierQuoteString());
                  results.put(Constants.JDBC_TABLE_MAP, getTableMap(metaData));

                  return null;
                }

                private Map<String, Table> getTableMap(DatabaseMetaData metaData)
                    throws SQLException {
                  Map<String, Table> tableMap = new LinkedHashMap<String, Table>();

                  ResultSet tables =
                      metaData.getTables(null, defaultSchema, null, new String[] {"TABLE", "VIEW"});
                  try {
                    while (tables.next()) {
                      ...
                      try {
                        ...
                        tableMap.put(
                            table,
                            new Table(
                                table, tables.getString("TABLE_TYPE").equals("VIEW"), columns));
                      } catch (Exception e) {
                ...
                private List<Column> getColumns(DatabaseMetaData metaData, String table)
                    throws SQLException {
                    ...
                    while (cols.next()) {
                      results.add(
                          new Column(
                              ...
                              cols.getString("TYPE_NAME"))); // $NON-NLS-2$
                    }
                  } finally {
                    cols.close();
                  }
   ...
   */
  @Override
  public IResultSetExt executeQuery(String query, final List<Object> params, final int maxRows)
      throws OdaException {
    ...
    try {
      ...
      return new JDBCOdaResultSet(
          connection, statement, dataSource, set, new MetadataBean(metaData), realMaxRows);
    } catch (SQLException sqle) {
  ...
  public MetadataBean getParameterMetadata(String query, final List<Object> params) {
    try {
      ...
      return (MetadataBean)
          new JdbcTemplate(getDataSourceHolder().getDataSource())
              .execute(
                  query,
  ...
  protected void addParamsToStatement(PreparedStatement statement, List<Object> params)
      throws SQLException {
    if (params != null) {
  ...
  private String processQuery(String query) {
    query =
        query.replaceAll(
            "CURRENT_INSTITUTION", Long.toString(CurrentInstitution.get().getDatabaseId()));
