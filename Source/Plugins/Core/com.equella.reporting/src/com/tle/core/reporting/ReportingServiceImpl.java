...
@Singleton
public class ReportingServiceImpl extends AbstractEntityServiceImpl<EntityEditingBean, Report, ReportingService>
	implements
		ReportingService
{
	...
	@Inject
	private LearningEdgeOdaDelegate odaInterface;
	@Inject
	...
	@Override
	protected void doValidation(EntityEditingSession<EntityEditingBean, Report> session, Report entity,
	...
	public String executeReport(SectionInfo info, Report report, String designFile, String format,
		IHTMLActionHandler actionHandler, Map<String, String[]> parameters,
		Map<String, String[]> parameterDisplayTexts, boolean forceExecution)
	{
		...
		final String hash = report.getUuid()
			+ SectionUtils.getParameterString(SectionUtils.getParameterNameValues(parameters, false))
		...
		try
		{
			...
			if( !fileSystemService.isFileCached(entityFile, outfile, CACHE_TIME) )
			{
				...
				html.setImageDirectory(getFileSystemService().getExternalFile(entityFile, "image").getAbsolutePath());
				...
				try( OutputStream out = getFileSystemService().getOutputStream(entityFile, outfile, false) )
				{
	...
	private IReportRunnable getReportDesign(IReportEngine engine, Report report, String designFile) throws IOException,
		EngineException
	{
		...
		return engine.openReportDesign(null, inStream, new IResourceLocator()
		{
			@Override
			...
			public URL findResource(ModuleHandle module, String filename, int arg2)
			{
				try
				{
					...
					if( otherReport != null )
					{
						return fsys.getExternalFile(new EntityFile(otherReport), otherReport.getFilename()).toURI()
							.toURL();
					}
					return null;
				}
	...
	private IGetParameterDefinitionTask createReportParametersTask(IReportEngine engine, IReportRunnable design)
	{
		try
		{
	...
	public void processReportDesign(String stagingId, String filename) throws IOException
	{
		...
		else
		{
			...
			if( reports.isEmpty() )
			{
				...
				throw new ReportingException("Archive contains no report design files (.rptdesign)", Type.NODESIGNS);
			}
	...
	private void cleanReport(StagingFile stagingFile, String reportFilename)
	{
		try( InputStream reportStream = fileSystemService.read(stagingFile, reportFilename) )
		{
			...
			if( cleaned )
			{
				fileSystemService.write(stagingFile, reportFilename, new StringReader(xml.toString()), false);
			}
		}
		catch( IOException e )
		{
