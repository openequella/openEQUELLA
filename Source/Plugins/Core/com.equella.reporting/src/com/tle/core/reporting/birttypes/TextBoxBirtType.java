...
public class TextBoxBirtType extends AbstractBirtType {
  ...
  protected WizardControl createControl(IGetParameterDefinitionTask paramTask) {
    ...
    if (useCalendar) {
      ...
      if (defValue != null) {
        defaultValue =
            new LocalDate((Date) defValue, TimeZone.getDefault()).format(Dates.ISO_MIDNIGHT);
      }
    } else {
      control = new EditBox();
    ...
    if (defaultValue != null) {
      items.add(
          new WizardControlItem(
              LangUtils.createTextTempLangugageBundle(defaultValue), defaultValue));
    }
    return control;
  }
  ...
  private ReportParameterConverter getConverter(String format) {
    com.ibm.icu.util.TimeZone icuTimeZone =
        com.ibm.icu.util.TimeZone.getTimeZone(CurrentTimeZone.get().getID());
    return new ReportParameterConverter(
        format, ULocale.forLocale(CurrentLocale.getLocale()), icuTimeZone);
  }
  ...
  public void convertToXml(PropBagEx docXml, Map<String, String[]> params) {
    ...
    if (firstValue != null) {
      ...
      if (parsed != null) {
        if (useCalendar) {
          xmlValue =
              new LocalDate((Date) parsed, TimeZone.getDefault()).format(Dates.ISO_DATE_ONLY);
        } else {
