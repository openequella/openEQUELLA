...
public abstract class AbstractBirtType {
  ...
  public void update(
      ...
      List<AbstractBirtType> parameterControls,
    ...
    for (int j = 0; j < locIndex; j++) {
      for (AbstractBirtType parameterControl : parameterControls) {
        if (parameterControl.getIndex() == j
            && group.getName().equals(parameterControl.getGroupname())) {
          thingsBefore[j] =
              convertToParam(parameters.get(parameterControl.getName()), parameterControl);
          break;
        }
      }
    }
