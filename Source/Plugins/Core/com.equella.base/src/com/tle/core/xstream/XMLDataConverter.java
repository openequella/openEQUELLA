...
/**
 * Used in conjunction with XStream to create a converter that can (hopefully) marshal/unmarshal
 * beans with a greater ease that setting one up manually.
 *
...
public class XMLDataConverter implements Converter {
  ...
  private void endNode(HierarchicalStreamWriter writer, Stack current, String[] nodes) {
    ...
    boolean isWithin =
        nodes.length > 0 && current.size() > 0 && current.get(0).toString().equals(nodes[0]);
  ...
  public Collection<AbstractMapping> getMapping(
      SortedMap<String, SortedSet<AbstractMapping>> nodeMapping, String path) {
