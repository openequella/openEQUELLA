...
public class ReplicatedConfiguration extends UserManagementSettings {
  ...
  public static final List<String> DIGESTS =
      Collections.unmodifiableList(Arrays.asList(DIGEST_PLAINTEXT, "SHA1", "MD5"));

  private static final String AUTHENTICATE =
      "SELECT ID, Password, Suspended FROM [User] WHERE Username = ?";
  private static final String USER_INFO =
      "SELECT ID, Username, FirstName, LastName, EmailAddress FROM [User] WHERE ID IN ?";
  private static final String GROUP_INFO = "SELECT ID, Name FROM [Group] WHERE ID IN ?";
  private static final String SEARCH_USERS =
      "SELECT ID FROM [User] WHERE (Username LIKE ? OR FirstName LIKE ? OR LastName LIKE ?) ";
  ...
  private static final String USERS_IN_GROUP_RECURSIVE =
  ...
  private static final String SEARCH_GROUPS_IN_GROUP =
  ...
  private static final String USERS_IN_GROUP =
