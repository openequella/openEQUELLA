...
/**
 * Basic "Where" Statement BNF: WHERE STATEMENT ::= "where"? BOOLEAN_EXPR BOOLEAN_EXPR ::=
 * OR_BOOLEAN_EXPR OR_BOOLEAN_EXPR ::= AND_BOOLEAN_EXPR ("or" AND_BOOLEAN_EXPR)* AND_BOOLEAN_EXPR
 * ::= CLAUSE ("and" CLAUSE)* CLAUSE ::= "not" CLAUSE | BRACKETS | COMPARISON | EXISTS_CLAUSE
 * BRACKETS ::= "(" BOOLEAN_EXPR ")" COMPARISON ::= XPATH COMPARISON_OP COMPARISON_RHS EXISTS_CLAUSE
 * ::= XPATH "exists" XPATH ::= "/" (ALPHA | NUMBER | [/._:@])+ COMPARISON_OP ::= "=" | "is" | "<>"
 * | "is not" | "<" | "<=" | ">" | ">=" | "like" | "not like" | "in" | "not in" COMPARISON_RHS ::=
 * "null" | NUMBER_VALUE | STRING_VALUE | GROUP_VALUE STRING_VALUE ::= "'" STRING "'" NUMBER_VALUE
 * ::= NUMBER+ GROUP_VALUE ::= "(" STRING_VALUE ("," STRING_VALUE)* ")" STRING ::= (ALPHA | [0-9] |
 * ...)* ALPHA ::= [a-zA-Z] NUMBER ::= [0-9]
 */
public final class WhereParser extends AbstractTopDownParser {
  ...
  public static FreeTextBooleanQuery parse(String whereClause) {
    ...
    if (whereClause.length() > 5
        && whereClause.substring(0, 5).equalsIgnoreCase("where")) // $NON-NLS-1$
    {
  ...
  /**
   * BOOLEAN_EXPR ::= OR_BOOLEAN_EXPR OR_BOOLEAN_EXPR ::= AND_BOOLEAN_EXPR ("or" AND_BOOLEAN_EXPR)*
   */
  ...
  private FreeTextQuery getComparison(String field) throws ParseException {
    ...
    if (op == Operator.EQUALS || op == Operator.NOT_EQUALS) {
      ...
      throw new InvalidWhereException(
          "Operator currently only works with ISO dates: " + op.toString() + ' ' + value);
    } else {
  ...
  /**
   * COMPARISON_OP ::= "=" | "is" | "<>" | "is not" | "<" | "<=" | ">" | ">=" | "like" | "not like"
   * | "in" | "not in"
   */
  ...
  private FreeTextQuery getFieldQueryForValue(
      String field, Operator op, String value, boolean possibleWildcard) {
  ...
  private FreeTextDateQuery tryToCompareAsDate(String field, Operator op, String value) {
    ...
    if (date == null) {
      ...
      return new FreeTextDateQuery(
          field, date, null, op == Operator.GREATER_THAN_OR_EQUAL_TO, false);
    } else {
