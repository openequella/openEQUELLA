...
public interface RemoteTLEAclManager {
  ...
  /**
   * Performs <code>setTargetList(Node, Object, TargetList)</code>, but then shoots off any
   * necessary item reindexing events. This is pretty much for use by the Admin Console. This
   * version of the method is invoked by Java 1.4 clients with no support for Enums.
   */
  ...
  /** Filters out privileges that the user does not have for the given domain object. */
  ...
  /**
   * Returns a list of entries that apply to a domain object, except for those applied directly to
   * it!
   */
  List<ACLEntryMapping> getAllEntriesForObjectOtherThanTheObject(
