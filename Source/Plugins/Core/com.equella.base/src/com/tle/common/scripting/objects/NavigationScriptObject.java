...
import com.tle.common.scripting.types.NavigationNodeScriptType;
import com.tle.common.scripting.types.NavigationTabScriptType;
import java.util.List;

/**
 * Referenced by the 'nav' variable in script. The item navigation tree is what is shown when
 * viewing the item with the navigation viewer (like IMS packages). Tabs are what ultimately display
 * attachments, attachments cannot be added directly to navigation nodes.
 *
 * @author aholland
 */
public interface NavigationScriptObject extends ScriptObject {
   ...
   * @return The NavigationNodeScriptType with a matching description, or null if not found
   */
   ...
   *
   * @param node The node to remove. If the node is not found in the tree the method will continue
   *     as normal.
   */
  void removeNode(NavigationNodeScriptType node);

  /**
   * Add a tab to a navigation node. The tab will be added to the end of the list of current tabs
   * (if any) If only one tab is added to a node, the tab <b>bar</b> is not displayed.
   *
  ...
  NavigationTabScriptType addTab(
      String description, AttachmentScriptType attachment, NavigationNodeScriptType node);

  /**
   * Create navigation nodes based on the current attachments. Does the same as the 'initialise'
   * button does on the navigation builder control.
   */
  void autocreate();
  ...
  /**
   * Clears the entire navigation tree. A slightly more efficient method of deleting all root nodes
   * than calling removeChildNodes(null)
   */
  void removeAll();
   ...
   *
   * @param parentNode The node to get the children of. If null, all root-level nodes are returned.
