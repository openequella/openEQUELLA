...
public final class PrivilegeTree {
  ...
  // this...
  public enum Node {
    INSTITUTION(true, PRIORITY_INSTITUTION),
    ALL_SCHEMAS(true, PRIORITY_ALL_SCHEMAS),
    SCHEMA(false, PRIORITY_SCHEMA),
    ALL_COLLECTIONS(true, PRIORITY_ALL_COLLECTIONS),
    COLLECTION(false, PRIORITY_COLLECTION),
    GLOBAL_ITEM_STATUS(true, PRIORITY_GLOBAL_ITEM_STATUS),
    ITEM_STATUS(true, PRIORITY_ITEM_STATUS),
    ITEM_METADATA(true, PRIORITY_ITEM_METADATA),
    DYNAMIC_ITEM_METADATA(false, PRIORITY_DYNAMIC_METADATA),
    ALL_WORKFLOWS(true, PRIORITY_ALL_WORKFLOWS),
    WORKFLOW(false, PRIORITY_WORKFLOW),
    WORKFLOW_TASK(false, PRIORITY_WORKFLOW_TASK),
    ALL_POWER_SEARCHES(true, PRIORITY_ALL_POWER_SEARCHES),
    POWER_SEARCH(false, PRIORITY_POWER_SEARCH),
    ALL_FEDERATED_SEARCHES(true, PRIORITY_ALL_FEDERATED_SEARCHES),
    FEDERATED_SEARCH(false, PRIORITY_FEDERATED_SEARCH),
    ALL_FILTER_GROUPS(true, PRIORITY_ALL_FILTER_GROUPS),
    FILTER_GROUP(false, PRIORITY_FILTER_GROUP),
    ITEM(false, PRIORITY_ITEM),
    ALL_REPORTS(true, PRIORITY_ALL_REPORTS),
    REPORT(false, PRIORITY_REPORT),
    ALL_SYSTEM_SETTINGS(true, PRIORITY_ALL_SYSTEM_SETTINGS),
    SYSTEM_SETTING(false, PRIORITY_SYSTEM_SETTING),
    HIERARCHY_TOPIC(false, PRIORITY_HIERARCHY_TOPIC),
    ALL_COURSE_INFO(true, PRIORITY_ALL_COURSE_INFO),
    COURSE_INFO(false, PRIORITY_COURSE_INFO),
    ALL_DYNA_COLLECTIONS(true, PRIORITY_ALL_DYNA_COLLECTIONS),
    DYNA_COLLECTION(false, PRIORITY_DYNA_COLLECTION),
    ALL_TAXONOMIES(true, PRIORITY_ALL_TAXONOMIES),
    TAXONOMY(false, PRIORITY_TAXONOMY),

    // Portlet HAX
    ALL_PORTLETS(true, PRIORITY_ALL_PORTLETS),
    PORTLET(false, PRIORITY_PORTLET),
    PORTLET_HTML(false, PRIORITY_PORTLET),
    PORTLET_RSS(false, PRIORITY_PORTLET),
    PORTLET_IFRAME(false, PRIORITY_PORTLET),
    PORTLET_FREEMARKER(false, PRIORITY_PORTLET),
    PORTLET_TASKS(false, PRIORITY_PORTLET),
    PORTLET_SEARCH(false, PRIORITY_PORTLET),
    PORTLET_MYRESOURCES(false, PRIORITY_PORTLET),
    PORTLET_FAVOURITES(false, PRIORITY_PORTLET),
    PORTLET_RECENT(false, PRIORITY_PORTLET),
    PORTLET_BROWSE(false, PRIORITY_PORTLET),
    PORTLET_TASKSTATISTICS(false, PRIORITY_PORTLET),
    // PORTLET_CONTRIB(false, PRIORITY_PORTLET) ??

    // Connectors HAX
    ALL_CONNECTORS(true, PRIORITY_ALL_CONNECTORS),
    CONNECTOR(false, PRIORITY_CONNECTOR),

    // Kaltura HAX
    ALL_KALTURAS(true, PRIORITY_ALL_KALTURAS),
    KALTURA(false, PRIORITY_KALTURA),

    // Echo 360 HAX
    ALL_ECHOS(true, PRIORITY_ALL_ECHOS),
    ECHO(false, PRIORITY_ECHO),

    // OAuth HAX
    ALL_OAUTH_CLIENTS(true, PRIORITY_ALL_OAUTH_CLIENTS),
    OAUTH_CLIENT(false, PRIORITY_OAUTH_CLIENT),

    // User scripts HAX
    ALL_USER_SCRIPTS(true, PRIORITY_ALL_USER_SCRIPTS),
    USER_SCRIPTS(false, PRIORITY_USER_SCRIPTS),

    // External Tools HAX
    ALL_EXTERNAL_TOOLS(true, PRIORITY_ALL_EXTERNAL_TOOLS),
    EXTERNAL_TOOL(false, PRIORITY_EXTERNAL_TOOL),

    // LTI Consumers HAX ON HAX ON HAX
    ALL_LTI_CONSUMERS(true, PRIORITY_ALL_LTI_CONSUMERS),
    LTI_CONSUMER(false, PRIORITY_LTI_CONSUMER),
    ...
    ALL_CUSTOM_LINKS(true, PRIORITY_ALL_CUSTOM_LINKS),
    CUSTOM_LINK(false, PRIORITY_CUSTOM_LINK),
    ALL_MANAGING(true, PRIORITY_ALL_MANAGING),
  ...
  public static int getPriority(Node node, String privilege) {
    ...
    if (privNode.getPrivileges().contains(privilege)) {
      ...
      throw new RuntimeException(
          "Privilege is not defined for this target: " + privilege + " - " + node);
    }
  }
  ...
  public static Map<String, Integer> getAllPrivilegesForNode(Node node) {
    ...
    getAllPrivilegesForNode(
        results, mapping.get(node), !node.isVirtual(), node.getOverridePriority());
    return results;
  }

  private static void getAllPrivilegesForNode(
      Map<String, Integer> results,
      PrivilegeNode node,
      boolean useObjectInstancePriority,
      int defaultPriority) {
  ...
  private static synchronized void ensureMapping() {
    ...
    // Advanced Searches
    PrivilegeNode powerSearches =
        buildBasic(Node.POWER_SEARCH, Node.ALL_POWER_SEARCHES, "POWER_SEARCH");
    powerSearches.getChildren().get(0).registerPrivilege("SEARCH_POWER_SEARCH");

    // Dynamic Collections
    PrivilegeNode dynaCollections =
        buildEntity(Node.DYNA_COLLECTION, Node.ALL_DYNA_COLLECTIONS, "DYNA_COLLECTION");
    ...
    // Federated Searches
    PrivilegeNode federatedSearches =
        buildBasic(Node.FEDERATED_SEARCH, Node.ALL_FEDERATED_SEARCHES, "FEDERATED_SEARCH");
    ...
    // Harvester Profiles
    PrivilegeNode harvesterProfiles =
        buildBasic(Node.HARVESTER_PROFILE, Node.ALL_HARVESTER_PROFILES, "HARVESTER_PROFILE");
    ...
    // Item Definitions
    PrivilegeNode itemDefinitions =
        buildEntity(Node.COLLECTION, Node.ALL_COLLECTIONS, "COLLECTION");
    ...
    // OAuth
    PrivilegeNode oauthClients =
        buildEntity(Node.OAUTH_CLIENT, Node.ALL_OAUTH_CLIENTS, "OAUTH_CLIENT");

    // Tiny MCE Plugins
    PrivilegeNode htmlEditorPluginss =
        buildBasic(Node.HTMLEDITOR_PLUGIN, Node.ALL_HTMLEDITOR_PLUGINS, "HTMLEDITOR_PLUGIN");

    // User scripts
    PrivilegeNode userScripts =
        buildBasic(Node.USER_SCRIPTS, Node.ALL_USER_SCRIPTS, "USER_SCRIPTS");

    // External Tools
    PrivilegeNode externalTools =
        buildBasic(Node.EXTERNAL_TOOL, Node.ALL_EXTERNAL_TOOLS, "EXTERNAL_TOOL");

    // LTI Consumers
    PrivilegeNode ltiConsumers =
        buildEntity(Node.LTI_CONSUMER, Node.ALL_LTI_CONSUMERS, "LTI_CONSUMER");
