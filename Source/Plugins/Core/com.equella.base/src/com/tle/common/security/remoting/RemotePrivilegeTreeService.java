...
public interface RemotePrivilegeTreeService {
  /**
   * Returns a map of pretty names for security targets IDs. For example, "B:1234" becomes "John's
   * Collection".
   */
   ...
   *
   * @param target the target node to return the children for. <code>null</code> indicates root
   *     targets.
   * @return list of SecurityTarget children. An empty list is returned if there are no children.
   */
  ...
  class SecurityTarget implements Serializable {
    ...
    /**
     * This contains real text rather an I18N key because the key could be dereferenced by the
     * resource centre or the admin console which are different bundle groups. When the security
     * tree editor is moved from the AC and into the RC, then this can change to being a key.
     */
    ...
    public SecurityTarget(
        String displayName, Node targetType, Serializable target, boolean childTargets) {
