...
/**
 * Used as the return results from ConnectionScriptType, which in-turn is retrieved by
 * UtilsScriptObject.getConnection(String)
 *
 * @author aholland
 */
public interface ResponseScriptType {
  /**
   * @return The response from the external site is an error code, or there was a problem connecting
   *     to the server.
   */
  boolean isError();
  ...
  /**
   * Get the data from the response in XML format This method will throw an exception if the
   * returned text is not valid XML.
   *
  ...
  /**
   * Get the HTML data from the response and tidy it to be valid XML. This method will throw an
   * exception if the returned text could not be tidied into valid XML.
   *
   * @return An XmlScriptType object representing the data in the response.
   */
