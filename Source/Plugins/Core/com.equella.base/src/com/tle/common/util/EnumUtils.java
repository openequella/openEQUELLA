...
public class EnumUtils {
  /**
   * Maps enum values to a bit in a bit mask. Can be used to retrieve an Enum value mapped to a
   * single number, or an EnumSet for a bitmask. A good example of both uses can be seen n
  ...
  public interface EnumMask<E extends Enum<E>> {
    ...
    /**
     * @param bit Bits are numbered starting from one, not zero. It makes more sense this way since
     *     a bit mask of "1" is indicating the first bit, so asking for bit 1 should give you the
    ...
    public EnumMaskBuilder<E> skipToBit(int bit) {
      if (bits.size() >= bit) {
        throw new IllegalArgumentException(
            "Bit " + bit + " has already been set as " + bits.get(bit - 1));
      }
          ...
          while (m != 0) {
            if ((m & 1) != 0) {
              ...
              if (e != null) {
                ...
                throw new IllegalArgumentException(
                    "Bit " + i + " for mask " + mask + " is not defined");
              }
            }
            m >>= 1;
            i++;
