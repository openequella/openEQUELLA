...
/**
 * Available as 'currentItem' in scripts, but also available from results of calls to methods on
 * 'items' and within search results return by 'utils.search(...)'.
 */
...
public interface ItemScriptType extends Serializable {
  ...
  /**
   * @return The description of the item in the current user's language (if language pack
   *     available).
   */
  String getDescription();

  /**
   * There is generally an 'xml' object available for the current item's xml, but you can use this
   * for getting xml from OTHER items. Note that this is READ-ONLY, any changes you make will not be
   * persisted.
   *
  ...
  /**
   * Get the status of the item. One of: DRAFT, LIVE, REJECTED, MODERATING, ARCHIVED, SUSPENDED,
   * DELETED, REVIEW, PERSONAL
   *
  ...
  /**
   * @return The unique ID of the owner. This may not be initialised in a wizard context as the item
   *     does not yet have an owner.
   */
  String getOwner();

  /**
   * Can only be used on the currentItem, and depending on the script context it may still not be
   * saved.
   *
  ...
  /**
   * Can only be used on the current item, and depending on the script context it may still not be
   * saved.
   *
   ...
   *
   * @param thumbnail Either "default", "none" or "custom:[uuid]" where [uuid] is the UUID of an
   *     attachment.
   */
