...
public final class OAuthToken implements Serializable, FieldEquality<OAuthToken> {
  ...
  /**
   * The 'code' parameter that initially created this token (if any). You are supposed to revoke all
   * tokens that were created from a code when the code is unsuccessfully tried or repeated.
   */
  @Column(length = 40)
  ...
  /**
   * The set of permissions that the client has requested and the user has allowed. Currently, apps
   * can do anything TODO: this should be on the an OAuthApproval object, so that the user only has
   * to approve the client once and the permissions can be taken from the approval
   */
