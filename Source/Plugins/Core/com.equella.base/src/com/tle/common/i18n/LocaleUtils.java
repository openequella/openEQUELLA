...
public final class LocaleUtils {
  ...
  public static String toHtmlLang(Locale locale) {
    ...
    return lang
        + (!Check.isEmpty(country) ? '-' + country : "")
        + (!Check.isEmpty(variant) ? '-' + variant : "");
  }

  /**
   * From the supplied list of locales, find the closest one to the supplies locale
   *
   * @param locales A collection possible locales to choose from
   * @param locale The locale you would like to match
   * @return The closest matching locale, or just the first available locale if no match
   */
  ...
  /** Transforms a locale to possible key strings, and passes them to a key gatherer. */
  ...
  /**
   * Transforms the given locale elements to possible key strings, and passes them to the gatherer.
   */
  private static <T> T getKeys(
      KeyGatherer<T> gatherer, String language, String country, String variant) {
