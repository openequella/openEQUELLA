...
public interface UtilsScriptObject extends ScriptObject {
   ...
   *
   * @param query A freetext query
   * @param where An xpath/SQL like where clause. E.g.: /xml/mynode like 'something%'
   * @param onlyLive Return only items with a status of Live (e.g. not Draft)
   * @param orderType Can be one of: 0 - Ranking (search relevance), 1 - Date Modified, 2 - Name
   * @param reverseOrder Specify true to reverse the order specified in orderType
   * @param offset First result index
   * @param maxResults Number of results to get
   * @return A SearchResultsScriptType object
   */
  SearchResultsScriptType searchAdvanced(
      String query,
      String where,
      boolean onlyLive,
      int orderType,
   ...
   * @param collectionUuids An array of collection definition UUIDs.
   * @param where An xpath/SQL like where clause. E.g.: /xml/mynode like 'something%'
   * @param facetXpath the XPath of the facet to return the terms and counts for.
   * @return A list of FacetCountResultScriptType objects sorted by highest to lowest frequency.
   */
  List<FacetCountResultScriptType> facetCount(
   ...
   * @deprecated Use {@link
   *     com.tle.common.scripting.objects.CollectionScriptObject#getFromUuid(String)} instead.
   */
  @Deprecated
  ...
  /**
   * Throws an exception. This will prevent any operation from completing normally, e.g. a wizard
   * page submit, or an item save. You should only use this method in critical situations (e.g. an
   ...
   * @return A full URL to this item. E.g. http://myinstitution.edu.au/items/323a213e21fd31e2/1
   */
