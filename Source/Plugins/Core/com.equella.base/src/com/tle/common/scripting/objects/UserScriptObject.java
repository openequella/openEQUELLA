...
/**
 * Referenced by the 'user' variable in script. Represents the currently logged in user as well as
 * provide an interface into the user service to retrieve information about groups and roles. This
 * is the SAME as CommonUserScriptObject
 *
 * @author aholland
 */
public interface UserScriptObject extends ScriptObject {
  ...
  /**
   * Get a list of groups that the user identified by userUniqueID belongs to. Will do a recursive
   * search of groups. This method has no relation to the logged in user.
   *
   ...
   * the query are implied. E.g. 'mit' will match 'smith'. This method has no relation to the logged
   * in user.
   *
  ...
  /**
   * Resolve the list of userUniqueIDs into a list of UserScriptType objects. The returned list of
   * users may be shorter than userUniqueIDs if any user ID is not found, and the order of the
   * result list bears no relation to the order of userUniqueIDs. This method has no relation to the
   * logged in user.
   *
  ...
  /**
   * Resolve the list of groupUniqueIDs into a list of GroupScriptType objects. The returned list of
   * groups may be shorter than groupUniqueIDs if any group ID is not found, and the order of the
   * result list bears no relation to the order of groupUniqueIDs. This method has no relation to
   * the logged in user.
   *
  ...
  /**
   * Same as searchUsers(String) but allows to filter the results by a group ID, and whether to
   * search for subgroups of the specified group. Passing null for the groupUniqueID will be the
   * equivalant of using searchUsers(String) Wildcards at the start and end of the query are
   * implied. E.g. 'mit' will match 'smith'. This method has no relation to the logged in user.
   *
   * @param query The username, first name or last name to search for
   * @param groupUniqueID The group to search within
   * @param recursive Search within subgroups of the group specified by groupUniqueID
   * @return A list of UserScriptType objects
   */
  List<UserScriptType> searchUsersInGroups(String query, String groupUniqueID, boolean recursive);

  /**
   * Find all groups with a name matching the query. Wildcards at the start and end of the query are
   * implied. E.g. 'tude' will match 'Students'. This method has no relation to the logged in user.
   *
  ...
  /**
   * Find all roles with a name matching the query. Wildcards at the start and end of the query are
   * implied. E.g. 'strat' will match 'Administrators'. This method has no relation to the logged in
   * user.
   *
