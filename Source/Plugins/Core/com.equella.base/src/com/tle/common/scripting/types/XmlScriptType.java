...
/**
 * Object for XML manipulation in scripts. Generally referred to as 'xml' in scripts, also available
 * as 'newxml' in New Version scripts and 'attributes' in Advanced Scripting wizard controls. Note
 * that all XPath parameters are not <b>true</b> XPaths and are limited to simple node/attribute
 * selection and node indexes. E.g. /xml/test/node[2]/@attribute is as complex as it will get.
 */
public interface XmlScriptType extends Serializable {
  ...
  /**
   * Add a new node and set its value. This will <b>always</b> add a new node, even if there is
   * already a node at the given XPath.
   *
  ...
  /**
   * Copy the text value of the node found at srcXpath into the text value of the node found at
   * destXpath.
   *
  ...
  /**
   * Returns an XmlScriptType object for the sub-tree that is rooted at the first node matching
   * XPath. If there is no node at XPath, then null is returned. This sub-tree is still linked to
   * the original tree, any changes made to the sub-tree will be reflected in the original.
   *
  ...
  /**
   * Same as getSubtree(String) only that if there is no node matching XPath, a new sub-tree is
   * created.
   *
   * @param xpath The XPath of the sub-tree root node.
   * @return The sub-tree found at XPath, or a brand new sub-tree rooted at XPath.
   */
  XmlScriptType getOrCreateSubtree(String xpath);

  /**
   * Creates a subtree rooted at XPath. This will always create a subtree, regardless of whether
   * there is a node at XPath already.
   *
   * @param xpath The XPath of the sub-tree root node.
   ...
   *
   * @param subtree The tree to remove from the xml (obtained via getSubtree, getOrCreateSubtree,
   *     createSubtree or getAllSubtrees)
   */
  ...
  /**
   * Appends the first-level of children from an XML document at the given XPath. This works the
   * same as <code>append(String, XmlScriptType)</code>, but does not include the root element of
   * the document.
   *
   * @param xpath The XPath to insert the new document into.
   * @param documentToAppend The XML document whose first-level children will be inserted from.
   */
  void appendChildren(String xpath, XmlScriptType documentToAppend);

  /**
   * Returns an XmlScriptType object for the sub-tree that is rooted at the node with an "id"
   * attribute value matching the parameter. If there is no node at XPath, then null is returned.
   * This sub-tree is still linked to the original tree, any changes made to the sub-tree will be
  ...
  /**
   * Returns an XmlScriptType object for the sub-tree that is rooted at 'this' object.
   *
  ...
  /**
   * Returns a List of XmlScriptType objects for the sub-trees rooted at each of the children of
   * 'this' object.
   *
