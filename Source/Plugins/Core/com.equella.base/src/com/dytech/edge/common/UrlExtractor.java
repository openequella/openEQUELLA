...
/**
 * Finds all URLs withing a piece of text (only HTTP or HTTPS URLs though) This could probably be
 * refactored into ConvertHtmlService
 *
 * @author aholland
 */
public class UrlExtractor {
  ...
  /**
   * This really would depend on the context of the URL, we have to be mind readers to a certain
   * extent.
   */
  ...
  /**
   * E.g. someone has written (http://www.google.com.au) in a text field. (Blame Yih) Since these
   * are valid URL characters, the second of each pair should be treated as a URL terminating
   * character IF AND ONLY IF the previous character before the URL start is the first of the pair.
   */
  private static final List<Pair<String, String>> SURROUNDERS =
    ...
    while ((offset = findUrlStart(val, offset)) >= 0) {
      ...
      boolean namespace =
          (testNamespaceAttribute
              && nsStart >= 0
              && val.substring(nsStart, offset).equals(NAMESPACE_START));
      if (!namespace) {
