...
public abstract class AbstractTLFPlugin<T extends AbstractTLFHarvesterSettings>
	extends
		HarvesterPlugin<T>
{
	protected static final String PLUGIN_PREFIX = "com.tle.admin.harvester.tool.";
	...
	public void initGUI()
	{
		...
		panel.addComponent(new JLabel(CurrentLocale.get(PLUGIN_PREFIX + getPluginsFieldString())));
		panel.addNameAndComponent(CurrentLocale.get(PLUGIN_PREFIX + "detailstab.user"), userField);
		panel.addNameAndComponent(CurrentLocale.get(PLUGIN_PREFIX + "detailstab.pass"), passField);

		panel.addNameAndComponent(CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.live"), liveOnly);
		panel.addNameAndComponent(CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.harvestlo"),
			harvestLearningObjects);
		panel.addNameAndComponent(CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.harvestre"),
			harvestResources);
	...
	public void validation() throws EditorException
	{
		...
		{
			throw new EditorException(CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.userfield"));
		}

		if( !harvestLearningObjects.isSelected() && !harvestResources.isSelected() )
		{
			throw new EditorException(CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.harvest"));
		}
	}
 ...
 CurrentLocale.get(PLUGIN_PREFIX + "loraxplugin.schema"));
		}
