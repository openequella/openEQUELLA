...
public class HarvesterActionsTab extends BaseEntityTab<HarvesterProfile> implements ActionListener
{
	...
	public HarvesterActionsTab(HarvesterProfileEditor editor)
	{
		...
		runBtn = new JButton(getString("run"));
		testBtn = new JButton(getString("test"));
		...
		namePanel.addNameAndComponent(getString("lastdate"), dateSelect);
		namePanel.addNameAndComponent(getString("testlabel"), testPanel);
		namePanel.addNameAndComponent(getString("runnow"), runPanel);
	...
	{
		return CurrentLocale.get("com.tle.admin.harvester.tool.actionstab." + partKey, params);
	}
}
