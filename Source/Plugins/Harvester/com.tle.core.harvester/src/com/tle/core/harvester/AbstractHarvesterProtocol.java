...
public abstract class AbstractHarvesterProtocol implements ContentRepository
{
	...
	public int setupAndRun(HarvesterProfile profile, boolean testOnly) throws Exception
	{
		...
		if( itemdef == null )
		{
			throw new RuntimeException(CurrentLocale.get("com.tle.core.harvester.error.collection"));
		}
	...
	public void processLearningObject(LearningObject lobject)
	{
		...
		try
		{
			...
			LOGGER.info(CurrentLocale.get("com.tle.core.harvester.log.complete"));

		}
		catch( Exception ex )
		{
			LOGGER.error(
				CurrentLocale.get("com.tle.core.harvester.error.upload", identifier, profile.getAttribute("server")),
				ex);
		}
	}
