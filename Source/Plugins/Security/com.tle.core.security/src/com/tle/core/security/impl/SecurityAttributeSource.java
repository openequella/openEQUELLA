...
public class SecurityAttributeSource
{
	...
	private static final Set<Class<?>> STATIC_CLASSES_TO_CHECK = ImmutableSet.of(BaseEntity.class, EntityPack.class,
		Item.class, ItemPack.class, ActivateRequest.class);

	final Map<CacheKey, SecurityAttribute> attributeCache = CollectionFactory.createConcurrentMapIfPossible(16);
	...
	private boolean addAnnotationAttrs(Annotation[] annotations, SecurityAttribute attr, Class<?> targetClass)
	{
		boolean have = false;
		for( Annotation annotation : annotations )
		{
			...
			else if( anType == RequiresPrivilege.class )
			{
				...
				attr.addOnCallPrivilege(resolvePrivilege(((RequiresPrivilege) annotation).priv(), targetClass));
				have = true;
			}
			else if( anType == RequiresPrivilegeWithNoTarget.class )
			{
				attr.addOnCallPrivilege(resolvePrivilege(((RequiresPrivilegeWithNoTarget) annotation).priv(),
					targetClass));
			...
			{
				attr.addFilterPrivilege(resolvePrivilege(((SecureOnReturn) annotation).priv(), targetClass));
	...
	private void setOnCallmode(SecurityAttribute attr, OnCallMode mode)
	{
		...
		{
			throw new Error("Can't mix RequiresPrivilege, RequiresPrivilegeWithNoTarget and SecureOnCall"); //$NON-NLS-1$
		}
	}
	...
	private String resolvePrivilege(String priv, Class<?> targetClass)
	{
		if( priv.endsWith(SecurityConstants.VIRTUAL_BASE_ENTITY) )
		{
			...
			priv = priv.substring(0, priv.length() - SecurityConstants.VIRTUAL_BASE_ENTITY.length())
				+ annotation.value();
		}
		return priv;
	}
