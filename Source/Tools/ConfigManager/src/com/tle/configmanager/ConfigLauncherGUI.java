...
public class ConfigLauncherGUI extends JFrame implements ActionListener
{
	...
	public static final String IMAGEMAGICK_CONFIG = "plugins/com.tle.core.imagemagick/config.properties.unresolved";
	...
	public void updateConfigs()
	{
		...
		if( configFiles != null )
		{
			...
			Collections.sort(configs, new Comparator<ConfigProfile>()
			{
				@Override
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if( e.getSource() == btnApply )
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Error loading configuration: \n" + ex.getMessage(), "Load Failed",
	...
	private void addProfile()
	{
		...
		if( configs != null && cmbConfigs.getSelectedItem() != null )
		{
			...
			int result = JOptionPane.showConfirmDialog(null,
				"Do you want to clone the currently selected configuration?: " + selectedProf.getProfName(),
				"Clone Confirmation", JOptionPane.YES_NO_OPTION);
	...
	private void deleteProfile()
	{
		...
		int result = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete this configuration?: "
			+ selectedProf.getProfName(), "Delete Confirmation", JOptionPane.YES_NO_OPTION);
		...
		if( result == JOptionPane.YES_OPTION )
		{
			...
			if( !success )
			{
				JOptionPane.showMessageDialog(null, "Unable to delete configuration: " + selectedProf.getProfName(),
	...
	private void loadProfile() throws FileNotFoundException, IOException, ConfigurationException
	{
		...
		if( oracleSelected )
		{
			org.apache.commons.io.FileUtils.copyFile(new File(srcDir + "/hibernate.properties.oracle"), new File(
				destDir + "/hibernate.properties"));
		}
		else if( selectedProf.getDbtype().equalsIgnoreCase(POSTGRESQL) )
		{
			FileUtils.copyFile(new File(srcDir + "/hibernate.properties.postgresql"), new File(destDir
				+ "/hibernate.properties"));
		}
		else if( selectedProf.getDbtype().equalsIgnoreCase(MSSQL) )
		{
			FileUtils.copyFile(new File(srcDir + "/hibernate.properties.sqlserver"), new File(destDir
				+ "/hibernate.properties"));
		}

		// Mandatory / Optional / Logging
		FileUtils.copyFile(new File(srcDir + "/mandatory-config.properties"), new File(destDir
			+ "/mandatory-config.properties"));
		FileUtils.copyFile(new File(srcDir + "/optional-config.properties"), new File(destDir
			+ "/optional-config.properties"));

		// Copy custom development logging file
		FileUtils.copyFile(new File("./learningedge-log4j.properties"), new File(destDir
			+ "/learningedge-log4j.properties"));

		// Other Miscellaneous Files
		FileUtils.copyFile(new File(srcDir + "/en-stopWords.txt"), new File(destDir + "/en-stopWords.txt"));
		FileUtils.copyFile(new File(srcDir + "/" + HIKARI_CONFIG), new File(destDir + "/" + HIKARI_CONFIG));
		...
		hibProp = hibProp.replace("${datasource/schema}",
		...
		mandProps.setProperty("http.port", !Check.isEmpty(http) ? http : !Check.isEmpty(portFromUrl) ? portFromUrl
			: "80");
		...
		{
			optProp = optProp.replace("#conversionService.disableConversion = false",
				"conversionService.disableConversion = true");
			optProp = optProp.replace(
			...
			optProp = optProp.replace("#pluginPathResolver.wrappedClass", "pluginPathResolver.wrappedClass");
		}
		else
		{
			optProp = optProp.replace("${install.path#t\\/}/conversion/conversion-service.jar",
		...
		JOptionPane.showMessageDialog(null, "The configuration: " + selectedProf.getProfName()
			+ " has been successfully loaded.", "Load Success", JOptionPane.INFORMATION_MESSAGE);
	}
