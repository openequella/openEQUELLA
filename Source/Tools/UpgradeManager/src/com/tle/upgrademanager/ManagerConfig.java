...
public class ManagerConfig extends EquellaConfig {
  ...
  public ManagerConfig(File installPath, InputStream inputStream) throws IOException {
    ...
    managerDetails =
        new ManagerDetails(new File(managerDir, "config.properties"), inputStream); // $NON-NLS-1$
  }
  ...
  public class ManagerDetails {
      ...
      if (inputStream != null) {
        ...
        fullVersion =
            MessageFormat.format(
                "{0} ({1})", vp.getProperty(VERSION_DISPLAY), vp.getProperty(VERSION_COMMIT));
      } else {
        fullVersion = "Unknown";
      }
    }
    ...
    public final void load() throws IOException {
      try (Reader reader = new FileReader(this.managerConfigFile)) {
        ...
        loadingTimeout =
            Long.parseLong(props.getProperty(LOADING_TIME_OUT_KEY, LOADING_TIME_OUT_DEFAULT));
      }
    }
