...
public abstract class AbstractDispatchHandler implements HttpHandler {
  ...
  private void handleNoAction(HttpExchange exchange) throws IOException {
    ...
    if (newAction == null) {
      HttpExchangeUtils.respondApplicationError(
          exchange, new Exception("No default action specified for " + getClass().getName()));
    } else {
  ...
  protected final Map<String, Pair<String, File>> getMultipartStreams(HttpExchange exchange) {
    return (Map<String, Pair<String, File>>)
        exchange.getAttribute(SuperDuperFilter.MULTIPART_STREAMS_KEY);
  }
