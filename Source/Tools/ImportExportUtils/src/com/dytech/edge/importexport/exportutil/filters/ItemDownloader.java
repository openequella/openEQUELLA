...
public class ItemDownloader extends Warehouse<Item> implements Runnable
{
	...
	private void processItem(Item item) throws Exception, IOException
	{
		...
		else
		{
			...
			if( packageFile != null )
			{
				...
				FileWriter writer = new FileWriter(new File(data.getSaveFolder(), "package-mapping.txt"), true);
	...
	{
		return new URL(data.getInstitutionUrl(), servlet + "/" + item.getUuid() + "/" //$NON-NLS-2$
			+ item.getVersion());
	}
	...
	private int showErrorDialog(Item item)
	{
		...
		final int result = JOptionPane
			.showOptionDialog(parent, "An error has occured downloading the item " + item.getName() + ".\n"
				+ "Do you want to continue?", "Error Downloading", JOptionPane.YES_NO_CANCEL_OPTION,
