...
public class UploadPage extends WizardPage
{
	...
	public void onShow()
	{
		try
		{
			...
			data.setCompletion(new Completion(successful, res.getSecond(), successful
				+ " item(s) uploaded successfully.<br/>" + failed + " item(s) failed to upload"));
			/*
	...
	private Pair<Integer, Integer> uploadItems() throws Exception
	{
		...
		for( int i = 0; i < items.size(); i++ )
		{
			...
			try
			{
				...
				{
					throw new Exception("Item " + item.getUuid() + "/" + item.getVersion() + " already exists");
				}
			}
			catch( Exception ex )
			{
				failed++;
				final int result = JOptionPane.showConfirmDialog(SwingUtilities.getWindowAncestor(this),
					"An error has occured uploading the item " + item.getName() + ":\n\n  " + ex.getMessage()
	...
	private void uploadFile(String staging, String relpath, File basepath) throws Exception, IOException
	{
