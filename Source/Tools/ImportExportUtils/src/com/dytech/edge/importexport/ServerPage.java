...
public class ServerPage extends WizardPage
{
	...
	private boolean setupSoapSession(String url)
	{
		...
		try
		{
			data.setSoapSession(new SoapSession(institutionUrl, username.getText(), new String(password.getPassword())));
		...
		{
			ex.printStackTrace();
			if( ex.getCause() != null && ex.getCause().getMessage().equals("Username or password incorrect") )
			{
