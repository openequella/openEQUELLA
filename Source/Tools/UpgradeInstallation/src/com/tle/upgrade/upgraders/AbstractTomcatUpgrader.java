...
public abstract class AbstractTomcatUpgrader extends AbstractUpgrader
{
	...
	public void upgrade(UpgradeResult result, File tleInstallDir) throws Exception
	{
		...
		{
			result.info("Error downloading " + tomcatZipFilename + ", trying to find it locally at "
		...
		{
			throw new Exception("Failed to backup existing Tomcat folder to " + tomcatBackup.getAbsolutePath()
				+ ". Possibly open in file system?", e);
		}
		...
		tomcatFolder.mkdir();
		ZipUtils.extract(new ZipInputStream(new BufferedInputStream(new FileInputStream(tomcatZip))), tomcatFolder);
	...
	protected void afterTomcatExtraction(UpgradeResult result, File tleInstallDir, File tomcatFolder,
		File tomcatBackupFolder) throws Exception
	{
		...
		{
			throw new Exception("Failed to copy existing webapps folder. Possibly open in file system?", e);
		}
	}
	...
	protected URL getDownloadUrl(File tleInstallDir) throws Exception
	{
		...
		try( FileReader reader = new FileReader(new File(config.getManagerDir(), "config.properties")) )
		{
